translate id w1ExHanaHangout_71a11dff:
    hana "Oh, [2]!"

translate id w1ExHanaHangout_97cb4f68:
    "My entrance interrupted the pair's conversation, with Hana giving me a wave over."

translate id w1ExHanaHangout_0318ed95:
    hana "Come join us."

translate id w1ExHanaHangout_701178c9:
    mc "What are you two up to?"

translate id w1ExHanaHangout_9bd99a85:
    hana "I was just listening to Lucy's problems."

translate id w1ExHanaHangout_bee43074:
    mc "You sound like a real bartender, you know that."

translate id w1ExHanaHangout_9308c79a:
    hana "What brings you here? The house girls usually bring the drinks."

translate id w1ExHanaHangout_05a185e0:
    mc "Nah, I left the show early. I'd seen enough."

translate id w1ExHanaHangout_45a129a6:
    hana "Good choice. You want something to wet your whistle?"

translate id w1ExHanaHangout_25423f98:
    mc "No thanks. How are you Lucy?"

translate id w1ExHanaHangout_8801b403:
    "I asked, although..."

translate id w1ExHanaHangout_7a79fcad:
    "I could guess the true answer wasn't positive."

translate id w1ExHanaHangout_d5f0c442:
    lucy "I'm... fine."

translate id w1ExHanaHangout_4c329085:
    mc "I guess I would have trouble imagining, huh?"

translate id w1ExHanaHangout_5a2aec5d:
    hana "She was just telling me the reason she's here. You're familiar, right?"

translate id w1ExHanaHangout_b07a65ed:
    lucy "I want to get my son into St. Ives."

translate id w1ExHanaHangout_3802c392:
    hana "Jeez, is that really worth whoring yourself out? What's so damn great about that school?"

translate id w1ExHanaHangout_fc3af1cd:
    mc "It means you get into your choice college for one. It sets you up for life."

translate id w1ExHanaHangout_f43ef3ac:
    lucy "Ah... what he said, basically."

translate id w1ExHanaHangout_dc49e503:
    hana "So? Having everything handed to you just turns a brat into a shitty adult."

translate id w1ExHanaHangout_93aab6d6:
    mc "I didn't peg you as a pull yourself up by your bootstraps kind of girl."

translate id w1ExHanaHangout_fa922ef7:
    hana "I know the value of hard work, but I guess I also know that only gets you so far..."

translate id w1ExHanaHangout_6803e6bc:
    hana "You must really care about your kid's future to be subjecting yourself to this."

translate id w1ExHanaHangout_47553bf0:
    lucy "That's the thing... I don't know anymore. Mrs. Pulman called it vanity and I don't know if I can one hundred percent deny that."

translate id w1ExHanaHangout_5db01cf9:
    hana "Don't let her fuck with your head. She likes to make whales believe they're minnows. She's a real nasty bitch."

translate id w1ExHanaHangout_be595ac0:
    lucy "..."

translate id w1ExHanaHangout_3d6dcc09:
    mc "Yeah, listen to Hana. My own mother endured something similar and it's not like she did it because I'd go hungry otherwise."

translate id w1ExHanaHangout_93a68364:
    hana "What do you mean?"

translate id w1ExHanaHangout_fa0b836b:
    mc "Well, I told you my mom did porn after my father's death."

translate id w1ExHanaHangout_09edf6de:
    hana "Yeah, we got that in common. Sort of."

translate id w1ExHanaHangout_5c2a02ee:
    mc "My mother worked as a porn star for awhile, following my father's death."

translate id w1ExHanaHangout_15268e0c:
    hana "Shit? For real?"

translate id w1ExHanaHangout_12825669:
    mc "As best as I can guess, she did it so things wouldn't change too much for me. So my life wouldn't be upended more than it already was."

translate id w1ExHanaHangout_c19837cb:
    mc "You know, so we wouldn't have to move. So I didn't have to change schools..."

translate id w1ExHanaHangout_b5708ae5:
    mc "I guess she thought it was important for a kid to have a sense of normalcy in their life after something like that. I dunno. Even now, I don't quite get it."

translate id w1ExHanaHangout_d9ee0e7b:
    mc "Point being, I don't think you're necessarily being frivolous."

translate id w1ExHanaHangout_b143e824:
    lucy "...thanks, [2]. I know you're just saying that, but I appreciate the sentiment."

translate id w1ExHanaHangout_2326045e:
    lucy "I need to go get washed up. Apparently I need to be available for after the exhibition."

translate id w1ExHanaHangout_ee5980f0:
    hana "Sure, see you around."

translate id w1ExHanaHangout_4d9a5d86:
    mc "Bye."

translate id w1ExHanaHangout_e3f47c7a:
    "Lucy left, leaving just Hana and myself."

translate id w1ExHanaHangout_98892a8f:
    hana "*sigh* For the record, I think using sex to unfairly get your kid into a school is pretty scummy, but I didn't want to kick her while she's down."

translate id w1ExHanaHangout_75c7f621:
    mc "The world is scummy. It's not my place to judge."

translate id w1ExHanaHangout_a5a87695:
    hana "Can you guess how I found her earlier?"

translate id w1ExHanaHangout_2ee28a14:
    mc "Tied up, stuffed with toys, left all alone?"

translate id w1ExHanaHangout_68427080:
    hana "Yeeeeeah..."

translate id w1ExHanaHangout_09e58afb:
    "There was a brief, but hefty moment of silence to our talk."

translate id w1ExHanaHangout_0c32afd2:
    hana "Welp, so you came to see little ol' me?"

translate id w1ExHanaHangout_5273bafd:
    mc "Just repaying your visit from earlier."

translate id w1ExHanaHangout_b19a2af2:
    hana "Heh, nice. Since you're here, you want to hit up the steam room? Always good to have a buddy in case you pass out."

translate id w1ExHanaHangout_3ee6c2bc:
    mc "Shouldn't you be at the bar?"

translate id w1ExHanaHangout_9df4bdda:
    hana "Shouldn't you be at the exhibition?"

translate id w1ExHanaHangout_b5fbd5a1:
    mc "Good point."

translate id w1ExHanaHangout_0fd4d656_1:
    "......"

translate id w1ExHanaHangout_a20cefa7_1:
    "..."

translate id w1ExHanaHangout_29b991e5:
    mct "(Aaaaaah...)"

translate id w1ExHanaHangout_9d6be023:
    mct "(After the first leg of the night, {1}damn, I'm tired{2}.)"

translate id w1ExHanaHangout_462fce45:
    "I was glad I went along with Hana's suggestion. It kinda felt like I was playing hooky."

translate id w1ExHanaHangout_e4958334:
    mct "(Plus... half-dressed in a sauna with a beauty like Hana... yeah, this was certainly a better way of ending the night than sticking around for a circle jerk.)"

translate id w1ExHanaHangout_82e44638:
    hana "This place does have its perks."

translate id w1ExHanaHangout_89813b97:
    hana "Why the hell does feeling like you're being smothered with heat feel sooooooo good?"

translate id w1ExHanaHangout_f223081d:
    mc "Do you actually want an answer to that?"

translate id w1ExHanaHangout_10dbbaf4:
    hana "You have one for me?"

translate id w1ExHanaHangout_f9741104:
    mc "The heat causes your blood vessels to expand, increasing blood flow, improving circulation, loosening tight muscles and clearing out built-up lactic acid."

translate id w1ExHanaHangout_56abfd47:
    hana "Ha, you fucking nerd."

translate id w1ExHanaHangout_5244e19e:
    "For a minute going on forever, Hana and I shared a comfortable silence. One of those rare moments where nothing in your head wants to get out."

translate id w1ExHanaHangout_de87ee5d:
    hana "Hmm...~ Hmmmhmmmh...~"

translate id w1ExHanaHangout_0fd4d656_2:
    "......"

translate id w1ExHanaHangout_a20cefa7_2:
    "..."

translate id w1ExHanaHangout_7fa5d3cb:
    hana "You ever been in love [2]?"

translate id w1ExHanaHangout_d72f91cd:
    mc "The hell? Where's that coming from?"

translate id w1ExHanaHangout_10f444fa:
    hana "Things were getting a little too comfortable, so I thought I'd ask you an embarrassing personal question."

translate id w1ExHanaHangout_e5c0d7a1:
    hana "You don't have to answer, but I'm a tit-for-tat girl."

translate id w1ExHanaHangout_1fa671c3:
    mc "You mean, I get to ask you something if I answer your question?"

translate id w1ExHanaHangout_ba3c7fb0:
    hana "Correctomundo."

translate id w1ExHanaHangout_08723ce3:
    mc "Okay, fine. Have I ever been in love, eh?"

translate id w1ExHanaHangout_1b1d7b5f:
    mc "It depends on how lax your definition of love is, I guess."

translate id w1ExHanaHangout_1316b88c:
    hana "We don't have to get philosophical about it. Why don't you tell me about your first crush?"

translate id w1ExHanaHangout_dd31afd5:
    hana "You're not from Mars. You had one of those right?"

translate id w1ExHanaHangout_a85cdc46:
    mc "Of course I did."

translate id w1ExHanaHangout_70eb7f77:
    hana "So?"

translate id w1ExHanaHangout_12caf58b:
    mc "Well, you're going to think it's dorky."

translate id w1ExHanaHangout_996c6ebe:
    hana "I like dorky. Tell me."

translate id w1ExHanaHangout_fb573f3a:
    mc "Okay... well, my first crush was an older woman."

translate id w1ExHanaHangout_fcd6955f:
    hana "What, like a schoolboy crush on your homeroom teacher?"

translate id w1ExHanaHangout_cee0429f:
    mc "It was the mother of a friend. For whatever reason, I've always had a thing for older women. It might've started with her."

translate id w1ExHanaHangout_a85e21c5:
    hana "It wasn't Ian's mom, was it?"

translate id w1ExHanaHangout_8114a09f:
    mc "Uh..."

translate id w1ExHanaHangout_fd851ad3:
    hana "Ha, really? You had a crush on that asshole's mom? Did he know about it?"

translate id w1ExHanaHangout_782cd9c4:
    mc "I don't think so, but maybe? This was a long, loooong time ago."

translate id w1ExHanaHangout_f5c10434:
    hana "What did you like about her?"

translate id w1ExHanaHangout_3f25f42e:
    mc "As a kid? I don't know. Just one of those things. I thought she was pretty I guess."

translate id w1ExHanaHangout_da327870:
    hana "That's kinda basic, but it's sweet."

translate id w1ExHanaHangout_c24ba296:
    hana "What about something more substantial?"

translate id w1ExHanaHangout_44520ca5:
    mc "That's a separate question. Don't I get to ask you something?"

translate id w1ExHanaHangout_aa4808d7:
    hana "C'mon, just answer it! I'll let you ask me something real good after."

translate id w1ExHanaHangout_4f8d25bf:
    mc "Okay. Something more substantial, you said? Hmm..."

translate id w1ExHanaHangout_c3e76745:
    mc "There was my \"great\" high school love."

translate id w1ExHanaHangout_3e5c6ff5:
    hana "That sounds more up my alley. Tell me more."

translate id w1ExHanaHangout_98d9a8f4:
    mc "It's more pathetic than anything. We'd been friends since grade school."

translate id w1ExHanaHangout_b04c4bb3:
    hana "What was she like?"

translate id w1ExHanaHangout_8664af64:
    mc "She was a tomboy. The kind of girl that wasn't afraid of getting dirty during recess."

translate id w1ExHanaHangout_91b615a3:
    mc "She was basically \"one of the boys\", until one day, she magically wasn't. She grew up and suddenly I was noticing new things about her."

translate id w1ExHanaHangout_ac08b134:
    hana "Puberty's a bitch."

translate id w1ExHanaHangout_a485bf35:
    mc "Anyway, that's when I started catching feelings for her. For the longest time, it was one sided."

translate id w1ExHanaHangout_288c5eb5:
    mc "It was unrequited all the way up to my junior year."

translate id w1ExHanaHangout_7ae902f7:
    hana "Seriously? You never asked her out or anything before that? After seeing you with your cock out in front of a room full of old perverts, I wouldn't think you'd be so timid."

translate id w1ExHanaHangout_28deb977:
    mc "It's not like I was a shy kid, but I did have a tendency to get stuck in my head and second-guess things. So I just kept my feelings under wraps and mostly out of mind."

translate id w1ExHanaHangout_8ef587ee:
    hana "What came after that longest time?"

translate id w1ExHanaHangout_e699dfdd:
    mc "{1}She asked me out{2}. We went to Junior-Senior prom. Dated a little while after that too, but things fizzled out pretty quick."

translate id w1ExHanaHangout_a12c8382:
    mc "All that bluster and yearning and I realized I didn't even know her that well in the first place. Turns out I didn't like her that much."

translate id w1ExHanaHangout_8448605a:
    mc "On the plus side, I learned about managing your expectations versus reality. After that came Marlow, but..."

translate id w1ExHanaHangout_ee49a79f:
    mc "That's a story for another time. I've spoken enough about my love life. It's your turn now."

translate id w1ExHanaHangout_9d4187eb:
    hana "Hmmm? Huh? My turn for what?"

translate id w1ExHanaHangout_1c1ceca2:
    mc "Don't give me that look. Tit-for-tat, right?"

translate id w1ExHanaHangout_7e3caced:
    hana "Instead of asking me a question, wouldn't you rather see my tits?"

translate id w1ExHanaHangout_c156a907:
    mc "Very funny."

translate id w1ExHanaHangout_9b7497d0:
    hana "I'm serious. You want a look? I don't like talking about myself, but I did promise I'd give something in return."

translate id w1ExHanaHangout_7bb1f072:
    mc "No, I want to ask you a question."

translate id w1ExHanaHangout_46f1ce56:
    hana "Huh? What? {1}Really{2}?"

translate id w1ExHanaHangout_c5fa541f:
    hana "I don't know if I should feel insulted or not."

translate id w1ExHanaHangout_ac71dcfc:
    mc "No offense, but there's a dozen topless women walking around downstairs."

translate id w1ExHanaHangout_d9981b9a:
    hana "Yeah, but you haven't seen mine."

translate id w1ExHanaHangout_60c3f003:
    mc "Nice try, but you're not wiggling out of this."

translate id w1ExHanaHangout_817ccb89:
    hana "Fine. Your choice. Ask me anything, [2]."

translate id w1ExHanaHangout_bf850b34:
    mc "Okay. I got to make this a good one, give me a second to think..."

translate id w1ExHanaHangout_8fea2db2:
    mc "Hmm..."

translate id w1ExHanaHangout_2e68a148:
    hana "Eeeeh? Stop looking at me like you're about to dissect me."

translate id w1ExHanaHangout_a8ac6177:
    mc "Okay, got it!"

translate id w1ExHanaHangout_59b30c9f:
    hana "Oh, boy... let me have it."

translate id w1ExHanaHangout_3c836e92:
    mc "What's your fondest memory?"

translate id w1ExHanaHangout_1d7857a0:
    hana "Is that a joke? That's what you want to ask me? You don't want to ask something embarrassing?"

translate id w1ExHanaHangout_27769dd6:
    mc "No. I want to hear you talk about something that makes you happy."

translate id w1ExHanaHangout_7ec45539:
    hana "Mmmmh...?"

translate id w1ExHanaHangout_19e27166:
    hana "You really are a dork."

translate id w1ExHanaHangout_4930e6d4:
    hana "Okay, fair's fair: my fondest memory..."

translate id w1ExHanaHangout_fb2bee58:
    "Hana paused a moment to collect her thoughts. She seemed to give it some genuine thought, before arriving at a satisfactory answer."

translate id w1ExHanaHangout_5b9eb8f4:
    hana "Got it."

translate id w1ExHanaHangout_c9e099be:
    hana "This is well before my mother got sick. Back then, it was just me, her, and my grandfather."

translate id w1ExHanaHangout_93539daa:
    hana "This is from when I was a kid. Back then it was just me, my mother, and my grandfather."

translate id w1ExHanaHangout_4a36c280:
    hana "All three of us lived together. My mom always told people she was looking after grandpop, but it was more like he was looking after her. She never quite had her shit together, y'know?"

translate id w1ExHanaHangout_1a0463fd:
    hana "I loved my grandpop. He always had a PBJ fixed when I got home from school. Glass of milk, crusts cut off... he called it \"the works\"."

translate id w1ExHanaHangout_37fd9f1b:
    mc "That sounds really nice. Is that your answer?"

translate id w1ExHanaHangout_a01b7d96:
    hana "Oh, no. Just giving you a little context first."

translate id w1ExHanaHangout_139cbc75:
    hana "Anyway, my grandpop loved music. He always talked about how music had the power to transport you back in time. That a song could be tied to specific moments in your life and just by hearing it, you could transport yourself back in time."

translate id w1ExHanaHangout_cb92a7d3:
    hana "Recall all kinds of memories... major life events, love and heart break, and even take you back to the first time you saw your favorite movie."

translate id w1ExHanaHangout_a9482623:
    mc "I like the way your grandfather viewed music. Is that where your love of music comes from?"

translate id w1ExHanaHangout_2dd7740f:
    hana "It's where it started, yeah. My grandpop introduced me to all kinds of awesome bands as a kid. All kinds of styles and genres, but the one that made the biggest mark on me was punk."

translate id w1ExHanaHangout_7270b444:
    hana "When I was ten, he took me to see my first live concert. The venue only held a few hundred people, but to a kid, that was the most people I'd ever seen in one place at the time."

translate id w1ExHanaHangout_4f1bc422:
    hana "It felt like all the people in the world were there to listen to the band play. That was crazy to me."

translate id w1ExHanaHangout_86fa46c4:
    mc "It is kinda crazy when you think about it. The way so many people's lives can be touched by the same thing."

translate id w1ExHanaHangout_126e205b:
    hana "Exactly. It was because of that I fell in love with rock music. I wasn't an outgoing child. I always went straight home after school, but that concert inspired me in more ways than one."

translate id w1ExHanaHangout_a2796549:
    hana "The place smelled horrible and the music was blown out, but the band had so much energy. They played loud and fast and everyone was losing their minds. Cheering uncontrollably."

translate id w1ExHanaHangout_972cdcda:
    hana "Not only that, the drummer was a chick too. That was so cool to me! All the drummers I'd seen up to that point, in the music my grandpop introduced to me, were ugly dudes."

translate id w1ExHanaHangout_873bc451:
    hana "That was the perfect night. That's when music became more than just an excuse to hang out with my grandpop, it became something that connected us."

translate id w1ExHanaHangout_5368fec7:
    hana "Any time I hear the first song they played that night, I feel like I'm reliving that night. I feel like a kid again."

translate id w1ExHanaHangout_8c914938:
    hana "I remember how tightly he gripped my hand, so we wouldn't get separated. I remember how large they seemed compared to mine. I remember sitting on his broad shoulders to get a better view of the band..."

translate id w1ExHanaHangout_5688da31:
    mc "The power to transport you back in time. Just like he talked about."

translate id w1ExHanaHangout_81b17eda:
    hana "Yeah, just like he said."

translate id w1ExHanaHangout_b160bc7c:
    hana "He's, uh... dead now."

translate id w1ExHanaHangout_bbe50918:
    mc "I'm sorry..."

translate id w1ExHanaHangout_3c347c88:
    hana "No. It feels really nice to reminisce."

translate id w1ExHanaHangout_1eff1db6:
    mc "What band did you see play that night?"

translate id w1ExHanaHangout_d3aee936:
    hana "You'd probably know them, actually. They blew up in the years after I saw them."

translate id w1ExHanaHangout_9194ee32:
    hana "Still, I'd rather not say. I like the fact that whenever they come on the radio, I'm the only person in the world who can connect them with that story."

translate id w1ExHanaHangout_c2730448:
    mc "Who's the dork now? That's so sentimental and corny."

translate id w1ExHanaHangout_e00f4217:
    hana "So what?"

translate id w1ExHanaHangout_d1c6754a:
    mc "So... I think that's really great. I love that. Thanks for sharing."

translate id w1ExHanaHangout_28aaec71:
    hana "No problem. Fair's fair, after all."

translate id w1ExHanaHangout_2c7f7320:
    "Hana smiled at me earnestly, in a way you rarely glimpse in your everyday life. Even from the people you're close to."

translate id w1ExHanaHangout_2ba230ef:
    hana "Let's get out of here. I feel like I'm about to boil."

translate id w1ExHanaHangout_91ffd0a5:
    mc "Heh, yeah. Me too."

translate id w1ExHanaHangout_0fd4d656_3:
    "......"

translate id w1ExHanaHangout_a20cefa7_3:
    "..."

translate id w1ExHanaHangout_c147eaaa:
    hana "Oh, [2]?"

translate id w1ExHanaHangout_184e7a19:
    "Hana looked me dead in the eyes, the corners of her mouth curling deviously into a mirthy grin."

translate id w1ExHanaHangout_def0ad19:
    mc "What is it?"

translate id w1ExHanaHangout_216cec6f:
    mc "Gh--!"

translate id w1ExHanaHangout_5b27eb0b:
    "In one unbroken motion, Hana hooked her fingertips into the underside of her bikini top and yanked it down in one quick motion, exposing her pale shapely breasts."

translate id w1ExHanaHangout_322265cb:
    mc "What are you...?"

translate id w1ExHanaHangout_9ac4a64b:
    hana "A dozen other topless women downstairs, eh?"

translate id w1ExHanaHangout_f9e05889:
    hana "Still think they're just a dime-a-dozen?"

translate id w1ExHanaHangout_b4d2273b:
    mc "Aaaah, I didn't mean it that way..."

translate id w1ExHanaHangout_215ace57:
    hana "You want to see these babies next time, maybe be more considerate with your words, jerk."

translate id w1ExHanaHangout_1cb1f8c2:
    mc "Wait, come back-!"

translate id w1ExHanaHangout_b97a2b99:
    "Hana left the sauna as quickly as she knocked me flat on my proverbial ass."

translate id w1ExHanaHangout_1d218c27:
    mc "Eh- oh...."

translate id w1ExHanaHangout_bea57e71:
    mct "(Wait, next time...?)"

translate id w1ExHanaHangout_0fd4d656_4:
    "......"

translate id w1ExHanaHangout_a20cefa7_4:
    "..."

translate id w1ExKatEnding_9ce7342b:
    "After whiling away the final leg of the exhibition with Hana and the club's patrons now enjoying the building's comforts, I found myself in Kathleen's office."

translate id w1ExKatEnding_ca73a78b:
    "With tonight's exhibition wrapping up and the patrons enjoying the club's amenities, booze, and women, I found myself in Kathleen's office."

translate id w1ExKatEnding_884555d7:
    "Alone, just the two of us."

translate id w1ExKatEnding_b7851052:
    kat "Good job tonight, [2]."

translate id w1ExKatEnding_ecd18889:
    kat "Performing under the spotlight as you did takes a non-trivial amount of mental fortitude."

translate id w1ExKatEnding_3ada1e95:
    mc "Not to mention, a few screws loose in the head and a troubling lack of shame."

translate id w1ExKatEnding_ab06b73a:
    kat "So candid. That's good."

translate id w1ExKatEnding_4990ab46:
    kat "However, I {1}was{2} sad to see you absent during the final game."

translate id w1ExKatEnding_266b8fce:
    kat "I know I told you to be a friend to Miss Rhodes, but don't let it interfere with your duties."

translate id w1ExKatEnding_63fffe7b:
    kat "It's good you're getting along with Miss Rhodes, but don't let it interfere with your duties."

translate id w1ExKatEnding_8f39730a:
    mc "Sorry, I should've asked you first to see if I was needed."

translate id w1ExKatEnding_b962e16f:
    kat "That's alright."

translate id w1ExKatEnding_41b16320:
    kat "It bodes well for our future relationship."

translate id w1ExKatEnding_8d692211:
    kat "All things considered, you performed to my standards tonight and made a positive impression on our members."

translate id w1ExKatEnding_666bc1b8:
    mc "After the initial hurdle of stepping on stage, it felt like a dream."

translate id w1ExKatEnding_41a4667e:
    kat "It's a peculiar feeling, isn't it? Everything begins to look soft under the harsh spotlight. It's strange."

translate id w1ExKatEnding_c148607e:
    mc "I felt like I couldn't even process my own thoughts at some points. That part of it wasn't totally unpleasant."

translate id w1ExKatEnding_22ced0e9:
    kat "You have a more refined sensibility than expected."

translate id w1ExKatEnding_6a0975cc:
    kat "Truthfully, I don't know if that's a bad thing or not, but it's a nice change of pace from the cruder element between these walls."

translate id w1ExKatEnding_fdcb1266:
    mc "Are you worried I can't do the job? You just said I lived up to your expectations."

translate id w1ExKatEnding_f3d46f90:
    kat "No, but your tendency to self-reflect can either help or hinder you in our work."

translate id w1ExKatEnding_5cdd23df:
    kat "After tonight, I can say I'm at least rooting for you."

translate id w1ExKatEnding_34ea3c27:
    kat "..."

translate id w1ExKatEnding_8a20aa92:
    "Mrs. Pulman's uncharacteristically gentle expression dissolved before my eyes, replaced with a gloomy-looking frown."

translate id w1ExKatEnding_843647e2:
    "Kathleen's uncharacteristically gentle expression dissolved before my eyes, replaced by a gloomy-looking frown."

translate id w1ExKatEnding_8cc62cea:
    mc "Is something the matter?"

translate id w1ExKatEnding_01ca4102:
    kat "No, it's nothing."

translate id w1ExKatEnding_cd49d8ee:
    mc "Wouldn't you rather enjoy yourself with the other guests? We could've discussed this downstairs."

translate id w1ExKatEnding_8fef5a2c:
    kat "What? Like I'm one of \"the boys\"?"

translate id w1ExKatEnding_9ba35caa:
    kat "No... after an exhibition, I prefer solitude. This place may be a world of my own creation, but I find parts of it aggravating and tiring."

translate id w1ExKatEnding_00bc7855:
    "Kathleen's words sounded very lonely to me."

translate id w1ExKatEnding_92afdff3:
    kat "However, you should go enjoy your night as you wish. I don't need you for anything else tonight."

translate id w1ExKatEnding_5b96b19a:
    mc "Well then, I think I'll head home."

translate id w1ExKatEnding_d637579e:
    kat "You're not going to stay and enjoy the party?"

translate id w1ExKatEnding_739a9193:
    mc "No, I've had enough tonight."

translate id w1ExKatEnding_ff0ddb73:
    kat "I understand."

translate id w1ExKatEnding_bcf0368d:
    kat "Take one of the USB sticks on the desk as a memento of your first exhibition of the club. It's footage of tonight's exhibition."

translate id w1ExKatEnding_f84142d7:
    mc "Eh...? Yeah, okay."

translate id w1ExKatEnding_a64e8bc0:
    mc "Thanks."

translate id w1ExKatEnding_1f17d9be:
    "I didn't {3}really{4} want it, but I decided turning it down would be more trouble than simply accepting it."

translate id w1ExKatEnding_07bb75e2:
    "Besides, I had another video I wanted to watch before I ended this absurd, abnormal night."

translate id w1ExKatEnding_278c7e29:
    "........."

translate id w1ExKatEnding_0fd4d656:
    "......"

translate id w1ExKatEnding_a20cefa7:
    "..."

translate id w1ExKatEnding_b78e1baa:
    man "Do you have anything to say to the viewers at home?"

translate id w1ExKatEnding_460289fd:
    vic "T-thanks for watching..."

translate id w1ExKatEnding_2b35b6fe:
    mc "..."

translate id w1ExKatEnding_dd00c517:
    "As soon as I got home, I put on one of my mother's old adult videos."

translate id w1ExKatEnding_f9d7957d:
    "Even if I found the contents repulsive, it was something I did from time to time. To me, it served as a tangible reminder of what she endured to give me a good home."

translate id w1ExKatEnding_bffb30fe:
    "It was bitterly sad to me that my mother thought she carried her past alone."

translate id w1ExKatEnding_1f5575c1:
    mct "(One day, I'll be honest with Mom.)"

translate id w1ExKatEnding_308a5464:
    "A few years ago, I made a promise to myself. I would tell her I knew about her life as a porn star and thank her from the bottom of my heart."

translate id w1ExKatEnding_effcfc76:
    mct "(On that day, I'd never watch these fucking things again.)"

translate id w1ExKatEnding_13e1de65:
    "Tonight though, I wanted to watch one for a different reason. To make sure I remembered that Rosalind, Veronica, and Felicia were human beings just like my mother."

translate id w1ExKatEnding_d899e368:
    "In the midst of hedonism, it's easy to lose your sense of empathy. I wanted to preempt that from happening."

translate id w1ExKatEnding_d1a5316b:
    "I do this, but at the same time..."

translate id w1ExKatEnding_c0067322:
    mct "(It was fun, wasn't it?)"

translate id w1ExKatEnding_c01fecde:
    "*Ring, ring. Ring, ring*"

translate id w1ExKatEnding_fa56c9f3:
    mct "(It's Mom...)"

translate id w1ExKatEnding_4e85cb4d:
    mc "Hello?"

translate id w1ExKatEnding_7ff35b4b:
    vic "Hey! I didn't wake you up, did I?"

translate id w1ExKatEnding_c67a6e0f:
    mc "No, I was still up. I was just watching a movie."

translate id w1ExKatEnding_7cc910c8:
    vic "Oh? What movie?"

translate id w1ExKatEnding_5859feb0:
    mc "The Life of Oharu."

translate id w1ExKatEnding_f4c7d9ab:
    vic "Why in the world are you watching that dreary thing? You should watch something more cheerful."

translate id w1ExKatEnding_e7056468:
    mc "You're probably right. Something more your speed, like {3}Black Belly of the Tarantula{4} or {3}Blood and Lace{4}?"

translate id w1ExKatEnding_f229227e:
    vic "Hehehe, that's just fantasy entertainment. Anyway, I'm calling you so late for a very special reason..."

translate id w1ExKatEnding_caadaac2:
    mc "Oh, I guess it is after midnight, isn't it?"

translate id w1ExKatEnding_94b33a55:
    vic "Bingo! Happy birthday, my baby boy! I'm so blessed to have you as my son!"

translate id w1ExKatEnding_c74b5716:
    mc "Thanks, Mom."

translate id w1ExKatEnding_25b90d0d:
    vic "What do you want to do tomorrow?"

translate id w1ExKatEnding_0fd4d656_1:
    "......"

translate id w1ExKatEnding_a20cefa7_1:
    "..."


translate id strings:
    old "Just be honest with them and speak from the heart."
    new "Just be honest with them and speak from the heart."

    old "Remind them of the relationship dynamic at play here first."
    new "Remind them of the relationship dynamic at play here first."

    old "Reassure Rosalind you're still in her corner."
    new "Reassure Rosalind you're still in her corner."

    old "{1}Show{2} Rosalind that the deal is still on."
    new "{1}Show{2} Rosalind that the deal is still on."

    old "{24}[14]{25} Make her prove herself to you."
    new "{24}[14]{25} Make her prove herself to you."

    old "{26}[14] Make her prove herself to you.{25}"
    new "{26}[14] Make her prove herself to you.{25}"

    old "Tease Ian about his childhood."
    new "Tease Ian about his childhood."

    old "Tell Ian that's all in the past."
    new "Tell Ian that's all in the past."

    old "What is my role tonight?"
    new "What is my role tonight?"

    old "What exactly did you mean by pop into the lounge and socialize?"
    new "What exactly did you mean by pop into the lounge and socialize?"

    old "How will the winner tonight be determined?"
    new "How will the winner tonight be determined?"

    old "That's all I wanted to ask."
    new "That's all I wanted to ask."

    old "Tell Hana you're sorry she has to put up with that."
    new "Tell Hana you're sorry she has to put up with that."

    old "Tell Hana she looks good in the outfit at least."
    new "Tell Hana she looks good in the outfit at least."

    old "Ask what that asshole's problem is."
    new "Ask what that asshole's problem is."

    old "Just go about your business."
    new "Just go about your business."

    old "Tell Warren this place is interesting."
    new "Tell Warren this place is interesting."

    old "Openly express your disgust."
    new "Openly express your disgust."

    old "Keep your thoughts to yourself."
    new "Keep your thoughts to yourself."

    old "Tell Jacob you'll defuse the situation."
    new "Tell Jacob you'll defuse the situation."

    old "Let Jacob break it up."
    new "Let Jacob break it up."

    old "Be professional."
    new "Be professional."

    old "Curtly remind Isaak of your name."
    new "Curtly remind Isaak of your name."

    old "{24}[8]{25} Speak the creep's language to get him to go away."
    new "{24}[8]{25} Speak the creep's language to get him to go away."

    old "{24}[15]{25} Keep your cool and respectfully redirect Isaak elsewhere."
    new "{24}[15]{25} Keep your cool and respectfully redirect Isaak elsewhere."

    old "Lie and say Hana is needed elsewhere."
    new "Lie and say Hana is needed elsewhere."

    old "Call this loser out on his shit."
    new "Call this loser out on his shit."

    old "{26}[8] Speak the creep's language to get him to go away.{25}."
    new "{26}[8] Speak the creep's language to get him to go away.{25}."

    old "{26}[15] Keep your cool and respectfully redirect Isaak elsewhere.{25}."
    new "{26}[15] Keep your cool and respectfully redirect Isaak elsewhere.{25}."

    old "Take a look at the feed."
    new "Take a look at the feed."

    old "Nah, I don't need to see other people fucking."
    new "Nah, I don't need to see other people fucking."

    old "Clear your throat and get Veronica's attention."
    new "Clear your throat and get Veronica's attention."

    old "Watch Veronica in silence."
    new "Watch Veronica in silence."

    old "Manhandle Veronica's tits."
    new "Manhandle Veronica's tits."

    old "Let your hands linger as you make sense of what's going on."
    new "Let your hands linger as you make sense of what's going on."

    old "Tell her she doesn't have anything to worry about."
    new "Tell her she doesn't have anything to worry about."

    old "Tell her not to get complacent."
    new "Tell her not to get complacent."

    old "Yes, continue."
    new "Yes, continue."

    old "No, let's look around some more."
    new "No, let's look around some more."

    old "Help Veronica get her business back on track."
    new "Help Veronica get her business back on track."

    old "Ignore your feelings, it could jeopardize your own dream."
    new "Ignore your feelings, it could jeopardize your own dream."

    old "Match Kathleen's example and show deference to the crowd."
    new "Match Kathleen's example and show deference to the crowd."

    old "Introduce and share a little bit about yourself."
    new "Introduce and share a little bit about yourself."

    old "Be concise with your introduction"
    new "Be concise with your introduction"

    old "Thank her for the opportunity."
    new "Thank her for the opportunity."

    old "Tell her you feel unsure at the prospect."
    new "Tell her you feel unsure at the prospect."

    old "You like a woman who can take charge and let loose."
    new "You like a woman who can take charge and let loose."

    old "You like a demure, obedient woman who can be slutty."
    new "You like a demure, obedient woman who can be slutty."

    old "You like an attentive woman that knows what buttons to push."
    new "You like an attentive woman that knows what buttons to push."

    old "You want to feel like a king."
    new "You want to feel like a king."

    old "You want to feel cared for."
    new "You want to feel cared for."

    old "You want to feel sadistic."
    new "You want to feel sadistic."

    old "You want to feel like you're being taken for a ride."
    new "You want to feel like you're being taken for a ride."

    old "Yes, Daddy."
    new "Yes, Daddy."

    old "Tell her she had it right the first time"
    new "Tell her she had it right the first time"

    old "You do like it when she calls you Daddy."
    new "You do like it when she calls you Daddy."

    old "{34}[16]{25} You'd prefer it if she called you..."
    new "{34}[16]{25} You'd prefer it if she called you..."

    old "Tell Felicia she's doing a good job."
    new "Tell Felicia she's doing a good job."

    old "Urge Felicia on with dirty talk"
    new "Urge Felicia on with dirty talk"

    old "Keep your trap shut and just focus on the pleasure."
    new "Keep your trap shut and just focus on the pleasure."

    old "Tell Felicia she's a freak."
    new "Tell Felicia she's a freak."

    old "Tell Felicia to get back to it."
    new "Tell Felicia to get back to it."

    old "Wait for Felicia to resume on her own."
    new "Wait for Felicia to resume on her own."

    old "Just ram your cock into Felicia's waiting mouth"
    new "Just ram your cock into Felicia's waiting mouth"

    old "Tease Felicia a little"
    new "Tease Felicia a little"

    old "That'll do. You can't wait anymore, anyway."
    new "That'll do. You can't wait anymore, anyway."

    old "Tell Felicia that's not good enough."
    new "Tell Felicia that's not good enough."

    old "Give Felicia's breasts a good whack."
    new "Give Felicia's breasts a good whack."

    old "Give Felicia's breasts a pull."
    new "Give Felicia's breasts a pull."

    old "Just keep hammering her throat."
    new "Just keep hammering her throat."

    old "Tell her you're doing your best to put on an entertaining show."
    new "Tell her you're doing your best to put on an entertaining show."

    old "Tell her you can't help it. It feels so good."
    new "Tell her you can't help it. It feels so good."

    old "Ignore her."
    new "Ignore her."

    old "Give her a kiss on the cheek."
    new "Give her a kiss on the cheek."

    old "That might give Kathleen the wrong idea."
    new "That might give Kathleen the wrong idea."

    old "Not yet. Inform Rosalind what name she will answer to tonight."
    new "Not yet. Inform Rosalind what name she will answer to tonight."

    old "Not yet. Inform Rosalind what she will call you tonight."
    new "Not yet. Inform Rosalind what she will call you tonight."

    old "Yes, tell her to stand back up."
    new "Yes, tell her to stand back up."

    old "From where you stand, she looks like a {1}slave{2} to you."
    new "From where you stand, she looks like a {1}slave{2} to you."

    old "You will call her what she is: a {1}whore{2}."
    new "You will call her what she is: a {1}whore{2}."

    old "Go with something cute: babe."
    new "Go with something cute: babe."

    old "Some roleplaying might be nice: call her {1}mom{2}"
    new "Some roleplaying might be nice: call her {1}mom{2}"

    old "{34}[16]{25} You're going to refer to her as..."
    new "{34}[16]{25} You're going to refer to her as..."

    old "You can't beat a classic: {1}Master{2}."
    new "You can't beat a classic: {1}Master{2}."

    old "You want to feel the affection: {1}Dear{2}."
    new "You want to feel the affection: {1}Dear{2}."

    old "You want this to feel taboo: {1}Son{2}."
    new "You want this to feel taboo: {1}Son{2}."

    old "Worshipped."
    new "Worshipped."

    old "Tormented."
    new "Tormented."

    old "Ask her to speed up."
    new "Ask her to speed up."

    old "Tell her to speed up."
    new "Tell her to speed up."

    old "{24}[17]{25} Relieve your bladder on Veronica's crumpled body."
    new "{24}[17]{25} Relieve your bladder on Veronica's crumpled body."

    old "{24}[18]{25} Make sure Veronica is okay."
    new "{24}[18]{25} Make sure Veronica is okay."

    old "Acknowledge her by slapping her ass."
    new "Acknowledge her by slapping her ass."

    old "Tell her no problem."
    new "Tell her no problem."

    old "Ejaculate on the prone woman's face."
    new "Ejaculate on the prone woman's face."

    old "Just let the time run out."
    new "Just let the time run out."

    old "{26}[17] Relieve your bladder on Veronica's crumpled body."
    new "{26}[17] Relieve your bladder on Veronica's crumpled body."

    old "{26}[18] Make sure Veronica is okay."
    new "{26}[18] Make sure Veronica is okay."

    old "Tell Killian to stop being a dick."
    new "Tell Killian to stop being a dick."

    old "Don't get in between the two."
    new "Don't get in between the two."

    old "No let the game decide"
    new "No let the game decide"

    old "Hit the low-setting button."
    new "Hit the low-setting button."

    old "Hit the medium setting button."
    new "Hit the medium setting button."

    old "Hit the high setting button."
    new "Hit the high setting button."

    old "Stick around and watch Rosalind get covered in cum."
    new "Stick around and watch Rosalind get covered in cum."

    old "Slip out of the room and find Hana."
    new "Slip out of the room and find Hana."

    old "Stick around and watch Felicia get covered in cum."
    new "Stick around and watch Felicia get covered in cum."

    old "Stick around and watch Veronica get covered in cum."
    new "Stick around and watch Veronica get covered in cum."

    old "Join in on the fun."
    new "Join in on the fun."

    old "Just keep filming."
    new "Just keep filming."
#========== AKHIR x-Week1Exhibition.rpy ==========

#========== AWAL x-Week2.rpy ==========
translate id june07start_3e30558f:
    "Due to a bug in the initial release, older saves will not have a certain variable properly registered. Please set that now."

translate id june07start_3b32c955:
    "Thank you. Please enjoy the update."

translate id june07start_21149ad7:
    "After the last couple of days, sleep came easily."

translate id june07start_d3bdc62e:
    "It felt like no sooner had I closed my eyes than I was already opening them, but the night darkening my windows had been replaced by sunlight pleasantly warming my face."

translate id june07start_fcdc7fbc:
    mct "(Mmh... what's that smell?)"

translate id june07start_78bcd2b5:
    "A faint smell hung in the air."

translate id june07start_f3e06bd3:
    mct "(Something like... {1}caramelized bacon{2}. Just like Mom used to...)"

translate id june07start_231f0b34:
    mct "(Just like Mom used to make Ian and me after a sleepover.)"

translate id june07start_6941efbe:
    mc "Yo, good morning."

translate id june07start_9975fdcd:
    kil "Happy birthday, dude."

translate id june07start_6bf249ed:
    mc "Thanks. I see you let yourself in."

translate id june07start_a6a764fc:
    kil "Yeah, you should remember to lock your door. Not everyone who creeps into your apartment will be doing it to cook you bacon."

translate id june07start_3daa5504:
    mc "I could've sworn I did lock it..."

translate id june07start_f5721348:
    kil "Missed you last night. You made off like a wallflower after the show ended."

translate id june07start_f27d4bcc:
    mc "I had my fill of the evening."

translate id june07start_6405081f:
    mc "You're up awful early, by the way. Did you even go home last night?"

translate id june07start_9701dde4:
    kil "Ha! Yep! Had to, Mina would've been pissed if I pulled that stunt twice in the same week."

translate id june07start_daced462:
    mc "What the hell, you're actually learning from your mistakes?"

translate id june07start_dde7ba2a:
    kil "Here you go, our mutual favorite."

translate id june07start_40f63bd5:
    mc "Thanks, Ian."

translate id june07start_33151f2d:
    kil "Of course, it's your birthday after all. You've got to start the day off right."

translate id june07start_f4d5347b:
    kil "Plus, you agreed to come with me to see my mom, remember?"

translate id june07start_fce3b604:
    mc "Ah, yeah... I did, huh? That kind of slipped my mind after last night's business."

translate id june07start_70176cf5:
    kil "You're not going to flake, right?"

translate id june07start_809bbf09:
    mc "No, I said I'll go with you, so I'll go. Condo or house?"

translate id june07start_3c3b2560:
    kil "House."

translate id june07start_daefeffc:
    mc "The one in Perigrine Terrace or...?"

translate id june07start_f49c9c7d:
    kil "The summer house."

translate id june07start_f747d362:
    mc "Dang, that's the furthest one."

translate id june07start_5b558fcc:
    kil "Don't worry. We'll be finished before lunch and I'll drop you off wherever you're meeting Vicky. Besides, my mom will be happy to see you!"

translate id june07start_19ffebde:
    mc "Really? As far as I could tell, she hated me."

translate id june07start_6df99916:
    kil "She didn't hate you! She just... didn't like me hanging out with you is all."

translate id june07start_7354ab27:
    mc "That always kind of bothered me, you know? Really fucked with my self-esteem..."

translate id june07start_398f5d16:
    kil "Oh, shut up. No it didn't! You jumped at any chance to piss her off."

translate id june07start_10e552d2:
    mc "Yeah, I guess it wasn't so unearned, was it?"

translate id june07start_2f6205fa:
    mc "What can I say, your mom looked fine as hell when she scolded us. Like a sexy librarian or something."

translate id june07start_17c4f93d:
    kil "Guh, not before we eat, please."

translate id june07start_f980d729:
    mc "What...? I'm sorry, am I making you uncomfortable?"

translate id june07start_8061d7b4:
    kil "Yeah, that's my fucking mom, you asshole! Just dig in before it gets cold!"

translate id june07start_364fd470:
    "All the while enjoying the nice breakfast Ian had made me, I continued to relentlessly tease him. It was my birthday."

translate id june07start_0fd4d656:
    "......"

translate id june07start_a20cefa7:
    "..."

translate id june07start_e63def69:
    "*Ding Dong*"

translate id june07start_9c023ddc:
    mc "Eh? You don't have a key?"

translate id june07start_ddd38aa3:
    kil "Not since last winter."

translate id june07start_adb59c02:
    mc "What happened last winter?"

translate id june07start_8de72f4d:
    kil "Eheh... I don't remember all too well, but the place had to be disinfected."

translate id june07start_9098da49:
    mc "Jeez..."

translate id june07start_33ac5c50:
    maid "Ah, boys...!"

translate id june07start_7095b653:
    maid "What's it been? Five or six years since I last saw the pair of you together?"

translate id june07start_437e0568:
    maid "You two always used to barrel through the front door after school and stomp through the house just to play your games quicker."

translate id june07start_88762a51:
    mc "Yeah, and you'd always ask us what the hurry was and tell us that they weren't going anywhere. How are you, Alice?"

translate id june07start_a4df8d33:
    alice "I'm good, [2]. A little surprised to see you."

translate id june07start_1b55ecd1:
    kil "Mom wanted to catch up, so I brought him along on an errand. She's not on a call or anything is she?"

translate id june07start_8b4b7475:
    alice "No. I believe she's just freshening up. I'll go tell her you're here."

translate id june07start_46a1b57c:
    "*Smack*"

translate id june07start_10338bac:
    alice "--!"

translate id june07start_160e4782:
    kil "Yeah, you go do that."

translate id june07start_bae62c8f:
    "The maid had an embarrassed, mortified look on her face."

translate id june07start_6bf79d56:
    alice "M-make yourself at home..."

translate id june07start_865ba007:
    mc "What the hell was that? Of all people, you're sexually harassing Alice now?"

translate id june07start_83371c47:
    kil "Heh, I guess I never told you."

translate id june07start_d3097eb9:
    mc "You really going to wait for me to ask \"told me what?\"...?"

translate id june07start_ed8c29a8:
    kil "Heh, I'm glad you asked. I tapped that about a couple of years back."

translate id june07start_c85ebc9f:
    mc "Huh...? You had sex with Alice? Isn't that kinda weird? She helped raise you."

translate id june07start_8bbb984a:
    kil "So? It wasn't like she did it out of love. She was paid to."

translate id june07start_c9ceca2d:
    mc "Man, you've got issues. How the hell did that even happen?"

translate id june07start_7dce7bc9:
    kil "Just kinda did. I was at the other house. I had just taken a shower and was feeling pretty horny when she walked in on me changing."

translate id june07start_e37de3ba:
    mc "Okay, but normally that's just an embarrassing situation. How'd that turn into you two screwing?"

translate id june07start_0ed244a2:
    kil "Oh, she was embarrassed. She apologized profusely, but before she could backtrack out the door, I yelled at her to stop."

translate id june07start_25ae98ce:
    kil "She got this funny look on her face. Like a deer {3}excited{4} to be caught in the headlights. That's when I knew. I told her to close the door and get her ass in there and before long..."

translate id june07start_47bd5287:
    kil "I had her screaming my name. With a little prodding, I even got her to admit I was fucking her better than her dead husband ever did. Can you believe it?"

translate id june07start_0c66f860:
    mc "No, I can't. This is just some fucked up fantasy you had as a teenager, isn't it?"

translate id june07start_847101c0:
    kil "I can prove it. I took some photos as a keepsake. You want me to send them to you later?"

translate id june07start_c0dc3e30:
    mct "(He took photos...?)"

translate id w2KillianAliceBet_c5b7798b:
    mc "Yeah, go ahead. I can just see them now... up close, grainy shots with no identifying characteristics."

translate id w2KillianAliceBet_ee54cde4:
    kil "Fine. It's a bet then."

translate id w2KillianAliceBet_9f840c87:
    mc "What are we wagering?"

translate id w2KillianAliceBet_797110e4:
    kil "The loser has to do what the other dares, redeemed at a point to be determined."

translate id w2KillianAliceBet_a3b21b14:
    mc "Within reason?"

translate id w2KillianAliceBet_9996a58e:
    kil "No. What you consider reasonable vastly differs from what I'd call reasonable. Let's say within the blast radius of reasonable. Sound good?"

translate id w2KillianAliceBet_2833915a:
    mct "(Hmm... he's sounding pretty confident about this. Do I make that wager?)"

translate id w2KillianAliceBet_6849b95f:
    mc "Wait, you're not kidding? Huh..."

translate id w2KillianAliceBet_c5382325:
    kil "I know that look. You're interested. Want to see what pudgy, homely Alice looks like when she's creaming herself on a cock."

translate id w2KillianAliceBet_db4e6513:
    mc "I'd be lying to say I wasn't curious. She was always around when we were kids and she was really nice, which..."

translate id w2KillianAliceBet_efbfd9d3:
    kil "Makes it that much more perverse and twisted, am I right?"

translate id w2KillianAliceBet_e6037426:
    mc "Yeah, so if you're not full of shit, I'm interested. Send me the pics."

translate id w2KillianAliceBet_3150575a:
    kil "Ha! Will do, bud."

translate id w2KillianAliceBet_4b279825:
    mc "No. Why would I want to see that? That's weird and pretty damn scummy."

translate id w2KillianAliceBet_62cd91d3:
    kil "Huh? She knows I took 'em."

translate id w2KillianAliceBet_0cde9d2c:
    mc "That's not what I'm getting at. On the slightest chance you're not full of crap, she wouldn't want you sharing them with me."

translate id w2KillianAliceBet_846fb7a7:
    kil "Ah, whatever. Suit yourself. You're pretty picky and choosy about the shit you get moralistic about."

translate id w2KillianAliceBet_b9d19ece:
    mc "Okay, then. You're on. Send me the photos. If I can tell who it is, you win."

translate id w2KillianAliceBet_fc4f0496:
    kil "And you'll do whatever I ask?"

translate id w2KillianAliceBet_d8abaed0:
    mc "Yep, I'll do whatever you ask. Same goes the other way though, yeah?"

translate id w2KillianAliceBet_e8e238ee:
    kil "It's a bet."

translate id w2KillianAliceBet_8df2ae37:
    mc "I'm not making that wager."

translate id w2KillianAliceBet_8e4b56be:
    kil "Is that because you believe me?"

translate id w2KillianAliceBet_485a447b:
    mc "Not necessarily, but it ain't worth the risk. To be honest, I would rather not know the truth."

translate id w2KillianAliceBet_86ec5efa:
    mc "I'd hate to know she had terrible taste in men."

translate id w2KillianAliceBet_90f79227:
    kil "'ey, fuck you. I made her slutty dreams come true. I'm a giver."

translate id w2KillianAliceBet_3038efed:
    grace "This is a nice surprise. You actually brought [2] along."

translate id w2KillianAliceBet_b8fcb7b6:
    "Cutting dangerously close to the end of our bawdy conversation, Mrs. Beaufort had appeared seemingly out of nowhere."

translate id w2KillianAliceBet_1e8aca52:
    "It was a bit nostalgic in a way. As kids and teens, she had a habit of sneaking up on us in the midst of mischief. The woman almost had no footsteps."

translate id w2KillianAliceBet_d928c0d9:
    kil "I said I would. You doubted it?"

translate id w2KillianAliceBet_76d9f528:
    grace "You do tend to forget things."

translate id w2KillianAliceBet_012e6917:
    grace "Hello, [2]. It's been some time."

translate id w2KillianAliceBet_ab70ddfc:
    mc "It has. It's nice to see you, Mrs. Beaufort. You look well."

translate id w2KillianAliceBet_32a56e94:
    grace "Thank you, as do you. Time has shaped you into a handsome young man."

translate id w2KillianAliceBet_085e3304:
    grace "Ian told me you were pursuing a career in medicine?"

translate id w2KillianAliceBet_184abb15:
    mc "I'm just pre-med right now, but I feel confident about my MCATs."

translate id w2KillianAliceBet_1251ed7c:
    grace "Have you decided on a specialty?"

translate id w2KillianAliceBet_295be0a5:
    mc "Well, I've always wanted to be a general practitioner, but I've given some thought about going into research recently."

translate id w2KillianAliceBet_85b050cf:
    grace "Everyone's different, but I think you'd find clinical work very rewarding. I do."

translate id w2KillianAliceBet_ef926c7b:
    mc "You don't get tired of working with kids?"

translate id w2KillianAliceBet_45378b56:
    "I thought back to my time as a tutor and how I came away from that with a stronger dislike for children."

translate id w2KillianAliceBet_8b6f2dc7:
    grace "No, that'd be a dire outcome for a pediatrician I would think."

translate id w2KillianAliceBet_0f91d505:
    grace "How have you been, son?"

translate id w2KillianAliceBet_71cfb899:
    kil "I've been good."

translate id w2KillianAliceBet_ac7a044a:
    grace "Why do I find that hard to believe?"

translate id w2KillianAliceBet_05fce681:
    kil "I've got all my fingers at least."

translate id w2KillianAliceBet_7eaaec80:
    grace "Don't be shy."

translate id w2KillianAliceBet_b4460441:
    "I think I must've given Mrs. Beaufort a reflexive, dubious look."

translate id w2KillianAliceBet_20d45413:
    grace "I know you do hugs, [2]."

translate id w2KillianAliceBet_d48c4656:
    "Never in my life had I given that chilly woman a hug."

translate id w2KillianAliceBet_fb1b5575:
    grace "Thank you."

translate id w2KillianAliceBet_92cb674d:
    "Maybe the distance in years had made her sentimental?"

translate id w2KillianAliceBet_2c947f85:
    grace "I hope coming all this way to just catch up wasn't too inconvenient for you?"

translate id w2KillianAliceBet_5477700f:
    mc "No, it's nice to see you, Mrs. Beaufort."

translate id w2KillianAliceBet_d4d85aab:
    kil "It's actually [2]'s birthday today."

translate id w2KillianAliceBet_327f8852:
    grace "Really? If I recall, you're a year older than Ian, which means you're turning 22?"

translate id w2KillianAliceBet_28ba79cd:
    mc "That's right."

translate id w2KillianAliceBet_909d47b7:
    grace "Happy birthday."

translate id w2KillianAliceBet_bb0eeaf4:
    mc "Thank you."

translate id w2KillianAliceBet_b486dc5b:
    grace "Would you like some tea?"

translate id w2KillianAliceBet_7e914af0:
    kil "No, we can't stay long. I just came to drop off that paperwork and then [2] needs to meet his mother for lunch."

translate id w2KillianAliceBet_c2f337e9:
    grace "Can you spare ten minutes at least?"

translate id w2KillianAliceBet_5fd89b46:
    kil "I don't-"

translate id w2KillianAliceBet_b0b1583e:
    mc "Of course. Ten minutes won't hurt."

translate id w2KillianAliceBet_cc7b0a4b:
    grace "Good. Then I would like to ask a small favor of my son, involving removing some boxes from the attic crawl space..."

translate id w2KillianAliceBet_a20cefa7:
    "..."

translate id w2KillianAliceBet_f510d811:
    mc "You sure you don't want me to help him?"

translate id w2KillianAliceBet_a7ff9b7d:
    grace "No. I actually wanted us to talk."

translate id w2KillianAliceBet_be82f248:
    mc "You mean catch up...?"

translate id w2KillianAliceBet_3299205a:
    grace "No."

translate id w2KillianAliceBet_732b4033:
    mct "(Just... no?)"

translate id w2KillianAliceBet_7a3a04ef:
    mc "What then?"

translate id w2KillianAliceBet_ba1ae20e:
    grace "Sit down, please."

translate id w2KillianAliceBet_9241fc5d:
    "Mrs. Beaufort had adopted a more familiar and stern tone."

translate id w2KillianAliceBet_1b0833a7:
    grace "I suggested Ian bring you along today for one specific reason."

translate id w2KillianAliceBet_c09ab9db:
    "She paused for a moment, to unblinkingly appraise me with her eyes."

translate id w2KillianAliceBet_59eca91e:
    grace "I have a proposition for you."

translate id w2KillianAliceBet_b12907fb:
    mc "Alright..."

translate id w2KillianAliceBet_a3278618:
    grace "For whatever reason, you always had a damnable hold over my son. He looked up to you, respected you, and even loved you."

translate id w2KillianAliceBet_1ad54a3c:
    grace "That is true, even now. He told me you two are working the same part-time job and I couldn't remember the last time I heard him so genuinely happy."

translate id w2KillianAliceBet_2b35b6fe:
    mc "..."

translate id w2KillianAliceBet_8d38850b:
    grace "To my surprise, you've actually become a respectable adult."

translate id w2KillianAliceBet_d2efecf2:
    mc "Thank you..."

translate id w2KillianAliceBet_5b60d6e1:
    grace "More respectable than Ian certainly. That boy isn't doing anything with his life."

translate id w2KillianAliceBet_ca589a90:
    mc "He's a photographer."

translate id w2KillianAliceBet_54426186:
    grace "No, he's a man-child using a camera to get laid."

translate id w2KillianAliceBet_52ca8892:
    mc "Eheh..."

translate id w2KillianAliceBet_25aab26e:
    mct "(It's not like I could refute her assessment exactly...)"

translate id w2KillianAliceBet_72532209:
    mc "What are you getting at, Mrs. Beaufort?"

translate id w2KillianAliceBet_3e41282d:
    grace "Yes, we should get straight to the point before Ian returns."

translate id w2KillianAliceBet_01e4945e:
    grace "I want you to use your influence over Ian to persuade him to go to college and pursue a proper career working with his father."

translate id w2KillianAliceBet_19b15881:
    mc "I don't know what you imagine, but Ian doesn't just do anything I tell him. Plus, y'know, it's his life..."

translate id w2KillianAliceBet_add9bad3:
    grace "You have more control over my son than you give yourself credit for. I've always despised that, but now I see it could work in his advantage."

translate id w2KillianAliceBet_21294bbb:
    grace "I don't know where I went wrong in raising him, but he's a philanderer and a leech. Something has got to give, his father is growing impatient."

translate id w2KillianAliceBet_e2d560bc:
    "Hearing Ian's own mother say those things made me feel sorry for him. Not that her appraisal was necessarily wrong, but the callous way she said it put some things into perspective."

translate id w2KillianAliceBet_974505ae:
    mc "Why would I do that?"

translate id w2KillianAliceBet_647914df:
    grace "I would hope it would be because he's your friend and this is what's best for him, but..."

translate id w2KillianAliceBet_965ff1ed:
    grace "Francis and I will pay you $5,000 if he enrolls next spring."

translate id w2KillianAliceBet_59334e94:
    mc "Mr. Beaufort knows about this?"

translate id w2KillianAliceBet_8d76e368:
    grace "Of course. It was my idea, but he agreed with it. He wants nothing more than to give Ian a job at his firm, but he needs a degree first, if only for credibility's sake."

translate id w2KillianAliceBet_8a370c74:
    mc "Shouldn't you just talk to him?"

translate id w2KillianAliceBet_24f0f05f:
    grace "I've tried dozens of times. He's impossible, so now I'm looking to you."

translate id w2KillianAliceBet_8a81f70c:
    mct "(Jeez...)"

translate id w2KillianAliceBet_b8620a34:
    grace "If you don't want the money, we can find another form of restitution."

translate id w2KillianAliceBet_8dbb5834:
    grace "For example, I know people. I could help you get into med school."

translate id w2KillianAliceBet_2b35b6fe_1:
    mc "..."

translate id w2KillianAliceBet_9ab3e3df:
    "All I could think was poor Ian."

translate id w2KillianAliceBet_de013c5e:
    mc "You do know he's turned out this way because of shit like this, right?"

translate id w2KillianAliceBet_b1c260fd:
    grace "It's crossed my mind, but I'm not going to entertain criticism from you of all people."

translate id w2KillianAliceBet_2b35b6fe_2:
    mc "..."

translate id w2KillianAliceBet_b12f72c5:
    grace "Shoot. It sounds like Ian's finished upstairs."

translate id w2KillianAliceBet_3824b08e:
    grace "I don't need an answer from you. If the terms are amenable, get it done. Then we can square away your payment. Whatever you want."

translate id w2KillianAliceBet_038067f2:
    grace "If you find this reprehensible, then I only ask you please treat this conversation as having never taken place. Surely you realize Ian knowing about this would hurt--"

translate id w2KillianAliceBet_39b8a48c:
    kil "Heh, I think I made record time."

translate id w2KillianAliceBet_9c5a015e:
    kil "What are you two talking about?"

translate id w2KillianAliceBet_4bd5bcb6:
    grace "Oh, he was just telling me about how Victoria is doing."

translate id w2KillianAliceBet_0fd4d656:
    "......"

translate id w2KillianAliceBet_a20cefa7_1:
    "..."

translate id w2KillianAliceBet_2a47d075:
    "On the ride back to the city, my mind was unsettled by my conversation with Mrs. Beaufort."

translate id w2KillianAliceBet_04c3a8f4:
    "I did my best to engage Ian in our usual small talk, but I couldn't shake the nagging pangs of sympathy for him."

translate id w2KillianAliceBet_4c41a05f:
    mct "(Not that Ian couldn't use a dose of growing up, but...)"

translate id w2KillianAliceBet_0d7420ab:
    mct "(He'd be livid if he knew what his mother just tried to bribe me to do.)"

translate id w2KillianAliceBet_354de412:
    kil "Hey, stop spacing out."

translate id w2KillianAliceBet_725f772a:
    kil "You've been acting weird ever since we left my mom. That bitch didn't say something shitty to you, did she?"

translate id w2KillianAliceBet_1a3ea20a:
    mc "Nah, we just talked about college. I just have other things on my mind is all."

translate id w2KillianAliceBet_40e40a69:
    kil "Wanna tell me about any of 'em? Maybe talking about it will help? At the very least, I'd like to know what's going on inside your head."

translate id w2KillianAliceBet_a20cefa7_2:
    "..."

translate id w2KillianAliceBet_cf0f17b7:
    vic "I'm glad you invited Ian to join us for lunch, but it's a shame he couldn't stick around for ice cream."

translate id w2KillianAliceBet_b76f7146:
    mc "It would've felt wrong not to ask him."

translate id w2KillianAliceBet_86851838:
    vic "The three of us used to spend a lot of birthdays together, huh?"

translate id w2KillianAliceBet_60488a1d:
    mc "Stop talking like you're an old lady."

translate id w2KillianAliceBet_01665682:
    vic "I am an old lady. I've got a 22 year old son."

translate id w2KillianAliceBet_5a8506ab:
    "She said it proudly, with a warm smile. Which pretty much put an end to the topic."

translate id w2KillianAliceBet_40a78592:
    vic "How are you enjoying your new job?"

translate id w2KillianAliceBet_81e2495e:
    mc "I think I've gotten used to it by now. Compared to my job as a tutor, the ups outweigh the downs."

translate id w2KillianAliceBet_2a667145:
    mc "Although in some sense, I guess I've traded entitled parents for another kind of pompous asshole."

translate id w2KillianAliceBet_178d60dd:
    vic "It's some sort of bar, right? Could I drop in and get a cup of coffee sometime? I have to admit, having my own son wait on me would be nice..."

translate id w2KillianAliceBet_93deaf69:
    mc "Ah..."

translate id w2KillianAliceBet_c14c584c:
    mc "Well..."

translate id w2KillianAliceBet_8e6a370e:
    vic "Oh, don't worry! I was just kidding. You wouldn't want your mother embarrassing you."

translate id w2KillianAliceBet_a425bab4:
    mc "Quiet. I'd never, no matter what you did, find you embarrassing. Not even for a second."

translate id w2KillianAliceBet_6fe56441:
    vic "[2]..."

translate id w2KillianAliceBet_2b35b6fe_3:
    mc "..."

translate id w2KillianAliceBet_fa7fb2ea:
    vic "You're so full of it!"

translate id w2KillianAliceBet_828010fd:
    mc "No, really. It's just that the place is... sort of exclusive. Men only."

translate id w2KillianAliceBet_9d74610f:
    vic "You're not working at some sort of strip club, are you?"

translate id w2KillianAliceBet_173d81a7:
    mc "No! It's like a cross between a lodge and a bar. Y'know, like a society."

translate id w2KillianAliceBet_1f040adc:
    vic "Like the Shriners?"

translate id w2KillianAliceBet_49db6e3b:
    mc "Something like that..."

translate id w2KillianAliceBet_a6653f9b:
    vic "That's great. It must be good for networking then."

translate id w2KillianAliceBet_4e62d107:
    mc "Yeah, that's one of the ups. How about you? How's work? How have you been?"

translate id w2KillianAliceBet_d49c75d0:
    "I most deliberately forced a change of topic."

translate id w2KillianAliceBet_5207f937:
    vic "Nothing worth mentioning... oh! I had the nicest conversation with a stranger on the phone this morning."

translate id w2KillianAliceBet_c51f7eda:
    mc "You had a conversation with a random caller?"

translate id w2KillianAliceBet_1ea9a452:
    vic "Not random. It was a woman who said she was in charge of a scholarship you applied for. A, uh... Katherine Grossman?"

translate id w2KillianAliceBet_48b2364c:
    mc "You don't mean..."

translate id w2KillianAliceBet_633d3cc8:
    mct "(Why the hell did that bitch...)"

translate id w2KillianAliceBet_2f34c1a0:
    mct "(Why the hell did that woman...)"

translate id w2KillianAliceBet_c5e3e065:
    mc "Kathleen Pulman?"

translate id w2KillianAliceBet_2ba9a852:
    vic "Ah-huh! That was the name. She was an extremely pleasant lady."

translate id w2KillianAliceBet_93f87835:
    mc "What... did... you... two talk about, exactly?"

translate id w2KillianAliceBet_99bc6110:
    vic "She said it was a character reference check, but we ended up talking about a little of everything."

translate id w2KillianAliceBet_d48ddf09:
    vic "I talked about how hard of a worker you are, shared some cute stories from when you were a kid, even told her it was your birthday today. She told me about her own children and ended up getting me to talk about myself a little."

translate id w2KillianAliceBet_b61db285:
    vic "We chatted for a whole hour."

translate id w2KillianAliceBet_7c4842a4:
    "I could feel my nose twitch in irritation and my stomach turn over on itself out of anger."

translate id w2KillianAliceBet_e9b37966:
    "My new world had intersected with the old. Not by accident, either..."

translate id w2KillianAliceBet_c7ca3906:
    mc "That sounds nice."

translate id w2KillianAliceBet_15423513:
    vic "Yeah, it was! That or I'm starved for a social life."

translate id w2KillianAliceBet_362f7a64:
    mc "Did she get everything she needed?"

translate id w2KillianAliceBet_675f4e22:
    vic "I think so. She agreed with me that you sounded like a deserving kid."

translate id w2KillianAliceBet_65d113f0:
    mct "(...I'll have to ask what the fuck that was about later.)"

translate id w2KillianAliceBet_18cc8531:
    "For now, I should just put it out of my mind."

translate id w2KillianAliceBet_43da8991:
    vic "Do you have any special plans tonight?"

translate id w2KillianAliceBet_07b7fccc:
    mc "No, I'm just gonna stay in. You know I don't like to do anything for my birthday."

translate id w2KillianAliceBet_a424f951:
    vic "I don't know how that ever happened. You used to get so excited about them."

translate id w2KillianAliceBet_811c8498:
    mc "Yeah, when I was like 9..."

translate id w2KillianAliceBet_5b08f856:
    vic "You don't have to try so hard. It's okay to be uncool sometimes."

translate id w2KillianAliceBet_3b8cef51:
    mc "I don't think you have to worry about anyone using that word to describe me."

translate id w2KillianAliceBet_acf8ff13:
    vic "You should give yourself more credit."

translate id w2KillianAliceBet_eda20958:
    vic "I think you're the coolest."

translate id w2KillianAliceBet_f964fd14:
    vic "Happy birthday. I love you."

translate id w2KillianAliceBet_c91384ea:
    mc "I love you too, mom."

translate id w2KillianAliceBet_03e4c950:
    vic "The ladies' room is calling."

translate id w2KillianAliceBet_6e87510d:
    mc "Don't worry. I won't try and make an escape."

translate id w2KillianAliceBet_11f5a66f:
    vic "Good. It would be futile."

translate id w2KillianAliceBet_bf66b0c4:
    mct "(...)"

translate id w2KillianAliceBet_17deefc9:
    "Just as I felt a smile coming on..."

translate id w2KillianAliceBet_1d5c3291:
    "*Chirp, chirp.*"

translate id w2KillianAliceBet_c73bfc57:
    "The face of my phone lit up and told me I had a message from Ian."

translate id w2KillianAliceBet_7ee3cd92:
    mct "(I wonder what this is about...)"

translate id w2KillianAliceBet_1cf7435b:
    mc "-gh! *cough* -gh! *cough, chough*"

translate id w2KillianAliceBet_2c7cccc9:
    "The message's attachment sent a shock through my system, causing me to momentarily sputter and choke on my own spit."

translate id w2KillianAliceBet_62cee127:
    "On screen, was Alice."

translate id w2KillianAliceBet_6c849a0e:
    "Mild mannered Alice, who had always been cordial to me when I was growing up, was in frame. Ian held a tight grip on one of her ankles, pulling her leg back past her shoulder and putting her wide hips and oozing cunt on display."

translate id w2KillianAliceBet_7f5c0f76:
    "The normally frumpy-looking woman had a dopey look on her face, eyes glazed over from pleasure and lips wet with drool and who knows what else. A wad of jizz sat squarely on her deceptively shapely breasts."

translate id w2KillianAliceBet_eb082119:
    "I was caught somewhere between astonishment at my friend's shameless conquest of his former nanny and disbelief at just how much of a horndog he actually was."

translate id _call_phone_end_kil_9_be1c9d90:
    mct "(...jeez, what the fuck is wrong with him?)"

translate id _call_phone_end_kil_9_e5f235b2:
    vic "You tired of me yet?"

translate id _call_phone_end_kil_9_1a5d0d35:
    mc "Of course not, why?"

translate id _call_phone_end_kil_9_63c0b538:
    vic "I could use a walk in the park after eating all that ice cream."

translate id _call_phone_end_kil_9_0f1be577:
    mc "Let's go then."

translate id _call_phone_end_kil_9_0fd4d656:
    "......"

translate id _call_phone_end_kil_9_a20cefa7:
    "..."

translate id _call_phone_end_kil_9_0fd4d656_1:
    "......"

translate id _call_phone_end_kil_9_a20cefa7_1:
    "..."

translate id _call_phone_end_kil_9_60c9e3e1:
    "After spending my afternoon in the park with mom, I was happy to be home. Free to decompress and relax."

translate id _call_phone_end_kil_9_0f1dc65b:
    "After a nice walk in the park to burn off the calories, I found myself back home. Glad to be able to decompress and relax."

translate id _call_phone_end_kil_9_c2ff343c:
    "The last week had felt jam-packed. At least to my sleepy, student standards."

translate id _call_phone_end_kil_9_6ccede00:
    "In the last couple of days, I felt like I had tasted more of life than I had previously in my 22 years."

translate id _call_phone_end_kil_9_78add2e0:
    mct "(An evening with just me, a pizza, and a couple of movies would be appreciated...)"

translate id _call_phone_end_kil_9_e63def69:
    "*Ding Dong*"

translate id _call_phone_end_kil_9_51e2f837:
    mc "...fuck."

translate id _call_phone_end_kil_9_64310d90:
    mct "(It's funny, just a few weeks ago I would've been embarrassed to answer the door like this, but that notion seems so... quaint now.)"

translate id _call_phone_end_kil_9_ab7c1b98:
    mc "Who is it?"

translate id _call_phone_end_kil_9_13879f01:
    hana "Your favorite bartender. Open up."

translate id _call_phone_end_kil_9_0030957f:
    hana "Whoa, there. You're not dressed? I'm definitely here on time..."

translate id _call_phone_end_kil_9_9bd4eb47:
    hana "Opening the door with only a towel on... this isn't some half-assed attempt to seduce me, is it?"

translate id _call_phone_end_kil_9_e0067dfd:
    mc "On time for what?"

translate id _call_phone_end_kil_9_9a84f8a5:
    hana "Ian said it was your birthday. Gave me your address and a time to get here... nice place, by the way."

translate id _call_phone_end_kil_9_999c8a9e:
    mc "Thanks, but it belongs to the club."

translate id _call_phone_end_kil_9_c70da488:
    hana "Heh, I guess my dad has both of us bought and paid for."

translate id _call_phone_end_kil_9_6c1d4175:
    mc "I was actually planning on spending the night alone."

translate id _call_phone_end_kil_9_bbd424ab:
    hana "I can dig the value of a lil' alone time, but no offense, that sounds pretty sad."

translate id _call_phone_end_kil_9_35402dfd:
    mc "What did Ian tell you exactly?"

translate id _call_phone_end_kil_9_5175df38:
    hana "He said you were having a small get together. Nothing big, just a few people."

translate id _call_phone_end_kil_9_6dfad9ee:
    hana "You were nice enough to come to my gig this week, so I thought I would pay you back and return the favor."

translate id _call_phone_end_kil_9_1a81da74:
    hana "I figured I'd be a good co-worker and stop by."

translate id _call_phone_end_kil_9_fd9c1515:
    mc "That's news to me."

translate id _call_phone_end_kil_9_1fbafb11:
    hana "Maybe it was supposed to be a surprise party?"

translate id _call_phone_end_kil_9_64361aae:
    mc "Pretty piss poor execution if it is."

translate id _call_phone_end_kil_9_10cb0be1:
    hana "You sound a little iffy..."

translate id _call_phone_end_kil_9_afe4142a:
    hana "I did come ALL this way, but do you want me to go?"

translate id _call_phone_end_kil_9_b9cb5b1d:
    "Hana stuck out her chest and batted her eyes at me in an atypical display of her womanly wiles."

translate id _call_phone_end_kil_9_e7db59e6:
    mc "What kind of man would I be if I turned you away?"

translate id _call_phone_end_kil_9_299c1b21:
    hana "A foolish one."

translate id _call_phone_end_kil_9_95410400:
    mc "Make yourself at home. I'm going to go get dressed."

translate id _call_phone_end_kil_9_520167f7:
    mc "Help yourself to the fridge."

translate id _call_phone_end_kil_9_64f433c6:
    hana "What did you have planned for all by your lonesome?"

translate id _call_phone_end_kil_9_a3a6b4c7:
    mc "Movies. A double bill, actually. You interested?"

translate id _call_phone_end_kil_9_342999f0:
    hana "Double features can be fun. What were you thinking?"

translate id _call_phone_end_kil_9_1432b1c7:
    mc "The first is {3}Tampopo{4}, a movie by Juzo Itami. It starred his wife in the titular role, playing a proprietress of a struggling noodle shop."

translate id _call_phone_end_kil_9_2798c4b7:
    mc "It has the peculiar distinction of being the first \"ramen\" western. Get it?"

translate id _call_phone_end_kil_9_34022371:
    hana "Yeah, that's funny."

translate id _call_phone_end_kil_9_bb113f9e:
    mc "It's a movie that always puts a smile on my face."

translate id _call_phone_end_kil_9_f884139c:
    hana "What's the other half of the bill?"

translate id _call_phone_end_kil_9_a83e6544:
    mc "The second film is a fun, satirical look at 80s American consumerism: {3}The Stuff{4}."

translate id _call_phone_end_kil_9_c9d8b085:
    mc "It's about an addictive cream-like alien substance that is put on grocery store shelves and sold to the general public, gradually turning the masses into mind-controlled zombies."

translate id _call_phone_end_kil_9_1af65611:
    hana "That's the guy who did Q the Winged Serpent, right? That's Spider's favorite movie."

translate id _call_phone_end_kil_9_13f895f3:
    mc "Correct. Those two movies not only share the same director, but also the same star."

translate id _call_phone_end_kil_9_4a34730e:
    mc "Can you guess what links the two?"

translate id _call_phone_end_kil_9_13ecbea5:
    hana "You made it pretty easy to guess: {1}food{2}."

translate id _call_phone_end_kil_9_f1d898eb:
    mc "Bingo! So, you hungry?"

translate id _call_phone_end_kil_9_a20cefa7_2:
    "..."

translate id _call_phone_end_kil_9_e0ab77b5:
    hana "What the hell? This tastes awesome."

translate id _call_phone_end_kil_9_a64e8bc0:
    mc "Thanks."

translate id _call_phone_end_kil_9_c0bcf51b:
    hana "Where'd you learn to cook like this?"

translate id _call_phone_end_kil_9_0c3efa1c:
    mc "Byproduct of being raised by a single mother I guess."

translate id _call_phone_end_kil_9_12016d7b:
    mc "You can relate, right?"

translate id _call_phone_end_kil_9_de443e14:
    hana "Nah. My grandfather did all the cooking when he was alive. After that, it was always takeout for my mom and I."

translate id _call_phone_end_kil_9_fe9c334e:
    hana "I wish I could say the same. My grandfather did all the cooking when he was alive. After that, it was always takeout for my mom and I."

translate id _call_phone_end_kil_9_33e45eae:
    mc "Right. Your grandfather. You told me about him."

translate id _call_phone_end_kil_9_e5107c53:
    hana "I mean, I can cook enough to sustain myself, but this is surprisingly delicious. Consider me impressed."

translate id _call_phone_end_kil_9_dd44229c:
    mc "It really isn't that impressive."

translate id _call_phone_end_kil_9_1d2f796c:
    hana "Take the compliment you dork."

translate id _call_phone_end_kil_9_e63def69_1:
    "*Ding Dong*"

translate id _call_phone_end_kil_9_1e8c1d15:
    kil "Coming in!"

translate id _call_phone_end_kil_9_e574ee9b:
    kil "Aw, damn. I never catch you doing anything fun."

translate id _call_phone_end_kil_9_e7c062a5:
    mina "Sorry about barging in!"

translate id _call_phone_end_kil_9_13e3850d:
    "Mina quickly elbowed past her boyfriend and apologetically gave a little bow."

translate id _call_phone_end_kil_9_41cbbcd7:
    mina "You know how he is..."

translate id _call_phone_end_kil_9_7bb780af:
    mina "Hello, [2]! Happy birthday!"

translate id _call_phone_end_kil_9_28302961:
    mc "Thank you, Mina. How are you?"

translate id _call_phone_end_kil_9_9168393b:
    mina "I'm great! Still riding the excitement from getting the part yesterday!"

translate id _call_phone_end_kil_9_80c30bea:
    kil "I know you'd rather be by yourself tonight, but fuck that! You're my best friend!"

translate id _call_phone_end_kil_9_e8beffdf:
    kil "I can't just let that pass. Plus, we brought presents and cake. So no bitching, okay?"

translate id _call_phone_end_kil_9_7aa84e4f:
    mc "Okay, no bitching."

translate id _call_phone_end_kil_9_8a28cba7:
    kil "Plus, I invited Hana. You two seemed to get along pretty well yesterday."

translate id _call_phone_end_kil_9_529020e7:
    mct "(He's really misreading things there... I've got no interest in her like that.)"

translate id _call_phone_end_kil_9_f2ee18d0:
    kil "Personally, I can't stand that bitch, but that's got nothing to do with getting you laid. I even told her the wrong time so you two could get some 1-on-1 time and work your charm before we got here."

translate id _call_phone_end_kil_9_1a366ea7:
    mc "Uh, thanks..."

translate id _call_phone_end_kil_9_93e39ac7:
    mina "Eeeey, what are you two whispering about?"

translate id _call_phone_end_kil_9_ba670617:
    hana "You're probably better off not knowing."

translate id _call_phone_end_kil_9_a9301777:
    mina "Oh! Right! We haven't been introduced yet."

translate id _call_phone_end_kil_9_54d29ce5:
    "With a cute hop, Mina bounded toward Hana and closed the distance between them."

translate id _call_phone_end_kil_9_fd2f65a5:
    hana "No, we haven't..."

translate id _call_phone_end_kil_9_e31b58fb:
    "Hana ran her gaze down Mina's body from top to bottom, as if making a careful appraisal of the bubbly woman."

translate id _call_phone_end_kil_9_57b5c0b5:
    hana "You're Ian's girlfriend?"

translate id _call_phone_end_kil_9_689feeaa:
    mina "Yup, I'm Mina! You're Hana, right?"

translate id _call_phone_end_kil_9_c1475ac9:
    hana "You're both exactly and not at all what I imagined you to be."

translate id _call_phone_end_kil_9_3cc5d9c7:
    mina "Uh... what? Eh..."

translate id _call_phone_end_kil_9_f14e7fc0:
    mina "I love your tattoos. They're really, {1}really{2} cool!"

translate id _call_phone_end_kil_9_75f0cc76:
    "Momentarily flabbergasted, Mina quickly found room for a rebound."

translate id _call_phone_end_kil_9_bc1e9a6e:
    hana "Hmmm... you kind of look familiar."

translate id _call_phone_end_kil_9_a7538dbc:
    mina "Just one of those faces I guess..."

translate id _call_phone_end_kil_9_95fdd712:
    kil "Mina's a model. Maybe you've seen her face on a bus stop ad or something."

translate id _call_phone_end_kil_9_0b7ba011:
    hana "Maybe, but I feel like it's something else..."

translate id _call_phone_end_kil_9_d47647d1:
    mina "We definitely haven't met before. I'd remember someone as beautiful as you."

translate id _call_phone_end_kil_9_4b9b58ea:
    hana "You really lay it on thick, don't you?"

translate id _call_phone_end_kil_9_905ae31a:
    mina "S-sorry..."

translate id _call_phone_end_kil_9_c3141f32:
    hana "Don't be. It's nice to meet you too, Mina. I'll try not hold your choice of boyfriend against you."

translate id _call_phone_end_kil_9_ab0b3f52:
    mina "Heheh~♥ I'd appreciate that."

translate id _call_phone_end_kil_9_b3603e7f:
    "Looking like an excited puppy dog, Mina emphatically shook the rocker girl's offered hand."

translate id _call_phone_end_kil_9_97824ca1:
    mc "Guess we're all friends now."

translate id _call_phone_end_kil_9_4bd20760:
    kil "The two of them mingling sends a chill up my spine..."

translate id _call_phone_end_kil_9_38319f15:
    mc "You only have yourself to blame for that."

translate id _call_phone_end_kil_9_0fd4d656_2:
    "......"

translate id _call_phone_end_kil_9_a20cefa7_3:
    "..."

translate id _call_phone_end_kil_9_9bc839a5:
    "After sharing in the cake Mina had bought, it was time to open the gifts. The normality of the scene felt striking to me, considering I was balls deep in yesterday's wall-to-wall debauchery."

translate id _call_phone_end_kil_9_621f9ef4:
    kil "Me first!"

translate id _call_phone_end_kil_9_a6950ca5:
    "Killian slid the wrapped present my way, wearing an eager grin."

translate id _call_phone_end_kil_9_990dc729:
    "Picking up the gift in front of me, I gave it a tentative shake, making note of its weight and shape like a kid on Christmas Eve."

translate id _call_phone_end_kil_9_380b55bb:
    "It had a decent heft to it and was tightly packaged."

translate id _call_phone_end_kil_9_18ba95f3:
    kil "Just go ahead and open it."

translate id _call_phone_end_kil_9_6cf1dad8:
    hana "Someone's excited..."

translate id _call_phone_end_kil_9_a20cefa7_4:
    "..."

translate id _call_phone_end_kil_9_189fe6c2:
    kil "I know you don't play games much anymore, but I couldn't think of much else."

translate id _call_phone_end_kil_9_aef6cddb:
    mc "This is an expensive gift, Ian..."

translate id _call_phone_end_kil_9_1326eee4:
    kil "Maybe we could start playing again sometime, eh?"

translate id _call_phone_end_kil_9_a664777f:
    "Killian flashed me a child-like smile."

translate id _call_phone_end_kil_9_744e6d85:
    mc "Yeah, we'll do that. Thank you."

translate id _call_phone_end_kil_9_2929cfac:
    mina "I'm next!"

translate id _call_phone_end_kil_9_5e925a1b:
    mina "I hope you like it."

translate id _call_phone_end_kil_9_718014de:
    kil "Me too. She spent a week probing me for information."

translate id _call_phone_end_kil_9_14c2e86d:
    "I thought back to the drinking game Felicia, Mina, Ian, and I played a while back."

translate id _call_phone_end_kil_9_4b6826c1:
    mct "(She actually remembered that?)"

translate id _call_phone_end_kil_9_163d3a1f:
    kil "She's weird about crap like that."

translate id _call_phone_end_kil_9_b1870678:
    mina "No I'm not! That's what normal people do."

translate id _call_phone_end_kil_9_a0ce3b4b:
    mc "With how much thought you put into it, I'm sure it's a great gift."

translate id _call_phone_end_kil_9_543379d3:
    "Mina beamed as she put the present in front of me."

translate id _call_phone_end_kil_9_14ea40db:
    "Just like before, I picked up the box in front of me and gave it a test shake."

translate id _call_phone_end_kil_9_ec5ee3c8:
    mct "(Hmm...)"

translate id _call_phone_end_kil_9_2ff01350:
    "It had much less heft to it than Ian's, but it was also tightly packaged."

translate id _call_phone_end_kil_9_a20cefa7_5:
    "..."

translate id _call_phone_end_kil_9_110e3ffe:
    mina "I hope you didn't already have one. I did a little research and that topped the list of things new med students needed..."

translate id _call_phone_end_kil_9_6d5bcead:
    mina "It's cardiology grade. That's good, right?"

translate id _call_phone_end_kil_9_bb27b6d0:
    kil "A little research? She spent a whole evening trying to narrow down the best one."

translate id _call_phone_end_kil_9_0fae98f4:
    mct "(This is... at least as expensive as Ian's gift. Maybe even more...)"

translate id _call_phone_end_kil_9_9fe5da6f:
    mc "This is an extremely, EXTREMELY thoughtful gift. Thank you."

translate id _call_phone_end_kil_9_eb9806c0:
    mc "You spent way too much money on this."

translate id _call_phone_end_kil_9_3e1c2755:
    mina "No, I didn't. Happy birthday, [2]. I'm glad we met!"

translate id _call_phone_end_kil_9_0a4de74f:
    "Mina leaned over and gave me a big, genuine hug that matched her warm words."

translate id _call_phone_end_kil_9_61407464:
    mc "No, you really did. These things cost-"

translate id _call_phone_end_kil_9_e2992c26:
    mina "Shuuush! I was raised not to talk about money. Just accept the dang gift!"

translate id _call_phone_end_kil_9_abdba75d:
    mc "Heh, alright... thanks again."

translate id _call_phone_end_kil_9_b8428a5a:
    hana "Bleugh..."

translate id _call_phone_end_kil_9_4766177a:
    kil "..."

translate id _call_phone_end_kil_9_7762f0dc:
    mina "I wish I would've went first."

translate id _call_phone_end_kil_9_1da04222:
    mina "It really isn't that great of a gift..."

translate id _call_phone_end_kil_9_b3b9ac41:
    mc "That's okay. We've only known each other a month. It's difficult enough giving gifts to people you've known your whole life."

translate id _call_phone_end_kil_9_23a75021:
    mc "I suck at it, at least."

translate id _call_phone_end_kil_9_59de5ca0:
    mina "Hehe... me too! Still, I hope you'll like it."

translate id _call_phone_end_kil_9_543379d3_1:
    "Mina beamed as she put the present in front of me."

translate id _call_phone_end_kil_9_14ea40db_1:
    "Just like before, I picked up the box in front of me and gave it a test shake."

translate id _call_phone_end_kil_9_ec5ee3c8_1:
    mct "(Hmm...)"

translate id _call_phone_end_kil_9_39076252:
    "It had much less heft to it than Ian's and with less give."

translate id _call_phone_end_kil_9_a20cefa7_6:
    "..."

translate id _call_phone_end_kil_9_0e14dedf:
    mina "Again, it was a bit last minute... I usually like to have more time to pick out gifts..."

translate id _call_phone_end_kil_9_77aac7de:
    kil "No kidding. Mina spent two weeks researching and picking out a graduation gift for her niece earlier this year."

translate id _call_phone_end_kil_9_163d3a1f_1:
    kil "She's weird about crap like that."

translate id _call_phone_end_kil_9_4a45fa5f:
    mina "No I'm not! That's just what normal people do."

translate id _call_phone_end_kil_9_472b1f53:
    "These were pretty expensive-looking sunglasses. Not exactly my style, but they seemed quality."

translate id _call_phone_end_kil_9_9a3b6d44:
    mc "Thank you Mina. I don't own a pair of sunglasses, this is a nice gift."

translate id _call_phone_end_kil_9_72ed2956:
    mina "I'm glad you like it. Happy birthday, [2]. I'm glad we met!"

translate id _call_phone_end_kil_9_0a4de74f_1:
    "Mina leaned over and gave me a big, genuine hug that matched her warm words."

translate id _call_phone_end_kil_9_b8428a5a_1:
    hana "Bleugh..."

translate id _call_phone_end_kil_9_7aaccbdc:
    kil "Now that the cake and gifts are out of the way..."

translate id _call_phone_end_kil_9_04090275:
    hana "It's time to start drinking?"

translate id _call_phone_end_kil_9_1d8a38bf:
    kil "Why, what a brilliant idea, Hana. What do you say, birthday boy?"

translate id _call_phone_end_kil_9_f4942a26:
    mc "Ah, hell... this place is stocked I guess, might as well bust open a bottle of gin or something."

translate id _call_phone_end_kil_9_8bb95b19:
    mina "{5}Why do hangouts always have to end with a hangover?{6}"

translate id _call_phone_end_kil_9_f27c126a:
    hana "So, the plan is to get liquored up and watch some movies?"

translate id _call_phone_end_kil_9_8fea2db2:
    mc "Hmm..."

translate id _call_phone_end_kil_9_836c241a:
    "I gave it some thought. Of course, I liked the idea of ending the night quietly, but..."

translate id _call_phone_end_kil_9_b801d322:
    mc "Why don't we play a drinking game instead? Something fun that will let us get to know each other better."

translate id _call_phone_end_kil_9_daeffc55:
    "Opening up and doing things differently wouldn't be so bad."

translate id _call_phone_end_kil_9_81ab8e69:
    kil "What did you have in mind?"

translate id _call_phone_end_kil_9_9aa3a934:
    mc "How about... {3}Never Have I Ever{4}?"

translate id _call_phone_end_kil_9_bef35012:
    mina "Yes, please! That is so much better than one of Ian's convoluted games!"

translate id _call_phone_end_kil_9_0fd4d656_3:
    "......"

translate id _call_phone_end_kil_9_a20cefa7_7:
    "..."

translate id _call_phone_end_kil_9_c420d7bb:
    mc "Everyone is familiar with how this goes?"

translate id _call_phone_end_kil_9_8a7ec3de:
    hana "Someone admits to something they've never done before and the people who've done it drink. That continues until we're all smashed."

translate id _call_phone_end_kil_9_20897057:
    mc "Exactly. It's not complicated, but there is some finesse to it. Try and pick an answer that will drag an embarrassing story out from someone."

translate id _call_phone_end_kil_9_d5ea2529:
    mc "However, let's make things more interesting."

translate id _call_phone_end_kil_9_64af01f0:
    kil "What do you got cooking in that head of yours?"

translate id _call_phone_end_kil_9_5e77106a:
    mc "Let's say... every time someone drinks on your turn, you get a point. The person with the most points by the end of the game will be declared the winner."

translate id _call_phone_end_kil_9_cb6e1c9d:
    mc "The losers will have to do something the winner commands. One big group dare."

translate id _call_phone_end_kil_9_ab2c99f2:
    hana "What if there's a tie?"

translate id _call_phone_end_kil_9_3b08b34d:
    mc "Then there will be multiple winners, but only one command."

translate id _call_phone_end_kil_9_b083ad9d:
    hana "Hmm, alright. I'm game."

translate id _call_phone_end_kil_9_211f3fd9:
    mina "Me too, I {1}guess{2}..."

translate id _call_phone_end_kil_9_73155e99:
    kil "Who goes first?"

translate id _call_phone_end_kil_9_32872453:
    mc "I'll start and then we'll just go around the room from there."

translate id _call_phone_end_kil_9_d88ca544:
    "There's a lot of things I {3}haven't{4} done, but I should make the first answer a good one..."

translate id _call_phone_end_kil_9_eed8e7d8:
    mc "Alright, let's see..."

translate id _call_phone_end_kil_9_02d534ad:
    m_dev "Six points yields Killian +3, otherwise results in Killian +1, Mina BiCurious +1"

translate id _call_phone_end_kil_9_078f4ed4:
    mc "Never have I ever... been inappropriately nude or exposed myself in a {1}public{7} space."

translate id _call_phone_end_kil_9_d1d323ee:
    "Ian and Hana each quickly knocked back a shot of gin with no hesitation."

translate id _call_phone_end_kil_9_d3280db0:
    kil "A few times, actually."

translate id _call_phone_end_kil_9_fbcfbcd0:
    hana "I did a drum solo topless one time."

translate id _call_phone_end_kil_9_a364215b:
    kil "Shit, for real? Haha!"

translate id _call_phone_end_kil_9_c13a5869:
    hana "It was our singers' idea."

translate id _call_phone_end_kil_9_51791c68:
    "To my surprise, Mina cocked her head back and slid the clear liquid down her throat."

translate id _call_phone_end_kil_9_92b003ea:
    mina "Eeugh, that's sharp!"

translate id _call_phone_end_kil_9_4148b87d:
    hana "Well, that was delayed."

translate id _call_phone_end_kil_9_04f8bcb2:
    mc "Eh? So, all of you then? Even you, Mina?"

translate id _call_phone_end_kil_9_e2e536d7:
    mina "Why do you single me out?"

translate id _call_phone_end_kil_9_8bf13dc7:
    mc "I'm just surprised is all."

translate id _call_phone_end_kil_9_dce00be1:
    kil "Me too. When did this happen?"

translate id _call_phone_end_kil_9_a28b1a5b:
    "Killian looked at Mina with an incredulous look in his eyes."

translate id _call_phone_end_kil_9_0d855311:
    "Mina uncharacteristically avoided making eye contact."

translate id _call_phone_end_kil_9_b2ed629d:
    mina "It's not what you're thinking... it's just..."

translate id _call_phone_end_kil_9_5459da17:
    mina "Some girls played a little joke on me one time by stealing my clothes when I was in the locker room shower."

translate id _call_phone_end_kil_9_107b48e0:
    mc "That doesn't sound like a joke."

translate id _call_phone_end_kil_9_a555c860:
    mina "No, it wasn't very funny... I had to wait until it got late and then run back to my dorm room naked."

translate id _call_phone_end_kil_9_1693a193:
    kil "When was this?"

translate id _call_phone_end_kil_9_f0446e6b:
    mina "When I was in boarding school... it was just a harmless prank though..."

translate id _call_phone_end_kil_9_8c51f268:
    hana "It sounds like a bunch of cunts."

translate id _call_phone_end_kil_9_1a3e9450:
    mina "No, it was just some girls having fun. Can we move on please?"

translate id _call_phone_end_kil_9_4c4503b8:
    "Mina squirmed uncomfortably in her seat and pointedly asked Hana to take her turn."

translate id _call_phone_end_kil_9_0428b28d:
    "The room shared a collective awkward silence, before the rocker girl had the decency to spare us."

translate id _call_phone_end_kil_9_7fe1d6a5:
    hana "Okay, yeah... my turn..."

translate id _call_phone_end_kil_9_5279de63:
    mc "Never have I ever used a vacuum cleaner on my junk."

translate id _call_phone_end_kil_9_8042fba0:
    kil "Oh, fuck you!"

translate id _call_phone_end_kil_9_72428b33:
    hana "Euh? Whaaaaat? You haven't..."

translate id _call_phone_end_kil_9_87d3b1d8:
    mina "Why am I not surprised? That SHOULD surprise me, but it doesn't..."

translate id _call_phone_end_kil_9_12cd7267:
    ham "Pfft... hahahaha...."

translate id _call_phone_end_kil_9_546e95dc:
    hana "Hahaha..."

translate id _call_phone_end_kil_9_6c7ca57b:
    mina "Hehehe..."

translate id _call_phone_end_kil_9_0b6b28d0:
    ham "Bahahaha-!"

translate id _call_phone_end_kil_9_14e14c36:
    kil "Okay, now you guys are being absurd."

translate id _call_phone_end_kil_9_f340a8d4:
    "The girls shared a hearty laugh at Ian's expense, to a degree that almost made me feel bad for Ian."

translate id _call_phone_end_kil_9_3839de24:
    "To his credit, he took his lump respectably."

translate id _call_phone_end_kil_9_4807e261:
    kil "Picking questions you know the answer to isn't really in the spirit of the game!"

translate id _call_phone_end_kil_9_74912524:
    hana "You've crammed your cock into a vacuum cleaner before?"

translate id _call_phone_end_kil_9_b36f993c:
    kil "I was curious! It, uh... wasn't for me."

translate id _call_phone_end_kil_9_ff8854a4:
    ham "Pft... hahahaha!"

translate id _call_phone_end_kil_9_1b2dc789:
    kil "It's your turn, Hana!"

translate id _call_phone_end_kil_9_f8ee7425:
    "Ian more or less begged the rocker girl to spare him from their laughter."

translate id _call_phone_end_kil_9_5b93fcb6:
    hana "Hahahaha...! F-Fine! Sure!"

translate id _call_phone_end_kil_9_0a18bb5e:
    hana "Hmm..."

translate id _call_phone_end_kil_9_32409458:
    hana "Never have I ever..."

translate id _call_phone_end_kil_9_68103ca0:
    hana "Never have I ever had sex while other people were in the room."

translate id _call_phone_end_kil_9_6b61a4b7:
    "Hana delivered her move with a devious smile, knowing full well that Ian and I fit the criteria if only for the club alone. I wasn't sure if she was playing to win or if she was just trying to make us squirm."

translate id _call_phone_end_kil_9_6f7b64ea:
    "He and I shared a sly look, before a slight roll of the shoulders signaled to me to go ahead and drink."

translate id _call_phone_end_kil_9_2b35b6fe:
    mc "..."

translate id _call_phone_end_kil_9_a53f69be:
    mina "What?!"

translate id _call_phone_end_kil_9_85c1da3b:
    hana "Oh, really? Both of you? That's interesting. You guys a pair of perverts or something?"

translate id _call_phone_end_kil_9_5e42f4d9:
    mina "How could either of you do something like that? That's... mortifying!"

translate id _call_phone_end_kil_9_622573e2:
    kil "I don't know about [2], but that's part of the thrill."

translate id _call_phone_end_kil_9_94c41850:
    hana "Your boyfriend is an exhibitionist."

translate id _call_phone_end_kil_9_4de25f79:
    mina "What about you, [2]?"

translate id _call_phone_end_kil_9_94a9f20b:
    mc "Well, you know... college dorm rooms can be small."

translate id _call_phone_end_kil_9_3d690fc9:
    "I lied. I had never stayed in a college dorm, but it's not like I could just say I got my dick sucked in front of a dozen rich perverts just yesterday."

translate id _call_phone_end_kil_9_76a98223:
    mina "Eeugh, I don't know how you could do that!"

translate id _call_phone_end_kil_9_067c38aa:
    kil "Want to try it some time?"

translate id _call_phone_end_kil_9_0eacd608:
    mina "No!"

translate id _call_phone_end_kil_9_8350fdca:
    hana "It's your go, Goldilocks."

translate id _call_phone_end_kil_9_fe4b6dc6:
    mina "Okay, okay... let me think..."

translate id _call_phone_end_kil_9_bd955b77:
    mct "(Eh? Goldilocks...?)"

translate id _call_phone_end_kil_9_71fb94ec:
    mina "{5}Geez, I still can't believe you two...{6}"

translate id _call_phone_end_kil_9_ddf2d5ac:
    mina "Okay! Never have I ever shoplifted."

translate id _call_phone_end_kil_9_b58dbdc4:
    "The three of us downed our drinks at about the same time."

translate id _call_phone_end_kil_9_1d2d8258:
    mina "Hehehe, yes! Three pointer!"

translate id _call_phone_end_kil_9_3e436ece:
    kil "You're actually into this?"

translate id _call_phone_end_kil_9_311615a2:
    mina "All I'm going to say is get ready to bow down to your new queen, because at the end of the game that's exactly what you're going to do."

translate id _call_phone_end_kil_9_2c8a2374:
    "Mina proudly puffed out her chest in an exaggerated display."

translate id _call_phone_end_kil_9_efa2a8b9:
    kil "This is only round one, babe. Don't count your chickens before they hatch."

translate id _call_phone_end_kil_9_f1b31ba8:
    hana " What are you, a country bumpkin?"

translate id _call_phone_end_kil_9_14de147f:
    kil "It's a saying!"

translate id _call_phone_end_kil_9_de35c655:
    hana "Yeah, one I'd not expect to come from your lips."

translate id _call_phone_end_kil_9_eec9202a:
    kil "Ah, whatever. It's my turn. I've got couple questions in mind, but the question is which one...?"

translate id _call_phone_end_kil_9_7c6bac26:
    mc "You don't have to make this complicated."

translate id _call_phone_end_kil_9_8e1f5109:
    kil "Alright. Here's a three pointer for you... never have I ever run from a cop."

translate id _call_phone_end_kil_9_0155ae29:
    mc "That's a surprisingly clean one..."

translate id _call_phone_end_kil_9_6afe0cd0:
    kil "Just a lil' icebreaker. Now, drink up you three."

translate id _call_phone_end_kil_9_6961dab1:
    "To my surprise, I wasn't the only one to raise a drink."

translate id _call_phone_end_kil_9_bf77c6b3:
    kil "Ah, damn. I knew I'd get [2] and Mina, but I took a gamble on you."

translate id _call_phone_end_kil_9_0e7d4700:
    hana "Hell, no. What do you think I get up to?"

translate id _call_phone_end_kil_9_0823264e:
    hana "I mean, why tack an extra misdemeanor onto a drunk and disorderly? That's just stupid."

translate id _call_phone_end_kil_9_a58e3a81:
    mc "{1}You've{2} run from the cops?"

translate id _call_phone_end_kil_9_d5aa2ac6:
    mina "K-kinda, heh..."

translate id _call_phone_end_kil_9_9d3bfc53:
    kil "It's a great story. You should tell it."

translate id _call_phone_end_kil_9_4e0270ea:
    mc "Yeah, I think I might need to hear this."

translate id _call_phone_end_kil_9_8fa571b8:
    mina "It's not a big deal. It was just a little misunderstanding..."

translate id _call_phone_end_kil_9_a8a37c30:
    mina "Ten months ago, just a little bit after Ian and I had started dating, we were standing on the corner of Oak and Old Row."

translate id _call_phone_end_kil_9_aed6c040:
    mina "Like... forty minutes earlier, I had just smoked a joint for the first time in my life."

translate id _call_phone_end_kil_9_961f4287:
    kil "Let me clarify: she had like two hits before she was all \"woaaaah\" and \"I feel weeeeird\"."

translate id _call_phone_end_kil_9_3e54d50c:
    mina "Eheh, yeah... anyway, we were standing on the corner of Oak and Old Row and I spotted a police officer coming our way."

translate id _call_phone_end_kil_9_376a45af:
    kil "He wasn't coming {3}our{4} way. He was just giving the buskers a hard time."

translate id _call_phone_end_kil_9_26cfe3fa:
    mina "Am I telling the story or are you?!"

translate id _call_phone_end_kil_9_4b244901:
    kil "Sorry. I'll let you tell it."

translate id _call_phone_end_kil_9_243121c9:
    mina "Thank you! So, I may have gotten the {1}teensiest{2} bit nervous. Like, I couldn't take my eyes off him."

translate id _call_phone_end_kil_9_2cf0b829:
    mina "I watched him walk past that retro candy-shop. Then that mexican place..."

translate id _call_phone_end_kil_9_1b82aa89:
    mina "He got close enough to make eye contact and give me a funny look."

translate id _call_phone_end_kil_9_678b7002:
    kil "It was a friendly nod."

translate id _call_phone_end_kil_9_2a816823:
    mina "So I just..."

translate id _call_phone_end_kil_9_ab581453:
    mina "Went in the opposite direction."

translate id _call_phone_end_kil_9_ebda67c6:
    kil "No. It was more like she hauled ass. She lost a heel."

translate id _call_phone_end_kil_9_ced37fc9:
    hana "Pfft,ha-wa-wait, the state decriminalized pot like... a couple of years ago!"

translate id _call_phone_end_kil_9_aeab8d16:
    kil "Yeah. I know! That's what made it so fucking funny. After Mina's little disappearance trick, the cop came over looking pretty confused. I told him she was just a little high and we had a good laugh about it."

translate id _call_phone_end_kil_9_efd10ccc:
    mina "I didn't know! It's not like I get a drug newsletter!"

translate id _call_phone_end_kil_9_54fc7f56:
    mina "What about you, [2]? What's your story?"

translate id _call_phone_end_kil_9_6a4d4af9:
    mc "Oh, it wasn't a big deal. As usual, Ian and I were kids. I think we were breaking bottles on the sidewalk or something stupid like that."

translate id _call_phone_end_kil_9_75fd40f4:
    mc "Cop stopped by and I took off. Ian froze in place."

translate id _call_phone_end_kil_9_82bd83f2:
    mina "Hehe, we have that in common."

translate id _call_phone_end_kil_9_6314a6b3:
    kil "Okay, so the tally for round one..."

translate id _call_phone_end_kil_9_a5ccc350:
    kil "3 for [2], 2 for Hana, 3 for Mina, and 2 for me."

translate id _call_phone_end_kil_9_ca1f9114:
    mina "Yes! Tied for first isn't bad."

translate id _call_phone_end_kil_9_0430636c:
    kil "1 for [2], 2 for Hana, 3 for Mina, and 2 for me."

translate id _call_phone_end_kil_9_06f5d5ea:
    mina "Yes!"

translate id _call_phone_end_kil_9_21c02b40:
    mina "Hey, where you going? I'm winning!"

translate id _call_phone_end_kil_9_fa29e001:
    kil "Relax, I'm just draining the lizard. I'll be RIGHT back and then we can get back to it, your majesty."

translate id _call_phone_end_kil_9_a20cefa7_8:
    "..."

translate id _call_phone_end_kil_9_f7c8d13b:
    "Ian's departure reset the conversation and a brief silence fell over the room."

translate id _call_phone_end_kil_9_89bb60f7:
    hana "So... you and Ian have been dating for ten months?"

translate id _call_phone_end_kil_9_dd4caefa:
    mina "Closer to eleven I guess. You work at Ian's uncle's place, right?"

translate id _call_phone_end_kil_9_f0a8be9f:
    "Before answering, Hana gave me a questioning look on the sly. I guess she was unclear on how much Mina knew."

translate id _call_phone_end_kil_9_abaec319:
    "To which, like some doofus, I kicked her foot in confirmation."

translate id _call_phone_end_kil_9_d3c9918b:
    hana "Yeah, that's right."

translate id _call_phone_end_kil_9_7a3c5818:
    mina "How long have you two known each other?"

translate id _call_phone_end_kil_9_535c3f50:
    hana "I've worked there a little over a year now."

translate id _call_phone_end_kil_9_1617522e:
    mina "Hmm..."

translate id _call_phone_end_kil_9_34ddbf93:
    "Mina took time to slowly give Hana a once-over before speaking again."

translate id _call_phone_end_kil_9_e5024284:
    mina "You two must be pretty good friends then."

translate id _call_phone_end_kil_9_56978409:
    hana "Eh, him and I don't exactly gel. It's a more {3}cordial{4} thing."

translate id _call_phone_end_kil_9_765b2c57:
    "Her choice of words had me scoffing inwardly."

translate id _call_phone_end_kil_9_a3cb4c5d:
    mina "Ha! That's a polite no if I've ever heard one."

translate id _call_phone_end_kil_9_615ade9b:
    hana "What do you do, Mina?"

translate id _call_phone_end_kil_9_91e6b851:
    mina "Oh, I'm just a student..."

translate id _call_phone_end_kil_9_03577b18:
    mc "Mina's a model and actress."

translate id _call_phone_end_kil_9_f42778ca:
    mc "She actually just landed a big gig on television."

translate id _call_phone_end_kil_9_158ff3b3:
    mina "It's not {1}BIG{2}."

translate id _call_phone_end_kil_9_5d0d3711:
    mc "It's a speaking part with a storyline isn't it?"

translate id _call_phone_end_kil_9_061f7553:
    mina "Yeah, still..."

translate id _call_phone_end_kil_9_65539f81:
    "Mina seemed unusually shy for whatever reason."

translate id _call_phone_end_kil_9_bbc2ded5:
    hana "Really? Have I seen anything you've done?"

translate id _call_phone_end_kil_9_2d67d0e1:
    mina "Well... uh, I recently got a tv-part."

translate id _call_phone_end_kil_9_86f3bd2d:
    hana "That's cool. Is it a show I know?"

translate id _call_phone_end_kil_9_1d950f8d:
    mina "Probably not... it's just a crappy soap opera. It's called {3}Loving Days in Lafayette{4}."

translate id _call_phone_end_kil_9_54188da3:
    hana "It sounds like a big deal to me. You should own that shit."

translate id _call_phone_end_kil_9_9fb9fbb9:
    mina "Eheh, you're right! It's the biggest role I've gotten! I'm really, really excited!"

translate id _call_phone_end_kil_9_a54c7ced:
    hana "That's better."

translate id _call_phone_end_kil_9_99cbd058:
    mina "Oh! Could I ask a {1}big{2} favor of you, [2]?"

translate id _call_phone_end_kil_9_def0ad19:
    mc "What is it?"

translate id _call_phone_end_kil_9_8322e959:
    mina "Would you... if you have the time... help do some line-readings tomorrow?"

translate id _call_phone_end_kil_9_e317c8d3:
    mina "I'd ask Ian, but he never takes anything like that seriously... plus given the kind of scene it is, he'll probably just try and get into my pants mid-scene."

translate id _call_phone_end_kil_9_3fbd4e76:
    "Mina looked at me expectantly, with a shy, hopeful smile. It really didn't leave me any choice."

translate id _call_phone_end_kil_9_80d526a6:
    mc "Sure, but I'm not an actor or anything... I don't know what good I'll be."

translate id _call_phone_end_kil_9_ad92080b:
    mina "Oh, you'll be plenty! All you have to do is read from a script!"

translate id _call_phone_end_kil_9_2e9e421c:
    mc "Reading from a piece of paper is one of my greatest abilities. Sure, I'd love to help."

translate id _call_phone_end_kil_9_4d56256f:
    mina "Thank you!"

translate id _call_phone_end_kil_9_13d5948a:
    hana "Eh...? G-get off of..."

translate id _call_phone_end_kil_9_38bc71a6:
    "Mina practically leapt over Hana to give me something resembling a hug."

translate id _call_phone_end_kil_9_a1f87b4e:
    mina "I'm SO glad you said yes. You're my good luck charm!"

translate id _call_phone_end_kil_9_55fa766b:
    mc "...good luck charm?"

translate id _call_phone_end_kil_9_4de72bf5:
    mina "Well, you did help me prepare for the audition and I got the part!"

translate id _call_phone_end_kil_9_44d29293:
    "Hana looked at Mina dubiously, but said nothing."

translate id _call_phone_end_kil_9_ec470a27:
    mina "Actors and actresses are superstitious, okay?!"

translate id _call_phone_end_kil_9_57f7ef3a:
    hana "I didn't say anything."

translate id _call_phone_end_kil_9_6e59467b:
    mina "So..."

translate id _call_phone_end_kil_9_6de59714:
    "Looking for a change of topic, Mina searched for a topic."

translate id _call_phone_end_kil_9_2f8c6010:
    mina "When did you get your first tattoo?"

translate id _call_phone_end_kil_9_bad130d0:
    hana "The day I turned eighteen."

translate id _call_phone_end_kil_9_251225a8:
    mina "That's so neat. I've always wanted one, but I'm a big wuss... did it hurt?"

translate id _call_phone_end_kil_9_c50127eb:
    hana "Some more than others. It just depends on the location."

translate id _call_phone_end_kil_9_a20cefa7_9:
    "..."

translate id _call_phone_end_kil_9_dd4793fd:
    "The two quickly found themselves excitedly talking amongst themselves, like I wasn't there."

translate id _call_phone_end_kil_9_51500d70:
    "Not that I minded. Watching the two carry on like friends filled me with warmth."

translate id _call_phone_end_kil_9_08c8e88c:
    kil "Sorry that took so long. My uncle called."

translate id _call_phone_end_kil_9_3fb52f6d:
    mc "No biggie. Ready to get back to the game?"

translate id _call_phone_end_kil_9_ef68a68f:
    mina "Yep!"

translate id _call_phone_end_kil_9_fa578f40:
    mc "Alright, time to get things rolling again. Hmm..."

translate id _call_phone_end_kil_9_168699c0:
    mct "(What should I pick here?)"

translate id _call_phone_end_kil_9_83e90dc4:
    mc "Never have I ever gone down on a person of the same sex."

translate id _call_phone_end_kil_9_b5efa79d:
    mc "Never have I ever... worn a pair of handcuffs."

translate id _call_phone_end_kil_9_115f170c:
    mina "For... ANY reason?"

translate id _call_phone_end_kil_9_28ba79cd:
    mc "That's right."

translate id _call_phone_end_kil_9_1044895c:
    mina "Uggg..."

translate id _call_phone_end_kil_9_159997fc:
    "After some thought, Mina quickly downed her shot of gin."

translate id _call_phone_end_kil_9_21cdb5a6:
    kil "Eh? You've never been arrested?"

translate id _call_phone_end_kil_9_4ca9ff21:
    mina "It's not that."

translate id _call_phone_end_kil_9_946403bf:
    kil "Nor have we ever done that in the..."

translate id _call_phone_end_kil_9_78ae95cb:
    mina "It's not that either! Yeesh..."

translate id _call_phone_end_kil_9_4d0cbf12:
    kil "What is \"it\" then?"

translate id _call_phone_end_kil_9_661ce611:
    mina "I don't want to say."

translate id _call_phone_end_kil_9_6fab769e:
    hana "Why not?"

translate id _call_phone_end_kil_9_224af216:
    mina "It's too embarrassing."

translate id _call_phone_end_kil_9_c1e2785e:
    mc "Alright, no one's gonna force you to tell it. It's your go, Hana."

translate id _call_phone_end_kil_9_2cb6a677:
    hana "Okay... hm, never have I ever..."

translate id _call_phone_end_kil_9_5477e57a:
    mina "W-wait! Don't just move on..."

translate id _call_phone_end_kil_9_1fb5a661:
    mc "You just said it was too embarrassing."

translate id _call_phone_end_kil_9_d56ebf2a:
    mina "Mmh... yeah, but..."

translate id _call_phone_end_kil_9_cb5c78f2:
    hana "But what...?"

translate id _call_phone_end_kil_9_5b287655:
    mina "I don't want you to think I'm lame."

translate id _call_phone_end_kil_9_b026b239:
    "All of us collectively raised our eyebrows for a moment."

translate id _call_phone_end_kil_9_7a3b91d6:
    mina "Just give me a moment..."

translate id _call_phone_end_kil_9_278c7e29:
    "........."

translate id _call_phone_end_kil_9_0fd4d656_4:
    "......"

translate id _call_phone_end_kil_9_a20cefa7_10:
    "..."

translate id _call_phone_end_kil_9_516d9b88:
    mina "A couple months ago, I was sleeping over at a friend's house. Her husband was out of town, so we were having a girl's night in."

translate id _call_phone_end_kil_9_7f610b51:
    mina "Her husband is like a HUGE jerk, gets massively crabby about even the smallest disturbance. So this was actually the first time I'd ever been over to her place."

translate id _call_phone_end_kil_9_85e7bb4d:
    mina "This friend... even though I don't exactly agree with how she lives her life... I still look up to her. She's got a ton of life experience, on top of being beautiful, sophisticated, and adventurous. She's done things I've only dreamed of."

translate id _call_phone_end_kil_9_8559ac92:
    mina "Which is why, when she left me alone in her apartment to run out to the corner store and get us some cheap wine, my curiosity got the better of me and I decided to take a look around."

translate id _call_phone_end_kil_9_85d1f6ec:
    mina "I don't mean like invade-a-person's privacy look around, I mean just a look on what was out in the open."

translate id _call_phone_end_kil_9_9af0cf93:
    mina "It can be fun. You learn a lot about a person from just the stuff they leave lying around."

translate id _call_phone_end_kil_9_5eae88ed:
    kil "You're always just hoping to find someone as messy and untidy as you."

translate id _call_phone_end_kil_9_afd2af4e:
    mina "I'm not messy! You're just a neat freak! Back me up, [2]."

translate id _call_phone_end_kil_9_01e26b68:
    mc "Eh...? Oh, yeah. Ian's always been fussy about cleanliness and putting things away."

translate id _call_phone_end_kil_9_00d146c9:
    kil "Just move on with the story."

translate id _call_phone_end_kil_9_91be03be:
    mina "Ehehe, okay... so anyway, I took a peek around her apartment and it wasn't long before I found something that piqued my interest."

translate id _call_phone_end_kil_9_e84c2449:
    mina "Just lying on the bar was a set of handcuffs. Not some fuzzy pink, for-her-comfort handcuffs. These were like a cop's set."

translate id _call_phone_end_kil_9_9d57ad5a:
    mina "Which... was a lot more appealing."

translate id _call_phone_end_kil_9_205e7cb0:
    hana "Appealing...?"

translate id _call_phone_end_kil_9_e2ab63d3:
    mina "I got... {3}curious{4}."

translate id _call_phone_end_kil_9_513a878f:
    mina "I'd never worn any, you see..."

translate id _call_phone_end_kil_9_d9da9e34:
    kil "You tried them on?"

translate id _call_phone_end_kil_9_4e2a8876:
    mina "..."

translate id _call_phone_end_kil_9_b1093798:
    mina "Yes."

translate id _call_phone_end_kil_9_22fdde61:
    mina "Some of my girlfriends talked like it was fun and I wanted to see what it felt like."

translate id _call_phone_end_kil_9_acc0c3e2:
    kil "I didn't know you were interested in that sort of thing."

translate id _call_phone_end_kil_9_c3c1c2a5:
    mina "I'm not! I was... {1}just being stupid{2}."

translate id _call_phone_end_kil_9_e845fe3d:
    "It might have just been wishful thinking, but she sounded pretty unconvincing in refuting Ian's conclusion."

translate id _call_phone_end_kil_9_218b12d6:
    mina "{5}So, anyway I left the key on the bar...{6}"

translate id _call_phone_end_kil_9_e6daf60b:
    hana "What was that?"

translate id _call_phone_end_kil_9_826a244a:
    mina "I said it really was stupid... because..."

translate id _call_phone_end_kil_9_4aac1ea8:
    mina "I handcuffed myself to a nearby railing and.."

translate id _call_phone_end_kil_9_848b125e:
    mina "I-left-the-{17}KEY-ON-THE{18}{20}BAR!{21}"

translate id _call_phone_end_kil_9_ee5e696e:
    mina "Ehehe..."

translate id _call_phone_end_kil_9_5003b59f:
    "Mina's face turned a beet red with her admission."

translate id _call_phone_end_kil_9_f0f9d1f3:
    mina "It's not like I'm a total moron. I didn't just see a pair of handcuffs and handcuff myself without thinking about how to unlock it, okay? I confirmed the key worked first, but then I just..."

translate id _call_phone_end_kil_9_da1b2558:
    mina "Forgot it on the table."

translate id _call_phone_end_kil_9_96f7425d:
    mc "Well, no harm, right? It sounded like your friend would be back pretty soon."

translate id _call_phone_end_kil_9_36af5a8f:
    mina "Ahahaha, no. Unfortunately, that's the punchline to all of this..."

translate id _call_phone_end_kil_9_3eb16080:
    mina "Grr... eh...!"

translate id _call_phone_end_kil_9_281ac602:
    mina "So... close...! C'mon...! C'mon...!"

translate id _call_phone_end_kil_9_823d03af:
    mina "Eeeeh....! You're so,so stupid, Mina!"

translate id _call_phone_end_kil_9_a20cefa7_11:
    "..."

translate id _call_phone_end_kil_9_2bfee0bb:
    mina "F-Felish...! You're back! I could use some..."

translate id _call_phone_end_kil_9_bdf58af4:
    elias "..."

translate id _call_phone_end_kil_9_4e096f0e:
    elias "You must be one of Felicia's {3}friends{4}."

translate id _call_phone_end_kil_9_ccb33e04:
    mina "Uh, um... I can explain..."

translate id _call_phone_end_kil_9_6d19faca:
    hana "I thought he was out of town!"

translate id _call_phone_end_kil_9_748ecbc3:
    mina "He was supposed to be, but his plans unexpectedly changed..."

translate id _call_phone_end_kil_9_dc6924e3:
    mc "Did he uncuff you at least?"

translate id _call_phone_end_kil_9_3048f401:
    mina "No. He just gave me a stern look and went to his office..."

translate id _call_phone_end_kil_9_c56d6981:
    hana "Ahahaha, no he didn't! That's hilarious!"

translate id _call_phone_end_kil_9_eae956da:
    mina "He did. He really, REALLY did."

translate id _call_phone_end_kil_9_097bcba3:
    hana "Did you figure out if you liked it at least?"

translate id _call_phone_end_kil_9_954f60a0:
    mina "No... I was too busy panicking..."

translate id _call_phone_end_kil_9_36af4c73:
    hana "You got a pair of handcuffs lying around, [2]?"

translate id _call_phone_end_kil_9_98a724d4:
    mina "S-shut up...!"

translate id _call_phone_end_kil_9_5020937d:
    mc "You're up, Hana."

translate id _call_phone_end_kil_9_5643ec8a:
    hana "I don't know how I'll follow that, but let's see..."

translate id w2HanaLezStory_8740700c:
    "No one immediately raised their glass, which wasn't a surprise. It was a Hail Mary type question."

translate id w2HanaLezStory_4a3e3272:
    hana "......"

translate id w2HanaLezStory_81d1cf98:
    hana "..."

translate id w2HanaLezStory_eb38d17a:
    "Which is why I felt a smile stretch across my face when Hana finally took a drink."

translate id w2HanaLezStory_0953f647:
    hana "Just me, huh?"

translate id w2HanaLezStory_dba96607:
    kil "You know, I always kind of thought you swung that way."

translate id w2HanaLezStory_3472ff82:
    hana "What? Because I don't swoon over you? Get bent, you jackass. I'm not gay."

translate id w2HanaLezStory_62691cc6:
    hana "I wasn't even testing the waters. I've always been fairly confident I loved dick."

translate id w2HanaLezStory_38cb0e88:
    hana "It was just a one time thing, in a... very unique circumstance."

translate id w2HanaLezStory_5a5671e6:
    kil "Now you've got to go into detail."

translate id w2HanaLezStory_3e737514:
    mina "I kinda want to hear about it too..."

translate id w2HanaLezStory_93f0deb3:
    "I could tell by the way her eyes were locked on Hana that there was no \"kinda\" about it."

translate id w2HanaLezStory_c435a3c0:
    hana "Sure, it's not like I'm embarrassed about it."

translate id w2HanaLezStory_a6f51595:
    mc "Even if you were, that is the name of the game."

translate id w2HanaLezStory_1b684c69:
    mina "Y-yeah, spill it."

translate id w2HanaLezStory_7d4db06e:
    hana "Alright, this was around two and a half years ago. I had just turned twenty and my best friend Jerrica got us tickets to see Blackwing Eternal, eh--"

translate id w2HanaLezStory_06eccd8e:
    hana "You ever heard of them? They're like my fourth favorite band of all time."

translate id w2HanaLezStory_739ef6e4:
    "We collectively shook our heads no."

translate id w2HanaLezStory_6fb44205:
    hana "They're a groove metal band. Think like Pantera or White Zombie."

translate id w2HanaLezStory_b88c3cff:
    "That didn't wipe the blank look off our faces."

translate id w2HanaLezStory_4579981b:
    hana "Ah, jeez. You guys are lame as fuck."

translate id w2HanaLezStory_ef3da938:
    mina "Come on, tell the story..."

translate id w2HanaLezStory_5ac81b33:
    kil "Get to the good stuff."

translate id w2HanaLezStory_80ae9862:
    hana "Okay, right. So like I said: it was not only my birthday, but this was the first time I had the chance to see them play live."

translate id w2HanaLezStory_cb143014:
    hana "If you ask me, even for the shittiest of bands, nothing really beats live music and Blackwing Eternal..."

translate id w2HanaLezStory_00b005ba:
    hana "Well, they're kickass. So, it was a fucking amazing time. That was even before I got to meet the band."

translate id w2HanaLezStory_7669175a:
    kil "I see where this is going. You got starstruck?"

translate id w2HanaLezStory_c0958121:
    hana "More than that. I practically creamed my pants when Jerrica told me her boyfriend's cousin was concert staff and could get us backstage for fifty bucks."

translate id w2HanaLezStory_a7ce7de1:
    hana "Flash forward, I got to meet the lead singer: {1}Zara Tessman{2}. She's a damn icon, one of the baddest bitches in metal and thankfully it wasn't a \"don't meet your heroes\" type deal."

translate id w2HanaLezStory_0c04d329:
    hana "She was super cool, and even though I acted like an annoying gaga-eyed fangirl, she invited us to the after party."

translate id w2HanaLezStory_beaff407:
    hana "We hit a couple of nearby watering holes, and a few hours later, I found myself back at her place. It was under the pretense of checking out her album collection, but I could read the air..."

translate id w2HanaLezStory_01b4708d:
    hana "With the way she sidled up close to me at the bar, the way she touched my shoulder when she got to the punchline of a joke, and the way she would casually brush stray hairs out of my face..."

translate id w2HanaLezStory_0ff1b39d:
    hana "She was expecting something for giving a stupid girl like me the time of night."

translate id w2HanaLezStory_ba1f348f:
    hana "When she told me..."

translate id w2HanaLezStory_d21cc7f6:
    zara "Get your tits out."

translate id w2HanaLezStory_70c337e5:
    hana "In a cocky voice that would've pissed me off 99.9 percent of the time..."

translate id w2HanaLezStory_341a3c41:
    mina "You didn't...?"

translate id w2HanaLezStory_cd6cddd4:
    hana "I did."

translate id w2HanaLezStory_15b43b85:
    hana "I knew she saw me as just another fame-stricken floozy to add as a notch in her bedpost."

translate id w2HanaLezStory_fa61b0f7:
    hana "You see it at every concert, countless women throwing themselves at gussied-up pretty boys with the charisma of rocks. Until that night, I could never wrap my head around it."

translate id w2HanaLezStory_dd786306:
    mina "So, why did you? Especially if you're not even really into girls..."

translate id w2HanaLezStory_a0aa3b2e:
    hana "Because, at that precise moment, I WAS a fame-stricken floozy."

translate id w2HanaLezStory_81ca0546:
    hana "She was fucking Zara Tessman. She was everything I wanted to be. She was so cool, so mature..."

translate id w2HanaLezStory_c7b5ee2d:
    mina "How was it?!"

translate id w2HanaLezStory_878b869f:
    mina "What happened next?"

translate id w2HanaLezStory_2908df04:
    hana "Oh? You seem awfully interested, Mina,"

translate id w2HanaLezStory_d8520c9b:
    mina "It's just you told such a detailed story... I'm just curious with how the rest went..."

translate id w2HanaLezStory_d8cf34ad:
    hana "Is that right? Well, since we're friends and all..."

translate id w2HanaLezStory_9e745f0e:
    hana "*Gluhg*"

translate id w2HanaLezStory_406581d4:
    hana "You want the grisly details?"

translate id w2HanaLezStory_196e3cc0:
    mina "Uh-huh..."

translate id w2HanaLezStory_bcd30054:
    hana "It started out with plenty of foreplay. I told her I'd never been with a chick before, so she wanted to ease me into things slowly."

translate id w2HanaLezStory_f18a3f88:
    hana "And shit, by the agonizing end of it all, I was begging her to move on. "

translate id w2HanaLezStory_0b855850:
    mina "Really?"

translate id w2HanaLezStory_99cf8229:
    hana "Oh, yeah. Goldilocks. Just picture it..."

translate id w2HanaLezStory_282439c8:
    hana "She got to know every inch of my body, exploring and caressing with both her hands and mouth."

translate id w2HanaLezStory_d5cdd1ed:
    hana "It was eye-opening, the way she used her fingers."

translate id w2HanaLezStory_cec52ef1:
    hana "Not just my breasts either, but parts of my body I'd never derived pleasure from."

translate id w2HanaLezStory_45c8a15b:
    hana "You ever realize your inner wrist is an erogenous zone?"

translate id w2HanaLezStory_da4937b3:
    mina "What? No it isn't!"

translate id w2HanaLezStory_175bba9f:
    hana "It is. The scalp too."

translate id w2HanaLezStory_273aa608:
    hana "The earlobe..."

translate id w2HanaLezStory_811bf828:
    mina "J-jeez!"

translate id w2HanaLezStory_7161819a:
    hana "Yep. She had her way with me."

translate id w2HanaLezStory_55cbb360:
    hana "At the height of it all, that twisted bitch had me calling her mommy and begging to cum."

translate id w2HanaLezStory_d9166f69:
    mina "Nu-uh? That's..."

translate id w2HanaLezStory_5926a229:
    hana "Oooh, lemme cum! Ah~♥! P-please!"

translate id w2HanaLezStory_30f908e6:
    "Hana looked like she was having the time of her life relaying the story to Mina. In fact, I'm pretty sure she had forgotten Ian and I were even here..."

translate id w2HanaLezStory_113ceee7:
    "Mina hung off of every word, with a noticeable blush on her cheeks."

translate id w2HanaLezStory_72ddfc7f:
    zara "Call me mommy, slut!"

translate id w2HanaLezStory_ecae2d6d:
    hana "M-mommy! P-please--"

translate id w2HanaLezStory_c243f085:
    hana "Aaaah--!!!!!!"

translate id w2HanaLezStory_02692b2c:
    mina "W-woah..."

translate id w2HanaLezStory_dd0e64b5:
    hana "That was just the start of the night."

translate id w2HanaLezStory_02692b2c_1:
    mina "W-woah..."

translate id w2HanaLezStory_a2f190d5:
    hana "Pfhahaha, you should see your face, Goldilocks."

translate id w2HanaLezStory_2a2700a9:
    mina "How did the rest of the night go?"

translate id w2HanaLezStory_4ecaa62c:
    "Despite the teasing, Mina's interest in Hana's story was unabated."

translate id w2HanaLezStory_0bfeee76:
    hana "The rest of the night...?"

translate id w2HanaLezStory_5ae4aab5:
    "Mina gave a wordless nod to the question."

translate id w2HanaLezStory_0a18bb5e:
    hana "Hmm..."

translate id w2HanaLezStory_7bb7d431:
    mina "W-what are you doing?"

translate id w2HanaLezStory_16dabaac:
    hana "First she had me eat out her cunt for about half an hour. I had never seen a vagina that close up, but I didn't have time to take in the sights."

translate id w2HanaLezStory_ed1073aa:
    hana "She grabbed me by the head and put me to work. It being my first time, I was awful at it - not to mention, pretty tuckered out after being put through the paces, but I think she liked how inexperienced I was."

translate id w2HanaLezStory_2619cb39:
    hana "Pretty sure she got off on teaching straight women how to give oral or something stupid like that."

translate id w2HanaLezStory_85f42f3e:
    mina "That's so... {3}perverted{4}."

translate id w2HanaLezStory_8e24cdc3:
    hana "The last part of the night is rather difficult to describe."

translate id w2HanaLezStory_aee13d3c:
    mina "It is?"

translate id w2HanaLezStory_0c488285:
    hana "Yes, it's best I show you."

translate id w2HanaLezStory_20963096:
    mina "W-wait..!"

translate id w2HanaLezStory_8a5a755b:
    mina "Gh- what are you doing?"

translate id w2HanaLezStory_b25563e2:
    hana "Pfft-hold still, Goldilocks. Just a demonstration!"

translate id w2HanaLezStory_ea29ea20:
    mina "Eeeg, y-you're heavy! Get off of me?"

translate id w2HanaLezStory_0d2cb484:
    mc "Should we put a stop to this?"

translate id w2HanaLezStory_35c88361:
    kil "Fuck no."

translate id w2HanaLezStory_ec18ef33:
    hana "Just hold still for a minute!"

translate id w2HanaLezStory_a18d51c7:
    mina "Hey... don't grab...!"

translate id w2HanaLezStory_98f02051:
    hana "Aaaaaaand that's about how the night ended. It's not an easy position to describe, is it?"

translate id w2HanaLezStory_7d07c881:
    mina "No, it isn't..."

translate id w2HanaLezStory_299dc1fa:
    "Mina's growing blush had hit a fever pitch. She looked like she had just run a few miles."

translate id w2HanaLezStory_512dbba3:
    hana "To be frank, she taught me more about sex over a span of a few hours than I had previously learned in my short adult life."

translate id w2HanaLezStory_8e98b203:
    hana "Or did it feel more like fucking...?"

translate id w2HanaLezStory_3b2b225a:
    mina "W-wow...."

translate id w2HanaLezStory_8143a665:
    kil "Holy shit..."

translate id w2HanaLezStory_71c76031:
    mc "Don't you think you got carried away there?"

translate id w2HanaLezStory_5b518d54:
    hana "She's just so easy and fun to tease. I couldn't help myself."

translate id w2HanaLezStory_1b23d5fc:
    mina "Um, could you... let go of me?"

translate id w2HanaLezStory_7330e7b0:
    hana "Oh, yeah! Sorry!"

translate id w2HanaLezStory_a18efa20:
    hana "So, that's the story of the first and so far only time I've ever gotten down with a woman."

translate id w2HanaLezStory_faec2810:
    hana "I guess it's my turn now? Feels like I just talked forever."

translate id w2HanaLezStory_9a87b924:
    mc "Yep, you're up. I don't know if anyone will be able to follow a story like that though."

translate id w2HanaLezStory_40e96a5c:
    hana "Don't worry, I'll cook up something good..."

translate id w2BirthdayGameR2Continue_8c177b9e:
    hana "Never have I ever... had sex with someone old enough to be my parent."

translate id w2BirthdayGameR2Continue_33f1d38c:
    mct "(...she's going for direct hits.)"

translate id w2BirthdayGameR2Continue_3615fbd1:
    "Naturally, Ian and I knocked back our glasses."

translate id w2BirthdayGameR2Continue_9843663b:
    mina "What? Both of you, again?!"

translate id w2BirthdayGameR2Continue_5e1d62c6:
    mina "It wasn't the same person, was it?!"

translate id w2BirthdayGameR2Continue_88d4fbc0:
    mc "No... why would you think that?"

translate id w2BirthdayGameR2Continue_9f993b7e:
    mina "I mean... you two are... well, at least Ian is..."

translate id w2BirthdayGameR2Continue_3c1cf3a2:
    "Mina trailed off, unsure of how to put it."

translate id w2BirthdayGameR2Continue_55326b06:
    kil "Good enough friends that we wouldn't have a problem tag teaming the same chick?"

translate id w2BirthdayGameR2Continue_87cd978b:
    mina "No, I was NOT going to say THAT."

translate id w2BirthdayGameR2Continue_66dc95e6:
    kil "Hey, if you're ever interested in that sort of thing, I'm sure [2] wouldn't mind-"

translate id w2BirthdayGameR2Continue_37826691:
    mina "OHMIGOSH shut up! I would n- I would never--!"

translate id w2BirthdayGameR2Continue_f13cf811:
    mc "Ehe..."

translate id w2BirthdayGameR2Continue_cc6ac589:
    "Hana just sat back, chuckling to herself."

translate id w2BirthdayGameR2Continue_caf6ac39:
    mct "(That woman is evil...)"

translate id w2BirthdayGameR2Continue_f7948c9d:
    mina "Okay, whatever... you guys are animals! It's my turn...!"

translate id w2BirthdayGameR2Continue_dc9459a4:
    mina "Never have I ever..."

translate id w2BirthdayGameR2Continue_05d4e29a:
    "Mina glanced about the room, trying to grasp at something, anything that would move the game along."

translate id w2BirthdayGameR2Continue_1f314455:
    mina "Uh... oh, I know!"

translate id w2BirthdayGameR2Continue_a198541a:
    mina "Never have I ever had a crush on a friend's parent."

translate id w2BirthdayGameR2Continue_7c741ba0:
    kil "You're just piggybacking off of Hana's question!"

translate id w2BirthdayGameR2Continue_8245a7be:
    mina "So?"

translate id w2BirthdayGameR2Continue_792d6673:
    "As per our conversation in the sauna, Hana gave me a knowing look before she took a drink herself."

translate id w2BirthdayGameR2Continue_eec61b03:
    "Hana nonchalantly shrugged before taking a drink."

translate id w2BirthdayGameR2Continue_fccca68a:
    mina "Ooooh, you have a thing for older men, eh?"

translate id w2BirthdayGameR2Continue_e017c4ce:
    hana "Experience has its appeal."

translate id w2BirthdayGameR2Continue_596ece04:
    "Meanwhile, Killian and I shared a dubious look at one another."

translate id w2BirthdayGameR2Continue_28ddd699:
    "On my side of things, with all my teasing about Ian's mother over the years, it wasn't even an open secret."

translate id w2BirthdayGameR2Continue_a8bb8a2b:
    kil "Drink up, mother fucker."

translate id w2BirthdayGameR2Continue_3767f16b:
    mc "Nice choice of words."

translate id w2BirthdayGameR2Continue_bc594e9e:
    "I had no qualms in admitting it, and despite his reaction to my jokes, I knew he didn't have a problem with it either."

translate id w2BirthdayGameR2Continue_c55a7502:
    "His mother is an attractive woman and we went through puberty together. It was an understandable, innocent thing for a kid. At least in my view."

translate id w2BirthdayGameR2Continue_4427d59a:
    mina "Eh? You think Grace is...? She scares me..."

translate id w2BirthdayGameR2Continue_af774ef6:
    "I had always assumed the same was true of Ian, but it was something he ardently denied once before."

translate id w2BirthdayGameR2Continue_466fb515:
    "He does pretty much think of her as a second mother, so maybe that's truly the case. At any rate, I'm not gonna twist his arm about it over a drinking game."

translate id w2BirthdayGameR2Continue_7b4456b4:
    mina "Hehehe, anyway, another two points!"

translate id w2BirthdayGameR2Continue_9f5a27d4:
    mina "Your go Ian."

translate id w2BirthdayGameR2Continue_6fd9ddca:
    kil "Never have I ever had a sexual fantasy I was too embarrassed to admit."

translate id w2BirthdayGameR2Continue_707ffc29:
    mina "You SHOULD be embarrassed..."

translate id w2BirthdayGameR2Continue_f0699ddb:
    "I didn't have to think very hard about it, instead I just dutifully paid my penance."

translate id w2BirthdayGameR2Continue_1a59914c:
    "To my surprise, I was the only one to do so."

translate id w2BirthdayGameR2Continue_59bfc5b9:
    mc "Everyone's got at least one, right?"

translate id w2BirthdayGameR2Continue_e2c36eee:
    hana "Nope, I'm usually pretty above board about what I want."

translate id w2BirthdayGameR2Continue_3042d374:
    mina "I mean, I've got some stuff I'm curious about, but I wouldn't call any of them fantasies..."

translate id w2BirthdayGameR2Continue_52e698e2:
    "Pretty sure that qualifies, but I wasn't about to rules lawyer a friendly game."

translate id w2BirthdayGameR2Continue_3a2cea23:
    mc "Guess I'm the only one with a proper sense of shame..."

translate id w2BirthdayGameR2Continue_51c228b7:
    "That said, I did let a little passive aggressiveness slip out."

translate id w2BirthdayGameR2Continue_bdbb4f86:
    mina "Ooooh, you got to tell us!"

translate id w2BirthdayGameR2Continue_5e59557d:
    mc "If it's too embarrassing to admit to a sexual partner, why would I...?"

translate id w2BirthdayGameR2Continue_502f852d:
    hana "That's the game."

translate id w2BirthdayGameR2Continue_8836b02c:
    hana "Plus you never know, that information could be useful for later."

translate id w2BirthdayGameR2Continue_74e04c43:
    mct "(I suppose she has a point.)"

translate id w2BirthdayGameR2Continue_d5ae2301:
    mc "Fair enough."

translate id w2BirthdayGameR2Continue_51889a7d:
    mc "I've always kind of wanted to..."

translate id mod_BirthdayGame_1_98c132e4:
    mc "I've always kind of wanted to try breath play on a partner."

translate id mod_BirthdayGame_1_060aa96f:
    "I did my best to put it in the least embarrassing, most legitimate way possible."

translate id mod_BirthdayGame_1_5193f9be:
    mina "Breath play? What the heck does that mean?"

translate id mod_BirthdayGame_1_3d925f8b:
    "Mina eyeballed me suspiciously."

translate id mod_BirthdayGame_1_facff0bb:
    mina "It's not a weird internet thing, right? Because I learned about those balloon people the other day..."

translate id mod_BirthdayGame_1_1b4bf7ae:
    hana "He means he wants to choke the shit out of a chick during sex."

translate id mod_BirthdayGame_1_5fbff0c0:
    mina "Whaa...?"

translate id mod_BirthdayGame_1_89f80cfe:
    "Mina eyeballed me again, not sure of what to make of this information."

translate id mod_BirthdayGame_1_8ebfa9e9:
    mina "Why on earth would you want to do that?"

translate id mod_BirthdayGame_1_0593d6fe:
    mc "Ehehe... I don't know..."

translate id mod_BirthdayGame_1_aa58590e:
    "The question made me feel exceedingly self-conscious. Making sense of your sexual desires and putting it plainly into words for someone else to understand is a tall ask during a casual social situation."

translate id mod_BirthdayGame_1_a8931f54:
    hana "That's not weird at all, Goldilocks. In fact, it's common enough that it's borderline innocent."

translate id mod_BirthdayGame_1_e3b9d190:
    hana "You haven't ever wanted someone to smack your ass, pull your hair, or shove your face into a pillow?"

translate id mod_BirthdayGame_1_1ed832a7:
    mina "No...?"

translate id mod_BirthdayGame_1_e762e0cc:
    hana "What about the other way around? You ever just want to slap some dude in his smug face?"

translate id mod_BirthdayGame_1_5bc7e452:
    mina "Um... of course not..."

translate id mod_BirthdayGame_1_a80d79e0:
    hana "Eh, don't knock it until you try it. With someone you trust, rough sex can be fun."

translate id mod_BirthdayGame_1_bc223803:
    mina "That just sounds scary to me..."

translate id mod_BirthdayGame_1_4c5a825c:
    "Now Mina was eyeballing Hana dubiously."

translate id mod_BirthdayGame_1_21737296:
    mina "Oh! Sorry, [2]! I'm not making fun of you, I was just trying to understand. I hope I'm not embarrassing you..."

translate id mod_BirthdayGame_1_0b15d407:
    mc "Don't worry about it."

translate id mod_BirthdayGame_2_6884f0bd:
    mc "I've always kind of wanted a woman to edge me."

translate id mod_BirthdayGame_2_41daa407:
    mina "What the heck does that mean?"

translate id mod_BirthdayGame_2_3d925f8b:
    "Mina eyeballed me suspiciously."

translate id mod_BirthdayGame_2_facff0bb:
    mina "It's not a weird internet thing, right? Because I learned about those balloon people the other day..."

translate id mod_BirthdayGame_2_7f1cf021:
    kil "It means he wants to put a cork in it before he pops."

translate id mod_BirthdayGame_2_4ba862a0:
    kil "You know, with it being his dick."

translate id mod_BirthdayGame_2_2d069cba:
    mc "That's a colorful way of describing it."

translate id mod_BirthdayGame_2_a82e4254:
    mina "What? Why would you want to do that? That defeats the purpose, doesn't it?"

translate id mod_BirthdayGame_2_81ca63f1:
    mc "How should I explain this...?"

translate id mod_BirthdayGame_2_caa79265:
    "I gave the next words out of my mouth some careful consideration."

translate id mod_BirthdayGame_2_29bd2d1f:
    mc "The idea behind is to {3}delay{4} orgasm, not deny it. When you finally do orgasm, it's supposed to make it more intense. Plus..."

translate id mod_BirthdayGame_2_41baf708:
    mct "(As much as I've lived my life, preoccupied with the notion of living my life in control, carefully steering it in the right direction...)"

translate id mod_BirthdayGame_2_6fee6142:
    mc "I find the idea of lying back, giving up control to my partner, and having them tease me kind of hot."

translate id mod_BirthdayGame_2_efe96ddd:
    hana "[2]'s a bit of a masochist."

translate id mod_BirthdayGame_2_801bc223:
    mc "Not really. I just don't mind being on the bottom of things sometimes."

translate id mod_BirthdayGame_2_93ea01e8:
    mina "I don't think I get it."

translate id mod_BirthdayGame_2_f7586e5a:
    mc "Well, just picture it for a second. Imagine if you could control when a dude came? You had him exactly where you want him, on the precipice of release, and then you just stop short of that."

translate id mod_BirthdayGame_2_b7d9161c:
    mc "You don't think his agonizing, pleading face would be hot?"

translate id mod_BirthdayGame_2_862ae955:
    mc "You haven't ever wanted Ian to last longer than a minute in-"

translate id mod_BirthdayGame_2_7d2a8c35:
    kil "Hey, fuck you buddy!"

translate id mod_BirthdayGame_2_4e2a8876:
    mina "..."

translate id mod_BirthdayGame_2_8c37b1a6:
    mina "Yeah, okay. I think I get it now. That actually sounds like it could be fun."

translate id mod_BirthdayGame_3_13678aea:
    "I paused for some time to carefully consider how I was going to put this."

translate id mod_BirthdayGame_3_845bc00a:
    "Long enough for the break in my words to feel pronounced and heavy."

translate id mod_BirthdayGame_3_aeb7cd48:
    mc "I've always wanted to..."

translate id mod_BirthdayGame_3_3cba9085:
    mc "Okay, I know this is gonna sound weird and fucked up, but this is just a fantasy okay?"

translate id mod_BirthdayGame_3_ac7e7d3f:
    hana "Well, that's a real good fucking way to preface something weird and fucked up."

translate id mod_BirthdayGame_3_ad94bada:
    mc "...I've always wanted to treat a woman like a literal object."

translate id mod_BirthdayGame_3_48b9c2a0:
    "Saying it out loud made it sound horrible. I felt like I had just set myself adrift smack dab in the middle of a sea of judgement."

translate id mod_BirthdayGame_3_1c95e187:
    hana "Go on."

translate id mod_BirthdayGame_3_9baf00cd:
    mc "Shit, it's like, um..."

translate id mod_BirthdayGame_3_4cad5764:
    mina "You mean like... uh, what was it called?"

translate id mod_BirthdayGame_3_75c92e7f:
    "Mina searched her mind for the word."

translate id mod_BirthdayGame_3_3ae1ff09:
    mina "Forniphilia...?"

translate id mod_BirthdayGame_3_ae0ed2d3:
    mc "Like what now?"

translate id mod_BirthdayGame_3_91bc6b34:
    mina "The fetish of creating and using people like human furniture!"

translate id mod_BirthdayGame_3_100caaeb:
    kil "You know the damn word for that?"

translate id mod_BirthdayGame_3_b8965048:
    mina "There was an article about that kinda weird crap in a women's mag I modeled for, okay?"

translate id mod_BirthdayGame_3_c94b705d:
    kil "...and you remembered it?"

translate id mod_BirthdayGame_3_1fd915d6:
    mina "I have a good memory!"

translate id mod_BirthdayGame_3_0fd4d656:
    "......"

translate id mod_BirthdayGame_3_a20cefa7:
    "..."

translate id mod_BirthdayGame_3_909192ca:
    mina "Anyway, it's like that?"

translate id mod_BirthdayGame_3_c8a5985d:
    mc "Uh, yeah. That could be included, but..."

translate id mod_BirthdayGame_3_447b9d5b:
    mc "What I had in mind is more like sex in the middle of everyday situations. Like while eating breakfast or in the middle of a meeting..."

translate id mod_BirthdayGame_3_57475539:
    mina "Interesting..."

translate id mod_BirthdayGame_3_9e6b1359:
    mc "Yeah... it is pretty weird, I guess."

translate id mod_BirthdayGame_3_7df014d7:
    hana "So basically, you're a huge pervert?"

translate id mod_BirthdayGame_3_f800bed0:
    mc "Yes."

translate id mod_BirthdayGame_3_38b4dfe7:
    hana "Heh, that's okay! There's nothing wrong with that."

translate id mod_BirthdayGame_3_73de812d:
    hana "A lot of people are."

translate id mod_BirthdayGame_3_2f3445fe:
    mc "Okay, if no one else is going to drink, I'm next then."

translate id mod_BirthdayGame_3_42292f41:
    "I happily seized the opportunity to move on from the topic."

translate id mod_BirthdayGame_3_e2b7999a:
    kil "Let's make this next round the last."

translate id mod_BirthdayGame_3_0a681aaa:
    mina "How come?"

translate id mod_BirthdayGame_3_351cf5bd:
    mc "Yeah, you're only half-blasted."

translate id mod_BirthdayGame_3_7ba24f79:
    kil "Well, I just thought if we ended a little early. We could finish the night with a movie like I know the birthday boy would prefer. Does that sound good to you all?"

translate id mod_BirthdayGame_3_e5eff3e8:
    mina "A movie sounds fun!"

translate id mod_BirthdayGame_3_fa4743fc:
    hana "I'm cool with it."

translate id mod_BirthdayGame_3_cd1b1fcc:
    "Sometimes, Ian could be deceptively considerate."

translate id mod_BirthdayGame_3_47d5ef83:
    mc "It's a good excuse to avoid a raging hangover."

translate id mod_BirthdayGame_3_1afc901c:
    kil "Alright, so make your last turns a doozy."

translate id mod_BirthdayGame_3_f7c474da:
    mc "Hmm, alright..."

translate id mod_BirthdayGame_3_fde785c0:
    "Scrambling to come up with an answer on the spot, my mind immediately latched onto two options."

translate id mod_BirthdayGame_3_649e84bc:
    "For some reason, the image of Ian flirting with that bar girl the night we went out with Mina and Felicia sprang to mind. As did Felicia's comment about it happening all the time."

translate id mod_BirthdayGame_3_07e914b8:
    "Mina seems almost begrudgingly accepting of it... I could use it to rustle Ian's feathers a little. Then again, would that even be taken as humor?"

translate id mod_BirthdayGame_3_18d85ef7:
    "No. There's teasing and then there's being a giant dickhole. You don't need to be a social savant to know this would pretty much be twisting the knife."

translate id mod_BirthdayGame_3_7d9a7776:
    "The second option is to pick something simple and try and score the most points."

translate id mod_BirthdayGame_3_c65b95d6:
    mct "(Never have I ever...)"

translate id mod_BirthdayGame_3_08d6b306:
    mc "Never have I ever… hit on, flirted with, or otherwise made advances to someone while on a date with someone else."

translate id mod_BirthdayGame_3_278c7e29:
    "........."

translate id mod_BirthdayGame_3_0fd4d656_1:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_1:
    "..."

translate id mod_BirthdayGame_3_93328d16:
    "No one made a move to take a shot."

translate id mod_BirthdayGame_3_bf9f3c2a:
    mc "We're all saints, hu--?"

translate id mod_BirthdayGame_3_807a7f31:
    mina "Just drink up."

translate id mod_BirthdayGame_3_df2e4998:
    "Mina cut Ian a cold glare, before plainly putting her command into words."

translate id mod_BirthdayGame_3_c92f9a6c:
    "The two of them shared a knowing look, before..."

translate id mod_BirthdayGame_3_941d0396:
    "Killian tilted his glass and drunk. He didn't look particularly pleased."

translate id mod_BirthdayGame_3_ef558068:
    "Meanwhile, Hana shot me a look like she was asking if I did that on purpose."

translate id mod_BirthdayGame_3_92d1e19f:
    mct "(...did I? Why would I?)"

translate id mod_BirthdayGame_3_e748c61a:
    "My eyes involuntary wondered over to Mina and part of me grasped the answer."

translate id mod_BirthdayGame_3_95a86d58:
    hana "Alright, mooooving on..."

translate id mod_BirthdayGame_3_245900bb:
    mc "Never have I ever pretended to be sick to get out of something I didn't want to do."

translate id mod_BirthdayGame_3_e6ffd30e:
    "I decided to go for the gold, so I went with a safe question. I hadn't, but I'm sure most people had."

translate id mod_BirthdayGame_3_b1f10d4e:
    "I was right."

translate id mod_BirthdayGame_3_6022cd99:
    hana "I played hooky from school every now and then. My grandfather let me get away with it occasionally."

translate id mod_BirthdayGame_3_44b71a6e:
    mina "I've gotten out of an agency Christmas party that way."

translate id mod_BirthdayGame_3_2655680d:
    kil "I did it to avoid the class bully."

translate id mod_BirthdayGame_3_6c288417:
    hana "Oh... for real?"

translate id mod_BirthdayGame_3_7e5c5ddb:
    mc "I don't remember you ever skipping school without me."

translate id mod_BirthdayGame_3_10d4f777:
    kil "It was actually a few weeks before we became friends. You remember Austin Kelly?"

translate id mod_BirthdayGame_3_66f0be7e:
    mc "He had a new target every week."

translate id mod_BirthdayGame_3_a8a62a19:
    kil "He did. Until he got around to me. For some reason, he got real hung up on me. It carried on for weeks. Remember how we first met?"

translate id mod_BirthdayGame_3_ed7e08f2:
    mc "Hmm... yeah, I do."

translate id mod_BirthdayGame_3_08f59328:
    kil "He gave anyone who tried to talk to me a hard time, until people started avoiding me. Eventually, it got to the point that if I sat down at the lunchroom table, other kids would get up and move."

translate id mod_BirthdayGame_3_8c675b0a:
    kil "[2] didn't though."

translate id mod_BirthdayGame_3_1b1a264d:
    kil "He made it a point to sit with me."

translate id mod_BirthdayGame_3_e67a8c4f:
    mc "Having a whole table to ourselves was pretty sweet."

translate id mod_BirthdayGame_3_a41bd242:
    kil "Fuck yeah it was. I still remember how gobsmacked that fatass looked when you told him to fuck off back to the trailer park. He looked like you sucker punched him."

translate id mod_BirthdayGame_3_34906fec:
    "That was an understatement. I still remembered how deeply hurt he looked and how instantaneously I regretted my choice of words."

translate id mod_BirthdayGame_3_c5007d89:
    kil "You always did know how to get under people's skin."

translate id mod_BirthdayGame_3_575476d3:
    mc "Well, it was obvious that would tick him off. I'm pretty sure your parents' money was why he bullied you so much longer than anyone else in the first place."

translate id mod_BirthdayGame_3_3c164b5c:
    kil "That was probably it."

translate id mod_BirthdayGame_3_365474c9:
    hana "So, the rest is history?"

translate id mod_BirthdayGame_3_d3440960:
    "Ian looked at me with nostalgia in his eyes."

translate id mod_BirthdayGame_3_cc5229df:
    kil "Yep. That was when things started to get better. I mean kids could still be mean, but [2] stuck to me like glue after that. I never felt alone."

translate id mod_BirthdayGame_3_d313245f:
    mina "That's sweet."

translate id mod_BirthdayGame_3_1f46c105:
    mc "Three points for me then."

translate id mod_BirthdayGame_3_01cae419:
    mc "Your turn, Hana."

translate id mod_BirthdayGame_3_b1871a4c:
    hana "Hmm... it's a little too serious now. Let's get the mood back to where it was. What am I at?"

translate id mod_BirthdayGame_3_9a79ef1f:
    mc "You have four points, Mina has five, Ian has three, and I have [19]."

translate id mod_BirthdayGame_3_32409458:
    hana "Never have I ever..."

translate id mod_BirthdayGame_3_1309871c:
    hana "Been caught with my pants down."

translate id mod_BirthdayGame_3_76fa540f:
    kil "You mean...?"

translate id mod_BirthdayGame_3_bfc67bf6:
    hana "Oh, you know. In an act of {3}sexual congress{4} or self-debasement. Either or."

translate id mod_BirthdayGame_3_a5176e05:
    kil "Ah, well... check and check."

translate id mod_BirthdayGame_3_d0f9a5b3:
    hana "Aw, really? Just him?"

translate id mod_BirthdayGame_3_a8777bf7:
    mc "I've always been careful. Thin walls growing up."

translate id mod_BirthdayGame_3_dcb678f5:
    mina "I would just... DIE. My mom isn't exactly the most sex-positive person..."

translate id mod_BirthdayGame_3_468136f3:
    kil "I still haven't met your parents, huh?"

translate id mod_BirthdayGame_3_71ea7bb8:
    mina "Nope. With any luck, you won't for a long time."

translate id mod_BirthdayGame_3_285bcd48:
    mc "How come?"

translate id mod_BirthdayGame_3_d621f6ad:
    mina "If my mother got one whiff of how successful Ian's parents are, she'd stick her nose where it didn't belong."

translate id mod_BirthdayGame_3_6d3f0b21:
    kil "So she says, but I think she's just embarrassed by me."

translate id mod_BirthdayGame_3_68740ad7:
    "Ian smiled, looking almost proud of this."

translate id mod_BirthdayGame_3_0a5f1376:
    mina "You remember me mentioning her backwards view on higher education, [2]?"

translate id mod_BirthdayGame_3_c748f50f:
    mc "Yeah... something about using it to find a well-to-do husband?"

translate id mod_BirthdayGame_3_93b59f6e:
    mina "She says college for a young lady should be about finding a husband."

translate id mod_BirthdayGame_3_c468ac19:
    mina "With Ian, she'd be like a shark with blood in the water..."

translate id mod_BirthdayGame_3_a71da0ce:
    kil "Would that really be so bad?"

translate id mod_BirthdayGame_3_a1acf2e9:
    mina "Yes! I'm 19! What sane girl my age is looking to settle down?"

translate id mod_BirthdayGame_3_4a388e8d:
    hana "If you find that right person, I don't think it's so bad giving up part of your youth to have something you could call a family."

translate id mod_BirthdayGame_3_9863e072:
    mina "Maybe... but my mother and my revolving door of stepdads is proof that tying the knot does not a family make. I guess your parents must've had a good marriage?"

translate id mod_BirthdayGame_3_113b9493:
    hana "Um..."

translate id mod_BirthdayGame_3_e10e27c2:
    mina "S-sorry if that's too personal of a question. It just sounded like you had a positive view of--"

translate id mod_BirthdayGame_3_b6d63e9b:
    hana "No, it's cool. The answer to your question is a big ol' fuck no. I was brought up by mom."

translate id mod_BirthdayGame_3_75b495d3:
    "Mina chose not to speak, instead she simply nodded in solidarity."

translate id mod_BirthdayGame_3_cdde350c:
    mct "(Sounds like she can sort of relate.)"

translate id mod_BirthdayGame_3_5e834ad0:
    hana "It's your go, Mina."

translate id mod_BirthdayGame_3_3668d29b:
    mina "Okay, I've got a pretty good one. Never have I ever..."

translate id mod_BirthdayGame_3_6db3c49e:
    mina "Lied during this game."

translate id mod_BirthdayGame_3_0fd4d656_2:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_2:
    "..."

translate id mod_BirthdayGame_3_17a56aba:
    "No one drank."

translate id mod_BirthdayGame_3_63859e6e:
    kil "It's a fun question, but did you forget you were trying really hard to win?"

translate id mod_BirthdayGame_3_4e2a8876:
    mina "..."

translate id mod_BirthdayGame_3_9ac93956:
    mina "Ah... DAMN!"

translate id mod_BirthdayGame_3_6fb17c48:
    hana "Pftt-!"

translate id mod_BirthdayGame_3_05d8318c:
    kil "If anyone HAS lied in the first place, why would you think they'd admit to it?"

translate id mod_BirthdayGame_3_4ed0e2df:
    hana "*Whisper* Ian's girlfriend isn't what I pictured."

translate id mod_BirthdayGame_3_d5dbbfe7:
    mina "I don't know! For dramatic effect?"

translate id mod_BirthdayGame_3_28663d9d:
    kil "Well, that puts you and Hana tied at 5, with [2] at [19]."

translate id mod_BirthdayGame_3_db8dd64e:
    kil "I'm at 3, so that means..."

translate id mod_BirthdayGame_3_dc1a24b8:
    kil "I have absolutely no hope of winning."

translate id mod_BirthdayGame_3_45551e82:
    hana "What? You're just ending it like that?"

translate id mod_BirthdayGame_3_a4145573:
    kil "Why not? Might as well let the birthday boy get straight to his kingly command."

translate id mod_BirthdayGame_3_302bcac1:
    kil "What would you have us do, My Lord?"

translate id mod_BirthdayGame_3_8f28b32d:
    mina "Welp, better [2] than Ian."

translate id mod_BirthdayGame_3_44d6dbd5:
    hana "You better not ask us to do something perverted."

translate id mod_BirthdayGame_3_c3fa0ba7:
    mina "[2] wouldn't do that. Would he...?"

translate id mod_BirthdayGame_3_274ce183:
    mc "Me? No..."

translate id mod_BirthdayGame_3_1906b0eb:
    "I knew {1}exactly{2} what I wanted them to do, but first..."

translate id mod_BirthdayGame_3_8ead1991:
    mc "The three of you line up in a row over there and kneel, please."

translate id mod_BirthdayGame_3_b7bd47a5:
    "Each of them looked at me doubtingly, but complied with my request, moving away from the couch and into the open."

translate id mod_BirthdayGame_3_d583e58b:
    mc "Now, what will I have you do...?"

translate id mod_BirthdayGame_3_b56f8a9e:
    mina "Huh? I thought you already knew because of this pose."

translate id mod_BirthdayGame_3_091c634f:
    mc "I just wanted to have your waiting faces looking up to me while I thought about it."

translate id mod_BirthdayGame_3_af6a46fa:
    hana "Pfft, jerkass!"

translate id mod_BirthdayGame_3_0aa6dc82:
    "That was a lie. I did know {1}exactly{2} what I wanted them to do, but I wanted to drag it out some."

translate id mod_BirthdayGame_3_d580891f:
    mc "I got it!"

translate id mod_BirthdayGame_3_577a326e:
    "I feigned like it had just hit me."

translate id mod_BirthdayGame_3_289d8872:
    mc "Now..."

translate id mod_BirthdayGame_3_4f0b2e80:
    mc "I dare you to say something nice about the other two."

translate id mod_BirthdayGame_3_a760c7a2:
    mina "Eh...? That's it?"

translate id mod_BirthdayGame_3_6cb7497d:
    hana "You got us down on our knees for that?"

translate id mod_BirthdayGame_3_0e5e64d8:
    mc "Yes. Why don't you start it off, Hana?"

translate id mod_BirthdayGame_3_51d4c115:
    mc "Say something nice about {1}Ian{2}..."

translate id mod_BirthdayGame_3_f804ef36:
    hana "Eugh..."

translate id mod_BirthdayGame_3_f5519bd5:
    "I folded my arms and patiently let Hana mull over ALL that she liked about Ian."

translate id mod_BirthdayGame_3_9d6d066a:
    hana "Mmh...?"

translate id mod_BirthdayGame_3_f36f40e7:
    "This was meant to be a small gesture to bring everyone closer together, but I'd be lying if I said making Hana say something nice about the man she despises didn't tickle a sadistic instinct in me."

translate id mod_BirthdayGame_3_ec355ac0:
    hana "Fine, I got it. Ian..."

translate id mod_BirthdayGame_3_2c210bae:
    hana "I like that you always bring coffee for everyone when you come into... {3}your Uncle's{4}. You always remember everyone's specific order..."

translate id mod_BirthdayGame_3_57074a40:
    kil "A squirt of cardomon to sweeten it up and a spritz of coconut oil to thicken up the drink."

translate id mod_BirthdayGame_3_f9a2d38e:
    hana "Yes... it's surprisingly conscientious and thoughtful..."

translate id mod_BirthdayGame_3_601dfbb7:
    "I could just hear the \"for you\" tacked onto the end of that in Hana's mind."

translate id mod_BirthdayGame_3_ffccfa2d:
    kil "Thank you, Hana. It's nice to feel appreciated."

translate id mod_BirthdayGame_3_1dcad8fd:
    "Especially with how smug Ian looked hearing just the smallest amount of praise."

translate id mod_BirthdayGame_3_35cbd5a5:
    hana "As for Mina... we just met, but she's been very friendly and warm toward me almost like we were already pretty good friends. Plus..."

translate id mod_BirthdayGame_3_f0ef7f95:
    hana "Her pigtails are good."

translate id mod_BirthdayGame_3_c930a6bf:
    mina "Hehe~ I got praised..."

translate id mod_BirthdayGame_3_343ae53c:
    mc "Your turn Ian."

translate id mod_BirthdayGame_3_22d20185:
    kil "I... respect that Hana is not afraid to freely speak her mind."

translate id mod_BirthdayGame_3_84c28a87:
    kil "I think that's a pretty cool way to go through life."

translate id mod_BirthdayGame_3_1e1dda1c:
    hana "Thanks."

translate id mod_BirthdayGame_3_27b85831:
    kil "As for Mina, I couldn't ask for a more attractive girlfriend."

translate id mod_BirthdayGame_3_dd50cfd6:
    mina "That's it?"

translate id mod_BirthdayGame_3_10a00fc5:
    kil "I mean, uh... how do I love thee? Let me count the ways..."

translate id mod_BirthdayGame_3_3076e72e:
    kil "I love thee to the depth and breadth and height--"

translate id mod_BirthdayGame_3_d4037943:
    mina "I like that Ian can still surprise me. He's goofy and it's almost like he's stumbling through life with blinders on, but..."

translate id mod_BirthdayGame_3_ab155d17:
    "Mina looked at Ian, for the briefest of moments as if looking back on their relationship."

translate id mod_BirthdayGame_3_ba83e068:
    mina "Sometimes he blindly walks into making a girl feel special. Not that this is one of those times..."

translate id mod_BirthdayGame_3_68c0ee92:
    "I got the sense Mina was speaking nostalgically."

translate id mod_BirthdayGame_3_f94303b6:
    mina "As for, Hana! Where do I start?"

translate id mod_BirthdayGame_3_6ed43276:
    mina "She is so, so cool. I already told her I love her... \"ink\", but I also love her sense of fashion and..."

translate id mod_BirthdayGame_3_e747de89:
    mina "Even the way she's looking at me is so cool! She's also here because of [2], which tells me she's got good taste in friends!"

translate id mod_BirthdayGame_3_9cbbe603:
    mina "Not to mention...!"

translate id mod_BirthdayGame_3_0fd4d656_3:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_3:
    "..."

translate id mod_BirthdayGame_3_10c02e6a:
    "Mina spent the next 30 seconds spewing all kinds of compliments Hana's direction, by the end of it all the would be rockstar was blushing."

translate id mod_BirthdayGame_3_dc1cfa0d:
    hana "Um... thanks. That was... a lot."

translate id mod_BirthdayGame_3_ea61e618:
    mina "Hehe~"

translate id mod_BirthdayGame_3_8f27be07:
    kil "Alright, my knees are killing me. That's enough of this pose."

translate id mod_BirthdayGame_3_e1ddda7a:
    kil "Let's pop in a movie and get cozy?"

translate id mod_BirthdayGame_3_34133de1:
    mc "Sure. I'll pop some popcorn. What are you feeling?"

translate id mod_BirthdayGame_3_0fd4d656_4:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_4:
    "..."

translate id mod_BirthdayGame_3_eab42824:
    mc "I got you where I want you."

translate id mod_BirthdayGame_3_5c5ff04c:
    mina "What the heck does that--"

translate id mod_BirthdayGame_3_c9ba147e:
    "Boop!"

translate id mod_BirthdayGame_3_c9ba147e_1:
    "Boop!"

translate id mod_BirthdayGame_3_c9ba147e_2:
    "Boop!"

translate id mod_BirthdayGame_3_e7190340:
    mc "Alright, I'm all done. We can watch a movie now."

translate id mod_BirthdayGame_3_05b7841e:
    mina "Pftt-tck-!"

translate id mod_BirthdayGame_3_161e0997:
    mina "*Snort* Khtt- haha~ *Snort* What...!"

translate id mod_BirthdayGame_3_c0dfd1e8:
    kil "That was it? You're just gonna squander your dare?"

translate id mod_BirthdayGame_3_8509b93a:
    mc "The drinking game itself was a reward enough. I got to know you all better."

translate id mod_BirthdayGame_3_e08f94d2:
    mina "Hehe~hahAHAHAHA!"

translate id mod_BirthdayGame_3_95c7633a:
    mc "Are you okay?"

translate id mod_BirthdayGame_3_93b39a3c:
    mina "Pftt...! No-! *Snort* I c-can't breathe!"

translate id mod_BirthdayGame_3_8f27be07_1:
    kil "Alright, my knees are killing me. That's enough of this pose."

translate id mod_BirthdayGame_3_e1ddda7a_1:
    kil "Let's pop in a movie and get cozy?"

translate id mod_BirthdayGame_3_a1f52f80:
    mc "Sure. I'll pop some popcorn while Mina comes back to her senses."

translate id mod_BirthdayGame_3_0fd4d656_5:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_5:
    "..."

translate id mod_BirthdayGame_3_94003d06:
    "I thought about it some, and there was no need to make this some long, protracted thing. A quick picture marking me as a winner and them as the loser would do."

translate id mod_BirthdayGame_3_faa284ba:
    mc "I think I'll use this opportunity to snag a victory photo."

translate id mod_BirthdayGame_3_263d0a3e:
    mina "Uh... why do you look so... evil?"

translate id mod_BirthdayGame_3_4d24641a:
    mc "You're just imagining things. Now, do you three know how to kotow?"

translate id mod_BirthdayGame_3_c74ee422:
    mc "More over to the left... you're not in the shot --"

translate id mod_BirthdayGame_3_4887cffa:
    hana "Like this?"

translate id mod_BirthdayGame_3_4f43e948:
    mc "A little more... perfect! You got it!"

translate id mod_BirthdayGame_3_0cf74344:
    mc "I want you to look like you just found God!"

translate id mod_BirthdayGame_3_deefd60a:
    kil "This is stupid..."

translate id mod_BirthdayGame_3_da9e3d5a:
    mc "Now, say \"You are the most charasmatic, savvy, magn--\"...!"

translate id mod_BirthdayGame_3_3620d34e:
    hana "Just take the damn photo!"

translate id mod_BirthdayGame_3_ea03c6fb:
    mc "Good enough."

translate id mod_BirthdayGame_3_8f27be07_2:
    kil "Alright, my knees are killing me. That's enough of this pose."

translate id mod_BirthdayGame_3_e1ddda7a_2:
    kil "Let's pop in a movie and get cozy?"

translate id mod_BirthdayGame_3_34133de1_1:
    mc "Sure. I'll pop some popcorn. What are you feeling?"

translate id mod_BirthdayGame_3_0fd4d656_6:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_6:
    "..."

translate id mod_BirthdayGame_3_70113ae9:
    kil "I could tie this up at least."

translate id mod_BirthdayGame_3_816970c2:
    kil "I could win this."

translate id mod_BirthdayGame_3_ddb9915d:
    kil "Never have I ever swallowed a pill."

translate id mod_BirthdayGame_3_0daff12f:
    hana "Are you kidding me? That's what you're gonna end this on?"

translate id mod_BirthdayGame_3_336203e0:
    kil "It's the truth. I haven't been sick since I was a kid."

translate id mod_BirthdayGame_3_74c853e7:
    mc "Why do you sound like you're bragging?"

translate id mod_BirthdayGame_3_6110de81:
    kil "Drink up. That's a critical hit, right?"

translate id mod_BirthdayGame_3_6b1fb256:
    "The three of us shared a mutual look, before knocking back our glasses."

translate id mod_BirthdayGame_3_e46f0296:
    "With that our short little drinking game came to an end."

translate id mod_BirthdayGame_3_5456ff7c:
    kil "That means [2] and I tied and that puts you two ladies on the chopping block."

translate id mod_BirthdayGame_3_3d29e0ca:
    mina "Why do you have to phrase it that way?"

translate id mod_BirthdayGame_3_c544ab5f:
    kil "[2], my buddy..."

translate id mod_BirthdayGame_3_7d90228d:
    mc "Yes, friend?"

translate id mod_BirthdayGame_3_47f99083:
    kil "Please, consult with me for a moment. We need to discuss a fitting prize."

translate id mod_BirthdayGame_3_56be9fa9:
    mc "Of course, let's confer in my office."

translate id mod_BirthdayGame_3_0fd4d656_7:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_7:
    "..."

translate id mod_BirthdayGame_3_b5795aad:
    mc "Oh, yeah. Sure. That's good! It needs a little something extra though..."

translate id mod_BirthdayGame_3_fcc9468d:
    kil "What do you have in mind, partner?"

translate id mod_BirthdayGame_3_2e27414a:
    mc "Oh.. I know! I saw something strange in a trunk the other day."

translate id mod_BirthdayGame_3_a20cefa7_8:
    "..."

translate id mod_BirthdayGame_3_7c5ba649:
    kil "I remember these are definitely Darius'."

translate id mod_BirthdayGame_3_461bcfae:
    mc "I think they're perfect."

translate id mod_BirthdayGame_3_986c9f4b:
    kil "Great! Stand just like that!"

translate id mod_BirthdayGame_3_4b91af18:
    hana "What is this? A photoshoot?"

translate id mod_BirthdayGame_3_4d872f41:
    mina "That is his thing."

translate id mod_BirthdayGame_3_95571ef7:
    kil "We're just gonna take a commemorative losers photo. Only, it's missing something..."

translate id mod_BirthdayGame_3_a676b7e1:
    mc "Luckily, I have just the thing lying around that'll add the perfect final touch."

translate id mod_BirthdayGame_3_adbf355a:
    mc "Hold still girls."

translate id mod_BirthdayGame_3_8ea71c6a:
    mina "He- what's this? Oh, c'mon..."

translate id mod_BirthdayGame_3_133bfd09:
    hana "This is stupid."

translate id mod_BirthdayGame_3_e6ffea7e:
    kil "Wow, you girls look great. Now, grab ahold of each other like long lost sisters!"

translate id mod_BirthdayGame_3_fd3720e9:
    kil "I want to feel your intensity!"

translate id mod_BirthdayGame_3_7a11f571:
    kil "No... no, that won't do. Closer! Like you haven't seen each other in a decade."

translate id mod_BirthdayGame_3_8454600e:
    kil "Perfect! You got it!"

translate id mod_BirthdayGame_3_fadb3bdd:
    kil "You girls look beautiful. Don't they, [2]?"

translate id mod_BirthdayGame_3_fb73f2e6:
    mc "They sure do. Like a pair of angels."

translate id mod_BirthdayGame_3_0cbe8c8f:
    mina "Grr..."

translate id mod_BirthdayGame_3_34662fdd:
    hana "Okay, that's enough! You two had your fun."

translate id mod_BirthdayGame_3_1c11491d:
    kil "You want a copy for your social media profiles?"

translate id mod_BirthdayGame_3_bca58097:
    "The scene that played out was utterly stupid, but... it was fun."

translate id mod_BirthdayGame_3_1dc602b5:
    mina "Can we watch that movie now?"

translate id mod_BirthdayGame_3_9cf6b821:
    mc "Sure. What are you guys feeling?"

translate id mod_BirthdayGame_3_81a681b9:
    mina "It's your birthday."

translate id mod_BirthdayGame_3_b754c0b9:
    kil "Hmm... you girls familiar with the work of Groucho Marx? I think you'd love his comedy..."

translate id mod_BirthdayGame_3_1c27098a:
    kil "...get it?"

translate id mod_BirthdayGame_3_2c2a4bfd:
    mina "I don't get it."

translate id mod_BirthdayGame_3_f7118056:
    mc "I'll cook some popcorn while you explain it."

translate id mod_BirthdayGame_3_0fd4d656_8:
    "......"

translate id mod_BirthdayGame_3_a20cefa7_9:
    "..."

translate id mod_BirthdayGame_3_83311d84:
    kil "That means I win and you three are up on the chopping block."

translate id mod_BirthdayGame_3_3d29e0ca_1:
    mina "Why do you have to phrase it that way?"

translate id mod_BirthdayGame_3_631d4c69:
    mc "Go easy on us, please."

translate id mod_BirthdayGame_3_0a717443:
    kil "Easy...? Hmm, well. It is your birthday and I am a magnanimous man."

translate id mod_BirthdayGame_3_12b239f8:
    kil "Sit tight for just a second, while I see what you got going in your fridge."

translate id mod_BirthdayGame_3_c3522054:
    mina "The fridge...? Please don't get food involved..."

translate id mod_BirthdayGame_3_97345cad:
    "We waited as Ian rummaged throughout the kitchen, until he found what he was looking in the freezer."

translate id mod_BirthdayGame_3_0e74e4a2:
    "He returned with a glass clutched tightly in his hand, giving it a demonstrative shake that produced a distinctive clank."

translate id mod_BirthdayGame_3_3dd2116e:
    mc "A glass of ice? What are you planning on having us do with that?"

translate id mod_BirthdayGame_3_e33a7ce6:
    kil "Simple. I want you three to pass it along to the person next to you, using only your mouth."

translate id mod_BirthdayGame_3_97593b6b:
    mc "Oh...?"

translate id mod_BirthdayGame_3_cf82c0b0:
    hana "Geez, that's not so bad, but I am a little perplexed at what you get out of this."

translate id mod_BirthdayGame_3_c9e6b8d2:
    "I knew right away what he was trying to do. He was playing wingman for me with Hana."

translate id mod_BirthdayGame_3_3cc8c508:
    kil "Maybe I just like to watch?"

translate id mod_BirthdayGame_3_7ef3c9b0:
    mina "He probably just wants to see you and I kiss."

translate id mod_BirthdayGame_3_61c5d3d3:
    hana "That's typical."

translate id mod_BirthdayGame_3_4f75b977:
    "Ian gave me a quick thumbs up on the sly, confirming my suspicion."

translate id mod_BirthdayGame_3_22513631:
    "I wanted to tell him I didn't need the help, but hey... he won fair and square. It's his dare."

translate id mod_BirthdayGame_3_ce296b76:
    "I wanted to tell him he had the wrong idea about Hana and I, but that would just make things awkward."

translate id mod_BirthdayGame_3_db138853:
    kil "[2] will start."

translate id mod_BirthdayGame_3_ca8d9680:
    mc "Just pop these in my mouth and pass it to Hana?"

translate id mod_BirthdayGame_3_9271c8c1:
    kil "Yep. You got it."

translate id w2BirthdayIanWins_7e190750:
    "I angled the glass and let the ice slide into my mouth on a bed of pooling water."

translate id w2BirthdayIanWins_b2df9c05:
    "Needing to be quick about it before it melted, I quickly sidled up next to Hana."

translate id w2BirthdayIanWins_05f37d44:
    "She looked dead in my eyes, lips pursed into a slight grin."

translate id w2BirthdayIanWins_a693ea78:
    hana "Well... let me have it, [2]."

translate id w2BirthdayIanWins_0d58461a:
    "She met my own awkward look with her own."

translate id w2BirthdayIanWins_d817042b:
    hana "Well... ready when you are."

translate id w2BirthdayIanWins_b97b2d13:
    "The chill of the ice quickly grew unpleasant to my gums, so I craned my neck down toward the rocker girl and she rose up to meet me."

translate id w2BirthdayIanWins_052b96fb:
    "Our lips met without ceremony and were soon engaged in the commission of our task."

translate id w2BirthdayIanWins_d0ec85fa:
    "I did take a moment to enjoy the sensation of Hana's soft lips pressing into mine. The warmth of it made an unusual contrast to the feeling of having ice sit square on the flesh of your tongue."

translate id w2BirthdayIanWins_c6cd0148:
    "I quickly found out the task required more concentration than one would've thought, owed largely to how crowded the inside of my mouth was."

translate id w2BirthdayIanWins_8dc70a48:
    "To help things along, Hana grabbed the back of my neck and used the leverage to redirect my efforts."

translate id w2BirthdayIanWins_b846e704:
    "One by one, the ice cubes slipped from my mouth into hers."

translate id w2BirthdayIanWins_106887fc:
    "Along the way, our tongues would naturally bump into one another, flooding my head with a pleasant feeling."

translate id w2BirthdayIanWins_b70159fd:
    "Sometimes it felt lingering, like they were intertwined longer than they had to. Still, eventually..."

translate id w2BirthdayIanWins_f61d9087:
    "They were gone. Our task was complete and Hana pulled herself back from me."

translate id w2BirthdayIanWins_1b8f25b9:
    "One by one, I slipped the ice cubes from my mouth into hers."

translate id w2BirthdayIanWins_465997f9:
    "Along the way, our tongues would naturally bump into one another, but we remained focused on the task."

translate id w2BirthdayIanWins_bbadc29d:
    "Eventually our task was complete and Hana pulled herself back from me."

translate id w2BirthdayIanWins_6d954750:
    "Hana wasted no time of her own and moved on to do the same to Mina."

translate id w2BirthdayIanWins_1fd36593:
    "They shared a fleeting look, causing a noticeable blush on the blonde's cheeks and an unduly long pause before the two set out on their given task."

translate id w2BirthdayIanWins_0c9e3eff:
    mina "Mmmh...!"

translate id w2BirthdayIanWins_e9d67ba3:
    "Mina let out a sharp, surprising sound as soon as their lips met."

translate id w2BirthdayIanWins_152785be:
    "The two briefly looked into each other's eyes, before setting out on the given task."

translate id w2BirthdayIanWins_34b7c42f:
    "The pair pressed their lips against one another and Hana began the process anew."

translate id w2BirthdayIanWins_9466e2aa:
    "Watching their lips gently undulate as the ice cubes passed from one to the other was a beautiful sight."

translate id w2BirthdayIanWins_05119e18:
    "Hana instinctually grabbed at Mina's upper arms, adding an illusion of intimacy between the two."

translate id w2BirthdayIanWins_906d60b2:
    "Meanwhile, Mina's lower half squirmed in place, hips ever-so-lightly swaying with an anxious energy."

translate id w2BirthdayIanWins_2640a92a:
    "Melted ice pooled around the corners of their mouth."

translate id w2BirthdayIanWins_7264c474:
    "By the time it occurred to me that I was maybe watching {3}too{4} intently, it was over."

translate id w2BirthdayIanWins_8ed4e125:
    "Mina looked like she was in a daze as she chewed on the ice in her mouth."

translate id w2BirthdayIanWins_7264c474_1:
    "By the time it occurred to me that I was maybe watching {3}too{4} intently, it was over."

translate id w2BirthdayIanWins_843643e1:
    "The two shared a smile, as Mina chewed on the ice in her mouth."

translate id w2BirthdayIanWins_a20cefa7:
    "..."

translate id w2BirthdayIanWins_d65e51e8:
    hana "So, movie now?"

translate id w2BirthdayIanWins_315ecf39:
    mc "Yeah, that sounds good to me."

translate id w2BirthdayIanWins_063b9d3e:
    mc "What are you guys feeling?"

translate id w2BirthdayIanWins_541b2679:
    hana "It's your birthday. Why don't you pick?"

translate id w2BirthdayIanWins_7feb2697:
    mc "Okay. You guys get settled first and I'll cook up some popcorn while I think about it."

translate id w2BirthdayIanWins_0fd4d656:
    "......"

translate id w2BirthdayIanWins_a20cefa7_1:
    "..."

translate id w2BirthdaySegue_461aa85f:
    "Over the next couple of hours, the night petered out."

translate id w2BirthdaySegue_03eb9c2d:
    hana "*Whisper* Wait... is that one of...?"

translate id w2BirthdaySegue_7b1da34a:
    "The film of choice: Terminal Action, one of the handful of B-movie action flicks that Samson Garcia was known for."

translate id w2BirthdaySegue_3a2786ed:
    hana "He never shuts up about them, but I've never actually seen one of his movies."

translate id w2BirthdaySegue_908a58e9:
    "I had recalled Mina mentioning her love for this one earlier this week. It also felt like a nice in-joke for the Carnation Club-adjacent of us."

translate id w2BirthdaySegue_75ed4dc9:
    "Something dumb and simple for our inebriated minds, with the added something for the Carnation Club-adjacent of us."

translate id w2BirthdaySegue_8e37b09b:
    mina "Oh, I love this part!"

translate id w2BirthdaySegue_4094dcf5:
    sam "We're gonna have us a little barbecue! How do you like your ribs, Senator?"

translate id w2BirthdaySegue_0ac018ab:
    man "Wait, please--!"

translate id w2BirthdaySegue_f8430c05:
    tv "*Explosion* Boom!"

translate id w2BirthdaySegue_6d9b6b2e:
    "The film was terrible as it had always been, but it felt like I was seeing it with new eyes. Veronica was on my mind."

translate id w2BirthdaySegue_a52070c9:
    mct "(What a fucking bastard...)"

translate id w2BirthdaySegue_2765d085:
    "I don't know how I'm going to do it, but I reaffirmed the promise I made to myself."

translate id w2BirthdaySegue_42103782:
    "I'm going to figure out exactly what that shithead is doing to tank Veronica's business and go from there."

translate id w2BirthdaySegue_72409d64:
    "The thought of him purposefully tanking Veronica's business pissed me off, but it was outside of anything I could help."

translate id w2BirthdaySegue_3f1940ce:
    hana "It's no wonder he wound up selling gym equipment."

translate id w2BirthdaySegue_9f34f9a9:
    mc "*Whisper* Tell me about it."

translate id w2BirthdaySegue_e0d68599:
    mina "Looks like they're out."

translate id w2BirthdaySegue_bb09c8c4:
    mina "I knew this would happen. I told Ian to stop drinking."

translate id w2BirthdaySegue_9856e802:
    mc "None of you have any business driving home. Why don't you sleep here tonight?"

translate id w2BirthdaySegue_91ed86d9:
    mina "Thanks, [2]. That would be for the best."

translate id w2BirthdaySegue_742889bc:
    mc "Why don't you and Hana take my bed. There's room for two."

translate id w2BirthdaySegue_196af05b:
    mina "I told Ian to stop drinking. I knew this would happen."

translate id w2BirthdaySegue_9856e802_1:
    mc "None of you have any business driving home. Why don't you sleep here tonight?"

translate id w2BirthdaySegue_91ed86d9_1:
    mina "Thanks, [2]. That would be for the best."

translate id w2BirthdaySegue_742889bc_1:
    mc "Why don't you and Hana take my bed. There's room for two."

translate id w2BirthdaySegue_fe0e1da3:
    mina "No, this is a big couch. I'll sleep here with Ian tonight. Make sure he doesn't puke all over your apartment."

translate id w2BirthdaySegue_8d522e37:
    mc "That... would be HIGHLY appreciated."

translate id w2BirthdaySegue_e6884a64:
    hana "Eh...? How long have I been out?"

translate id w2BirthdaySegue_c61a561e:
    mc "The last 15 minutes of the movie. Mina and I were just discussing sleeping arrangements. Do you want to stay here or should I call you a taxi?"

translate id w2BirthdaySegue_fe68f2f8:
    hana "Here, if you don't mind. I'm not that tipsy, but better safe than sorry."

translate id w2BirthdaySegue_18ccbd35:
    hana "Plus, my bike's here and I don't want to spend the money going home and then coming back to get it."

translate id w2BirthdaySegue_4c482077:
    mc "Smart."

translate id w2BirthdaySegue_3571621d:
    "After getting Ian and Mina settled downstairs with blankets, I showed Hana to my bed."

translate id w2BirthdaySegue_1c0227e9:
    hana "So, this is where the magic happens?"

translate id w2BirthdaySegue_13000acc:
    mc "It's a new place. No magic yet."

translate id w2BirthdaySegue_d4e4c5dc:
    hana "That right?"

translate id w2BirthdaySegue_2f6237ee:
    mc "If you need anything, I'll be asleep in a chair downstairs."

translate id w2BirthdaySegue_21d524e8:
    hana "Eh? Don't be ridiculous. There's enough room for both of us."

translate id w2BirthdaySegue_314d9c94:
    mc "No, that's okay. I wouldn't want you to feel..."

translate id w2BirthdaySegue_72efc3ff:
    hana "Uncomfortable? I'd be more uncomfortable knowing I kicked you out of your own bed on the night of your birthday of all nights."

translate id w2BirthdaySegue_824d733a:
    hana "We're friends. Just get in the freakin' bed!"

translate id w2BirthdaySegue_a32a5421:
    hana "We're pretty much friends now. Just get in the freakin' bed!"

translate id w2BirthdaySegue_f7e82294:
    "No point in arguing with her."

translate id w2BirthdaySegue_0fd4d656:
    "......"

translate id w2BirthdaySegue_a20cefa7:
    "..."

translate id w2BirthdaySegue_c14c584c:
    mc "Well..."

translate id w2BirthdaySegue_89543236:
    "I gave Hana one long look before finishing my train of thought."

translate id w2BirthdaySegue_6500fd44:
    "I wouldn't mind sharing a bed with her, but of course I didn't say as much."

translate id w2BirthdaySegue_d5a65383:
    mc "You've convinced me."

translate id w2BirthdaySegue_a828bf5b:
    "After getting a little more comfortable, it didn't take long for either of us to settle in."

translate id w2BirthdaySegue_0e6e4fc0:
    "Soon..."

translate id w2BirthdaySegue_ee96883f:
    "A feeling of drowsiness ushered me into..."

translate id w2BirthdaySegue_16243dba:
    hana "You awake?"

translate id w2BirthdaySegue_8b388b39:
    mc "Yes, I do believe I am. What's up?"

translate id w2BirthdaySegue_f56d2db0:
    "Both of us spoke in a hushed tone."

translate id w2BirthdaySegue_47554500:
    hana "I had fun tonight."

translate id w2BirthdaySegue_4a7b7d82:
    mc "Me too, I'm glad you came, but you sound almost surprised."

translate id w2BirthdaySegue_85ff929d:
    hana "Heh, yeah. I am. I wouldn't ever dream of spending an evening drinking with that jackass downstairs."

translate id w2BirthdaySegue_626fc0a9:
    mc "Yeah, but you DID come."

translate id w2BirthdaySegue_0ba9829c:
    hana "Don't get cocky. I owed you one from the concert."

translate id w2BirthdaySegue_488172fe:
    mc "Is that the only reason you came?"

translate id w2BirthdaySegue_fc0e5754:
    hana "No."

translate id w2BirthdaySegue_6d1c24fd:
    mc "Well, I'm glad to hear it. Thanks for being here, Hana."

translate id w2BirthdaySegue_7e268579:
    hana "My pleasure, [2]."

translate id w2BirthdaySegue_2b35b6fe:
    mc "..."

translate id w2BirthdaySegue_e26fa964:
    mct "(What a nice smile...)"

translate id w2BirthdaySegue_92c6ada0:
    hana "You're tired?"

translate id w2BirthdaySegue_03a428f2:
    mc "Mmmhmh... got up pretty early today... visited Ian's mom... zZZzz... had lunch with... zZZzz..."

translate id w2BirthdaySegue_9d024537:
    hana "I have trouble picturing what his mom would be like."

translate id w2BirthdaySegue_b7f091f0:
    mc "...mmh, that's a tough nut to crack."

translate id w2BirthdaySegue_2a5797f4:
    hana "How come?"

translate id w2BirthdaySegue_e0250003:
    mc "Well, she's... a bitch? I mean, she cares about him. I'm ninety nine percent sure about that, but she's..."

translate id w2BirthdaySegue_a819c9f9:
    hana "Got a funny way of showing it?"

translate id w2BirthdaySegue_d026e8bf:
    mc "Yes. That's a good way of putting it."

translate id w2BirthdaySegue_b94929d3:
    hana "The world is filled with people like that."

translate id w2BirthdaySegue_e7eff76f:
    mc "Mmhmh..."

translate id w2BirthdaySegue_88cc229e:
    hana "You know, it occurs to me I haven't given you a birthday present yet."

translate id w2BirthdaySegue_2cea7f09:
    mc "Mmh... that's fine. I wasn't expecting anything. It was sprung on you at the last minute."

translate id w2BirthdaySegue_36950566:
    hana "...that's not what I was getting at, stupid."

translate id w2BirthdaySegue_eaa96161:
    "I felt a cold grip grab ahold of my hand, pulling me once more from the edge of sleep."

translate id w2BirthdaySegue_0913a130:
    mc "Eh, your hands are cold... what are you...?"

translate id w2BirthdaySegue_74ad4770:
    hana "I know. I need help warming them up."

translate id w2BirthdaySegue_6233a9ca:
    "Her voice, which had been delivered to my ears in a pleasantly soft bedside whisper, had dropped even lower and pulled me from my half-asleep state with its smokey timbre."

translate id w2BirthdaySegue_87990d0f:
    mc "You could always get underneath the covers."

translate id w2BirthdaySegue_1007ffdf:
    "I teased Hana, playing painfully dumb."

translate id w2BirthdaySegue_688fba36:
    hana "Come on, [2]..."

translate id w2BirthdaySegue_72762494:
    "It was a husky, needy voice that made her intention feverishly clear."

translate id w2BirthdaySegue_a4f6d030:
    hana "Don't try and poke holes in a girl's excuse."

translate id w2BirthdaySegue_5a3d8aaa:
    "The sultry way she formed her words had lit up a feeling of desire in me as fast and violent as a gasoline fire."

translate id w2BirthdaySegue_8e1cd133:
    "Inwardly, I marveled at how the right woman could fan the flames of arousal using only her voice."

translate id w2BirthdaySegue_83a17eb2:
    mc "This feels... abrupt."

translate id w2BirthdaySegue_5b3abe6d:
    hana "Don't let your head go and ruin a nice thing either."

translate id w2BirthdaySegue_575ff465:
    "For just a brief moment, her expression shifted subtly and she looked at me with lonely, borderline despondent eyes."

translate id w2BirthdaySegue_0e5439c5:
    "Truth be told, I wasn't sure I wanted to go barking up this tree."

translate id w2BirthdaySegue_b3afe142:
    "The proverbial tree being the boss' daughter."

translate id w2BirthdaySegue_463fa4da:
    "However, that look she was giving me, I couldn't fully rebuke her without feeling like a piece of shit."

translate id w2BirthdaySegue_635a46b2:
    "So I pulled her close to me and held her."

translate id w2BirthdaySegue_f06b4a14:
    mc "Sorry, I'm feeling a little tired. If you don't mind, do you think we could just...?"

translate id w2BirthdaySegue_81d1cf98:
    hana "..."

translate id w2BirthdaySegue_4385f3b7:
    "Hana looked at me curiously, with a briefly frustrated expression."

translate id w2BirthdaySegue_f6397952:
    hana "Yeah, of course."

translate id w2BirthdaySegue_8ec9286a:
    hana "This can be nice too..."

translate id w2BirthdaySegue_0e81c6b8:
    mc "Is everything okay with you?"

translate id w2BirthdaySegue_502604ab:
    hana "Why do you ask?"

translate id w2BirthdaySegue_e09354e9:
    mc "I don't know. Just seemed like the question to ask."

translate id w2BirthdaySegue_93c86429:
    hana "Mmmh... yeah, I'm fine. Just the usual stress in my life."

translate id w2BirthdaySegue_431830ce:
    mc "Want to tell me about it?"

translate id w2BirthdaySegue_c319d33c:
    hana "Not right now, but... maybe later?"

translate id w2BirthdaySegue_0af2ffad:
    mc "I'd be happy to hear it."

translate id w2BirthdaySegue_6ad6f50a:
    hana "...thanks, [2]."

translate id w2BirthdaySegue_596a936d:
    "Just like that, we fell into a comfortable silence."

translate id w2BirthdaySegue_3ec15445:
    "So comfortable that..."

translate id w2BirthdaySegue_5e9fe091:
    mc "......."

translate id w2BirthdaySegue_81d1cf98_1:
    hana "..."

translate id w2BirthDayHanaPresent_a19d0b3a:
    "That was all it took for the doors of self control to fling wide open."

translate id w2BirthDayHanaPresent_b9e51bcc:
    "I wasted no time in burying my face in the pale, beautiful expanse of her neckline."

translate id w2BirthDayHanaPresent_c2c2a3e5:
    "Hana met my fervor by hiking her naked legs up and around my waist, pulling me closer and locking our bodies together."

translate id w2BirthDayHanaPresent_29327186:
    "My hands naturally worked their way down and groped at the rocker girl's ass, fingers sinking pleasantly into the soft flesh and succeeding in pulling our waists closer together still."

translate id w2BirthDayHanaPresent_11aabde6:
    "Hana began to gently rock her hips as if on cue, grinding her crotch into mine and igniting a spreading warmth in my crotch as the outline of her vulva traced repeatedly against my tenting cock."

translate id w2BirthDayHanaPresent_ab519b3f:
    hana "Fuck, I need thi--!"

translate id w2BirthDayHanaPresent_fdaf1c5f:
    hana "{1}Mmh...!{1}"

translate id w2BirthDayHanaPresent_b7fe52a1:
    "In a bout of passion, I violently pressed my lips to Hana's, conveniently cutting her excited cry short from any listening ears downstairs."

translate id w2BirthDayHanaPresent_ed60dc87:
    "One of my hands clumsily found perch on a breast, fingers pawing at the thin cotton material that painfully separated them from making direct contact."

translate id w2BirthDayHanaPresent_b9e15613:
    mc "Oh, god. You're so--"

translate id w2BirthDayHanaPresent_73f12029:
    "This time it was Hana that kissed me, cutting off my words in a change of initiative."

translate id w2BirthDayHanaPresent_48dd7f12:
    hana "Ammh...♥"

translate id w2BirthDayHanaPresent_31afcd34:
    "It occurred to me that it felt nice to lock lips with a woman that had nothing to gain from it, but I immediately put that out of mind."

translate id w2BirthDayHanaPresent_a15a5467:
    "Instead, I focused on the wiry movements of Hana's tongue as it snaked past mine in a long, impassioned game of tonsil hockey."

translate id w2BirthDayHanaPresent_9e3f6f30:
    hana "Hhhmmm...!"

translate id w2BirthDayHanaPresent_5c9b1923:
    "When the kiss finally broke, both of us had to catch our breaths."

translate id w2BirthDayHanaPresent_15571517:
    hana "[2]..."

translate id w2BirthDayHanaPresent_5bf52654:
    "Hana placed a firm set of hands on my chest and gave me a suggestive push."

translate id w2BirthDayHanaPresent_b5f94305:
    "A suggestion I was happy to follow, as I rolled over and allowed Hana to slip into the driver's seat."

translate id w2BirthDayHanaPresent_0f82eda6:
    hana "I meant it about needing this."

translate id w2BirthDayHanaPresent_c7ae08f7:
    "With one quick motion, the oversized shirt that had been distracting me all night lay in a clump on the bedroom floor."

translate id w2BirthDayHanaPresent_2da3c516:
    "My hands wasted no time, sinking nice and deep into the supple flesh of Hana's glorious tits."

translate id w2BirthDayHanaPresent_02c086e3:
    hana "Nng..! Ah, yeah...! It's been... an embarrassing amount o-of... time since anyone's... touched me... like this..."

translate id w2BirthDayHanaPresent_0ff8f22f:
    "Her words came out in clumps, as if being modulated by my finger tips."

translate id w2BirthDayHanaPresent_56c974f3:
    mc "How long?"

translate id w2BirthDayHanaPresent_8a8365d5:
    hana "Let's see..."

translate id w2BirthDayHanaPresent_05f05b22:
    "Hana and I worked together to wiggle out of my pants and they soon joined Hana's shirt in a pile on the floor."

translate id w2BirthDayHanaPresent_a231b1b9:
    hana "A year."

translate id w2BirthDayHanaPresent_2d03db73:
    hana "Y'know, ever since I got a glimpse of this thing at the exhibition last night..."

translate id w2BirthDayHanaPresent_46289291:
    "Without a modicum of shyness, Hana wrapped her fingers around my stiffened cock."

translate id w2BirthDayHanaPresent_254f2733:
    hana "I've been wanting to see this thing up close."

translate id w2BirthDayHanaPresent_74ebcc4a:
    mc "Mmh... r-really?"

translate id w2BirthDayHanaPresent_cff0b437:
    "Her cold fingers wrapped around my burning cock felt amazing."

translate id w2BirthDayHanaPresent_79974610:
    hana "Uh huh."

translate id w2BirthDayHanaPresent_9d6a202f:
    "My eyes were drawn to the thin piece of fabric that covered her womanly bits, keenly aware of its close proximity to my own sex."

translate id w2BirthDayHanaPresent_e4cffafa:
    "The heat radiating from it was palpable, causing my dick to eagerly twitch for what was ahead."

translate id w2BirthDayHanaPresent_ea290036:
    "Instead of what I was imagining however, Hana decided to tease me."

translate id w2BirthDayHanaPresent_e04c836b:
    "Rubbing and touching the slightly exposed lips of her opening."

translate id w2BirthDayHanaPresent_643243cb:
    hana "Ah... oh... mmh...!"

translate id w2BirthDayHanaPresent_177d3eb2:
    mc "What are you doing?"

translate id w2BirthDayHanaPresent_e86e8612:
    "I said it in a tone that was more {3}what are you waiting for{4}?"

translate id w2BirthDayHanaPresent_a083f5c7:
    hana "Don't be so eager, lover. I'll give you your present soon enough, but for now..."

translate id w2BirthDayHanaPresent_71dd6cc6:
    mc "N-now...?"

translate id w2BirthDayHanaPresent_f575b83d:
    hana "I want you to watch."

translate id w2BirthDayHanaPresent_632b5b7c:
    "Her intonation was utterly obscene at this point."

translate id w2BirthDayHanaPresent_1ead401c:
    "I hesitated to call it pornographic, because there was no artifice to it. It was words wrapped in pure, raw, sensual need."

translate id w2BirthDayHanaPresent_47a48c09:
    "All I could do was control myself and wait patiently, as her fingers played and teased at her vulva, ever so occasionally pushing aside her panties and burrowing to parts unseen."

translate id w2BirthDayHanaPresent_53d3cd71:
    mc "Fuck..."

translate id w2BirthDayHanaPresent_b478b140:
    "The sight itself was enough to make me want to cum."

translate id w2BirthDayHanaPresent_016bd88e:
    mc "You're enjoying teasing me..."

translate id w2BirthDayHanaPresent_f660a207:
    hana "I'm enjoying the look on your face."

translate id w2BirthDayHanaPresent_3664db29:
    hana "It {3}really{4} turns me on."

translate id w2BirthDayHanaPresent_28053ca5:
    "The pent-up expression on her face said it all."

translate id w2BirthDayHanaPresent_3c743862:
    mct "(A {1}year{2}, huh? Not since she started working at the Club...)"

translate id w2BirthDayHanaPresent_2f4edf74:
    hana "I REALLY am enjoying that look on your face, but..."

translate id w2BirthDayHanaPresent_c64f4260:
    "The way she trailed off left me hopeful."

translate id w2BirthDayHanaPresent_0c652225:
    hana "It's not like I'm going to make you beg."

translate id w2BirthDayHanaPresent_ae4a598b:
    "Hana leaned back and braced herself on my knees, extending her legs past me, and kissing the underside of my cock with her yet uncovered sex."

translate id w2BirthDayHanaPresent_c16224d2:
    hana "I don't... want to go all the way tonight."

translate id w2BirthDayHanaPresent_99146670:
    "My stomach tightened when I heard the sad news. It must've shown on my face, because..."

translate id w2BirthDayHanaPresent_dd95e5d8:
    hana "S-sorry...! I mean that doesn't mean either of us won't be getting off to--"

translate id w2BirthDayHanaPresent_59a7c4f0:
    mc "Of course. We'll only go as far as you're comfortable with."

translate id w2BirthDayHanaPresent_7bf6e424:
    "I did my best to cover up my disappointment with a reassuring smile, which was returned with a relieved smile of the rocker girl's own."

translate id w2BirthDayHanaPresent_41d7f5d9:
    "We shared a few moments of silence between us, before things turned unbearably lewd again as Hana rocked her hips back and forth, using her panties and the outline of her twat to stimulate my near-painfully engorged dick."

translate id w2BirthDayHanaPresent_24f91bfe:
    hana "Ah...♥ You ever... done it like this before?"

translate id w2BirthDayHanaPresent_4901e12b:
    mc "N-no...! This is a first..."

translate id w2BirthDayHanaPresent_7b2c407e:
    hana "Me either, but it's really. Fucking. Hot."

translate id w2BirthDayHanaPresent_138379d5:
    "The spreading damp spot on her panties testified to that true enough."

translate id w2BirthDayHanaPresent_b58d1b17:
    hana "The only thing stopping your fat cock from gorging my insides is this little piece of fabric."

translate id w2BirthDayHanaPresent_7b5a22de:
    hana "Oooh..! Mmmh...♥ I can feel that bad boy twitch."

translate id w2BirthDayHanaPresent_6c423a94:
    "Was she getting off on denying herself or from teasing me? I didn't know which."

translate id w2BirthDayHanaPresent_58f4c62d:
    "Not that I cared, the feeling of the sheer material of her underwear felt amazing. Instead of the full, bloated feeling of penetrating a woman, the pleasure came in almost unbearably ticklish waves."

translate id w2BirthDayHanaPresent_ec4f7016:
    "The rough, non-patterned movements created a myriad of sensations. The material of her underwear, damp with her juices, would cling ever-so-slightly to my dick, at varying times, for varying lengths..."

translate id w2BirthDayHanaPresent_706c4c1e:
    hana "Ngh..! Y-you're feeling good, right?"

translate id w2BirthDayHanaPresent_c61be073:
    mc "I'm feeling amazing..."

translate id w2BirthDayHanaPresent_bee2d78d:
    hana "G-good! I'm glad...!"

translate id w2BirthDayHanaPresent_3c38a7d6:
    "The next swath of time saw us sharing in a a heavy silence, as we let all pretense of speaking fall away and be replaced by the sounds of sexual exhaustion."

translate id w2BirthDayHanaPresent_978d2ce3:
    "We just looked at one another."

translate id w2BirthDayHanaPresent_71c7e60c:
    "Hana's eyes would erratically dart between my face and the cock sitting between her legs, while I enjoyed the incredible view of her tits bouncing up and down."

translate id w2BirthDayHanaPresent_c04b74c7:
    "My mind invariably wandered to what it would feel like to be inside her. How tight she would feel, how needily her cunni would clamp down on my dick after a year of sexual starvation."

translate id w2BirthDayHanaPresent_0a2bd6aa:
    mc "Ah, H-hana!"

translate id w2BirthDayHanaPresent_5b384731:
    "I called out like a girl, the words reflexively forming and leaving my lips in an instant."

translate id w2BirthDayHanaPresent_4662cd8f:
    hana "Heh, I wonder if Goldilocks is hearing any of this?"

translate id w2BirthDayHanaPresent_88642b32:
    mc "Keep it down then!"

translate id w2BirthDayHanaPresent_6643ef2b:
    hana "Heh, maybe she's lying down there. Pretending to be asleep."

translate id w2BirthDayHanaPresent_49aabb82:
    hana "Maybe she's touching herself...♥ Does that excite you even more?"

translate id w2BirthDayHanaPresent_26aac6ef:
    "Hana was becoming more and more cocksure with herself."

translate id w2BirthDayHanaPresent_e877bdfe:
    mc "Fuck yeah, it does."

translate id w2BirthDayHanaPresent_465901b0:
    hana "I know, right? Mmh... it's easy to see... despite how cute she acts, she's horny as hell."

translate id w2BirthDayHanaPresent_ab6975ca:
    hana "I bet you want to teach her a few things, don't you? Put your hands on your best friend's girlfriend."

translate id w2BirthDayHanaPresent_7437809d:
    mc "...ooohm, n-no comment..."

translate id w2BirthDayHanaPresent_6b216c55:
    mc "I think the thought turns you on more than me, you dirty slut."

translate id w2BirthDayHanaPresent_0d0ad033:
    hana "Mmmh... can't say it wouldn't be... interesting to teach her a few things...♥"

translate id w2BirthDayHanaPresent_8a850203:
    hana "She's just so adorable... makes me want to... bully her...!"

translate id w2BirthDayHanaPresent_1ce20a34:
    mc "Right now... I just want to focus on me and you, Hana."

translate id w2BirthDayHanaPresent_922c3d6f:
    mc "You're b-beautiful..."

translate id w2BirthDayHanaPresent_1b74af33:
    hana "Nngh...ah, mmh...♥ Y-yeah, I am..."

translate id w2BirthDayHanaPresent_d72b05af:
    "Despite her words, she looked pleased to hear it."

translate id w2BirthDayHanaPresent_da2c42d4:
    "Hana picked up the pace, almost painfully so. The already uneven movements of her hips went feverishly erratic."

translate id w2BirthDayHanaPresent_008787a6:
    hana "[2]...!"

translate id w2BirthDayHanaPresent_2bf90118:
    "Her panties were {1}drenched{2} at this point, coated in a mixture of both our love juices. Her lower lips were so plump with arousal that it almost felt like she was wearing nothing at all."

translate id w2BirthDayHanaPresent_1d08b298:
    hana "{1}[2]...!{2}"

translate id w2BirthDayHanaPresent_ebf6815c:
    "The closer and closer I got to orgasm, the more it felt like I was actually inside and fucking her."

translate id w2BirthDayHanaPresent_f27be5d6:
    hana "{1}[2]...♥{2}"

translate id w2BirthDayHanaPresent_0a2ed76d:
    "It felt like my head was taking in individual parts of her body all at once."

translate id w2BirthDayHanaPresent_e5aaccae:
    "My eyes would dart from her sweat-lined tummy to the pendulous sway of her breasts."

translate id w2BirthDayHanaPresent_0cc615ad:
    "They would focus on the source of the action."

translate id w2BirthDayHanaPresent_1634e0ca:
    "They would even curiously marvel at the way her elbows bent and retracted as she used her body like a rocking horse."

translate id w2BirthDayHanaPresent_44277734:
    "Most of all, my eyes would always return to her face. The way she pursed her lips, the way it scrunched up in pleasure, and the way the pools of her eyes had swamped over with muggy sexual arousal. "

translate id w2BirthDayHanaPresent_c5f74c7a:
    "She felt like a work of art to me."

translate id w2BirthDayHanaPresent_fe2ea295:
    hana "Oh damn, I love the way you're looking at me!"

translate id w2BirthDayHanaPresent_0e1a9f2a:
    mc "Ngh...! D-don't stop...!"

translate id w2BirthDayHanaPresent_fc64bae6:
    hana "You're close?"

translate id w2BirthDayHanaPresent_779c051a:
    mc "Yes!"

translate id w2BirthDayHanaPresent_090a00d0:
    hana "How close? Tell me."

translate id w2BirthDayHanaPresent_ff44f57a:
    "Our voices had mostly been quiet up to this point, but now it dropped down to a sensual whisper."

translate id w2BirthDayHanaPresent_6acedc70:
    mc "Really, really close! Keep going! You're about to make me--!"

translate id w2BirthDayHanaPresent_9c63c5df:
    mc "Eugh~!"

translate id w2BirthDayHanaPresent_4d8ea76c:
    "In a flash of blinding pleasure, my balls churned violently and forced a stream of white-hot cum out through the opening of my engorged penis."

translate id w2BirthDayHanaPresent_55feb8f4:
    mc "--!"

translate id w2BirthDayHanaPresent_e7d490c3:
    "Through half-lidded eyes I watched my ejaculate rain down on the would-be rock star's lower half, semen spattering her stomach."

translate id w2BirthDayHanaPresent_3d5da914:
    hana "Is that a normal amount?"

translate id w2BirthDayHanaPresent_5826d9dc:
    "After a few blurry moments, my senses finally came back to me and I took a second to admire my - or rather her - handiwork."

translate id w2BirthDayHanaPresent_d41596b2:
    "A beautiful woman dripping with my cum... it filled me with perverse satisfaction and gave me an even more perverse idea."

translate id w2BirthDayHanaPresent_9fa49158:
    "Wordlessly, I raked the tip of my index finger across her stomach, coating it with my spunk and presented it to Hana."

translate id w2BirthDayHanaPresent_19da6283:
    hana "Eh? You want me to...?"

translate id w2BirthDayHanaPresent_6151b2c8:
    "I answered her question with a nod."

translate id w2BirthDayHanaPresent_cfa5079c:
    "For a brief moment, she looked unsure."

translate id w2BirthDayHanaPresent_af5514d8:
    hana "Not gonna lie, that seems kinda gross, but..."

translate id w2BirthDayHanaPresent_fd7bf07e:
    hana "Happy birthday."

translate id w2BirthDayHanaPresent_f92c2d6e:
    "With a cute wink, she went above and beyond my expectations, guiding my finger into her mouth up with one unbroken motion to the knuckle."

translate id w2BirthDayHanaPresent_f43b1a2e:
    hana "Mmmh...~♥"

translate id w2BirthDayHanaPresent_0a32c762:
    "Her tongue wrapped around the digit, lapping enthusiastically at the glob of cum."

translate id w2BirthDayHanaPresent_4795f62c:
    "It didn't take long for her to have it cleaned, but she didn't stop there."

translate id w2BirthDayHanaPresent_ea3890fe:
    "She made a show of fellating my finger, lewdly simulating a blowjob."

translate id w2BirthDayHanaPresent_3ad9a9fc:
    "Sucking, bobbing, using it to probe the inside of her cheeks..."

translate id w2BirthDayHanaPresent_04a3836e:
    "The sight caused another swell of desire in me. I wished for that to be my dick."

translate id w2BirthDayHanaPresent_0ccb4d8a:
    hana "*Pop* M'wah..!"

translate id w2BirthDayHanaPresent_566f776a:
    "Hana concluded her stimulatory fellatio with an exaggerated plop."

translate id w2BirthDayHanaPresent_24f857ac:
    hana "That was fun, [2]. Thanks for indulging me."

translate id w2BirthDayHanaPresent_5d999d2a:
    mc "Why are you talking like we're finished?"

translate id w2BirthDayHanaPresent_e08ad3c1:
    hana "You want to go again?"

translate id w2BirthDayHanaPresent_c2744ead:
    mc "You haven't cum yet."

translate id w2BirthDayHanaPresent_10fed2ff:
    hana "That's alright, I think--"

translate id w2BirthDayHanaPresent_648b7636:
    hana "Oohm...?!"

translate id w2BirthDayHanaPresent_ae7f6c77:
    "With a gentle, but quick push I put the rocker girl on her back."

translate id w2BirthDayHanaPresent_7bd12e75:
    "After the initial shock wore off, she peered up at me with growing need in her eyes."

translate id w2BirthDayHanaPresent_baf35f13:
    hana "Alright... have it your way."

translate id w2BirthDayHanaPresent_cbd2e8ec:
    hana "Hey, careful not to tear 'em! ...eh, you're cleaning it up with your shirt?"

translate id w2BirthDayHanaPresent_92176ae1:
    mc "Wouldn't be the first time."

translate id w2BirthDayHanaPresent_5c5b7fc6:
    "After playing a veritable game of twister with our limbs, I had Hana's sex completely exposed and in front of me."

translate id w2BirthDayHanaPresent_66d3536b:
    hana "Nnhee... your breath is... this position is..."

translate id w2BirthDayHanaPresent_7d01d465:
    "I had Hana right where I wanted her, a firm hand on her tush and back arched like a whore."

translate id w2BirthDayHanaPresent_4655830c:
    "The heat radiating from her twat was already obvious from this distance. I could only imagine how blazing she'll be to the touch..."

translate id w2BirthDayHanaPresent_1d085b4c:
    mct "(Well, what am I waiting for? It's an all you can eat buffet!)"

translate id w2BirthDayHanaPresent_7842602d:
    "I dived right into it. No need for testing the water with a few half-hearted licks, no... I was propelled by the singular desire to return the pleasure Hana gave in equal measure."

translate id w2BirthDayHanaPresent_d64761f4:
    hana "Mmmh..."

translate id w2BirthDayHanaPresent_b889dd9f:
    "No - I was going to return the pleasure she gave to me two fold."

translate id w2BirthDayHanaPresent_80268fd9:
    hana "You're not wasting any time-!"

translate id w2BirthDayHanaPresent_1933f9c7:
    "Sinking my hands deeper into her fat ass, I used my grip to get just the {3}right{4} angle, sending my tongue barreling into her opening with full gusto."

translate id w2BirthDayHanaPresent_e43948eb:
    "She was already so wet, primed and raring to go that it met little resistance on the way in."

translate id w2BirthDayHanaPresent_38d2c554:
    "Instead, she welcomed me fully. Her inner walls gently kissing my tongue, undulating like she was trying to swallow it whole."

translate id w2BirthDayHanaPresent_4f63d6d0:
    hana "Mmmh, mmhhh...♥"

translate id w2BirthDayHanaPresent_88a44000:
    hana "That's... that's nice, [2]."

translate id w2BirthDayHanaPresent_66094a87:
    "I thought the same too. It was heavenly being wedged between Hana's thighs. The sheer thrill of having such a headstrong woman practically bent over backwards in your hands..."

translate id w2BirthDayHanaPresent_27ceab9e:
    mct "(Fuck, I'm so turned on!)"

translate id w2BirthDayHanaPresent_e462f9d5:
    "Desire swelled in me."

translate id w2BirthDayHanaPresent_46a96e2c:
    hana "Oh, [2]...!"

translate id w2BirthDayHanaPresent_5980f3e6:
    "I intended to make the most of this chance. Who knows if Hana and I would ever do anything like this again after all?"

translate id w2BirthDayHanaPresent_875a6632:
    hana "*pant* Ha... ha...!"

translate id w2BirthDayHanaPresent_ec16f427:
    "I would use my tongue to make these feelings clear to her."

translate id w2BirthDayHanaPresent_9ad0d32e:
    hana "Mmmh, ha... ha...♥"

translate id w2BirthDayHanaPresent_247af629:
    "I reached as far back as I could and then some, pulling my tongue back in alternating shallow and deep motions, probing and tasting every inch of her sex."

translate id w2BirthDayHanaPresent_24d6c5b2:
    hana "Oh, fuck I really did need this! Don't stop, please."

translate id w2BirthDayHanaPresent_73016610:
    hana "Don't stop until I cum, okay? Please?"

translate id w2BirthDayHanaPresent_9ec7c388:
    "There was a heavy and torrid twang in her voice."

translate id w2BirthDayHanaPresent_d113b8f0:
    "I wanted to scoff that she didn't have to ask, but that would needlessly get in the way of tongue fucking her."

translate id w2BirthDayHanaPresent_813577d8:
    hana "Ohoho....!"

translate id w2BirthDayHanaPresent_9049ce04:
    "I could feel Hana unconsciously begin to rock her hips, her body trembling in my hands."

translate id w2BirthDayHanaPresent_52766680:
    "I scraped and I scraped, I teased, I licked, I caressed, grazed, fondled her all with my mouth."

translate id w2BirthDayHanaPresent_63dd103b:
    hana "Ehehe...!"

translate id w2BirthDayHanaPresent_66ff2073:
    "I intermittently changed my focus, switching between her love tunnel and giving the engorged, pearly pink nub the attention it deserved."

translate id w2BirthDayHanaPresent_57898555:
    hana "F-fuck me, that's...!"

translate id w2BirthDayHanaPresent_eb83d13a:
    "Her clit was fully out of its hood and I used everything in my toolbox to tease it. Tongue, nose, even going so far as to graze it ever-so-slightly with my teeth."

translate id w2BirthDayHanaPresent_ec036923:
    hana "Emmh-♥"

translate id w2BirthDayHanaPresent_6761c3d6:
    "Hana chomped down on her hand, using it as a makeshift gag to stifle her moans. A conscientious effort that I appreciated, given Ian and Mina were not 20 feet away downstairs."

translate id w2BirthDayHanaPresent_ba9c26a2:
    "The thought was a salacious one, wasn't it?"

translate id w2BirthDayHanaPresent_3f5c662f:
    "My sex-addled lizard brain wanted to imagine the bubbly blonde knuckle deep in her own snatch listening to us go at it. Of course, that was just a fantasy, but still..."

translate id w2BirthDayHanaPresent_61fe91b1:
    mct "(That's so hot...!)"

translate id w2BirthDayHanaPresent_f7eb7b0c:
    hana "Mmmfhh-!"

translate id w2BirthDayHanaPresent_cedb81aa:
    "With a violent jerk, Hana arched her back even further, wrapping her legs around my head and clamping down to pull me even impossibly deeper into her snatch."

translate id w2BirthDayHanaPresent_0a267976:
    hana "C-chhmhm-!"

translate id w2BirthDayHanaPresent_33b2f4c3:
    hana "C-chhmhm-! C-chhmhm-!"

translate id w2BirthDayHanaPresent_db66c423:
    mct "(She's close!)"

translate id w2BirthDayHanaPresent_9f1c9d57:
    "I extended and twisted my tongue to the point of soreness, spurred on by knowing how close she was."

translate id w2BirthDayHanaPresent_078abf92:
    "At a certain point, all thought disappeared from my head. It was just strenuous focus on a singular task."

translate id w2BirthDayHanaPresent_b8bbd97a:
    "I was no longer [2], the student."

translate id w2BirthDayHanaPresent_36eef2b9:
    hana "Gh..mmmffh...! Mffhh...!♥"

translate id w2BirthDayHanaPresent_b232d8f8:
    "I was [2], the tongue."

translate id w2BirthDayHanaPresent_436754b5:
    hana "Nggghh......!!!!♥"

translate id w2BirthDayHanaPresent_41c12eb0:
    "Hana clamped down around my neck even tighter, cutting off my airflow and signaling with gusto the arrival of her orgasm."

translate id w2BirthDayHanaPresent_6d52f342:
    mc "Ghmm-!"

translate id w2BirthDayHanaPresent_e459db1b:
    "Equally, the inner walls of her love hole tugged at the length of my tongue, squeezing and massaging the wiry, slimy muscle like it was a cock to be milked."

translate id w2BirthDayHanaPresent_c11c7122:
    "Just for a moment, breathing on both of our sides ceased - Hana in the throes of sexual release and me, head crushed between the rocker girl's pale thighs."

translate id w2BirthDayHanaPresent_de7c188e:
    hana "Geh...!"

translate id w2BirthDayHanaPresent_393ca7af:
    "Just like that it was over."

translate id w2BirthDayHanaPresent_23033fa8:
    "A sudden, short, and sharp orgasm and Hana released me from her hold."

translate id w2BirthDayHanaPresent_ec9633cb:
    "Breathing resumed. On both of our ends."

translate id w2BirthDayHanaPresent_5104a809:
    "Erratic, short, soft breaths."

translate id w2BirthDayHanaPresent_6ce90603:
    hana "Good o-one, [2]. That was...."

translate id w2BirthDayHanaPresent_9a3da0c9:
    hana "Awesome."

translate id w2BirthDayHanaPresent_184b5e48:
    mc "C'mere you..."

translate id w2BirthDayHanaPresent_6a1ed6ac:
    hana "Mmmh, cuddles..."

translate id w2BirthDayHanaPresent_3dbc3171:
    "Pulling her close so we could share in a collected post-coital bliss, Hana snuggled next to me."

translate id w2BirthDayHanaPresent_931105f4:
    mc "Didn't take you for the type."

translate id w2BirthDayHanaPresent_e2b72283:
    hana "Eh? EVERYONE loves cuddling, numbskull."

translate id w2BirthDayHanaPresent_6115d4b0:
    mc "That's not true. Try to picture Mrs. Pulman..."

translate id w2BirthDayHanaPresent_7327c896:
    hana "First of all, gross."

translate id w2BirthDayHanaPresent_0dc342e1:
    hana "Secondly... anyone who's NOT a vampire likes to cuddle."

translate id w2BirthDayHanaPresent_d44eb9d5:
    "She rubbed her face into my chest like some kind of cat."

translate id w2BirthDayHanaPresent_286591de:
    hana "Let's stay like this awhile, please..."

translate id w2BirthDayHanaPresent_dd363cdc:
    mc "Yeah, of course..."

translate id w2BirthDayHanaPresent_0e81c6b8:
    mc "Is everything okay with you?"

translate id w2BirthDayHanaPresent_502604ab:
    hana "Why do you ask?"

translate id w2BirthDayHanaPresent_acac0177:
    mc "Just felt like the question to ask."

translate id w2BirthDayHanaPresent_93c86429:
    hana "Mmmh... yeah, I'm fine. Just the usual stress in my life."

translate id w2BirthDayHanaPresent_431830ce:
    mc "Want to tell me about it?"

translate id w2BirthDayHanaPresent_c319d33c:
    hana "Not right now, but... maybe later?"

translate id w2BirthDayHanaPresent_0af2ffad:
    mc "I'd be happy to hear it."

translate id w2BirthDayHanaPresent_6ad6f50a:
    hana "...thanks, [2]."

translate id w2BirthDayHanaPresent_596a936d:
    "Just like that, we fell into a comfortable silence."

translate id w2BirthDayHanaPresent_3ec15445:
    "So comfortable that..."

translate id w2BirthDayHanaPresent_278c7e29:
    "........."

translate id w2BirthDayHanaPresent_5e9fe091:
    mc "......."

translate id w2BirthDayHanaPresent_81d1cf98:
    hana "..."

translate id w2June08Start_6b62f65a:
    mc "Why do I have to apologize? They had him pinned on the ground!"

translate id w2June08Start_38ec0c3e:
    vic "It's good that you wanted to stand up for your friend, but there's such a thing as a disproportionate response. Do you know what that means, [2]?"

translate id w2June08Start_1a219cf6:
    mc "Ummm..."

translate id w2June08Start_3e0cb855:
    vic "That's okay. It's a big word, isn't it?"

translate id w2June08Start_2c626205:
    vic "Let's say I grounded you for a month and took away your TV privileges for chewing with your mouth open. How would that make you feel?"

translate id w2June08Start_3d47fc6c:
    mc "Angry..."

translate id w2June08Start_05699fb5:
    vic "Exactly. It wouldn't be fair. The punishment doesn't fit the crime."

translate id w2June08Start_3dfb3ee1:
    vic "Do you see what I'm getting at? A rock is a dangerous weapon, it can seriously hurt or kill someone."

translate id w2June08Start_5d5b546f:
    vic "You're lucky that boy only needed a few stitches and that his father is a reasonable man, but you've got to go to his house and apologize in person."

translate id w2June08Start_e488ec6d:
    mc "But, mom... I don't FEEL sorry about it. It even felt good to--"

translate id w2June08Start_7ac8dd04:
    vic "I know, and that's something you and I will work on together, but for now you need to at least pretend you're sorry."

translate id w2June08Start_833a2088:
    mc "You want me to lie?"

translate id w2June08Start_ba3df2b1:
    vic "You've got to pretend. I know it's not something a good mother would say, but it's the only way to keep the school out of it. You could get expelled."

translate id w2June08Start_a10527fd:
    mc "I understand. I'll do it."

translate id w2June08Start_702d603f:
    vic "Thank you, [2]."

translate id w2June08Start_a20cefa7:
    "..."

translate id w2June08Start_5adf8148:
    mc "Ummm, Mom..."

translate id w2June08Start_93a55853:
    vic "Yes, dear?"

translate id w2June08Start_2c4d0071:
    mc "Am I bad person if I liked hitting him?"

translate id w2June08Start_d31908d6:
    vic "[2]... that's..."

translate id w2June08Start_7e67b076:
    vic "A person can't help how they feel. What they can control is what they choose to do."

translate id w2June08Start_3bf08ab5:
    vic "Your heart was in the right place in wanting to help Ian. That's one of the things I love the most about you."

translate id w2June08Start_08a93a1b:
    vic "Your father was the same way with his nonprofit work. It's just... if there's no adults around to help you, use your... fists?"

translate id w2June08Start_4a9cb735:
    mc "So punching them is not a disproportionate response?"

translate id w2June08Start_032272ed:
    vic "{5}Oh god, I'm so bad at this...{6}"

translate id w2June08Start_b6e748a7:
    mc "Okay, I'll punch them next time."

translate id w2June08Start_d934a894:
    vic "No, I didn't mean-- mmmg, o-only if there isn't an adult to help and it's your only option!"

translate id w2June08Start_0fd4d656:
    "......"

translate id w2June08Start_a20cefa7_1:
    "..."

translate id w2June08Start_0611e7d1:
    mc "Mmmh..."

translate id w2June08Start_d4545b0b:
    "The morning rolled around in an unexpectedly quiet way for having three house guests."

translate id w2June08Start_926b8892:
    "My eyes opened on their own, not pried open by any commotion or noise."

translate id w2June08Start_7b694ba3:
    mct "(Hana must already be up.)"

translate id w2June08Start_408c9a71:
    mct "(...or maybe she's already left?)"

translate id w2June08Start_eb7ec79f:
    mct "(Ian and Mina are nowhere in sight, either.)"

translate id w2June08Start_597f6457:
    mct "(Well, at least one of them is in the shower.)"

translate id w2June08Start_cceb385c:
    mct "(I'd be surprised if it was all three...)"

translate id w2June08Start_610a6cac:
    mct "(Whoever it is, will be greeted with a hot cup of coffee when they get out.)"

translate id w2June08Start_0fd4d656_1:
    "......"

translate id w2June08Start_a20cefa7_2:
    "..."

translate id w2June08Start_473c8bbd:
    mct "(...oh?)"

translate id w2June08Start_e48006c8:
    "It seems Ian left me a text message while I slept."

translate id w2June08Start_fc15390c:
    "It read: \"I wanted to give you two lovebirds some space, so Mina and I split. Just don't forget your weekly meeting with Kat today."

translate id w2June08Start_f8f27675:
    mct "(Oh, give me a break...)"

translate id w2June08Start_f8f27675_1:
    mct "(Oh, give me a break...)"

translate id w2June08Start_16083738:
    mct "(He misunderstood things.)"

translate id w2June08Start_f8f27675_2:
    mct "(Oh, give me a break...)"

translate id w2June08Start_047456ff:
    mct "(He's still misunderstanding things...)"

translate id w2June08Start_b0296a70:
    mct "(Right though, it is Monday. Which means I need to be at the Club by noon.)"

translate id w2June08Start_69c98865:
    mct "(It's when we all learn what the week's theme will be.)"

translate id w2June08Start_5637c46c:
    hana "*yawn* Moooorning."

translate id w2June08Start_2eede6e8:
    mc "Hey. I made some coffee. Take it."

translate id w2June08Start_baad00cd:
    mc "Hey. I made some coffee. Want some?"

translate id w2June08Start_6e431689:
    hana "I used some of your body wash, I hope you don't mind."

translate id w2June08Start_3dc19a55:
    hana "Definitely needed it after the mess we made."

translate id w2June08Start_83dbf714:
    mc "Heh, yeah... of course I don't mind. Did you sleep well?"

translate id w2June08Start_c504d0d1:
    hana "Hehehe, after what you did to me last night, what do you think?"

translate id w2June08Start_adf730ec:
    hana "Thanks for the coffee."

translate id w2June08Start_1e170ffa:
    hana "Like a baby. Last night was... nice."

translate id w2June08Start_adf730ec_1:
    hana "Thanks for the coffee."

translate id w2June08Start_b60adb55:
    hana "Yep, all good. Thanks for letting me use your bed."

translate id w2June08Start_426d70df:
    mc "I was just being a good host like I was taught."

translate id w2June08Start_84de620d:
    hana "Thanks for the coffee too."

translate id w2June08Start_cabdefcb:
    mc "Y'know, Miss Rhodes... you're quite the polite young woman."

translate id w2June08Start_afa875e5:
    hana "Oh, shut up. Did you take me for some sort of ingrate?"

translate id w2June08Start_fca01aee:
    mc "You do cultivate a certain persona..."

translate id w2June08Start_d043547c:
    hana "Okay, well fuck you for the coffee then."

translate id w2June08Start_f3ca80b1:
    mc "That's more like it."

translate id w2June08Start_81d1cf98:
    hana "..."

translate id w2June08Start_49606f8b:
    "Hana looked down at her coffee as if suddenly lost in thought."

translate id w2June08Start_96235f48:
    mc "Is everything okay?"

translate id w2June08Start_9c061037:
    hana "Yeah, it is, you're not going to be weird about this going forward, right?"

translate id w2June08Start_fe972dba:
    mc "Are you asking if I think you're my girlfriend now? Don't be silly."

translate id w2June08Start_d58cc1c5:
    hana "Oh? Would that be silly?"

translate id w2June08Start_8c2e2faf:
    "She had a blank expression that was hard to read."

translate id w2June08Start_28ec021c:
    mc "No, it's just I know we were both just a little drunk and horny."

translate id w2June08Start_a288aa01:
    hana "Actually, I don't normally do that even if I'm shitfaced. I'm usually a more traditional kind of girl..."

translate id w2June08Start_409b0491:
    hana "Yeah, uh... I just hope you don't think I'm a weirdo for what I tried to do last night."

translate id w2June08Start_81d1cf98_1:
    hana "..."

translate id w2June08Start_70f1ab6c:
    "A thick silence hung in the air between us as I considered my words."

translate id w2June08Start_dc2b7f2b:
    mc "Hey, not at all... I just hope you're not insulted I turned you down."

translate id w2June08Start_02283c1c:
    hana "No, I get it. I don't usually just throw myself at someone I know a few weeks. I'm actually a more traditional kind of girl..."

translate id w2June08Start_17148f8c:
    hana "I owe that to being raised by a woman like my mom I guess."

translate id w2June08Start_5715bece:
    mc "Would you tell me a little about her? I'm a bit curious, given our similar childhoods."

translate id w2June08Start_a2978115:
    hana "I guess I wouldn't mind talking about her, but it's kind of a bummer for morning coffee. You sure you want to hear it?"

translate id w2June08Start_55e30427:
    "I gave Hana the go-ahead to talk my ear off with a nod."

translate id w2June08Start_0870494b:
    hana "Okay..."

translate id w2June08Start_5830f0c1:
    hana "You remember what I said about August and her sickness?"

translate id w2June08Start_41fae2a0:
    mc "You said she had a neurodegenerative disease, right?"

translate id w2June08Start_46f6bcf0:
    hana "Uh huh. Huntington's. You know anything about it?"

translate id w2June08Start_6703f2fa:
    mc "A little..."

translate id w2June08Start_a7929b55:
    "Huntington's disease was a terrible thing, but the most sobering and pressing thing about it at this very moment was..."

translate id w2June08Start_81d1cf98_2:
    hana "..."

translate id w2June08Start_282d0139:
    mc "Do you have...?"

translate id w2June08Start_8fb79ef1:
    hana "No, thank god. I got lucky. Fifty fifty shot of her passing down the gene and it missed me. It's about the only thing I'm thankful to August for."

translate id w2June08Start_7f466f77:
    "Hana continued to stare a hole in the surface of her drink, before looking up and smiling at me."

translate id w2June08Start_7c1cbdc1:
    hana "That part is kind of funny in a cosmic, fucked up \"God laughs\" sort of way."

translate id w2June08Start_f366d26f:
    "All I did was simply return her smile. No doubt she'd heard the usual platitudes in droves. Stuff like \"I can't imagine what you're going through\" or something else as equally vacant."

translate id w2June08Start_1e8886b4:
    mc "How bad is she?"

translate id w2June08Start_ce6a82f8:
    hana "She had her first onset of symptoms in her mid 30s. Some coordination loss, depression, irritability..."

translate id w2June08Start_e5a35dad:
    hana "It got bad a year and a half ago. Difficulty walking, memory loss, trouble speaking... that's when she had to get live-in care. I couldn't cut it by myself anymore."

translate id w2June08Start_8f633ba4:
    mc "That's what August pays for?"

translate id w2June08Start_f2992ecc:
    hana "Among other things. He takes care of some of her other bills too."

translate id w2June08Start_6eee404c:
    mc "Does she know?"

translate id w2June08Start_1a649887:
    hana "Fuck, no. She'd absolutely refuse his help - which I get, trust me, but there's also being pragmatic."

translate id w2June08Start_ef1d3a91:
    hana "Do you think I'm wrong to hide that from her?"

translate id w2June08Start_2d42601f:
    mc "No. You should do what you've got to do to keep her comfortable and make things easier for you."

translate id w2June08Start_feff33c5:
    hana "Thanks. That's what Jerrica says too, but it's more convincing coming from a person who hasn't pushed a car into a lake to defend my honor."

translate id w2June08Start_319fa36c:
    "I raised a curious eyebrow."

translate id w2June08Start_6b0c3090:
    hana "Hehe, yeah. She's my ride-or-die bitch. Maybe I'll tell you about it later?"

translate id w2June08Start_b97bf2e6:
    mc "Yeah, I'd like to hear THAT story."

translate id w2June08Start_85aa935f:
    hana "She's usually all \"fuck what she wants\", but she's blinded by her concern for me."

translate id w2June08Start_636f2a87:
    hana "I don't think she fathoms how scary it must be to slowly lose your ability to walk or the sheer terror and confusion at not being able to speak sometimes."

translate id w2June08Start_f077697b:
    mc "Was that what was on your mind last night?"

translate id w2June08Start_5690f83e:
    hana "Uh huh, we had a fight about something stupid and small yesterday... oh!"

translate id w2June08Start_3d5cfdbe:
    hana "Please don't get the wrong idea. I didn't jump your bones just because I was feeling bad and you were a convenient lay."

translate id w2June08Start_ecf63793:
    hana "Please don't get the wrong idea. I didn't try and jump your bones just because I was feeling bad and you were a convenient lay.."

translate id w2June08Start_94d14e19:
    hana "I wouldn't just do that with anyone. I do think you're cool."

translate id w2June08Start_fd6f4245:
    mc "You do, do you?"

translate id w2June08Start_6e2f3efa:
    hana "Ugh, that fucking look. I'm not saying I want to date you, it's just something clicked the other night after that run in with Warren and I just thought we could both have some fun last night."

translate id w2June08Start_8953106c:
    mc "So I guess you don't mind if I tell people we're friends then?"

translate id w2June08Start_62645463:
    hana "I can live with that."

translate id w2June08Start_0fb801cc:
    "Hana and I shared a moment of solidarity. I wasn't really sure of the path we were going down. Friend or something more, but it felt good."

translate id w2June08Start_80401f57:
    hana "So, what's on your busy schedule today? Should I get out of your hair?"

translate id w2June08Start_2a4f8c79:
    hana "Yep, everything is fine. What are you getting up to today? Should I get out of your hair?"

translate id w2June08Start_2003a303:
    mc "I have to be at the club by noon. There's a weekly meeting about the exhibition."

translate id w2June08Start_c987434c:
    hana "Blech."

translate id w2June08Start_7d700aca:
    hana "What could you possibly need to meet about? You got graphs and pie charts about the number of times those old fucks cream their pants?"

translate id w2June08Start_9fb02dbc:
    mc "I haven't seen that so far, but it's still early in the month."

translate id w2June08Start_ae94c7a2:
    mc "{5}Wouldn't be surprised, though...{6}"

translate id w2June08Start_864fa911:
    hana "Noon? You don't have a car, right?"

translate id w2June08Start_02c7389b:
    hana "Want a ride? I've got to go in sometime today and speak with the old man. Might as well be then."

translate id w2June08Start_1ff164b0:
    mc "Really? Yeah, that'd be great."

translate id w2June08Start_81601384:
    hana "Alright, then. Guess you need to get ready?"

translate id w2June08Start_bef14604:
    mc "Yeah, I'll take a shower first. Just make yourself at home."

translate id w2June08Start_6809579c:
    hana "Will do."

translate id w2June08Start_91047a3a:
    ".........."

translate id w2June08Start_0fd4d656_2:
    "......"

translate id w2June08Start_a20cefa7_3:
    "..."

translate id w2June08Start_3e26eaf1:
    "After a trek up to the top, Hana and I found Warren in Jacob's usual spot."

translate id w2June08Start_e8f96560:
    "The way Warren asked if we came in together bugged me for some reason."

translate id w2June08Start_a5c0cb2b:
    war "Meeting's in the studio this week, kid."

translate id w2June08Start_285bcd48:
    mc "How come?"

translate id w2June08Start_3119b053:
    war "Head back down and find out."

translate id w2June08Start_d1d5453d:
    hana "Is the old man in his office?"

translate id w2June08Start_8d95c989:
    war "As far as I know. Want me to check for you?"

translate id w2June08Start_6fa64642:
    hana "No, I'll just knock on the door myself."

translate id w2June08Start_895d3583:
    war "I was hoping you'd say that."

translate id w2June08Start_42add13e:
    "With that, Hana and I split off and I made my way to the room where I interviewed the girls earlier last week."

translate id w2June08Start_8f225a7b:
    "With that, Hana and I split off and I made my way to the room where I interviewed Rosalind earlier last week."

translate id w2June08Start_279c75e2:
    "With that, Hana and I split off and I made my way to the room where I interviewed Felicia earlier last week. "

translate id w2June08Start_af4e1a4d:
    "With that, Hana and I split off and I made my way to the room where I interviewed Veronica earlier last week. "

translate id w2June08Start_ec8fac09:
    "In the studio, stood Kat. All by her lonesome."

translate id w2June08Start_fcda2907:
    kat "Good afternoon, [2]. You're right on time."

translate id w2June08Start_9f7b3fc3:
    "Without even looking at me, she eerily greeted me by name as if she had eyes on the back of her head."

translate id w2June08Start_7f9900d3:
    mc "Is it just me so far?"

translate id w2June08Start_03bec1f3:
    kat "The girls have already arrived and are getting dressed as per my instructions. How are you doing today?"

translate id w2June08Start_d45442f7:
    mc "Pretty good actually, thanks for asking."

translate id w2June08Start_5cc9506b:
    kat "I'm happy to hear that. Since yesterday was your birthday, you must've spent it wisely."

translate id w2June08Start_2c922d41:
    kat "One should celebrate their youth while they have the chance."

translate id w2June08Start_c046aa36:
    mc "That's right, it was. I had lunch with my mom and then spent the evening with Ian and Hana."

translate id w2June08Start_26a6e696:
    mc "Speaking of which..."

translate id w2June08Start_449a3a93:
    "The conversation I had with my mother yesterday struck me like a bolt of lightning and sparked a fit of anger."

translate id w2June08Start_a9dbac24:
    kat "You suddenly look agitated. Is something wrong?"

translate id w2June08Start_7a387e1d:
    mct "(Am I really making it that obvious...?)"

translate id w2June08Start_989f835c:
    "Whatever. Putting that aside, her trespass into my personal life wasn't something I could simply overlook. I had to bring it up and say something."

translate id w2June08Start_a9ec13f4:
    mc "There is, actually. Your bit of subterfuge the other day with my mother."

translate id w2June08Start_baecda10:
    mc "What was that about, Mrs. Pulman?"

translate id w2June08Start_c98b1fe7:
    "I swallowed my anger and delivered the question with a steady voice."

translate id w2June08Start_4eef46e1:
    kat "Oh, that?"

translate id w2June08Start_90e07f44:
    mc "Yes. {1}That{2}."

translate id w2June08Start_b7d1ca76:
    kat "Mmmh, you hide it well [2], but I can tell that underneath that placid expression you're angry with me."

translate id w2June08Start_f800bed0:
    mc "Yes."

translate id w2June08Start_65ac0de4:
    kat "I love that look in your eye by the way. It reminds me of the suffocating silence just before a lioness snatches up its prey. You're not going to gobble me up, are you?"

translate id w2June08Start_c142fdff:
    mc "Can't you just answer me like a normal person"

translate id w2June08Start_ac96e94b:
    kat "Sorry. You have every right to be mad with me."

translate id w2June08Start_5a860251:
    kat "Your work for the club and private life are separate things. From your perspective, I had no business crossing the two all willy nilly like that."

translate id w2June08Start_3df270b7:
    kat "Please understand, I didn't do it to get a rise out of you nor did I expect it to be a secret that wouldn't get back to you."

translate id w2June08Start_b99c838d:
    mc "Yeah, I was just wondering what business it is of yours to be calling my mother with some bullshit excuse."

translate id w2June08Start_c0d28c34:
    mc "What was that about, {3}Kathleen{4}?"

translate id w2June08Start_8eacb615:
    "I decided here and now that some transgressions fall outside the need for carefully considered, pro-and-con, calculated reaction."

translate id w2June08Start_4eef46e1_1:
    kat "Oh, that?"

translate id w2June08Start_c6761c70:
    mc "That...?!"

translate id w2June08Start_8f5d58d5:
    kat "Mmmh, that look in your eyes..."

translate id w2June08Start_708dd673:
    kat "You're very angry with me, aren't you?"

translate id w2June08Start_4deae454:
    mct "(No shit...)"

translate id w2June08Start_584ade6b:
    kat "Interesting."

translate id w2June08Start_e4068727:
    mc "What is?"

translate id w2June08Start_4dd15708:
    kat "You look like you want to smack me and put me in my place, which is... fair."

translate id w2June08Start_5a860251_1:
    kat "Your work for the club and private life are separate things. From your perspective, I had no business crossing the two all willy nilly like that."

translate id w2June08Start_6fc76a7e:
    mc "I think you just like toying with people and pushing their buttons."

translate id w2June08Start_a8290626:
    kat "There is truth in that accusation. That's my job here after all, but please understand my phone call to your mother wasn't me twisting your screws."

translate id w2June08Start_dbefc1ae:
    mc "Why did you do it then?"

translate id w2June08Start_c1fc866e:
    kat "I was turning over some rocks, looking to glean a little more about you."

translate id w2June08Start_9d83a412:
    mc "Was the background check you ran not enough?"

translate id w2June08Start_d5fcce74:
    kat "You can only learn so much from a piece of paper. Hearing it straight from a mother's mouth has its own advantages."

translate id w2June08Start_f47ec1c4:
    mc "What were you hoping to learn?"

translate id w2June08Start_4b66a9ec:
    kat "Anything that could be useful in knowing how to motivate and manage you. Normally I wouldn't just come out and say it, but I'm well aware of how your mind works."

translate id w2June08Start_814da1c2:
    kat "You do your due diligence in securing your own best interests, so I hope you would respect me doing the same."

translate id w2June08Start_a3a45a09:
    mc "I'm sorry, but \"manage\" me? Why would you ever...?"

translate id w2June08Start_0e406adb:
    kat "Darius... the answer to your question is {1}Darius{2}. The last person who had your job {1}blackmailed{2} me."

translate id w2June08Start_dddb6f7e:
    mc "........."

translate id w2June08Start_3b21f354:
    mc "......"

translate id w2June08Start_2b35b6fe:
    mc "..."

translate id w2June08Start_55feb8f4:
    mc "--!"

translate id w2June08Start_1220cbf1:
    "A chill ran down my spine as my mind drew what felt like a foregone conclusion."

translate id w2June08Start_ac49d3ba:
    chuck "The lad who had your job before you, Darius, used to live here as well - before he suddenly disappeared on us, that is."

translate id w2June08Start_d00abbc0:
    mc "He blackmailed you and just... \"disappeared\"?"

translate id w2June08Start_5f5bb4eb:
    kat "Oh...?"

translate id w2June08Start_d61ef90f:
    kat "Hahaha! Oh! Seriously?"

translate id w2June08Start_3edd5e94:
    kat "I'm glad you think I'm capable of something like that, but no. I didn't say he {3}tried{4} to blackmail me. He succeeded"

translate id w2June08Start_e5df2b50:
    kat "He up and absconded after he got what he was after. Basically, he fell in love and wanted to void an agreement we had with one of the whores."

translate id w2June08Start_21bb7156:
    mct "(Why was she telling me this...?)"

translate id w2June08Start_ab70cd70:
    "I traced the lines of her face in my mind, looking to discern what lurked behind her composure."

translate id w2June08Start_cc0328bf:
    kat "A trifling reason for such a monumentally stupid action, but a lesson learned for me. A fool will always find a way to surprise you..."

translate id w2June08Start_6066cd38:
    kat "As far as August and Chuck are concerned, he simply left. August would've overreacted if he knew the details of his departure."

translate id w2June08Start_2779d9db:
    mct "(Dealing with this woman was exhausting.)"

translate id w2June08Start_8ebd242e:
    mc "I don't give a damn why you made the call, just don't poke your nose into my family life."

translate id w2June08Start_5b43feaf:
    mc "I don't care why you made the call, just don't go poking around in my family life."

translate id w2June08Start_27d89d66:
    mc "As good as this job is, that's non-negotiable for me."

translate id w2June08Start_6770345e:
    kat "One conversation with your lovely mother was enough. On the plus side, it did help me come up with a present for-"

translate id w2June08Start_2ad3d767:
    fel "Got to say, Kat, you've got some interesting pieces in your wardrobe."

translate id w2June08Start_1cf752b5:
    fel "You wear this at home for Judge Hubby?"

translate id w2June08Start_14f257ca:
    "Felicia made her appearance adorned in a mouth-droppingly skimpy..."

translate id w2June08Start_319033c0:
    mct "(Is that even a bikini?)"

translate id w2June08Start_a8619713:
    "Whatever you'd call it, the small straps of embossed material that nearly constituted the entire outfit were tautly stretched across her torso, held up from the sheer shapeliness of her curves."

translate id w2June08Start_d8faa995:
    kat "Wouldn't you like to know what I get up to."

translate id w2June08Start_a689f5b3:
    fel "I'm sure you get into all kinds of freaky stuff."

translate id w2June08Start_8d61681f:
    "*Clank*"

translate id w2June08Start_867b7569:
    mct "(...eh?)"

translate id w2June08Start_cbcab301:
    kat "I'm glad your sense of humor is still intact after the last exhibition."

translate id w2June08Start_79454251:
    fel "Why wouldn't it be? I barely got to participate."

translate id w2June08Start_c2da239a:
    fel "The second game was pretty rough, I'll give you that, but if that was only the first week..."

translate id w2June08Start_67e21889:
    "*Clank* *Clank*"

translate id w2June08Start_07ba248d:
    "A tin-like ring grew less and less distant."

translate id w2June08Start_a55ca905:
    mct "(Is that a... bell?)"

translate id w2June08Start_fdcdc052:
    "*Clank* *Clank* *Clank*"

translate id w2June08Start_a0061924:
    fel "I'm looking forward to hearing what's next."

translate id w2June08Start_a208e01a:
    kat "You won't be waiting long, since it seems the other two were on your heel."

translate id w2June08Start_55feb8f4_1:
    mc "--!"

translate id w2June08Start_9ee428a8:
    "Where Felicia was risqué, the other two were equally ridiculous but in different ways."

translate id w2June08Start_ff5237c3:
    "From a glance, the three outfits had nothing else in common. If there was a unifying theme here, it was lost on me."

translate id w2June08Start_eae2fe50:
    "Rosalind, at least when compared to Felicia, wore a moderately modest bikini. The kicker was the cow print pattern and the bell draped from a leather collar around her neck."

translate id w2June08Start_2a61efd1:
    "Veronica wore an overly cute, obnoxiously pink set of lingerie accented with numerous pearlescent beads. It was, to say the least, totally incongruent with her fiery personality and Amazonian build."

translate id w2June08Start_c8c4a4e9:
    "Both of them wore muddied expressions. Rosalind looked like she was trying to hide her unease, but Veronica set her eyes straight ahead, in an alluring mix of embarrassment and anger."

translate id w2June08Start_4e8940ab:
    rose "Ngh... what was... wrong with the bunny costume?"

translate id w2June08Start_699735b5:
    kat "Don't be so self-conscious dear. You look adorable."

translate id w2June08Start_82e73a0a:
    kat "You remind me of the cute Holstein dairy cow we had on our ranch growing up. Mother gave her to me as a pet."

translate id w2June08Start_0407acea:
    kat "Unlike you however, she was wickedly smart. She was always trying to escape."

translate id w2June08Start_c0b01e40:
    ver "I'm sorry you asked."

translate id w2June08Start_efd82ed0:
    kat "How do you find your attire, Miss Lynch? I thought you could use a touch of femininity in your life."

translate id w2June08Start_188becc8:
    ver "You're not going to get a rise out of me."

translate id w2June08Start_29aa6be9:
    kat "Why are you blushing then dear?"

translate id w2June08Start_b524ab48:
    fel "That looks really good on you, Red! You think so too, right [2]?"

translate id w2June08Start_5cdb6af9:
    "Jumping in to beat back Veronica's self-consciousness, Felicia tossed a question in my lap like a game of hot potato and brought me into a conversation I was happily staying out of."

translate id w2June08Start_27307fac:
    mc "I agree, she looks smoking hot."

translate id w2June08Start_1394f087:
    mc "I agree, I think she looks cute."

translate id w2June08Start_8dab2be7:
    ver "Pssh, who asked you?"

translate id w2June08Start_83458ac2:
    mc "Felicia did."

translate id w2June08Start_0b1ba6cf:
    ver "So she did..."

translate id w2June08Start_16ea5a47:
    ver "Gee, thanks."

translate id w2June08Start_e4c8f983:
    fel "So... We're all here then?"

translate id w2June08Start_c58f91cd:
    kat "Not all. I sent Ian to grab me a coffee, but I can begin explaining this week's theme and then I'll tease your little \"side\" mission."

translate id w2June08Start_5dc11fd9:
    "That last bit perilously stuck out like a sore thumb."

translate id w2June08Start_6fd13522:
    kat "Take a seat, girls."

translate id w2June08Start_e5c069bb:
    kat "Good. Now, this week's theme is a fun one... {1}shame{2}."

translate id w2June08Start_25f344fc:
    kat "Some people believe that shame is a social construct, a thing that only exists when perceived through the lens of a culture's mores and norms."

translate id w2June08Start_a2188325:
    kat "Other people believe it's an evolutionary part of the human condition, an adaptive trait for a social, group-living animal."

translate id w2June08Start_a7beaee8:
    kat "It has many other words: humiliation, indignity, disgrace, self-abasement..."

translate id w2June08Start_11216e7b:
    ver "We know what the word means."

translate id w2June08Start_e672553e:
    rose "How is that any different from last week?"

translate id w2June08Start_d8e8dcbe:
    kat "The difference is shame was the byproduct last week. This time it's the focus."

translate id w2June08Start_72355d50:
    fel "What does that mean practically speaking?"

translate id w2June08Start_7ab69c5a:
    kat "You'll have to wait and see. If I gave you time to prepare, it would lessen the impact."

translate id w2June08Start_f81be831:
    kat "Now, where was I?"

translate id w2June08Start_fbe8801b:
    mc "The two theories."

translate id w2June08Start_40bdf42d:
    kat "Right! Thank you, [2]."

translate id w2June08Start_88cbf82e:
    kat "Now, speaking from personal experience, I'm inclined to believe the first theory. However, it really doesn't matter what shame {3}is{4}."

translate id w2June08Start_2e8849d0:
    kat "What's important is that in your training to become proper whores, you learn to {1}weaponize{2} it. To use being ashamed as a tool to entice men."

translate id w2June08Start_0e94a306:
    ver "Oh, of course. That's totally a natural way of thinking."

translate id w2June08Start_6b4c8ae0:
    kat "Sarcasm aside, you're right. Using your own embarrassment to your advantage is an unnatural thing, that's why over the next few days [2] and each of you girls will carry out a \"mission\" for our patrons' pleasure that will help that."

translate id w2June08Start_6eb5271e:
    rose "I'm afraid to ask..."

translate id w2June08Start_5b29b14a:
    kat "Just know that each one of you will be called upon sometime over the next four days to participate in a photoshoot. You'll be filled in on the details closer to each of your appointed missions."

translate id w2June08Start_58de8697:
    kat "Speaking of photoshoots, that's also why we're in the studio today. We're going to prepare a small photo album for our patrons."

translate id w2June08Start_c96d43f0:
    fel "I kinda figured as much."

translate id w2June08Start_4cc3a189:
    kil "Hey! Sorry, I'm late. They fucked your order."

translate id w2June08Start_244da9af:
    mc "You sure you weren't flirting with the barista?"

translate id w2June08Start_a51b41be:
    kil "Very funny."

translate id w2June08Start_c5837704:
    kil "Guess I'm now the official gofer."

translate id w2June08Start_dc0ff26a:
    mc "Sorry."

translate id w2June08Start_a537b690:
    kil "You kidding? It's great that you're here."

translate id w2June08Start_2df3bb3d:
    kil "So, you have a good time last night bunking down with our moody princess?"

translate id w2June08Start_1cb6178b:
    mc "I think you're reading too much into it. We didn't do anything."

translate id w2June08Start_8b77b766:
    kil "Even if that's true, I still think there's something there."

translate id w2June08Start_1d27bdef:
    mc "I'll just say I'm glad you invited her."

translate id w2June08Start_639a297b:
    kil "Hehe, you're welcome."

translate id w2June08Start_97aadbd9:
    mc "You've got the completely wrong idea, dude."

translate id w2June08Start_af264bc1:
    kil "Eh, really? I could've sworn I was picking up on some mutual attraction thing."

translate id w2June08Start_b59cbc9e:
    kat "Both Mr. Beaufort and [2] will be photographers for this session."

translate id w2June08Start_789c8daf:
    kil "Two is a bit unusual.."

translate id w2June08Start_56d13a4b:
    kat "[2] will be acting independently for me later in the week, so this will be a good chance for him to get acclimated to using a camera. You'll show him the ropes, won't you?"

translate id w2June08Start_c88afbae:
    kil "Sure, it's not too complicated."

translate id w2ShamePhotoshoot_bc3d3421:
    "Please set [2]'s personality (toughness score) for this replay."

translate id w2ShamePhotoshoot_1f6c1206:
    "After a quick tutorial from Ian about how to work the camera, which basically boiled down to what some odd five buttons did, we were ready to start the shoot."

translate id w2ShamePhotoshoot_0415f8aa:
    mc "Good to go. Should they start posing?"

translate id w2ShamePhotoshoot_92b6c946:
    kat "I'll let you direct the models, Mr. [1]. Instruct them on what to do."

translate id w2ShamePhotoshoot_13f9cb94:
    mc "Me? Okay, uh..."

translate id w2ShamePhotoshoot_b03d25af:
    "Put on the spot, I was at a loss on how to begin."

translate id w2ShamePhotoshoot_aa4caacc:
    kil "Start with some basic posturing and go from there. It might give you some idea on how to continue."

translate id w2ShamePhotoshoot_f7d3cdb3:
    mc "That makes sense."

translate id w2ShamePhotoshoot_a4701f50:
    mc "Alright, girls let's get some poses going."

translate id w2ShamePhotoshoot_2e048132:
    "A few shots of the Carnations together seemed like a good way to start things off."

translate id w2ShamePhotoshoot_8c7edb56:
    mc "You're looking a bit uninterested, Veronica."

translate id w2ShamePhotoshoot_a331393c:
    ver "Sorry?"

translate id w2ShamePhotoshoot_6d3031f1:
    mct "(Well, it is kind of attractive in its own weird way.)"

translate id w2ShamePhotoshoot_dc0d7b1d:
    mc "Get closer together like you're all best friends."

translate id w2ShamePhotoshoot_f9c2bb17:
    "Veronica pulled the girls closer."

translate id w2ShamePhotoshoot_bbc70479:
    mc "That's good, maybe smile a little more?"

translate id w2ShamePhotoshoot_84093af0:
    "One of the few times you can tell a woman to smile without being an asshole."

translate id w2ShamePhotoshoot_0ada7512:
    kat "What is this, a family photo?"

translate id w2ShamePhotoshoot_aa1ad72f:
    mc "Right. How about something more flirty?"

translate id w2ShamePhotoshoot_c00d99c4:
    rose "Flirty...?"

translate id w2ShamePhotoshoot_27893121:
    mc "Just do what comes to your head."

translate id w2ShamePhotoshoot_5f674697:
    mc "Hmm, something's missing..."

translate id w2ShamePhotoshoot_fea2e982:
    mc "Veronica, why don't you strike a pose?"

translate id w2ShamePhotoshoot_654e1e05:
    ver "Like this?"

translate id w2ShamePhotoshoot_85785fdb:
    mc "Perfect."

translate id w2ShamePhotoshoot_17476bb5:
    kil "We should get some solo action I think."

translate id w2ShamePhotoshoot_3b59195c:
    kat "That's a good idea, Mr. Beaufort."

translate id w2ShamePhotoshoot_1faf28f2:
    mc "Alright, girls you heard 'em. Let's get individual shots. We'll start with Rosalind."

translate id w2ShamePhotoshoot_25bc823b:
    rose "Where do you want me?"

translate id w2ShamePhotoshoot_22a05587:
    mc "Right where you are."

translate id w2ShamePhotoshoot_02c6d389:
    kil "That means you two need to get out of our shot."

translate id w2ShamePhotoshoot_07a769cc:
    "Veronica and Felicia left Rosalind alone under the scrutiny of our viewfinders."

translate id w2ShamePhotoshoot_134f509c:
    rose "Hehe..."

translate id w2ShamePhotoshoot_cec120f1:
    kil "You're the star now, Rosie. Give us something good."

translate id w2ShamePhotoshoot_8205262c:
    rose "Um..."

translate id w2ShamePhotoshoot_2b74ef15:
    "Rosalind contorted her body into an odd, distinctly unflattering pose."

translate id w2ShamePhotoshoot_60601932:
    "We were both inexperienced with this, but I was sure as hell glad to be on this side of the camera."

translate id w2ShamePhotoshoot_dbf1d9a5:
    rose "This is no good... is it?"

translate id w2ShamePhotoshoot_00fe4a6e:
    "If I had to guess, Rosalind probably picked that up from our expressions."

translate id w2ShamePhotoshoot_d42264ef:
    kil "Something more sexy."

translate id w2ShamePhotoshoot_c7e29fd3:
    "She looked at me expectantly, as if asking me for more concrete direction."

translate id w2ShamePhotoshoot_31f4df15:
    mc "Hmm, how about..."

translate id w2ShamePhotoshoot_ca4decd6:
    "One obvious route came to mind, given the theme of her outfit."

translate id w2ShamePhotoshoot_a5ee41a1:
    mc "You strike a pose that shows off those \"udders\" of yours?"

translate id w2ShamePhotoshoot_a73e4b6c:
    mc "How about you strike a pose that shows off those beautiful breasts of yours? Seems fitting."

translate id w2ShamePhotoshoot_01dfe89b:
    mc "How about you strike a pose that shows off your bust. Y'know considering what you're wearing..."

translate id w2ShamePhotoshoot_153bca8e:
    "Rosalind thought for a moment, before deciding on a pose."

translate id w2ShamePhotoshoot_1a94be62:
    "Slowly she lifted her arms up, stuck out her chest, and placed her arms behind her head. The pose lent her already large breasts an even greater sense of heft."

translate id w2ShamePhotoshoot_5e611965:
    "I waited for the eye-pleasing jiggle of her breasts to settle, before..."

translate id w2ShamePhotoshoot_2cbd0410:
    "*Snap!*"

translate id w2ShamePhotoshoot_1de8841e:
    mc "Perfect. Let's get a couple more. Sit down on the couch and use your forearm to cup your breasts."

translate id w2ShamePhotoshoot_28c3c8ef:
    rose "...like this?"

translate id w2ShamePhotoshoot_77085d2a:
    mc "Even more perfect."

translate id w2ShamePhotoshoot_9bcb0cb1:
    rose "T-thanks..."

translate id w2ShamePhotoshoot_904efa8c:
    mc "Same pose, but pull your bikini top to the side this time."

translate id w2ShamePhotoshoot_67030bca:
    rose "Okay..."

translate id w2ShamePhotoshoot_cb68aeec:
    "Rosalind did as I asked, exposing her pinkish pale bud."

translate id w2ShamePhotoshoot_26594b16:
    mc "Let's get one last shot. Something with some oompph to finish off the set."

translate id w2ShamePhotoshoot_15738dda:
    rose "What does \"oompfh\" mean?"

translate id w2ShamePhotoshoot_30d5e9fe:
    mc "Do you have..."

translate id w2ShamePhotoshoot_8f66d616:
    kil "I uh... I can run up and grab some."

translate id w2ShamePhotoshoot_6d3a1e77:
    mc "Yes, do that."

translate id w2ShamePhotoshoot_0d18e8fa:
    mc "Would you, please?"

translate id w2ShamePhotoshoot_6ed6930d:
    mc "No worries, I can go get it myself."

translate id w2ShamePhotoshoot_0fd4d656:
    "......"

translate id w2ShamePhotoshoot_a20cefa7:
    "..."

translate id w2ShamePhotoshoot_2b3597a5:
    rose "It's c-cold!"

translate id w2ShamePhotoshoot_79f372de:
    mc "Just bear with me."

translate id w2ShamePhotoshoot_5c90777b:
    mc "It makes you look like a proper dairy cow. What was the name of your cow, Mrs. Pulman?"

translate id w2ShamePhotoshoot_ed19832d:
    kat "Clarabelle."

translate id w2ShamePhotoshoot_457e5263:
    mc "Alright, Rose. Do your best \"Clarabelle\" impression."

translate id w2ShamePhotoshoot_ccb15b7c:
    rose "That's not funny..."

translate id w2ShamePhotoshoot_d4f97ce2:
    mc "Smile!"

translate id w2ShamePhotoshoot_dafc2bf6:
    mc "It really hammers home the motif."

translate id w2ShamePhotoshoot_24ab13a2:
    kil "Ten minutes behind the camera and he's talking about \"motifs\"..."

translate id w2ShamePhotoshoot_78a45357:
    rose "The motif being that I'm a cow..."

translate id w2ShamePhotoshoot_4bbe5891:
    mc "Sorry..."

translate id w2ShamePhotoshoot_755b25e0:
    rose "It is what it is..."

translate id w2ShamePhotoshoot_9aeae6be:
    mc "Smile for me, this will be the last one."

translate id w2ShamePhotoshoot_da95669b:
    mc "That should do it."

translate id w2ShamePhotoshoot_a32d06b6:
    mc "Thank you, Rose."

translate id w2ShamePhotoshoot_2a318816:
    kil "Who next?"

translate id w2ShamePhotoshoot_00925828:
    mc "Veronica. Get your ass over here."

translate id w2ShamePhotoshoot_5d67625b:
    mc "Veronica. Switch with Rosalind."

translate id w2ShamePhotoshoot_5727ad43:
    kil "You're really getting into this directing thing."

translate id w2ShamePhotoshoot_7a60999b:
    mc "Guess the feeling of being behind a camera does something to you."

translate id w2ShamePhotoshoot_92043a8d:
    ver "Is this where you want me?"

translate id w2ShamePhotoshoot_bfa2416d:
    mc "Yep, you've got it."

translate id w2ShamePhotoshoot_37a6208c:
    kil "You've got modeling experience, right?"

translate id w2ShamePhotoshoot_9e322a4d:
    ver "For some sports magazines..."

translate id w2ShamePhotoshoot_75e588ce:
    kil "A body like yours is made for showing off."

translate id w2ShamePhotoshoot_f6e371eb:
    mc "Put your arms above your head."

translate id w2ShamePhotoshoot_c8ed6c45:
    ver "This good?"

translate id w2ShamePhotoshoot_e608e28b:
    mc "Now get on your stomach like you're crawling."

translate id w2ShamePhotoshoot_c58e6d10:
    ver "Crawling...?"

translate id w2ShamePhotoshoot_cc63fc4d:
    "As insulting, fucked up, and backwards as Mrs. Pulman's thought process was for Veronica's costume, it was the obvious thing to lean on for the shoot."

translate id w2ShamePhotoshoot_1df95243:
    "As insulting, fucked up, and backwards as Kathleen's thought process was for Veronica's costume, it was the obvious thing to lean on for the shoot."

translate id w2ShamePhotoshoot_46ae406f:
    mc "Yep, pretend you're a... defeated and humiliated warrior that's been forced to parade around in embarrassing lingerie."

translate id w2ShamePhotoshoot_1008abee:
    ver "...okay."

translate id w2ShamePhotoshoot_2628e5de:
    ver "Is this defeated enough for you?"

translate id w2ShamePhotoshoot_695ba27b:
    mc "Beautiful. Kat, would you mind putting a foot on her back?"

translate id w2ShamePhotoshoot_847c6e5d:
    "Veronica glared up at me with a sour expression on her face."

translate id w2ShamePhotoshoot_07ba12ca:
    "I made sure to mouth a wordless apology."

translate id w2ShamePhotoshoot_4fb09005:
    kat "Oh? ...sure, it'd be my pleasure."

translate id w2ShamePhotoshoot_812156e2:
    kat "Is this to your satisfaction?"

translate id w2ShamePhotoshoot_ba0c143d:
    mc "It is. Let's expand on this concept some more."

translate id w2ShamePhotoshoot_92ac03fe:
    "The question is, who'll be the prop?"

translate id w2ShamePhotoshoot_21e0d37a:
    "Well, she's already involved in the shoot. Might as well conclude the shoot with her."

translate id w2ShamePhotoshoot_b01aaabf:
    mc "Would you mind sitting back on the couch for me, Mrs. Pulman?"

translate id w2ShamePhotoshoot_c1508dec:
    kat "What are you thinking?"

translate id w2ShamePhotoshoot_09c8d18e:
    mc "I just think you've been on your feet too long. They could use some rest and care."

translate id w2ShamePhotoshoot_c7322d65:
    ver "Ugh... let's get it over with."

translate id w2ShamePhotoshoot_26dce30d:
    ver "Ggggh..."

translate id w2ShamePhotoshoot_1e901af6:
    mc "You have it. Now hold that pose."

translate id w2ShamePhotoshoot_cfbcc398:
    mc "Give it a quick taste?"

translate id w2ShamePhotoshoot_d14637c4:
    ver "..."

translate id w2ShamePhotoshoot_d167c6b2:
    mc "Last shot, I promise."

translate id w2ShamePhotoshoot_c616db96:
    ver "Mmh..."

translate id w2ShamePhotoshoot_d099ffe7:
    "Veronica begrudgingly slid her tongue across the sole of Mrs. Pulman's black stockings."

translate id w2ShamePhotoshoot_ffe32c37:
    "Veronica begrudgingly slid her tongue across the sole of Kathleen's black stockings."

translate id w2ShamePhotoshoot_6e02c2bd:
    kat "That's a good look for you, Miss Lynch."

translate id w2ShamePhotoshoot_53a2907c:
    mc "Thanks for doing that, Veronica."

translate id w2ShamePhotoshoot_cfa8ebc0:
    kil "Did I already remark that you seem to be really getting into this?"

translate id w2ShamePhotoshoot_f07ad649:
    mc "We'll move onto Felicia now."

translate id w2ShamePhotoshoot_4bf358ba:
    "Ian seemed to have a thing for Veronica over the last week, so why not let him be the model? Throw the dog a bone, so to speak."

translate id w2ShamePhotoshoot_19b1b3cc:
    mc "Ian would you mind stepping in as a model? I'll get all the shots we need."

translate id w2ShamePhotoshoot_7d945560:
    kil "What do you want me to do?"

translate id w2ShamePhotoshoot_efb2291a:
    mc "Some sort of pose that says domineering. Your choice."

translate id w2ShamePhotoshoot_24a9ce85:
    kil "Domineering, eh...? Hmm, I've got an idea."

translate id w2ShamePhotoshoot_a7b530e2:
    kil "Let me know if I'm squeezing too tight."

translate id w2ShamePhotoshoot_d114cfd1:
    ver "You don't have to worry about that, pencil arms. Just get the photo quick."

translate id w2ShamePhotoshoot_70982de2:
    "Ian positioned himself behind Veronica, one arm around her neck and the other pulling her back in an impressive limber display."

translate id w2ShamePhotoshoot_1dba942d:
    "It definitely fit what I was going for, and deep down, there was something about the position that my lizard brain found pleasing. "

translate id w2ShamePhotoshoot_da3dee0a:
    mct "(Nice one, Ian.)"

translate id w2ShamePhotoshoot_6be4c783:
    "Of course, this was all just a pose. Veronica could easily turn it around on Ian if she wanted."

translate id w2ShamePhotoshoot_46df1028:
    mc "Alright, now drop your arm like you just passed out."

translate id w2ShamePhotoshoot_24665532:
    mc "Alright, I got the shots."

translate id w2ShamePhotoshoot_53a2907c_1:
    mc "Thanks for doing that, Veronica."

translate id w2ShamePhotoshoot_12e504de:
    ver "Honestly, a headlock is pretty low on the list of things you could've asked me to do."

translate id w2ShamePhotoshoot_cfa8ebc0_1:
    kil "Did I already remark that you seem to be really getting into this?"

translate id w2ShamePhotoshoot_f07ad649_1:
    mc "We'll move onto Felicia now."

translate id w2ShamePhotoshoot_77be03c4:
    mc "I'll jump in and be the model real quick."

translate id w2ShamePhotoshoot_276d30ab:
    kil "You?"

translate id w2ShamePhotoshoot_10fa5bc8:
    mc "I've got a pretty good pose in mind. You can get the shot."

translate id w2ShamePhotoshoot_7e72ab42:
    kil "Yeah, sure. Photographer, model, doctor... you're a real triple threat."

translate id w2ShamePhotoshoot_b029e152:
    mc "Don't you forget it."

translate id w2ShamePhotoshoot_a20cefa7_1:
    "..."

translate id w2ShamePhotoshoot_13d02858:
    mc "Open up."

translate id w2ShamePhotoshoot_bd6a9753:
    ver "Just get the shot."

translate id w2ShamePhotoshoot_44b85b56:
    mc "You alright with this?"

translate id w2ShamePhotoshoot_42aa6269:
    ver "Yeah, it's fine just get the shot."

translate id w2ShamePhotoshoot_f1b454f7:
    ver "Mmmfggh..."

translate id w2ShamePhotoshoot_2ca61c74:
    "I stuck my finger into Veronica's mouth with one hand, while the other roughly groped at a breast."

translate id w2ShamePhotoshoot_ba24b6eb:
    ver "Mmmh, oha!"

translate id w2ShamePhotoshoot_ce8546cc:
    "The idea was to make Veronica look like she was snared and in distress, but an ever-so-slight moan left me with a different impression."

translate id w2ShamePhotoshoot_1191dda5:
    mct "(Is this slut enjoying this?)"

translate id w2ShamePhotoshoot_629d3df4:
    mct "(Is she enjoying this?)"

translate id w2ShamePhotoshoot_ab535790:
    "Do I dare to find out...?"

translate id w2ShamePhotoshoot_d149021f:
    "Yes, I was so very, very curious."

translate id w2ShamePhotoshoot_57cdeee8:
    ver "G-ghu..!"

translate id w2ShamePhotoshoot_9dee4d62:
    "Setting aside any sense of propriety, I pushed my fingers deeper into Veronica's mouth and used my other hand to tweak her stiffening buds through the fabric of her lingerie."

translate id w2ShamePhotoshoot_169ab87c:
    ver "*Cough* *Cough* --!"

translate id w2ShamePhotoshoot_c0cbf8ea:
    "The tips of my two fingers brushed the back of her throat, effectively deep throating them and causing her to sputter in surprise."

translate id w2ShamePhotoshoot_a3fcfbad:
    ver "Mmmh...!"

translate id w2ShamePhotoshoot_094a4c49:
    mc "*Whisper* You're supposed to be acting distressed. What's the matter?"

translate id w2ShamePhotoshoot_673df7d4:
    ver "Eurh-beeing-twoo-"

translate id w2ShamePhotoshoot_7fee8368:
    ver "Ack-!"

translate id w2ShamePhotoshoot_7a94de55:
    mct "(That was rhetorical.)"

translate id w2ShamePhotoshoot_e1ef6e5a:
    mc "You must be pent up."

translate id w2ShamePhotoshoot_7882d37e:
    "I don't know what had possessed me all of a sudden, but I pushed my luck, prying the Amazon's soft pink tongue from her mouth and holding it between my fingers."

translate id w2ShamePhotoshoot_7700987b:
    mc "You getting some good shots here?"

translate id w2ShamePhotoshoot_8c73986d:
    kat "You're supposed to be posing, not molesting the model."

translate id w2ShamePhotoshoot_04447b78:
    kil "We're getting some {1}damn{2} good shots here."

translate id w2ShamePhotoshoot_26203f30:
    "Veronica was totally blanking, just freely letting my hands dig into the pale speckled flesh of her breasts with nary a peep to the contrary. It felt totally unlike her."

translate id w2ShamePhotoshoot_9f0855e5:
    "So much so that I involuntarily found myself softly stroking her head and massaging her scalp as if to comfort her."

translate id w2ShamePhotoshoot_f7243693:
    "Meanwhile, my other hand was up to something much more naughty. Her nipples were as hard as diamonds, completely engorged and just begging to be played with."

translate id w2ShamePhotoshoot_79318946:
    "So that's what my right hand did, lackadaisically brushing its fingertips against the stoney protuberance, flicking and teasing..."

translate id w2ShamePhotoshoot_a62eb6f0:
    ver "Ah...♥"

translate id w2ShamePhotoshoot_7beae906:
    kil "I think I got enough though."

translate id w2ShamePhotoshoot_cfa8ebc0_2:
    kil "Did I already remark that you seem to be really getting into this?"

translate id w2ShamePhotoshoot_dba83d79:
    mc "Thanks for doing that, Veronica. It was all for the shoot."

translate id w2ShamePhotoshoot_63250fbc:
    ver "The h-hell it was, you perv."

translate id w2ShamePhotoshoot_001b74c4:
    mc "Hey, you seemed to enjoy it."

translate id w2ShamePhotoshoot_63250fbc_1:
    ver "The h-hell it was, you perv."

translate id w2ShamePhotoshoot_61f6f51a:
    mc "Hehe, no honest! Really!"

translate id w2ShamePhotoshoot_1d337a5b:
    "After taking a few moments to collect herself, Veronica fixed her \"clothes\" and scrambled off the backdrop."

translate id w2ShamePhotoshoot_a034a1aa:
    kil "That leaves Felicia for next."

translate id w2ShamePhotoshoot_10e675ae:
    mct "(Better not push it.)"

translate id w2ShamePhotoshoot_6033c2d4:
    mc "You got the shot?"

translate id w2ShamePhotoshoot_ef476c06:
    kil "Yep, I think we're good."

translate id w2ShamePhotoshoot_cfa8ebc0_3:
    kil "Did I already remark that you seem to be really getting into this?"

translate id w2ShamePhotoshoot_dba83d79_1:
    mc "Thanks for doing that, Veronica. It was all for the shoot."

translate id w2ShamePhotoshoot_edf1ac9b:
    ver "Not the worst thing you could've asked me to do."

translate id w2ShamePhotoshoot_a034a1aa_1:
    kil "That leaves Felicia for next."

translate id w2ShamePhotoshoot_d67c7d4c:
    "Veronica did her best to give the camera a distressed, uncomfortable look. Not all of it was acting I'm sure."

translate id w2ShamePhotoshoot_5c6de901:
    "Even if it was just pretend, it felt nice having Veronica in my grasp like this."

translate id w2ShamePhotoshoot_b6acc8bb:
    kil "Alright. I got the shot."

translate id w2ShamePhotoshoot_53a2907c_2:
    mc "Thanks for doing that, Veronica."

translate id w2ShamePhotoshoot_edf1ac9b_1:
    ver "Not the worst thing you could've asked me to do."

translate id w2ShamePhotoshoot_cfa8ebc0_4:
    kil "Did I already remark that you seem to be really getting into this?"

translate id w2ShamePhotoshoot_f07ad649_2:
    mc "We'll move onto Felicia now."

translate id w2ShamePhotoshoot_a6f29029:
    "Felicia was a natural, right out of the gate."

translate id w2ShamePhotoshoot_1f5531ec:
    "Instead of waiting for direction, she quickly switched from pose to pose."

translate id w2ShamePhotoshoot_b4aeb8af:
    "No doubt from her experience helping out at Mina's modeling agency."

translate id w2ShamePhotoshoot_5e58a828:
    mct "(Damn, she looked great.)"

translate id w2ShamePhotoshoot_02a38f56:
    "*Snap*"

translate id w2ShamePhotoshoot_02a38f56_1:
    "*Snap*"

translate id w2ShamePhotoshoot_59e3578f:
    fel "It'd be a shame if you only got one side of me."

translate id w2ShamePhotoshoot_02a38f56_2:
    "*Snap*"

translate id w2ShamePhotoshoot_618d750c:
    fel "I think I remember you saying you were an ass man, [2]."

translate id w2ShamePhotoshoot_e5e0b68b:
    fel "I always thought I could've been in a rap video."

translate id w2ShamePhotoshoot_b52e5f3e:
    "After a couple minutes of continuous posing, Felicia was on the floor, looking hot and flustered for the camera."

translate id w2ShamePhotoshoot_ad13ef68:
    "Hand placed between her legs. Not masturbating, but it certainly looked like she was barely containing herself."

translate id w2ShamePhotoshoot_479cf7ab:
    kat "Ha! The slut got herself worked up just from being photographed."

translate id w2ShamePhotoshoot_969e1558:
    "It occurred to me that as good as Felicia's portion of the shoot was, it lacked the same thematic punch the other two had. This week was about shame. Rosalind was dressed up like an animal and Veronica was wearing something frilly and totally out of character, but she was the odd woman out."

translate id w2ShamePhotoshoot_5287279c:
    "How do you shame an audacious woman like Felicia?"

translate id w2ShamePhotoshoot_b51f25ce:
    kat "I have one last picture I want you boys to get of our star. One that'll surely be a favorite. I'll need to borrow your penis though, [2]."

translate id w2ShamePhotoshoot_9dbbea1c:
    mc "...{1}of course{2}, you will."

translate id w2ShamePhotoshoot_bd9ee1fa:
    "It sounded like she was about to answer my question."

translate id w2ShamePhotoshoot_1615b5d6:
    fel "Where did you get that photo...?"

translate id w2ShamePhotoshoot_f8e900b6:
    kat "It wasn't hard, even if you and Elias don't have a social media presence."

translate id w2ShamePhotoshoot_3521da8a:
    "Mrs. Pulman's \"choice shot\" was a twisted one. She had pulled up a photograph of Felicia and her husband on her phone and was now having the conspicuously silent slut wife hold it up like a placard in a mugshot."

translate id w2ShamePhotoshoot_2617be98:
    "Kathleen's \"choice shot\" was a twisted one. She had pulled up a photograph of Felicia and her husband on her phone and was now having the conspicuously silent slut wife hold it up like a placard in a mugshot."

translate id w2ShamePhotoshoot_4811d656:
    kat "You're a surprisingly private person, Mrs. Ford."

translate id w2ShamePhotoshoot_d6ce9717:
    fel "..."

translate id w2ShamePhotoshoot_202eab09:
    "I guess it was one thing to talk about it on stage, but holding up a photo of her marital life was another thing altogether."

translate id w2ShamePhotoshoot_012d7af8:
    kat "Now open your whore mouth and stick out your tongue, pig."

translate id w2ShamePhotoshoot_c96178d8:
    fel "Mmh..."

translate id w2ShamePhotoshoot_f435a10d:
    kat "Did you not hear me...?"

translate id w2ShamePhotoshoot_16ecd0e4:
    fel "Aaaah..."

translate id w2ShamePhotoshoot_392d3994:
    "Felicia did as the old woman commanded, adding a not-so-zealous flourish to it."

translate id w2ShamePhotoshoot_90c9973a:
    kil "Got the shot. We good?"

translate id w2ShamePhotoshoot_1d9caa31:
    kat "Almost, I wanted to get one last group photo."

translate id w2ShamePhotoshoot_0fd4d656_1:
    "......"

translate id w2ShamePhotoshoot_a20cefa7_2:
    "..."

translate id w2ShamePhotoshoot_55c75e5b:
    kat "Yep, just like that. Hold out your hands, Miss Lynch."

translate id w2ShamePhotoshoot_c309f204:
    kat "Perfect. Between this and what's coming later this week, our patrons are going to {1}treasure{2} this set."

translate id w2ShamePhotoshoot_d9fa89dd:
    rose "What's coming later?"

translate id w2ShamePhotoshoot_3803d48e:
    kat "I already told you. That'll be a surprise."

translate id w2ShamePhotoshoot_c441c028:
    kat "We're done for now. You two can go."

translate id w2ShamePhotoshoot_7b334c24:
    kat "Take that camera with you, [2]. You'll need it for when you're in the \"field\" later."

translate id w2ShamePhotoshoot_3198506a:
    "Naturally, whatever she has in store for the Carnations I'll be dragged into."

translate id w2ShamePhotoshoot_6bc631e6:
    mc "That's all? You don't need help cleaning up any of this?"

translate id w2ShamePhotoshoot_b371af79:
    kat "That's okay. You boys must be hungry. Go grab lunch or something. I'll see you later."

translate id w2ShamePhotoshoot_56afc01b:
    "Glad to be dismissed, Ian and I parted and went separate ways."

translate id w2ShamePhotoshoot_c6898ad3:
    "He had some errands to run, which thankfully he didn't try to pull me into."

translate id w2ShamePhotoshoot_a52f87f8:
    mct "(I guess my day's wide open. What should I do with it...?)"

translate id w2ShamePhotoshoot_768f2aac:
    mct "(Ah, yeah! Didn't Mina mention something about helping her rehearse for her TV gig sometime today?)"

translate id w2ShamePhotoshoot_a65d56f2:
    "I'll give her a call."

translate id w2ShamePhotoshoot_980bb43f:
    mct "(I guess I'll just do jack shit.)"

translate id w2ShamePhotoshoot_ccc8ae46:
    "First, I made sure to spend a nominal amount of effort trying to think of something more productive to do... just so I could at least say I tried before arriving at my current lazy course."

translate id w2ShamePhotoshoot_0fd4d656_2:
    "......"

translate id w2ShamePhotoshoot_a20cefa7_3:
    "..."

translate id w2MinaLineReadingStart_08a756eb:
    "{3}*Ring... ring...*{4}"

translate id w2MinaLineReadingStart_78198e6b:
    "{3}*Ring... ring... ring...*{4}"

translate id w2MinaLineReadingStart_21bf4128:
    mina "Heeeeey! Heh, I was just thinking about you!"

translate id w2MinaLineReadingStart_52ca8854:
    smv "Who are you talking to?"

translate id w2MinaLineReadingStart_4d41cfca:
    "A soft-spoken question came from a tenor like voice on the other line."

translate id w2MinaLineReadingStart_0e9169a3:
    mina "{5}A friend.{6}"

translate id w2MinaLineReadingStart_d6ccec1f:
    smv "It's not your asshole--"

translate id w2MinaLineReadingStart_e5cd2a54:
    mina "Hey! Only I can... {5} call him an asshole!{6}"

translate id w2MinaLineReadingStart_407be2a1:
    mina "Sorry about that, [2]."

translate id w2MinaLineReadingStart_386d19d0:
    mc "No problem. I hope I'm not calling at a bad time."

translate id w2MinaLineReadingStart_4bebb11e:
    mina "Nope! Just sitting at home at the moment."

translate id w2MinaLineReadingStart_d364ca60:
    mc "Well, I'm glad I'm not interrupting anything then. I was just leaving work and remembered you asked me to help you rehearse today."

translate id w2MinaLineReadingStart_bbc5cce2:
    mina "Oooh, yeah! You actually remembered that?"

translate id w2MinaLineReadingStart_f57704a6:
    mc "I didn't get that bombed last night."

translate id w2MinaLineReadingStart_8cff891b:
    mina "Even still, I'm happy you remembered. Since I'm the one imposing, what time would work best for you?"

translate id w2MinaLineReadingStart_b837b3b7:
    mc "Uh, well... how about now?"

translate id w2MinaLineReadingStart_42a231c3:
    "It sounded like she had company so I was a bit hesitant in picking that time, but I decided I'd leave the ball in her court."

translate id w2MinaLineReadingStart_5fff951b:
    mina "Now works great! Where do you want to meet?"

translate id w2MinaLineReadingStart_ef25f6c4:
    mc "Is your place fine? I bet I'm closer to you than you are to my place."

translate id w2MinaLineReadingStart_76a8dcc1:
    mina "Sure, let me give you the address...!"

translate id w2MinaLineReadingStart_0fd4d656:
    "......"

translate id w2MinaLineReadingStart_a20cefa7:
    "..."

translate id w2MinaLineReadingStart_ea8874f3:
    "My hunch was right. Mina lived reasonably close to the city center, which meant it was a closer trek from the outskirts where the Carnation Club sat."

translate id w2MinaLineReadingStart_29dff7d9:
    mct "(Hmm... do I just let myself in? There's the buzzer.)"

translate id w2MinaLineReadingStart_90a75ae2:
    mct "(Ah, crap she didn't tell me her apartment number. I guess I should give her another call...)"

translate id w2MinaLineReadingStart_eb8abde1:
    "As if on cue, just as I was about to reach for my phone, Mina came out her apartment building with a blonde boy in tow."

translate id w2MinaLineReadingStart_58d1d19a:
    mc "Hey! Just in time, I guess!"

translate id w2MinaLineReadingStart_4d199ad8:
    mina "Hehe, looks like it!"

translate id w2MinaLineReadingStart_1d0b038b:
    "Mina and company made the short jaunt past the stoop and down the apartment building's stairs until we came face-to-face."

translate id w2MinaLineReadingStart_bf8a406e:
    mc "Who's this?"

translate id w2MinaLineReadingStart_478e72d4:
    "The boy stood sheepishly to Mina's side. Not quite hiding behind her, but he was clearly feeling pretty timid."

translate id w2MinaLineReadingStart_862bd046:
    "I immediately noticed the bruising and swelling underneath his right eye."

translate id w2MinaLineReadingStart_e2d43489:
    mc "I'm [2]. Nice to meet you. I'm a friend of Mina's."

translate id w2MinaLineReadingStart_13e98e76:
    "The boy sized me up in a familiar, almost nostalgic way. Before..."

translate id w2MinaLineReadingStart_4279d6a3:
    sora "I'm Sora. I'm Mina's brother."

translate id w2MinaLineReadingStart_da475ff8:
    mina "My little brother! Isn't he cute?"

translate id w2MinaLineReadingStart_83cb7c5c:
    mc "I should see the other guy, right?"

translate id w2MinaLineReadingStart_ee8e79dd:
    smv "..."

translate id w2MinaLineReadingStart_b0fda298:
    mina "Aw, c'mon Sora. Don't be shy. He's a friend. Let me introduce you to [2]."

translate id w2MinaLineReadingStart_325d9617:
    sora "Hello..."

translate id w2MinaLineReadingStart_33481c19:
    mina "[2], this is my little brother Sora. Isn't he cute?"

translate id w2MinaLineReadingStart_47970e92:
    mct "(Mina and Sora, eh?)"

translate id w2MinaLineReadingStart_9c3db261:
    "I agreed with her sisterly assessment, but I treated the question rhetorically to avoid further embarrassing him."

translate id w2MinaLineReadingStart_25d9fa7b:
    mc "I can see the family resemblance."

translate id w2MinaLineReadingStart_d9ec515e:
    mina "Hehe!"

translate id w2MinaLineReadingStart_0bf68ba9:
    sora "Yeah, right..."

translate id w2MinaLineReadingStart_8147527d:
    mc "I'm not interrupting family time, am I?"

translate id w2MinaLineReadingStart_9ce0b629:
    mina "No. Sora was paying his lonely big sis a visit, but he has to sadly get going to violin practice."

translate id w2MinaLineReadingStart_a6102d70:
    mc "Oh? I'm not very good at it, but I know a little bit about playing a musical instrument."

translate id w2MinaLineReadingStart_56254d90:
    sora "I'm not that good either. It's stupid, Mom just makes me take lessons... not like I'm going to use it for anything."

translate id w2MinaLineReadingStart_8fea2db2:
    mc "Hmm..."

translate id w2MinaLineReadingStart_1c2342fb:
    mc "This is dumb! Why do I have to learn this? I'm not ever going to use it."

translate id w2MinaLineReadingStart_81196f2f:
    vic "Not everything needs to be fun or immediately useful, [2]. Learning piano will help teach you a number of things that are important in life."

translate id w2MinaLineReadingStart_b2aba0e3:
    mc "Like what?"

translate id w2MinaLineReadingStart_0a86124d:
    vic "Well... it'll improve your concentration. That's one."

translate id w2MinaLineReadingStart_45a7b3a6:
    vic "It'll also teach you discipline and improve your listening skills, both of which you're in dire need of if the last teacher-parent conference is anything to go by."

translate id w2MinaLineReadingStart_3af258fe:
    mc "Mmmh... but it's boring!"

translate id w2MinaLineReadingStart_e53ddb2a:
    mc "Hmm... stick with it. Not everything is immediately useful, but you'll be glad you did."

translate id w2MinaLineReadingStart_31f37773:
    "Sora looked at me dubiously, in the way only a teenager can."

translate id w2MinaLineReadingStart_f8bd592e:
    mc "Sorry, didn't mean to lecture you."

translate id w2MinaLineReadingStart_a260a780:
    sora "No, it's cool... I'll take your word for it."

translate id w2MinaLineReadingStart_b71306db:
    mina "You need to get going, Sora. Have a safe trip."

translate id w2MinaLineReadingStart_035b4fe6:
    sora "Right, love ya sis."

translate id w2MinaLineReadingStart_52460839:
    mina "I love you too."

translate id w2MinaLineReadingStart_5084c80c:
    sora "It was nice meeting you, [2]."

translate id w2MinaLineReadingStart_c0effead:
    mc "You too, Sora."

translate id w2MinaLineReadingStart_a20cefa7_1:
    "..."

translate id w2MinaLineReadingStart_bfe241d0:
    mc "He seems like a nice kid."

translate id w2MinaLineReadingStart_1dae377f:
    mina "You seem like a good judge of character."

translate id w2MinaLineReadingStart_b48fb26f:
    "Mina playfully punched my shoulder and flashed a proud grin."

translate id w2MinaLineReadingStart_5628e913:
    mina "He's just ... going through a rough patch right now. More than I did at that age."

translate id w2MinaLineReadingStart_b85f42ff:
    mc "Does that have anything to do with that black eye"

translate id w2MinaLineReadingStart_60334218:
    mina "He wouldn't tell me where he got it, but from what I can piece together, some kids started giving him a lot of crap this year."

translate id w2MinaLineReadingStart_e9da09ba:
    mina "Mom tells him to just focus on school and ignore them and they'll go away."

translate id w2MinaLineReadingStart_22b0a870:
    mc "That's... not how that works."

translate id w2MinaLineReadingStart_232923b2:
    mina "Yeah, even I could tell you that's awful advice. She lives in another world."

translate id w2MinaLineReadingStart_fc39e50b:
    mc "You should get Ian to talk to him. He might have some insight he could pass along."

translate id w2MinaLineReadingStart_a4b438f8:
    mina "Maybe..."

translate id w2MinaLineReadingStart_fa8eb408:
    mina "Well, you wanna come in?"

translate id w2MinaLineReadingStart_0fd4d656_1:
    "......"

translate id w2MinaLineReadingStart_a20cefa7_2:
    "..."

translate id w2MinaLineReadingStart_a7425004:
    "After a quick tour of her apartment, Mina led me to her living space."

translate id w2MinaLineReadingStart_97174703:
    mc "You have a nice place."

translate id w2MinaLineReadingStart_dd09e11a:
    mina "Thanks, but my parents foot the bill. Not really anything to be proud of."

translate id w2MinaLineReadingStart_d4ecfc5e:
    mc "They may pay the rent, but all the warmth is your doing, right? Just take the compliment."

translate id w2MinaLineReadingStart_f516ca4d:
    mina "Sorry, you're right. I just wish I could be more independent."

translate id w2MinaLineReadingStart_f160ce3e:
    mc "This TV job is a step toward that, right?"

translate id w2MinaLineReadingStart_6c6c8fdf:
    mina "Um hum, hopefully!"

translate id w2MinaLineReadingStart_c40793d2:
    mct "(This place looks just about how I imagined it would.)"

translate id w2MinaLineReadingStart_62268701:
    "It had a lived-in touch you'd expect from any person's bedroom, but there was a particular blend of cute and stylish that was distinctly Mina."

translate id w2MinaLineReadingStart_50acfb84:
    mina "Hey! You hungry? It's way past lunch and I'm starving."

translate id w2MinaLineReadingStart_26cb2efe:
    mc "I could go for something."

translate id w2MinaLineReadingStart_361bd3f3:
    mina "Great! You like sushi?"

translate id w2MinaLineReadingStart_307fd2c0:
    mc "Sushi would be great. I don't usually get the chance to eat it."

translate id w2MinaLineReadingStart_3156316b:
    mina "Sit tight then, I've got a menu in the kitchen. I'll put in an order for delivery."

translate id w2MinaLineReadingStart_71d039c2:
    mina "Oh, and I'm paying for it. As thanks for you taking time out of your day. No arguments!"

translate id w2MinaLineReadingStart_b59d1ae2:
    "She was artfully already out of the door when she added that last part. Not that I was complaining. Being fed on someone else's dime was a college student's pleasure."

translate id w2MinaLineReadingStart_4797e213:
    mct "(I'm not hurting for cash anymore though, I guess.)"

translate id w2MinaLineReadingStart_d43313ee:
    "With Mina momentarily out of the room and nothing else to do, I decided to take a closer look at the wall of photos that were definitely the centerpiece of her bedroom."

translate id w2MinaLineReadingStart_b70afb55:
    "It was lined with various memories of things that I'm certain were important to Mina."

translate id w2MinaLineReadingStart_9ce9b878:
    "Should I inspect any of these?"

translate id w2MinaFashionShow_912ebc9b:
    "It's a poster for a fashion show. Could be solely decorative or perhaps even a souvenir from something Mina attended."

translate id w2MinaFashionShow_aae2f500:
    mct "(To be honest, I don't really get fashion. Not that I can't appreciate a good-looking pair of boots or an attractive shirt, but the artistic side of it is lost on me.)"

translate id w2MinaFamilyPhoto_fed21790:
    "It was a recent-looking photo of Mina, her brother, and a remarkably beautiful woman who I presumed to be their mother."

translate id w2MinaFamilyPhoto_126efc9e:
    "The photo reminded me about Mina's comment last night about having a \"revolving door of stepdads\", but they looked reasonably happy and not in a smile-for-the-camera sort of way."

translate id w2MinaFamilyPhoto_a3fe2ddf:
    "There was a genuine warmth found in the photo that reinforced what I already firmly believed. Life doesn't have to be picture perfect to have happiness."

translate id w2MinaFamilyPhoto_de9cac55:
    mct "(Then again, that's a wild assumption to make for a single photo.)"

translate id w2MinaGlamourMagazine_889da9b1:
    "It's a magazine cover with Mina on it from earlier this year. Her hair is down and she's wearing a revealing set of white netted lingerie."

translate id w2MinaGlamourMagazine_992a02c8:
    "This must have been an important gig to her if she memorialized it on her wall like this."

translate id w2MinaGlamourMagazine_30fd8805:
    mct "(If I looked that good, I'd probably have a poster of me on the wall too.)"

translate id w2MinaMinaDad_191f294b:
    "It's a picture of a blonde man holding an infant. While tired, he also looks incredibly proud and happy. He clearly loves being a father."

translate id w2MinaMinaDad_aa080356:
    "Maybe baby Mina and her dad?"

translate id w2MinaMinaDad_a28f0b88:
    mct "(Whoever it is, that is a {1}fabulous{2} head of hair.)"

translate id w2MinaKillian_6e94bf7e:
    "It's a picture of Mina and Ian, shoulder to shoulder. It's difficult to make out where they are, but it looks like they're on a date."

translate id w2MinaKillian_24a5db7b:
    "They look happy. I know they met during a photoshoot, but maybe I'll ask her more about it sometime. "

translate id w2MinaKillian_a6806c33:
    mct "(I can't help but wonder what drew her to Ian in the first place and why she's stuck around so long.)"

translate id w2MinaBridesOfTheHunt_e9cac721:
    "It's a poster for the fantasy television drama \"Brides of the Hunt\", displayed prominently on the wall. She must really be a fan of that show."

translate id w2MinaBridesOfTheHunt_2467d499:
    "I actually just caught that one recently too."

translate id w2MinaBridesOfTheHunt_0ebc16a4:
    "Maybe I'll have to check it out some time."

translate id w2MinaBridesOfTheHunt_b733c8d9:
    mct "(These foreign dramas have really blown up in popularity.)"

translate id w2MinaArchery_285644b9:
    "It's a surprising photograph of Mina and two other girls, each of them brandishing a bow."

translate id w2MinaArchery_7ea0400d:
    "They look happy, like they had just done well at an event."

translate id w2MinaArchery_b8cd8490:
    mct "(It looks like an important memory for Mina.)"

translate id w2ExJuliet_01c1d9b2:
    "It's a photograph of Mina in what appears to be a stage play. A woman is cradling Mina in her arms, looking despondent."

translate id w2ExJuliet_c7e55121:
    "If I had to hazard a guess, going by the costumes and from what's going on in the picture, I'd say this is an all-girls production of Romeo and Juliet."

translate id w2ExJuliet_f67c46e4:
    "It looked really high quality. Either this must have been a fairly large show or their costume designer was at least big league."

translate id w2ExJuliet_6bf2666d:
    mct "(I wonder what it feels like to be in the spot light like that? I mean, I only have my recent experiences to go off of, but...)"

translate id w2ExJuliet_2e72dc97:
    mct "(I imagine performing a piece of literature probably hits a little differently than getting your dick sucked for the viewing pleasure of a room full of perverts.)"

translate id w2MinaLineReadingContinue_ec5ee3c8:
    mct "(Hmm...)"

translate id w2MinaLineReadingContinue_7f9b52ab:
    mina "I just ordered us a variety platter. That's cool, right?"

translate id w2MinaLineReadingContinue_7a8a3b39:
    mc "Works for me."

translate id w2MinaLineReadingContinue_18885e1a:
    mina "What were you doing?"

translate id w2MinaLineReadingContinue_72b2d57b:
    mc "Just taking a look at some of the stuff on your wall. You keep a lot of memories."

translate id w2MinaLineReadingContinue_a6cf8da3:
    mina "Oh yeah... hope you don't think I'm a smidge narcissistic."

translate id w2MinaLineReadingContinue_aaee15bd:
    mc "Why would I ever think that?"

translate id w2MinaLineReadingContinue_e1043e2d:
    mina "I mean, do you know many people who keep a blown up, half-naked photo of themselves on their bedroom wall?"

translate id w2MinaLineReadingContinue_b334e227:
    mc "I don't know. Did Ian ever take his down?"

translate id w2MinaLineReadingContinue_61d7958b:
    mina "*Snort* Ha! Funny!"

translate id w2MinaLineReadingContinue_ab273f47:
    mc "No, but for real, I'm sure you have a good reason besides vanity for every single one of these."

translate id w2MinaLineReadingContinue_b4139f5d:
    "For example..."

translate id w2MinaWallMenu_29e4bdd1:
    mc "For example, it isn't weird to have a family photo. That's your mom next to you and your brother?"

translate id w2MinaWallMenu_4b672bab:
    mc "That's your mom next to you and your brother?"

translate id w2MinaWallMenu_a61319d4:
    mina "It is indeed. That is Mrs. Eiko Umeda HYPHEN Wright, formerly Mrs. Eiko Scott, and even more formerly Mrs. Eiko Adams. First of her names."

translate id w2MinaWallMenu_63958b1d:
    mina "She's very beautiful, isn't she?"

translate id w2MinaWallMenu_f9332188:
    mc "She is."

translate id w2MinaWallMenu_701d8646:
    mina "Hehe~ people tell me I get my looks from her, so thanks!"

translate id w2MinaWallMenu_3948d1b0:
    mc "Smidge narcissistic were your words, huh?"

translate id w2MinaWallMenu_c0b11597:
    mina "Oof. Using my words against me... that's mean! You play dirty, [2]."

translate id w2MinaWallMenu_e7dc1bdf:
    "Mina and I shared a dumb look between us before she moved the conversation along."

translate id w2MinaWallMenu_4551baef:
    mina "She's a great woman. We may butt heads sometimes over what's \"traditional\", but I love her dearly. She's got a generous heart."

translate id w2MinaWallMenu_c44e3727:
    mina "Maybe a little too generous when it comes to the men, but hey..."

translate id w2MinaWallMenu_5d57b172:
    mc "Was this taken on vacation?"

translate id w2MinaWallMenu_f5386836:
    mina "Yep! I'd always wanted to go to Italy and she surprised me last summer out of the blue. It took some convincing to get Sora to come along, but I wouldn't take no for an answer."

translate id w2MinaWallMenu_a46ce806:
    mina "That photo was taken in Venice. No roads or cars, just canals... isn't that amazing? A floating city. It's like something out of a story."

translate id w2MinaWallMenu_a89bb21e:
    mc "That does sound peaceful."

translate id w2MinaWallMenu_4923a509:
    mina "Yep, I had a great time. We all did, even if my brother won't admit it. You ever been to a foreign country?"

translate id w2MinaWallMenu_494ad780:
    mc "No. I've barely been out of the state. It feels like a small world sometimes."

translate id w2MinaWallMenu_2cc93c56:
    mina "You have any place you'd like to visit if you got the chance?"

translate id w2MinaWallMenu_bf997c3b:
    mc "I don't know, I always thought that..."

translate id w2MinaWallMenu_3b241118:
    mc "I always thought Iceland sounded beautiful. Volcanoes, geysers, hot springs... not sure about the fermented shark, but it sounds like a nice place to visit."

translate id w2MinaWallMenu_93356080:
    mina "Oooh, yeah! It really does. Hot springs, that sounds SO nice right now..."

translate id w2MinaWallMenu_75a14113:
    mc "I always thought Australia would be neat to visit. See the Great Barrier Reef, see the rugged outback of the western parts, try the barbecue..."

translate id w2MinaWallMenu_d858151e:
    mina "Mmmh... barbecue. Don't mention food to me, I'm hungry!"

translate id w2MinaWallMenu_3f6e9339:
    mc "I always thought it'd be cool to visit Japan. See Tokyo and the sights, drive the country side, stay at an old inn and enjoy a hot spring."

translate id w2MinaWallMenu_1a22542e:
    mina "I always wanted to go there too. That's where a lot of my favorite dramas are filmed. Heh, I guess that's a pretty dorky reason..."

translate id w2MinaWallMenu_5c72d36d:
    mc "It's as good a reason as any other."

translate id w2MinaWallMenu_977e6c73:
    mc "You made Venice sound pretty nice. Seriously, no cars? That's sounds like heaven to someone who's lived in a city suburb most his life."

translate id w2MinaWallMenu_0e77fecf:
    mina "No fair. You should pick your own answer."

translate id w2MinaWallMenu_541d7ebc:
    mc "Nope, I still pick Venice. Sorry."

translate id w2MinaWallMenu_d8349961:
    "A small moment of silence passed between us, before Mina deftly maneuvered the conversation back on topic."

translate id w2MinaWallMenu_5017e271:
    mina "Do you see yourself traveling much when you get out of school?"

translate id w2MinaWallMenu_70eb476b:
    mc "I haven't really thought about it. One thing at a time. That's usually how my mind works."

translate id w2MinaWallMenu_29a13842:
    mc "Get into medical school. Focus on my studies. Become a doctor."

translate id w2MinaWallMenu_43eea2ef:
    mina "You're a pretty serious person, [2]."

translate id w2MinaWallMenu_dc0ff26a:
    mc "Sorry."

translate id w2MinaWallMenu_089ca7eb:
    mina "No, I like that about you. It's a good quality in a man."

translate id w2MinaWallMenu_0fd4d656:
    "......"

translate id w2MinaWallMenu_a20cefa7:
    "..."

translate id w2MinaWallMenu_a24c4e6a:
    mc "Aw, I'm a man? Thanks!"

translate id w2MinaWallMenu_fa80153f:
    mina "Shut up!"

translate id w2MinaWallMenu_b3a70358:
    mc "Anyway, it sounds like you had a nice trip with your family. Big memory, big photo."

translate id w2MinaWallMenu_191baa7e:
    mc "Nothing narcistic about that."

translate id w2MinaWallMenu_3c07274d:
    mina "Yeah, I really did. It was one of the best trips in my life."

translate id w2MinaWallMenu_b346406e:
    mc "I'd like to hear about another one of these photos."

translate id w2MinaWallMenu_ecba54a0:
    mina "Sure. It's fun to reminisce. Which one do you want to hear about?"

translate id w2MinaWallMenu_58133f14:
    mc "For example, who's that man holding a baby?"

translate id w2MinaWallMenu_948f5f90:
    mc "Who's that man holding a baby?"

translate id w2MinaWallMenu_3d8f5cf5:
    mina "That's my dad and little brother."

translate id w2MinaWallMenu_83141cb0:
    mc "He looks pretty stoked to be a dad."

translate id w2MinaWallMenu_3195705b:
    mina "He's great. He and my mom split when I was six and even though he lives in another city now due to work, he still calls me two times a week."

translate id w2MinaWallMenu_af71603f:
    mina "I like to look at that photo when we talk and pretend that he's here."

translate id w2MinaWallMenu_0dcd8cd0:
    mc "He sounds like a good dad. What does he do?"

translate id w2MinaWallMenu_9fe85eff:
    mina "He's an inventor or well, er... his official title is something fancy, but that's the word he always used and that's how I always thought of him. How about your dad?"

translate id w2MinaWallMenu_a0e54057:
    mc "He was a social worker for the state. My mom tells me he also helped grow this charity that supports the Morehead Hill's library. She speaks very fondly about that."

translate id w2MinaWallMenu_20e1fb09:
    mina "Was...? I'm sorry."

translate id w2MinaWallMenu_954784cf:
    mc "It's alright. He passed when I was little."

translate id w2MinaWallMenu_4e2a8876:
    mina "..."

translate id w2MinaWallMenu_2cae020d:
    mc "It looks like you get your head of hair from your dad, huh?"

translate id w2MinaWallMenu_7971dd16:
    mina "Yep, my brother and I both."

translate id w2MinaWallMenu_2778e69d:
    mc "Uh, this might be weird, but would you mind...?"

translate id w2MinaWallMenu_c82f304c:
    mina "Go ahead! I don't know what it is about pigtails, but you get used to people wanting to touch them."

translate id w2MinaWallMenu_3a7c5d44:
    "Taking the proffered pigtail in my hand, I lightly ran my finger tips over the flaxen strands."

translate id w2MinaWallMenu_46dc45b0:
    "It was incredibly soft and had a healthy, almost glow-like shine to it. She took good care of her hair, as befitting her profession, and most likely prided herself in it."

translate id w2MinaWallMenu_ea4ec40d:
    mina "You like blondes, [2]?"

translate id w2MinaWallMenu_a74d1a41:
    "Mina looked at me in a teasing way, a few steps removed from her usual geniality."

translate id w2MinaWallMenu_d52ff6f4:
    mc "I guess I do."

translate id w2MinaWallMenu_6013d0c0:
    mina "Ha! I knew it!"

translate id w2MinaWallMenu_76aa2b4c:
    mina "Is that what's got you taken with Felicia? Hmmm?"

translate id w2MinaWallMenu_0d94ffb7:
    mc "Come on, we just had dinner one time. I'm not taken with her."

translate id w2MinaWallMenu_5b2ca027:
    mina "Really...?"

translate id w2MinaWallMenu_c404f210:
    mc "Don't worry, I remember your warning: She's married, be careful."

translate id w2MinaWallMenu_f6a3a151:
    mina "Heh, that's good!"

translate id w2MinaWallMenu_85953eed:
    "For just a moment, we shared in a comfortable silence punctuated by Mina's warm smile."

translate id w2MinaWallMenu_b1aecd59:
    "Summoning my most dead serious expression, I caught Mina's eyes with mine and peered into them unfalteringly..."

translate id w2MinaWallMenu_8082c1f6:
    mc "I like YOUR blonde hair. It's very pretty."

translate id w2MinaWallMenu_f4b2b101:
    "Mina's eyes briefly lit up for a moment, before..."

translate id w2MinaWallMenu_83c384a6:
    mina "Ugh, saying it so convincingly as a joke is cheating!"

translate id w2MinaWallMenu_86cd6c27:
    mc "You started it."

translate id w2MinaWallMenu_79143253:
    mina "You jerk."

translate id w2MinaWallMenu_d929ff84:
    "She did her best to hide her grin, but I saw it."

translate id w2MinaWallMenu_85953eed_1:
    "For just a moment, we shared in a comfortable silence punctuated by Mina's warm smile."

translate id w2MinaWallMenu_59cea1e5:
    mina "That's... Ppft...! that's... Pfft...!"

translate id w2MinaWallMenu_f1aa3906:
    "She did her best not to burst out laughing, but failed."

translate id w2MinaWallMenu_3efc1909:
    mina "Hahaha!"

translate id w2MinaWallMenu_11455b53:
    mc "What?"

translate id w2MinaWallMenu_99dccd29:
    mina "You just... pfft... you just said it so seriously! With such a straight face! It's funny!"

translate id w2MinaWallMenu_8639a17d:
    mc "Pfft...!"

translate id w2MinaWallMenu_fa6c151c:
    "Mina's mirth proved contagious and we shared in the laughter."

translate id w2MinaWallMenu_ad1195a6:
    mc "That was my best! Don't laugh!"

translate id w2MinaWallMenu_1dc5e68b:
    "The laughter eventually died down and Mina and I shared a comfortable silence, punctuated by Mina's warm smile."

translate id w2MinaWallMenu_d42cc9bc:
    mc "Eh, I don't really have a preference. Pretty girls come in all different packages."

translate id w2MinaWallMenu_733a14d2:
    mina "Blah! You're no fun. I was teasing you and you gave me a serious answer."

translate id w2MinaWallMenu_e036bed0:
    mc "Heh, sorry!"

translate id w2MinaWallMenu_85953eed_2:
    "For just a moment, we shared in a comfortable silence punctuated by Mina's warm smile."

translate id w2MinaWallMenu_faec977c:
    mc "Would you mind telling me about another one of these photos, Mina?"

translate id w2MinaWallMenu_ecba54a0_1:
    mina "Sure. It's fun to reminisce. Which one do you want to hear about?"

translate id w2MinaWallMenu_d172a083:
    mc "Do you actually know how to use that thing?"

translate id w2MinaWallMenu_1eaaa06f:
    "I pointed at the photo of Mina and the two other girls, each sporting some serious looking compound bows."

translate id w2MinaWallMenu_6bd4fd86:
    mina "I would hope so. I was captain of my high school archery team."

translate id w2MinaWallMenu_e28acc19:
    mc "I didn't know schools around here had those."

translate id w2MinaWallMenu_2333c519:
    mina "There's a couple, but it was more of a club than a sports team. We were student funded and the chance to compete didn't come up often."

translate id w2MinaWallMenu_c55f6596:
    mc "So... hypothetically, if you decided to kill me, how far away would I have to be before I had a fighting chance at escape?"

translate id w2MinaWallMenu_88bb6597:
    mina "I'm accurate up to 50 yards."

translate id w2MinaWallMenu_d71e171e:
    "Mina puffed up her chest in an exaggerated fashion. I'm guessing that was a respectable number."

translate id w2MinaWallMenu_f5235fdb:
    mc "So, what's the story with that photo in particular then?"

translate id w2MinaWallMenu_297b43d9:
    mina "You're looking at half of the archery team there. The ones whose parents allowed them to travel out of state and compete regionally."

translate id w2MinaWallMenu_50eae118:
    mina "We had to provide our own transport, so the girls and I carpooled to the event. Just the three of us, without adult supervision."

translate id w2MinaWallMenu_e3edb7b8:
    mc "When was this?"

translate id w2MinaWallMenu_1d3a9aba:
    mina "Our senior year of high school."

translate id w2MinaWallMenu_e14cd011:
    mina "Those girls are a hoot, I had such a blast that weekend. We don't talk as much as we should anymore, but I guess that's what happens after high school."

translate id w2MinaWallMenu_af38ef3b:
    mc "Yeah, you go in different directions."

translate id w2MinaWallMenu_36508535:
    mina "Not you and Ian though?"

translate id w2MinaWallMenu_8ef56931:
    mc "Actually... we did? I mean, we kept in touch, mainly from the effort on his part, but until he got me a job at his Uncle's..."

translate id w2MinaWallMenu_8a34160c:
    "I dug deep to remember our previous conversation about Dr. Chuck and what she thought it was."

translate id w2MinaWallMenu_f97c3d58:
    mc "...jazz bar, I'd say we were on the outs."

translate id w2MinaWallMenu_7dc72489:
    mina "The way he always talked about you, I figured you guys still carried on like schoolboys. You came up, like a lot. All his anecdotes involved you."

translate id w2MinaWallMenu_86097170:
    mc "Yeah, that's pretty weird, huh?"

translate id w2MinaWallMenu_d9c125e0:
    mina "Hehe, sometimes I joked that he was in love with you, but now I think he just didn't have many friends."

translate id w2MinaWallMenu_a20cefa7_1:
    "..."

translate id w2MinaWallMenu_3f699eef:
    "We both let that thought hang in the air between us for a number of moments."

translate id w2MinaWallMenu_895146ae:
    mc "Well, there was that one time we promised to marry each other if we were still single when we turned 40."

translate id w2MinaWallMenu_27018746:
    mina "R-really...?!"

translate id w2MinaWallMenu_6b4c8b01:
    mc "No, I'm kidding."

translate id w2MinaWallMenu_fc938147:
    mina "Iknewthat!"

translate id w2MinaWallMenu_cb0aa9e1:
    mc "Sure you did, you dweeb. You looked worried!"

translate id w2MinaWallMenu_fb58eff0:
    mina "Okay fine, you got me. Hehe~♥"

translate id w2MinaWallMenu_d4e12666:
    mc "So going by your expression in the photo, you guys must've done pretty good at your competition?"

translate id w2MinaWallMenu_586eb445:
    mina "Nope! We came fourth out of five teams. Any and all smiles was all us."

translate id w2MinaWallMenu_3a1dbb21:
    mc "You don't look defeated at all."

translate id w2MinaWallMenu_19cc26f1:
    mina "Going on a trip with friends was more important to us than landing center of target."

translate id w2MinaWallMenu_681cb298:
    "The look on her face told me she really did mean that."

translate id w2MinaWallMenu_9bc472e5:
    "While I was here to help her prepare for her role, I was enjoying learning more about Mina."

translate id w2MinaWallMenu_b82b6873:
    mct "(I'll ask her about one more thing before moving on.)"

translate id w2MinaWallMenu_cb70afb1:
    mc "So..."

translate id w2MinaWallMenu_87eb2613:
    mc "Is that you performing Shakespeare?"

translate id w2MinaWallMenu_ccb81f59:
    mina "Yeah, an all-girl rendition of Romeo and Juliet. Would you believe I wanted the role of Mercutio? I got stuck with Juliet."

translate id w2MinaWallMenu_98b3823a:
    mc "You didn't want to be the star of the play?"

translate id w2MinaWallMenu_cdf84741:
    mina "Heck no! Juliet is a dumb kid who dies for a man she knew less than a week. Mercutio, on the other hand, is hilarious."

translate id w2MinaWallMenu_46ed7b37:
    mina "He calls Romeo out on his bull crap and he uses his dying words to make a pun. That's way more fun than playing a love sick floozy!"

translate id w2MinaWallMenu_2fb4ae64:
    mc "Well, when you put it that way..."

translate id w2MinaWallMenu_c93bf818:
    mina "To be honest, I pushed hard for something other than Romeo and Juliet. I wanted King Lear or Merchant of Venice, but for some stupid reason Romeo and Juliet is tradition for an all girl's school."

translate id w2MinaWallMenu_559ac647:
    mina "I would've {1}loved{2} to play Cordelia."

translate id w2MinaWallMenu_c9ba0d2e:
    mc "Doesn't she also die for a stupid reason?"

translate id w2MinaWallMenu_c0905f6e:
    mina "No, it's a great death! Juliet dies for a stranger. Cordelia dies for her father. Even if he's a jerk, there's a world of difference in my book."

translate id w2MinaWallMenu_4eb79506:
    mina "I mean, she's cast out by King Lear for her unflinching honesty in the very first act and what does she do? She doesn't swear revenge or harbor any hatred for her father like she probably should."

translate id w2MinaWallMenu_85f29212:
    mina "Instead, she proves her devotion by returning with an army and coming to his aid in the final acts. You could say she's unrealistic, but that's okay. Sometimes characters in stories should exemplify something bigger than ourselves."

translate id w2MinaWallMenu_c4eadabe:
    "Mina's face was practically alight, animated by a surprising degree of enthusiasm. More so than her usual manufactured amount."

translate id w2MinaWallMenu_d8e4907a:
    mc "You seem to really love this stuff. Is that why you want to be an actor?"

translate id w2MinaWallMenu_c8d54ac2:
    mina "I actually fell into acting just like I did modeling: it was something I tried out and just happened to learn how much fun it was."

translate id w2MinaWallMenu_61e30af2:
    mc "Heh, I bet a ton of struggling actors, whose lifelong dream it is, would hate you if they heard that. "

translate id w2MinaWallMenu_a568505c:
    mina "Did you always want to be a doctor?"

translate id w2MinaWallMenu_634b4930:
    mc "Always? Hmm... I mean I probably wanted to be a superhero when I was really young, but it was the first {3}realistic{4} thing."

translate id w2MinaWallMenu_8768d7f4:
    mina "That's fascinating how some people always knew."

translate id w2MinaWallMenu_69c12d30:
    mc "Oh? You find me fascinating do you?"

translate id w2MinaWallMenu_caf3f538:
    mina "Of course I do! You're a pretty interesting guy, [2]."

translate id w2MinaWallMenu_89c67602:
    "The way she answered my teasing matter-of-factly, like it was an obvious thing, actually caused me to blush."

translate id w2MinaWallMenu_d01da5a8:
    mina "Me? I've always struggled with who and what I want to be, but I think I've found it now. That's what you're here lending me a hand for."

translate id w2MinaWallMenu_41242acc:
    "She was right. I was here for a purpose, but I was enjoying learning more about Mina."

translate id w2MinaWallMenu_be705e07:
    mct "(I'll ask her about one more thing before we move on.)"

translate id w2MinaWallMenu_42b896f8:
    "So..."

translate id w2MinaWallMenu_0a9c30a3:
    mc "Tell me about the blown up, half-naked photo of yourself that you keep on your bedroom wall."

translate id w2MinaWallMenu_bc80185d:
    mina "Oh, you jerk!"

translate id w2MinaWallMenu_89073228:
    mc "I mean, you DID draw attention to it."

translate id w2MinaWallMenu_0cad3b63:
    mina "D-don't look so closely at it!"

translate id w2MinaWallMenu_64dfd591:
    mc "What are you suddenly embarrassed about? Didn't you think you looked good enough to hang it up on your wall?"

translate id w2MinaWallMenu_0c9e3eff:
    mina "Mmmh...!"

translate id w2MinaWallMenu_2acedb14:
    mc "Heh, sorry! I couldn't resist. You seem a tad self-concious about it, but here it is. What is the story with it?"

translate id w2MinaWallMenu_5bc2a1e7:
    mina "Ah, well... it's pretty simple really. Earlier this year was the first time I was on a magazine cover."

translate id w2MinaWallMenu_dfa3b388:
    mc "That sounds like a big deal."

translate id w2MinaWallMenu_14e0fa82:
    mina "It is...! Although, sticking it on the wall wasn't my idea. I mean, it is..."

translate id w2MinaWallMenu_7d542fe2:
    mc "A blown up, half-naked photo of yourself."

translate id w2MinaWallMenu_40471676:
    mina "Stop using my exact words, please and thank you!"

translate id w2MinaWallMenu_813dee5a:
    mina "...but yeah, would you believe it was a gift from my mother? She never really liked me modeling, but the moment she saw me on that cover, she was very proud."

translate id w2MinaWallMenu_9f232979:
    mc "What, she showed all her friends?"

translate id w2MinaWallMenu_be7a1757:
    mina "Um... yes, actually. It was really embarrassing. She even showed her pastor! Still..."

translate id w2MinaWallMenu_e0bcb988:
    mina "It was also very reaffirming, you know? It felt really good! So much so that when she had it framed and insisted I hang it up in my apartment, I could hardly refuse her."

translate id w2MinaWallMenu_56f2008b:
    mina "Not that I REALLY mind. Every time I look at it, I remember my mom's reaction and get that same nice feeling again. You have anything like that? That you look at and it reminds you of your feelings?"

translate id w2MinaWallMenu_3b21f354:
    mc "......"

translate id w2MinaWallMenu_2b35b6fe:
    mc "..."

translate id w2MinaWallMenu_5b5f980e:
    mct "(\"My mom's porn videos\", but it's not like I can tell her that.)"

translate id w2MinaWallMenu_d6e794b6:
    mc "...I'm not really the sentimental type I guess."

translate id w2MinaWallMenu_6cf71942:
    mina "Hehe, that's okay! Not many guys are."

translate id w2MinaWallMenu_8db247f2:
    "Hmm... while I was here to help her prepare for her role, I was also enjoying learning more about Mina."

translate id w2MinaWallMenu_59804b8f:
    mct "(I'll ask her about one more thing before moving on to the main reason I'm here.)"

translate id w2MinaWallMenu_cb70afb1_1:
    mc "So..."

translate id w2MinaWallMenu_f5e2d51a:
    mc "Ah, what do you say we finally get down to why I'm here?"

translate id w2MinaWallMenu_2a49fd2f:
    mina "Sure! We can stop and have lunch when the food gets here."

translate id w2MinaWallMenu_ad129542:
    mc "Helping an actor rehearse is a first for me. What do I need to do?"

translate id w2MinaWallMenu_04d09925:
    mina "Read from a piece of paper! Err.. well, a screen."

translate id w2MinaWallMenu_8dc89c94:
    mina "I have the script for the first episode right here on my tablet. I've got two scenes in it."

translate id w2MinaWallMenu_6ef41f99:
    mc "If I remember correctly, you said you were playing a student who's having an affair with her professor?"

translate id w2MinaWallMenu_775587e2:
    mina "Good memory! Yeah, it's not a huuuge role, but I am playing adjacent one of the show's leading men."

translate id w2MinaWallMenu_d33fac70:
    mct "(Loving Days in Lafayette...)"

translate id w2MinaWallMenu_ac31b525:
    mc "Seems pretty tawdry for daytime television."

translate id w2MinaWallMenu_9b24fd51:
    mina "It's television for bored housewives and aging spinsters. Tawdry is what sells."

translate id w2MinaWallMenu_08e873c4:
    mc "I'm reading for the part of Nick?"

translate id w2MinaWallMenu_8ccde408:
    mina "You are. Dr. Nick Rawlings, a southern gentleman, married man, father of three, and professor of history at Lafayette University."

translate id w2MinaWallMenu_3a6c1808:
    mina "He's sexually frustrated and has recently found his eyes wandering up co-ed's legs and down their shirts. "

translate id w2MinaWallMenu_c78cad10:
    mc "That's... colorful."

translate id w2MinaWallMenu_a3124d7a:
    mina "I'm playing Elizabeth 'Liv' Forester, one of his history students that has her sights set on him. She's pretty crazy."

translate id w2MinaWallMenu_d21bf900:
    mc "I see here the scene opens with me sitting down?"

translate id w2MinaWallMenu_ed50ab1d:
    mina "That's correct. The scene opens pre-affair, with Nick grading papers in his office after hours. He's trying to hurry and get home to his wife when..."

translate id w2MinaWallMenu_f3610318:
    "Mina gave a cute bunny hop for emphasis."

translate id w2MinaWallMenu_b5b5d654:
    mina "I enter, a siren who'll stop at nothing to seduce and ensnare the good professor with her womanly wiles. Muhaha!"

translate id w2MinaWallMenu_167e3eba:
    mc "You're already into it, huh?"

translate id w2MinaWallMenu_a44e5e08:
    mina "Just setting the mood. You don't feel like you have to match my energy or even act, I can work off of just a basic reading. Whatever you're most comfortable with, [2]."

translate id w2MinaWallMenu_c64818b2:
    mc "I don't know how good I'll be, but it could be fun. I'll at least try to give you something to work off of."

translate id w2MinaWallMenu_39f84f0b:
    mina "Hehe, thanks! Okay, I'm going to go out in the hall. When I enter, that'll be your cue! You have the first line"

translate id w2MinaWallMenu_0fd4d656_1:
    "......"

translate id w2MinaWallMenu_a20cefa7_2:
    "..."

translate id w2MinaWallMenu_d6886216:
    mc "We didn't have an appointment..."

translate id w2MinaWallMenu_495cb281:
    mc "...did we Miss Forester?"

translate id w2MinaWallMenu_e72ae987:
    mina "We did not, Professor Rawlings -- and call me Liv."

translate id w2MinaWallMenu_9138d501:
    mc "Can I help you with something? The campus is about to close down."

translate id w2MinaWallMenu_6bb3ede0:
    mina "I know, the building is almost empty. All the other offices are locked up and I reckon it's just {1}you and lil' ol' me{2} on this floor."

translate id w2MinaWallMenu_47b41c50:
    mc "That didn't answer my question. I'm in a rush to get grading done, so if you could just please tell me how I might help you..."

translate id w2MinaWallMenu_3f9dd6d5:
    mina "I noticed something inappropriate in yesterday's class and I wanted to discuss it."

translate id w2MinaWallMenu_edcac3cd:
    mc "Inappropriate? Yesterday was about generational conflict at the turn of the 19th century. What could you possibly have a problem with?"

translate id w2MinaWallMenu_43b27992:
    mina "Oh, it wasn't anything to do with the lecture, Professor Rawlings. Not at all. Your lesson was simply impeccable, as always."

translate id w2MinaWallMenu_60e46391:
    mc "What did you find inappropriate then?"

translate id w2MinaWallMenu_77a06996:
    "Mina bent forward, pointing her chest square in my face as if she was leaning over an imaginary desk."

translate id w2MinaWallMenu_ca043548:
    mina "The way you kept staring down my shirt."

translate id w2MinaWallMenu_6d6d1922:
    "A quick glance down at the tablet only denoted to leave a pregnant pause before the next line. I guess how I act out that pause is up to me..."

translate id w2MinaWallMenu_3567cfac:
    m_dev "If you get 6 or more scene points, you'll have the option to end the scene with a kiss."

translate id w2MinaDateSC_715eac08:
    "Taking a second to consider how a burgeoning pervert might actually respond, I let my gaze linger on Mina's chest."

translate id w2MinaDateSC_540cbc04:
    "While her being in sweats certainly made things easier, her large bust crammed into a tight hoodie still made me feel like I was being impolite for staring."

translate id w2MinaDateSC_6c036d3e:
    "Even though she was only wearing sweats, it was still an enticing sight thanks to her large bust being crammed inside the tight hoodie."

translate id w2MinaDateSC_75d9bcef:
    "It didn't take much \"acting\" for my eyes to wantonly linger on her chest before delivering my character's half-hearted denial."

translate id w2MinaDateSC_94cf2b77:
    mc "That's outrageous. I did no such thing!"

translate id w2MinaDateSC_e517b1d6:
    "While her being in sweats made Mina's pose less suggestive, her large bust crammed into the tight hoodie still made me feel like I was being impolite for staring."

translate id w2MinaDateSC_0a8d70ae:
    "Naturally, I felt like I should look away."

translate id w2MinaDateSC_0ff3dfc8:
    mc "That's outrageous. I did no such thing."

translate id w2MinaDateSC_9ebd7767:
    "My delivery felt pretty flat, but then again, I'm not an actor."

translate id w2MinaDateSC_a4325005:
    mct "(I'm supposed to be playing a pervert, right...?)"

translate id w2MinaDateSC_9f4b513a:
    "With that in mind, I kept my eyeline firmly on her chest."

translate id w2MinaDateSC_e83cd842:
    "While her being in sweats made Mina's pose less suggestive, it was still quite the sight. Her large chest was crammed into a tight hoodie and shoved in front of my face after all."

translate id w2MinaDateSC_ce5a242f:
    "The civilized part of me felt averse to it, but I decided it was what my character would do."

translate id w2MinaDateSC_2594bda3:
    mc "...that's outrageous. I did no such thing."

translate id w2MinaDateSC_095fb581:
    mina "That's not a very convincing denial, Professor."

translate id w2MinaDateSC_403b8585:
    "The next line read: \"I don't care what you believe, it simply isn't true.\""

translate id w2MinaDateSC_ac7e1f9b:
    mc "I don't care what you believe, it simply isn't true!"

translate id w2MinaDateSC_ffab8a03:
    "Summoning as much conviction as I could muster, I vehemently denied the accusation as if it was actually being levied against me."

translate id w2MinaDateSC_ab16561f:
    mina "Don't misunderstand me, sir. I said it was inappropriate, but I didn't say I was offended."

translate id w2MinaDateSC_0fd4d656:
    "......"

translate id w2MinaDateSC_a20cefa7:
    "..."

translate id w2MinaDateSC_563fc7e5:
    mct "(Ah! Wait! I was supposed to stand up.)"

translate id w2MinaDateSC_89fc98ea:
    mc "I don't care what you believe, it simply isn't true."

translate id w2MinaDateSC_2d4a34d0:
    "Standing up. A total power play. That seemed to me to be the correct way to deliver the line."

translate id w2MinaDateSC_ab16561f_1:
    mina "Don't misunderstand me, sir. I said it was inappropriate, but I didn't say I was offended."

translate id w2MinaDateSC_27905252:
    mc "I'll tell you what's inappropriate, you coming in here and--"

translate id w2MinaDateSC_775296bc:
    mc "Ah, shit. Sorry! Are you...?"

translate id w2MinaDateSC_28fc3ab7:
    mina "I'm alright, just keep going. You're getting into it!"

translate id w2MinaDateSC_50715fc8:
    "She was right. This was kind of fun."

translate id w2MinaDateSC_db7ef424:
    mct "(Right, um... where was I? Oh!)"

translate id w2MinaDateSC_8c348618:
    mc "*clear throats* Ah-um, I'll tell you what's inappropriate, you coming in here and making such outlandish claims..."

translate id w2MinaDateSC_c45b1e37:
    "Mina's going to cut me off at..."

translate id w2MinaDateSC_e9d7f956:
    mc "I'm a respectable--"

translate id w2MinaDateSC_b8bdfe6f:
    mina "Relax. Even assuming what I said was true, it's not like it's a crime if your gaze just happens to drop below a girl's neckline. Nor do I have any proof, nor would any take my word against the word of the esteemed, {1}respectable{2} Dr. Rawlings."

translate id w2MinaDateSC_dbc24939:
    mc "Then why are you here?"

translate id w2MinaDateSC_5089bc03:
    mina "For a very smart man, you sure are dense, Professor. What I'm trying to say is... I {1}like it{2} when you stare down my shirt in class."

translate id w2MinaDateSC_451a236e:
    mc "You do...?"

translate id w2MinaDateSC_dc35e443:
    mina "Mmmhum, I certainly do."

translate id w2MinaDateSC_746d98c9:
    "The script called for another pause. However, Mina's next line read \"Would you like to touch them?\" but it didn't prescribe any direction before that."

translate id w2MinaDateSC_dfd0849b:
    "Logically, the Professor's eyes would be drawn down to Elizabeth's bust and then she would deliver the offer."

translate id w2MinaDateSC_2b35b6fe:
    mc "..."

translate id w2MinaDateSC_f447dc1a:
    mina "Would you like to touch them?"

translate id w2MinaDateSC_770f9143:
    "There was something about Mina's close proximity that made grabbing a hold of her waist feel natural, so that was what I did."

translate id w2MinaDateSC_2b35b6fe_1:
    mc "..."

translate id w2MinaDateSC_f447dc1a_1:
    mina "Would you like to touch them?"

translate id w2MinaDateSC_ef67f1a1:
    mct "(Hmmm, physically rebuke...?)"

translate id w2MinaDateSC_60485088:
    mc "That's enough of that! I don't know what you think you're doing, but this is wholly out of line. Get out of my office, before I report your clumsy attempt at seduction to the dean!"

translate id w2MinaDateSC_8fc95d10:
    mc "That's enough of that! I don't know what you think you're doing, but this is wholly out of line."

translate id w2MinaDateSC_ad0047aa:
    mc "Get out of my office, before I report your clumsy attempt at seduction to the dean!"

translate id w2MinaDateSC_edf97808:
    mina "Alright, Professor Rawlings. Have it your way. Just let it be known, I'm {1}very{2} willing."

translate id w2MinaDateSC_49097ffc:
    mina "If you change your mind, let me know with one of your pervy gazes. I'll come running~♥"

translate id w2MinaDateSC_096cedf1:
    "Mina finished out the scene with an improvised wink."

translate id w2MinaDateSC_c9d35b1b:
    mina "Aaaaaand scene! Great job, [2]."

translate id w2MinaDateSC_44d5bf2b:
    mina "You've never acted before?"

translate id w2MinaDateSC_333bec9e:
    mc "Never."

translate id w2MinaDateSC_c117f3ed:
    mina "Well, you could fool me. You seem like a natural."

translate id w2MinaDateSC_f6ef5c18:
    mina "That was good. You really gave me a lot to work with."

translate id w2MinaDateSC_a64e8bc0:
    mc "Thanks."

translate id w2MinaDateSC_e6ebe569:
    mina "No, thank you. I have one more scene I'd like to run through with you. There's a few minor things in between, but it's all nonverbal. The teacher perving out some more, some dirty looks in the classroom..."

translate id w2MinaDateSC_4105d501:
    mina "All of it amounts to your character finally succumbing to the temptation of a hot and horny co-ed and throwing his better judgement out the window."

translate id w2MinaDateSC_9ce573a8:
    mc "That makes a ton of sense to me."

translate id w2MinaDateSC_312270ba:
    mina "Did I make a convincing seductress?"

translate id w2MinaDateSC_c3e0c183:
    mc "I don't know about Professor Rawlings, but I probably would've fallen for it."

translate id w2MinaDateSC_8059937a:
    mina "Hehe, I'll take it!"

translate id w2MinaDateSC_0ba1c044:
    mina "If the next scene makes you uncomfortable, it's TOTALLY fine if you just give me a line reading. We don't have to emote it as--"

translate id w2MinaDateSC_aa944bf9:
    mina "Oooh, I hope that's the food! I'm starving!"

translate id w2MinaDateSC_724576a2:
    mct "(What happens in the next scene?)"

translate id w2MinaDateSC_2d92943f:
    mc "Thanks for lunch."

translate id w2MinaDateSC_8b7b8d56:
    mina "Mmmh~dhont mhenthionet!"

translate id w2MinaDateSC_8ef88768:
    "The pair of us dug into the platter, an assortment of various types of sushi. Not a bad payment for a couple of hours of my time."

translate id w2MinaDateSC_b5407bb1:
    mc "Is this that place on the corner?"

translate id w2MinaDateSC_e40f62c4:
    mina "Uhuh, it's conveniently close."

translate id w2MinaDateSC_9c5423ca:
    mc "It was pretty fun running through that with you."

translate id w2MinaDateSC_7fde9a63:
    mina "Oh, yeah? I'm glad to hear I'm not just sucking up your time."

translate id w2MinaDateSC_be780b71:
    mc "Don't worry about that, I've got jack shit to do this summer now that I'm not tutoring brats any more."

translate id w2MinaDateSC_27611926:
    mc "Don't worry about that, I've got almost nothing to do this summer now that I'm tutoring anymore. "

translate id w2MinaDateSC_f397f3f1:
    mina "Did you not enjoy teaching?"

translate id w2MinaDateSC_43775b79:
    mc "Eh, well... it was a cushy job. It paid well, for what was basically just making sure someone's kid kept their nose in the books and grilling them on the material."

translate id w2MinaDateSC_6768fab9:
    mina "...but?"

translate id w2MinaDateSC_beb5a24d:
    mc "It was grating dealing with the parents. That and most of them were just rich assholes that took a lot of things for granted."

translate id w2MinaDateSC_db449b8c:
    mina "Life's too short, right? It's a good thing you left it behind then."

translate id w2MinaDateSC_1826f085:
    mc "Let's move away from \"don't likes\". What do you like about acting so much that you want to do it as a career?"

translate id w2MinaDateSC_afb252ba:
    mina "Hmm... this will probably sound cliché, but it IS true for me."

translate id w2MinaDateSC_c21878d8:
    mina "The thing I like best about acting is being someone else. I can leave indecisive Mina and her baggage at the door and totally forget about my silly worries."

translate id w2MinaDateSC_7fa71650:
    mina "A lot of actors try and draw from their own personal experience to genuinely portray a character, but that doesn't work for me. I need to be in the moment, to get out of my own head and operate on pure instinct."

translate id w2MinaDateSC_f2242376:
    mc "So, acting as a form of escapism?"

translate id w2MinaDateSC_c109ee0a:
    mina "Maybe. I just wouldn't have fun if I had to do it inside of my head."

translate id w2MinaDateSC_89172ab3:
    "I had always written off things like modeling and acting as vain pursuits, but to hear Mina talk about acting so whole heartedly with full pathos had me feeling a little foolish."

translate id w2MinaDateSC_ca6f7f62:
    mc "I think that's really cool."

translate id w2MinaDateSC_fb763343:
    mina "Hehe~ thanks, Ian never wants to talk about work, mine or his, so it's nice that you asked."

translate id w2MinaDateSC_87475d86:
    mina "Oh, you got a piece of..."

translate id w2MinaDateSC_523367fd:
    mc "Here?"

translate id w2MinaDateSC_712b0127:
    mina "No, no... let me..."

translate id w2MinaDateSC_dc14af27:
    "Mina got up close, bringing her dainty hand up to my chin and extending a delicate finger to wipe off the offending detritus."

translate id w2MinaDateSC_e306cb9d:
    mina "There!"

translate id w2MinaDateSC_fa45f281:
    mc "Thanks..."

translate id w2MinaDateSC_9ebed1e9:
    "I wasn't sure what it was about this particular moment, but I was acutely and abruptly aware that her and I were {3}alone{4} in her room."

translate id w2MinaDateSC_f8593417:
    "She didn't NEED to do that, I thought."

translate id w2MinaDateSC_8307ff59:
    mina "So... Hana was super cool. Are you two...?"

translate id w2MinaDateSC_bdd91c0e:
    mc "No, we're not. Why do you ask?"

translate id w2MinaDateSC_ba95b182:
    mc "That's... no, not really. Why do you ask?"

translate id w2MinaDateSC_2f1ab392:
    mc "No, nothing of the sort. Why do you ask?"

translate id w2MinaDateSC_c27fe8a2:
    mina "With the way Ian pushed me out the door this morning mumbling about giving you two some space, I got the impression you guys might be... involved?"

translate id w2MinaDateSC_6b61de6b:
    "Hana and I did fool around last night, but {3}involved{4} didn't quite fit it."

translate id w2MinaDateSC_4fa51609:
    mc "Well, you're right about her being cool, but we're not dating. That was the second time we've ever hung out."

translate id w2MinaDateSC_4a88f5d3:
    mc "Well, you're right about her being cool, but we're not dating. That was the first time we've hung out."

translate id w2MinaDateSC_4ed92665:
    mc "Yeesh, thinking about it, I've been single for a couple of years now."

translate id w2MinaDateSC_3d53b633:
    mina "How come? Too focused on school?"

translate id w2MinaDateSC_51222727:
    mc "Mmmhh... I guess, but it's not like I purposefully put it on the backburner. Meeting people to date just never happened for me."

translate id w2MinaDateSC_df532dda:
    mina "Ah... I can't say anything, up until 11 months ago, I'd never been in a relationship."

translate id w2MinaDateSC_0fd4d656_1:
    "......"

translate id w2MinaDateSC_a20cefa7_1:
    "..."

translate id w2MinaDateSC_f0a1503b:
    mc "Ian's your first boyfriend?"

translate id w2MinaDateSC_bdea47f9:
    "That... made all the sense in the world, actually. THERE was my answer about why she's put up with his crap for so long."

translate id w2MinaDateSC_8b318f6e:
    mina "I w-went to an all girl's school, okay? It's not like I had a ton of options for normal romance growing up."

translate id w2MinaDateSC_ce3fb92f:
    mina "I did have a \"girlfriend\" for a couple of months, but that was just something the student body did for fun since there weren't any boys around."

translate id w2MinaDateSC_2b0f460c:
    mc "Don't get self-conscious about it. You're nineteen. That's not TOO weird."

translate id w2MinaDateSC_29b1fb25:
    mina "It's... not?"

translate id w2MinaDateSC_70810866:
    mc "No. You just skipped a bunch of hormonal, awkward shit."

translate id w2MinaDateSC_f54d4282:
    mina "Mmmh, Felicia teases me about it though."

translate id w2MinaDateSC_6e9b38d9:
    mc "Heh, I wouldn't look to her as an arbiter of what's normal."

translate id w2MinaDateSC_a2e6e787:
    mina "Hehe~ you're right. She IS kinda a ho -- in a lovable way, of course!"

translate id w2MinaDateSC_e94d5ff9:
    mina "Speaking of, let's finish eating. I've got another scene to slut it up in!"

translate id w2MinaDateSC_2e451cdd:
    "Hearing her say that with a pure-hearted smile was a detonative combination."

translate id w2MinaDateSC_0fd4d656_2:
    "......"

translate id w2MinaDateSC_a20cefa7_2:
    "..."

translate id w2MinaLineReadingPart2_5f2de45f:
    "Just as before, Mina exited and re-entered the room to kickstart the scene."

translate id w2MinaLineReadingPart2_6193c6cb:
    "As she had described, apparently between the first scene and this one, Nick Rawlings had tossed all professional propriety out the window through a montage of goo-goo eyed classroom flirtations."

translate id w2MinaLineReadingPart2_0019c7b5:
    mct "(Got to get to the good stuff quick for the viewers at home, I suppose.)"

translate id w2MinaLineReadingPart2_8ce60d3b:
    mina "You wanted to see me, Professor?"

translate id w2MinaLineReadingPart2_32d435e3:
    mc "Get out of the doorway and come in, Miss Forester."

translate id w2MinaLineReadingPart2_b1187ee7:
    mina "It sounded serious, is something the matter?"

translate id w2MinaLineReadingPart2_b6387e54:
    "Let's see... the script says to leer at her before moving onto the next line."

translate id w2MinaLineReadingPart2_2b35b6fe:
    mc "..."

translate id w2MinaLineReadingPart2_df866f63:
    "Perhaps a worrying thought, but slipping into the right mindset came shockingly easy to me as I slid my eyes past Mina's shapely thighs and well-pronounced hips, up the flat of her tummy and lingered on her breasts to fully admire the view."

translate id w2MinaLineReadingPart2_470bea2c:
    "It was a proper and despicable eye-fucking, grimy and filthy, the kind that would not fail to send a shiver up someone's back. Man or woman."

translate id w2MinaLineReadingPart2_d47295cf:
    "It was a look that I would never, in my life, dare to make."

translate id w2MinaLineReadingPart2_9e89ec4f:
    mct "(Isn't acting great?)"

translate id w2MinaLineReadingPart2_7db1133d:
    mc "I've noticed your attention drifting in class this week."

translate id w2MinaLineReadingPart2_d68d3a83:
    mina "Ehmm...?"

translate id w2MinaLineReadingPart2_4e2a8876:
    mina "..."

translate id w2MinaLineReadingPart2_170c6b0f:
    mina "Line please?"

translate id w2MinaLineReadingPart2_9badf2a7:
    "Mina looked at me expectantly and I glanced down at the tablet."

translate id w2MinaLineReadingPart2_b5fd6aec:
    mc "Sorry, professor. I have just had a lot on my mind."

translate id w2MinaLineReadingPart2_1cd8b674:
    mina "Right, right... sorry professor..."

translate id w2MinaLineReadingPart2_dc5d50fd:
    mc "When you're in my class, I require nothing short of your undivided attention."

translate id w2MinaLineReadingPart2_554abd30:
    "I decided to circle around Mina like she was a piece of meat."

translate id w2MinaLineReadingPart2_65e0c83c:
    "Once behind her, I took in the sights, running my gaze up her shapely thighs and fixated on her perfectly round bubble butt."

translate id w2MinaLineReadingPart2_5428e2e1:
    "The slight arch in her back would draw my gaze ever so often, and I couldn't help but see in my mind's eye a more exaggerated picture of Mina being ridden."

translate id w2MinaLineReadingPart2_a1b5c364:
    "It was a little worrying how easily I slipped into this sort of grimy mindset and I was glad she couldn't see the eye-fucking I was giving her all in the pursuit of a convincing portrayal."

translate id w2MinaLineReadingPart2_9e89ec4f_1:
    mct "(Isn't acting great?)"

translate id w2MinaLineReadingPart2_7db1133d_1:
    mc "I've noticed your attention drifting in class this week."

translate id w2MinaLineReadingPart2_099e08c0:
    mina "Sorry, professor. I have just had a lot on my mind."

translate id w2MinaLineReadingPart2_dc5d50fd_1:
    mc "When you're in my class, I require nothing short of your undivided attention."

translate id w2MinaLineReadingPart2_62a24b38:
    mina "That's the problem. I can't focus on the material when I've got my eyes on you, Professor."

translate id w2MinaLineReadingPart2_369c1cff:
    mct "(Ugh, who writes this crap?)"

translate id w2MinaLineReadingPart2_cd6956db:
    mc "That's... unacceptable. We've got to fix that."

translate id w2MinaLineReadingPart2_8f18d40e:
    "Looking ahead a handful of lines, it looks like my character is going to be taking an active, borderline {1}aggressive{2} role this time."

translate id w2MinaLineReadingPart2_4b2d3d55:
    mina "How do you propose we remedy this problem, professor?"

translate id w2MinaLineReadingPart2_36fab246:
    "Gently, I brushed the underside of my fingers against the supple skin of her cheek."

translate id w2MinaLineReadingPart2_543ba0aa:
    mc "That depends on you, {1}Elizabeth{2}."

translate id w2MinaLineReadingPart2_823fe34b:
    "Slowly, Mina looked up at me, eyes brimming with anticipation, like a prisoner who was about to be set free."

translate id w2MinaLineReadingPart2_903ade85:
    mct "(She's a damn good actor.)"

translate id w2MinaLineReadingPart2_373fef46:
    mina "Depends on what, sir?"

translate id w2MinaLineReadingPart2_c2550e5f:
    mc "We do this my way."

translate id w2MinaLineReadingPart2_1ae6105a:
    "Taking care not to do it too rough, but still putting a bit of gusto into it, I forcefully grabbed Mina by the chin and leveled her gaze with mine."

translate id w2MinaLineReadingPart2_543ba0aa_1:
    mc "That depends on you, {1}Elizabeth{2}."

translate id w2MinaLineReadingPart2_97ac57af:
    "Mina's eyes smoldered with sexual tension, challenging me with a dirty look that would give a lecher pause."

translate id w2MinaLineReadingPart2_903ade85_1:
    mct "(She's a damn good actor.)"

translate id w2MinaLineReadingPart2_373fef46_1:
    mina "Depends on what, sir?"

translate id w2MinaLineReadingPart2_c2550e5f_1:
    mc "We do this my way."

translate id w2MinaLineReadingPart2_f7c21205:
    mc "This can't leave this room. You understand? The university has a strict no fraternization policy between teachers and students."

translate id w2MinaLineReadingPart2_0c97ab05:
    mina "Of course, my lips are sealed. Where~EVER you'd like them."

translate id w2MinaLineReadingPart2_7199ce77:
    "Mina's inflection dripped with sex appeal, one that perfectly suited the incongruency of Mina's innocent, sweet-looking face and her bombastic curves."

translate id w2MinaLineReadingPart2_de3f262e:
    "She could easily have men at her feet if she wanted. The fact that she seemingly doesn't know that only added to her charm."

translate id w2MinaLineReadingPart2_979b68fe:
    mina "Next line?"

translate id w2MinaLineReadingPart2_e35bc5ff:
    mc "Eh, oh...? I guess I got lost in your eyes for a second."

translate id w2MinaLineReadingPart2_ce22f3d7:
    mina "Oh..."

translate id w2MinaLineReadingPart2_7b348fd3:
    mc "Let's go back a line, please."

translate id w2MinaLineReadingPart2_4e2a8876_1:
    mina "..."

translate id w2MinaLineReadingPart2_72e35c40:
    mina "Of course, my lips are sealed. WhereEVER you'd like them."

translate id w2MinaLineReadingPart2_c24dc03b:
    mc "Elizabeth..."

translate id w2MinaLineReadingPart2_9440d514:
    mina "Y-yes, Professor?"

translate id w2MinaLineReadingPart2_3d2980dd:
    mc "You're a good girl, right?"

translate id w2MinaLineReadingPart2_4ffbd28d:
    mina "If that's what you want."

translate id w2MinaLineReadingPart2_8c10c0b4:
    mc "I need you to be serious."

translate id w2MinaLineReadingPart2_544400eb:
    mc "I need you to promise me, Elizabeth. I'm a married man, this is nothing more than a fling. If word got out..."

translate id w2MinaLineReadingPart2_067eb230:
    mina "I am, Professor. Feel!"

translate id w2MinaLineReadingPart2_55feb8f4:
    mc "--!"

translate id w2MinaLineReadingPart2_876bb255:
    mina "Feel how fast my heart is beating, I'm very serious about this."

translate id w2MinaLineReadingPart2_8b7ce216:
    "Bereft of any hesitation, Mina took ahold of my hand in hers and boldly placed it on her left breast."

translate id w2MinaLineReadingPart2_ca92a1f7:
    mina "I promise. We'll do this on your terms."

translate id w2MinaLineReadingPart2_fe670a96:
    "The tips of my fingers all but clamped down, held back at the last microsecond by a snap response of ingrained civility."

translate id w2MinaLineReadingPart2_98e19feb:
    mina "This is all strictly just a professor helping out a student with her problem."

translate id w2MinaLineReadingPart2_bab3ef1c:
    mct "(This woman, does she not see me as a man?)"

translate id w2MinaLineReadingPart2_d4865db8:
    "Of course, there was no way I could feel Mina's heartbeat through a handful of meaty tit flesh, but I DID spot an inkling of inhibition."

translate id w2MinaLineReadingPart2_e7ee88e9:
    "The moment my palm made hand-over-cloth contact with the upbeat actress' breast, Mina tensed up and a spot of color hit her face."

translate id w2MinaLineReadingPart2_67c457ac:
    mct "(Fuck, what was the next line...?)"

translate id w2MinaLineReadingPart2_2ae9d9f1:
    "Given the circumstances, how could I hope to recall the line I had read just a handful of seconds before?"

translate id w2MinaLineReadingPart2_967d43eb:
    mina "I think the line is something like..."

translate id w2MinaLineReadingPart2_a74263ad:
    "Right, Mina jarred my memory."

translate id w2MinaLineReadingPart2_6660d796:
    "If she had put my hand there without thinking, she wasn't backing down. Part of me considered that she did it simply because she didn't care. This could all just be an exercise in acting to her."

translate id w2MinaLineReadingPart2_3aff9aef:
    "In the back of my head however, my thoughts swung the other way. The way she was looking at me..."

translate id w2MinaLineReadingPart2_e4a5133c:
    mc "You certainly {3}feel{4} sincere."

translate id w2MinaLineReadingPart2_1ffb018f:
    mct "(Could she be purposefully pushing boundaries?)"

translate id w2MinaLineReadingPart2_dcb70c14:
    mina "OooOh, um... ahem..."

translate id w2MinaLineReadingPart2_b38717a5:
    "Whether or not that was actually the case, I sunk my fingers deeper into the buoyant flesh of her breast. Enough to get her attention, but taking care not to be too overt about it."

translate id w2MinaLineReadingPart2_ef55dae5:
    mina "O-oh...!"

translate id w2MinaLineReadingPart2_908b8bad:
    "The extra attention appeared to confound her, as no line came from her lips. Instead, her head drooped down and her mind veered off elsewhere."

translate id w2MinaLineReadingPart2_e7735a7f:
    "It was hard to say if she was simply feeling shy from being called on her game of chicken or if I had taken it too far. Either way, she was suddenly pacified."

translate id w2MinaLineReadingPart2_c1f9f7d8:
    mina "Ha.. ha... [2]?"

translate id w2MinaLineReadingPart2_d1dfb99b:
    mina "Sorry, but that felt so SURPRISINGLY good that I forgot my line, hehe..."

translate id w2MinaLineReadingPart2_d320f3db:
    "Hearing her just coming out and saying it shocked me. She actually doesn't think any of this is crossing the line?"

translate id w2MinaLineReadingPart2_23214395:
    mina "You could always be a masseuse if the whole med student thing doesn't work out."

translate id w2MinaLineReadingPart2_4c5c5ae7:
    mc "Very funny, okay so you do this next and then the line is... Then I..."

translate id w2MinaLineReadingPart2_5de12713:
    mina "Ahem..."

translate id w2MinaLineReadingPart2_69669349:
    mina "Ngh, [2]...?"

translate id w2MinaLineReadingPart2_7f7101df:
    mina "S-sorry, I forgot the line!"

translate id w2MinaLineReadingPart2_01c9f9d4:
    mc "Okay, so you... and then the line is... Then I..."

translate id w2MinaLineReadingPart2_5de12713_1:
    mina "Ahem..."

translate id w2MinaLineReadingPart2_6660d796_1:
    "If she had put my hand there without thinking, she wasn't backing down. Part of me considered that she did it simply because she didn't care. This could all just be an exercise in acting to her."

translate id w2MinaLineReadingPart2_a46d1519:
    "In the back of my head however, my thoughts swung the other way. Could she be purposefully pushing boundaries?"

translate id w2MinaLineReadingPart2_e4a5133c_1:
    mc "You certainly {3}feel{4} sincere."

translate id w2MinaLineReadingPart2_5fac0dc6:
    "Either way, it was best I didn't push it."

translate id w2MinaLineReadingPart2_e212a28c:
    mc "Hold on a second, let me check my next few lines."

translate id w2MinaLineReadingPart2_d8bbd638:
    mct "(This really airs on TV?)"

translate id w2MinaLineReadingPart2_1d905c59:
    mc "Okay, we were at..."

translate id w2MinaLineReadingPart2_00c9c67e:
    mina "Is there anything else you'd like to feel, Professor?"

translate id w2MinaLineReadingPart2_050a618e:
    "The moment Mina turned around and pressed her butt into me confirmed that she truly had no qualms with this sort of contact."

translate id w2MinaLineReadingPart2_9728ca14:
    "She even took it a step further, subtly rubbing herself against me like an animal seeking warmth. Perfectly suited for the scene perhaps, but nonetheless an obstacle in trying to remember my goddamn line."

translate id w2MinaLineReadingPart2_d52d8f20:
    "The action also confirmed what I had previously suspected. She had an IMPECCABLE ass, round and tight, with the right amount of heft a man could get behind."

translate id w2MinaLineReadingPart2_77d99034:
    "I felt a pull in my arms, telling me to grab her hips. It just seemed right, given the circumstances..."

translate id w2MinaLineReadingPart2_6d17f08b:
    mct "(It's part of the scene, right?)"

translate id w2MinaLineReadingPart2_d52d8f20_1:
    "The action also confirmed what I had previously suspected. She had an IMPECCABLE ass, round and tight, with the right amount of heft a man could get behind."

translate id w2MinaLineReadingPart2_77d99034_1:
    "I felt a pull in my arms, telling me to grab her hips. It just seemed right, given the circumstances..."

translate id w2MinaLineReadingPart2_6d17f08b_1:
    mct "(It's part of the scene, right?)"

translate id w2MinaLineReadingPart2_71b243e9:
    "Clasping my hands around her hip bone, I brought the gentle sway to a halt and pressed Mina more firmly into me."

translate id w2MinaLineReadingPart2_25d4c536:
    "Clasping my hands around her hip bone, I pressed Mina more firmly into me."

translate id w2MinaLineReadingPart2_1a7b858b:
    mc "I can think of a FEW things."

translate id w2MinaLineReadingPart2_1a7b858b_1:
    mc "I can think of a FEW things."

translate id w2MinaLineReadingPart2_3ffbfe10:
    "The script didn't specifically call for it, but I felt the urge to..."

translate id w2MinaLineReadingPart2_699f10e9:
    "*Smack!*"

translate id w2MinaLineReadingPart2_219762e3:
    mina "Oh...!"

translate id w2MinaLineReadingPart2_e2ae23c9:
    "I gave Mina's tight rear end a light tap, the impact of which elicited a cute yelp from my acting partner."

translate id w2MinaLineReadingPart2_699f10e9_1:
    "*Smack!*"

translate id w2MinaLineReadingPart2_f1f47b0a:
    mina "Eeeh?"

translate id w2MinaLineReadingPart2_71b867a9:
    "Give Mina's tight rear end a light tap, the impact of which produced a whelp of confusion from my acting partner.."

translate id w2MinaLineReadingPart2_c6e03f7b:
    "No. Don't touch. Bad."

translate id w2MinaLineReadingPart2_3cc32f22:
    "With a weapons-grade level of pep, Mina spun on the heel of her foot like a ballerina doing a pirouette before twisting her mouth into a predatory, shark-like grin."

translate id w2MinaLineReadingPart2_c6f42d93:
    mina "Don't just think of them, [2]-er, I mean... Professor."

translate id w2MinaLineReadingPart2_c91023ca:
    mina "Don't just think of them, Professor."

translate id w2MinaLineReadingPart2_f7836116:
    mina "Do them. "

translate id w2MinaLineReadingPart2_20e46bf1:
    "What was called for next by the script was a kiss, shared between our characters, that one would naturally skip over in a situation like this. That, however, didn't seem to be the case."

translate id w2MinaLineReadingPart2_c49848dc:
    "Instead, Mina looked deep into my eyes with a disarming sensuality that was distantly removed from the cheap, tasteless material we were reciting."

translate id w2MinaLineReadingPart2_12ed90e5:
    "It was like she was patiently waiting for me to make MY move, like it wasn't an unusual prospect to kiss your friend's girlfriend under the guise of acting practice."

translate id w2MinaLineReadingPart2_905b468f:
    "In fact, the certainty in her expression actually made it seem innocent."

translate id w2MinaLineReadingPart2_b9776ac7:
    mina "Mmmh...?"

translate id w2MinaLineReadingPart2_0f8c33c1:
    "Okay, maybe not so patiently."

translate id w2MinaLineReadingPart2_f45cc878:
    "Wrapping my hands around Mina, I gave her one last long look in the eyes, before pulling her tightly close to me like an action movie hero who just got the girl."

translate id w2MinaLineReadingPart2_4099919f:
    mct "(This is kind of weird...)"

translate id w2MinaLineReadingPart2_ec3cc1a1:
    "...is what I thought, but the more her emerald green eyes looked back at me, the less of a sticking point it became."

translate id w2MinaLineReadingPart2_1bf81c1e:
    mina "Nhwah..."

translate id w2MinaLineReadingPart2_7dbdde68:
    "A cute, sharp coo and then the taste of vanilla mint chapstick."

translate id w2MinaLineReadingPart2_b176fc12:
    mct "(Not a bad flavor...)"

translate id w2MinaLineReadingPart2_123ac8ce:
    "It wasn't a deeply lewd kiss or anything, but our lips were wholly and fully locked together."

translate id w2MinaLineReadingPart2_f61f9e53:
    "It skirted the line between passionate and tasteful, like a kiss shared between a bride and groom on their wedding day."

translate id w2MinaLineReadingPart2_f3b8ca2c:
    "However..."

translate id w2MinaLineReadingPart2_780af5a5:
    "She must have not been content with how I was doing it, because she pulled me deeper into the kiss."

translate id w2MinaLineReadingPart2_1b57bbae:
    "My hands found perch on her ass, prodding and teasing it, almost completely of their own volition."

translate id w2MinaLineReadingPart2_e6019d07:
    mina "Mmmmh...♥"

translate id w2MinaLineReadingPart2_4af1f3a6:
    "Another, even more sharp coo. Not that I wasn't enjoying it either..."

translate id w2MinaLineReadingPart2_9e7317cd:
    "By this point, my cock was firmly pressing into Mina, poking her lower body in a way that would be impossible to miss."

translate id w2MinaLineReadingPart2_985e34e0:
    "Not that she had any room to complain, with the way she was rubbing her body against me like a bitch in heat."

translate id w2MinaLineReadingPart2_3753f42b:
    "Part of me felt embarrassed about it on a instinctual level, but if she did indeed notice, it didn't deter her in the least."

translate id w2MinaLineReadingPart2_ae9cbd82:
    "By the time we broke up our kiss, it had felt like it had gone on for far longer than it had. Everything felt hazy on my end and Mina's glassed over expression told me she was likely feeling the same."

translate id w2MinaLineReadingPart2_2422a262:
    mina "Ha...♥"

translate id w2MinaLineReadingPart2_b14674dd:
    "...not that I needed it, considering I could've known that by the way she humped my leg like a wayward wind-up toy slowly losing its pep."

translate id w2MinaLineReadingPart2_89c38c1d:
    mina "Ah, isn't acting grand...?"

translate id w2MinaLineReadingPart2_08fdc04f:
    mc "...um, Mina? I forgot the next line."

translate id w2MinaLineReadingPart2_ed79fefc:
    mina "Oh? Um, h-hold on... let me think..."

translate id w2MinaLineReadingPart2_a878749c:
    mina "Good... \"{3}acting{4}\" there, partner."

translate id w2MinaLineReadingPart2_347396ce:
    mc "...ngh, yeah. You too."

translate id w2MinaLineReadingPart2_08fdc04f_1:
    mc "...um, Mina? I forgot the next line."

translate id w2MinaLineReadingPart2_bf5547db:
    "When we broke from our kiss, it hadn't been an inordinate amount of time. Still, I could feel a blush in my cheeks and I could see Mina doing the same."

translate id w2MinaLineReadingPart2_4e2a8876_2:
    mina "..."

translate id w2MinaLineReadingPart2_1caf82e3:
    mct "(My thoughts exactly...)"

translate id w2MinaLineReadingPart2_08fdc04f_2:
    mc "...um, Mina? I forgot the next line."

translate id w2MinaLineReadingPart2_55055d98:
    mina "You say \"How was that?\" and then I say, \"That was everything I was hoping it'd be.\" or something like that."

translate id w2MinaLineReadingPart2_ee48fd79:
    mina "Hehe~, I'm a little fuzzy on the lines all of a sudden for some reason."

translate id w2MinaLineReadingPart2_f44c83b9:
    mct "(This woman...)"

translate id w2MinaLineReadingPart2_acef1af6:
    "She carried on, business as usual. Unconcerned with what we just did."

translate id w2MinaLineReadingPart2_0314786e:
    mina "Then it's fade to black and all my parts are done for the episode, hehe~"

translate id w2MinaLineReadingPart2_5188763b:
    mc "Ah, crap. I forgot my next line."

translate id w2MinaLineReadingPart2_0c2d74d8:
    "I had a semi-plausible, convenient excuse, but I didn't know if kissing my friend's girlfriend was a river I wanted to forge - even if, quite frankly, I wasn't entirely sure he'd give a shit."

translate id w2MinaLineReadingPart2_94e3b07f:
    mina "Hmm... I think..."

translate id w2MinaLineReadingPart2_55055d98_1:
    mina "You say \"How was that?\" and then I say, \"That was everything I was hoping it'd be.\" or something like that."

translate id w2MinaLineReadingPart2_0314786e_1:
    mina "Then it's fade to black and all my parts are done for the episode, hehe~"

translate id w2MinaLineReadingPart2_198419d8:
    mina "...then there's a couple more lines."

translate id w2MinaLineReadingPart2_55055d98_2:
    mina "You say \"How was that?\" and then I say, \"That was everything I was hoping it'd be.\" or something like that."

translate id w2MinaLineReadingPart2_0314786e_2:
    mina "Then it's fade to black and all my parts are done for the episode, hehe~"

translate id w2MinaLineReadingPart2_777be2e4:
    mina "It's a small couple of scenes, but things pick up in later episodes - or so I'm told, if they don't kill off my character by having her accidently fall into a volcano or something."

translate id w2MinaLineReadingPart2_1f40c557:
    mc "...a volcano?"

translate id w2MinaLineReadingPart2_fe030d18:
    mina "I'm told that one has has happened in the show's history."

translate id w2MinaLineReadingPart2_2b35b6fe_1:
    mc "..."

translate id w2MinaLineReadingPart2_e73a6708:
    mina "Hey, I know I'm not making art here!"

translate id w2MinaLineReadingPart2_9527ff1b:
    mina "Pfft--"

translate id w2MinaLineReadingPart2_ba49f04d:
    "The two of us shared in a laugh, completely dispelling any lingering tension from the previous scene."

translate id w2MinaLineReadingPart2_0fd4d656:
    "......"

translate id w2MinaLineReadingPart2_a20cefa7:
    "..."

translate id w2MinaLineReadingPart2_f2279fad:
    mina "You sure you don't want me to drive you?"

translate id w2MinaLineReadingPart2_d800b180:
    mc "Nah, I think I'll walk. I haven't been getting enough exercise lately. I'm getting pretty round around the edges."

translate id w2MinaLineReadingPart2_725f5c4c:
    mina "Speaking from recent experience, I know that's not true."

translate id w2MinaLineReadingPart2_00c31bcc:
    mc "Hehe..."

translate id w2MinaLineReadingPart2_e0d043f2:
    mina "Anyway, you suuuuuuure?"

translate id w2MinaLineReadingPart2_9d193e45:
    mc "Yeah, but thanks for offering - and for the lunch. It was delicious."

translate id w2MinaLineReadingPart2_ca565c7b:
    mina "Of course...!"

translate id w2MinaLineReadingPart2_5ba08197:
    mina "...hey, by the way, you doing anything this Friday?"

translate id w2MinaLineReadingPart2_11bc456b:
    mc "Not yet. Why?"

translate id w2MinaLineReadingPart2_7b652b31:
    mina "Well, my brother and I have some standing plans. We'll probably go to an arcade or something, you want to join?"

translate id w2MinaLineReadingPart2_9551f9ac:
    mc "You want the three of us to go to an arcade?"

translate id w2MinaLineReadingPart2_beb7482c:
    mina "Yep, why not? It'll be fun and, well..."

translate id w2MinaLineReadingPart2_f2e9ee40:
    mina "It's not like I want you to play big brother or anything, but it would do him some good to socialize with someone like you."

translate id w2MinaLineReadingPart2_e95da68b:
    mc "Someone like me?"

translate id w2MinaLineReadingPart2_fb316a4f:
    mina "You know, a young adult male that has his head screwed on straight."

translate id w2MinaLineReadingPart2_8fa837ca:
    mc "Let him see what's at the end of the tunnel?"

translate id w2MinaLineReadingPart2_ad979c73:
    mina "Exactly! Jeez, you always know how to put things."

translate id w2MinaLineReadingPart2_be66917f:
    mc "Sure, I'll be happy to tag along. Not so sure how he'll feel about it though."

translate id w2MinaLineReadingPart2_048ae5c0:
    mina "Eh, he'll be shy at first, but he'll get over it! He needs to get out of his comfort zone from time to time."

translate id w2MinaLineReadingPart2_f1536ed1:
    mina "Anywaaaaay...."

translate id w2MinaLineReadingPart2_c04b1e25:
    mina "Um... hug goodbye?"

translate id w2MinaLineReadingPart2_41782ea0:
    "Mina suddenly became... awkward? She's asking for a hug instead of simply throwing herself into one?"

translate id w2MinaLineReadingPart2_ea61e618:
    mina "Hehe~"

translate id w2MinaLineReadingPart2_df18f83a:
    mina "I'll see you later again. Be safe!"

translate id w2MinaLineReadingPart2_cd57a3ac:
    mina "Anywaaaaay.... I'll see you later."

translate id w2MinaLineReadingPart2_c03e0a18:
    mina "Be safe on the way home, okay?"

translate id w2MinaLineReadingPart2_977af6a7:
    mc "Thanks, I will. Text me the details about Friday later."

translate id w2MinaLineReadingPart2_c4b3ad07:
    mina "Will do."

translate id w2MinaLineReadingPart2_0fd4d656_1:
    "......"

translate id w2MinaLineReadingPart2_a20cefa7_1:
    "..."

translate id w2June08Evening_b9d238d1:
    "I went straight home from the club, determined to be productive with my time.."

translate id w2June08Evening_a37d7403:
    "After leaving Mina's and walking home, the afternoon was pretty much waning. Still, I was determined to make productive use of my time."

translate id w2June08Evening_22730c15:
    "I ran through some MCAT exercises, looked into how the hell I'm going to file my taxes next year, and watched videos of people falling down on the internet."

translate id w2June08Evening_1846a4f9:
    "Two out of three ain't bad."

translate id w2June08Evening_3f9d71e9:
    mc "Mmmmmm...."

translate id w2June08Evening_72cb1e92:
    mc "Muaahh!"

translate id w2June08Evening_78076d2a:
    mct "(Come to think of it... fuck, I'm hungry. I only had a snack when I came in...)"

translate id w2June08Evening_7cd25bd7:
    mct "(Come to think of it, I'm getting pretty hungry. That sushi was good, but it wasn't very filling...)"

translate id w2June08Evening_21b01ada:
    mct "(I don't really feel like preparing anything, but takeout twice in one day seems wasteful. What to do?)"

translate id w2June08Evening_08a756eb:
    "{3}*Ring... ring...*{4}"

translate id w2June08Evening_2b35b6fe:
    mc "..."

translate id w2June08Evening_78198e6b:
    "{3}*Ring... ring... ring...*{4}"

translate id w2June08Evening_68ae2005:
    mct "(It's Mrs. Pulman...)"

translate id w2June08Evening_48f1d0ea:
    mct "(It's Kathleen...)"

translate id w2June08Evening_aca26b4b:
    "I very seriously considered ignoring it."

translate id w2June08Evening_4e85cb4d:
    mc "Hello?"

translate id w2June08Evening_562e02be:
    kat "I need you to get down here {1}now{2}. Don't dally."

translate id w2June08Evening_15c8cf0c:
    mc "Is something w--"

translate id w2June08Evening_09c53f29:
    mc "...wrong?"

translate id w2June08Evening_b52d8661:
    "That was it. She had hung up on me."

translate id w2June08Evening_3b6724e9:
    mct "(That sounded urgent.)"

translate id w2June08Evening_91486f42:
    "Naturally, from the terse message, I imagined the worst."

translate id w2June08Evening_0108d338:
    mct "(It sounded REALLY urgent.)"

translate id w2June08Evening_c316c6e6:
    mct "(No time to change!)"

translate id w2June08Evening_278c7e29:
    "........."

translate id w2June08Evening_0fd4d656:
    "......"

translate id w2June08Evening_a20cefa7:
    "..."

translate id w2June08Evening_7681322d:
    mct "(Hurrying, hurrying, hurrying...)"

translate id w2June08Evening_318e5f7e:
    "I busted my ass to catch the next train and managed to make it to the club in record time."

translate id w2June08Evening_fb254665:
    mct "(How did she put it...?)"

translate id w2June08Evening_c15b9444:
    mct "(I think it was... \"I need you to get down here NOW. Don't dally.\" Or was there an emphasis on the need?)"

translate id w2June08Evening_7a8d65f3:
    mct "(Fuck, no one's here. I guess she's in her office.)"

translate id w2June08Evening_15779752:
    mct "(She's not here... ah! The security room.)"

translate id w2June08Evening_a8171ef0:
    "*Knock* Knock*"

translate id w2June08Evening_90744145:
    war "Come on in, kid."

translate id w2June08Evening_96d4ca32:
    mc "Hey, I'm looking for--"

translate id w2June08Evening_16b5a2dd:
    mc "Shit, sorry!"

translate id w2June08Evening_74101b84:
    war "What's with that reaction? You should be used to this shit by now."

translate id w2June08Evening_efd9219c:
    mc "Good point..."

translate id w2June08Evening_cb959fee:
    mc "I'm looking for--"

translate id w2June08Evening_8fd92757:
    hgirl "Ghhyyah...!"

translate id w2June08Evening_b21a85b4:
    war "I didn't tell you to stop. Keep shaking that ass!"

translate id w2June08Evening_b8269be3:
    "Wordlessly, the house girl picked up the pace, rocking her hips and grinding her insides against Warren's cock."

translate id w2June08Evening_a04344c7:
    war "You're looking for Mrs. P?"

translate id w2June08Evening_0d065f0d:
    mc "Yeah, she called me and told me to get down here. It sounded urgent."

translate id w2June08Evening_c7cb599d:
    war "She's in the obsidian room."

translate id w2June08Evening_2b35b6fe_1:
    mc "..."

translate id w2June08Evening_ca44bd68:
    "The funny-looking expression on my face moved him to continue."

translate id w2June08Evening_f1e5831f:
    war "Exhibition level. Room #5."

translate id w2June08Evening_b61643ab:
    mct "(Shit, I got to go all the way to the basement.)"

translate id w2June08Evening_a64e8bc0:
    mc "Thanks."

translate id w2June08Evening_1af32e2f:
    war "Hehe, {1}enjoy{2}."

translate id w2June08Evening_56c8652e:
    mct "(Enjoy...?)"

translate id w2June08Evening_a8540ef3:
    war "Ah, get up there like a dog, you fucking bitch. I can't cum with those half-assed movements."

translate id w2KatBirthdayGift_bc3d3421:
    "Please set [2]'s personality (toughness score) for this replay."

translate id w2KatBirthdayGift_0af6e649:
    mc "Room 205..."

translate id w2KatBirthdayGift_0b2c50df:
    mc "There it is!"

translate id w2KatBirthdayGift_a8171ef0:
    "*Knock* Knock*"

translate id w2KatBirthdayGift_6cc83496:
    kat "That you, [2]?"

translate id w2KatBirthdayGift_60cae9c5:
    mc "Yes, Ma'am."

translate id w2KatBirthdayGift_f800bed0:
    mc "Yes."

translate id w2KatBirthdayGift_7405aa2e:
    kat "Come on in."

translate id w2KatBirthdayGift_28a57833:
    "As soon as I entered the room #5, my eyes beheld a shocking sight."

translate id w2KatBirthdayGift_c7fdace8:
    "Harper was strung upside down, legs secured to some trapeze like thing and hands bound behind her back."

translate id w2KatBirthdayGift_06cc7361:
    "She wore a gag and blindfold, as she dangled gently above what appeared to be a ink-black bath tub."

translate id w2KatBirthdayGift_5ba98ea8:
    kat "Hello, [2]. Lucky for Harper, you got here fast."

translate id w2KatBirthdayGift_11ee9f31:
    mc "...you said I needed to get down here?"

translate id w2KatBirthdayGift_dbc443f1:
    "The scene in front of me had vacated all sense of urgency from my body, replacing it with a sinking feeling of uncertainty."

translate id w2KatBirthdayGift_1e320f9b:
    kat "I know I said not to dally, but it wasn't {3}really{4} urgent. I just couldn't help myself. "

translate id w2KatBirthdayGift_2b672450:
    kat "I wanted to give you your present as soon as possible."


