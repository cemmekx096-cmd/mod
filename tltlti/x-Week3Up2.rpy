translate id w3FeliciaMinaOpening_0fd4d656:
    # "......"
    ""

translate id w3FeliciaMinaOpening_a20cefa7:
    # "..."
    ""

translate id w3FeliciaMinaOpening_eebb6431:
    # mina "Hnnnzzzz...! Ah..."
    mina ""

translate id w3FeliciaMinaOpening_77dcfe91:
    # fel "Mmmhh... ehh...?"
    fel ""

translate id w3FeliciaMinaOpening_d24648ec:
    # mina "Huuuuhnnzzz..."
    mina ""

translate id w3FeliciaMinaOpening_412da75b:
    # fel "Ahhhmmm... your..."
    fel ""

translate id w3FeliciaMinaOpening_7285b1c5:
    # fel "Your breath stinks, kid."
    fel ""

translate id w3FeliciaMinaOpening_b327f4e2:
    # mina "Yeah, you too, slut."
    mina ""

translate id w3FeliciaMinaOpening_ae6a2b9f:
    # fel "Who is waking up in a loose woman's bed right now?"
    fel ""

translate id w3FeliciaMinaOpening_3a54e4e9:
    # mina "Heh! {b}Good morning--!{/b}"
    mina ""

translate id w3FeliciaMinaOpening_9bfa0ae5:
    # mina "Unng... ah... my head..."
    mina ""

translate id w3FeliciaMinaOpening_a4320d08:
    # mina "Why are wine hangovers the worst hangovers?"
    mina ""

translate id w3FeliciaMinaOpening_bd4ffb88:
    # fel "You should've listened to me and gone with vodka."
    fel ""

translate id w3FeliciaMinaOpening_7ed9458d:
    # mina "Thanks for watching all those awful episodes with me.... I don't think that \"research\" did me any good, though."
    mina ""

translate id w3FeliciaMinaOpening_05f65ec0:
    # fel "Why not?"
    fel ""

translate id w3FeliciaMinaOpening_dfa62362:
    # mina "It's ALL blending together right now..."
    mina ""

translate id w3FeliciaMinaOpening_f76e822c:
    # fel "You're really going to be on that hokey show, huh?"
    fel ""

translate id w3FeliciaMinaOpening_73d4e8ee:
    # fel "......"
    fel ""

translate id w3FeliciaMinaOpening_4e2a8876:
    # mina "..."
    mina ""

translate id w3FeliciaMinaOpening_4b92712e:
    # fel "I do remember something from last night."
    fel ""

translate id w3FeliciaMinaOpening_a8216773:
    # mina "...what?"
    mina ""

translate id w3FeliciaMinaOpening_41857851:
    # fel "You revealed something preeeetty juicy."
    fel ""

translate id w3FeliciaMinaOpening_5128123a:
    # mina "Whaaaaat...?"
    mina ""

translate id w3FeliciaMinaOpening_12f6b963:
    # fel "You got [mcf] to help with your little list."
    fel ""

translate id w3FeliciaMinaOpening_5fbba185:
    # mina "Well... you know... it was pretty easy to convince him."
    mina ""

translate id w3FeliciaMinaOpening_b576ce48:
    # fel "It always is... for stuff like {i}that{/i} at least."
    fel ""

translate id w3FeliciaMinaOpening_319138f2:
    # fel "Just be careful, okay?"
    fel ""

translate id w3FeliciaMinaOpening_e597a73a:
    # mina "Don't worry. [mcf]'s nice."
    mina ""

translate id w3FeliciaMinaOpening_b47ef2a9:
    # fel "...I know you think you're keeping a casual spin on it, but he's only your second man."
    fel ""

translate id w3FeliciaMinaOpening_01ebda75:
    # mina "My \"second man\"...? Jeez! Why make it sound so biblical?"
    mina ""

translate id w3FeliciaMinaOpening_c9ba2731:
    # mina "Besides, we haven't {i}done it{/i} done it yet."
    mina ""

translate id w3FeliciaMinaOpening_1381351a:
    # fel "So you told me, but still... {b}be careful{/b}, please? Don't get {b}too{/b} serious about him."
    fel ""

translate id w3FeliciaMinaOpening_5f2ac024:
    # fel "He may be more like Ian than you'd like."
    fel ""

translate id w3FeliciaMinaOpening_8cc0c817:
    # mina "Do you know what's funny? I told [mcf] the same thing about you."
    mina ""

translate id w3FeliciaMinaOpening_40d4ec62:
    # fel "Heh. You're a good girl."
    fel ""

translate id w3FeliciaMinaOpening_bc08377d:
    # mina "What about you, though? Are you two...?"
    mina ""

translate id w3FeliciaMinaOpening_d207ae45:
    # fel "Fucking for funsies? Would that make you jealous?"
    fel ""

translate id w3FeliciaMinaOpening_3d5ab20a:
    # mina "Hmm... honestly? Naaaaah."
    mina ""

translate id w3FeliciaMinaOpening_a81926e1:
    # mina "If he's still holding your interest, that bodes well for my bucket list, doesn't it?"
    mina ""

translate id w3FeliciaMinaOpening_a5bdc7ed:
    # fel "You know, Mina... you're kinda scary."
    fel ""

translate id w3FeliciaMinaOpening_db186c71:
    # mina "Seriously though. Are you two still \"fucking for funsies\"...?"
    mina ""

translate id w3FeliciaMinaOpening_1d5fa506:
    # fel "{b}Yeah{/b}. We're actually going to Denise's art exhibit on Wednesday."
    fel ""

translate id w3FeliciaMinaOpening_73b5fee5:
    # mina "Wait, r-really?!"
    mina ""

translate id w3FeliciaMinaOpening_e39cc521:
    # fel "You said you didn't care."
    fel ""

translate id w3FeliciaMinaOpening_108e9e99:
    # mina "I don't; I'm just surprised. Isn't that kind of public?"
    mina ""

translate id w3FeliciaMinaOpening_b17e88a3:
    # fel "Not really. Not the kind of public that would get back to Elias."
    fel ""

translate id w3FeliciaMinaOpening_ad89f16b:
    # fel "...and even if it did, not like we're gonna fuck on the exhibition floor."
    fel ""

translate id w3FeliciaMinaOpening_d69b9b76:
    # mina "Eh, eheh, huh... I hope you have fun. I'd..."
    mina ""

translate id w3FeliciaMinaOpening_133ab86b:
    # mina "I'd like to see you two go at it."
    mina ""

translate id w3FeliciaMinaOpening_8f749ba3:
    # fel "Is voyeurism on your list?"
    fel ""

translate id w3FeliciaMinaOpening_34f37caf:
    # mina "It's not {i}not{/i} on my list, but... *ahem* speaking from experience, [mcf]'s kinda intense, isn't he?"
    mina ""

translate id w3FeliciaMinaOpening_b2c81483:
    # fel "I... suppose? Was that your experience?"
    fel ""

translate id w3FeliciaMinaOpening_081da9ec:
    # mina "Y-yeah... he {i}manhandled me{/i}, but like... in a good way?"
    mina ""

translate id w3FeliciaMinaOpening_f731d3ff:
    # mina "I could barely think. He had me back and forth, bent this way and that way... ah, anyway..."
    mina ""

translate id w3FeliciaMinaOpening_0c5428f8:
    # mina "I just think watching you two have sex... would be interesting."
    mina ""

translate id w3FeliciaMinaOpening_46fb3b17:
    # fel "What? Do you want to watch us go at it Wednesday?"
    fel ""

translate id w3FeliciaMinaOpening_80ff4fb8:
    # mina "Are you seriously asking?"
    mina ""

translate id w3FeliciaMinaOpening_42419f46:
    # fel "I {i}do{/i} like an audience."
    fel ""

translate id w3FeliciaMinaOpening_04d16d64:
    # mina "Then... yes, I would."
    mina ""

translate id w3FeliciaMinaOpening_3a660641:
    # fel "Ha, you're so cute when you're honest!"
    fel ""

translate id w3FeliciaMinaOpening_2bd07e2d:
    # fel "I'll give you a call then when we do it."
    fel ""

translate id w3FeliciaMinaOpening_57f06940:
    # fel "...by the way, good job breaking it off cleanly with Ian."
    fel ""

translate id w3FeliciaMinaOpening_a24690f7:
    # fel "Eh, we had some fun a couple weeks ago, but that's fizzled out by now."
    fel ""

translate id w3FeliciaMinaOpening_0a681aaa:
    # mina "How come?"
    mina ""

translate id w3FeliciaMinaOpening_fd98a69a:
    # fel "He turned me down."
    fel ""

translate id w3FeliciaMinaOpening_ff7e5e16:
    # mina "Really? That's..."
    mina ""

translate id w3FeliciaMinaOpening_70e498cc:
    # fel "Why are you smiling?"
    fel ""

translate id w3FeliciaMinaOpening_ead85b7a:
    # mina "It's just... that's all it takes for it to be over with you, huh?"
    mina ""

translate id w3FeliciaMinaOpening_7ec262bf:
    # fel "Absolutely."
    fel ""

translate id w3FeliciaMinaOpening_57f06940_1:
    # fel "...by the way, good job breaking it off cleanly with Ian."
    fel ""

translate id w3FeliciaMinaOpening_ad43d16a:
    # fel "We fooled around the night we met, but it stopped there."
    fel ""

translate id w3FeliciaMinaOpening_7cb7fc1c:
    # mina "Did it? How come?"
    mina ""

translate id w3FeliciaMinaOpening_fd98a69a_1:
    # fel "He turned me down."
    fel ""

translate id w3FeliciaMinaOpening_ff7e5e16_1:
    # mina "Really? That's..."
    mina ""

translate id w3FeliciaMinaOpening_70e498cc_1:
    # fel "Why are you smiling?"
    fel ""

translate id w3FeliciaMinaOpening_ead85b7a_1:
    # mina "It's just... that's all it takes for it to be over with you, huh?"
    mina ""

translate id w3FeliciaMinaOpening_7ec262bf_1:
    # fel "Absolutely."
    fel ""

translate id w3FeliciaMinaOpening_57f06940_2:
    # fel "...by the way, good job breaking it off cleanly with Ian."
    fel ""

translate id w3FeliciaMinaOpening_744b3de5:
    # fel "We never did anything."
    fel ""

translate id w3FeliciaMinaOpening_d721beba:
    # mina "What, really?! I thought you did."
    mina ""

translate id w3FeliciaMinaOpening_0a681aaa_1:
    # mina "How come?"
    mina ""

translate id w3FeliciaMinaOpening_13be0796:
    # fel "Well, I did make a move the night we met, but he... heh, {b}he turned me down.{/b}"
    fel ""

translate id w3FeliciaMinaOpening_01a7e76e:
    # mina "Ha!"
    mina ""

translate id w3FeliciaMinaOpening_c6b3ef73:
    # fel "What's so funny?"
    fel ""

translate id w3FeliciaMinaOpening_2483451d:
    # mina "He turned you down and not me."
    mina ""

translate id w3FeliciaMinaOpening_3380de77:
    # fel "...you bitch. {i}Savor{/i} that feeling."
    fel ""

translate id w3FeliciaMinaOpening_57f06940_3:
    # fel "...by the way, good job breaking it off cleanly with Ian."
    fel ""

translate id w3FeliciaMinaOpening_2a248615:
    # fel "Your clumsy attempt at seduction got rejected!"
    fel ""

translate id w3FeliciaMinaOpening_42fcf348:
    # fel "I mean... pfhha, c'mon... \"..am I really not enough, sad face\"...?"
    fel ""

translate id w3FeliciaMinaOpening_86a59f1b:
    # mina "Don't be a bitch! He didn't want to two-time his friend!"
    mina ""

translate id w3FeliciaMinaOpening_a9d2990e:
    # fel "Ha! Perhaps!"
    fel ""

translate id w3FeliciaMinaOpening_ac9e9a74:
    # fel "It must've been a shock."
    fel ""

translate id w3FeliciaMinaOpening_fdca3676:
    # mina "I felt like an idiot. {i}He{/i} told me we were still friends!"
    mina ""

translate id w3FeliciaMinaOpening_10c67774:
    # mina "He would've been so lucky."
    mina ""

translate id w3FeliciaMinaOpening_54d9738a:
    # fel "Don't feel stupid. It sounded like he-- aha, I mean...!"
    fel ""

translate id w3FeliciaMinaOpening_77b589b5:
    # fel "He's the dumb one, girl. What kind of cockless idiot turns down a hot young blonde with an extensive sexual to-do list?"
    fel ""

translate id w3FeliciaMinaOpening_390394a7:
    # mina "*Sigh* Ha, thanks. But, he went the extra distance to make me {i}not{/i} feel stupid afterward."
    mina ""

translate id w3FeliciaMinaOpening_b2080c24:
    # mina "Still, it sucks being rejected."
    mina ""

translate id w3FeliciaMinaOpening_5e9f2f92:
    # fel "Growing pains, kid. Speaking of which... good job breaking it off cleanly with Ian."
    fel ""

translate id w3FeliciaMinaOpening_7d6f593d:
    # mina "I was too slow doing it. I should've done it the moment I saw that video."
    mina ""

translate id w3FeliciaMinaOpening_464d5eee:
    # fel "All that matters is that you did it."
    fel ""

translate id w3FeliciaMinaOpening_26098dce:
    # mina "We had this conversation last night."
    mina ""

translate id w3FeliciaMinaOpening_f5966211:
    # fel "We had something like it, but I didn't have the benefit of saying it sober."
    fel ""

translate id w3FeliciaMinaOpening_a348f128:
    # mina "You've been a good friend to me these past few days, Felicia."
    mina ""

translate id w3FeliciaMinaOpening_8a2945e0:
    # fel "Well, you're a good kid and I don't have any friends. Got to make it count."
    fel ""

translate id w3FeliciaMinaOpening_1295d24e:
    # mina "So, you're saying you like having someone young like me who'll listen to your old butt?"
    mina ""

translate id w3FeliciaMinaOpening_4a236c5d:
    # fel "I just see a lot of myself in you. Not because of similar circumstances or anything, just..."
    fel ""

translate id w3FeliciaMinaOpening_0d5c32d2:
    # mina "Heh. I could only hope to be half as cool as you."
    mina ""

translate id w3FeliciaMinaOpening_13257458:
    # fel "Ah, c'mon... I--"
    fel ""

translate id w3FeliciaMinaOpening_e9076b9f:
    # mina "Hopefully, I'll have it more put together than you and not be hanging out with a dumb 19-year-old."
    mina ""

translate id w3FeliciaMinaOpening_31f99658:
    # fel "{b}Cunt!{/b}"
    fel ""

translate id w3FeliciaMinaOpening_995a091a:
    # mina "Do you want to get breakfast?"
    mina ""

translate id w3FeliciaMinaOpening_8d5a40c5:
    # fel "Sure, but it's gotta be a quick one. I've got some stuff to do today."
    fel ""

translate id w3FeliciaMinaOpening_a9045898:
    # mina "What stuff?"
    mina ""

translate id w3FeliciaMinaOpening_340d2eff:
    # fel "A meeting to discuss the week's theme for a sex club I'm involved in."
    fel ""

translate id w3FeliciaMinaOpening_ce7f2ea4:
    # mina "Uh... um... ahuh...?"
    mina ""

translate id w3FeliciaMinaOpening_6d374976:
    # fel "I'm just kidding! I've got some errands!"
    fel ""

translate id w3FeliciaMinaOpening_ebc64646:
    # fel "Let's get breakfast."
    fel ""

translate id w3FeliciaMinaOpening_0fd4d656_1:
    # "......"
    ""

translate id w3FeliciaMinaOpening_a20cefa7_1:
    # "..."
    ""

translate id w3June15HanaOpening_fcb3c8e0:
    # "We slept hard, and we slept in late."
    ""

translate id w3June15HanaOpening_b9015a7c:
    # "Turns out fucking until you almost pass out is a pretty good non-prescription sleep aid."
    ""

translate id w3June15HanaOpening_278c7e29:
    # "........."
    ""

translate id w3June15HanaOpening_0fd4d656:
    # "......"
    ""

translate id w3June15HanaOpening_a20cefa7:
    # "..."
    ""

translate id w3June15HanaOpening_6ff58049:
    # hana "Christ, I fuckin' stunk. How did you not kick my ass out of bed?"
    hana ""

translate id w3June15HanaOpening_ef5ebb54:
    # mc "I didn't even notice. You put my ass to sleep."
    mc ""

translate id w3June15HanaOpening_9c4f6615:
    # hana "Heh~ yeah..."
    hana ""

translate id w3June15HanaOpening_5153cef1:
    # mc "You're cuter than you smelled, I guess."
    mc ""

translate id w3June15HanaOpening_5790f48f:
    # hana "You guess?"
    hana ""

translate id w3June15HanaOpening_a3a87fd3:
    # mc "Okay, you're just too cute."
    mc ""

translate id w3June15HanaOpening_6a240526:
    # hana "Thought so."
    hana ""

translate id w3June15HanaOpening_51def7db:
    # mc "I liked the way you smelled."
    mc ""

translate id w3June15HanaOpening_08d71fe9:
    # hana "Eh...? {i}Really{/i}?"
    hana ""

translate id w3June15HanaOpening_c4a27f54:
    # mc "Yep. I {b}made{/b} you smell that way."
    mc ""

translate id w3June15HanaOpening_ea9456e2:
    # mc "It was hot. Reminded me of how you were screaming my name last night."
    mc ""

translate id w3June15HanaOpening_ceb9bf36:
    # mc "It was hot. Reminded me of how I had you calling me Daddy last night."
    mc ""

translate id w3June15HanaOpening_62f38b1c:
    # hana "Oh, fuck off."
    hana ""

translate id w3June15HanaOpening_9bc50259:
    # mc "{b}I am{/b} a god among men."
    mc ""

translate id w3June15HanaOpening_9d283fab:
    # hana "Well..."
    hana ""

translate id w3June15HanaOpening_73097d63:
    # hana "I don't have an issue helping my boyfriend feel like a god."
    hana ""

translate id w3June15HanaOpening_8c935d49:
    # hana "You were pretty good, but don't get a big head."
    hana ""

translate id w3June15HanaOpening_2aafc710:
    # hana "By the way, you should've taken a shower with me. I would've blown you."
    hana ""

translate id w3June15HanaOpening_3b21f354:
    # mc "......"
    mc ""

translate id w3June15HanaOpening_2b35b6fe:
    # mc "..."
    mc ""

translate id w3June15HanaOpening_881bfc71:
    # mc "I'll remember that in the future."
    mc ""

translate id w3June15HanaOpening_c3e48b79:
    # hana "Hehe~"
    hana ""

translate id w3June15HanaOpening_8be73308:
    # mc "Still, you otherwise would not have had this warm breakfast waiting for you. So, hurry up and get down here and eat."
    mc ""

translate id w3June15HanaOpening_c6550355:
    # hana "Since when... are you so commanding?"
    hana ""

translate id w3June15HanaOpening_3814580f:
    # mc "What's with that look?"
    mc ""

translate id w3June15HanaOpening_3fd627f7:
    # hana "Have I mentioned to you how much I love your shoulders?"
    hana ""

translate id w3June15HanaOpening_11da5040:
    # mc "You're gonna make me blush."
    mc ""

translate id w3June15HanaOpening_9aade2b2:
    # hana "*Chwup* Mhh..."
    hana ""

translate id w3June15HanaOpening_238da2e1:
    # hana "Haaa... you don't mind a little morning sugar, do you?"
    hana ""

translate id w3June15HanaOpening_9b3362c6:
    # mc "It's a good pick me up..."
    mc ""

translate id w3June15HanaOpening_d0d5438d:
    # hana "Heh, hehh...!"
    hana ""

translate id w3June15HanaOpening_6e70eb36:
    # hana "*Fhwup* Haaah..."
    hana ""

translate id w3June15HanaOpening_3435340c:
    # hana "I shouldn't let your hard work go to waste by letting the food get cold."
    hana ""

translate id w3June15HanaOpening_cc9bc439:
    # "My work wasn't the only hard thing, but I was pleased to let the morning go to a place..."
    ""

translate id w3June15HanaOpening_e4894871:
    # "...more lazy, listless, and comfortable. There was the nagging feeling that I should get up and prepare for my day, but I felt tethered to Hana."
    ""

translate id w3June15HanaOpening_5ffb2986:
    # hana "Hey, look."
    hana ""

translate id w3June15HanaOpening_bc32baa7:
    # "Unwanting and adverse to leaving the cozy bubble we were in."
    ""

translate id w3June15HanaOpening_97593b6b:
    # mc "Oh...?"
    mc ""

translate id w3June15HanaOpening_bcd2ce06:
    # mc "That's you and Jerrica."
    mc ""

translate id w3June15HanaOpening_aee19e72:
    # hana "Yep."
    hana ""

translate id w3June15HanaOpening_c7cc5f0f:
    # mc "Cynthia was right. You haven't really changed much."
    mc ""

translate id w3June15HanaOpening_070c8f38:
    # mc "Not outwardly, at least. I like the pink hair."
    mc ""

translate id w3June15HanaOpening_4577c530:
    # hana "Right? Cute as fuck."
    hana ""

translate id w3June15HanaOpening_d6cf1971:
    # hana "Should I give it a spin again?"
    hana ""

translate id w3June15HanaOpening_54a71459:
    # mc "You're asking if you should dye your hair...?"
    mc ""

translate id w3June15HanaOpening_8fea2db2:
    # mc "Hmm..."
    mc ""

translate id w3June15HanaOpening_abcf3d40:
    # mc "{b}Nah{/b}. Your hair's beautiful - so black that you can get lost in it."
    mc ""

translate id w3June15HanaOpening_563be83c:
    # mc "I like you the way you are."
    mc ""

translate id w3June15HanaOpening_963503b3:
    # hana "That's that."
    hana ""

translate id w3June15HanaOpening_320d44a3:
    # mc "What about me?"
    mc ""

translate id w3June15HanaOpening_442187ac:
    # hana "I don't think pink's your color."
    hana ""

translate id w3June15HanaOpening_d9af6597:
    # mc "No?"
    mc ""

translate id w3June15HanaOpening_79f1d620:
    # hana "{b}No...{/b}"
    hana ""

translate id w3June15HanaOpening_e7402604:
    # hana "W~oooow, you're blind, huh?"
    hana ""

translate id w3June15HanaOpening_5c26723b:
    # "In short order, it was easy to see why I was so quick to promise Hana something I shouldn't have done last night."
    ""

translate id w3June15HanaOpening_4a47f79c:
    # mc "I can make you out just fine."
    mc ""

translate id w3June15HanaOpening_3b8dd330:
    # "Moments like this quieted the mind. Reduced and simplified."
    ""

translate id w3June15HanaOpening_34cdb7a6:
    # mc "C'mere..."
    mc ""

translate id w3June15HanaOpening_d64761f4:
    # hana "Mmmh..."
    hana ""

translate id w3June15HanaOpening_1cd2240d:
    # "But {b}I did{/b} promise her something difficult."
    ""

translate id w3June15HanaOpening_28cf0128:
    # "There was Mina, who in her loneliness and anger, spilled her guts to me. I promised to help her with her list."
    ""

translate id w3June15HanaOpening_2ea0ec4e:
    # "There was Rosalind and our deal, with all the extracurricular activities that entailed..."
    ""

translate id w3June15HanaOpening_9729ab11:
    # "{b}Especially{/b} the extracurricular activities..."
    ""

translate id w3June15HanaOpening_e02cc8c3:
    # "There was Felicia, exhilarating to be around."
    ""

translate id w3June15HanaOpening_92d2406f:
    # "I had her art exhibition this Wednesday, plus I promised I'd try out to be her sugar baby..."
    ""

translate id w3June15HanaOpening_b1b0b033:
    # "I promised I'd go to that art exhibition with her this Wednesday..."
    ""

translate id w3June15HanaOpening_a17d973b:
    # "There was no point in explaining the convoluted circumstances to Hana."
    ""

translate id w3June15HanaOpening_08b92cd6:
    # "The choice was simple: keep my promise to Hana or don't."
    ""

translate id w3June15HanaOpening_0fd4d656_1:
    # "......"
    ""

translate id w3June15HanaOpening_a20cefa7_1:
    # "..."
    ""

translate id w3June15HanaOpening_13a1b86c:
    # hana "...I think you should go brush your teeth?"
    hana ""

translate id w3June15HanaOpening_2918b8b5:
    # mc "Ha! Same to you!"
    mc ""

translate id w3June15HanaOpening_fc1ee1c8:
    # hana "I don't have a toothbrush here."
    hana ""

translate id w3June15HanaOpening_5a627507:
    # "I should savor moments like this, I thought."
    ""

translate id w3June15HanaOpening_34cdb7a6_1:
    # mc "C'mere..."
    mc ""

translate id w3June15HanaOpening_d64761f4_1:
    # hana "Mmmh..."
    hana ""

translate id w3June15HanaOpening_3e32335e:
    # "Really make the most of it and count my stars at being so lucky."
    ""

translate id w3June15HanaOpening_353ca6b1:
    # "My life is a lot of fun right now. There was cock-starved Hana and..."
    ""

translate id w3June15HanaOpening_509b27b5:
    # "Curious and perverted Mina."
    ""

translate id w3June15HanaOpening_3ef3df71:
    # "Rosalind and our deal."
    ""

translate id w3June15HanaOpening_d4551d43:
    # "Of which I've well taken advantage of..."
    ""

translate id w3June15HanaOpening_e02cc8c3_1:
    # "There was Felicia, exhilarating to be around."
    ""

translate id w3June15HanaOpening_92d2406f_1:
    # "I had her art exhibition this Wednesday, plus I promised I'd try out to be her sugar baby..."
    ""

translate id w3June15HanaOpening_b1b0b033_1:
    # "I promised I'd go to that art exhibition with her this Wednesday..."
    ""

translate id w3June15HanaOpening_0fd4d656_2:
    # "......"
    ""

translate id w3June15HanaOpening_a20cefa7_2:
    # "..."
    ""

translate id w3June15HanaOpening_65f50e65:
    # mct "(Fuck yeah! My life is dope!)"
    mct ""

translate id w3June15HanaOpening_0fd4d656_3:
    # "......"
    ""

translate id w3June15HanaOpening_a20cefa7_3:
    # "..."
    ""

translate id w3June15HanaOpening_13a1b86c_1:
    # hana "...I think you should go brush your teeth?"
    hana ""

translate id w3June15HanaOpening_2918b8b5_1:
    # mc "Ha! Same to you!"
    mc ""

translate id w3June15HanaOpening_7ccdb826:
    # hana "I don't have a tooth-brush here."
    hana ""

translate id w3June15HanaOpening_20c73d78:
    # mc "Give me back my glasses!"
    mc ""

translate id w3June15HanaOpening_0fd4d656_4:
    # "......"
    ""

translate id w3June15HanaOpening_a20cefa7_4:
    # "..."
    ""

translate id w3June15HanaOpening_4b58177e:
    # "Inevitably though, that tether had to be cut. Hana had to check in on her mom and answer her many questions, and me... well..."
    ""

translate id w3June15SoloOpening_a316d022:
    # mc "Mmmhh..."
    mc ""

translate id w3June15SoloOpening_4345c36b:
    # "I wanted to go back to sleep, but..."
    ""

translate id w3June15SoloOpening_8c3855c1:
    # mc "I didn't get any play last night with Hana, did I? But so what...?"
    mc ""

translate id w3June15SoloOpening_49ea801c:
    # "Why is this thing so out of control?"
    ""

translate id w3June15SoloOpening_0fd4d656:
    # "......"
    ""

translate id w3June15SoloOpening_a20cefa7:
    # "..."
    ""

translate id w3June15SoloOpening_09a80430:
    # "Today was Monday, which meant I had to go to the club in a few hours... I should get ready... {b}also{/b}..."
    ""

translate id w3June15SoloOpening_41a319e4:
    # "...and it would probably be best if I wasn't {i}pent up.{/b}"
    ""

translate id w3June15SoloOpening_4bcfe393:
    # "Never know what that old woman has in store..."
    ""

translate id w3June15SoloOpening_3be44d60:
    # mct "(Sorry, Ian.)"
    mct ""

translate id w3June15SoloOpening_1d82a7e7:
    # mc "...!"
    mc ""

translate id w3June15SoloOpening_a4bf45df:
    # "A tried and true favorite."
    ""

translate id w3June15SoloOpening_20dc71e7:
    # "She hated me for no good reason... I always wanted to see what that stuck-up bitch's face would look like getting fucked in the ass."
    ""

translate id w3June15SoloOpening_7a526028:
    # mct "(Gah, s-stupid cunt!)"
    mct ""

translate id w3June15SoloOpening_8d8919ef:
    # "In my fantasies, my desires had free rein, without consequence."
    ""

translate id w3June15SoloOpening_061fb09e:
    # mc "Hnngg...!"
    mc ""

translate id w3June15SoloOpening_3b21f354:
    # mc "......"
    mc ""

translate id w3June15SoloOpening_a7b62312:
    # mct "(...I should go clean up.)"
    mct ""

translate id w3June15SoloOpening_6252c6fa:
    # "I {i}was{/i} free to do {b}some{/b} of the things I desired..."
    ""

translate id w3June15SoloOpening_278c7e29:
    # "........."
    ""

translate id w3June15SoloOpening_0fd4d656_1:
    # "......"
    ""

translate id w3June15SoloOpening_2b35b6fe:
    # mc "..."
    mc ""

translate id w3June15SoloOpening_955ea274:
    # mct "(I wonder what Kathleen has in store for us today.)"
    mct ""

translate id w3June15SoloOpening_475f1d0d:
    # "Us...?"
    ""

translate id w3June15SoloOpening_7beabec2:
    # "It seemed I unconsciously included myself among the Carnations."
    ""

translate id w3June15SoloOpening_e9e3e8d9:
    # "Was that good or bad?"
    ""

translate id w3June15SoloOpening_0fd4d656_2:
    # "......"
    ""

translate id w3June15SoloOpening_a20cefa7_1:
    # "..."
    ""

translate id w3ThemeIntro_1aaa7de1:
    # "The walk to the club was dull and uneventful, my mind preoccupied with countless trivialities - just as it might for anyone else's daily commute."
    ""

translate id w3ThemeIntro_705148aa:
    # "Once inside, I saw a pair of familiar faces and was greeted by the friendly smile of my co-worker."
    ""

translate id w3ThemeIntro_ab33412e:
    # "The mundanity of it all was striking."
    ""

translate id w3ThemeIntro_5de152d4:
    # "Dr. Van Doren's assistant stood at Jacob's post. My arrival had either interrupted their exchange or had tidily coincided with its conclusion."
    ""

translate id w3ThemeIntro_491c6f12:
    # jacob "[mcf]!"
    jacob ""

translate id w3ThemeIntro_1901e159:
    # mc "So... I've got a question."
    mc ""

translate id w3ThemeIntro_2924de4c:
    # jacob "Do I look like a help desk to you?"
    jacob ""

translate id w3ThemeIntro_88ecfd6e:
    # mc "I've always wondered... you're posted way up here, so what happens if someone finds a way to wander in?"
    mc ""

translate id w3ThemeIntro_940c4032:
    # jacob "The cameras will send me an alert."
    jacob ""

translate id w3ThemeIntro_e7535362:
    # mc "Oh, yeah... that makes sense."
    mc ""

translate id w3ThemeIntro_6f1b1b7a:
    # jacob "Doesn't ever really happen anyway. It's just floor after floor of unoccupied office space, after all."
    jacob ""

translate id w3ThemeIntro_2e1a1d7a:
    # mc "Never?"
    mc ""

translate id w3ThemeIntro_88490ca5:
    # jacob "Well, there was one time a homeless guy pried open a service door we forgot to latch in order to get out of the cold."
    jacob ""

translate id w3ThemeIntro_04ba50c3:
    # mc "What did you do?"
    mc ""

translate id w3ThemeIntro_a2b0ca65:
    # jacob "I sent him on his way with a cup of coffee. The man was just cold and this is just an empty office building, right?"
    jacob ""

translate id w3ThemeIntro_4376bef6:
    # mc "Right on."
    mc ""

translate id w3ThemeIntro_59c0608e:
    # jacob "Actually, I'm glad to see you. I wanted to say thanks for getting Emma the night off."
    jacob ""

translate id w3ThemeIntro_4a94962c:
    # mc "Well, it wasn't quite a night off, but I did manage to get her away from the patrons thanks to Ian."
    mc ""

translate id w3ThemeIntro_1ab687fa:
    # mc "Hopefully, he didn't give her too hard of a time."
    mc ""

translate id w3ThemeIntro_7928fe9d:
    # jacob "She's dealt with worse."
    jacob ""

translate id w3ThemeIntro_ed156559:
    # mc "It wasn't much; I just asked Dr. Chuck for a favor."
    mc ""

translate id w3ThemeIntro_8f85d479:
    # jacob "It might not feel like much, but she and I appreciated it."
    jacob ""

translate id w3ThemeIntro_19409061:
    # mc "It wasn't quite the night off, but I managed to take her away from the patrons."
    mc ""

translate id w3ThemeIntro_38355dd6:
    # jacob "Yeah, she explained your little ruse."
    jacob ""

translate id w3ThemeIntro_222406bb:
    # mc "Plausible deniability."
    mc ""

translate id w3ThemeIntro_174787de:
    # jacob "Heh, for sure."
    jacob ""

translate id w3ThemeIntro_5ef7cffb:
    # jacob "Anyway, I owe you one. If you ever need a favor, I'm your man, alright?"
    jacob ""

translate id w3ThemeIntro_ad5ff3ee:
    # mc "Sure, I'll remember that."
    mc ""

translate id w3ThemeIntro_6bc46605:
    # mct "(I can't imagine what kind of favor that would be, though...)"
    mct ""

translate id w3ThemeIntro_b801f9e9:
    # mc "So, Mrs. Pulman in her office?"
    mc ""

translate id w3ThemeIntro_75ebc592:
    # jacob "Her words were to tell you to report to the photo studio. You know where to find it?"
    jacob ""

translate id w3ThemeIntro_c1abbeaa:
    # mc "Yep. Well-acquainted with it."
    mc ""

translate id w3ThemeIntro_9b9c9572:
    # mc "I should get down there."
    mc ""

translate id w3ThemeIntro_6710b010:
    # mc "Oh, by the way, is Dr. Van Doren here?"
    mc ""

translate id w3ThemeIntro_504ff943:
    # jacob "He isn't."
    jacob ""

translate id w3ThemeIntro_71ca1b96:
    # mc "Oh... I've never seen Sophia without him."
    mc ""

translate id w3ThemeIntro_92351ad8:
    # jacob "For the last couple of weeks, Dr. Lundgren has come alone often. Almost daily, actually."
    jacob ""

translate id w3ThemeIntro_eef34210:
    # mc "Huh? What's she doing?"
    mc ""

translate id w3ThemeIntro_86d111a1:
    # jacob "No idea. I guess she and Mrs. Pulman are friends?"
    jacob ""

translate id w3ThemeIntro_507eeea6:
    # mct "(I doubt they were social visits...)"
    mct ""

translate id w3ThemeIntro_fcf43158:
    # mc "Yeah, that must be it."
    mc ""

translate id w3ThemeIntro_9245a7c2:
    # jacob "Fine-ass woman, isn't she?"
    jacob ""

translate id w3ThemeIntro_c15583c1:
    # mc "She's definitely beautiful."
    mc ""

translate id w3ThemeIntro_e17497b4:
    # jacob "Nah, man. It's more than that."
    jacob ""

translate id w3ThemeIntro_e3212758:
    # jacob "Every time she passes by, I feel like I'm a fuckin' teenager again with all the urges."
    jacob ""

translate id w3ThemeIntro_df95f1aa:
    # mc "Ah, yeah... {b}it's the perfume{/b}. Try not to inhale it as much when she comes around."
    mc ""

translate id w3ThemeIntro_3ee9786e:
    # jacob "Perfume? Yeah, {b}right{/b}. I don't think it's that."
    jacob ""

translate id w3ThemeIntro_3492666e:
    # mc "Juuuust give standing a couple more feet apart a try, alright? {b}Trust me.{/b}"
    mc ""

translate id w3ThemeIntro_cf428e34:
    # mc "See you around, Jacob."
    mc ""

translate id w3ThemeIntro_4cde231a:
    # jacob "Later man."
    jacob ""

translate id w3ThemeIntro_649bd294:
    # "In the studio, I was greeted by another increasingly mundane sight. The Carnations were all here, wearing very little, but..."
    ""

translate id w3ThemeIntro_71c55f87:
    # "{b}Scratch that.{/b}"
    ""

translate id w3ThemeIntro_3f587654:
    # "The three dolled up in red leather was still a glorious sight."
    ""

translate id w3ThemeIntro_7aaf1af8:
    # "Each woman wore a corset, intricately distinguished from your bog standard BDSM attire by a few strategically placed straps of leather that both obscured and accentuated their naughty bits."
    ""

translate id w3ThemeIntro_6d7aa67d:
    # kat "[mcf], good, you're here."
    kat ""

translate id w3ThemeIntro_329954fd:
    # mc "I am... and here I thought I was pretty early, but the girls are already dressed."
    mc ""

translate id w3ThemeIntro_0b0f1abf:
    # kat "Don't they look lovely?"
    kat ""

translate id w3ThemeIntro_7dba7348:
    # "I left the question unanswered, as given the dull looks on their faces, a yes felt like it would be ostentatious."
    ""

translate id w3ThemeIntro_7061cf2e:
    # kat "{b}You are early{/b}, dear. Things still need to be fully prepared."
    kat ""

translate id w3ThemeIntro_01ee70bc:
    # kat "Warren's prepping the Obsidian Room, and we'll need to wait for Ian to start the shoot. Have you spoken to him today?"
    kat ""

translate id w3ThemeIntro_382dc96a:
    # mc "I have not. Why?"
    mc ""

translate id w3ThemeIntro_921e9fea:
    # kat "Considering his historic {i}tardiness{/i}, I tried calling him."
    kat ""

translate id w3ThemeIntro_86d49d4f:
    # mc "...no luck?"
    mc ""

translate id w3ThemeIntro_1ae9a23e:
    # kat "You may be more fortunate."
    kat ""

translate id w3ThemeIntro_8a307e81:
    # "I finally noticed the cattle prod in Mrs. Pulman's hands... a portent of what's to come, no doubt."
    ""

translate id w3ThemeIntro_9c188a56:
    # "I finally noticed the cattle prod in Kathleen's hands... a portent of what's to come, no doubt."
    ""

translate id w3ThemeIntro_68f8e6aa:
    # kat "We're not supposed to start for at least another thirty minutes, but it'd be nice to know if I can count on him today."
    kat ""

translate id w3ThemeIntro_7deaf661:
    # mc "I'll give him a call."
    mc ""

translate id w3ThemeIntro_ad6a3b53:
    # kat "Good!"
    kat ""

translate id w3ThemeIntro_41fd3db0:
    # "*Crack*"
    ""

translate id w3ThemeIntro_7e15900a:
    # mc "Ah-- yyhhup-!!!"
    mc ""

translate id w3ThemeIntro_a89aeb31:
    # "A jolt of pain seared my core as a playful smile spread on Mrs. Pulman's face."
    ""

translate id w3ThemeIntro_b02f0e24:
    # "A jolt of pain seared my core as a playful smile spread on Kathleen's face."
    ""

translate id w3ThemeIntro_48f31bb6:
    # mc "Godda-- ah, what's the big-"
    mc ""

translate id w3ThemeIntro_90cff426:
    # kat "How's the intensity? Do you think it needs more juice?"
    kat ""

translate id w3ThemeIntro_640c94ce:
    # mc "Gah..."
    mc ""

translate id w3ThemeIntro_9d7affa4:
    # "In the wake of the shock, the area she prodded was left with a slowly dissipating burning sensation."
    ""

translate id w3ThemeIntro_5c0e78bf:
    # mc "Ah... ha... maybe I'm a pussy, but that fucking sucked."
    mc ""

translate id w3ThemeIntro_c3ef82c6:
    # kat "I do appreciate your input, dear - {i}and the cute way you yelp.{/i}"
    kat ""

translate id w3ThemeIntro_6eed0ee8:
    # mc "...I'll make the call now."
    mc ""

translate id w3ThemeIntro_4bdc5fd5:
    # kat "Please do-"
    kat ""

translate id w3ThemeIntro_03a3c987:
    # kat "Ha! Don't be so jumpy!"
    kat ""

translate id w3ThemeIntro_e1aa63e8:
    # "*Ring, ring...*"
    ""

translate id w3ThemeIntro_60649794:
    # "It was rare for Ian not to pick up on the first or second ring..."
    ""

translate id w3ThemeIntro_6a9081db:
    # "*Ring, ring, ring...*"
    ""

translate id w3ThemeIntro_00ee975a:
    # "Maybe he's..."
    ""

translate id w3ThemeIntro_c7933489:
    # kil "Ugh... y-yo...! D-doc!"
    kil ""

translate id w3ThemeIntro_5f0dd898:
    # mc "You sound like shit."
    mc ""

translate id w3ThemeIntro_33ae1b0a:
    # kil "I... guhh... I drank way too much last night."
    kil ""

translate id w3ThemeIntro_4eaa4267:
    # "Considering his tolerance... {b}that couldn't be an understatement.{/b}"
    ""

translate id w3ThemeIntro_23e6ce99:
    # mc "Taking the breakup that hard, eh?"
    mc ""

translate id w3ThemeIntro_73214f22:
    # kil "{b}No...!{/b} I just got a bit carried away fucking that old hag, that's all."
    kil ""

translate id w3ThemeIntro_fc804250:
    # alice "Ah, don't--"
    alice ""

translate id w3ThemeIntro_06773641:
    # kil "Relax. He already knows."
    kil ""

translate id w3ThemeIntro_74f05a4e:
    # alice "..."
    alice ""

translate id w3ThemeIntro_507b7d63:
    # kil "Say he--..."
    kil ""

translate id w3ThemeIntro_3e7baaa3:
    # kil "Eughh...!"
    kil ""

translate id w3ThemeIntro_a7de5ce8:
    # "I could hear Ian momentarily fight back a wretch from his end of the phone."
    ""

translate id w3ThemeIntro_9b1c0dcc:
    # kil "Say hello."
    kil ""

translate id w3ThemeIntro_a8d696d1:
    # alice "*Sigh*..."
    alice ""

translate id w3ThemeIntro_eb0a99f9:
    # alice "Hi, [mcf]."
    alice ""

translate id w3ThemeIntro_93deaf69:
    # mc "Ah..."
    mc ""

translate id w3ThemeIntro_61ad8d8d:
    # "I wasn't expecting... this..."
    ""

translate id w3ThemeIntro_8ce5b283:
    # alice "...this is awkward, huh?"
    alice ""

translate id w3ThemeIntro_5dc02253:
    # mc "Not really..."
    mc ""

translate id w3ThemeIntro_eb21f1f6:
    # "That was a fucking lie."
    ""

translate id w3ThemeIntro_bf27a0a0:
    # mc "{b}You're both adults{/b}... *Ahem*... how are you doing, Alice?"
    mc ""

translate id w3ThemeIntro_933ab5f6:
    # alice "Well, I'm about to..."
    alice ""

translate id w3ThemeIntro_8da9c9c1:
    # kil "Alice... help me..."
    kil ""

translate id w3ThemeIntro_a938ba83:
    # kil "...bacon-egg-cheese sandwich."
    kil ""

translate id w3ThemeIntro_bb584cae:
    # mc "He's got you nursing his hangover?"
    mc ""

translate id w3ThemeIntro_472e9c68:
    # alice "What else?"
    alice ""

translate id w3ThemeIntro_6d46f4c7:
    # mc "He doesn't deserve it."
    mc ""

translate id w3ThemeIntro_bbf46346:
    # alice "I'm gonna give the phone back now."
    alice ""

translate id w3ThemeIntro_8d03ca85:
    # mc "Sure. Good talking, Alice..."
    mc ""

translate id w3ThemeIntro_05969f8f:
    # kil "Ughh... hey, dude..."
    kil ""

translate id w3ThemeIntro_43779bd7:
    # mc "You're fucking shameless, you know that? Taking advantage of your nanny like that."
    mc ""

translate id w3ThemeIntro_797f6528:
    # kil "Hey...! She enjoys it! ...for some fuckin' reason."
    kil ""

translate id w3ThemeIntro_edfc8825:
    # mc "You're not coming into work, are you?"
    mc ""

translate id w3ThemeIntro_33f038fe:
    # kil "Aaaaaaaaaaaah.... {i}shiiiiiit{/i}."
    kil ""

translate id w3ThemeIntro_3d1f4efa:
    # kil "Kathy's dumb photo shoots... right..."
    kil ""

translate id w3ThemeIntro_a4b417b8:
    # kil "...tell her I sounded like I'm dying? Through absolutely no fault of my own? Heh...?"
    kil ""

translate id w3ThemeIntro_2b35b6fe:
    # mc "..."
    mc ""

translate id w3ThemeIntro_5451d9ff:
    # kil "You're not mad? You can point a camera and snap a few pictures."
    kil ""

translate id w3ThemeIntro_61da4b9d:
    # mc "It's not that. I'm upset I won't see my friend's smiling face today."
    mc ""

translate id w3ThemeIntro_4997e2fe:
    # kil "Yeah right, jackass!"
    kil ""

translate id w3ThemeIntro_e926a157:
    # mc "...I'll cover for you, I guess."
    mc ""

translate id w3ThemeIntro_0f41d2ee:
    # kil "Yeah?"
    kil ""

translate id w3ThemeIntro_a269a9b2:
    # kil "Ughh... h-hey man..."
    kil ""

translate id w3ThemeIntro_b60e0997:
    # mc "You sound fucked up."
    mc ""

translate id w3ThemeIntro_fee37608:
    # kil "I am... drank a little too much last night."
    kil ""

translate id w3ThemeIntro_ab53346a:
    # mc "A little?"
    mc ""

translate id w3ThemeIntro_e3403368:
    # kil "A little bit of a lot... h-heh... ugh..."
    kil ""

translate id w3ThemeIntro_42ecfdbb:
    # mc "Taking the break up hard, eh?"
    mc ""

translate id w3ThemeIntro_04979d8a:
    # kil "N-no... I just got carried away with... ah..."
    kil ""

translate id w3ThemeIntro_383b0d3a:
    # kil "Get off my arm, will you? It's fall--"
    kil ""

translate id w3ThemeIntro_b399f8d7:
    # mc "I got the picture."
    mc ""

translate id w3ThemeIntro_0f41d2ee_1:
    # kil "Yeah?"
    kil ""

translate id w3ThemeIntro_b20741cc:
    # mc "{b}Yeah{/b}. You're not coming into work today."
    mc ""

translate id w3ThemeIntro_0fd4d656:
    # "......"
    ""

translate id w3ThemeIntro_a20cefa7:
    # "..."
    ""

translate id w3ThemeIntro_33f038fe_1:
    # kil "Aaaaaaaaaaaah.... {i}shiiiiiit{/i}."
    kil ""

translate id w3ThemeIntro_3d1f4efa_1:
    # kil "Kathy's dumb photo shoots... right..."
    kil ""

translate id w3ThemeIntro_a4b417b8_1:
    # kil "...tell her I sounded like I'm dying? Through absolutely no fault of my own? Heh...?"
    kil ""

translate id w3ThemeIntro_2b35b6fe_1:
    # mc "..."
    mc ""

translate id w3ThemeIntro_668bbf70:
    # kil "You're not mad, are you?"
    kil ""

translate id w3ThemeIntro_61da4b9d_1:
    # mc "It's not that. I'm upset I won't see my friend's smiling face today."
    mc ""

translate id w3ThemeIntro_4997e2fe_1:
    # kil "Yeah right, jackass!"
    kil ""

translate id w3ThemeIntro_070ceea3:
    # mc "...yeah, alright. I'll cover for you, I guess."
    mc ""

translate id w3ThemeIntro_34dd2749:
    # kil "You will?"
    kil ""

translate id w3ThemeIntro_0a7a7d42:
    # mc "{b}Yeeeeeeep.{/b}"
    mc ""

translate id w3ThemeIntro_b28c7b90:
    # "Sorry dude, the jig is already up."
    ""

translate id w3ThemeIntro_d7acceea:
    # mc "I'll tell her you've got the crud going around right now."
    mc ""

translate id w3ThemeIntro_3b43c724:
    # kil "Thanks, man... ugh..."
    kil ""

translate id w3ThemeIntro_fd6e2765:
    # mc "Talk to you later."
    mc ""

translate id w3ThemeIntro_672cc5a1:
    # kil "Ugh... think I'm actually going to pu-- bye!"
    kil ""

translate id w3ThemeIntro_b8a7705e:
    # "*Beep*"
    ""

translate id w3ThemeIntro_0fd4d656_1:
    # "......"
    ""

translate id w3ThemeIntro_a20cefa7_1:
    # "..."
    ""

translate id w3ThemeIntro_a46e5f72:
    # mc "...uhhh, he sounded pretty sick."
    mc ""

translate id w3ThemeIntro_81026771:
    # kat "Was that a question mark at the end?"
    kat ""

translate id w3ThemeIntro_dd22b719:
    # mc "No...?"
    mc ""

translate id w3ThemeIntro_01842058:
    # kat "Well, no matter. You can fill in for his side of things."
    kat ""

translate id w3ThemeIntro_8adc3b97:
    # kat "You had ample experience last week."
    kat ""

translate id w3ThemeIntro_8898e0e5:
    # mc "I'm a professional by this point."
    mc ""

translate id w3ThemeIntro_fb8514c9:
    # kat "*Sigh* I SHOULD just stop relying on him altogether. It would simplify things."
    kat ""

translate id w3ThemeIntro_23d2b21e:
    # mc "Don't you think he adds a bit of charm to the proceedings?"
    mc ""

translate id w3ThemeIntro_88a8350a:
    # kat "The only thing that scud adds is adequately pointing a camera."
    kat ""

translate id w3ThemeIntro_7ee43ed1:
    # mc "He's got other redeeming qualities..."
    mc ""

translate id w3ThemeIntro_816f4065:
    # "You've got to say it more enthusiastically, [mcf]..."
    ""

translate id w3ThemeIntro_fe18bb0a:
    # mc "These things happen."
    mc ""

translate id w3ThemeIntro_aa81806d:
    # kat "Too often."
    kat ""

translate id w3ThemeIntro_bd1b7387:
    # "Well, whatever. Not like any of this is actual work..."
    ""

translate id w3ThemeIntro_63b93385:
    # kat "We are getting started early, girls. Please try and contain your excitement."
    kat ""

translate id w3ThemeIntro_02f6f069:
    # ver "Greeeeeat."
    ver ""

translate id w3ThemeIntro_a4f3ced6:
    # rose "Get started on what?"
    rose ""

translate id w3ThemeIntro_523980ed:
    # kat "This week's promotional material."
    kat ""

translate id w3ThemeIntro_d8531f20:
    # fel "I hope it will be more fun than the previous one. Unfortunately, it was a little too much talking for my tastes."
    fel ""

translate id w3ThemeIntro_069fa125:
    # kat "Oh, have no fear, Mrs. Ford. This is {b}hell week{/b}."
    kat ""

translate id w3ThemeIntro_73d4e8ee:
    # fel "......"
    fel ""

translate id w3ThemeIntro_8838af23:
    # ver "......"
    ver ""

translate id w3ThemeIntro_abd837cd:
    # rose "..."
    rose ""

translate id w3ThemeIntro_6632479f:
    # ver "...okay?"
    ver ""

translate id w3ThemeIntro_c20788f5:
    # kat "I know that doesn't mean much right now."
    kat ""

translate id w3ThemeIntro_c71a05ca:
    # kat "It doesn't need to. Your experiences during this exhibition will impart my meaning in a way words won't suffice."
    kat ""

translate id w3ThemeIntro_cda634fa:
    # rose "Are you trying to scare us?"
    rose ""

translate id w3ThemeIntro_470f1cea:
    # kat "Grab the camera, [mcf]."
    kat ""

translate id w3ThemeIntro_39de506d:
    # mc "On it."
    mc ""

translate id w3ThemeIntro_d8f2fb17:
    # kat "We will play a brief game today and then move on to you three's punishment for failing to win week 2."
    kat ""

translate id w3ThemeIntro_0f89c453:
    # kat "We will play a brief game today, then move on to Miss Lynch and Mrs. Ford's punishment for losing week 2."
    kat ""

translate id w3ThemeIntro_15eacd0f:
    # kat "We will play a brief game today, then move on to Mrs. Ford's punishment for losing week 2."
    kat ""

translate id w3ThemeIntro_297ea0e7:
    # kat "We will play a brief game today, then move on to Miss Lynch's punishment for losing week 2."
    kat ""

translate id w3ThemeIntro_80971190:
    # kat "Both of which will be documented for our viewers' betting pleasure. So put on your best sad, miserable faces, girls."
    kat ""

translate id w3ThemeIntro_a20cefa7_2:
    # "..."
    ""

translate id w3ThemeIntro_e993541b:
    # "They looked primarily unperturbed by Mrs. Pulman's words - and in some ways, it WAS the same old same old, but..."
    ""

translate id w3ThemeIntro_a1c5f59b:
    # "They looked primarily unperturbed by Kathleen's words - and in some ways, it WAS the same old same old, but... "
    ""

translate id w3ThemeIntro_93712e6c:
    # kat "Get some good shots of their costumes before we begin."
    kat ""

translate id w3ThemeIntro_b5c71fb8:
    # "Something about the cruel woman's newfound brevity waved a red flag."
    ""

translate id w3ThemeIntro_9541c0c3:
    # mc "Like last week? You want poses and...?"
    mc ""

translate id w3ThemeIntro_dc6fbe7d:
    # "Unusually amongst the three, only Felicia's face showed a degree of distinguishable concern."
    ""

translate id w3ThemeIntro_bb73c4af:
    # kat "Whatever you feel is appropriate. {b}Just keep it brief.{/b}"
    kat ""

translate id w3ThemeIntro_bb4b5fe6:
    # mc "Alright, girls. Let's, uh..."
    mc ""

translate id w3ThemeIntro_15779702:
    # "With that missive in mind, I arranged a series of group shots. Shots that would hopefully be evocative of the Carnation's personality and--"
    ""

translate id w3ThemeIntro_a6a0fca6:
    # "{i}I decided to take a series of group shots that were hot as hell.{/i} But, I mean, what else was there to do beyond following my boner here?"
    ""

translate id w3ThemeIntro_4dd87191:
    # "I paired each of them off in every combination."
    ""

translate id w3ThemeIntro_1e1b9bb3:
    # mct "({i}Put your tits in her face!{/i} ...yeah.)"
    mct ""

translate id w3ThemeIntro_5f74984f:
    # mct "(You're a goddamn {b}genius{/b}, [mcf].)"
    mct ""

translate id w3ThemeIntro_2fede2a7:
    # kat "That should be good enough."
    kat ""

translate id w3ThemeIntro_3da4a882:
    # kat "The real meat of it is before us."
    kat ""

translate id w3ThemeIntro_40a3b87e:
    # "Part of me was a little disappointed that my fun was cut short. This aspect of the job stimulated the inner voyeur in me."
    ""

translate id w3ThemeIntro_6ca04048:
    # "A trait Ian and I had in common, it seems..."
    ""

translate id w3ThemeIntro_3bfb4f29:
    # "I was getting a taste for this aspect. Part of me was a little disappointed that my fun was cut short."
    ""

translate id w3ThemeIntro_2abbc195:
    # kat "Oh, before we continue, I should christen this week with a more formal title."
    kat ""

translate id w3ThemeIntro_5e2d100e:
    # kat "Let's see... service... shame... {i}suffering{/i} maybe?"
    kat ""

translate id w3ThemeIntro_2758a506:
    # mc "I think \"hell week\" alone suffices. It has... {b}impact.{/b}"
    mc ""

translate id w3ThemeIntro_78dbcbc5:
    # kat "You think?"
    kat ""

translate id w3ThemeIntro_1ce14d4c:
    # "An honest, divorced thought slipped from my tongue as easily as if I were picking a team name for trivia night."
    ""

translate id w3ThemeIntro_d01ae9c9:
    # mc "The patrons will respond to it. I'm sure of it."
    mc ""

translate id w3ThemeIntro_edc3812c:
    # kat "What do you girls think?"
    kat ""

translate id w3ThemeIntro_4e0d1823:
    # "She looked at the Carnations as if awaiting their opinion on the branding."
    ""

translate id w3ThemeIntro_089d9b17:
    # carnations "..."
    carnations ""

translate id w3ThemeIntro_77ca3e8b:
    # "Naturally, they didn't have shit to say about it."
    ""

translate id w3ThemeIntro_37bde673:
    # kat "Come on, you must have an opinion."
    kat ""

translate id w3ThemeIntro_6c4951fa:
    # kat "This month is all about YOU. Take an interest."
    kat ""

translate id w3ThemeIntro_616a8c2f:
    # fel "...it kinda lends a sense of anticipation, doesn't it?"
    fel ""

translate id w3ThemeIntro_b6105d77:
    # kat "It's settled, then. Welcome to {b}hell{/b} week, girls."
    kat ""

translate id w3ThemeIntro_ed4e73f3:
    # fel "G-ahh, sh--"
    fel ""

translate id w3ThemeIntro_9701c320:
    # fel "Don't suddenly--"
    fel ""

translate id w3ThemeIntro_d9a3d198:
    # kat "All three of you: {b}on your knees.{/b}"
    kat ""

translate id w3ThemeIntro_366433c8:
    # fel "Gah...!"
    fel ""

translate id w3ThemeIntro_f5c76fd0:
    # kat "{b}Hands behind your head.{/b}"
    kat ""

translate id w3ThemeIntro_dfc8116e:
    # kat "Come on now, don't dally."
    kat ""

translate id w3ThemeIntro_15420095:
    # "I was starting to think this was the old woman's favorite position."
    ""

translate id w3ThemeIntro_1f0de0aa:
    # kat "Grab the video camera on the table, [mcf]. Our patrons will appreciate some sound for this following sequence."
    kat ""

translate id w3ThemeIntro_fa747ef2:
    # ver "Which is...?"
    ver ""

translate id w3ShockGame_3661da75:
    # kat "I told you. You girls are going to play a game."
    kat ""

translate id w3ShockGame_aef876da:
    # kat "Starting at the lowest setting, I will use this on you."
    kat ""

translate id w3ShockGame_6bf31cc7:
    # fel "Ah, goddamn it..."
    fel ""

translate id w3ShockGame_cc9fa18d:
    # kat "If you find it unpleasant, you only need to say... I don't know... 'switch' and you'll earn a reprieve. I will change my target to the next person in line."
    kat ""

translate id w3ShockGame_de565fe9:
    # kat "However, just so you know, if it loops back to you... I go up a setting. {b}There's five{/b}."
    kat ""

translate id w3ShockGame_7e415fb2:
    # kat "This is only a prelude to the main event, so we needn't drag this on. The game will last only five minutes."
    kat ""

translate id w3ShockGame_1b32b87f:
    # kat "Try to endure instead of selfishly passing your misery onto your neighbor."
    kat ""

translate id w3ShockGame_eb7bfb0e:
    # ver "What's the upside to this?"
    ver ""

translate id w3ShockGame_462681e3:
    # kat "What do you mean {i}upside{/i}?"
    kat ""

translate id w3ShockGame_8584fc45:
    # ver "You usually attach some strings to these things. Last week, during our photoshoots, there was an advantage at stake."
    ver ""

translate id w3ShockGame_14ccc91b:
    # kat "You're asking what makes this worth your while?"
    kat ""

translate id w3ShockGame_4e5b4fd3:
    # kat "Doesn't that go without saying?"
    kat ""

translate id w3ShockGame_b3161eef:
    # kat "Our \"contract\" is void if you don't perform satisfactorily."
    kat ""

translate id w3ShockGame_689bb0c6:
    # ver "Sounds like you could pull the rug out from underneath us whenever you want."
    ver ""

translate id w3ShockGame_fd032f1e:
    # kat "This isn't a bank. We're all in the honor system, Miss Lynch."
    kat ""

translate id w3ShockGame_2f32c94c:
    # ver "Exactly. Just saying... it wouldn't hurt to have an immediate incentive."
    ver ""

translate id w3ShockGame_d2637a34:
    # kat "You ballsy bitch. You're two weeks into this thing and you're getting ideas, huh?"
    kat ""

translate id w3ShockGame_b3254ae8:
    # ver "Maybe I just don't feel like being shocked today after that shit you pulled with the spider. Calling my dad was off limits!"
    ver ""

translate id w3ShockGame_20f73c91:
    # kat "Limits...? {b}Limits?!{/b}"
    kat ""

translate id w3ShockGame_c7892ca2:
    # ver "The way I see it, you need us to put on a good show. Not nearly as much as we need you clearly, but enough that--"
    ver ""

translate id w3ShockGame_12064d38:
    # "This was the same talk she brought up during our group lunch..."
    ""

translate id w3ShockGame_b7b210cb:
    # kat "*Sigh* I was hoping to have successfully taught you just how small and pathetic you really were, but now you're threatening me with an ultimatum?"
    kat ""

translate id w3ShockGame_4c9462e8:
    # "It didn't seem like a good idea then, and the enraged look on the boss' face didn't make it sound like a good idea now."
    ""

translate id w3ShockGame_bd41045e:
    # kat "Come to think of it, on the subject of putting on a good show, replacing you would be a good bit of drama for the halfway mark."
    kat ""

translate id w3ShockGame_3d039b7d:
    # kat "I can understand if you decide your business isn't worth further degrading yourself."
    kat ""

translate id w3ShockGame_d14637c4:
    # ver "..."
    ver ""

translate id w3ShockGame_f9bf7378:
    # kat "In fact, it'd be the first thing you did that made me feel an ounce of respect for you."
    kat ""

translate id w3ShockGame_d14637c4_1:
    # ver "..."
    ver ""

translate id w3ShockGame_ff4cf590:
    # "Veronica looked to either of her compatriots for help, but to no avail."
    ""

translate id w3ShockGame_70f61a96:
    # "All sense of leverage she thought she had was immediately uprooted when she realized neither Rosalind nor Felicia was on her side."
    ""

translate id w3ShockGame_cf201151:
    # kat "If you want, this can all be for naught. We can split amicably, although I caution you about getting overly emotional. If need be..."
    kat ""

translate id w3ShockGame_854f1fa1:
    # mct "(I didn't believe that for one fucking second.)"
    mct ""

translate id w3ShockGame_03225f63:
    # kat "I can {b}ruin{/b} you, Veronica."
    kat ""

translate id w3ShockGame_3da6ddaf:
    # "Now, {b}that{/b} one, I did believe."
    ""

translate id w3ShockGame_d14637c4_2:
    # ver "..."
    ver ""

translate id w3ShockGame_504e2970:
    # kat "Assume the position, Miss Lynch."
    kat ""

translate id w3ShockGame_d14637c4_3:
    # ver "..."
    ver ""

translate id w3ShockGame_a1d91863:
    # kat "Perfect! Now--"
    kat ""

translate id w3ShockGame_526a9f02:
    # kat "Now that is settled... let us..."
    kat ""

translate id w3ShockGame_485a7f95:
    # kat "Oh, dear... I forgot."
    kat ""

translate id w3ShockGame_a1c98508:
    # kat "Hold that position, girls. I'll be right back."
    kat ""

translate id w3ShockGame_93deaf69:
    # mc "Ah..."
    mc ""

translate id w3ShockGame_73d4e8ee:
    # fel "......"
    fel ""

translate id w3ShockGame_8838af23:
    # ver "......"
    ver ""

translate id w3ShockGame_abd837cd:
    # rose "..."
    rose ""

translate id w3ShockGame_7a7aa42d:
    # mc "So, uh... how are you three doing? Have a nice weekend?"
    mc ""

translate id w3ShockGame_9664eac9:
    # ver "Oh, yeah. Real fuckin' chipper. Can't you tell?"
    ver ""

translate id w3ShockGame_b80d3909:
    # rose "She said to hold the position..."
    rose ""

translate id w3ShockGame_52c8ac9a:
    # ver "Save yourself some discomfort."
    ver ""

translate id w3ShockGame_508330fd:
    # fel "...*sigh* the camera is rolling."
    fel ""

translate id w3ShockGame_c8637365:
    # ver "What's your point? She isn't going any easier on us than she plans to."
    ver ""

translate id w3ShockGame_381b241a:
    # rose "Do you think this is going to hurt?"
    rose ""

translate id w3ShockGame_c2fbc91d:
    # "Felicia alone held the position."
    ""

translate id w3ShockGame_5ed5be50:
    # mc "It hurts, but nothing too bad."
    mc ""

translate id w3ShockGame_03c2a8d6:
    # fel "It {b}fucking{/b} hurts."
    fel ""

translate id w3ShockGame_2c42bf6b:
    # rose "Oh..."
    rose ""

translate id w3ShockGame_80b5e245:
    # fel "Which end do you think she's going to start with?"
    fel ""

translate id w3ShockGame_8303b00c:
    # ver "Definitely you. You put a target on your back last week."
    ver ""

translate id w3ShockGame_b57cab4d:
    # fel "Did I?"
    fel ""

translate id w3ShockGame_1d0e95d1:
    # ver "Gah. I bet the bitch didn't even need to make a call. This is some mind-game shit."
    ver ""

translate id w3ShockGame_41589862:
    # rose "Maybe..."
    rose ""

translate id w3ShockGame_839e324c:
    # rose "Old people are pretty forgetful."
    rose ""

translate id w3ShockGame_7ab9f62b:
    # fel "All I'm saying is, let's play this smart, alright? This is a classic prisoner's dilemma."
    fel ""

translate id w3ShockGame_ddff0f0b:
    # fel "We just need to bite our tongues for 100 seconds each, spread across maybe a couple of rounds. That'd be easiest for all of us."
    fel ""

translate id w3ShockGame_1f654e8d:
    # rose "Heh. I'll try my best."
    rose ""

translate id w3ShockGame_0220b60b:
    # rose "I can be skittish when it comes to..."
    rose ""

translate id w3ShockGame_d6ce9717:
    # fel "..."
    fel ""

translate id w3ShockGame_4e07f2c5:
    # ver "Everyone has a plan until they get punched in the mouth, Blondie."
    ver ""

translate id w3ShockGame_76534dcb:
    # ver "Every week, I see smart people who lack the willpower to endure the pain of even a little exercise."
    ver ""

translate id w3ShockGame_c9aeff91:
    # fel "What's your point, Red?"
    fel ""

translate id w3ShockGame_22cd1f75:
    # ver "My point is planning for this dumb game is setting yourself up for disappointment when you realize you don't have what it takes to see it through."
    ver ""

translate id w3ShockGame_b41a545c:
    # fel "For a bitch whose job is self-improvement, you sure are fatalistic."
    fel ""

translate id w3ShockGame_69cef728:
    # ver "...I'm not saying {i}I'm{/i} the one who won't be able to endure it. Myself... I'll do my best to cut you a break."
    ver ""

translate id w3ShockGame_89639eda:
    # fel "...*sigh* I guess we'll see how this goes, huh?"
    fel ""

translate id w3ShockGame_2ef87b5d:
    # ver "Besides, where did the crazy bluster you had on stage go? You were acting pretty damn invincible."
    ver ""

translate id w3ShockGame_b060fda2:
    # fel "Well, in the moment, two dozen eyes on you can have an intoxicating effect."
    fel ""

translate id w3ShockGame_231a1121:
    # fel "It's spending the following Sunday nursing the after-effects of having a sharp triangle shoved up your cooch that has a girl angling to play the next day a little more prudently."
    fel ""

translate id w3ShockGame_ab9d8132:
    # ver "Hmmpfh. So you CAN bleed."
    ver ""

translate id w3ShockGame_dd9b092f:
    # rose "That's good to know."
    rose ""

translate id w3ShockGame_9a1ee980:
    # fel "Gah, that's not the takeaway I was looking for here."
    fel ""

translate id w3ShockGame_06ae7484:
    # "From where I stood, even if she wasn't employing one, Mrs. Pulman's mind game was a rousing success."
    ""

translate id w3ShockGame_101917c5:
    # "From where I stood, even if she wasn't employing one, Mrs. Kathleen's mind game was a rousing success."
    ""

translate id w3ShockGame_d9d7fb9e:
    # "The Carnations fell into a collective silence, their expressions more scrutable than before."
    ""

translate id w3ShockGame_fc2ffbb5:
    # "A few minutes passed, and then..."
    ""

translate id w3ShockGame_19a77ac8:
    # kat "Alright. My apologies, now we can--"
    kat ""

translate id w3ShockGame_e00c1301:
    # kat "I thought I told you bitches to hold your position?"
    kat ""

translate id w3ShockGame_05153fa8:
    # kat "What the hell is it with people today? Can't anyone do what they're fucking told?"
    kat ""

translate id w3ShockGame_7e389bb1:
    # rose "G-ggguuu That-"
    rose ""

translate id w3ShockGame_c4827113:
    # "*Tceeeek!*"
    ""

translate id w3ShockGame_c22b8e0c:
    # rose "A-aah...!"
    rose ""

translate id w3ShockGame_b1f9d84d:
    # "The game's beginning was heralded by the oppressive sound of electricity jumping from the wand's barbs and tailing off into a short yet painfully sharp yelp."
    ""

translate id w3ShockGame_22d6d920:
    # rose "Ow, g-gah...! Tha- sthin-..."
    rose ""

translate id w3ShockGame_02da7c61:
    # kat "{b}Remember{/b}, you may switch whenever you please."
    kat ""

translate id w3ShockGame_d7c2b89e:
    # rose "Hnng..."
    rose ""

translate id w3ShockGame_278c7e29:
    # "........."
    ""

translate id w3ShockGame_0fd4d656:
    # "......"
    ""

translate id w3ShockGame_a20cefa7:
    # "..."
    ""

translate id w3ShockGame_5603af07:
    # "*Thiiick~!*"
    ""

translate id w3ShockGame_165ceeba:
    # rose "W-whwhaaa...!"
    rose ""

translate id w3ShockGame_fe7e87ff:
    # "Mrs. Pulman seemed to be playing for keeps, determined to get the MILF to quickly swap by targeting the sensitive gap between Rosalind's ribs."
    ""

translate id w3ShockGame_d38f7574:
    # "Kathleen seemed to be playing for keeps, determined to get the MILF to quickly swap by targeting the sensitive gap between Rosalind's ribs."
    ""

translate id w3ShockGame_f1457a0e:
    # rose "Unggh... fwwh--"
    rose ""

translate id w3ShockGame_9ad8f4a0:
    # rose "-wahhha...?!"
    rose ""

translate id w3ShockGame_da337e29:
    # "A shockless touch drew an anticipatory flinch from her fat-tittied target."
    ""

translate id w3ShockGame_dc69af6d:
    # "The game had barely begun, and I could already see Mrs. Pulman's plan: keep an unsteady rhythm to the shocks and wield the Carnation's resulting anticipation as a weapon."
    ""

translate id w3ShockGame_fbca87fb:
    # "The game had barely begun, and I could already see Kathleen's plan: keep an unsteady rhythm to the shocks and wield the Carnation's resulting anticipation as a weapon."
    ""

translate id w3ShockGame_a95e87a0:
    # kat "No...?"
    kat ""

translate id w3ShockGame_ef02846f:
    # "As visual punctuation for the camera's benefit, she toyed and physically prodded Rosalind's breast, letting the anticipation grow thick before--"
    ""

translate id w3ShockGame_383d7875:
    # rose "G-eehh, s-switch...!"
    rose ""

translate id w3ShockGame_c3a30710:
    # "Attacking a different target altogether."
    ""

translate id w3ShockGame_8a9a5064:
    # fel "Ah, crap... already?!"
    fel ""

translate id w3ShockGame_ac8a49bc:
    # rose "W-wait, I mean..."
    rose ""

translate id w3ShockGame_19250380:
    # "Rather than being at her limit, it seemed Rosalind had blurted the words out of surprise, but it was too late. She had already passed the buck."
    ""

translate id w3ShockGame_1ae88b8f:
    # kat "Looks like it's your turn, Miss Lynch."
    kat ""

translate id w3ShockGame_289e1785:
    # kat "Mmmh... hmm~ hmm~ hmm~ Veronica... ah..."
    kat ""

translate id w3ShockGame_68493067:
    # "Mrs. Pulman's voice adopted an atypical, girlish lilt."
    ""

translate id w3ShockGame_f05362ce:
    # kat "A body like yours... was made to be put on its knees like this."
    kat ""

translate id w3ShockGame_d14637c4_4:
    # ver "..."
    ver ""

translate id w3ShockGame_2b1b2400:
    # "Veronica didn't say anything, most likely content with letting Mrs. Pulman run down the clock syllable by syllable."
    ""

translate id w3ShockGame_44074ff5:
    # "Veronica didn't say anything, most likely content with letting Kathleen run down the clock syllable by syllable."
    ""

translate id w3ShockGame_1154cffc:
    # kat "You can take a lot without breaking, can't you?"
    kat ""

translate id w3ShockGame_5a6338ac:
    # "But if anyone was more keenly aware of the time than the Carnations, it would be the old woman. This was her dance, and every pause was a deliberate step for our unseen audience."
    ""

translate id w3ShockGame_6d132a67:
    # kat "I bet you think you'll be the star of round 1~ that you'll hold out magnificently and show the other Carnations just..."
    kat ""

translate id w3ShockGame_e24d1730:
    # ver "Tsk...!"
    ver ""

translate id w3ShockGame_22a26df1:
    # kat "...how. Tough. You. Are."
    kat ""

translate id w3ShockGame_1088b6ec:
    # "*Tchwiick!*"
    ""

translate id w3ShockGame_f4101033:
    # ver "Hnngg..."
    ver ""

translate id w3ShockGame_fbd4334c:
    # "Two short shocks, delivered in quick succession to the tender skin of Veronica's armpit..."
    ""

translate id w3ShockGame_a3b08886:
    # "*Zhhhiip!*"
    ""

translate id w3ShockGame_5345cca8:
    # ver "Hnngg...!"
    ver ""

translate id w3ShockGame_2a08fc06:
    # "Three..."
    ""

translate id w3ShockGame_27d8e0e4:
    # "*Zhaaaaap!*"
    ""

translate id w3ShockGame_d1b9c486:
    # "Four..."
    ""

translate id w3ShockGame_141e5313:
    # ver "Ahh, hhnngg-!"
    ver ""

translate id w3ShockGame_42a57ae3:
    # kat "Hmmmm, no doubt~ you really are a tough cookie, slut."
    kat ""

translate id w3ShockGame_bd4f0712:
    # kat "Open your mouth and stick out your pretty pink tongue, please."
    kat ""

translate id w3ShockGame_2a104594:
    # "Veronica naturally hesitated."
    ""

translate id w3ShockGame_137f5724:
    # "*Thwiiiick!*"
    ""

translate id w3ShockGame_ae702b50:
    # kat "Show me your tongue!"
    kat ""

translate id w3ShockGame_3666b6f2:
    # ver "G-gahh...!"
    ver ""

translate id w3ShockGame_fdf95a78:
    # kat "{b}Lick it{/b}."
    kat ""

translate id w3ShockGame_d14637c4_5:
    # ver "..."
    ver ""

translate id w3ShockGame_6bfdef59:
    # "*Zaaaaap*!"
    ""

translate id w3ShockGame_9a1a51cd:
    # "For added effect, she shocked the air, urging Veronica's compliance."
    ""

translate id w3ShockGame_656c3333:
    # kat "Ah, that's right..."
    kat ""

translate id w3ShockGame_374f5014:
    # "I waited for the shoe to drop."
    ""

translate id w3ShockGame_4da537e1:
    # kat "That's a good bitch..."
    kat ""

translate id w3ShockGame_86f25590:
    # "A shock delivered to the tongue would hurt like a bitch..."
    ""

translate id w3ShockGame_877fcc9c:
    # kat "...but don't think I've forgotten..."
    kat ""

translate id w3ShockGame_0808a95a:
    # "It didn't come, however. Instead..."
    ""

translate id w3ShockGame_2252f9d3:
    # "*Zap*"
    ""

translate id w3ShockGame_20e5c338:
    # "*Za, zap!*"
    ""

translate id w3ShockGame_e3143630:
    # "*Za, thwack, craaaaack!*"
    ""

translate id w3ShockGame_d92e93a0:
    # ver "Gghhhuuu, ah {b}s-son of a bitch..!{/b}"
    ver ""

translate id w3ShockGame_418d0b74:
    # "Another fake out in favor of a critical hit on the spot she had been working on."
    ""

translate id w3ShockGame_bf80f7ac:
    # ver "Switch!"
    ver ""

translate id w3ShockGame_d7a54a5c:
    # "Despite the Amazon's earlier claims, Mrs. Pulman had made short, skillful work of her."
    ""

translate id w3ShockGame_3d451edd:
    # "Despite the Amazon's earlier claims, Kathleen had made short, skillful work of her."
    ""

translate id w3ShockGame_79822ec7:
    # fel "Ah, fuck..."
    fel ""

translate id w3ShockGame_19c59ca0:
    # "She had barely lasted longer than Rosalind."
    ""

translate id w3ShockGame_9c802b33:
    # kat "You're nothing, Miss Lynch. Absolutely nothing."
    kat ""

translate id w3ShockGame_b107386c:
    # kat "How many times do I have to prove that? Now, get your arms back up or it won't count~"
    kat ""

translate id w3ShockGame_73313161:
    # kat "Now, Mrs. Ford... our industrious, hard-working gold-digging bitch~ whore~ slut~"
    kat ""

translate id w3ShockGame_d12f6198:
    # fel "Ah... you're really enjoying this--"
    fel ""

translate id w3ShockGame_6d4031cf:
    # fel "G-gaaah....!"
    fel ""

translate id w3ShockGame_96e82a94:
    # fel "Tsk...! Hnngg..."
    fel ""

translate id w3ShockGame_74157804:
    # fel "W-wuuuah, {b}switch!{/b}"
    fel ""

translate id w3ShockGame_1c5df4e9:
    # kat "Huh...? I..."
    kat ""

translate id w3ShockGame_ce5a63b6:
    # "Both the other Carnations and Mrs. Pulman herself looked dumbfounded at Felicia's quick submission."
    ""

translate id w3ShockGame_6dccd704:
    # "Both the other Carnations and Kathleen herself looked dumbfounded at Felicia's quick submission."
    ""

translate id w3ShockGame_39ddbde5:
    # ver "You got to be kidding me..."
    ver ""

translate id w3ShockGame_45a59209:
    # "The quickest of any of them."
    ""

translate id w3ShockGame_d2f60ba4:
    # kat "{i}Interesting{/i}. After your showing Saturday, I was expecting you to hold out much longer."
    kat ""

translate id w3ShockGame_67785a13:
    # fel "It fucking hurts!"
    fel ""

translate id w3ShockGame_4272864c:
    # ver "No shit! And now it's going to--"
    ver ""

translate id w3ShockGame_cf7652f9:
    # rose "Y-yyyeee!"
    rose ""

translate id w3ShockGame_9ce150fd:
    # "The round began anew, this time on the second setting."
    ""

translate id w3ShockGame_e3788cfe:
    # "This one went about as quick."
    ""

translate id w3ShockGame_6d8bbf6d:
    # "Rosalind lasted about the same."
    ""

translate id w3ShockGame_f7ad3ca7:
    # "Veronica valiantly held out longer than previously."
    ""

translate id w3ShockGame_0915047c:
    # fel "S-switch!"
    fel ""

translate id w3ShockGame_12cce825:
    # "...and Felicia once again folded to a couple of shocks."
    ""

translate id w3ShockGame_5a0f44a1:
    # rose "Again?!"
    rose ""

translate id w3ShockGame_d8b5c3f4:
    # "It made sense. Felicia was a hedonist, not a masochist. This game, divorced from physical, sexual gratification or even the arousal of having many men view her naked body, didn't suit her."
    ""

translate id w3ShockGame_3c40343b:
    # rose "After all your talk about playing this smart?!"
    rose ""

translate id w3ShockGame_fcee5dc4:
    # "Rosalind, next up on the chopping block, was uncharacteristically pissed."
    ""

translate id w3ShockGame_2fe5806e:
    # fel "I'd be more motivated if you two didn't fold in twenty seconds!"
    fel ""

translate id w3ShockGame_37a9d74b:
    # ver "It's better than 5!"
    ver ""

translate id w3ShockGame_78fd2d50:
    # "*Tchhiiiik!*"
    ""

translate id w3ShockGame_84ca11b0:
    # rose "Hnngg...! S-switch!"
    rose ""

translate id w3ShockGame_747080d4:
    # "Everyone quickly folded from there."
    ""

translate id w3ShockGame_4d7c840b:
    # "To her credit, Veronica lasted longer in the third round but quickly gave up in the fourth."
    ""

translate id w3ShockGame_c7585c9f:
    # fel "Switch!"
    fel ""

translate id w3ShockGame_87e3af19:
    # kat "Did you really just try that, Mrs. Ford? I have yet to even shock you."
    kat ""

translate id w3ShockGame_3adf9eb7:
    # fel "Hehehe... well... worth a fuckin' shot, right?"
    fel ""

translate id w3ShockGame_012100f6:
    # "Of all of them, Felicia proved to be the weakest link."
    ""

translate id w3ShockGame_d44f9616:
    # kat "Get back into position!"
    kat ""

translate id w3ShockGame_1643a1d9:
    # "With just under two minutes to spare, the Carnations were already facing down round 5 and the highest setting."
    ""

translate id w3ShockGame_84e36b2f:
    # kat "Somehow, you girls managed to subvert even my low expectations. I didn't believe we'd get past round 3."
    kat ""

translate id w3ShockGame_06d5720d:
    # rose "Hnngg...?"
    rose ""

translate id w3ShockGame_509b3633:
    # kat "Hmmm..."
    kat ""

translate id w3ShockGame_34ea3c27:
    # kat "..."
    kat ""

translate id w3ShockGame_c9450ba8:
    # kat "Get some POV shots of you doing it."
    kat ""

translate id w3ShockGame_0fd4d656_1:
    # "......"
    ""

translate id w3ShockGame_a20cefa7_1:
    # "..."
    ""

translate id w3ShockGame_3de5250e:
    # kat "[mcf]?"
    kat ""

translate id w3ShockGame_e7ba29c9:
    # "By this point, I had practically melded with the video camera. After that, I hadn't a thought in my head."
    ""

translate id w3ShockGame_a91636af:
    # "It took me a moment to even register that the old woman was talking to me as {i}a person{/i}."
    ""

translate id w3ShockGame_eea3baed:
    # mc "...what about the time?"
    mc ""

translate id w3ShockGame_d7cec9f6:
    # kat "Time's ticking, but since there's no point in switching anymore, let's say they each get three, and then we'll move on to the {b}fun{/b} part."
    kat ""

translate id w3ShockGame_8a9d4bf7:
    # rose "Hehehe.. he... this... wasn't the fun part?"
    rose ""

translate id w3ShockGame_ded68731:
    # mc "...s'okay."
    mc ""

translate id w3ShockGame_35b3f675:
    # "It's not like me or her doing it really made a difference here."
    ""

translate id w3ShockGame_9a01cc99:
    # "At least, that is what I told myself as I felt a sickeningly sweet knot form in my stomach."
    ""

translate id w3ShockGame_4e9d0088:
    # "This baby was on the menu during Felicia's leg of the last exhibition, but I had purposefully avoided using it. If not for the gold digger's sake, then my own."
    ""

translate id w3ShockGame_095d5e0c:
    # "I knew it would rival the feeling I got from torturing Harper."
    ""

translate id w3ShockGame_e6c74d26:
    # "Just the very thought of using this thing enflamed my urges."
    ""

translate id w3ShockGame_3c4a74d0:
    # "This will look stupid, but in my mind, it was a matter of utmost prudence. It was my opinion that if you're going to inflict pain on someone for either of your gratifications..."
    ""

translate id w3ShockGame_061fb09e:
    # mc "Hnngg...!"
    mc ""

translate id w3ShockGame_5130ed79:
    # "...the person who inflicted it should try it themselves."
    ""

translate id w3ShockGame_a48e096d:
    # mct "(Ah, that was...)"
    mct ""

translate id w3ShockGame_4c2e3415:
    # "The first shock confirmed what I had suspected by watching: knowing the precise time it was coming lessened the effect of the shock."
    ""

translate id w3ShockGame_d5712108:
    # "Mind you, it still hurt like hell. At this setting, a horrible pinch immediately robbed me of whole seconds of my breath from me."
    ""

translate id w3ShockGame_a248a34a:
    # mct "(Again...!)"
    mct ""

translate id w3ShockGame_60ec9dc0:
    # mc "Huuung--!"
    mc ""

translate id w3ShockGame_72f5eb65:
    # "On top of that, the place the barb made contact left a burning sensation that persisted, grew, and ached with time."
    ""

translate id w3ShockGame_e372c738:
    # mct "(One more...)"
    mct ""

translate id w3ShockGame_25c48cee:
    # mc "Ah, ha-haak...!"
    mc ""

translate id w3ShockGame_06f255ac:
    # mct "(Fuck, that hhh...!)"
    mct ""

translate id w3ShockGame_7b93d9f4:
    # "It was unpleasant, but it wasn't unbearable."
    ""

translate id w3ShockGame_48d2d6de:
    # kat "Was that meant to be a show of solidarity?"
    kat ""

translate id w3ShockGame_8d6ddd77:
    # "There were clearly things I could do to exacerbate or mitigate the effect."
    ""

translate id w3ShockGame_fb9ca507:
    # mc "I was just curious."
    mc ""

translate id w3ShockGame_690cdf91:
    # mc "Let's get on with it."
    mc ""

translate id w3ShockGame_690cdf91_1:
    # mc "Let's get on with it."
    mc ""

translate id w3ShockGame_19f38a1b:
    # "Rosalind looked up at me with a peculiar glint in her eye."
    ""

translate id w3ShockGame_2a9f9499:
    # "It read one part apprehensive, 2 parts get this the fuck over with..."
    ""

translate id w3ShockGame_40414c1d:
    # mc "Stand up."
    mc ""

translate id w3ShockGame_b1924401:
    # mc "I'm going on three."
    mc ""

translate id w3ShockGame_71b3ef80:
    # mc "One... two..."
    mc ""

translate id w3ShockGame_106e3219:
    # rose "Hnngg, ah...!"
    rose ""

translate id w3ShockGame_1ec659c4:
    # "I made a point to target the least sensitive part of the body, the trunk."
    ""

translate id w3ShockGame_9e940dd2:
    # mc "One, two..."
    mc ""

translate id w3ShockGame_1426c9c2:
    # rose "G-gahhh...!"
    rose ""

translate id w3ShockGame_9b833538:
    # "Going again immediately after the first shock might've seemed like I was being rough. Still, I truly believed the anticipation was a multiplying factor. "
    ""

translate id w3ShockGame_9e940dd2_1:
    # mc "One, two..."
    mc ""

translate id w3ShockGame_a6a591f2:
    # rose "F-fhhhh, shit--"
    rose ""

translate id w3ShockGame_8648da4e:
    # mc "{b}All done{/b}, Rose."
    mc ""

translate id w3ShockGame_b0becfaf:
    # mc "That wasn't so bad, was it?"
    mc ""

translate id w3ShockGame_8ed98e2c:
    # rose "Actually..."
    rose ""

translate id w3ShockGame_da6de714:
    # rose "I guess not?"
    rose ""

translate id w3ShockGame_8895fafc:
    # kat "Veronica is waiting, Mr. [mcl] - and put a little more pizzazz in it this time, please."
    kat ""

translate id w3ShockGame_a46aa999:
    # mc "On your feet."
    mc ""

translate id w3ShockGame_d6b1f786:
    # mc "On your heels. Back straight."
    mc ""

translate id w3ShockGame_4aa9a992:
    # "This was my job."
    ""

translate id w3ShockGame_f6d2c005:
    # "{i}A convenient excuse for doing what I wanted to do anyway.{/i}"
    ""

translate id w3ShockGame_439a4c0d:
    # mc "Push those sloppy tits of yours together, whore."
    mc ""

translate id w3ShockGame_80becd0f:
    # mc "Push your tits together. Make it look good."
    mc ""

translate id w3ShockGame_27c1ad6c:
    # rose "Yes, sir..."
    rose ""

translate id w3ShockGame_c28c8c7b:
    # mc "I didn't tell you to speak. The only sound I want you to make..."
    mc ""

translate id w3ShockGame_f4484c73:
    # rose "Gyeee--!"
    rose ""

translate id w3ShockGame_0fa26257:
    # "Rosalind had sensitive breasts. In choosing my target, this was a fact I was well aware of."
    ""

translate id w3ShockGame_8036ca9f:
    # rose "Hnngg, f-fuck!"
    rose ""

translate id w3ShockGame_fa7f11f3:
    # mc "That's one..."
    mc ""

translate id w3ShockGame_023b7dd5:
    # mc "How many are left, Rose?"
    mc ""

translate id w3ShockGame_50a33a95:
    # rose "T-two... h-ha..."
    rose ""

translate id w3ShockGame_2d64e59e:
    # rose "Two mor--"
    rose ""

translate id w3ShockGame_00943c5f:
    # rose "Morooorewwwa{b}SHIT-!!!{/b}"
    rose ""

translate id w3ShockGame_6c02a0cb:
    # mc "One more - {i}can't you count?{/i}"
    mc ""

translate id w3ShockGame_c1799fc8:
    # rose "Euughh...! Just get it over with!"
    rose ""

translate id w3ShockGame_9e085d72:
    # "She was giving me the same pissed-off look she was giving Felicia earlier. A look that filled me with a conflicting sense of..."
    ""

translate id w3ShockGame_ad9687ca:
    # mct "(This is what Mrs. Pulman chases in her shows, right?)"
    mct ""

translate id w3ShockGame_bd10184e:
    # mct "(This is what the old woman chases in her shows, right?)"
    mct ""

translate id w3ShockGame_abd837cd_1:
    # rose "..."
    rose ""

translate id w3ShockGame_839e01e0:
    # "Almost instinctively, I had reached out and tenderly pawed at the tender stretch of tit-flesh I had targeted."
    ""

translate id w3ShockGame_ad18c852:
    # rose "Hnnggg..."
    rose ""

translate id w3ShockGame_eb4dec35:
    # mc "Alright, but give the camera your best expression. You ready?"
    mc ""

translate id w3ShockGame_60437a4b:
    # rose "Yeah... go ahead..."
    rose ""

translate id w3ShockGame_23d98e95:
    # rose "Euuggh- ah..."
    rose ""

translate id w3ShockGame_355450c3:
    # "Her face twisted in anticipation of what didn't come."
    ""

translate id w3ShockGame_ad18c852_1:
    # rose "Hnnggg..."
    rose ""

translate id w3ShockGame_fd23cf4c:
    # mc "Count to 3."
    mc ""

translate id w3ShockGame_cc18b544:
    # rose "Three--"
    rose ""

translate id w3ShockGame_4efcf435:
    # rose "Fh, hhhuu-- {b}goddamn it!{/b}"
    rose ""

translate id w3ShockGame_0eb8d94c:
    # mc "Hehe... you should've seen that one coming."
    mc ""

translate id w3ShockGame_d0aed578:
    # rose "{b}Should I?!{/b}"
    rose ""

translate id w3ShockGame_f8941e1b:
    # mc "Well... there was precedent."
    mc ""

translate id w3ShockGame_2f6e5d1e:
    # mc "Just... keep your eyes on the prize, Rose."
    mc ""

translate id w3ShockGame_130e13e4:
    # rose "Right..."
    rose ""

translate id w3ShockGame_77e41719:
    # kat "Cover back up, Mrs. Carter."
    kat ""

translate id w3ShockGame_9f32301c:
    # kat "Veronica is waiting, Mr. [mcl]."
    kat ""

translate id w3ShockGame_60cae9c5:
    # mc "Yes, Ma'am."
    mc ""

translate id w3ShockGame_de0a3aa5:
    # mc "Hey, Ronnie."
    mc ""

translate id w3ShockGame_7355575a:
    # "She gave me an unusual smile."
    ""

translate id w3ShockGame_1b3a091a:
    # ver "Hey, bones."
    ver ""

translate id w3ShockGame_1b4baeb5:
    # mc "Hey, Veronica."
    mc ""

translate id w3ShockGame_481835bd:
    # ver "Hey, yourself."
    ver ""

translate id w3ShockGame_1a936750:
    # ver "Well, what are you waiting for?"
    ver ""

translate id w3ShockGame_2b35b6fe:
    # mc "..."
    mc ""

translate id w3ShockGame_5566b3ee:
    # "Veronica had previously encouraged me to not go easy on her but that WAS a different time and place..."
    ""

translate id w3ShockGame_5566b3ee_1:
    # "Veronica had previously encouraged me to not go easy on her but that WAS a different time and place..."
    ""

translate id w3ShockGame_4f0ddeee:
    # mc "What do you think you're doing? Dogs don't look humans in the eyes."
    mc ""

translate id w3ShockGame_45e5ab32:
    # mc "{b}Avert your gaze.{/b}"
    mc ""

translate id w3ShockGame_6077dc9a:
    # "Since I was told it was such a hit, but mostly because I just enjoyed the dynamic, I decided to harken back to last week's promo."
    ""

translate id w3ShockGame_d14637c4_6:
    # ver "..."
    ver ""

translate id w3ShockGame_02256a45:
    # mc "That's better."
    mc ""

translate id w3ShockGame_c4296785:
    # "This probably seemed contrary to all my talk of friendship, but I think she understood we were each playing the role we chose."
    ""

translate id w3ShockGame_ba77774a:
    # mct "(At least, I hope she did...)"
    mct ""

translate id w3ShockGame_2a67ff8d:
    # mc "Now, on your arches. Hands like a begging puppy dog."
    mc ""

translate id w3ShockGame_df13d362:
    # "I would've thought being repeatedly shocked would've put her more on edge than that, but thankfully she painlessly complied with my extraneous demand with only a {i}mildly{/i} annoyed look on her face."
    ""

translate id w3ShockGame_2ef0d782:
    # ver "Let me guess... you want me to beg for it?"
    ver ""

translate id w3ShockGame_8c74340e:
    # ver "{b}Eeuugn{/b}-!"
    ver ""

translate id w3ShockGame_7ea299d2:
    # ver "Ddd--"
    ver ""

translate id w3ShockGame_d4fda04a:
    # "I deliberately chose that unsteady position for a reason."
    ""

translate id w3ShockGame_a83bae8e:
    # ver "{b}Damn it!{/b}"
    ver ""

translate id w3ShockGame_e08a7792:
    # "...and as expected, the recoil from being shocked caused the Amazon to lose her footing."
    ""

translate id w3ShockGame_426e7a45:
    # ver "The neck? Really--"
    ver ""

translate id w3ShockGame_d7c58c39:
    # mc "Down, girl."
    mc ""

translate id w3ShockGame_e5013de8:
    # mct "(Ah, shit...)"
    mct ""

translate id w3ShockGame_29d473cb:
    # "It's gratifying when the plan in your head takes the physical form exactly as you imagined."
    ""

translate id w3ShockGame_5be8a687:
    # ver "...g-gah, w-why didn't we just start like this then?"
    ver ""

translate id w3ShockGame_6797c3ad:
    # "I had the large woman pinned under my heel, my cock instantly growing hard from the unearned feeling of power I held over her."
    ""

translate id w3ShockGame_884d16ad:
    # "For all my talk of friendship, this seemed contrary indeed..."
    ""

translate id w3ShockGame_e65a485d:
    # "I could feel myself slipping."
    ""

translate id w3ShockGame_def9c585:
    # "*{b}Zhhiiiaaap!*{/b}"
    ""

translate id w3ShockGame_3cab6625:
    # ver "Hnngg, ghgg--"
    ver ""

translate id w3ShockGame_c2ff8018:
    # mc "You know why."
    mc ""

translate id w3ShockGame_86d55d3b:
    # "For added emphasis, I pushed down harder and ground the redhead's breast beneath my foot, delighting in the feeling of resistance as her diaphragm struggled to adapt to my weight."
    ""

translate id w3ShockGame_a85cfd20:
    # mc "If the {i}shoe{/i} was on the other foot..."
    mc ""

translate id w3ShockGame_62fe2faf:
    # ver "Y-yeahh, yeah... I'd stick it up your ass."
    ver ""

translate id w3ShockGame_6f8590f0:
    # ver "-- -ah, hehe... maybe I shouldn't have given you ideas, huh?"
    ver ""

translate id w3ShockGame_c7451072:
    # mc "Get up."
    mc ""

translate id w3ShockGame_2ab23889:
    # ver "Up, down... up, down--"
    ver ""

translate id w3ShockGame_93eac2ff:
    # ver "{b}Ghhhuuhuh...!{/b}"
    ver ""

translate id w3ShockGame_043c4720:
    # mc "That's three."
    mc ""

translate id w3ShockGame_b7b1aebf:
    # mc "Sorry, I went a little overboard."
    mc ""

translate id w3ShockGame_d14637c4_7:
    # ver "..."
    ver ""

translate id w3ShockGame_12c26e87:
    # ver "Thanks."
    ver ""

translate id w3ShockGame_599aa3a8:
    # ver "It wasn't so bad."
    ver ""

translate id w3ShockGame_f602f2ff:
    # ver "Can't help but notice that you went easier on one of us..."
    ver ""

translate id w3ShockGame_599aa3a8_1:
    # ver "It wasn't so bad."
    ver ""

translate id w3ShockGame_f602f2ff_1:
    # ver "Can't help but notice that you went easier on one of us..."
    ver ""

translate id w3ShockGame_c7ddee63:
    # kat "Marvelous, [mcf]. You're a natural!"
    kat ""

translate id w3ShockGame_055a7aeb:
    # mc "Stand up and touch your toes."
    mc ""

translate id w3ShockGame_2fbf6306:
    # ver "...you gotta work on your pick-up lines."
    ver ""

translate id w3ShockGame_01ca372a:
    # "An ass as large as Veronica's offered the best of both worlds."
    ""

translate id w3ShockGame_a07d5c27:
    # "It would look great for the camera while offering a relatively less painful target due to the sheer amount of fat and muscle."
    ""

translate id w3ShockGame_2f122483:
    # "It would add the bit of \"pizzazz\" the old woman instructed me to while offering a relatively less painful target due to the sheer amount of fat and muscle."
    ""

translate id w3ShockGame_96718fd1:
    # ver "Well, let me fucking have it, eh?"
    ver ""

translate id w3ShockGame_72365294:
    # "Instead of a countdown, I let her know it was coming with a slight touch."
    ""

translate id w3ShockGame_1f56feb7:
    # mc "Ready?"
    mc ""

translate id w3ShockGame_081ded8d:
    # ver "Ye--"
    ver ""

translate id w3ShockGame_bef6a526:
    # "One."
    ""

translate id w3ShockGame_1cd09b9b:
    # "{b}*Thiiiik!*{/b}"
    ""

translate id w3ShockGame_d2d50c6e:
    # "Two."
    ""

translate id w3ShockGame_a85a88b0:
    # mc "One more..."
    mc ""

translate id w3ShockGame_6b096efa:
    # ver "{b}G-gahh!{/b}"
    ver ""

translate id w3ShockGame_6326ef8d:
    # "Oops, that one was a bit TOO close to her, uh..."
    ""

translate id w3ShockGame_9be43212:
    # rose "Hnng. She had it easier..."
    rose ""

translate id w3ShockGame_26b26309:
    # kat "Not quite the added sauce I was hoping for, but..."
    kat ""

translate id w3ShockGame_136764c5:
    # kat "Move onto Mrs. Ford, will you?"
    kat ""

translate id w3ShockGame_60cae9c5_1:
    # mc "Yes, Ma'am."
    mc ""

translate id w3ShockGame_1fd3937e:
    # kat "Oh... [mcf]?"
    kat ""

translate id w3ShockGame_6e33c38a:
    # kat "Come here!"
    kat ""

translate id w3ShockGame_7fbd31a0:
    # kat "You're enjoying yourself, aren't you?"
    kat ""

translate id w3ShockGame_50c4cba9:
    # mc "I'm doing my job."
    mc ""

translate id w3ShockGame_c71b21ed:
    # kat "Uh huh. Then why are you smiling right now?"
    kat ""

translate id w3ShockGame_0513656e:
    # mc "If it's going to be done, I might as well--"
    mc ""

translate id w3ShockGame_cf5074a7:
    # kat "You don't need to explain yourself to me. I just wanted to tell you..."
    kat ""

translate id w3ShockGame_3c69529b:
    # kat "Let that Felicia cunt REALLY have it. Consider it an {b}order{/b}."
    kat ""

translate id w3ShockGame_2b35b6fe_1:
    # mc "..."
    mc ""

translate id w3ShockGame_a6d6e012:
    # "I didn't quite know what that meant..."
    ""

translate id w3ShockGame_91382a1c:
    # kat "I {b}order you{/b} to go all out on Mrs. Ford."
    kat ""

translate id w3ShockGame_47be7d78:
    # kat "I'd love to see it, you understand?"
    kat ""

translate id w3ShockGame_719540e7:
    # mct "(...all out?)"
    mct ""

translate id w3ShockGame_1ddab3d8:
    # mc "...understood."
    mc ""

translate id w3ShockGame_9e4c85d5:
    # fel "...ah, what's with that intense look on your face?"
    fel ""

translate id w3ShockGame_f1d13e3c:
    # mc "Just thinking for a second."
    mc ""

translate id w3ShockGame_060ebd29:
    # fel "If it's {i}just thinking{/i}, don't be so ominous! Damn!"
    fel ""

translate id w3ShockGame_3b21f354:
    # mc "......"
    mc ""

translate id w3ShockGame_2b35b6fe_2:
    # mc "..."
    mc ""

translate id w3ShockGame_ff7bd8a2:
    # fel "What are you--"
    fel ""

translate id w3ShockGame_c361625d:
    # mc "--{b}got it!{/b}"
    mc ""

translate id w3ShockGame_b433b68b:
    # fel "{b}Huh--?{/b}"
    fel ""

translate id w3ShockGame_94e01824:
    # "With perhaps more force than I meant to, I grabbed Felicia by her ponytail and yanked her head back, forcing us to see eye-to-eye."
    ""

translate id w3ShockGame_c0543923:
    # "In this way, there'd be no mistaking it. I could convey my intentions and show Felicia where my head was at."
    ""

translate id w3ShockGame_be5660be:
    # mc "You know, it seems you have a target on your back. Mrs. Pulman ordered you to get the worst of it, so..."
    mc ""

translate id w3ShockGame_cba05243:
    # mc "You know, it seems you have a target on your back. Kathleen ordered you to get the worst of it, so..."
    mc ""

translate id w3ShockGame_e5df4c8e:
    # mc "Apologies in advance. She was rather {b}emphatic{/b} about it."
    mc ""

translate id w3ShockGame_0000e888:
    # "I might be able to fool the camera, but I wouldn't fool the old woman. Felicia, of all people, surely understood getting your hands dirty to preserve your meal ticket."
    ""

translate id w3ShockGame_3e8aa13a:
    # fel "Hnng, {i}great{/i}."
    fel ""

translate id w3ShockGame_1f56feb7_1:
    # mc "Ready?"
    mc ""

translate id w3ShockGame_4318f90d:
    # fel "As I'll ever be--"
    fel ""

translate id w3ShockGame_2e9a677a:
    # mc "{b}You fuckin' sow{/b}... ahh~!"
    mc ""

translate id w3ShockGame_41a31b3b:
    # mc "*Fweee!*"
    mc ""

translate id w3ShockGame_6370d2fa:
    # "Instinctually, Felicia looked pissed. Anyone would."
    ""

translate id w3ShockGame_f394df61:
    # fel "Grr, f-fuck you!"
    fel ""

translate id w3ShockGame_5e68ee0a:
    # "After all, spitting on someone was practically a {i}primal{/i} form of disrespect."
    ""

translate id w3ShockGame_cb7c6492:
    # mc "Damn, that's a good look..."
    mc ""

translate id w3ShockGame_b6bd19b0:
    # "That's twice in the past three days that I spit on the gold digging blonde... {b}shit{/b}, {i}was this going to be a new thing with me or what?{/i}"
    ""

translate id w3ShockGame_e9192fd6:
    # "A sickening feeling flooded my stomach, telling me to push it further."
    ""

translate id w3ShockGame_5213711e:
    # fel "W-wah..!"
    fel ""

translate id w3ShockGame_5b517247:
    # mc "Crawl to the couch, pig."
    mc ""

translate id w3ShockGame_257463cb:
    # "With the exact amount of enthusiasm I intended, I tossed Felicia to the ground and barked my order."
    ""

translate id w3ShockGame_ca3e7cae:
    # "*Zhhiiiiiack!*"
    ""

translate id w3ShockGame_576d7e2d:
    # mc "Hurry the fuck up!"
    mc ""

translate id w3ShockGame_938117c2:
    # fel "F-hwwha, j-Jesus Christ! I am!"
    fel ""

translate id w3ShockGame_24dacf11:
    # "Along the way, I captured the hypnotic way her ass danced as she crawled on all fours. Relishing, not quite so secretly, the sight of the rich blonde on her knees."
    ""

translate id w3ShockGame_bd753e88:
    # fel "Hnngg..."
    fel ""

translate id w3ShockGame_62938579:
    # mc "The sow has a nice piggy-cunt."
    mc ""

translate id w3ShockGame_7b9ec194:
    # fel "S-stop dragging this out."
    fel ""

translate id w3ShockGame_2b35b6fe_3:
    # mc "..."
    mc ""

translate id w3ShockGame_60c293ee:
    # fel "G-gah...!"
    fel ""

translate id w3ShockGame_c7f375ab:
    # mc "That one doesn't count. Spread your legs."
    mc ""

translate id w3ShockGame_c0d7ac04:
    # fel "What the hell do you mean it doesn't count?!"
    fel ""

translate id w3ShockGame_30e90ec0:
    # mc "Did that count, Mrs. Pulman?"
    mc ""

translate id w3ShockGame_65c19241:
    # kat "I don't think you made clean contact. You still got two."
    kat ""

translate id w3ShockGame_3db1f51e:
    # fel "This is bullshit!"
    fel ""

translate id w3ShockGame_4d03ac0a:
    # kat "It's what you signed up for, Mrs. Ford."
    kat ""

translate id w3ShockGame_2207baf6:
    # fel "Hnggg..."
    fel ""

translate id w3ShockGame_0b874f35:
    # mc "Spread your legs."
    mc ""

translate id w3ShockGame_4453d35c:
    # fel "You're enjoying this too much!"
    fel ""

translate id w3ShockGame_51701a75:
    # mc "Tits out."
    mc ""

translate id w3ShockGame_fa1855cd:
    # fel "*Sigh* Fair enough."
    fel ""

translate id w3ShockGame_666d4a28:
    # mc "You look beautiful, Felicia."
    mc ""

translate id w3ShockGame_94c6e69e:
    # fel "Yeah, I fuckin--"
    fel ""

translate id w3ShockGame_c2455ecf:
    # fel "Hnnng, s-shit....!"
    fel ""

translate id w3ShockGame_2cae41da:
    # mc "Dumb bitch."
    mc ""

translate id w3ShockGame_ff171829:
    # mc "That one didn't count ei--"
    mc ""

translate id w3ShockGame_b0640c7e:
    # fel "Are you fucking kidding me?!"
    fel ""

translate id w3ShockGame_94a68d23:
    # kat "It didn't count."
    kat ""

translate id w3ShockGame_b523abbf:
    # fel "Hnnng...! What the HELL is sexy about this?"
    fel ""

translate id w3ShockGame_5352132a:
    # kat "Do you want to quit?"
    kat ""

translate id w3ShockGame_015a21c5:
    # fel "Of course not!"
    fel ""

translate id w3ShockGame_4021c304:
    # kat "Then it didn't count."
    kat ""

translate id w3ShockGame_1e02ce32:
    # fel "...{b}can't help but notice the disparity between my turn and the others here!{/b}"
    fel ""

translate id w3ShockGame_0fd4d656_2:
    # "......"
    ""

translate id w3ShockGame_a20cefa7_2:
    # "..."
    ""

translate id w3ShockGame_88908f3e:
    # fel "Please... this isn't fucking fair. Two more... l-let them count?"
    fel ""

translate id w3ShockGame_6c9326fc:
    # "I don't know if the pitiful look on her face was genuine right now or if she was trying to manipulate me, but her expression..."
    ""

translate id w3ShockGame_502ab92d:
    # "It shamefully excited me even more."
    ""

translate id w3ShockGame_22938cd9:
    # mc "Okay, {b}two more{/b}."
    mc ""

translate id w3ShockGame_15cdc9d0:
    # "It made me feel like a real prick."
    ""

translate id w3ShockGame_22938cd9_1:
    # mc "Okay, {b}two more{/b}."
    mc ""

translate id w3ShockGame_b1557dde:
    # mc "You gotta make them count, though. So show your camera your best expression."
    mc ""

translate id w3ShockGame_8f76ae69:
    # fel "*Gulp* Got it..."
    fel ""

translate id w3ShockGame_76b9b1bd:
    # fel "Go ahead."
    fel ""

translate id w3ShockGame_2e64fff9:
    # mc "Good girl..."
    mc ""

translate id w3ShockGame_257ab703:
    # mc "One...!"
    mc ""

translate id w3ShockGame_e47dbf31:
    # fel "H-hhhaaa...! HNngng... uuuhhh..."
    fel ""

translate id w3ShockGame_0b2dd2ff:
    # fel "T-two...! Give me number two, you bast--"
    fel ""

translate id w3ShockGame_4e3f2c20:
    # fel "Huuunngghh-!"
    fel ""

translate id w3ShockGame_02aa013c:
    # mc "Now for number thr--"
    mc ""

translate id w3ShockGame_5f072344:
    # fel "W-whn, wait, what?"
    fel ""

translate id w3ShockGame_fe3ad14e:
    # fel "HNngguguuuuhgghh.....!"
    fel ""

translate id w3ShockGame_5693a819:
    # mc "Huh..."
    mc ""

translate id w3ShockGame_b03ad6d2:
    # "The last surprise shock caused the disheveled housewife to piss herself a little."
    ""

translate id w3ShockGame_f0ad7ee4:
    # fel "F-fuck...! Goddamn it-! Ah...!"
    fel ""

translate id w3ShockGame_2b35b6fe_4:
    # mc "..."
    mc ""

translate id w3ShockGame_ded4ffba:
    # fel "{b}You f-fuckin' jerk!{/b} Hnngg...!"
    fel ""

translate id w3ShockGame_b2281737:
    # ver "Holy shit..."
    ver ""

translate id w3ShockGame_209af82a:
    # rose "Huh... that's..."
    rose ""

translate id w3ShockGame_6f5958a8:
    # mc "Was that to your satisfaction, Mrs. Pulman?"
    mc ""

translate id w3ShockGame_9c8a9d24:
    # kat "Was that to my...?"
    kat ""

translate id w3ShockGame_870aeaa7:
    # kat "{b}That was beautiful!{/b}"
    kat ""

translate id w3ShockGame_98cfdd43:
    # kat "I knew you had it in you..."
    kat ""

translate id w3ShockGame_06b2e59e:
    # mc "I only did what you asked..."
    mc ""

translate id w3ShockGame_43b55c3b:
    # kat "We gotta work on your modesty, but..."
    kat ""

translate id w3ShockGame_b8e151ca:
    # "Her eyes wandered down to my bulging crotch."
    ""

translate id w3ShockGame_a65a9fc9:
    # kat "...only in certain areas."
    kat ""

translate id w3ShockGame_d540dfe2:
    # mc "What's next?"
    mc ""

translate id w3ShockGame_00bbcb06:
    # kat "For you? Go to the bar and kill some time for half an hour. I'll retrieve you when I need you."
    kat ""

translate id w3ShockGame_4532be06:
    # "So that was that, huh?"
    ""

translate id w3ShockGame_79287c64:
    # "-- a wink. One that I hoped read, \"trust me\" rather than I'm about to sneeze."
    ""

translate id w3ShockGame_7f71855d:
    # mc "*Whisper* Work with me a little here, eh?"
    mc ""

translate id w3ShockGame_12aabd8a:
    # "In a way, we were both on the hook here, but Felicia didn't have the luxury of being an active participant. That meant the onus was on me to deliver us to the end of this asinine game."
    ""

translate id w3ShockGame_278c7e29_1:
    # "........."
    ""

translate id w3ShockGame_0fd4d656_3:
    # "......"
    ""

translate id w3ShockGame_ba81317d:
    # fel "...what are you waiting for?"
    fel ""

translate id w3ShockGame_9bff4b6e:
    # "Directing her with my own eyes, I said..."
    ""

translate id w3ShockGame_11fd0351:
    # mc "Now... {i}piggie{/i}..."
    mc ""

translate id w3ShockGame_420a531b:
    # mc "...*whisper* brace yourself."
    mc ""

translate id w3ShockGame_3e41c992:
    # fel "W-wah..."
    fel ""

translate id w3ShockGame_5780ead2:
    # mc "Crawl to the fuckin' couch."
    mc ""

translate id w3ShockGame_38ab8e69:
    # "With the exact amount of enthusiasm I intended, I threw Felicia off balance and barked my order."
    ""

translate id w3ShockGame_9097ad89:
    # mc "Hurry up!"
    mc ""

translate id w3ShockGame_b70ce4e2:
    # fel "G-ghh...!"
    fel ""

translate id w3ShockGame_028e79a2:
    # "That was number {b}one{/b}, delivered as just one aspect of a flurry of activity that Felicia had no time to dwell on."
    ""

translate id w3ShockGame_b824c602:
    # fel "I'm going, I'm go--"
    fel ""

translate id w3ShockGame_cfba19a9:
    # "Along the way, I captured the hypnotic way her ass danced as she crawled on all fours, secretly relishing the sight of the rich blonde on her knees."
    ""

translate id w3ShockGame_2294d9ac:
    # fel "Hnngg... what are you doing?"
    fel ""

translate id w3ShockGame_0b8f9558:
    # mc "I'm documenting your body, but you shouldn't focus on what I'm doing now."
    mc ""

translate id w3ShockGame_ae882fbf:
    # fel "Yeah...? What should I be...?"
    fel ""

translate id w3ShockGame_0b874f35_1:
    # mc "Spread your legs."
    mc ""

translate id w3ShockGame_d6ce9717_1:
    # fel "..."
    fel ""

translate id w3ShockGame_c486c867:
    # "She did as I asked, granting me further access down the long stretch of her toned legs."
    ""

translate id w3ShockGame_11aa15dd:
    # mc "*Whisper* Lotta people are going to see this. Which Felicia are you going to show them?"
    mc ""

translate id w3ShockGame_d6ce9717_2:
    # fel "..."
    fel ""

translate id w3ShockGame_35f42b6b:
    # "She understood my words. This setup might seem cold and clinical, but she did have an audience - that held some meaning to Felicia, didn't it?"
    ""

translate id w3ShockGame_b8f8cf22:
    # mc "*Whisper* Just two shocks in just as many settings. You've done more unpleasant things, right?"
    mc ""

translate id w3ShockGame_f3ef0876:
    # mc "I mean, this has got to be more palatable than half the fat old bastards you've fucked. Am I wrong?"
    mc ""

translate id w3ShockGame_c7a28ee3:
    # "This part, I said loud enough for the old woman to hear."
    ""

translate id w3ShockGame_bcfedd07:
    # fel "Oh, fuck you, asshole!"
    fel ""

translate id w3ShockGame_1bf1f01f:
    # mc "Yeah, yeah... Get your disgusting tits out, you sow."
    mc ""

translate id w3ShockGame_b5692b42:
    # fel "Fair enough."
    fel ""

translate id w3ShockGame_0501829f:
    # fel "Is this--"
    fel ""

translate id w3ShockGame_e39667d8:
    # fel "G-hhggg...! D-damn it!"
    fel ""

translate id w3ShockGame_8c31d27c:
    # "I aimed at a pretty tender area, but it wasn't the worst."
    ""

translate id w3ShockGame_0ee56ac0:
    # mc "You got one more. Let's make it count."
    mc ""

translate id w3ShockGame_477c20dd:
    # fel "What do you...?"
    fel ""

translate id w3ShockGame_0160a014:
    # "I did my best to gesture back to where Kat was."
    ""

translate id w3ShockGame_388af02a:
    # "One more. {b}Make it count.{/b}"
    ""

translate id w3ShockGame_6469fb38:
    # "Then it would be over."
    ""

translate id w3ShockGame_ece06eea:
    # fel "Heh... ah... s-shit..."
    fel ""

translate id w3ShockGame_10792946:
    # fel "Bah! Go ahead!"
    fel ""

translate id w3ShockGame_39744488:
    # "As a man, I couldn't imagine how this might feel, but Felicia had her game face on, and it should satisfy the old woman at the least."
    ""

translate id w3ShockGame_ad8ff35f:
    # fel "Ghh, ahhhoowowowow--"
    fel ""

translate id w3ShockGame_084842bc:
    # fel "WHYDIDISAY--"
    fel ""

translate id w3ShockGame_415a8122:
    # fel "Ah... {i}that's three...{/i}"
    fel ""

translate id w3ShockGame_4e8744b7:
    # mc "Yep. All done."
    mc ""

translate id w3ShockGame_6f5958a8_1:
    # mc "Was that to your satisfaction, Mrs. Pulman?"
    mc ""

translate id w3ShockGame_8b8196da:
    # kat "......"
    kat ""

translate id w3ShockGame_3c4aee24:
    # kat "...good enough, I suppose. *Sigh* You didn't quite get that look about you I was hoping for."
    kat ""

translate id w3ShockGame_394385b3:
    # mc "Wasn't this about Felicia?"
    mc ""

translate id w3ShockGame_1347f658:
    # kat "It's all connected..."
    kat ""

translate id w3ShockGame_0bd784ea:
    # "She looked through me in a way that stopped me from asking further questions. The fact THAT wasn't rough enough to satisfy her was telling..."
    ""

translate id w3ShockGame_d540dfe2_1:
    # mc "What's next?"
    mc ""

translate id w3ShockGame_00bbcb06_1:
    # kat "For you? Go to the bar and kill some time for half an hour. I'll retrieve you when I need you."
    kat ""

translate id w3ShockGame_4532be06_1:
    # "So that was that, huh?"
    ""

translate id w3ShockGame_f02888e9:
    # kat "...and for them? I've got to get them set up and hooked in for the punishment game."
    kat ""

translate id w3ShockGame_4b675914:
    # rose "Ummm, hooked in?"
    rose ""

translate id w3ShockGame_8d515308:
    # kat "Yep. All three of you are going to have some fun."
    kat ""

translate id w3ShockGame_6e1c0575:
    # kat "Well, Miss Lynch and Mrs. Ford, at least. Rosie, you'll take a more {i}active role.{/i}"
    kat ""

translate id w3ShockGame_03b3fb43:
    # kat "Well, Mrs. Ford at least. You and Miss Lynch will take a more... {i}active{/i} role."
    kat ""

translate id w3ShockGame_21cfe9bf:
    # kat "Well, Miss Lynch at least. You and Mrs. Ford will take a more... {i}active{/i} role."
    kat ""

translate id w3ShockGame_9334855f:
    # ver "Looking forward to it..."
    ver ""

translate id w3ShockGame_84909c4e:
    # kat "Before you go... do you want to hear something amusing?"
    kat ""

translate id w3ShockGame_167689dd:
    # mct "(This will be good...)"
    mct ""

translate id w3ShockGame_63c3f877:
    # kat "This thing doesn't even have multiple settings. It's all one power!"
    kat ""

translate id w3ShockGame_fd3168c8:
    # mc "Huh... I did wonder about that."
    mc ""

translate id w3ShockGame_bec4a8ab:
    # "The only button that I noticed was the trigger."
    ""

translate id w3ShockGame_7ea9d450:
    # mc "That's... that's actually kinda funny. It still stung like hell, though."
    mc ""

translate id w3ShockGame_d0a30b22:
    # kat "True, but never more or less. Isn't the power of suggestion a wonderful thing?"
    kat ""

translate id w3ShockGame_850dd6ac:
    # mc "Yeah, thanks for the teaching moment..."
    mc ""

translate id w3ShockGame_83fd9acc:
    # "Suddenly, an intrusive thought took root in my brain. I wanted to..."
    ""

translate id w3ShockGame_2a00ce14:
    # kat "Now, why don't you--"
    kat ""

translate id w3ShockGame_8fc9aef3:
    # kat "G-gh...! Y-you...!"
    kat ""

translate id w3ShockGame_41d1a080:
    # "Without a second thought, I did just that."
    ""

translate id w3ShockGame_1a2086bf:
    # kat "You've wanted to do that since before the phone call, haven't you?"
    kat ""

translate id w3ShockGame_5bb0a769:
    # kat "Well... fair's fair. {b}I guess{/b}."
    kat ""

translate id w3ShockGame_0fd4d656_4:
    # "......"
    ""

translate id w3ShockGame_a20cefa7_3:
    # "..."
    ""

translate id w3ShockGame_ec9d9754:
    # kat "Now... why don't you get your ass to the bar and give that fanatical slut some company while you wait?"
    kat ""

translate id w3ShockGame_f54ff9bc:
    # mc "Um... who?"
    mc ""

translate id w3ShockGame_1a146525:
    # kat "Sorry. I mean... {b}Miss Lundgren{/b}. That's just my pet name for her."
    kat ""

translate id w3ShockGame_a50fd88c:
    # mc "Seems like you two are excellent friends."
    mc ""

translate id w3ShockGame_3750965c:
    # kat "Go now."
    kat ""

translate id w3ShockGame_60cae9c5_2:
    # mc "Yes, Ma'am."
    mc ""

translate id w3ShockGame_4b4e55ef:
    # kat "Now, why don't you go ahead and wait at the bar? While you're there, you can give Miss Lundgren company."
    kat ""

translate id w3ShockGame_60cae9c5_3:
    # mc "Yes, Ma'am."
    mc ""

translate id w3ShockGame_9d06c1e7:
    # kat "Good boy."
    kat ""

translate id w3ShockGame_aa54fa83:
    # "Even here, the worst part of any job is standing around waiting."
    ""

translate id w3ShockGame_0fd4d656_5:
    # "......"
    ""

translate id w3ShockGame_a20cefa7_4:
    # "..."
    ""

translate id w3SophiaBarRendezvous_c0be0f77:
    # "The moment I entered the bar, Sophia's eyes were upon me."
    ""

translate id w3SophiaBarRendezvous_a498ffe9:
    # sophia "Say... what do you have to do to get some service in this place?"
    sophia ""

translate id w3SophiaBarRendezvous_fb11d80a:
    # mc "Be the change you want to see in the world. Pour me a glass of water, babe."
    mc ""

translate id w3SophiaBarRendezvous_f0765d63:
    # sophia "...was that a joke?"
    sophia ""

translate id w3SophiaBarRendezvous_a824894b:
    # mc "...it was a joke, yes."
    mc ""

translate id w3SophiaBarRendezvous_cf2ea2fa:
    # sophia "Hmmm..."
    sophia ""

translate id w3SophiaBarRendezvous_5cc04389:
    # sophia "It wasn't very humorous."
    sophia ""

translate id w3SophiaBarRendezvous_044e73f1:
    # mc "No, it really wasn't. You got any funny--"
    mc ""

translate id w3SophiaBarRendezvous_d303759c:
    # sophia "......"
    sophia ""

translate id w3SophiaBarRendezvous_8b848749:
    # sophia "...okay, stop me if you've heard this one."
    sophia ""

translate id w3SophiaBarRendezvous_29b6af41:
    # "Well, {b}shit{/b}. I didn't expect that answer, especially not from her."
    ""

translate id w3SophiaBarRendezvous_6d67715b:
    # mc "S'alright..."
    mc ""

translate id w3SophiaBarRendezvous_e36b25f2:
    # "That's the first time anyone's ever called my bluff on that line..."
    ""

translate id w3SophiaBarRendezvous_bded67a9:
    # mct "(This should be good.)"
    mct ""

translate id w3SophiaBarRendezvous_5f2af733:
    # sophia "A man walks into a bar and sees two large pieces of meat hanging from the ceiling. Naturally he's confused."
    sophia ""

translate id w3SophiaBarRendezvous_edc60f0e:
    # sophia "So he asks the bartender, pointedly, {i}what the hell is with the meat, Jim?{/i} - ah, Jim... that's the bartender's name by the way."
    sophia ""

translate id w3SophiaBarRendezvous_d99d8866:
    # mc "I... {b}got it{/b}."
    mc ""

translate id w3SophiaBarRendezvous_e2c383c5:
    # sophia "The bartender tells him {i}it's a game I like to play. I bet everyone who comes in here $20 they can't pull one of them down. Care to try?{/i}"
    sophia ""

translate id w3SophiaBarRendezvous_e877f10a:
    # sophia "The man pauses a moment, giving it some serious thought, before finally answering..."
    sophia ""

translate id w3SophiaBarRendezvous_35c236f4:
    # sophia "{b}No.{/b} The {i}STEAKS{/i} are too high."
    sophia ""

translate id w3SophiaBarRendezvous_dddb6f7e:
    # mc "........."
    mc ""

translate id w3SophiaBarRendezvous_3b21f354:
    # mc "......"
    mc ""

translate id w3SophiaBarRendezvous_97389b90:
    # "She looked at me expectantly, confidently awaiting a laugh."
    ""

translate id w3SophiaBarRendezvous_c9dada07:
    # mc "...oh, yeah. I get it."
    mc ""

translate id w3SophiaBarRendezvous_10ec15af:
    # sophia "Your sense of humor must be off. Abel chuckled at that one."
    sophia ""

translate id w3SophiaBarRendezvous_85e80ebb:
    # mc "Did he...?"
    mc ""

translate id w3SophiaBarRendezvous_d68edbaa:
    # mc "Well, never tell a pun to a kleptomaniac.... they're always taking things {i}literally.{/i}"
    mc ""

translate id w3SophiaBarRendezvous_cf2ea2fa_1:
    # sophia "Hmmm..."
    sophia ""

translate id w3SophiaBarRendezvous_1e6d5a1f:
    # sophia "...that's a pretty good one."
    sophia ""

translate id w3SophiaBarRendezvous_0f7ea136:
    # mct "(No, it fucking wasn't...)"
    mct ""

translate id w3SophiaBarRendezvous_5b6962c2:
    # mc "Feel free to use it on Dr. Van Doren if you want."
    mc ""

translate id w3SophiaBarRendezvous_7a849299:
    # "Ah... this was a bizarre way to start a conversation..."
    ""

translate id w3SophiaBarRendezvous_ac334ff6:
    # sophia "Let's talk on the couch."
    sophia ""

translate id w3SophiaBarRendezvous_ea28aaa3:
    # mc "I... no, thank you. I'd like to leave room for Jesus, if you know what I mean."
    mc ""

translate id w3SophiaBarRendezvous_60460967:
    # sophia "Are you always this reserved around women?"
    sophia ""

translate id w3SophiaBarRendezvous_3208d57b:
    # mc "Only the women prone to chemically dosing me into a stupor. Do you always wear that perfume wherever you go?"
    mc ""

translate id w3SophiaBarRendezvous_d8eeaaf7:
    # sophia "Only sometimes. Just whenever it doesn't represent a problem."
    sophia ""

translate id w3SophiaBarRendezvous_7632437d:
    # mc "What is it, field testing?"
    mc ""

translate id w3SophiaBarRendezvous_1678d5d8:
    # sophia "Oh, no. That would be... {b}absurd.{/b}"
    sophia ""

translate id w3SophiaBarRendezvous_bb621fa3:
    # sophia "Chemistry is a discipline. It's rigid and controlled; this is..."
    sophia ""

translate id w3SophiaBarRendezvous_d6e7574f:
    # sophia "Just a bit of chaos."
    sophia ""

translate id w3SophiaBarRendezvous_90dcf277:
    # sophia "{b}Come.{/b} Sit with me on the couch. I know you have some time to kill."
    sophia ""

translate id w3SophiaBarRendezvous_a056611e:
    # mc "Ah, shit..."
    mc ""

translate id w3SophiaBarRendezvous_9f5a84f4:
    # "Guess the reason Mrs. Pulman pointed me this way for the interim was to babysit..."
    ""

translate id w3SophiaBarRendezvous_12e12b92:
    # "I could already feel Sophia's \"just a bit of chaos\" work its way into my system, my skin growing flush as the already captivating beauty and her tendered hand made the turn to outright magnetic."
    ""

translate id w3SophiaBarRendezvous_e4dd88a1:
    # mc "Alright. Turning you down twice would be rude."
    mc ""

translate id w3SophiaBarRendezvous_0f0f93ca:
    # "Part of my job was to kiss up to the club's constituents, and silver lining, this would at least be an exercise in control. I was positive this wouldn't be my last time around Sophia's bewitching concoction."
    ""

translate id w3SophiaBarRendezvous_c2ae1cd8:
    # sophia "Ah!"
    sophia ""

translate id w3SophiaBarRendezvous_dc941ee7:
    # sophia "--{i}to make enemies by unnecessary and willful incivility is just as insane a proceeding as to set your house on fire. For politeness is like a counter--an avowedly false coin, with which it is foolish to be stingy.{/i}"
    sophia ""

translate id w3SophiaBarRendezvous_1a43f8b7:
    # mc "Huh, no... I wasn't... uh, who said that?"
    mc ""

translate id w3SophiaBarRendezvous_e53f4604:
    # sophia "I... don't know who said it originally, but it's a maxim imparted to me by Abel."
    sophia ""

translate id w3SophiaBarRendezvous_040f6411:
    # mc "I see... well, for the record... is everything you do Machiavellian?"
    mc ""

translate id w3SophiaBarRendezvous_5816132f:
    # sophia "--{i}a prince must want to have a reputation for compassion rather than for cruelty: nonetheless, he must be careful that he does not make bad use of compassion.{/i}"
    sophia ""

translate id w3SophiaBarRendezvous_b6ec8446:
    # mc "I'm... starting to think I should turn you down, Miss Lundgren."
    mc ""

translate id w3SophiaBarRendezvous_dbf2d94a:
    # sophia "No taksie backsies, [mcf]."
    sophia ""

translate id w3SophiaBarRendezvous_e31f68d3:
    # "I was fast gathering that Sophia was an odd woman, but maybe that was just the mark of a genius."
    ""

translate id w3SophiaBarRendezvous_45b9c313:
    # mc "So, you must be an amazing chemist."
    mc ""

translate id w3SophiaBarRendezvous_09a13412:
    # "...and even if it wasn't, \"odd\" was just as quickly losing meaning as a qualifier within these walls."
    ""

translate id w3SophiaBarRendezvous_f5c45dd1:
    # mc "Sure. Let's sit down."
    mc ""

translate id w3SophiaBarRendezvous_4dd83f3d:
    # "Schmoozing the club's constituents was part of the job and one I accepted, but the way my body rebelled under the effects of Sophia's bewitching concoction simply left a bad taste in my mouth. "
    ""

translate id w3SophiaBarRendezvous_bd576da2:
    # sophia "..."
    sophia ""

translate id w3SophiaBarRendezvous_e9dbaa2b:
    # "Still, this woman was a certified genius. It wouldn't be such bad company - ah, gah... I couldn't even be sure if those were my own thoughts!"
    ""

translate id w3SophiaBarRendezvous_45b9c313_1:
    # mc "So, you must be an amazing chemist."
    mc ""

translate id w3SophiaBarRendezvous_24fa8f9f:
    # "The quality of her company withstanding, I trusted that aspect of my thoughts."
    ""

translate id w3SophiaBarRendezvous_2c035743:
    # sophia "No, not at all. Amazing is a strong word."
    sophia ""

translate id w3SophiaBarRendezvous_6dbc6507:
    # mc "How old are you and don't give me never ask a woman her--"
    mc ""

translate id w3SophiaBarRendezvous_7fe7725b:
    # sophia "Thirty-one."
    sophia ""

translate id w3SophiaBarRendezvous_1896094d:
    # mc "Considering your position at Waller Scientific, that is amazing."
    mc ""

translate id w3SophiaBarRendezvous_3b190e38:
    # sophia "No, no... a man like Herr Krüger was worthy of being called an amazing chemist. Me...?"
    sophia ""

translate id w3SophiaBarRendezvous_1a2b8f97:
    # "Again she refuted me. She must be incredibly humbl--"
    ""

translate id w3SophiaBarRendezvous_05ffc5d1:
    # sophia "I am... {i}simply{/i} the best."
    sophia ""

translate id w3SophiaBarRendezvous_708d50cb:
    # "{b}Nevermind.{/b}"
    ""

translate id w3SophiaBarRendezvous_c708819a:
    # mc "...that you know of?"
    mc ""

translate id w3SophiaBarRendezvous_85de87c3:
    # sophia "I was precise with my words."
    sophia ""

translate id w3SophiaBarRendezvous_568687c3:
    # "The oddest thing was I detected not a single iota of pride or hubris in that statement. It was as if Sophia was telling me the sky was blue."
    ""

translate id w3SophiaBarRendezvous_dca5868c:
    # mc "Herr Krüger was a mentor, I presume?"
    mc ""

translate id w3SophiaBarRendezvous_ba30e819:
    # sophia "Among many other things."
    sophia ""

translate id w3SophiaBarRendezvous_9ec5b23c:
    # mc "...and did you always want to be a chemist?"
    mc ""

translate id w3SophiaBarRendezvous_b0a18ef1:
    # sophia "Are you conducting an interview, Mr. [mcl]?"
    sophia ""

translate id w3SophiaBarRendezvous_27da027b:
    # mc "Just call me [mcf]. Please. {b}Seriously.{/b?}"
    mc ""

translate id w3SophiaBarRendezvous_c0e83981:
    # sophia "Okay, [mcf] please seriously - {b}you are asking a lot of questions back-to-back.{/b}."
    sophia ""

translate id w3SophiaBarRendezvous_51b78708:
    # mc "Hey...! Maybe I'm just a shitty conversationalist, okay?"
    mc ""

translate id w3SophiaBarRendezvous_0fd4d656:
    # "......"
    ""

translate id w3SophiaBarRendezvous_a20cefa7:
    # "..."
    ""

translate id w3SophiaBarRendezvous_37cf6a17:
    # mc "*Ahem* We're making small talk, so I won't promise that will be the last one you'll hear. You are a fascinating person."
    mc ""

translate id w3SophiaBarRendezvous_30366d92:
    # sophia "Uh huh, I bet..."
    sophia ""

translate id w3SophiaBarRendezvous_cfacaab9:
    # mc "*Ahem* I was bidden to make small talk, so I can't promise that will be the last one you'll hear."
    mc ""

translate id w3SophiaBarRendezvous_3c1e1c1c:
    # sophia "Are you feeling okay? Your eyes look a bit distant."
    sophia ""

translate id w3SophiaBarRendezvous_13ab2508:
    # mc "You're... {i}very{/i} attractive."
    mc ""

translate id w3SophiaBarRendezvous_72fc48ef:
    # "Perhaps alarmingly, I said that all too easily."
    ""

translate id w3SophiaBarRendezvous_8b102c7f:
    # sophia "What do you like better? My thighs or my stomach?"
    sophia ""

translate id w3SophiaBarRendezvous_057ea832:
    # mc "Why... those two?"
    mc ""

translate id w3SophiaBarRendezvous_f9b46ae5:
    # "A better question might've been why she was even asking, but in the moment, questioning the question behind the question felt {i}laborious{/i}."
    ""

translate id w3SophiaBarRendezvous_1c6bc1c5:
    # sophia "Following a man's eye line is an essential skill for any woman, especially one in a position like mine."
    sophia ""

translate id w3SophiaBarRendezvous_28b29092:
    # sophia "Your glances are... not furtive."
    sophia ""

translate id w3SophiaBarRendezvous_ef17b704:
    # mc "I imagine they wouldn't, considering..."
    mc ""

translate id w3SophiaBarRendezvous_a0c95d47:
    # mc "Well, no shit, I imagine they wouldn't be. Considering..."
    mc ""

translate id w3SophiaBarRendezvous_af4c7881:
    # mct "(Yeah...)"
    mct ""

translate id w3SophiaBarRendezvous_e5329e3f:
    # sophia "Now, answer my question. It's valuable information."
    sophia ""

translate id w3SophiaBarRendezvous_855167ff:
    # mc "Answer my question first."
    mc ""

translate id w3SophiaBarRendezvous_81876ea8:
    # sophia "...what did you ask? Did I {i}want{/i} to be a chemist?"
    sophia ""

translate id w3SophiaBarRendezvous_6dc4da1e:
    # mc "The emphasis was supposed to be on {i}always.{/i}"
    mc ""

translate id w3SophiaBarRendezvous_301acf7f:
    # sophia "Asking me that is akin to me asking if you wanted to be born a human being. I AM a chemist."
    sophia ""

translate id w3SophiaBarRendezvous_32399264:
    # mc "Aha! I like it! That's intense!"
    mc ""

translate id w3SophiaBarRendezvous_04a17bb8:
    # "That... came out WAY stronger than I meant, but those were also my true feelings."
    ""

translate id w3SophiaBarRendezvous_36170c03:
    # mc "You're saying you feel like you were born to do what you do. That kind of certainty is admirable."
    mc ""

translate id w3SophiaBarRendezvous_32629f34:
    # mc "You're saying you feel like you were born to be a chemist? That kind of certainty is admirable."
    mc ""

translate id w3SophiaBarRendezvous_cd78d791:
    # sophia "Ah, that's not quite... is it admirable to be born tall?"
    sophia ""

translate id w3SophiaBarRendezvous_432475a5:
    # mc "You're taking your metaphor a little too seriously. I imagine you've worked hard to land where you're at."
    mc ""

translate id w3SophiaBarRendezvous_46bfba5d:
    # mc "There must be many people who have discounted you because of your age and gender. You're entitled to act like you kick ass."
    mc ""

translate id w3SophiaBarRendezvous_27cd1ff6:
    # mc "There must be many people who have discounted you because of your age and gender. Is false humility a product of that?"
    mc ""

translate id w3SophiaBarRendezvous_d303759c_1:
    # sophia "......"
    sophia ""

translate id w3SophiaBarRendezvous_4d41e2ab:
    # sophia "...chemistry, in the loosest sense, is about systematically isolating inputs in a concerted effort to achieve a desired range of outputs."
    sophia ""

translate id w3SophiaBarRendezvous_689d2b5d:
    # sophia "...in a lot of the same way, with efficacious foresight and the proper guidance, personal want becomes a dependent variable that can be controlled."
    sophia ""

translate id w3SophiaBarRendezvous_39d311f3:
    # mc "...wait, you {i}literally{/i} think of yourself as an effect, not the cause?"
    mc ""

translate id w3SophiaBarRendezvous_b2cada2b:
    # "Not that I didn't sympathize with or understand that line of thinking or find it seductive, for that matter, but..."
    ""

translate id w3SophiaBarRendezvous_27b67a4d:
    # sophia "We have gone down a rabbit hole I didn't remotely intend."
    sophia ""

translate id w3SophiaBarRendezvous_ab4c74d3:
    # "Tossing away a sense of culpability in your life, even if it's ultimately an illusion, felt tantamount to admitting defeat. Defeat over {b}what{/b} I wasn't entirely sure, but it was a dreadful feeling that I emphatically rejected."
    ""

translate id w3SophiaBarRendezvous_24c896a4:
    # sophia "But what I've gathered from this is... you're a man who highly values certainty?"
    sophia ""

translate id w3SophiaBarRendezvous_1bf8a71d:
    # mc "Well... kinda. I know that certainty is a pipe dream, but being sure of yourself is fundamental to every aspect of life."
    mc ""

translate id w3SophiaBarRendezvous_9a68f0e1:
    # mc "Everything you do and every choice you make is built on the foundation of your own self-image. So maybe when I called it admirable, I should've used the word... enviable?"
    mc ""

translate id w3SophiaBarRendezvous_8f7a7eac:
    # "The awkwardness of our initial words had quickly collapsed, and I was freely professing my thoughts almost unfiltered. Leaving me with the obvious question of {i}why{/i} did I find Sophia so easy to talk to?"
    ""

translate id w3SophiaBarRendezvous_f1a077d4:
    # sophia "Self-image, huh...?"
    sophia ""

translate id w3SophiaBarRendezvous_40e06fce:
    # "Sophia closes her eyes, letting herself momentarily get washed away in thought."
    ""

translate id w3SophiaBarRendezvous_97fe3c40:
    # sophia "Sounds fickle and prone to change."
    sophia ""

translate id w3SophiaBarRendezvous_d9ed29ad:
    # mc "It totally is, but... ah... heh. Actually, how did we even end up talking about this again?"
    mc ""

translate id w3SophiaBarRendezvous_486eb085:
    # sophia "You lobbed an unfunny, sexist joke in my lap."
    sophia ""

translate id w3SophiaBarRendezvous_6aca458d:
    # mc "Oh, {b}right{/b}..."
    mc ""

translate id w3SophiaBarRendezvous_73b3d9a0:
    # sophia "Men should find their self-worth in ideas and a purpose outside of themselves."
    sophia ""

translate id w3SophiaBarRendezvous_d0fe5de8:
    # mc "Is it another one of Dr. Van Doren's maxims?"
    mc ""

translate id w3SophiaBarRendezvous_df4a8df7:
    # sophia "It is something I've learned myself."
    sophia ""

translate id w3SophiaBarRendezvous_f22bda27:
    # mc "I don't disagree, but..."
    mc ""

translate id w3SophiaBarRendezvous_f22bda27_1:
    # mc "I don't disagree, but..."
    mc ""

translate id w3SophiaBarRendezvous_54c30040:
    # "For no rhyme or reason, the lush color of Sophia's lips and the cavernous suckhole that lay beyond their plump borders diverted my train of thought."
    ""

translate id w3SophiaBarRendezvous_afbfe913:
    # mct "(Ah... focus, you dumb animal.)"
    mct ""

translate id w3SophiaBarRendezvous_a4512649:
    # sophia "You don't disagree, buuuuut...?"
    sophia ""

translate id w3SophiaBarRendezvous_08570ef2:
    # mc "...I don't disagree, but y-you got to know yourself to know which ideas are worthwhile, right?"
    mc ""

translate id w3SophiaBarRendezvous_5c054ca0:
    # sophia "Hmmm... not really?"
    sophia ""

translate id w3SophiaBarRendezvous_5442515b:
    # sophia "In my experience, the most worthwhile of purposes finds you."
    sophia ""

translate id w3SophiaBarRendezvous_453f3fbe:
    # "Her voice dropped to a sultry octave, sending a shiver down my spine and wresting my attention from the hypnotizing curl of her cockholster and back on the devil herself."
    ""

translate id w3SophiaBarRendezvous_8f43023a:
    # mc "To be frank, I'm not sure what we're talking about anymore."
    mc ""

translate id w3SophiaBarRendezvous_def9bc8a:
    # sophia "Neither do I. We're just kinda... yapping and killing time, are we not?"
    sophia ""

translate id w3SophiaBarRendezvous_db7174fe:
    # mc "You're... waiting too?"
    mc ""

translate id w3SophiaBarRendezvous_d52f2512:
    # sophia "Do you think I'm just fucking around in an empty bar?"
    sophia ""

translate id w3SophiaBarRendezvous_6b0ed824:
    # mc "Um..."
    mc ""

translate id w3SophiaBarRendezvous_0fd4d656_1:
    # "......"
    ""

translate id w3SophiaBarRendezvous_4fb1e767:
    # mc "...yeeeeees?"
    mc ""

translate id w3SophiaBarRendezvous_278ee85b:
    # sophia "Remember when I said I knew you had time to kill? Well, that's because I also have time to kill, for the same reason as you."
    sophia ""

translate id w3SophiaBarRendezvous_1dd30f36:
    # sophia "I'll be sitting alongside you for Kathleen's little farce."
    sophia ""

translate id w3SophiaBarRendezvous_c78e92fd:
    # sophia "Let's get along, Mr. [mcl]."
    sophia ""

translate id w3SophiaBarRendezvous_2db55e0e:
    # mc "Back to the mister shit, are we?"
    mc ""

translate id w3SophiaBarRendezvous_a8eab06b:
    # sophia "By the way, you owe me an answer."
    sophia ""

translate id w3SophiaBarRendezvous_e3359900:
    # mc "To what?"
    mc ""

translate id w3SophiaBarRendezvous_02b3a0f0:
    # sophia "My stomach or my thighs?"
    sophia ""

translate id w3SophiaBarRendezvous_4b6f3d76:
    # mc "Are you...?"
    mc ""

translate id w3SophiaBarRendezvous_a26a41c3:
    # "She {b}was.{/b}"
    ""

translate id w3SophiaBarRendezvous_7ccd2b41:
    # mc "Fine. Your stomach... you could balance a quarter on it."
    mc ""

translate id w3SophiaBarRendezvous_0f8194dc:
    # sophia "Noted, thank you -- but, uh... why did you pick a quarter of all things?"
    sophia ""

translate id w3SophiaBarRendezvous_e5f3fe63:
    # mc "That wasn't really the point. I'm saying you have an abdomen that I want to eat soup out-- {b}d-damn it{/b}, why can I only think of object-based compliments right now?"
    mc ""

translate id w3SophiaBarRendezvous_5484843e:
    # sophia "Heh~ hehe."
    sophia ""

translate id w3SophiaBarRendezvous_7eac266f:
    # mc "By the way, how is that \"useful\" information?"
    mc ""

translate id w3SophiaBarRendezvous_a377dc66:
    # sophia "It's not really. I was just bored."
    sophia ""

translate id w3SophiaBarRendezvous_82fd2fcb:
    # sophia "Can I offer you a drink?"
    sophia ""

translate id w3SophiaBarRendezvous_4b6f3d76_1:
    # mc "Are you...?"
    mc ""

translate id w3SophiaBarRendezvous_49ef4942:
    # sophia "Show me where this \"velvet room\" is, please."
    sophia ""

translate id w3SophiaBarRendezvous_e25fcd48:
    # "She {b}wasn't{/b}, but her swinging hips held my attention hostage as we cut through the halls to yet another one of Mrs. Pulman's games."
    ""

translate id w3SophiaBarRendezvous_0fd4d656_2:
    # "......"
    ""

translate id w3SophiaBarRendezvous_a20cefa7_1:
    # "..."
    ""

translate id w3SophiaBarRendezvous_14c788cc:
    # mc "Fine. Your thighs."
    mc ""

translate id w3SophiaBarRendezvous_f9c2fd0d:
    # mc "They're like a horizon that entices you to journey to what's beyond."
    mc ""

translate id w3SophiaBarRendezvous_96562aa8:
    # sophia "Noted. Thank you."
    sophia ""

translate id w3SophiaBarRendezvous_7eac266f_1:
    # mc "By the way, how is that \"useful\" information?"
    mc ""

translate id w3SophiaBarRendezvous_a377dc66_1:
    # sophia "It's not really. I was just bored."
    sophia ""

translate id w3SophiaBarRendezvous_155e6613:
    # sophia "...but would you like to take a glimpse beyond the horizon?"
    sophia ""

translate id w3SophiaBarRendezvous_4b6f3d76_2:
    # mc "Are you...?"
    mc ""

translate id w3SophiaBarRendezvous_49ef4942_1:
    # sophia "Show me where this \"velvet room\" is, please."
    sophia ""

translate id w3SophiaBarRendezvous_133a6cc3:
    # "She {b}wasn't{/b}, but her swinging hips held attention hostage as we cut through the halls to yet another one of Mrs. Pulman's games."
    ""

translate id w3SophiaBarRendezvous_0fd4d656_3:
    # "......"
    ""

translate id w3SophiaBarRendezvous_a20cefa7_2:
    # "..."
    ""

translate id w3SophiaBarRendezvous_9c5728c8:
    # mc "You know, I don't feel comfortable saying. I have a girlfriend."
    mc ""

translate id w3SophiaBarRendezvous_2f2d0420:
    # "That... felt weird to say. How long had I been single...?"
    ""

translate id w3SophiaBarRendezvous_edb8ca96:
    # sophia "Oh? That's, uh... is that a joke?"
    sophia ""

translate id w3SophiaBarRendezvous_c02ec249:
    # "I didn't actually want to count."
    ""

translate id w3SophiaBarRendezvous_42e8ec08:
    # mc "It actually isn't."
    mc ""

translate id w3SophiaBarRendezvous_ce1ac1ee:
    # sophia "Too bad. That would've been the first humorous thing you said."
    sophia ""

translate id w3SophiaBarRendezvous_de1eb058:
    # mc "Fuuuuuck yoooou."
    mc ""

translate id w3SophiaBarRendezvous_d9034036:
    # sophia "Well, We could. We may have some time before Kathleen finishes setting up."
    sophia ""

translate id w3SophiaBarRendezvous_4b6f3d76_3:
    # mc "Are you...?"
    mc ""

translate id w3SophiaBarRendezvous_49ef4942_2:
    # sophia "Show me where this \"velvet room\" is, please."
    sophia ""

translate id w3SophiaBarRendezvous_8d4f79e6:
    # "She mercifully {b}wasn't{/b}. With how turned on I was, I wasn't exactly confident in the upper limits of my self-control..."
    ""

translate id w3SophiaBarRendezvous_ba1c0d38:
    # "Still, her swinging hips held my attention hostage as we cut through the halls to yet another one of Mrs. Pulman's games."
    ""

translate id w3SophiaBarRendezvous_0fd4d656_4:
    # "......"
    ""

translate id w3SophiaBarRendezvous_a20cefa7_3:
    # "..."
    ""

translate id w3Ex2PunishmentGame_a265e8cb:
    # "Once I had shown her to the big, dumb, blue room in question... I seized the chance to put some distance between us."
    ""

translate id w3Ex2PunishmentGame_2c88043a:
    # sophia "I guess she's still...? How long does it take to set things up?"
    sophia ""

translate id w3Ex2PunishmentGame_b1083d55:
    # "Distance that I hoped would place my desire into a waning state, but..."
    ""

translate id w3Ex2PunishmentGame_abf3fe70:
    # sophia "*Sigh* I should've offered my help with more insistence..."
    sophia ""

translate id w3Ex2PunishmentGame_bb29371d:
    # "...instead, it just filled me with a wicked sense of yearning."
    ""

translate id w3Ex2PunishmentGame_23f38636:
    # sophia "Let me ask you something. As I understand it, you haven't worked here for very long, correct?"
    sophia ""

translate id w3Ex2PunishmentGame_3b21f354:
    # mc "......"
    mc ""

translate id w3Ex2PunishmentGame_2b35b6fe:
    # mc "..."
    mc ""

translate id w3Ex2PunishmentGame_8bd7f51d:
    # sophia "I said... YOU HAVEN'T WORKED HERE VERY LONG, have you, Mr. [mcl]?"
    sophia ""

translate id w3Ex2PunishmentGame_3a199d4d:
    # mc "...ah, um... yeah. That's right."
    mc ""

translate id w3Ex2PunishmentGame_ecea4fcb:
    # sophia "What's your take on Kathleen? How do you evaluate her as... {i}a person?{/i}"
    sophia ""

translate id w3Ex2PunishmentGame_dfa57a95:
    # mc "As a person...?"
    mc ""

translate id w3Ex2PunishmentGame_6591d262:
    # mct "(She's...)"
    mct ""

translate id w3Ex2PunishmentGame_98c7aaba:
    # "*Clack, clack, clack...*"
    ""

translate id w3Ex2PunishmentGame_e43f6087:
    # sophia "Forget that thought."
    sophia ""

translate id w3Ex2PunishmentGame_85f2bb58:
    # kat "You're here? I told you I'd come and get you at the bar when things had been fully prepared."
    kat ""

translate id w3Ex2PunishmentGame_eae6978e:
    # sophia "I was getting impatient, so I asked [mcf] to show me here."
    sophia ""

translate id w3Ex2PunishmentGame_07437a9c:
    # sophia "I don't understand why we can't watch the proceedings in person."
    sophia ""

translate id w3Ex2PunishmentGame_cf9e625e:
    # kat "I've explained it already; that's the magic to--"
    kat ""

translate id w3Ex2PunishmentGame_48d1c557:
    # mc "Where are the Carnations?"
    mc ""

translate id w3Ex2PunishmentGame_3578b401:
    # kat "In the obsidian room, hooked up and... {i}growing antsy{/} themselves. You see..."
    kat ""

translate id w3Ex2PunishmentGame_cf2bd653:
    # kat "Mrs. Ford's punishment for coming in last will lie in the hands of her competitors."
    kat ""

translate id w3Ex2PunishmentGame_ee4ea68e:
    # kat "...and as I've already explained to Miss Lundgren, we will review the scene remotely so that Rosalind and Veronica might act with more... {i}impunity.{/i}"
    kat ""

translate id w3Ex2PunishmentGame_b5f4adf9:
    # kat "Miss Lynch's punishment for coming in last will lie in the hands of her competitors."
    kat ""

translate id w3Ex2PunishmentGame_de6b314a:
    # kat "...and as I've already explained to Miss Lundgren, we will review the scene remotely so that Rosalind and Felicia might act with more... {i}impunity.{/i}"
    kat ""

translate id w3Ex2PunishmentGame_bdd3ed8a:
    # kat "Miss Lynch and Mrs. Ford's punishment for tying for last place will lie in the hands of sweet little Rosalind."
    kat ""

translate id w3Ex2PunishmentGame_a5f60b5a:
    # kat "...and as I've already explained to Miss Lundgren, we will review the scene remotely so that she might act with more... {i}impunity.{/i}"
    kat ""

translate id w3Ex2PunishmentGame_010c63e6:
    # kat "Those sluts' punishment over tying for last place lies in their own hands - well, more specifically, Miss Lynch drew lots to take a more \"active\" role... but she won't have an easy time with it."
    kat ""

translate id w3Ex2PunishmentGame_a5f60b5a_1:
    # kat "...and as I've already explained to Miss Lundgren, we will review the scene remotely so that she might act with more... {i}impunity.{/i}"
    kat ""

translate id w3Ex2PunishmentGame_85cc8bb0:
    # kat "A sense of privacy will let them play off each other more naturally, and more importantly, feel a greater responsibility and animosity for what transpires."
    kat ""

translate id w3Ex2PunishmentGame_493cd948:
    # sophia "They {i}are{/i} aware they're being watched?"
    sophia ""

translate id w3Ex2PunishmentGame_07f0ed9f:
    # kat "Of course. The girls aren't that stupid."
    kat ""

translate id w3Ex2PunishmentGame_4f44252b:
    # sophia "Then isn't it the same if I were to observe closely?"
    sophia ""

translate id w3Ex2PunishmentGame_5bbaf5ff:
    # kat "Trust me on this one. Just removing a physical presence can have a powerful effect on a person's psychology, camera or no camera."
    kat ""

translate id w3Ex2PunishmentGame_3ba67f47:
    # kat "I know you don't care about the artistry of this, but you {b}will{/b} observe a more potent result this way."
    kat ""

translate id w3Ex2PunishmentGame_bd576da2:
    # sophia "..."
    sophia ""

translate id w3Ex2PunishmentGame_1507af1f:
    # kat "Sit down. {i}Kick your feet up.{/i}"
    kat ""

translate id w3Ex2PunishmentGame_2ad5b373:
    # kat "Perhaps [mcf] can go fetch you a drink while you wait for the show to begin?"
    kat ""

translate id w3Ex2PunishmentGame_5375e6ee:
    # sophia "No, thank you."
    sophia ""

translate id w3Ex2PunishmentGame_daebd2a4:
    # mc "Uh... am I even needed for this?"
    mc ""

translate id w3Ex2PunishmentGame_7146cf68:
    # mct "(If you don't mind, I'd like to find a toilet stall to beat off in...)"
    mct ""

translate id w3Ex2PunishmentGame_735a70c4:
    # kat "What a silly question. How could you fulfill your role as moral support if you don't know what the girls experience?"
    kat ""

translate id w3Ex2PunishmentGame_219b8abd:
    # kat "Besides, two is better than being alone in that woman's company."
    kat ""

translate id w3Ex2PunishmentGame_1044f5df:
    # mc "I... *ahem*... {b}got it{/b}."
    mc ""

translate id w3Ex2PunishmentGame_b19ef3b7:
    # kat "Hmmm? What's the matter, [mcf]?"
    kat ""

translate id w3Ex2PunishmentGame_1f65875f:
    # kat "Are you feeling alright? You're looking flushed."
    kat ""

translate id w3Ex2PunishmentGame_8da4e472:
    # mct "(An exercise in control {b}my ass.{/b})"
    mct ""

translate id w3Ex2PunishmentGame_a24c45c0:
    # mct "This... this is going to take a while, and I'm..."
    mct ""

translate id w3Ex2PunishmentGame_4cc22ed3:
    # "I'm going to die."
    ""

translate id w3PunishGame_298dd111:
    # fel "Ugggh... aawwwahh... ahh..."
    fel ""

translate id w3PunishGame_a8c11204:
    # rose "How long has it been?"
    rose ""

translate id w3PunishGame_1f29bc2e:
    # fel "S-seriously, ug-ugh...!"
    fel ""

translate id w3PunishGame_20743986:
    # ver "About roughly fifteen minutes."
    ver ""

translate id w3PunishGame_0be47a14:
    # rose "{b}Oh.{/b} She said to begin after ten. I guess we should--"
    rose ""

translate id w3PunishGame_10a31d5f:
    # fel "G-goddammit, you bitches! Let me have a drink of that water. That shit she gave me is making me--"
    fel ""

translate id w3PunishGame_c5a479a7:
    # rose "I don't know what to do..."
    rose ""

translate id w3PunishGame_98422834:
    # ver "No, the old bitch sure as shit wasn't explicit... I mean, \"have fun and make it a good show\"...?"
    ver ""

translate id w3PunishGame_0fca1b53:
    # rose "You should start."
    rose ""

translate id w3PunishGame_199fc335:
    # ver "Are you trying to pawn this off on me?"
    ver ""

translate id w3PunishGame_16e4aaea:
    # fel "Unnggg....!"
    fel ""

translate id w3PunishGame_2bd41901:
    # rose "No! Just... I don't really have any experience with this sort of... {i}thing{/i}."
    rose ""

translate id w3PunishGame_a143deb0:
    # ver "And you think I do?"
    ver ""

translate id w3PunishGame_eba878f6:
    # fel "J-just fucking begin...! The sooner you do, the sooner I can--"
    fel ""

translate id w3PunishGame_70c5e774:
    # ver "What about you, Blondie? You've done this sort of thing before, right?"
    ver ""

translate id w3PunishGame_e992b351:
    # ver "What should we do?"
    ver ""

translate id w3PunishGame_bdcb2d25:
    # fel "E-eh? I haven't ever-- unngg, whatever just-- g-grab one of the dildos..."
    fel ""

translate id w3PunishGame_eb478c5d:
    # ver "Alright..."
    ver ""

translate id w3PunishGame_72f3543a:
    # rose "Wait... would that really be {i}tormenting{/i} her?"
    rose ""

translate id w3PunishGame_62847b17:
    # rose "Mrs. Pulman said to punish her..."
    rose ""

translate id w3PunishGame_ea60ecdb:
    # ver "I mean... I don't know? She's strung out of her mind on that aphrodisiac shit. A little touch and go could be pretty mean."
    ver ""

translate id w3PunishGame_abd837cd:
    # rose "..."
    rose ""

translate id w3PunishGame_95629181:
    # ver "What, you got a better idea?"
    ver ""

translate id w3PunishGame_d069b9e9:
    # rose "I know what would work on me..."
    rose ""

translate id w3PunishGame_5662076c:
    # fel "CAN YOU GIVE ME SOME FUCKING WATER FIRST?!"
    fel ""

translate id w3PunishGame_ca8f737d:
    # ver "I just got an idea myself."
    ver ""

translate id w3PunishGame_d314c80c:
    # kat "Ha! By the end of this week, I will have these bitches hating each other."
    kat ""

translate id w3PunishGame_0ec5cdbf:
    # sophia "The blonde is abnormally thirsty..."
    sophia ""

translate id w3PunishGame_2f4a0b39:
    # ver "Unnngg, aaahh..."
    ver ""

translate id w3PunishGame_a1da6f4e:
    # rose "What do you think we should do?"
    rose ""

translate id w3PunishGame_bd6a1526:
    # ver "C-c'mon...! It's--"
    ver ""

translate id w3PunishGame_96e47324:
    # fel "You heard the lady. She told us to have fun, punish Red, and make it a good show."
    fel ""

translate id w3PunishGame_7c0b49ae:
    # rose "Y-yeah, but... uh, you want to start then?"
    rose ""

translate id w3PunishGame_31556a35:
    # ver "It's past time! L-let's get this rolling!"
    ver ""

translate id w3PunishGame_80aa2d94:
    # fel "I could. Lotta stuff to work with over there... just pick and point, right?"
    fel ""

translate id w3PunishGame_09069aa6:
    # rose "A lot of that stuff is scary..."
    rose ""

translate id w3PunishGame_7d9cb252:
    # fel "It all comes down to how you use it. We just got to remember that--"
    fel ""

translate id w3PunishGame_f1225a3a:
    # ver "Haa, hhhaa... unnghh..."
    ver ""

translate id w3PunishGame_d7c8d399:
    # fel "...she's hopped up on that sex drug. We might fry her brain or something if we're not careful."
    fel ""

translate id w3PunishGame_a48856dd:
    # rose "That's not possible, right?"
    rose ""

translate id w3PunishGame_e0851857:
    # fel "Who the hell knows..."
    fel ""

translate id w3PunishGame_fabe88b5:
    # rose "Go ahead, pick something..."
    rose ""

translate id w3PunishGame_d1156700:
    # ver "Unnngggh, f-fuck I'm burning up..."
    ver ""

translate id w3PunishGame_23b4ee36:
    # fel "Oh no. You think you'll be able to just stand around and not get your hands dirty, don't you?"
    fel ""

translate id w3PunishGame_cdadb216:
    # rose "N-no! It's just... I've never done anything like this before. I know you have, right?"
    rose ""

translate id w3PunishGame_8e6beacb:
    # fel "Why the fuck would you think that?"
    fel ""

translate id w3PunishGame_fd387fb7:
    # rose "Well... you know... you're kinda... um... n-nothing."
    rose ""

translate id w3PunishGame_73d4e8ee:
    # fel "......"
    fel ""

translate id w3PunishGame_345d28a6:
    # fel "You make it a habit of pushing your problems on other people, don't you?"
    fel ""

translate id w3PunishGame_5baf5fe2:
    # rose "What are you talking about?"
    rose ""

translate id w3PunishGame_18394760:
    # fel "I'm talking about [mcf] and the 5000 dollars I-- ah, whatever."
    fel ""

translate id w3PunishGame_d6ce9717:
    # fel "..."
    fel ""

translate id w3PunishGame_8ca83968:
    # fel "Point is, you're going to help."
    fel ""

translate id w3PunishGame_80bdc9f6:
    # fel "I have a preeetty good idea what will make her cum unrelentingly, buuuut... you'll take the other end."
    fel ""

translate id w3PunishGame_efc0ed61:
    # rose "...sure. I have some ideas too."
    rose ""

translate id w3PunishGame_76d3ba48:
    # fel "I knew you did, you sneaky bitch."
    fel ""

translate id w3PunishGame_af4d8aa6:
    # rose "Fuck you, Felicia."
    rose ""

translate id w3PunishGame_d314c80c_1:
    # kat "Ha! By the end of this week, I will have these bitches hating each other."
    kat ""

translate id w3PunishGame_bace2a89:
    # sophia "Did you get the dosage right? The redhead only seems mildly affected."
    sophia ""

translate id w3PunishGame_bbcec532:
    # mct "(You call that mild?!)"
    mct ""

translate id w3PunishGame_2b68d81b:
    # kat "Of course, I did. Veronica's just got a stiff upper lip."
    kat ""

translate id w3PunishGame_6ad8d0d0:
    # ver "Hnnng, hhhhaaa..."
    ver ""

translate id w3PunishGame_6f8637e3:
    # fel "C-can you get me some water, Rosie? Please?"
    fel ""

translate id w3PunishGame_08115afb:
    # rose "Mrs. Pulman said not to start for ten minutes."
    rose ""

translate id w3PunishGame_2b858708:
    # fel "It's been ten minutes!"
    fel ""

translate id w3PunishGame_77f29052:
    # rose "Has it?"
    rose ""

translate id w3PunishGame_1cb3064c:
    # fel "Y-yes! S-stop twiddling your thumbs and begin!"
    fel ""

translate id w3PunishGame_2c5de24e:
    # fel "This s-shit she gave us is making me... uugghh..."
    fel ""

translate id w3PunishGame_57b79db5:
    # ver "I a-agree with Blondie. Let's get this going...!"
    ver ""

translate id w3PunishGame_67030bca:
    # rose "Okay..."
    rose ""

translate id w3PunishGame_500ed26f:
    # rose "What should I use?"
    rose ""

translate id w3PunishGame_dce94c65:
    # fel "Huuhgg, like I know!"
    fel ""

translate id w3PunishGame_54676b2c:
    # rose "All she said was to punish you and make it look good... ugh..."
    rose ""

translate id w3PunishGame_20776796:
    # ver "U-use what's on the table... g-grab one of those dildos or something!"
    ver ""

translate id w3PunishGame_e41453c6:
    # rose "...would that be enough?"
    rose ""

translate id w3PunishGame_86fb9230:
    # ver "P-probably!"
    ver ""

translate id w3PunishGame_27f7dea8:
    # fel "What the hell do you mean would that be--"
    fel ""

translate id w3PunishGame_698e3d0d:
    # rose "I mean... I gotta do a good job, right? I need to... get creative."
    rose ""

translate id w3PunishGame_d4c56bdd:
    # ver "Ah- ha... y-yeah, I suppose she expects you to... gg-gah... damn it, my shoulders hurt. Just get started! Please, Rosie?"
    ver ""

translate id w3PunishGame_d65df59a:
    # rose "......"
    rose ""

translate id w3PunishGame_abd837cd_1:
    # rose "..."
    rose ""

translate id w3PunishGame_a9f4a6f7:
    # rose "Don't bear a grudge over this, okay?"
    rose ""

translate id w3PunishGame_d314c80c_2:
    # kat "Ha! By the end of this week, I will have these bitches hating each other."
    kat ""

translate id w3PunishGame_786b71bd:
    # sophia "The blonde seems abnormally thirsty and the redhead only mildly affected."
    sophia ""

translate id w3PunishGame_bbcec532_1:
    # mct "(You call that mild?!)"
    mct ""

translate id w3PunishGame_108686e2:
    # sophia "Did you get the dosage right?"
    sophia ""

translate id w3PunishGame_a7205a1d:
    # kat "Of course, I did. She's just got a stiff upper lip."
    kat ""

translate id w3PunishGame_82813fe3:
    # rose "Hhhngg..."
    rose ""

translate id w3PunishGame_01da591d:
    # fel "A-ah, fuck I'm thirsty... this stuff is making me... unngg..."
    fel ""

translate id w3PunishGame_3cc244c1:
    # fel "Why the fuck are you just standing there, staring like some perverted cunt?!"
    fel ""

translate id w3PunishGame_59e39bef:
    # ver "Because I'm... a-ah..."
    ver ""

translate id w3PunishGame_0cd0853f:
    # ver "She gave that shit to all of us! Just standing here, I feel like my head's about to melt!"
    ver ""

translate id w3PunishGame_e8010c6c:
    # fel "It's b-better than being locked into this thing! F-fucking...!"
    fel ""

translate id w3PunishGame_7a85d5b1:
    # ver "It's not my fault you drew the short straws! But, honestly, I'd rather be where you are than have to..."
    ver ""

translate id w3PunishGame_8f948a65:
    # rose "...h-have to what?"
    rose ""

translate id w3PunishGame_7dfd5bc3:
    # ver "...h-have to control my urges! That old bitch said to put on a good show and punish you... and the fucking worst thing is..."
    ver ""

translate id w3PunishGame_8e7aca98:
    # ver "That sounds like SUCH a good idea right now! I'm so fuckin' horny... I just want t--"
    ver ""

translate id w3PunishGame_5c7e0b7e:
    # rose "A-hhn, hggg... wh-what?!"
    rose ""

translate id w3PunishGame_c18854a3:
    # ver "You two are gorgeous... no shit, but I've thought that... like always...? Ah... what am I saying...?"
    ver ""

translate id w3PunishGame_eac67469:
    # ver "Even Felicia, even though I think you're a dumb bitch for signing up for this, I can't deny that you're..."
    ver ""

translate id w3PunishGame_31d8c184:
    # fel "Oh, god damn it. Just do something! I feel like all the moisture in my body is oozing out my cunt!"
    fel ""

translate id w3PunishGame_40ae9b4b:
    # rose "Y-yeah... uh... I mean... go e-easy on us, but... you gotta do something, right?"
    rose ""

translate id w3PunishGame_3f20d533:
    # rose "She said to start in ten minutes and, I... uh... it's been more than that, right?"
    rose ""

translate id w3PunishGame_dd590266:
    # ver "I don't know... I have no concept of time right now. I guess so?"
    ver ""

translate id w3PunishGame_3299b087:
    # ver "So... there's a lot of stuff on here."
    ver ""

translate id w3PunishGame_f9ef31ee:
    # ver "What should I..."
    ver ""

translate id w3PunishGame_d314c80c_3:
    # kat "Ha! By the end of this week, I will have these bitches hating each other."
    kat ""

translate id w3PunishGame_a376cdc2:
    # sophia "The redhead seems particularly affected."
    sophia ""

translate id w3PunishGame_998f93cd:
    # kat "Deep down, she's a big pervert... but now a pervert with much less inhibition in the tank, two juicy slabs of meat at her fingertips, and a good excuse."
    kat ""

translate id w3PunishGame_ea37dd63:
    # "Like before, Mrs. Pulman was once again pitting the Carnations against each other... in one sense, I found this was directionless and crude, but the more affected part of me..."
    ""

translate id w3PunishGame_54f122c2:
    # mct "(...\"feel a greater responsibility and animosity for what transpires\", huh?)"
    mct ""

translate id w3PunishGame_66571d08:
    # "--found it dramatic and compelling. The three were already separated by their status as competitors, but now she sought to drive an even greater wedge between them."
    ""

translate id w3PunishGame_55b41f2a:
    # kat "Anticipating the show, [mcf]?"
    kat ""

translate id w3PunishGame_322265cb:
    # mc "What are you...?"
    mc ""

translate id w3PunishGame_1502899f:
    # "The old woman's palm played across the length of my package with a fervor that rivaled even the most drunken letch."
    ""

translate id w3PunishGame_f1e2293d:
    # kat "Or did shocking the girls get you {i}this{/i} hot and bothered?"
    kat ""

translate id w3PunishGame_978ac170:
    # mc "Uunng... d-don't fuck with me."
    mc ""

translate id w3PunishGame_2a972546:
    # "Actually... come to think of it, why doesn't that foul perfume affect either of them? I know for a fact it affects women..."
    ""

translate id w3PunishGame_5a0188ed:
    # "The Carnations were proof enough of that. Was it as simple as them being used to it? Maybe even inoculated against it...?"
    ""

translate id w3PunishGame_7fd2c8f2:
    # mc "Hnngg..."
    mc ""

translate id w3PunishGame_98aa56aa:
    # "...how did I even know it wasn't affecting Mrs. Pulman? After all, her hand was currently rubbing my cock."
    ""

translate id w3PunishGame_a905ea17:
    # "...how did I even know it wasn't affecting Kathleen? After all, her hand was currently rubbing my cock."
    ""

translate id w3PunishGame_65407909:
    # "...but that was kinda in character for her, wasn't it?"
    ""

translate id w3PunishGame_0084e271:
    # sophia "...eh? {b}Heh.{/b}"
    sophia ""

translate id w3PunishGame_f90d36fe:
    # sophia "Him, I could understand, but you..."
    sophia ""

translate id w3PunishGame_e427ad97:
    # kat "{i}Please{/i}, I've seen you in much worse states."
    kat ""

translate id w3PunishGame_9193dc9d:
    # sophia "There's a fundamental difference."
    sophia ""

translate id w3PunishGame_fa1fccca:
    # kat "The difference is I'm my own woman, doing what I please and not what I'm told."
    kat ""

translate id w3PunishGame_1232f43b:
    # "Suddenly I thought... this might not even be about me, huh?"
    ""

translate id w3PunishGame_1e45dd4e:
    # "Nevertheless, even just touching me over my pants offered a soothing proposition of release..."
    ""

translate id w3PunishGame_cfb5a557:
    # mc "Hnnnggg..."
    mc ""

translate id w3PunishGame_112dcea5:
    # "On top of it all, I was also getting extremely pissed..."
    ""

translate id w3PunishGame_10abb4ed:
    # "I felt the only reason she sent me to keep an eye on Sophia was to fuck with me. That was her very nature, to senselessly poke and prod."
    ""

translate id w3PunishGame_7cb6948a:
    # mc "I said, don't fuck with me, you stupid--"
    mc ""

translate id w3PunishGame_659ff2e9:
    # kat "Oh! Looks like the girls are ready to begin!"
    kat ""

translate id w3PunishGame_8b5c16a1:
    # "Mrs. Pulman's hand darted from my crotch and I felt an immediate and {b}massive{/b} pang of regret."
    ""

translate id w3PunishGame_3765f1bb:
    # "Kathleen's hand darted from my crotch and I felt an immediate and {b}massive{/b} pang of regret."
    ""

translate id w3PunishGame_c0e33a80:
    # "By instinct, I roughly grabbed the old woman's wrist."
    ""

translate id w3PunishGame_661b22fd:
    # mc "Put your hand back."
    mc ""

translate id w3PunishGame_5f5bb4eb:
    # kat "Oh...?"
    kat ""

translate id w3PunishGame_92e0209c:
    # "Turns out, my horniness outweighed my indignation at being chemically manipulated."
    ""

translate id w3PunishGame_2098f75a:
    # kat "My, my... you must really be hurt--"
    kat ""

translate id w3PunishGame_68dbe27f:
    # mc "{b}Finish{/b} what you started."
    mc ""

translate id w3PunishGame_8b8196da:
    # kat "......"
    kat ""

translate id w3PunishGame_34ea3c27:
    # kat "..."
    kat ""

translate id w3PunishGame_1bbe9986:
    # kat "Don't look at me. Watch {b}my{/b} show."
    kat ""

translate id w3PunishGame_73caf320:
    # "Her fingers started moving and--"
    ""

translate id w3PunishGame_c86d11a6:
    # "*Ziiiiiiiiip*"
    ""

translate id w3PunishGame_8dfcb4bc:
    # "Although technically {i}inside the club{/i}, I knew Hana would balk if she saw this scene. Still... mitigating circumstances, right?"
    ""

translate id w3PunishGame_1704dabb:
    # "Regret I would live with."
    ""

translate id w3PunishGame_43f723b0:
    # "Somehow and someway, I fought against the violent urge to place the old hag's hand back on my dick."
    ""

translate id w3PunishGame_0f562cf4:
    # "Current plan: grit my teeth, sit here quietly, and then when it's all said and done... go jerk off in a stall or something."
    ""

translate id w3PunishGame_2a18400c:
    # mct "(Yeah... solid plan...)"
    mct ""

translate id w3PunishGame_55b41f2a_1:
    # kat "Anticipating the show, [mcf]?"
    kat ""

translate id w3PunishGame_796b13d8:
    # mc "Ummm... h-huh?"
    mc ""

translate id w3PunishGame_cc517887:
    # kat "Do I have to spell it out?"
    kat ""

translate id w3PunishGame_ad7d6428:
    # "I knew what she meant."
    ""

translate id w3PunishGame_b3eb075e:
    # "Sophia knew what she meant too."
    ""

translate id w3PunishGame_a9341f9c:
    # "Both of them were acutely aware of the angry bulge in my pants before we even sat down and I felt zero shame about it."
    ""

translate id w3PunishGame_9a11636b:
    # kat "What do you think Sophie?"
    kat ""

translate id w3PunishGame_0d974926:
    # "Hell, even if I was in my right mind right now, I still might not feel ashamed about it. By now, well... popping a boner in front of this stupid hag was pretty standard."
    ""

translate id w3PunishGame_d6ca61a9:
    # kat "Pretty impressive for a dim-looking guy, isn't it? You wanna see it up close?"
    kat ""

translate id w3PunishGame_f3c4e5c4:
    # mc "Hnnng... don't fuck with me..."
    mc ""

translate id w3PunishGame_2a972546_1:
    # "Actually... come to think of it, why doesn't that foul perfume affect either of them? I know for a fact it affects women..."
    ""

translate id w3PunishGame_90de623f:
    # sophia "Does he normally talk to you like that?"
    sophia ""

translate id w3PunishGame_fb912398:
    # "The Carnations were proof enough of that, but neither of them seemed all that bothered. Did they hide it well?"
    ""

translate id w3PunishGame_c8263820:
    # kat "I'll let it slide, considering the circumstances."
    kat ""

translate id w3PunishGame_51b7139c:
    # "Was it as simple as them being used to it? Maybe even inoculated against it...?"
    ""

translate id w3PunishGame_f5e0d12e:
    # kat "--seriously though, I could ask him to whip it out and he'd do it."
    kat ""

translate id w3PunishGame_c24d8940:
    # sophia "No, thank you. I'm not like you, Kathleen."
    sophia ""

translate id w3PunishGame_2e326c38:
    # kat "Really? With what I've seen you do at the behest of Abel, all with a smile on your face, well..."
    kat ""

translate id w3PunishGame_66394962:
    # kat "{b}Could've fooled me.{/b}"
    kat ""

translate id w3PunishGame_f7d6b5e3:
    # sophia "There's a difference."
    sophia ""

translate id w3PunishGame_57ddf694:
    # kat "The difference is I'm my own woman, doing what I please and not what I'm to-- oh!"
    kat ""

translate id w3PunishGame_ee88e929:
    # kat "Looks like it's getting started."
    kat ""

translate id w3PunishGame_08cc850b:
    # kat "This should be good. Pay close attention, Mr. [mcl]."
    kat ""

translate id w3PunishGame_510298a8:
    # kat "This should be a good lesson for you."
    kat ""

translate id w3PunishGame_7661ac28:
    # mc "What the hell are you...?"
    mc ""

translate id w3PunishGame_5006aa8c:
    # fel "You know what--"
    fel ""

translate id w3PunishGame_022a3944:
    # "*Glug, glug, guuuuuuuhg...!*"
    ""

translate id w3PunishGame_38d6cddc:
    # fel "A-ah... shiiiiiit...! That's... {b}GOOD{/b}! So fucking... {b}hhhuuuaah{/b}...!"
    fel ""

translate id w3PunishGame_95753d9d:
    # ver "Huh... uh... want some more?"
    ver ""

translate id w3PunishGame_f2a103d1:
    # fel "Hnnng... n-no. N-now my other end is {i}aching{/i} more than my... hhhnngg..."
    fel ""

translate id w3PunishGame_a2e8cb24:
    # fel " C'mon... I've got places to be... s-so can we just get started doing whatever it is you bitches are going to do?"
    fel ""

translate id w3PunishGame_5846be58:
    # ver "Hmm? You sure? There's two other bott--"
    ver ""

translate id w3PunishGame_54c3f506:
    # rose "{b}Sooooo{/b}.... I think I came up with the plan of attack..."
    rose ""

translate id w3PunishGame_ed34b4af:
    # fel "Shit. Y-you're actually taking the initiative for once? Wow, {b}s-shocked{/b}!"
    fel ""

translate id w3PunishGame_3d56f8d9:
    # ver "What are you going to do with that?"
    ver ""

translate id w3PunishGame_12074ab1:
    # rose "Hold this, please."
    rose ""

translate id w3PunishGame_7828b160:
    # ver "...and do what with it?"
    ver ""

translate id w3PunishGame_392f3f93:
    # rose "Just take it."
    rose ""

translate id w3PunishGame_d24cc523:
    # ver "Ooooookay."
    ver ""

translate id w3PunishGame_c89dabb9:
    # rose "I... ah... huh..."
    rose ""

translate id w3PunishGame_f6870e0f:
    # rose "This is all new to me, so it might be a little clumsy, but..."
    rose ""

translate id w3PunishGame_c1f32e4a:
    # "*Chwup..."
    ""

translate id w3PunishGame_0bacc4f8:
    # fel "Eh? What are you..."
    fel ""

translate id w3PunishGame_81cc3c0f:
    # fel "Mmmhh...!"
    fel ""

translate id w3PunishGame_36ba3bc7:
    # ver "Whaa, huh?!"
    ver ""

translate id w3PunishGame_e6a0d07f:
    # fel "Mmmmh, mmmhhh... mmmmhh...!"
    fel ""

translate id w3PunishGame_9ce02c94:
    # "The scene playing out on the television screen was..."
    ""

translate id w3PunishGame_62f678cd:
    # "It was... it was..."
    ""

translate id w3PunishGame_2672ef18:
    # "Kinda... uh..."
    ""

translate id w3PunishGame_6976785e:
    # mct "(Hnnng!!!!)"
    mct ""

translate id w3PunishGame_fb98fce5:
    # kat "Huh. I didn't expect that."
    kat ""

translate id w3PunishGame_12858723:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_c655ba40:
    # kat "Not a bad tactic, however~ mixing affection and punishment does funny things to a person's head."
    kat ""

translate id w3PunishGame_523cb585:
    # "No, in my current state, my mind was entirely consumed by the old woman's grip on my cock."
    ""

translate id w3PunishGame_3a010861:
    # mc "H-haahaa...!"
    mc ""

translate id w3PunishGame_02a50c9a:
    # kat "Doubt that's what the cow has in mind, though."
    kat ""

translate id w3PunishGame_b3fd8974:
    # sophia "Is that why you kissed the redhead the other night?"
    sophia ""

translate id w3PunishGame_8b3d1846:
    # kat "Not at all. That was just the heat of the moment. Veronica really was beautiful, right then and there..."
    kat ""

translate id w3PunishGame_93651fc0:
    # mc "G-gahh...!"
    mc ""

translate id w3PunishGame_22eda143:
    # "The pitiful noise of a rutting animal punctuated the women's conversation."
    ""

translate id w3PunishGame_f2bc21df:
    # kat "Sadly, that moment was quickly lost, and she's now back to being that contemptible creature on the screen. Beauty is fleeting, isn't it?"
    kat ""

translate id w3PunishGame_5ddac938:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_66527633:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious..."
    kat ""

translate id w3PunishGame_5f8d0d83:
    # mc "Huuuhghh...!"
    mc ""

translate id w3PunishGame_a7f739bd:
    # kat "What gets a woman like you going?"
    kat ""

translate id w3PunishGame_16f23753:
    # mc "H-huhh... ack...!"
    mc ""

translate id w3PunishGame_90eda0c7:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_4c9db659:
    # kat "We got that in common at least - {b}all three of us.{/b}"
    kat ""

translate id w3PunishGame_29e481f2:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_fb98fce5_1:
    # kat "Huh. I didn't expect that."
    kat ""

translate id w3PunishGame_12858723_1:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_953926ff:
    # kat "Not a bad tactic, however~ mixing affection and punishment does funny things to a human's head."
    kat ""

translate id w3PunishGame_02a50c9a_1:
    # kat "Doubt that's what the cow has in mind, though."
    kat ""

translate id w3PunishGame_f624db85:
    # mc "She... ah... she definitely didn't."
    mc ""

translate id w3PunishGame_b3fd8974_1:
    # sophia "Is that why you kissed the redhead the other night?"
    sophia ""

translate id w3PunishGame_3d46653f:
    # kat "Not at all. That was just the heat of the moment. She really was beautiful right then and there..."
    kat ""

translate id w3PunishGame_dfeaca01:
    # kat "Sadly, that moment was quickly lost, and she's now back to being that contemptible creature on the screen. Beauty is fleeting, isn't it??"
    kat ""

translate id w3PunishGame_5ddac938_1:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_1c55b166:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious... what gets a woman like you going?"
    kat ""

translate id w3PunishGame_90eda0c7_1:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019_1:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355_1:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_68542e62:
    # kat "Oh, lovely, at least we have that in common."
    kat ""

translate id w3PunishGame_29e481f2_1:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_b30609f6:
    # "*Cwhup, fhwhup, chuuup!*"
    ""

translate id w3PunishGame_6d646bb9:
    # "She was going for it."
    ""

translate id w3PunishGame_f35e88ae:
    # fel "Mmmh... mmmhhh..."
    fel ""

translate id w3PunishGame_b72bcc5a:
    # "She was really, {b}really{/b} going for it."
    ""

translate id w3PunishGame_6eb70cd4:
    # rose "*Chwup, fwhup...!* MMmhh--"
    rose ""

translate id w3PunishGame_284fe66e:
    # fel "Hnng, a-aaah...!"
    fel ""

translate id w3PunishGame_850e3d90:
    # fel "W-wha was... hnnngg..."
    fel ""

translate id w3PunishGame_ef29fd9b:
    # "Felicia and I both were in a fog; I could tell that much. Even a simple, if surprisingly passionate, kiss had upended all the sense in her head."
    ""

translate id w3PunishGame_f63a45a9:
    # ver "What was that?"
    ver ""

translate id w3PunishGame_1b5c882c:
    # rose "Well... ah... you should always begin the foreplay with a kiss, right? It's like a rule..."
    rose ""

translate id w3PunishGame_a4291a47:
    # fel "We're not making love you fat-tittied idiot! Hnngg... gah... even just that... I'm fucking burning up!"
    fel ""

translate id w3PunishGame_a3fdebc1:
    # fel "Stuff that thing in my cunt already!"
    fel ""

translate id w3PunishGame_21500c6e:
    # rose "Sorry, Felicia. That's... NOT happening yet."
    rose ""

translate id w3PunishGame_f910fb0f:
    # fel "...AND WHY THE HELL NOT?!"
    fel ""

translate id w3PunishGame_883cb972:
    # rose "You know why, hun."
    rose ""

translate id w3PunishGame_515a3728:
    # fel "S-so your plan is to just let me hang here?!"
    fel ""

translate id w3PunishGame_488bee5c:
    # rose "No... my plan is..."
    rose ""

translate id w3PunishGame_366b2f5c:
    # "*SWIIICK!*"
    ""

translate id w3PunishGame_89279c84:
    # fel "Hnnggg...! What the--"
    fel ""

translate id w3PunishGame_b04dba26:
    # "*THWAP!"
    ""

translate id w3PunishGame_5c4aa1c7:
    # fel "--t-he fuck!"
    fel ""

translate id w3PunishGame_28be5c36:
    # rose "I've always wondered just {i}how{/i} sensitive other girls are right here... I know what my girlfriends TOLD me, but it's hard to picture it without seeing it yourself..."
    rose ""

translate id w3PunishGame_5b6af53a:
    # rose "Me? I remember running around the school's track in my gym clothes made my chest feel so... uh... {b}yeah{/b}."
    rose ""

translate id w3PunishGame_5a634b6d:
    # fel "What?! You're going to treat my tits like a guinea pig?!"
    fel ""

translate id w3PunishGame_df0e8b7d:
    # rose "I mean, it's not like I want to... it's part of the game."
    rose ""

translate id w3PunishGame_2b96f998:
    # fel "F-fine, whatever, just.... huuuunhh... give me some fucking r-relief from this, a-ahh...!"
    fel ""

translate id w3PunishGame_1efc7ee4:
    # rose "Well, if you're anything like me..."
    rose ""

translate id w3PunishGame_dd9db520:
    # rose "...this might just make you faint."
    rose ""

translate id w3PunishGame_84c5741d:
    # kat "I'm surprised you haven't soiled my hand yet."
    kat ""

translate id w3PunishGame_5bb63c34:
    # mc "Hnngg!"
    mc ""

translate id w3PunishGame_7f926880:
    # kat "I know one time won't suffice. You don't have to fear; I'll milk you as much as you require~"
    kat ""

translate id w3PunishGame_acee149f:
    # "At this point, a stiff breeze would have gotten me off, but for how slow it was, the old woman's technique was thorough and {i}immaculate{/i} in its consistency. She never slowed or sped up; she just jerked me like her wrist operated on a piston."
    ""

translate id w3PunishGame_7a26e7dc:
    # mc "G-ahh, s-shut the fuck up and just k-keep--!"
    mc ""

translate id w3PunishGame_40ab8cb0:
    # "The real magic lay in how she used her pinky, tracing its black nail up and down the underside of my shaft, never deviating a single millimeter from the same tract of overwrought nerve endings."
    ""

translate id w3PunishGame_994fce30:
    # kat "*Whisper* You should speak kindly to someone who's doing you a favor, [mcf]."
    kat ""

translate id w3PunishGame_fa952a65:
    # mc "*Gulp* H-a, haa... favor...? Yeah right..."
    mc ""

translate id w3PunishGame_cd82bac0:
    # "It was like the words appeared on my lips without even being thought of. I said what I felt without control or filter."
    ""

translate id w3PunishGame_9eeb44eb:
    # mc "T-take a seat and spin on it if you really want to do me a favor!"
    mc ""

translate id w3PunishGame_a2ad0a0e:
    # kat "You're adorable right now, you know that?"
    kat ""

translate id w3PunishGame_8086b9e3:
    # fel "U-uh... W-WHAT THE HELL is that for?!"
    fel ""

translate id w3PunishGame_8bc61bf0:
    # kat "Oh! Didn't expect that would even get used."
    kat ""

translate id w3PunishGame_dd9db520_1:
    # rose "...this might just make you faint."
    rose ""

translate id w3PunishGame_40068a53:
    # kat "Well, well, well... this is taking an exciting turn."
    kat ""

translate id w3PunishGame_39be1732:
    # mc "Hhhngg... y-yeah..."
    mc ""

translate id w3PunishGame_c88ce2aa:
    # mct "(I couldn't argue against that...)"
    mct ""

translate id w3PunishGame_56d4a825:
    # "With the way I counted it, I had a couple of options for managing my sickening arousal."
    ""

translate id w3PunishGame_10efc326:
    # "I could let my mind wander, try to think of something else... but invariably, my senses fell on the women next to me, to the curves of their busts and their waists..."
    ""

translate id w3PunishGame_e4fa996b:
    # "These two women were so far above me... the old woman decades my senior. Sophia, a brilliant scientist... it was so easy to imagine the delight in grabbing one of them right now and just--"
    ""

translate id w3PunishGame_2389bacb:
    # mct "(Y-yeah... no... I should focus on what's playing out in front of me...)"
    mct ""

translate id w3PunishGame_3427b534:
    # "The other option was to let Kathleen's game preoccupy my attention."
    ""

translate id w3PunishGame_e5d1e093:
    # kat "A-aah..."
    kat ""

translate id w3PunishGame_20ec7c8b:
    # mct "(...that's gotta be the best way of dealing with this, right?)"
    mct ""

translate id w3PunishGame_ca44b0b9:
    # kat "Do you need to be excused, Mr. [mcl]? Perhaps you need to go to the bathroom?"
    kat ""

translate id w3PunishGame_c8c82b40:
    # mc "N-no, I'm f-fine..."
    mc ""

translate id w3PunishGame_1050a010:
    # "She teased me, and strangely, I felt an unusual rush of pride. Of course, it was irrational, but that would be akin to losing..."
    ""

translate id w3PunishGame_54c72150:
    # "Losing what and to who, I did not know... but my willpower... I was stronger than this..."
    ""

translate id w3PunishGame_041c023d:
    # mct "(I ain't a fucking animal... right?)"
    mct ""

translate id w3PunishGame_8086b9e3_1:
    # fel "U-uh... W-WHAT THE HELL is that for?!"
    fel ""

translate id w3PunishGame_d9f8003b:
    # kat "Oh! A {i}truly{/i} exciting turn."
    kat ""

translate id w3PunishGame_e705ad54:
    # fel "N-not funny, you fucking cunt!"
    fel ""

translate id w3PunishGame_6fd14b03:
    # rose "R-relax, stop wriggling around!"
    rose ""

translate id w3PunishGame_05c1ae61:
    # fel "Like hell I will! Get that thing away from me!"
    fel ""

translate id w3PunishGame_90501b5d:
    # rose "Relax, Sweetie... I'm not planning on burning you... I'm just..."
    rose ""

translate id w3PunishGame_69226807:
    # fel "Hnngg... g-gahh..."
    fel ""

translate id w3PunishGame_5341af55:
    # rose "I read about this sort of thing in a book."
    rose ""

translate id w3PunishGame_d0da0e37:
    # fel "G-gah, wh-what kinda fucking books do you read?!"
    fel ""

translate id w3PunishGame_bceb8490:
    # rose "Uh... it's something another mom recommended?"
    rose ""

translate id w3PunishGame_6724ecad:
    # fel "You're lying, you perverted cow! {b}Hnnngg--{/b} that's f-fucking hot!"
    fel ""

translate id w3PunishGame_e91e87da:
    # rose "Well, that IS the point, hun... but I'm not burning you am I?"
    rose ""

translate id w3PunishGame_2b92810a:
    # fel "H-huh...?! I g-guess not... it actually feels kinda... my s-skin is-- a-aahh...!"
    fel ""

translate id w3PunishGame_0ef8c6f6:
    # rose "See, {b}trust me.{/b}"
    rose ""

translate id w3PunishGame_36fcb692:
    # fel "Ahhhaaah, why does it feel like a massage? My head's all fuckin' hhngggg...!"
    fel ""

translate id w3PunishGame_151f207e:
    # rose "Ha, good... then I'm doing it right."
    rose ""

translate id w3PunishGame_a7ba436f:
    # fel "Have you never done this before?!"
    fel ""

translate id w3PunishGame_00275970:
    # rose "Oh, heavens no... I'm not a deviant like you. Like I said, I've only read about it..."
    rose ""

translate id w3PunishGame_db5d974c:
    # fel "W-well fuck you too, bitch!"
    fel ""

translate id w3PunishGame_98fda3c2:
    # "*Thwack!*"
    ""

translate id w3PunishGame_97b67416:
    # rose "Don't be rude!"
    rose ""

translate id w3PunishGame_ff3a719e:
    # fel "G-gah, w-what the fuck?! That... h-hnngg...! F-fuck me, why did I almost cum from that?"
    fel ""

translate id w3PunishGame_e68f2b71:
    # rose "Because you're perverted."
    rose ""

translate id w3PunishGame_b83836d0:
    # ver "It's probably because of the drug..."
    ver ""

translate id w3PunishGame_1876d4a7:
    # rose "Uh, sorry... I don't mean that... well, not all of it... there's a camera, so I must make it sound good too."
    rose ""

translate id w3PunishGame_0e66a619:
    # fel "B-bullshit... you're enjoying this..."
    fel ""

translate id w3PunishGame_060ecd6f:
    # rose "Don't be rude."
    rose ""

translate id w3PunishGame_6ba9e56c:
    # rose "Still... that felt more intense than the first time I slapped you, right?"
    rose ""

translate id w3PunishGame_efb00d77:
    # fel "Y-yeah...?"
    fel ""

translate id w3PunishGame_0a150f64:
    # rose "Good. It worked then."
    rose ""

translate id w3PunishGame_98bce41e:
    # fel "Gah, my head's all f-fucked right nooooow... mmmwww..."
    fel ""

translate id w3PunishGame_a75dc9dc:
    # fel "G-gah...?!"
    fel ""

translate id w3PunishGame_be8ef4cc:
    # rose "You've got lovely skin, Felicia... I'm jealous of how evenly you tan."
    rose ""

translate id w3PunishGame_4089eb86:
    # fel "N-not the time for a compliment y-you, a-hh, h-hey... Red... how about you stick that in my--"
    fel ""

translate id w3PunishGame_4b75703a:
    # rose "{b}Not yet{/b}, please."
    rose ""

translate id w3PunishGame_ba55e27f:
    # ver "I wasn't planning on it. I'm actually curious to see where you're taking this..."
    ver ""

translate id w3PunishGame_574ea052:
    # fel "H-hatt, what are you... g-gah..."
    fel ""

translate id w3PunishGame_66e1b451:
    # fel "H--haat, f-fhuck... every part of me is on fire and... hnngg... e-every time you touch it it's like my ah... I'm out of my body?"
    fel ""

translate id w3PunishGame_9526e275:
    # ver "You're more sadistic than you let on, huh?"
    ver ""

translate id w3PunishGame_14e93a2d:
    # rose "It's part of the game!"
    rose ""

translate id w3PunishGame_ad1e0f04:
    # ver "You have a big smile on your face right now."
    ver ""

translate id w3PunishGame_fa0f4daa:
    # rose "It's nothing like that. It's just nice to be on this side of things for a change...?"
    rose ""

translate id w3PunishGame_99a95cb3:
    # fel "Ha-hhaa, h-haaaat! Gah, s-stop blathering so much! I can't think!"
    fel ""

translate id w3PunishGame_bad56b2c:
    # ver "Awww, is Blondie frustrated she doesn't hold 100%% of the attention?"
    ver ""

translate id w3PunishGame_b311bddc:
    # fel "G-ahh, why are you bitches getting so mean all of a sudden?! J-just... hng... m-make me feel good; I'm dying here!"
    fel ""

translate id w3PunishGame_af3cd922:
    # rose "Ask politely, please."
    rose ""

translate id w3PunishGame_70f9d317:
    # fel "Hhhngg... f-fuck no. I'm not begging you for a thing!"
    fel ""

translate id w3PunishGame_15a1dd3b:
    # rose "That kind of obstinacy is not really helping anything... you're sending some mixed messages here."
    rose ""

translate id w3PunishGame_eaefb8cb:
    # rose "If you want something, you need to learn to ask. Even my daughter has more manners than you."
    rose ""

translate id w3PunishGame_4e20a382:
    # ver "Drink up, Blondie."
    ver ""

translate id w3PunishGame_b8dcdbd4:
    # fel "H-huh?"
    fel ""

translate id w3PunishGame_be7535bc:
    # ver "I said to drink up!"
    ver ""

translate id w3PunishGame_a236ad1f:
    # kat "It's always interesting when you give a woman like Rosalind the opportunity to blow off steam. They're so stressed, pent up, inhibited..."
    kat ""

translate id w3PunishGame_fad2c9d1:
    # mc "H-haa..."
    mc ""

translate id w3PunishGame_330ee2ef:
    # kat "They don't show you their true colors as much as their whole color palette blends together in one fucked up mix."
    kat ""

translate id w3PunishGame_274a48d8:
    # "Like hell my mind was registering any of her bullshit right now."
    ""

translate id w3PunishGame_9e84ae9d:
    # sophia "You counted on this kind of... {i}fervor{/i}?"
    sophia ""

translate id w3PunishGame_83473487:
    # mc "H-a, hnnggaa..."
    mc ""

translate id w3PunishGame_e7a38a2a:
    # "The inside of my head was mush right now. My capacity for language was utterly eroded."
    ""

translate id w3PunishGame_93fd3301:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_1abbdcae:
    # kat "I'm well acquainted with the peculiar ways people in desperate situations respond. In a way, it's a more potent means of control than any drug."
    kat ""

translate id w3PunishGame_47d2a767:
    # sophia "I'm actually impressed for once."
    sophia ""

translate id w3PunishGame_2b5022f9:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your owner."
    kat ""

translate id w3PunishGame_d2a0be0e:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_ed6f1cb8:
    # ver "Huh... that's an interesting direction..."
    ver ""

translate id w3PunishGame_087867e3:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_2c1786b9:
    # ver "How are you...?"
    ver ""

translate id w3PunishGame_be7535bc_1:
    # ver "I said to drink up!"
    ver ""

translate id w3PunishGame_a236ad1f_1:
    # kat "It's always interesting when you give a woman like Rosalind the opportunity to blow off steam. They're so stressed, pent up, inhibited..."
    kat ""

translate id w3PunishGame_330ee2ef_1:
    # kat "They don't show you their true colors as much as their whole color palette blends together in one fucked up mix."
    kat ""

translate id w3PunishGame_7fbd2e1f:
    # "To my annoyance, whenever one of them spoke, my attention would turn back to just how fuckable the women next to me were. "
    ""

translate id w3PunishGame_9e84ae9d_1:
    # sophia "You counted on this kind of... {i}fervor{/i}?"
    sophia ""

translate id w3PunishGame_96a9d185:
    # mct "(Christ, I want to make that mouthy hag choke on my dick until she passes out!)"
    mct ""

translate id w3PunishGame_f94b31a8:
    # "Intrusive thought upon intrusive thought flooded my head."
    ""

translate id w3PunishGame_93fd3301_1:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_1abbdcae_1:
    # kat "I'm well acquainted with the peculiar ways people in desperate situations respond. In a way, it's a more potent means of control than any drug."
    kat ""

translate id w3PunishGame_47d2a767_1:
    # sophia "I'm actually impressed for once."
    sophia ""

translate id w3PunishGame_5a5d8551:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your Master."
    kat ""

translate id w3PunishGame_d2a0be0e_1:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df_1:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108_1:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418_1:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_ed6f1cb8_1:
    # ver "Huh... that's an interesting direction..."
    ver ""

translate id w3PunishGame_087867e3_1:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_2c1786b9_1:
    # ver "How are you...?"
    ver ""

translate id w3PunishGame_ee631cb7:
    # fel "G-gah, you read about this in a book too?!"
    fel ""

translate id w3PunishGame_3bcdf300:
    # rose "Not {i}quite{/i}... how does it feel?"
    rose ""

translate id w3PunishGame_56338313:
    # fel "Hnngg... what do you mean how does it feel? It's like- It's like I'm getting my tits sucked on by a glass cup!"
    fel ""

translate id w3PunishGame_ecb82960:
    # fel "Gh-"
    fel ""

translate id w3PunishGame_94213866:
    # rose "How about now?"
    rose ""

translate id w3PunishGame_1600b97d:
    # fel "Hhhn, s-shitt...!"
    fel ""

translate id w3PunishGame_11c607d9:
    # rose "Not quite the reaction I was looking for, but I guess it has just been a couple of pumps..."
    rose ""

translate id w3PunishGame_28827e79:
    # fel "Hhhhhhaaaaaa! Gh, haaa--! You c-cunt!"
    fel ""

translate id w3PunishGame_df26b83a:
    # rose "How many times do I have to ask you not to be rude?"
    rose ""

translate id w3PunishGame_226ebebf:
    # fel "Ga-, ggaaaah... ggaaahh...!"
    fel ""

translate id w3PunishGame_95e337f2:
    # "The impact of the fourth pump was quite dramatic. Felicia's eyes rolled into the back of her head and she howled like a dying animal."
    ""

translate id w3PunishGame_299c06cd:
    # rose "I bet that feels heavy, like nursing a baby?"
    rose ""

translate id w3PunishGame_d6e638ac:
    # ver "More like nursing a vacuum cleaner..."
    ver ""

translate id w3PunishGame_eb74705f:
    # fel "G-ggaaah... it's pulling my nipples off!"
    fel ""

translate id w3PunishGame_10282f92:
    # rose "Oh, come on, don't be dramatic."
    rose ""

translate id w3PunishGame_5ce954c8:
    # fel "H, haaaahht.. they're going to come off! They're coming off!"
    fel ""

translate id w3PunishGame_3a33674c:
    # ver "Yeah, seriously, don't be so dramatic."
    ver ""

translate id w3PunishGame_bc29b524:
    # rose "Hang in there."
    rose ""

translate id w3PunishGame_04fdb47e:
    # "With that particular choice of words, the two retreated back to let Felicia fight against the unyielding suck of the two pumps dangling off her chest in an unwinnable battle of time."
    ""

translate id w3PunishGame_774d513b:
    # fel "Hnnng, gaaahh...! Shit, shit, shit, shit, shit...!"
    fel ""

translate id w3PunishGame_0e8bb8e8:
    # "The longer she hung there, with the breast pumps dangling from her chest, the more desperate she sounded."
    ""

translate id w3PunishGame_4002ba21:
    # "It didn't take very long, maybe just two or three minutes, before the signs of defeat were plastered on her face."
    ""

translate id w3PunishGame_68cb3a11:
    # fel "Can't think...! It's alallllglhhhh...!"
    fel ""

translate id w3PunishGame_941b2d76:
    # fel "Gahhahaha, hggnnnggaaaa...! I'm ddddyyingg, my tits areee..."
    fel ""

translate id w3PunishGame_0bdfd0de:
    # fel "I'm! I'm! I'm! I'm! I'm! I'm! I'm! CHwwwuummingngg!"
    fel ""

translate id w3PunishGame_f609379b:
    # fel "*Hwugfff... hwhuuff... *Wheeeze* Hnnggaa..."
    fel ""

translate id w3PunishGame_49865e12:
    # "The unceasing suction brought Felicia to the brink in a snap instant."
    ""

translate id w3PunishGame_04acd840:
    # ver "Holy shit. Did she just...?"
    ver ""

translate id w3PunishGame_8d232c0e:
    # rose "Woah..."
    rose ""

translate id w3PunishGame_2148117d:
    # ver "I think you broke her."
    ver ""

translate id w3PunishGame_76578736:
    # fel "Hngg, ahh... sgwwhohoo sshlloohpy, shwwaahh..."
    fel ""

translate id w3PunishGame_adb694dc:
    # rose "I didn't think her reaction would be that extreme..."
    rose ""

translate id w3PunishGame_4215c854:
    # fel "Hnnggg... givehh mee... shhovovee it in mee..."
    fel ""

translate id w3PunishGame_7efa6c8a:
    # rose "Oh dear... I didn't realize it was that extreme."
    rose ""

translate id w3PunishGame_8fb2497e:
    # rose "Go ahead and give her what she wants, please."
    rose ""

translate id w3PunishGame_70bfa92b:
    # ver "Hmm... not yet. She's still got one more water bottle left."
    ver ""

translate id w3PunishGame_01d6aee3:
    # kat "I'm starting to get offended, Mr. [mcl]. Does my hand not suffice?"
    kat ""

translate id w3PunishGame_06035a29:
    # kat "Don't you want to taste release?"
    kat ""

translate id w3PunishGame_31a83b72:
    # mc "G-gah, f-fuck! It's not like I have any control over it!"
    mc ""

translate id w3PunishGame_fd6b335d:
    # sophia "Hmm..."
    sophia ""

translate id w3PunishGame_501f6d67:
    # "Every fiber of my being desperately anticipated climax. My reason had been eroded and my purpose replaced by the critical need to expunge every trace of nauseating desire from my body through my cock."
    ""

translate id w3PunishGame_15dcbcac:
    # "The hairs on my arm stood on end, the feeling in my extremities came and went like the tide, and a pervasive tingling feeling floated from my neck down to my tailbone - by all means, my body told me that I {b}should{/b} be coming."
    ""

translate id w3PunishGame_cc175353:
    # sophia "Interesting."
    sophia ""

translate id w3PunishGame_2438123c:
    # "But I wasn't. My bloated dick burned against the soft palm of the old woman's hand, pulsating and twitching from the ceaseless stimulation, filling me with the irrational fear that I might literally explode if I didn't soon go careening into the void of sexual release."
    ""

translate id w3PunishGame_5a852968:
    # sophia "All things considered, he is showing a {b}surprising{/b} degree of longevity..."
    sophia ""

translate id w3PunishGame_26f61ebe:
    # kat "Maybe your fragrance isn't as effective as you think it is."
    kat ""

translate id w3PunishGame_a2f58e6b:
    # mc "G-gaahh!"
    mc ""

translate id w3PunishGame_ecfe8072:
    # sophia "Maybe..."
    sophia ""

translate id w3PunishGame_46144357:
    # mct "(No! It was definitely fucking effective!)"
    mct ""

translate id w3PunishGame_5db42400:
    # sophia "...but unlikely. Hey, [mcf]..."
    sophia ""

translate id w3PunishGame_dab66f0d:
    # "Through lidded eyes, I could sense the blonde scooching closer and closer."
    ""

translate id w3PunishGame_08efb90d:
    # sophia "The issue is that a young, well-traveled guy like you simply can't get off on an old woman's touch, right?"
    sophia ""

translate id w3PunishGame_d36256e0:
    # mc "That's not--"
    mc ""

translate id w3PunishGame_93779299:
    # sophia "{b}{i}Isn't it?{/b}{/i}"
    sophia ""

translate id w3PunishGame_76f7a8ba:
    # "Even in my lust-addled state..."
    ""

translate id w3PunishGame_db1a6cb6:
    # "My brain registered amazement over the willful way Sophia flipped the switch, going from detached aloofness to a man-killer just as easily as one would shed their clothes. "
    ""

translate id w3PunishGame_061fb09e:
    # mc "Hnngg...!"
    mc ""

translate id w3PunishGame_beef1333:
    # kat "*Scoff* Feeling left out, were you?"
    kat ""

translate id w3PunishGame_bfa0648c:
    # sophia "Give them a comparison."
    sophia ""

translate id w3PunishGame_18b36fa6:
    # "My hand responded independently, sinking into the pleasant give of the doctor's tit-flesh in an instant as what remained of my faculties disappeared into the whirlpool of her eyes."
    ""

translate id w3PunishGame_04a18dd7:
    # sophia "You got a mess lurking somewhere in there that you want to get out?"
    sophia ""

translate id w3PunishGame_0f885ed4:
    # mc "Ha... hnngg...! F-fuck it!"
    mc ""

translate id w3PunishGame_5d2b485c:
    # sophia "...?!"
    sophia ""

translate id w3PunishGame_cc522673:
    # "I kissed her, just as you'd expect a man on the brink of starvation to steal a loaf of bread."
    ""

translate id w3PunishGame_02060d5f:
    # sophia "Mmmhhhh....!"
    sophia ""

translate id w3PunishGame_8c6f3eee:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Mrs. Pulman."
    ""

translate id w3PunishGame_b5e1efa6:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Kathleen."
    ""

translate id w3PunishGame_e13d5d67:
    # kat "H-hey, ahh--"
    kat ""

translate id w3PunishGame_2430bc3d:
    # "Spurred on by having the attention of two beautiful women, even if it was just the fucked up equivalent of cooking ants with a magnifying glass to them, I knew it wouldn't be long until I finally had my release."
    ""

translate id w3PunishGame_87732b5f:
    # kat "Gah, ah... ha... hmpfh."
    kat ""

translate id w3PunishGame_353b1cca:
    # "Although I could feel her eyes on my neck, the old woman continued to milk me."
    ""

translate id w3PunishGame_61937b68:
    # "*Cwhup, fwhup, chwup...!*"
    ""

translate id w3PunishGame_28eed101:
    # kat "I've somehow ended up feeling like a third wheel."
    kat ""

translate id w3PunishGame_278c7b3e:
    # "I pawed at both women's breasts with a virgin-like fervor, wholly enamored by an opportunity I might never get again."
    ""

translate id w3PunishGame_5d459ae6:
    # mct "(The head of a charity and a world-class scientist... ha, fucking sluts!)"
    mct ""

translate id w3PunishGame_e128df9b:
    # "I felt like a king, not of my own making of course, but my brain was so cooked as to not know the difference. Thinking was hard, but what few thoughts I could miraculously formulate were like tidal waves breaking against the shore."
    ""

translate id w3PunishGame_0ed4c646:
    # mct "(Who the hell do you think you're fucking with? You think you can toy with me just because of money?!)"
    mct ""

translate id w3PunishGame_b117f61e:
    # "I felt smothered, suffocated, and hot. More akin to a wick that was about to burn out than a human being."
    ""

translate id w3PunishGame_9c75dc6b:
    # mct "(I'M A GUINEA PIG TO YOU?!)"
    mct ""

translate id w3PunishGame_07bbfc1c:
    # "Every sensation felt heightened, and every second felt prolonged... I genuinely feared that the moment I popped would be when I was snuffed out."
    ""

translate id w3PunishGame_a73faf04:
    # mct "{size=+30}{font=/gui/fonts/MB-Thin_Worms.ttf}(You like to toy with pe-){/font}{/size}"
    mct ""

translate id w3PunishGame_61867711:
    # "My brain gave out, but if I ceased to exist after this, I instinctively knew one thing: these last minutes of my life should be bold."
    ""

translate id w3PunishGame_ee7c2e13:
    # mc "I'm about to b-blow..."
    mc ""

translate id w3PunishGame_01a79887:
    # kat "You don't need my permission."
    kat ""

translate id w3PunishGame_2221f5ba:
    # mc "Can you finish me with your mouth?"
    mc ""

translate id w3PunishGame_fde31e51:
    # kat "Hmmmmm, not going to happen."
    kat ""

translate id w3PunishGame_eefd493d:
    # mc "A-ah, pretty please? With a cherry on top?"
    mc ""

translate id w3PunishGame_4469f1d1:
    # kat "Why don't you ask Miss Lundgren to do it?"
    kat ""

translate id w3PunishGame_6032c009:
    # mc "...because I want you, Ma'am."
    mc ""

translate id w3PunishGame_7d48d431:
    # kat "...hmmmmmmmm. You have the eyes of a crazed animal, yet..."
    kat ""

translate id w3PunishGame_2d6e0df6:
    # kat "You're asking so nicely. God, I love that."
    kat ""

translate id w3PunishGame_af9d9795:
    # "She smirked like she had won against me, and perhaps even against Sophia."
    ""

translate id w3PunishGame_76125b85:
    # kat "It really is goddamn adorable. Ask me again, [mcf]."
    kat ""

translate id w3PunishGame_8ce7b843:
    # "A desperate, cloying urge inside me told me her mouth was the only place I wanted to be right now and if it took a bit of pleading to get me into a position where I could fill that mean bitch's stomach with my seed, then so be it."
    ""

translate id w3PunishGame_ca96d738:
    # mc "Hhh, haa... would you PLEASE finish me with your mouth, Mrs. Pulman?"
    mc ""

translate id w3PunishGame_e2b980af:
    # "Without another word, Mrs. Pulman lowered herself and..."
    ""

translate id w3PunishGame_fc8cf5a3:
    # "Without another word, Kathleen lowered herself and..."
    ""

translate id w3PunishGame_109a7847:
    # mc "G-gah...!"
    mc ""

translate id w3PunishGame_d66b4def:
    # "The moment her lips wrapped themselves around my cock, I felt like I might pass out."
    ""

translate id w3PunishGame_cca0b4ef:
    # "Her seal was so airtight that her mouth felt like an oven. Steady puffs of hot breath passed over the head of my cock as it made its way out through her nose."
    ""

translate id w3PunishGame_d6fb4ea7:
    # mc "F-fuck, damn it!"
    mc ""

translate id w3PunishGame_1bd17e11:
    # "*Shwuck, fgwhhuck, thwuuuck!*"
    ""

translate id w3PunishGame_bd9356d3:
    # "What little air could escape from her lungs transformed into {b}obscene{/b} noises as she inched herself up and down my shaft."
    ""

translate id w3PunishGame_63238d75:
    # mc "H-holy-!"
    mc ""

translate id w3PunishGame_ad464d04:
    # "The running narration in my head was a jumbled mess, but through all the muddied noise from the pleasure soaking into my brain, I grasped a simple truth."
    ""

translate id w3PunishGame_fdbb746f:
    # mc "Hnnng, you've sucked a lot of cock in your time, haven't you?!"
    mc ""

translate id w3PunishGame_dad9bfec:
    # "*Thwaahp, fwhap, swhhuck~!*"
    ""

translate id w3PunishGame_72c8cf4a:
    # "The answer to that question played across my glans every time I wedged myself comfortably in her throat with zero fuss."
    ""

translate id w3PunishGame_6b12669e:
    # "Swhuck, hhhwhuck, fhhggguuk!*"
    ""

translate id w3PunishGame_a3af0240:
    # "{b}Damn it!{/b} A rich woman like her had no business sucking dick this well."
    ""

translate id w3PunishGame_d86d228f:
    # mc "Ghhn, haaat, hnnngg...!"
    mc ""

translate id w3PunishGame_88387910:
    # "She sucked me with the skill of a woman who had {b}nothing{/b} to her name."
    ""

translate id w3PunishGame_cdfdc63e:
    # kat "Mmmhm, hhmhm~"
    kat ""

translate id w3PunishGame_93b1b0e6:
    # "Just like her hand job, there was no wasted effort. She didn't tackle it in a clumsy frenzy, nor did she need to cautiously get used to my size."
    ""

translate id w3PunishGame_d628853d:
    # "*Shwhuhuck, fwhhucuk, ghhwhuhuk!*"
    ""

translate id w3PunishGame_41aa32dc:
    # "She sucked me like she had been sucking {b}my{/b} dick her whole life, pleasuring me at a steady and comfortable pace. I marveled at how, through a mere blowjob, a woman so cold and cruel could instantly imprint herself on my body as warm and welcoming."
    ""

translate id w3PunishGame_d93b0c5e:
    # "I was quickly understanding that \"adorable\" disparity she had just spoken."
    ""

translate id w3PunishGame_7a3794d6:
    # mc "Hnng, h-how?!"
    mc ""

translate id w3PunishGame_2aa14e7d:
    # sophia "It's not performing brain surgery."
    sophia ""

translate id w3PunishGame_9f2a1802:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Mrs. Pulman's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_bc9afe80:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Kathleen's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_7f97a9d7:
    # mct "(Haaaa... hope you like the show, bitch. You're the impetus for this.)"
    mct ""

translate id w3PunishGame_0d35f1a2:
    # "*Fwhhuck, ghhhukh, khggguuhhk*"
    ""

translate id w3PunishGame_b1ef8758:
    # mc "Ahh... y-you dick swallowing--! Hnnngg...!"
    mc ""

translate id w3PunishGame_57623603:
    # "It was coming."
    ""

translate id w3PunishGame_a9d0bbe1:
    # mc "You think I'm adorable?!"
    mc ""

translate id w3PunishGame_f8d06f5f:
    # "I was moments from blowing my load, and with it, the indignation from earlier swelled in me. I had wanted a bold finish before I met my end, and well..."
    ""

translate id w3PunishGame_26cd0617:
    # mct "(I'll show you adorable!)"
    mct ""

translate id w3PunishGame_72c12088:
    # mc "I'm c-cumming, fucking drown in it, you damn--"
    mc ""

translate id w3PunishGame_ec061ea7:
    # kat "Mmmmh?!"
    kat ""

translate id w3PunishGame_5c240311:
    # "Before she could even register what was happening, ropes of piping hot cum barreled down her esophagus on a one-way trip to her stomach."
    ""

translate id w3PunishGame_10060088:
    # kat "Ghhkka, hkkaa-!"
    kat ""

translate id w3PunishGame_4bb38f46:
    # "I pressed down hard, holding the old woman's head in place as she gargled my seed in abject surprise."
    ""

translate id w3PunishGame_2745bcd2:
    # mc "Hnngg, all of it!"
    mc ""

translate id w3PunishGame_716f46e4:
    # "I pressed down so hard it hurt my groin."
    ""

translate id w3PunishGame_4e4c5b02:
    # kat "Ghhuk, hhuuuh, hwwwhuuuck...!"
    kat ""

translate id w3PunishGame_fe1ef6b3:
    # "It was massive, like I was cumming my very soul down the old woman's throat."
    ""

translate id w3PunishGame_4500ed7e:
    # kat "Ghuk, khak, uuhhk-!"
    kat ""

translate id w3PunishGame_2239e61c:
    # mc "Gah, c-choke on it!"
    mc ""

translate id w3PunishGame_ab7d9ac7:
    # "Not all of my load was making its way down her throat. To my perverse glee, I could {i}feel{/i} myself coating the lining of her throat."
    ""

translate id w3PunishGame_7b62aa4f:
    # kat "Ghhuk, hhuuuk...!"
    kat ""

translate id w3PunishGame_e09b7aa7:
    # mc "Hope you like the fucking taste!"
    mc ""

translate id w3PunishGame_8747fdde:
    # kat "Mmmh, hhhngg?!?!"
    kat ""

translate id w3PunishGame_3698f7b8:
    # sophia "I don't think she can breathe..."
    sophia ""

translate id w3PunishGame_6d4b68da:
    # mc "Good! She can breathe when I'm fucking--!"
    mc ""

translate id w3PunishGame_de4a6cab:
    # ".........!"
    ""

translate id w3PunishGame_2068a0d5:
    # "......!"
    ""

translate id w3PunishGame_1d9b8a71:
    # mc "A-aahh...!"
    mc ""

translate id w3PunishGame_869e1b4e:
    # kat "Ugghhuhh..."
    kat ""

translate id w3PunishGame_ba8a394f:
    # "Peace."
    ""

translate id w3PunishGame_a48af359:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d:
    # "Satisfaction."
    ""

translate id w3PunishGame_92962e6b:
    # "In this moment, I had it all."
    ""

translate id w3PunishGame_e38ecbb2:
    # "With that, I happily died. The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8b8ceb72:
    # "............"
    ""

translate id w3PunishGame_278c7e29:
    # "........."
    ""

translate id w3PunishGame_0fd4d656:
    # "......"
    ""

translate id w3PunishGame_a20cefa7:
    # "..."
    ""

translate id w3PunishGame_87f2fe4b:
    # kat "Why are you holding your hands out like that? It's weird."
    kat ""

translate id w3PunishGame_2dd84f14:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_c2d77e46:
    # sophia "She's right. It's distracting."
    sophia ""

translate id w3PunishGame_1516ad37:
    # "I didn't know how long I was out of it, but by the time I opened my eyes, the two women had collected themselves, and what I thought was a bold display of defiance was made pitiful and wholly expected by their bemused faces."
    ""

translate id w3PunishGame_69e22dc5:
    # "Mrs. Pulman looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_d1fa2418:
    # "Kathleen looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_e7145261:
    # kat "Oh, by the way... you said some pretty outlandish things to me."
    kat ""

translate id w3PunishGame_35f3c20f:
    # mc "If I recall, I did more than that..."
    mc ""

translate id w3PunishGame_2e195b22:
    # kat "It was {b}adorable{/b}."
    kat ""

translate id w3PunishGame_c5563c4b:
    # "She had indeed won."
    ""

translate id w3PunishGame_b028d682:
    # fel "Gha--"
    fel ""

translate id w3PunishGame_d9d8b219:
    # "I knew what I wanted."
    ""

translate id w3PunishGame_3576545e:
    # sophia "Mmmhh, hhhhh..."
    sophia ""

translate id w3PunishGame_7202d9ed:
    # "And that last act of defiance would be leaving the doctor, the root cause of all this, something to remember me by."
    ""

translate id w3PunishGame_293b8be4:
    # "*Chwup, fhwup~*"
    ""

translate id w3PunishGame_7a9110af:
    # "I wanted to hear this aloof bitch moan, so I focused everything on the woman whose mouth I was currently exploring."
    ""

translate id w3PunishGame_01fdbaa9:
    # sophia "Mmmh, hhmmm...?"
    sophia ""

translate id w3PunishGame_1efdf9e0:
    # "It was a battle against my wits. Sophia was literally intoxicating. Focusing my senses all on her had me burning twice as fast."
    ""

translate id w3PunishGame_d14b5b96:
    # "By instinct, I felt her breast over her clothes."
    ""

translate id w3PunishGame_79a3ccdd:
    # sophia "Mmhh..."
    sophia ""

translate id w3PunishGame_a90ff98f:
    # "Her coos told me she was receptive, but it wasn't quite the noise I wanted to drag out from the blonde."
    ""

translate id w3PunishGame_754fce10:
    # sophia "Ghu, gha~ghhuu...!"
    sophia ""

translate id w3PunishGame_be4204b5:
    # "Deciding that over the clothes touching wouldn't do it, I switched focus on the erogenous zone in her mouth, intensifying my efforts in an intricate dance to subdue her tongue with my own."
    ""

translate id w3PunishGame_f43fc47f:
    # sophia "Mmmmh, mmhhhh- *chwuph, fhwhup~*"
    sophia ""

translate id w3PunishGame_b0084126:
    # "She wasn't a passive observer in my hunt; her tongue played coy, darting away and slipping from my grasp whenever I thought I had her pinned."
    ""

translate id w3PunishGame_a03c3549:
    # "It was mere seconds, but--"
    ""

translate id w3PunishGame_27c1fdac:
    # "*Chwhu, fhwwwup, chup!*"
    ""

translate id w3PunishGame_f4de7f68:
    # "--but, in my fogged-over state, time held little sway over my perception. What was in reality a clumsy tussle felt like a long and protracted siege."
    ""

translate id w3PunishGame_2ec99952:
    # "I don't know if it was because of the perfume, but all bodily sensations gradually began to meld into one."
    ""

translate id w3PunishGame_45c987c8:
    # "The feeling of the old woman's hand milking my cock, the wet warmness of our kiss, the bubbling desire so strong that I was confident it would be the end of me..."
    ""

translate id w3PunishGame_15b0522f:
    # mc "Mmmh, hhhmmm-!"
    mc ""

translate id w3PunishGame_7c16ed74:
    # "All of it spelled an overload melded and combined into a singular feeling of otherness. I was sure I had lost this battle until..."
    ""

translate id w3PunishGame_7d144a01:
    # "My brain registered a change."
    ""

translate id w3PunishGame_fbfb812a:
    # mct "Was she...?"
    mct ""

translate id w3PunishGame_7459555f:
    # sophia "Mmmh, hhmhm~aahhh...!"
    sophia ""

translate id w3PunishGame_68d04ab6:
    # "She was closer."
    ""

translate id w3PunishGame_08a39e71:
    # "She was pressing into me."
    ""

translate id w3PunishGame_883ad25a:
    # "She felt it."
    ""

translate id w3PunishGame_b0685c2c:
    # "She was {b}heated{/b}."
    ""

translate id w3PunishGame_5c6a5db6:
    # "{i}She was kissing me back.{/i}"
    ""

translate id w3PunishGame_26629cc2:
    # "Whether it was done deliberately or on an instinctual level, her body responded favorably."
    ""

translate id w3PunishGame_a4d39526:
    # sophia "Ahh, hhhnnggg, mmmmhh-!"
    sophia ""

translate id w3PunishGame_d8ca8c90:
    # "My hand found a place on her ass and I used that to spur her on even more. With this, I had physical confirmation of my success."
    ""

translate id w3PunishGame_44602cec:
    # mct "(Fuck, what a great-!)"
    mct ""

translate id w3PunishGame_9fec863b:
    # "I could feel the hot dampness of her delicate spot."
    ""

translate id w3PunishGame_cb435f7f:
    # sophia "Mmhh, hhaa...♥♥♥"
    sophia ""

translate id w3PunishGame_71583afd:
    # "She purred openly and I felt a petty relief over my trivial victory. The succubus driving me to this desperate state wouldn't escape our battle unscathed."
    ""

translate id w3PunishGame_3d67a16d:
    # "*Chwup, fhwwup, khwwup*!"
    ""

translate id w3PunishGame_b6fdc206:
    # "--and that wasn't the only sense of relief I felt."
    ""

translate id w3PunishGame_cb9361ae:
    # mc "{b}Hnng, ah-!!!{/b}"
    mc ""

translate id w3PunishGame_cbea8557:
    # "I didn't know when it started, but ropes of cum geysered from my cock and rained down from the sky."
    ""

translate id w3PunishGame_4380fa10:
    # mc "Ghh, aaahh- sssshit--"
    mc ""

translate id w3PunishGame_a3230c6c:
    # kat "Huh, {b}WOW{/b}."
    kat ""

translate id w3PunishGame_74ff6633:
    # mc "Hngg, hhhhmmmhh-~"
    mc ""

translate id w3PunishGame_290360e4:
    # "All the suffocating vileness in me was expelled and released into the world. All my desperation spilled out onto our clothes and thighs."
    ""

translate id w3PunishGame_83af968d:
    # "Soon need was replaced with..."
    ""

translate id w3PunishGame_ba8a394f_1:
    # "Peace."
    ""

translate id w3PunishGame_a48af359_1:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d_1:
    # "Satisfaction."
    ""

translate id w3PunishGame_2a3c0c6e:
    # "Emptiness."
    ""

translate id w3PunishGame_ea0f012b:
    # "The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8728f5e5:
    # "I was dead."
    ""

translate id w3PunishGame_8b8ceb72_1:
    # "............"
    ""

translate id w3PunishGame_278c7e29_1:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_1:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_1:
    # "..."
    ""

translate id w3PunishGame_c49ce7ae:
    # sophia "Great, now we'll spend the rest of the show reeking of semen."
    sophia ""

translate id w3PunishGame_2dd84f14_1:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_2f1146d1:
    # kat "You should've thought of that before you entered the crossfire."
    kat ""

translate id w3PunishGame_9177b213:
    # sophia "You were taking too long. I wanted to put an end to the distraction."
    sophia ""

translate id w3PunishGame_0e5cc7d3:
    # "I didn't know how long I was out of it. Still, by the time I opened my eyes, the two women had collected themselves. The damage I thought I had done to Sophia with my counter-attack was made pitiful by their bemused faces."
    ""

translate id w3PunishGame_23aa7f4f:
    # kat "Oh, please. Are those the kind of sounds you make matter-of-factly?"
    kat ""

translate id w3PunishGame_a5d22b22:
    # "Sophia looked as composed as ever, and except for a little blush, her attention turned back to the Carnations."
    ""

translate id w3PunishGame_449fa9fb:
    # kat "I hope that provided you a measure of relief, Mr. [mcl]. That certainly was an... {i}explosive{/i} finish."
    kat ""

translate id w3PunishGame_b85bc4ed:
    # mc "A, ahem... thaaaaaanks?"
    mc ""

translate id w3PunishGame_fada21bd:
    # kat "Don't mention it. It was... interesting."
    kat ""

translate id w3PunishGame_b028d682_1:
    # fel "Gha--"
    fel ""

translate id w3PunishGame_dddb6f7e:
    # mc "........."
    mc ""

translate id w3PunishGame_3b21f354:
    # mc "......"
    mc ""

translate id w3PunishGame_5b4c9fd4:
    # "...this was my limit."
    ""

translate id w3PunishGame_964e84bc:
    # mc "Aaeeh, I'll be riiiiiight back."
    mc ""

translate id w3PunishGame_0fd4d656_2:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_2:
    # "..."
    ""

translate id w3PunishGame_e6cfc671:
    # kat "Is that a milder scent you are wearing?"
    kat ""

translate id w3PunishGame_837e9b2b:
    # sophia "It's the same batch as the bottle I gave you."
    sophia ""

translate id w3PunishGame_256d5f3e:
    # kat "The effect seemed... less? Why's that?"
    kat ""

translate id w3PunishGame_548f5e80:
    # sophia "It's possible that his body adapted and partially metabolized the odor molecules. All I can say is that it's remarkable he held out that long."
    sophia ""

translate id w3PunishGame_b4f48c1e:
    # sophia "It could be a matter of willpower."
    sophia ""

translate id w3PunishGame_caedd2e6:
    # kat "When I had dinner with Charles the other night, he didn't even flinch."
    kat ""

translate id w3PunishGame_b5500f59:
    # sophia "Dr. Kohler is a scary man."
    sophia ""

translate id w3PunishGame_4510a9da:
    # kat "Is that your appraisal after demolishing him game after game?"
    kat ""

translate id w3PunishGame_64ecae1b:
    # sophia "How else should I describe a man who has no need but keeps his passions tightly controlled."
    sophia ""

translate id w3PunishGame_610ec2fb:
    # kat "That's why he and I are incompatible. He treats fucking like it's Christmas."
    kat ""

translate id w3PunishGame_560752c9:
    # kat "You don't have anything to worry about as long as you don't lose."
    kat ""

translate id w3PunishGame_b19fae41:
    # sophia "I don't plan on it, but I have a feeling he eventually wins one way or another."
    sophia ""

translate id w3PunishGame_f588a7d6:
    # kat "Abel wouldn't let it come to that, would he?"
    kat ""

translate id w3PunishGame_0fd4d656_3:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_3:
    # "..."
    ""

translate id w3PunishGame_1324c38b:
    # sophia "Thank you for indulging my little whim, Kathleen. It was amusing."
    sophia ""

translate id w3PunishGame_4db92c34:
    # kat "Don't mention it. One should make their own fun in life. Still..."
    kat ""

translate id w3PunishGame_0cb22879:
    # kat "...you're very competent. Don't you ever desire more?"
    kat ""

translate id w3PunishGame_0cdff961:
    # sophia "A frivolous bitch like you has no business saying that to me."
    sophia ""

translate id w3PunishGame_9765acec:
    # kat "Ah, fuck. There it is."
    kat ""

translate id w3PunishGame_55e6ab51:
    # kat "I almost felt sorry for you."
    kat ""

translate id w3PunishGame_4780d0f8:
    # fel "--aaahhhhnn! It's t-to-tooooooo muuuuchh!"
    fel ""

translate id w3PunishGame_ec61481d:
    # ver "C'mon, Blondie... A woman like you would be able to take as much."
    ver ""

translate id w3PunishGame_d630ec37:
    # fel "Gghh, hhhhhnnnggaaaaaawwwah! It's fucking strong!"
    fel ""

translate id w3PunishGame_ab77a85c:
    # mct "(Right, I was so caught up in my lust that I had forgotten that this was still happening...)"
    mct ""

translate id w3PunishGame_ad8d563c:
    # fel "Gh, hhnngg, hhnnngg...! Oh, hhhhnngngg...! N-no!"
    fel ""

translate id w3PunishGame_fed2c95c:
    # ver "Hmmm? This is what you asked for, isn't it?"
    ver ""

translate id w3PunishGame_686e6370:
    # fel "T-the vibe is m-moving the beads and m-making me wanna--"
    fel ""

translate id w3PunishGame_03fc6bb6:
    # ver "Pee? Well, you did drink three water bottles..."
    ver ""

translate id w3PunishGame_c5c134a9:
    # rose "You planned this?"
    rose ""

translate id w3PunishGame_0a1bc2b9:
    # ver "Well, I had to pull my weight for this \"game\", right?"
    ver ""

translate id w3PunishGame_7ea4d186:
    # fel "It f-feels to-ttoooo good, but I d-don't w-wanna...!"
    fel ""

translate id w3PunishGame_4210c199:
    # ver "You don't want to piss yourself like an idiot in front of your future club members?"
    ver ""

translate id w3PunishGame_936b2d65:
    # fel "Ghh, hh--"
    fel ""

translate id w3PunishGame_a0939cf0:
    # ver "What an odd fucking line to draw, huh? You think volunteering for this gives you a veneer of dignity?"
    ver ""

translate id w3PunishGame_57cb9923:
    # ver "Newsflash: sluts like you don't have any dignity."
    ver ""

translate id w3PunishGame_fd16f1a4:
    # rose "Eh, don't you think you're being too mean?"
    rose ""

translate id w3PunishGame_cafdb3ad:
    # fel "Gghhh, f-hhh--"
    fel ""

translate id w3PunishGame_7bdfd7fb:
    # ver "Mean? You and I are fighting for our futures, and she's just a bored rich bitch looking for a kick."
    ver ""

translate id w3PunishGame_e83b00b5:
    # ver "I'm just letting her know she isn't shit... for the purpose of the game, you know..."
    ver ""

translate id w3PunishGame_90e4a059:
    # rose "Right, yeah... the game."
    rose ""

translate id w3PunishGame_fec6b8fc:
    # fel "Why should I care about your sob stories? It d-doesn't matter who wins; it only matters if you lose!"
    fel ""

translate id w3PunishGame_31c6bb42:
    # fel "Do you think Rose will give a shit if you win over me? It's all the same to her as it would be to you!"
    fel ""

translate id w3PunishGame_47d4ca5b:
    # ver "Oh, yeah?"
    ver ""

translate id w3PunishGame_0f084c2b:
    # fel "That's right! You should be happy to have a bored rich bitch as your opponent! Would you rather three desperate people get paraded like show dogs over two?! Why the hell are you even thinking about me?!"
    fel ""

translate id w3PunishGame_ccf2ac52:
    # fel "Focus on winning you--- gah, you're so fucking short-sight--"
    fel ""

translate id w3PunishGame_cfeac5d6:
    # fel "Gh, hhuhukhhh...?!"
    fel ""

translate id w3PunishGame_398cf3fb:
    # ver "Just shut up and piss yourself, you fucking cumrag."
    ver ""

translate id w3PunishGame_abe01903:
    # fel "Mmmhhh, mmhmhh, hnnnggg!"
    fel ""

translate id w3PunishGame_1a1367bf:
    # ver "When you lose, you can go back to your damn penthouse! Take a dip in your pool! Eat your fancy grub!"
    ver ""

translate id w3PunishGame_163de1c2:
    # ver "A good price for your dignity, because I'm certainly selling mine for much less!"
    ver ""

translate id w3PunishGame_d14637c4:
    # ver "..."
    ver ""

translate id w3PunishGame_7720e84e:
    # fel "Hnnngg, hhh--"
    fel ""

translate id w3PunishGame_39527c0a:
    # fel "Mmmhh, mmggmmhhh, hhhmmm--"
    fel ""

translate id w3PunishGame_92fe5ba8:
    # rose "Hey..."
    rose ""

translate id w3PunishGame_d050e5d9:
    # fel "UUUhh, hhhehhhh---"
    fel ""

translate id w3PunishGame_7398edc4:
    # rose "You alright...?"
    rose ""

translate id w3PunishGame_265f2dc9:
    # ver "Yeah, I'm fine, Rosie... I just thought being in the driver's seat would be more fun."
    ver ""

translate id w3PunishGame_c29db021:
    # rose "That's what I mean; we all still have our dignity. None of this has--"
    rose ""

translate id w3PunishGame_e2a5628d:
    # fel "Mmhmhwwahhahahh, ghhgaaa-a---hhhhhaaa"
    fel ""

translate id w3PunishGame_36f9f68c:
    # ver "There she goes. Well..."
    ver ""

translate id w3PunishGame_ed531c82:
    # fel "Hnnhhsshwwwiiiitt, n-nooohhh, fhhhkk---"
    fel ""

translate id w3PunishGame_80c39355:
    # ver "This {i}was{/i} kinda fun..."
    ver ""

translate id w3PunishGame_69228eb4:
    # fel "Hnnggg..."
    fel ""

translate id w3PunishGame_0b90c4a6:
    # rose "Better her than us, at least."
    rose ""

translate id w3PunishGame_207206f6:
    # "As things came to a close, I was left with a prickly thought."
    ""

translate id w3PunishGame_2677b915:
    # "That was as honest as they had ever been with each other."
    ""

translate id w3PunishGame_37744ee8:
    # fel "It's kinda obvious, but you don't really like me, do you?"
    fel ""

translate id w3PunishGame_d989fac2:
    # ver "What the f-fuck does that matter, Blondie?"
    ver ""

translate id w3PunishGame_3b7c5953:
    # fel "It matters because you're the one tied up right now."
    fel ""

translate id w3PunishGame_9475ff66:
    # ver "Hnnng... I like you just fine?"
    ver ""

translate id w3PunishGame_a6f943ca:
    # fel "No, you don't, Red. You've never hidden it before, don't try and hide the truth now just because you're in the palm of who's asking."
    fel ""

translate id w3PunishGame_da8d7d31:
    # fel "It's out of character."
    fel ""

translate id w3PunishGame_4d253e29:
    # ver "Again, w-what does it matter? Just hurry up and get rolling!"
    ver ""

translate id w3PunishGame_b777d1e1:
    # fel "I know you think I'm frivolous and undeserving - and that's okay; I get where you're coming from."
    fel ""

translate id w3PunishGame_f91d7c81:
    # ver "Oh, just shut up! If you don't start, how about you just kill me?"
    ver ""

translate id w3PunishGame_e373fc38:
    # ver "How about you, Rosie? Will you p-put me out of my m-misery here?"
    ver ""

translate id w3PunishGame_ea9c70ff:
    # rose "Heh, I'd like to start, but uh... she's got the lead right now. I wouldn't want to interrupt and be rude."
    rose ""

translate id w3PunishGame_6499e543:
    # fel "I'm going to be nice to you, Red. I'm going to treat you how I'd like to be treated."
    fel ""

translate id w3PunishGame_894add04:
    # fel "I know you don't care about this either, but I like you. I really do."
    fel ""

translate id w3PunishGame_af116192:
    # ver "Yeah, riiiiiight..."
    ver ""

translate id w3PunishGame_81d21ac1:
    # fel "Oh, c'mon. I've caught you stealing glances before."
    fel ""

translate id w3PunishGame_cfb67c86:
    # fel "I'm used to people looking down on me and I'm also used to those same people wanting to fuck me."
    fel ""

translate id w3PunishGame_82f35a2f:
    # fel "{b}You{/b} want to fuck me, Red. Admit it."
    fel ""

translate id w3PunishGame_73e87eee:
    # ver "G-gah, you're fucking full of yourself!"
    ver ""

translate id w3PunishGame_f4e06282:
    # ver "Mmmh, hhmmmm?!"
    ver ""

translate id w3PunishGame_8a791c4d:
    # "Felicia, as if on cue, kissed Veronica's neck. Nothing exaggerated, just something..."
    ""

translate id w3PunishGame_72d20a44:
    # ver "Mmmmhn, mmmhh...!"
    ver ""

translate id w3PunishGame_304235f1:
    # "Just something to make her point."
    ""

translate id w3PunishGame_90f1d821:
    # fel "You're about to be full of something too."
    fel ""

translate id w3PunishGame_9f389a9a:
    # ver "W-what are you...? Ah, f-f--"
    ver ""

translate id w3PunishGame_fbcaa4b8:
    # fel "Just sit back and enjoy. Don't go anywhere."
    fel ""

translate id w3PunishGame_1c4e69ea:
    # fel "Oh, by the way... you have [mcf] to thank for this part. He taught me just how effective these little devils are."
    fel ""

translate id w3PunishGame_1bee629e:
    # fel "For the record, I managed four of these."
    fel ""

translate id w3PunishGame_e99ea460:
    # fel "For the record, I managed three of these."
    fel ""

translate id w3PunishGame_5de3a55f:
    # kat "Oh, aren't you proud, Mr. [mcl]? It sounds like you left an impression on the girl."
    kat ""

translate id w3PunishGame_12858723_2:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_4162e4a3:
    # kat "Although, honestly... I bet our slut wife friend could've handled five."
    kat ""

translate id w3PunishGame_4b026443:
    # "No, in my current state, my mind was completely consumed by the old woman's grip on my cock."
    ""

translate id w3PunishGame_3a010861_1:
    # mc "H-haahaa...!"
    mc ""

translate id w3PunishGame_0d64cc63:
    # kat "Still, all this talk about being nice. Did she forget this is supposed to be a punishment?"
    kat ""

translate id w3PunishGame_245b9454:
    # sophia "You know very well it can be both. You're the one who arranged the tools."
    sophia ""

translate id w3PunishGame_36d9b302:
    # kat "You're right... it might feel good, but she will resent being at their mercy. She's too pathetically predictable."
    kat ""

translate id w3PunishGame_93651fc0_1:
    # mc "G-gahh...!"
    mc ""

translate id w3PunishGame_22eda143_1:
    # "The pitiful noise of a rutting animal punctuated the women's conversation."
    ""

translate id w3PunishGame_0e8fd45d:
    # kat "...ah, I love it! I really do love Miss Lynch. I wish I had more than a month with her."
    kat ""

translate id w3PunishGame_5ddac938_2:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_66527633_1:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious..."
    kat ""

translate id w3PunishGame_5f8d0d83_1:
    # mc "Huuuhghh...!"
    mc ""

translate id w3PunishGame_a7f739bd_1:
    # kat "What gets a woman like you going?"
    kat ""

translate id w3PunishGame_16f23753_1:
    # mc "H-huhh... ack...!"
    mc ""

translate id w3PunishGame_90eda0c7_2:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019_2:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355_2:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_4c9db659_1:
    # kat "We got that in common at least - {b}all three of us.{/b}"
    kat ""

translate id w3PunishGame_29e481f2_2:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_5de3a55f_1:
    # kat "Oh, aren't you proud, Mr. [mcl]? It sounds like you left an impression on the girl."
    kat ""

translate id w3PunishGame_12858723_3:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_4162e4a3_1:
    # kat "Although, honestly... I bet our slut wife friend could've handled five."
    kat ""

translate id w3PunishGame_0d64cc63_1:
    # kat "Still, all this talk about being nice. Did she forget this is supposed to be a punishment?"
    kat ""

translate id w3PunishGame_73380823:
    # mc "She... ah..."
    mc ""

translate id w3PunishGame_245b9454_1:
    # sophia "You know very well it can be both. You're the one who arranged the tools."
    sophia ""

translate id w3PunishGame_36d9b302_1:
    # kat "You're right... it might feel good, but she will resent being at their mercy. She's too pathetically predictable."
    kat ""

translate id w3PunishGame_0e8fd45d_1:
    # kat "...ah, I love it! I really do love Miss Lynch. I wish I had more than a month with her."
    kat ""

translate id w3PunishGame_5ddac938_3:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_1c55b166_1:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious... what gets a woman like you going?"
    kat ""

translate id w3PunishGame_90eda0c7_3:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019_3:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355_3:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_68542e62_1:
    # kat "Oh, lovely, at least we have that in common."
    kat ""

translate id w3PunishGame_29e481f2_3:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_f49440b9:
    # fel "...and THREE!"
    fel ""

translate id w3PunishGame_199fec6e:
    # fel "You had no trouble whatsoever. They just slid right in there!"
    fel ""

translate id w3PunishGame_746fa0fe:
    # ver "N-no shit, I'm fucking drippi-- gaaah, whwahaaha!"
    ver ""

translate id w3PunishGame_9f03f7b0:
    # fel "Yeah, I know. [mcf] had a whole remote, but I don't. So you're just going to have to deal with \"on\" for as long as they're in you."
    fel ""

translate id w3PunishGame_34c68efe:
    # ver "Hnnngg hhhaaaaa..."
    ver ""

translate id w3PunishGame_6714daf4:
    # fel "Now, I said I would be nice, but there's two of us - and Rosie isn't as sweet as she pretends to be, are you beautiful?"
    fel ""

translate id w3PunishGame_b9c3cddd:
    # rose "Ah... don't blame me... one of us has to be the bad cop..."
    rose ""

translate id w3PunishGame_8c093dd0:
    # rose "It's part of the game, right?"
    rose ""

translate id w3PunishGame_53a608ec:
    # ver "Ghh, hhhngngg--"
    ver ""

translate id w3PunishGame_825c56e9:
    # fel "That's not very convincing when you're smiling, Rosie. It's nice having the shoe on the other foot?"
    fel ""

translate id w3PunishGame_b90e43e5:
    # rose "Not at all..."
    rose ""

translate id w3PunishGame_d3a8edba:
    # fel "Would you rather switch places?"
    fel ""

translate id w3PunishGame_d98a4208:
    # rose "I... uh... didn't say that either."
    rose ""

translate id w3PunishGame_ea293064:
    # fel "Might as well enjoy it then. There's no need for pretenses between us."
    fel ""

translate id w3PunishGame_e3f83ba9:
    # fel "Let your mask come off."
    fel ""

translate id w3PunishGame_e2153337:
    # rose "T-tha, aahh... w-why are you?"
    rose ""

translate id w3PunishGame_722e37a2:
    # fel "Oh, right. Your tits are like mega-sensitive."
    fel ""

translate id w3PunishGame_35e6f356:
    # fel "My bad. Don't want to undermine your part here. What do you got cooking?"
    fel ""

translate id w3PunishGame_7b1bb61b:
    # rose "Well, uhhh... I just thought about what might work on me and... that position you're in, with your chest out... I just know that I would..."
    rose ""

translate id w3PunishGame_e228d7d0:
    # rose "It's better if I just show you."
    rose ""

translate id w3PunishGame_cd226ccf:
    # fel "Well, whatever. Remember, you get the front, I got the back. We're going to two team this slut."
    fel ""

translate id w3PunishGame_8bd88d03:
    # fel "Get ready to have your mind melted, Red!"
    fel ""

translate id w3PunishGame_d466953e:
    # ver "Ghh... hnnanaa, I'm already m-melting, you--"
    ver ""

translate id w3PunishGame_e1f1ab69:
    # ver "G-gahhh, your tongue's not going to--"
    ver ""

translate id w3PunishGame_627c907c:
    # ver "...fiiiiiiiiIIITIiiitttttthhh--!!!"
    ver ""

translate id w3PunishGame_84c5741d_1:
    # kat "I'm surprised you haven't soiled my hand yet."
    kat ""

translate id w3PunishGame_5bb63c34_1:
    # mc "Hnngg!"
    mc ""

translate id w3PunishGame_7f926880_1:
    # kat "I know one time won't suffice. You don't have to fear; I'll milk you as much as you require~"
    kat ""

translate id w3PunishGame_c6f5cfb8:
    # "At this point, a stiff breeze would have gotten me off, but for how slow it was, the old woman's technique was thorough and {i}immaculate{/i} in its consistency. She never slowed down or sped up; she just jerked me like her wrist operated on a piston."
    ""

translate id w3PunishGame_7a26e7dc_1:
    # mc "G-ahh, s-shut the fuck up and just k-keep--!"
    mc ""

translate id w3PunishGame_40ab8cb0_1:
    # "The real magic lay in how she used her pinky, tracing its black nail up and down the underside of my shaft, never deviating a single millimeter from the same tract of overwrought nerve endings."
    ""

translate id w3PunishGame_ca9271b5:
    # kat "*Whisper* You should speak kindly to someone doing you a favor, [mcf]."
    kat ""

translate id w3PunishGame_fa952a65_1:
    # mc "*Gulp* H-a, haa... favor...? Yeah right..."
    mc ""

translate id w3PunishGame_56b5e418:
    # "It was like the words apparated on my lips without even being thought of. I said what I felt without control or filter."
    ""

translate id w3PunishGame_93bce481:
    # mc "T-take a sit and spin on it if you really want to do me a favor!"
    mc ""

translate id w3PunishGame_a2ad0a0e_1:
    # kat "You're adorable right now, you know that?"
    kat ""

translate id w3PunishGame_52443a01:
    # ver "G-uhaaah, y-you're pullingtheeehhmopffhh---!"
    ver ""

translate id w3PunishGame_066326f3:
    # kat "Oh! Look at them stretch!"
    kat ""

translate id w3PunishGame_627c907c_1:
    # ver "...fiiiiiiiiIIITIiiitttttthhh--!!!"
    ver ""

translate id w3PunishGame_c2aac401:
    # kat "Well, well, well... this is taking an interesting turn."
    kat ""

translate id w3PunishGame_a473b670:
    # kat "Oh, how I wish there were two of me."
    kat ""

translate id w3PunishGame_39be1732_1:
    # mc "Hhhngg... y-yeah..."
    mc ""

translate id w3PunishGame_c88ce2aa_1:
    # mct "(I couldn't argue against that...)"
    mct ""

translate id w3PunishGame_56d4a825_1:
    # "With the way I counted it, I had a couple of options for managing my sickening arousal."
    ""

translate id w3PunishGame_10efc326_1:
    # "I could let my mind wander, try to think of something else... but invariably, my senses fell on the women next to me, to the curves of their busts and their waists..."
    ""

translate id w3PunishGame_e4fa996b_1:
    # "These two women were so far above me... the old woman decades my senior. Sophia, a brilliant scientist... it was so easy to imagine the delight in grabbing one of them right now and just--"
    ""

translate id w3PunishGame_2389bacb_1:
    # mct "(Y-yeah... no... I should focus on what's playing out in front of me...)"
    mct ""

translate id w3PunishGame_3427b534_1:
    # "The other option was to let Kathleen's game preoccupy my attention."
    ""

translate id w3PunishGame_e5d1e093_1:
    # kat "A-aah..."
    kat ""

translate id w3PunishGame_d4164143:
    # mct "(...that's gotta be the best way of dealing with this, right?"
    mct ""

translate id w3PunishGame_ca44b0b9_1:
    # kat "Do you need to be excused, Mr. [mcl]? Perhaps you need to go to the bathroom?"
    kat ""

translate id w3PunishGame_c8c82b40_1:
    # mc "N-no, I'm f-fine..."
    mc ""

translate id w3PunishGame_1050a010_1:
    # "She teased me, and strangely, I felt an unusual rush of pride. Of course, it was irrational, but that would be akin to losing..."
    ""

translate id w3PunishGame_54c72150_1:
    # "Losing what and to who, I did not know... but my willpower... I was stronger than this..."
    ""

translate id w3PunishGame_041c023d_1:
    # mct "(I ain't a fucking animal... right?)"
    mct ""

translate id w3PunishGame_52443a01_1:
    # ver "G-uhaaah, y-you're pullingtheeehhmopffhh---!"
    ver ""

translate id w3PunishGame_066326f3_1:
    # kat "Oh! Look at them stretch!"
    kat ""

translate id w3PunishGame_3a3ec578:
    # "Back on screen, they were indeed doubling up on poor Veronica."
    ""

translate id w3PunishGame_80804373:
    # ver "MMmh, hhhhhmm-!"
    ver ""

translate id w3PunishGame_f3a62d2c:
    # "Felicia explored the Amazon's business end with her tongue, while Rosalind frenched the redheaded woman and roughly tugged at her nipples."
    ""

translate id w3PunishGame_3d597928:
    # ver "Ha, mmmwwwhaa~♥"
    ver ""

translate id w3PunishGame_4bb41333:
    # "It wasn't what came to mind when I pictured \"bad cop\", but it wrenched muffled cries from Veronica's throat."
    ""

translate id w3PunishGame_b3acc831:
    # ver "*Chwu, fwwhhup-!* Wwwhhaaauh-!"
    ver ""

translate id w3PunishGame_c2f31a80:
    # "Attacked on both ends, with Dr. Lundgren's poison coursing through her veins, she had little choice but to moan, jerk, and writhe as her brain was inundated with more endorphins than it knew what to do with."
    ""

translate id w3PunishGame_fad0d96a:
    # "*Chwup, fhwup, khhwwup-!*"
    ""

translate id w3PunishGame_1b9b219a:
    # ver "Mmmhh...♥ Hnn...♥"
    ver ""

translate id w3PunishGame_1b7fcdfb:
    # "As the pair persisted, the sweet sounds of lust pouring out from the tv's speakers became more agreeable and proactive."
    ""

translate id w3PunishGame_164d8f89:
    # ver "Ghh, hhhaaa...♥♥ Mmmmhh...♥♥♥"
    ver ""

translate id w3PunishGame_bdeab173:
    # "She was into it."
    ""

translate id w3PunishGame_3d8d16c9:
    # ver "Hhhhyyeah, hhhhh-♥♥♥"
    ver ""

translate id w3PunishGame_a98acb92:
    # "She was {i}definitely{/i} into it."
    ""

translate id w3PunishGame_e336e112:
    # "*Chwaup, fhhwwup!*"
    ""

translate id w3PunishGame_c4be6347:
    # "The only distinction to be made lay in whether it was done for the camera's benefit or to egg the sources of her pleasure on."
    ""

translate id w3PunishGame_85b563da:
    # mct "(G-ahh...!)"
    mct ""

translate id w3PunishGame_bf0b2a15:
    # "What was certain was that the unrestrained sounds of the physically restrained Amazon poured fuel on my already out-of-control lust."
    ""

translate id w3PunishGame_47978ce9:
    # ver "*Shlick, fwhhick-!* Ghhh♥♥♥"
    ver ""

translate id w3PunishGame_24a088e7:
    # "Meanwhile, dwarfed by Veronica's wide ass, Felicia was proving herself to be a dutiful cunt licker."
    ""

translate id w3PunishGame_1b4fe330:
    # ver "Dmmhmhheeeht! Hnnn...!"
    ver ""

translate id w3PunishGame_4cb83b99:
    # "At this angle, her exact technique was left a mystery for the camera, but even when you factored in Veronica's heightened state..."
    ""

translate id w3PunishGame_7c3bb3b5:
    # ver "Ghhh, mmmh, hhhtwwwahht♥♥♥♥♥♥"
    ver ""

translate id w3PunishGame_d007482c:
    # "You could tell she was putting on a riveting performance."
    ""

translate id w3PunishGame_7fff2685:
    # "So the two carried on for about a minute or an eternity, not a single one slowing down."
    ""

translate id w3PunishGame_31f4aaf9:
    # "Felicia licked, Rosalind manhandled, and Veronica accepted what the two offered - all until the MILF pulled herself from her lips."
    ""

translate id w3PunishGame_1aad3548:
    # rose "Mhh-ahh... haa...!"
    rose ""

translate id w3PunishGame_0b0856cc:
    # ver "W-what--♥♥ ahhh, that--hwwwha~!!!"
    ver ""

translate id w3PunishGame_6771e809:
    # rose "Wo-woah... I feel lightheaded... "
    rose ""

translate id w3PunishGame_14123ba3:
    # ver "Ah... that w-was... I mean, w-what is it, my birthday? Heh, haa...♥♥"
    ver ""

translate id w3PunishGame_cfde874a:
    # rose "Ha, hope that was alright... you should always begin foreplay with a kiss."
    rose ""

translate id w3PunishGame_807f8ef7:
    # ver "T-that, aahhh, a p-personal rule of yours...?"
    ver ""

translate id w3PunishGame_68931983:
    # rose "...more of a wish?"
    rose ""

translate id w3PunishGame_afea4de4:
    # ver "Y-you {b}-hnnn!-{/b} e-ever kiss a-"
    ver ""

translate id w3PunishGame_7bb8ff2b:
    # rose "First time."
    rose ""

translate id w3PunishGame_83c0e645:
    # ver "Well, aren't I spe-"
    ver ""

translate id w3PunishGame_1dd6bc23:
    # "*Bzzzt, bzzzzt, bzzzzt!!!!*"
    ""

translate id w3PunishGame_6e7319c6:
    # ver "Hnn...! Fuck! {b}Y-you're moving them around with y-your tongue?!{/b}"
    ver ""

translate id w3PunishGame_1dd6bc23_1:
    # "*Bzzzt, bzzzzt, bzzzzt!!!!*"
    ""

translate id w3PunishGame_84608952:
    # ver "Gyyy-♥♥♥♥♥♥. Y-you sure y-you're s-straight, Blondie?!"
    ver ""

translate id w3PunishGame_32909a43:
    # ver "You're better at that than h-half the--"
    ver ""

translate id w3PunishGame_dd491506:
    # ver "{b}Hhhhnnn--♥♥♥ What do you got t-there?{/b}"
    ver ""

translate id w3PunishGame_9ea3d33a:
    # rose "This? This is going up your keist-- *ahem."
    rose ""

translate id w3PunishGame_c31c2cda:
    # rose "{b}I mean{/b}, this is going up your ass, you... slut."
    rose ""

translate id w3PunishGame_58993c7e:
    # ver "Ahh, hhaaa... y-you're not very convincing in that role, Rosie -- and besides, c'mon... it's kinda already -hnnn!- crowded b-back there!"
    ver ""

translate id w3PunishGame_9713229e:
    # rose "We'll see about that."
    rose ""

translate id w3PunishGame_a20cefa7_4:
    # "..."
    ""

translate id w3PunishGame_4d26f01c:
    # rose "Swap sides with me for a minute, will you?"
    rose ""

translate id w3PunishGame_43f1f69a:
    # "*Shlick, fwhick, fwhick-!*"
    ""

translate id w3PunishGame_bade8b39:
    # fel "E-eh...?"
    fel ""

translate id w3PunishGame_6575b20d:
    # rose "Swap sides with me."
    rose ""

translate id w3PunishGame_31f8053f:
    # ver "Seriously, I don't think that's going to--"
    ver ""

translate id w3PunishGame_7b6ed5f1:
    # ver "*Bzzzzt!* {b}Gaaahfffeeiiiiiit!{/b} Hnn...!!!"
    ver ""

translate id w3PunishGame_5a233002:
    # fel "Of course it will fit. A big girl like her has plenty of room."
    fel ""

translate id w3PunishGame_ecfb23f9:
    # ver "Gh, hhh-- f-fuck!"
    ver ""

translate id w3PunishGame_8e634efb:
    # rose "You're tight..."
    rose ""

translate id w3PunishGame_cdc4def6:
    # ver "Of c-course I am - ahh... I don't use my ass for s-stuff like this!"
    ver ""

translate id w3PunishGame_400b8fa9:
    # ver "*Bzzzzt!* {b}Dddhhhhmmnit, it's m-making them r-rattle! G-gahhh...!"
    ver ""

translate id w3PunishGame_1625af01:
    # rose "Come on, honey. You can take it. Don't push ba--"
    rose ""

translate id w3PunishGame_a3997c91:
    # ver "Ynnnggg-♥♥♥♥ S-sh-eeehhtt!"
    ver ""

translate id w3PunishGame_0fb221f7:
    # rose "Good job! I knew you could do it."
    rose ""

translate id w3PunishGame_eb708c5f:
    # kat "It's always interesting when you give a woman like Rosalind the opportunity to take the reins. They're so stressed, pent up, inhibited..."
    kat ""

translate id w3PunishGame_86537f41:
    # ver "Yyhhhhoooooohhhh♥♥♥♥♥"
    ver ""

translate id w3PunishGame_6d462edb:
    # kat "They don't show you their true colors as much as their whole color palette blends together in one fucked up mix. I wonder where this will go."
    kat ""

translate id w3PunishGame_4a991672:
    # mc "H-hhhhngg...!"
    mc ""

translate id w3PunishGame_274a48d8_1:
    # "Like hell my mind was registering any of her bullshit right now."
    ""

translate id w3PunishGame_70256af8:
    # ver "H-heuuuh?! Y-you're gonna use t-that t-too?!"
    ver ""

translate id w3PunishGame_9e84ae9d_2:
    # sophia "You counted on this kind of... {i}fervor{/i}?"
    sophia ""

translate id w3PunishGame_83473487_1:
    # mc "H-a, hnnggaa..."
    mc ""

translate id w3PunishGame_e7a38a2a_1:
    # "The inside of my head was mush right now. My capacity for language was utterly eroded."
    ""

translate id w3PunishGame_93fd3301_2:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_1abbdcae_2:
    # kat "I'm well acquainted with the peculiar ways people in desperate situations respond. In a way, it's a more potent means of control than any drug."
    kat ""

translate id w3PunishGame_47d2a767_2:
    # sophia "I'm actually impressed for once."
    sophia ""

translate id w3PunishGame_2b5022f9_1:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your owner."
    kat ""

translate id w3PunishGame_d2a0be0e_2:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df_2:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108_2:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418_2:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_ed6f1cb8_2:
    # ver "Huh... that's an interesting direction..."
    ver ""

translate id w3PunishGame_087867e3_2:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_0fb221f7_1:
    # rose "Good job! I knew you could do it."
    rose ""

translate id w3PunishGame_eb708c5f_1:
    # kat "It's always interesting when you give a woman like Rosalind the opportunity to take the reins. They're so stressed, pent up, inhibited..."
    kat ""

translate id w3PunishGame_86537f41_1:
    # ver "Yyhhhhoooooohhhh♥♥♥♥♥"
    ver ""

translate id w3PunishGame_6d462edb_1:
    # kat "They don't show you their true colors as much as their whole color palette blends together in one fucked up mix. I wonder where this will go."
    kat ""

translate id w3PunishGame_7fbd2e1f_1:
    # "To my annoyance, whenever one of them spoke, my attention would turn back to just how fuckable the women next to me were. "
    ""

translate id w3PunishGame_70256af8_1:
    # ver "H-heuuuh?! Y-you're gonna use t-that t-too?!"
    ver ""

translate id w3PunishGame_9e84ae9d_3:
    # sophia "You counted on this kind of... {i}fervor{/i}?"
    sophia ""

translate id w3PunishGame_96a9d185_1:
    # mct "(Christ, I want to make that mouthy hag choke on my dick until she passes out!)"
    mct ""

translate id w3PunishGame_f94b31a8_1:
    # "Intrusive thought upon intrusive thought flooded my head."
    ""

translate id w3PunishGame_93fd3301_3:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_1abbdcae_3:
    # kat "I'm well acquainted with the peculiar ways people in desperate situations respond. In a way, it's a more potent means of control than any drug."
    kat ""

translate id w3PunishGame_47d2a767_3:
    # sophia "I'm actually impressed for once."
    sophia ""

translate id w3PunishGame_5a5d8551_1:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your Master."
    kat ""

translate id w3PunishGame_d2a0be0e_3:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df_3:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108_3:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418_3:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_ed6f1cb8_3:
    # ver "Huh... that's an interesting direction..."
    ver ""

translate id w3PunishGame_087867e3_3:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_193e6115:
    # "*Thwick!*"
    ""

translate id w3PunishGame_9a89e21b:
    # ver "O-ohhh, gghhh..."
    ver ""

translate id w3PunishGame_88f750f9:
    # rose "This is..."
    rose ""

translate id w3PunishGame_193e6115_1:
    # "*Thwick!*"
    ""

translate id w3PunishGame_d0200ee2:
    # rose "This is just like when you and Felicia went head-to-head in the first week."
    rose ""

translate id w3PunishGame_193e6115_2:
    # "*Thwick!*"
    ""

translate id w3PunishGame_4b49c5e0:
    # rose "I didn't experience what it was like to have your butt smacked while it was so stuffed."
    rose ""

translate id w3PunishGame_f72d748b:
    # rose "This is just like when you and I went head-to-head in the first week."
    rose ""

translate id w3PunishGame_193e6115_3:
    # "*Thwick!*"
    ""

translate id w3PunishGame_d603d8a7:
    # rose "We both know what it's like to have your butt clamp down with every impact."
    rose ""

translate id w3PunishGame_f91c4fbf:
    # rose "This is just like when Felicia and I went head-to-head in the first week."
    rose ""

translate id w3PunishGame_193e6115_4:
    # "*Thwick!*"
    ""

translate id w3PunishGame_9bd4a7fb:
    # rose "You didn't experience what it was like to get your butt smacked while it was stuffed."
    rose ""

translate id w3PunishGame_193e6115_5:
    # "*Thwick!*"
    ""

translate id w3PunishGame_7f96a156:
    # ver "Gyjhhh- hhhgnnngg-♥♥♥"
    ver ""

translate id w3PunishGame_5fbb0e51:
    # rose "We should all be used to this sort of play by now..."
    rose ""

translate id w3PunishGame_715a7718:
    # ver "Gah, fuck you! Switch places with me and we'll s-see what kind of ugly sounds y-you make!"
    ver ""

translate id w3PunishGame_9df262e3:
    # rose "Fuck me...?"
    rose ""

translate id w3PunishGame_b4acd330:
    # rose "You were so tough when we were getting shocked. You know, if I'm being honest... {b}it was actually irritating.{/b}"
    rose ""

translate id w3PunishGame_193e6115_6:
    # "*Thwick!*"
    ""

translate id w3PunishGame_c99c2605:
    # ver "Ghh, oohoh--oogogohohhhhhh--"
    ver ""

translate id w3PunishGame_193e6115_7:
    # "*Thwick!*"
    ""

translate id w3PunishGame_62b9d44d:
    # fel "Holy shit, Red. Your pussy is amazing!"
    fel ""

translate id w3PunishGame_8834c44e:
    # rose "Can I ask you something? Why do you act like you're better than Felicia? Better than me?"
    rose ""

translate id w3PunishGame_c0022500:
    # fel "{size=10}Seriously... it was strong enough to...{/size}"
    fel ""

translate id w3PunishGame_9a377a1b:
    # ver "W-what, hhhaaa.. n-no I don't?!"
    ver ""

translate id w3PunishGame_6d451e79:
    # fel "{size=10}I mean, imagine if you were taking a...{/size}"
    fel ""

translate id w3PunishGame_ba798bc3:
    # rose "You constantly act like you're too good to be here."
    rose ""

translate id w3PunishGame_35408b7c:
    # ver "Hnn... wwhhhaay"
    ver ""

translate id w3PunishGame_bb9c21ed:
    # rose "I swallow my pride, yet you always run your mouth and just say what I'm thinking too."
    rose ""

translate id w3PunishGame_cf6165a0:
    # rose "It's annoying. It's fucking arrogant! It's..."
    rose ""

translate id w3PunishGame_278c7e29_2:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_4:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_5:
    # "..."
    ""

translate id w3PunishGame_58faa965:
    # fel "Don't take it personally. I think she's madder at herself than you."
    fel ""

translate id w3PunishGame_6bf8d537:
    # ver "Huuuaahh, s-shut the fuck up--"
    ver ""

translate id w3PunishGame_c9e0d169:
    # rose "Shut up!"
    rose ""

translate id w3PunishGame_73c5469a:
    # fel "Bah, you fucking bitches."
    fel ""

translate id w3PunishGame_0fb78e02:
    # rose "I was thinking..."
    rose ""

translate id w3PunishGame_b3699287:
    # rose "...why should we do any work when we could just let some batteries and gravity do it for us?"
    rose ""

translate id w3PunishGame_01d6aee3_1:
    # kat "I'm starting to get offended, Mr. [mcl]. Does my hand not suffice?"
    kat ""

translate id w3PunishGame_06035a29_1:
    # kat "Don't you want to taste release?"
    kat ""

translate id w3PunishGame_31a83b72_1:
    # mc "G-gah, f-fuck! It's not like I have any control over it!"
    mc ""

translate id w3PunishGame_fd6b335d_1:
    # sophia "Hmm..."
    sophia ""

translate id w3PunishGame_501f6d67_1:
    # "Every fiber of my being desperately anticipated climax. My reason had been eroded and my purpose replaced by the critical need to expunge every trace of nauseating desire from my body through my cock."
    ""

translate id w3PunishGame_15dcbcac_1:
    # "The hairs on my arm stood on end, the feeling in my extremities came and went like the tide, and a pervasive tingling feeling floated from my neck down to my tailbone - by all means, my body told me that I {b}should{/b} be coming."
    ""

translate id w3PunishGame_cc175353_1:
    # sophia "Interesting."
    sophia ""

translate id w3PunishGame_2438123c_1:
    # "But I wasn't. My bloated dick burned against the soft palm of the old woman's hand, pulsating and twitching from the ceaseless stimulation, filling me with the irrational fear that I might literally explode if I didn't soon go careening into the void of sexual release."
    ""

translate id w3PunishGame_5a852968_1:
    # sophia "All things considered, he is showing a {b}surprising{/b} degree of longevity..."
    sophia ""

translate id w3PunishGame_26f61ebe_1:
    # kat "Maybe your fragrance isn't as effective as you think it is."
    kat ""

translate id w3PunishGame_a2f58e6b_1:
    # mc "G-gaahh!"
    mc ""

translate id w3PunishGame_ecfe8072_1:
    # sophia "Maybe..."
    sophia ""

translate id w3PunishGame_46144357_1:
    # mct "(No! It was definitely fucking effective!)"
    mct ""

translate id w3PunishGame_5db42400_1:
    # sophia "...but unlikely. Hey, [mcf]..."
    sophia ""

translate id w3PunishGame_dab66f0d_1:
    # "Through lidded eyes, I could sense the blonde scooching closer and closer."
    ""

translate id w3PunishGame_08efb90d_1:
    # sophia "The issue is that a young, well-traveled guy like you simply can't get off on an old woman's touch, right?"
    sophia ""

translate id w3PunishGame_d36256e0_1:
    # mc "That's not--"
    mc ""

translate id w3PunishGame_93779299_1:
    # sophia "{b}{i}Isn't it?{/b}{/i}"
    sophia ""

translate id w3PunishGame_76f7a8ba_1:
    # "Even in my lust-addled state..."
    ""

translate id w3PunishGame_db1a6cb6_1:
    # "My brain registered amazement over the willful way Sophia flipped the switch, going from detached aloofness to a man-killer just as easily as one would shed their clothes. "
    ""

translate id w3PunishGame_061fb09e_1:
    # mc "Hnngg...!"
    mc ""

translate id w3PunishGame_beef1333_1:
    # kat "*Scoff* Feeling left out, were you?"
    kat ""

translate id w3PunishGame_bfa0648c_1:
    # sophia "Give them a comparison."
    sophia ""

translate id w3PunishGame_18b36fa6_1:
    # "My hand responded independently, sinking into the pleasant give of the doctor's tit-flesh in an instant as what remained of my faculties disappeared into the whirlpool of her eyes."
    ""

translate id w3PunishGame_04a18dd7_1:
    # sophia "You got a mess lurking somewhere in there that you want to get out?"
    sophia ""

translate id w3PunishGame_0f885ed4_1:
    # mc "Ha... hnngg...! F-fuck it!"
    mc ""

translate id w3PunishGame_5d2b485c_1:
    # sophia "...?!"
    sophia ""

translate id w3PunishGame_cc522673_1:
    # "I kissed her, just as you'd expect a man on the brink of starvation to steal a loaf of bread."
    ""

translate id w3PunishGame_02060d5f_1:
    # sophia "Mmmhhhh....!"
    sophia ""

translate id w3PunishGame_8c6f3eee_1:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Mrs. Pulman."
    ""

translate id w3PunishGame_b5e1efa6_1:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Kathleen."
    ""

translate id w3PunishGame_e13d5d67_1:
    # kat "H-hey, ahh--"
    kat ""

translate id w3PunishGame_2430bc3d_1:
    # "Spurred on by having the attention of two beautiful women, even if it was just the fucked up equivalent of cooking ants with a magnifying glass to them, I knew it wouldn't be long until I finally had my release."
    ""

translate id w3PunishGame_87732b5f_1:
    # kat "Gah, ah... ha... hmpfh."
    kat ""

translate id w3PunishGame_353b1cca_1:
    # "Although I could feel her eyes on my neck, the old woman continued to milk me."
    ""

translate id w3PunishGame_61937b68_1:
    # "*Cwhup, fwhup, chwup...!*"
    ""

translate id w3PunishGame_28eed101_1:
    # kat "I've somehow ended up feeling like a third wheel."
    kat ""

translate id w3PunishGame_278c7b3e_1:
    # "I pawed at both women's breasts with a virgin-like fervor, wholly enamored by an opportunity I might never get again."
    ""

translate id w3PunishGame_5d459ae6_1:
    # mct "(The head of a charity and a world-class scientist... ha, fucking sluts!)"
    mct ""

translate id w3PunishGame_e128df9b_1:
    # "I felt like a king, not of my own making of course, but my brain was so cooked as to not know the difference. Thinking was hard, but what few thoughts I could miraculously formulate were like tidal waves breaking against the shore."
    ""

translate id w3PunishGame_0ed4c646_1:
    # mct "(Who the hell do you think you're fucking with? You think you can toy with me just because of money?!)"
    mct ""

translate id w3PunishGame_b117f61e_1:
    # "I felt smothered, suffocated, and hot. More akin to a wick that was about to burn out than a human being."
    ""

translate id w3PunishGame_9c75dc6b_1:
    # mct "(I'M A GUINEA PIG TO YOU?!)"
    mct ""

translate id w3PunishGame_07bbfc1c_1:
    # "Every sensation felt heightened, and every second felt prolonged... I genuinely feared that the moment I popped would be when I was snuffed out."
    ""

translate id w3PunishGame_a73faf04_1:
    # mct "{size=+30}{font=/gui/fonts/MB-Thin_Worms.ttf}(You like to toy with pe-){/font}{/size}"
    mct ""

translate id w3PunishGame_61867711_1:
    # "My brain gave out, but if I ceased to exist after this, I instinctively knew one thing: these last minutes of my life should be bold."
    ""

translate id w3PunishGame_ee7c2e13_1:
    # mc "I'm about to b-blow..."
    mc ""

translate id w3PunishGame_01a79887_1:
    # kat "You don't need my permission."
    kat ""

translate id w3PunishGame_2221f5ba_1:
    # mc "Can you finish me with your mouth?"
    mc ""

translate id w3PunishGame_fde31e51_1:
    # kat "Hmmmmm, not going to happen."
    kat ""

translate id w3PunishGame_eefd493d_1:
    # mc "A-ah, pretty please? With a cherry on top?"
    mc ""

translate id w3PunishGame_4469f1d1_1:
    # kat "Why don't you ask Miss Lundgren to do it?"
    kat ""

translate id w3PunishGame_6032c009_1:
    # mc "...because I want you, Ma'am."
    mc ""

translate id w3PunishGame_7d48d431_1:
    # kat "...hmmmmmmmm. You have the eyes of a crazed animal, yet..."
    kat ""

translate id w3PunishGame_2d6e0df6_1:
    # kat "You're asking so nicely. God, I love that."
    kat ""

translate id w3PunishGame_af9d9795_1:
    # "She smirked like she had won against me, and perhaps even against Sophia."
    ""

translate id w3PunishGame_76125b85_1:
    # kat "It really is goddamn adorable. Ask me again, [mcf]."
    kat ""

translate id w3PunishGame_8ce7b843_1:
    # "A desperate, cloying urge inside me told me her mouth was the only place I wanted to be right now and if it took a bit of pleading to get me into a position where I could fill that mean bitch's stomach with my seed, then so be it."
    ""

translate id w3PunishGame_ca96d738_1:
    # mc "Hhh, haa... would you PLEASE finish me with your mouth, Mrs. Pulman?"
    mc ""

translate id w3PunishGame_e2b980af_1:
    # "Without another word, Mrs. Pulman lowered herself and..."
    ""

translate id w3PunishGame_fc8cf5a3_1:
    # "Without another word, Kathleen lowered herself and..."
    ""

translate id w3PunishGame_109a7847_1:
    # mc "G-gah...!"
    mc ""

translate id w3PunishGame_d66b4def_1:
    # "The moment her lips wrapped themselves around my cock, I felt like I might pass out."
    ""

translate id w3PunishGame_cca0b4ef_1:
    # "Her seal was so airtight that her mouth felt like an oven. Steady puffs of hot breath passed over the head of my cock as it made its way out through her nose."
    ""

translate id w3PunishGame_d6fb4ea7_1:
    # mc "F-fuck, damn it!"
    mc ""

translate id w3PunishGame_1bd17e11_1:
    # "*Shwuck, fgwhhuck, thwuuuck!*"
    ""

translate id w3PunishGame_b39227ac:
    # "What little air could escape from her hold transformed into {b}obscene{/b} noises as she inched herself up and down my shaft."
    ""

translate id w3PunishGame_63238d75_1:
    # mc "H-holy-!"
    mc ""

translate id w3PunishGame_ad464d04_1:
    # "The running narration in my head was a jumbled mess, but through all the muddied noise from the pleasure soaking into my brain, I grasped a simple truth."
    ""

translate id w3PunishGame_fdbb746f_1:
    # mc "Hnnng, you've sucked a lot of cock in your time, haven't you?!"
    mc ""

translate id w3PunishGame_dad9bfec_1:
    # "*Thwaahp, fwhap, swhhuck~!*"
    ""

translate id w3PunishGame_72c8cf4a_1:
    # "The answer to that question played across my glans every time I wedged myself comfortably in her throat with zero fuss."
    ""

translate id w3PunishGame_6b12669e_1:
    # "Swhuck, hhhwhuck, fhhggguuk!*"
    ""

translate id w3PunishGame_a3af0240_1:
    # "{b}Damn it!{/b} A rich woman like her had no business sucking dick this well."
    ""

translate id w3PunishGame_d86d228f_1:
    # mc "Ghhn, haaat, hnnngg...!"
    mc ""

translate id w3PunishGame_88387910_1:
    # "She sucked me with the skill of a woman who had {b}nothing{/b} to her name."
    ""

translate id w3PunishGame_cdfdc63e_1:
    # kat "Mmmhm, hhmhm~"
    kat ""

translate id w3PunishGame_93b1b0e6_1:
    # "Just like her hand job, there was no wasted effort. She didn't tackle it in a clumsy frenzy, nor did she need to cautiously get used to my size."
    ""

translate id w3PunishGame_d628853d_1:
    # "*Shwhuhuck, fwhhucuk, ghhwhuhuk!*"
    ""

translate id w3PunishGame_41aa32dc_1:
    # "She sucked me like she had been sucking {b}my{/b} dick her whole life, pleasuring me at a steady and comfortable pace. I marveled at how, through a mere blowjob, a woman so cold and cruel could instantly imprint herself on my body as warm and welcoming."
    ""

translate id w3PunishGame_d93b0c5e_1:
    # "I was quickly understanding that \"adorable\" disparity she had just spoken."
    ""

translate id w3PunishGame_7a3794d6_1:
    # mc "Hnng, h-how?!"
    mc ""

translate id w3PunishGame_2aa14e7d_1:
    # sophia "It's not performing brain surgery."
    sophia ""

translate id w3PunishGame_9f2a1802_1:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Mrs. Pulman's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_bc9afe80_1:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Kathleen's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_7f97a9d7_1:
    # mct "(Haaaa... hope you like the show, bitch. You're the impetus for this.)"
    mct ""

translate id w3PunishGame_0d35f1a2_1:
    # "*Fwhhuck, ghhhukh, khggguuhhk*"
    ""

translate id w3PunishGame_b1ef8758_1:
    # mc "Ahh... y-you dick swallowing--! Hnnngg...!"
    mc ""

translate id w3PunishGame_57623603_1:
    # "It was coming."
    ""

translate id w3PunishGame_a9d0bbe1_1:
    # mc "You think I'm adorable?!"
    mc ""

translate id w3PunishGame_f8d06f5f_1:
    # "I was moments from blowing my load, and with it, the indignation from earlier swelled in me. I had wanted a bold finish before I met my end, and well..."
    ""

translate id w3PunishGame_26cd0617_1:
    # mct "(I'll show you adorable!)"
    mct ""

translate id w3PunishGame_72c12088_1:
    # mc "I'm c-cumming, fucking drown in it, you damn--"
    mc ""

translate id w3PunishGame_ec061ea7_1:
    # kat "Mmmmh?!"
    kat ""

translate id w3PunishGame_5c240311_1:
    # "Before she could even register what was happening, ropes of piping hot cum barreled down her esophagus on a one-way trip to her stomach."
    ""

translate id w3PunishGame_10060088_1:
    # kat "Ghhkka, hkkaa-!"
    kat ""

translate id w3PunishGame_4bb38f46_1:
    # "I pressed down hard, holding the old woman's head in place as she gargled my seed in abject surprise."
    ""

translate id w3PunishGame_2745bcd2_1:
    # mc "Hnngg, all of it!"
    mc ""

translate id w3PunishGame_716f46e4_1:
    # "I pressed down so hard it hurt my groin."
    ""

translate id w3PunishGame_4e4c5b02_1:
    # kat "Ghhuk, hhuuuh, hwwwhuuuck...!"
    kat ""

translate id w3PunishGame_fe1ef6b3_1:
    # "It was massive, like I was cumming my very soul down the old woman's throat."
    ""

translate id w3PunishGame_4500ed7e_1:
    # kat "Ghuk, khak, uuhhk-!"
    kat ""

translate id w3PunishGame_2239e61c_1:
    # mc "Gah, c-choke on it!"
    mc ""

translate id w3PunishGame_ab7d9ac7_1:
    # "Not all of my load was making its way down her throat. To my perverse glee, I could {i}feel{/i} myself coating the lining of her throat."
    ""

translate id w3PunishGame_7b62aa4f_1:
    # kat "Ghhuk, hhuuuk...!"
    kat ""

translate id w3PunishGame_e09b7aa7_1:
    # mc "Hope you like the fucking taste!"
    mc ""

translate id w3PunishGame_8747fdde_1:
    # kat "Mmmh, hhhngg?!?!"
    kat ""

translate id w3PunishGame_3698f7b8_1:
    # sophia "I don't think she can breathe..."
    sophia ""

translate id w3PunishGame_6d4b68da_1:
    # mc "Good! She can breathe when I'm fucking--!"
    mc ""

translate id w3PunishGame_de4a6cab_1:
    # ".........!"
    ""

translate id w3PunishGame_2068a0d5_1:
    # "......!"
    ""

translate id w3PunishGame_1d9b8a71_1:
    # mc "A-aahh...!"
    mc ""

translate id w3PunishGame_869e1b4e_1:
    # kat "Ugghhuhh..."
    kat ""

translate id w3PunishGame_ba8a394f_2:
    # "Peace."
    ""

translate id w3PunishGame_a48af359_2:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d_2:
    # "Satisfaction."
    ""

translate id w3PunishGame_92962e6b_1:
    # "In this moment, I had it all."
    ""

translate id w3PunishGame_e38ecbb2_1:
    # "With that, I happily died. The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8b8ceb72_2:
    # "............"
    ""

translate id w3PunishGame_278c7e29_3:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_5:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_6:
    # "..."
    ""

translate id w3PunishGame_87f2fe4b_1:
    # kat "Why are you holding your hands out like that? It's weird."
    kat ""

translate id w3PunishGame_2dd84f14_2:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_c2d77e46_1:
    # sophia "She's right. It's distracting."
    sophia ""

translate id w3PunishGame_1516ad37_1:
    # "I didn't know how long I was out of it, but by the time I opened my eyes, the two women had collected themselves, and what I thought was a bold display of defiance was made pitiful and wholly expected by their bemused faces."
    ""

translate id w3PunishGame_69e22dc5_1:
    # "Mrs. Pulman looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_d1fa2418_1:
    # "Kathleen looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_e7145261_1:
    # kat "Oh, by the way... you said some pretty outlandish things to me."
    kat ""

translate id w3PunishGame_526a5615:
    # mc "If I recall, I did a bit more than that..."
    mc ""

translate id w3PunishGame_2e195b22_1:
    # kat "It was {b}adorable{/b}."
    kat ""

translate id w3PunishGame_c5563c4b_1:
    # "She had indeed won."
    ""

translate id w3PunishGame_9a64515e:
    # ver "Ghh, hhaan, streetthhhchggingg-!!!"
    ver ""

translate id w3PunishGame_d9d8b219_1:
    # "I knew what I wanted."
    ""

translate id w3PunishGame_3576545e_1:
    # sophia "Mmmhh, hhhhh..."
    sophia ""

translate id w3PunishGame_7202d9ed_1:
    # "And that last act of defiance would be leaving the doctor, the root cause of all this, something to remember me by."
    ""

translate id w3PunishGame_293b8be4_1:
    # "*Chwup, fhwup~*"
    ""

translate id w3PunishGame_7a9110af_1:
    # "I wanted to hear this aloof bitch moan, so I focused everything on the woman whose mouth I was currently exploring."
    ""

translate id w3PunishGame_01fdbaa9_1:
    # sophia "Mmmh, hhmmm...?"
    sophia ""

translate id w3PunishGame_1efdf9e0_1:
    # "It was a battle against my wits. Sophia was literally intoxicating. Focusing my senses all on her had me burning twice as fast."
    ""

translate id w3PunishGame_d14b5b96_1:
    # "By instinct, I felt her breast over her clothes."
    ""

translate id w3PunishGame_79a3ccdd_1:
    # sophia "Mmhh..."
    sophia ""

translate id w3PunishGame_a90ff98f_1:
    # "Her coos told me she was receptive, but it wasn't quite the noise I wanted to drag out from the blonde."
    ""

translate id w3PunishGame_754fce10_1:
    # sophia "Ghu, gha~ghhuu...!"
    sophia ""

translate id w3PunishGame_be4204b5_1:
    # "Deciding that over the clothes touching wouldn't do it, I switched focus on the erogenous zone in her mouth, intensifying my efforts in an intricate dance to subdue her tongue with my own."
    ""

translate id w3PunishGame_f43fc47f_1:
    # sophia "Mmmmh, mmhhhh- *chwuph, fhwhup~*"
    sophia ""

translate id w3PunishGame_b0084126_1:
    # "She wasn't a passive observer in my hunt; her tongue played coy, darting away and slipping from my grasp whenever I thought I had her pinned."
    ""

translate id w3PunishGame_a03c3549_1:
    # "It was mere seconds, but--"
    ""

translate id w3PunishGame_27c1fdac_1:
    # "*Chwhu, fhwwwup, chup!*"
    ""

translate id w3PunishGame_f4de7f68_1:
    # "--but, in my fogged-over state, time held little sway over my perception. What was in reality a clumsy tussle felt like a long and protracted siege."
    ""

translate id w3PunishGame_2ec99952_1:
    # "I don't know if it was because of the perfume, but all bodily sensations gradually began to meld into one."
    ""

translate id w3PunishGame_45c987c8_1:
    # "The feeling of the old woman's hand milking my cock, the wet warmness of our kiss, the bubbling desire so strong that I was confident it would be the end of me..."
    ""

translate id w3PunishGame_15b0522f_1:
    # mc "Mmmh, hhhmmm-!"
    mc ""

translate id w3PunishGame_7c16ed74_1:
    # "All of it spelled an overload melded and combined into a singular feeling of otherness. I was sure I had lost this battle until..."
    ""

translate id w3PunishGame_7d144a01_1:
    # "My brain registered a change."
    ""

translate id w3PunishGame_fbfb812a_1:
    # mct "Was she...?"
    mct ""

translate id w3PunishGame_7459555f_1:
    # sophia "Mmmh, hhmhm~aahhh...!"
    sophia ""

translate id w3PunishGame_68d04ab6_1:
    # "She was closer."
    ""

translate id w3PunishGame_08a39e71_1:
    # "She was pressing into me."
    ""

translate id w3PunishGame_883ad25a_1:
    # "She felt it."
    ""

translate id w3PunishGame_b0685c2c_1:
    # "She was {b}heated{/b}."
    ""

translate id w3PunishGame_5c6a5db6_1:
    # "{i}She was kissing me back.{/i}"
    ""

translate id w3PunishGame_26629cc2_1:
    # "Whether it was done deliberately or on an instinctual level, her body responded favorably."
    ""

translate id w3PunishGame_a4d39526_1:
    # sophia "Ahh, hhhnnggg, mmmmhh-!"
    sophia ""

translate id w3PunishGame_d8ca8c90_1:
    # "My hand found a place on her ass and I used that to spur her on even more. With this, I had physical confirmation of my success."
    ""

translate id w3PunishGame_44602cec_1:
    # mct "(Fuck, what a great-!)"
    mct ""

translate id w3PunishGame_9fec863b_1:
    # "I could feel the hot dampness of her delicate spot."
    ""

translate id w3PunishGame_cb435f7f_1:
    # sophia "Mmhh, hhaa...♥♥♥"
    sophia ""

translate id w3PunishGame_71583afd_1:
    # "She purred openly and I felt a petty relief over my trivial victory. The succubus driving me to this desperate state wouldn't escape our battle unscathed."
    ""

translate id w3PunishGame_3d67a16d_1:
    # "*Chwup, fhwwup, khwwup*!"
    ""

translate id w3PunishGame_b6fdc206_1:
    # "--and that wasn't the only sense of relief I felt."
    ""

translate id w3PunishGame_cb9361ae_1:
    # mc "{b}Hnng, ah-!!!{/b}"
    mc ""

translate id w3PunishGame_cbea8557_1:
    # "I didn't know when it started, but ropes of cum geysered from my cock and rained down from the sky."
    ""

translate id w3PunishGame_4380fa10_1:
    # mc "Ghh, aaahh- sssshit--"
    mc ""

translate id w3PunishGame_a3230c6c_1:
    # kat "Huh, {b}WOW{/b}."
    kat ""

translate id w3PunishGame_74ff6633_1:
    # mc "Hngg, hhhhmmmhh-~"
    mc ""

translate id w3PunishGame_290360e4_1:
    # "All the suffocating vileness in me was expelled and released into the world. All my desperation spilled out onto our clothes and thighs."
    ""

translate id w3PunishGame_83af968d_1:
    # "Soon need was replaced with..."
    ""

translate id w3PunishGame_ba8a394f_3:
    # "Peace."
    ""

translate id w3PunishGame_a48af359_3:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d_3:
    # "Satisfaction."
    ""

translate id w3PunishGame_2a3c0c6e_1:
    # "Emptiness."
    ""

translate id w3PunishGame_ea0f012b_1:
    # "The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8728f5e5_1:
    # "I was dead."
    ""

translate id w3PunishGame_8b8ceb72_3:
    # "............"
    ""

translate id w3PunishGame_278c7e29_4:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_6:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_7:
    # "..."
    ""

translate id w3PunishGame_c49ce7ae_1:
    # sophia "Great, now we'll spend the rest of the show reeking of semen."
    sophia ""

translate id w3PunishGame_2dd84f14_3:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_2f1146d1_1:
    # kat "You should've thought of that before you entered the crossfire."
    kat ""

translate id w3PunishGame_9177b213_1:
    # sophia "You were taking too long. I wanted to put an end to the distraction."
    sophia ""

translate id w3PunishGame_0e5cc7d3_1:
    # "I didn't know how long I was out of it. Still, by the time I opened my eyes, the two women had collected themselves. The damage I thought I had done to Sophia with my counter-attack was made pitiful by their bemused faces."
    ""

translate id w3PunishGame_23aa7f4f_1:
    # kat "Oh, please. Are those the kind of sounds you make matter-of-factly?"
    kat ""

translate id w3PunishGame_a5d22b22_1:
    # "Sophia looked as composed as ever, and except for a little blush, her attention turned back to the Carnations."
    ""

translate id w3PunishGame_449fa9fb_1:
    # kat "I hope that provided you a measure of relief, Mr. [mcl]. That certainly was an... {i}explosive{/i} finish."
    kat ""

translate id w3PunishGame_b85bc4ed_1:
    # mc "A, ahem... thaaaaaanks?"
    mc ""

translate id w3PunishGame_fada21bd_1:
    # kat "Don't mention it. It was... interesting."
    kat ""

translate id w3PunishGame_9a64515e_1:
    # ver "Ghh, hhaan, streetthhhchggingg-!!!"
    ver ""

translate id w3PunishGame_dddb6f7e_1:
    # mc "........."
    mc ""

translate id w3PunishGame_3b21f354_1:
    # mc "......"
    mc ""

translate id w3PunishGame_5b4c9fd4_1:
    # "...this was my limit."
    ""

translate id w3PunishGame_964e84bc_1:
    # mc "Aaeeh, I'll be riiiiiight back."
    mc ""

translate id w3PunishGame_0fd4d656_7:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_8:
    # "..."
    ""

translate id w3PunishGame_e6cfc671_1:
    # kat "Is that a milder scent you are wearing?"
    kat ""

translate id w3PunishGame_837e9b2b_1:
    # sophia "It's the same batch as the bottle I gave you."
    sophia ""

translate id w3PunishGame_256d5f3e_1:
    # kat "The effect seemed... less? Why's that?"
    kat ""

translate id w3PunishGame_548f5e80_1:
    # sophia "It's possible that his body adapted and partially metabolized the odor molecules. All I can say is that it's remarkable he held out that long."
    sophia ""

translate id w3PunishGame_b4f48c1e_1:
    # sophia "It could be a matter of willpower."
    sophia ""

translate id w3PunishGame_caedd2e6_1:
    # kat "When I had dinner with Charles the other night, he didn't even flinch."
    kat ""

translate id w3PunishGame_b5500f59_1:
    # sophia "Dr. Kohler is a scary man."
    sophia ""

translate id w3PunishGame_4510a9da_1:
    # kat "Is that your appraisal after demolishing him game after game?"
    kat ""

translate id w3PunishGame_64ecae1b_1:
    # sophia "How else should I describe a man who has no need but keeps his passions tightly controlled."
    sophia ""

translate id w3PunishGame_610ec2fb_1:
    # kat "That's why he and I are incompatible. He treats fucking like it's Christmas."
    kat ""

translate id w3PunishGame_560752c9_1:
    # kat "You don't have anything to worry about as long as you don't lose."
    kat ""

translate id w3PunishGame_b19fae41_1:
    # sophia "I don't plan on it, but I have a feeling he eventually wins one way or another."
    sophia ""

translate id w3PunishGame_f588a7d6_1:
    # kat "Abel wouldn't let it come to that, would he?"
    kat ""

translate id w3PunishGame_0fd4d656_8:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_9:
    # "..."
    ""

translate id w3PunishGame_1324c38b_1:
    # sophia "Thank you for indulging my little whim, Kathleen. It was amusing."
    sophia ""

translate id w3PunishGame_4db92c34_1:
    # kat "Don't mention it. One should make their own fun in life. Still..."
    kat ""

translate id w3PunishGame_0cb22879_1:
    # kat "...you're very competent. Don't you ever desire more?"
    kat ""

translate id w3PunishGame_0cdff961_1:
    # sophia "A frivolous bitch like you has no business saying that to me."
    sophia ""

translate id w3PunishGame_9765acec_1:
    # kat "Ah, fuck. There it is."
    kat ""

translate id w3PunishGame_55e6ab51_1:
    # kat "I almost felt sorry for you."
    kat ""

translate id w3PunishGame_fe051951:
    # ver "Hnngggg- hhaaa-"
    ver ""

translate id w3PunishGame_d65df59a_1:
    # rose "......"
    rose ""

translate id w3PunishGame_d6ce9717_1:
    # fel "..."
    fel ""

translate id w3PunishGame_4794e259:
    # ver "Waahh, wwwwhhhuuuat-"
    ver ""

translate id w3PunishGame_d65df59a_2:
    # rose "......"
    rose ""

translate id w3PunishGame_d6ce9717_2:
    # fel "..."
    fel ""

translate id w3PunishGame_5ff796c4:
    # ver "Ggaahhhk, y-you jerks are really j-just gonna stand there?! H-hhhaaa...!"
    ver ""

translate id w3PunishGame_a6aafe90:
    # fel "I mean, I started out wanting to play nice, but Rosalind..."
    fel ""

translate id w3PunishGame_1f6ba896:
    # rose "This is just part of the game."
    rose ""

translate id w3PunishGame_b19576e2:
    # fel "She does have a point."
    fel ""

translate id w3PunishGame_454566da:
    # ver "Haaa, phhh- can't you j-just- *Bzzzt, bzzzt!* hnnn!"
    ver ""

translate id w3PunishGame_ff93c83e:
    # fel "It's not like you're not getting any satisfaction. How many times have those toys made you cum?"
    fel ""

translate id w3PunishGame_0db57903:
    # ver "Gaa-h, it's n-not enough! E-every time I think I'm c-coming down-- I g-gett hho-tt, ag--gaggin..! Hnn!!"
    ver ""

translate id w3PunishGame_5141fdae:
    # ver "C'mon... u-use your mouth again, that was more..."
    ver ""

translate id w3PunishGame_b219741b:
    # fel "Yeesh. I'm feeling kinda..."
    fel ""

translate id w3PunishGame_4ac5f38c:
    # rose "We're supposed to be punishing her."
    rose ""

translate id w3PunishGame_9549efde:
    # fel "...she does have a point. Should we focus on winning?"
    fel ""

translate id w3PunishGame_d0d4dc70:
    # ver "Guhhk, hhhnnanaaa-♥ D-damn it! Focus on winning?!"
    ver ""

translate id w3PunishGame_c0d6cacd:
    # ver "W-what the hell do you stand to w-win here?! The company of other a-sshhholehhh?!"
    ver ""

translate id w3PunishGame_24cbdcbc:
    # fel "We all have our reasons, Red."
    fel ""

translate id w3PunishGame_2512a002:
    # ver "Your reason is bullshit!"
    ver ""

translate id w3PunishGame_93c6977b:
    # fel "Why should I care about your sob stories? It doesn't matter who wins, it only matters if you lose."
    fel ""

translate id w3PunishGame_90b21e04:
    # fel "Do you think Rose will give a shit if you win over me? It's all the same to her as it would be to you."
    fel ""

translate id w3PunishGame_a15b72f3:
    # ver "..y-yeah?"
    ver ""

translate id w3PunishGame_649afa68:
    # fel "You should be happy to have a bored rich bitch as your opponent. Would you rather three desperate people get paraded like show dogs over two?"
    fel ""

translate id w3PunishGame_f32a8883:
    # ver "If you win, two desperate people are h-hosed! Gg-ah-♥"
    ver ""

translate id w3PunishGame_7674f0e6:
    # fel "The math's the same either way. Just focus on what you have to do."
    fel ""

translate id w3PunishGame_abd837cd_2:
    # rose "..."
    rose ""

translate id w3PunishGame_4ce2d13c:
    # ver "What the fuck are you even talking about?! Ahhhhh, d-damn it it pisses me off!"
    ver ""

translate id w3PunishGame_da7c9634:
    # ver "W-when you l-lose, you can go back to your d-damn penthouse! Take a dip in your pool!"
    ver ""

translate id w3PunishGame_6cf9dd6e:
    # fel "Sure I can."
    fel ""

translate id w3PunishGame_066a5d75:
    # ver "I get why you want to join! Y-you're not any fucking different than the old cunt!"
    ver ""

translate id w3PunishGame_345b6025:
    # ver "You'd fit right in r-rreeeaeehhhl -hnnn!- nicely, if only y-you weren't b-born {b}TRAILER TRASH!{/b}"
    ver ""

translate id w3PunishGame_d6ce9717_3:
    # fel "..."
    fel ""

translate id w3PunishGame_75589f4e:
    # rose "Ah... ha... heh, c'mon... this is all a game. Don't-"
    rose ""

translate id w3PunishGame_54999ce8:
    # fel "We're just going in circles - just hang there and cum, Red. It'll be more fun that way."
    fel ""

translate id w3PunishGame_2b194d62:
    # ver "You--"
    ver ""

translate id w3PunishGame_c61c4c63:
    # ver "Gwwwhhhhahahahahhhh- hh-"
    ver ""

translate id w3PunishGame_81ab84e1:
    # ver "Huuuahhhht-! C-ccumm--!"
    ver ""

translate id w3PunishGame_dff81826:
    # fel "Shit... right on cue..."
    fel ""

translate id w3PunishGame_c4f55ef4:
    # ver "Hhgnn, ugugghh--♥♥♥♥"
    ver ""

translate id w3PunishGame_d79ff74e:
    # fel "Guess we're finished."
    fel ""

translate id w3PunishGame_9fa4b1ca:
    # rose "I doubt it. We should probably keep going until Mrs. Pulman stops us."
    rose ""

translate id w3PunishGame_b699991e:
    # fel "How long do you think that's going to be?"
    fel ""

translate id w3PunishGame_e1289893:
    # rose "Who knows."
    rose ""

translate id w3PunishGame_d6a1cabd:
    # rose "There's only one of me and two of you..."
    rose ""

translate id w3PunishGame_d65df59a_3:
    # rose "......"
    rose ""

translate id w3PunishGame_abd837cd_3:
    # rose "..."
    rose ""

translate id w3PunishGame_997203d6:
    # rose "Do you girls have a preference on who I should start with?"
    rose ""

translate id w3PunishGame_04074c19:
    # ver "Of-c-couuuurse n-ot--"
    ver ""

translate id w3PunishGame_f76789b3:
    # fel "M-meeeh! I'm d-dying here! -Hnnn- C'mon!"
    fel ""

translate id w3PunishGame_0a17cddf:
    # rose "Is it really that bad?"
    rose ""

translate id w3PunishGame_cdf09485:
    # fel "M-my whole body is on f-fire! I can't even think without--"
    fel ""

translate id w3PunishGame_f6a93b78:
    # rose "Then I guess you'll have to sit tight while I give Veronica a hand."
    rose ""

translate id w3PunishGame_27cc8d90:
    # fel "W-whaaa...?"
    fel ""

translate id w3PunishGame_85d05eee:
    # rose "She's hurting just as bad as you. The only difference is, well... she's not... {b}you{/b}."
    rose ""

translate id w3PunishGame_fd1b5462:
    # fel "H-huh? What do you mean by that?!"
    fel ""

translate id w3PunishGame_4472ba1a:
    # rose "It's nothing personal. Just... trying to be utilitarian here."
    rose ""

translate id w3PunishGame_e0b294f2:
    # fel "Bullshit, you two-faced bitch!"
    fel ""

translate id w3PunishGame_1943b1f3:
    # rose "Is that how you ask nicely for something?"
    rose ""

translate id w3PunishGame_97937b8f:
    # fel "You want me to beg?!"
    fel ""

translate id w3PunishGame_60633ff5:
    # fel "Is t-that what you did with [mcf] when you got him barking up my tree for some money? O-or m-maybe you did something more like me than you'd care to--"
    fel ""

translate id w3PunishGame_50da98b2:
    # rose "No. I want you to be patient and \"sit there\" quietly. This is supposed to be a punishment."
    rose ""

translate id w3PunishGame_e3b1e1bf:
    # rose "Why would I just give you what you ask for?"
    rose ""

translate id w3PunishGame_07722c23:
    # fel "A-aahh, y-yeah right... you and Red don't like me, do you?"
    fel ""

translate id w3PunishGame_beb24fed:
    # rose "I'm grateful for your money, but... do you want me to be honest?"
    rose ""

translate id w3PunishGame_29383ebd:
    # rose "Do you want me to be honest?"
    rose ""

translate id w3PunishGame_d65df59a_4:
    # rose "......"
    rose ""

translate id w3PunishGame_abd837cd_4:
    # rose "..."
    rose ""

translate id w3PunishGame_5744ddc8:
    # rose "I don't care about either of you one way or another."
    rose ""

translate id w3PunishGame_f926f7fd:
    # ver "H-haah! See? Not everything is about you, Blondie!"
    ver ""

translate id w3PunishGame_d3e3e75e:
    # ver "Some of us have heavier shit on our mi--"
    ver ""

translate id w3PunishGame_61ffa0f8:
    # ver "H--hhhnaaaaahh, fhhh-ddd--ahk--"
    ver ""

translate id w3PunishGame_7e4ff8df:
    # rose "I wish you'd sit here quietly too."
    rose ""

translate id w3PunishGame_a4835ca5:
    # kat "It didn't take long for Rosalind to become comfortable with the situation, did it?"
    kat ""

translate id w3PunishGame_12858723_4:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_82ab18ae:
    # kat "Sluts like her sure are adaptable. It's a strength, but it can be a little boring when they're totally obedient."
    kat ""

translate id w3PunishGame_4b026443_1:
    # "No, in my current state, my mind was completely consumed by the old woman's grip on my cock."
    ""

translate id w3PunishGame_3a010861_2:
    # mc "H-haahaa...!"
    mc ""

translate id w3PunishGame_1a2c9cc0:
    # sophia "Unlike Lynch?"
    sophia ""

translate id w3PunishGame_93651fc0_2:
    # mc "G-gahh...!"
    mc ""

translate id w3PunishGame_22eda143_2:
    # "The pitiful noise of a rutting animal punctuated the women's conversation."
    ""

translate id w3PunishGame_4da5704e:
    # kat "Exactly! Ah, I love it! I really do love Miss Lynch. I wish I had more than a month with her."
    kat ""

translate id w3PunishGame_7d9cd6ca:
    # kat "Still, it should be fun to watch Mrs. Carter let loose. Of course, she'll justify it was done under instruction, but she won't be able to hide her enjoyment of this from the other two."
    kat ""

translate id w3PunishGame_5ddac938_4:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_66527633_2:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious..."
    kat ""

translate id w3PunishGame_5f8d0d83_2:
    # mc "Huuuhghh...!"
    mc ""

translate id w3PunishGame_a7f739bd_2:
    # kat "What gets a woman like you going?"
    kat ""

translate id w3PunishGame_16f23753_2:
    # mc "H-huhh... ack...!"
    mc ""

translate id w3PunishGame_90eda0c7_4:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019_4:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355_4:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_4c9db659_2:
    # kat "We got that in common at least - {b}all three of us.{/b}"
    kat ""

translate id w3PunishGame_29e481f2_4:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_1f22ee2d:
    # kat "Oh, look at them stretch!"
    kat ""

translate id w3PunishGame_a4835ca5_1:
    # kat "It didn't take long for Rosalind to become comfortable with the situation, did it?"
    kat ""

translate id w3PunishGame_12858723_5:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_82ab18ae_1:
    # kat "Sluts like her sure are adaptable. It's a strength, but it can be a little boring when they're totally obedient."
    kat ""

translate id w3PunishGame_73380823_1:
    # mc "She... ah..."
    mc ""

translate id w3PunishGame_1a2c9cc0_1:
    # sophia "Unlike Lynch?"
    sophia ""

translate id w3PunishGame_4da5704e_1:
    # kat "Exactly! Ah, I love it! I really do love Miss Lynch. I wish I had more than a month with her."
    kat ""

translate id w3PunishGame_7d9cd6ca_1:
    # kat "Still, it should be fun to watch Mrs. Carter let loose. Of course, she'll justify it was done under instruction, but she won't be able to hide her enjoyment of this from the other two."
    kat ""

translate id w3PunishGame_5ddac938_5:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_1c55b166_2:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious... what gets a woman like you going?"
    kat ""

translate id w3PunishGame_90eda0c7_5:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019_5:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355_5:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_68542e62_2:
    # kat "Oh, lovely, at least we have that in common."
    kat ""

translate id w3PunishGame_29e481f2_5:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_1f22ee2d_1:
    # kat "Oh, look at them stretch!"
    kat ""

translate id w3PunishGame_c2108a56:
    # ver "Hnngg, w-what the fuck?! My- ahh... p-pulling- hnhnn!"
    ver ""

translate id w3PunishGame_5289c621:
    # "Back on screen, Rosalind had selected an interesting tool and had carefully suctioned them around Veronica's pale teats."
    ""

translate id w3PunishGame_cea4da3c:
    # ver "HHh-hhh... ff-"
    ver ""

translate id w3PunishGame_58cb7aa2:
    # ver "Woooaoahhhnnn--"
    ver ""

translate id w3PunishGame_2b3cdd56:
    # rose "Two pumps. Let me know when to stop."
    rose ""

translate id w3PunishGame_b9e17986:
    # ver "Stohh--"
    ver ""

translate id w3PunishGame_146d9ff6:
    # ver "Hnnggh, hhkhk-ooohwwah--"
    ver ""

translate id w3PunishGame_b76af5fc:
    # rose "Three."
    rose ""

translate id w3PunishGame_0104a888:
    # ver "Stop! It feels like my tieiits--"
    ver ""

translate id w3PunishGame_0156060e:
    # ver "Hngngg, y-you cunt!"
    ver ""

translate id w3PunishGame_3ea65f43:
    # rose "Sorry, Veronica."
    rose ""

translate id w3PunishGame_6c9b2225:
    # rose "One more than is comfortable is adequate punishment. This is-"
    rose ""

translate id w3PunishGame_affc7888:
    # ver "Hheee-ehheu-♥"
    ver ""

translate id w3PunishGame_ee36850c:
    # rose "Well, you seem taken care of on both ends."
    rose ""

translate id w3PunishGame_ebf4f5b7:
    # ver "W-wwhaaeeeuch...♥"
    ver ""

translate id w3PunishGame_7f6d1dd6:
    # kat "Oh my. No edging into things."
    kat ""

translate id w3PunishGame_f9abbb40:
    # rose "I should move on to Felicia now."
    rose ""

translate id w3PunishGame_e270c796:
    # kat "The constant pull on her breasts and the ceaseless ribbed vibration on her sex might just end up breaking her in this state."
    kat ""

translate id w3PunishGame_140586c6:
    # fel "E-hhuhhh...? Ah... fuck, what do you have in store for me?"
    fel ""

translate id w3PunishGame_2f5bdf08:
    # rose "That's a good question. The way you're set up kinda limits my options..."
    rose ""

translate id w3PunishGame_8bb3a560:
    # ver "Hnngg, hhaaa- sghbh--"
    ver ""

translate id w3PunishGame_a46a445b:
    # fel "Hnng, w-whatever, j-just... d-do something! I'm dying-"
    fel ""

translate id w3PunishGame_7901e886:
    # rose "Hold that thought."
    rose ""

translate id w3PunishGame_d90f844c:
    # rose "I read about this in a book once."
    rose ""

translate id w3PunishGame_c223f135:
    # fel "...eh?! What are you going to do with that?!"
    fel ""

translate id w3PunishGame_84c5741d_2:
    # kat "I'm surprised you haven't soiled my hand yet."
    kat ""

translate id w3PunishGame_5bb63c34_2:
    # mc "Hnngg!"
    mc ""

translate id w3PunishGame_7f926880_2:
    # kat "I know one time won't suffice. You don't have to fear; I'll milk you as much as you require~"
    kat ""

translate id w3PunishGame_c6f5cfb8_1:
    # "At this point, a stiff breeze would have gotten me off, but for how slow it was, the old woman's technique was thorough and {i}immaculate{/i} in its consistency. She never slowed down or sped up; she just jerked me like her wrist operated on a piston."
    ""

translate id w3PunishGame_7a26e7dc_2:
    # mc "G-ahh, s-shut the fuck up and just k-keep--!"
    mc ""

translate id w3PunishGame_40ab8cb0_2:
    # "The real magic lay in how she used her pinky, tracing its black nail up and down the underside of my shaft, never deviating a single millimeter from the same tract of overwrought nerve endings."
    ""

translate id w3PunishGame_ca9271b5_1:
    # kat "*Whisper* You should speak kindly to someone doing you a favor, [mcf]."
    kat ""

translate id w3PunishGame_fa952a65_2:
    # mc "*Gulp* H-a, haa... favor...? Yeah right..."
    mc ""

translate id w3PunishGame_56b5e418_1:
    # "It was like the words apparated on my lips without even being thought of. I said what I felt without control or filter."
    ""

translate id w3PunishGame_93bce481_1:
    # mc "T-take a sit and spin on it if you really want to do me a favor!"
    mc ""

translate id w3PunishGame_a2ad0a0e_2:
    # kat "You're adorable right now, you know that?"
    kat ""

translate id w3PunishGame_7a90e79a:
    # fel "G-ggah, get that the fuck away from me!"
    fel ""

translate id w3PunishGame_8460add1:
    # rose "Relax, Sweetie, you don't think I'd actually..."
    rose ""

translate id w3PunishGame_c223f135_1:
    # fel "...eh?! What are you going to do with that?!"
    fel ""

translate id w3PunishGame_6c0f4632:
    # kat "What a surprise. I didn't think she would use that."
    kat ""

translate id w3PunishGame_6d591c03:
    # mc "Hhhngg...?"
    mc ""

translate id w3PunishGame_56d4a825_2:
    # "With the way I counted it, I had a couple of options for managing my sickening arousal."
    ""

translate id w3PunishGame_10efc326_2:
    # "I could let my mind wander, try to think of something else... but invariably, my senses fell on the women next to me, to the curves of their busts and their waists..."
    ""

translate id w3PunishGame_e4fa996b_2:
    # "These two women were so far above me... the old woman decades my senior. Sophia, a brilliant scientist... it was so easy to imagine the delight in grabbing one of them right now and just--"
    ""

translate id w3PunishGame_2389bacb_2:
    # mct "(Y-yeah... no... I should focus on what's playing out in front of me...)"
    mct ""

translate id w3PunishGame_3427b534_2:
    # "The other option was to let Kathleen's game preoccupy my attention."
    ""

translate id w3PunishGame_e5d1e093_2:
    # kat "A-aah..."
    kat ""

translate id w3PunishGame_d4164143_1:
    # mct "(...that's gotta be the best way of dealing with this, right?"
    mct ""

translate id w3PunishGame_ca44b0b9_2:
    # kat "Do you need to be excused, Mr. [mcl]? Perhaps you need to go to the bathroom?"
    kat ""

translate id w3PunishGame_c8c82b40_2:
    # mc "N-no, I'm f-fine..."
    mc ""

translate id w3PunishGame_1050a010_2:
    # "She teased me, and strangely, I felt an unusual rush of pride. Of course, it was irrational, but that would be akin to losing..."
    ""

translate id w3PunishGame_54c72150_2:
    # "Losing what and to who, I did not know... but my willpower... I was stronger than this..."
    ""

translate id w3PunishGame_041c023d_2:
    # mct "(I ain't a fucking animal... right?)"
    mct ""

translate id w3PunishGame_7a90e79a_1:
    # fel "G-ggah, get that the fuck away from me!"
    fel ""

translate id w3PunishGame_8460add1_1:
    # rose "Relax, Sweetie, you don't think I'd actually..."
    rose ""

translate id w3PunishGame_247faaa2:
    # rose "...burn you do you? No way! This is just..."
    rose ""

translate id w3PunishGame_20378f5f:
    # fel "What kind of fucking books do you read?!"
    fel ""

translate id w3PunishGame_17f6bb05:
    # rose "Eh...? Oh, it's... just something another mom recommended..."
    rose ""

translate id w3PunishGame_da9ad6b6:
    # fel "Yeah, right! You perverted cow! Is this the kind of shit you're into?"
    fel ""

translate id w3PunishGame_e9bc283b:
    # rose "I don't like that insinuation, nor the tone in your voice..."
    rose ""

translate id w3PunishGame_b07ccd06:
    # fel "The tone in MY voice?! Fuck you! You're the one-- aahh- s-shit, that's fucking w-warm!"
    fel ""

translate id w3PunishGame_73934a4d:
    # rose "Well, that IS the point, hun... but it's not hurting you, is it?"
    rose ""

translate id w3PunishGame_a5ed954e:
    # fel "H-huh...?! N-no... I g-guess not... it actually feels kinda... my s-skin is-- a-aahh...!"
    fel ""

translate id w3PunishGame_4e4f87d0:
    # fel "Ah, hhnngg it must be this shit Kat gave me... it f-feels like a massage?"
    fel ""

translate id w3PunishGame_3740d596:
    # rose "Oh, good!"
    rose ""

translate id w3PunishGame_97d66900:
    # fel "Y-you're surprised?!"
    fel ""

translate id w3PunishGame_134b84ea:
    # rose "Well, this is the first time I've actually done this!"
    rose ""

translate id w3PunishGame_d91d1123:
    # fel "G-gahh-!"
    fel ""

translate id w3PunishGame_828fa653:
    # rose "It's supposed to be like an intense but relaxing massage... one that might leave your skin, or in this case your nipples, a little agitated..."
    rose ""

translate id w3PunishGame_dc758c15:
    # fel "A-aahh-♥ You d-damn freak!"
    fel ""

translate id w3PunishGame_621dd2b5:
    # rose "Don't be rude. This is part of the exhibition."
    rose ""

translate id w3PunishGame_8680da6c:
    # fel "Bullshit, there's a WHOLE lot of shit on the table and you picked-"
    fel ""

translate id w3PunishGame_e9c7e41f:
    # fel "Ghh, hnng, f-fuuuuck!"
    fel ""

translate id w3PunishGame_d4d3f822:
    # fel "G-, aaahn, hhhngg--"
    fel ""

translate id w3PunishGame_cd393bc8:
    # fel "Nueeeueuhh--"
    fel ""

translate id w3PunishGame_ff3a719e_1:
    # fel "G-gah, w-what the fuck?! That... h-hnngg...! F-fuck me, why did I almost cum from that?"
    fel ""

translate id w3PunishGame_bdf6e05b:
    # rose "Maybe it's because you're the perverted one, Felicia."
    rose ""

translate id w3PunishGame_d0471294:
    # rose "I'm not the one doing this for--"
    rose ""

translate id w3PunishGame_4cfffc2d:
    # ver "Ghh-hhhnngg, hhaaa-- it, eehhhn waahhnt sttthhhoop--"
    ver ""

translate id w3PunishGame_bbdac979:
    # ver "Gguuuhhaahhh- sbnnbbbuuuzzzh--"
    ver ""

translate id w3PunishGame_abe2b67a:
    # rose "I'm not the one doing this for fun or pretending like I'm too good to be here."
    rose ""

translate id w3PunishGame_0d23329b:
    # fel "W-what...?"
    fel ""

translate id w3PunishGame_6d382812:
    # rose "Both of you are so damn annoying. You, for obvious reasons and even Veronica is..."
    rose ""

translate id w3PunishGame_d4b92a22:
    # ver "GUgg-hhhnngg♥♥♥"
    ver ""

translate id w3PunishGame_86119bae:
    # rose "I swallow my pride and she runs her mouth, saying what I'm thinking. It makes me feel..."
    rose ""

translate id w3PunishGame_540e254c:
    # fel "How do you really feel, you two-faced bitch?"
    fel ""

translate id w3PunishGame_c88900b6:
    # rose "You got me figured out, don't you, hun?"
    rose ""

translate id w3PunishGame_eb708c5f_2:
    # kat "It's always interesting when you give a woman like Rosalind the opportunity to take the reins. They're so stressed, pent up, inhibited..."
    kat ""

translate id w3PunishGame_aa7fc22b:
    # fel "Guh, hhnng, s-shit!"
    fel ""

translate id w3PunishGame_6d462edb_2:
    # kat "They don't show you their true colors as much as their whole color palette blends together in one fucked up mix. I wonder where this will go."
    kat ""

translate id w3PunishGame_4a991672_1:
    # mc "H-hhhhngg...!"
    mc ""

translate id w3PunishGame_274a48d8_2:
    # "Like hell my mind was registering any of her bullshit right now."
    ""

translate id w3PunishGame_70256af8_2:
    # ver "H-heuuuh?! Y-you're gonna use t-that t-too?!"
    ver ""

translate id w3PunishGame_9e84ae9d_4:
    # sophia "You counted on this kind of... {i}fervor{/i}?"
    sophia ""

translate id w3PunishGame_83473487_2:
    # mc "H-a, hnnggaa..."
    mc ""

translate id w3PunishGame_e7a38a2a_2:
    # "The inside of my head was mush right now. My capacity for language was utterly eroded."
    ""

translate id w3PunishGame_93fd3301_4:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_1abbdcae_4:
    # kat "I'm well acquainted with the peculiar ways people in desperate situations respond. In a way, it's a more potent means of control than any drug."
    kat ""

translate id w3PunishGame_47d2a767_4:
    # sophia "I'm actually impressed for once."
    sophia ""

translate id w3PunishGame_2b5022f9_2:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your owner."
    kat ""

translate id w3PunishGame_d2a0be0e_4:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df_4:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108_4:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418_4:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_087867e3_4:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_eb708c5f_3:
    # kat "It's always interesting when you give a woman like Rosalind the opportunity to take the reins. They're so stressed, pent up, inhibited..."
    kat ""

translate id w3PunishGame_aa7fc22b_1:
    # fel "Guh, hhnng, s-shit!"
    fel ""

translate id w3PunishGame_6d462edb_3:
    # kat "They don't show you their true colors as much as their whole color palette blends together in one fucked up mix. I wonder where this will go."
    kat ""

translate id w3PunishGame_7fbd2e1f_2:
    # "To my annoyance, whenever one of them spoke, my attention would turn back to just how fuckable the women next to me were. "
    ""

translate id w3PunishGame_70256af8_3:
    # ver "H-heuuuh?! Y-you're gonna use t-that t-too?!"
    ver ""

translate id w3PunishGame_9e84ae9d_5:
    # sophia "You counted on this kind of... {i}fervor{/i}?"
    sophia ""

translate id w3PunishGame_96a9d185_2:
    # mct "(Christ, I want to make that mouthy hag choke on my dick until she passes out!)"
    mct ""

translate id w3PunishGame_f94b31a8_2:
    # "Intrusive thought upon intrusive thought flooded my head."
    ""

translate id w3PunishGame_93fd3301_5:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_1abbdcae_5:
    # kat "I'm well acquainted with the peculiar ways people in desperate situations respond. In a way, it's a more potent means of control than any drug."
    kat ""

translate id w3PunishGame_47d2a767_5:
    # sophia "I'm actually impressed for once."
    sophia ""

translate id w3PunishGame_5a5d8551_2:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your Master."
    kat ""

translate id w3PunishGame_d2a0be0e_5:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df_5:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108_5:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418_5:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_087867e3_5:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_334889c4:
    # fel "Ghhu, hnhng-♥"
    fel ""

translate id w3PunishGame_768e911b:
    # fel "Hnngg, hh-aaa-♥♥"
    fel ""

translate id w3PunishGame_046266df:
    # fel "D-damn, it, gaaa♥♥♥"
    fel ""

translate id w3PunishGame_fdffd4b3:
    # fel "W-what the f-fucckkhhh iswwrrggon♥♥♥♥"
    fel ""

translate id w3PunishGame_8f379cdb:
    # fel "Wtrrrrornnggggweeethmeeeeee♥♥♥♥♥"
    fel ""

translate id w3PunishGame_1577ec97:
    # rose "Wow..."
    rose ""

translate id w3PunishGame_14d9e1da:
    # fel "Guhh, hh-aaa, d-did I just...?"
    fel ""

translate id w3PunishGame_9667273a:
    # rose "You came."
    rose ""

translate id w3PunishGame_b65869a1:
    # fel "Ah- w-why am I still...?"
    fel ""

translate id w3PunishGame_6795a523:
    # rose "I wanted to see if I could get you to with just your chest, but I didn't think it would be that easy..."
    rose ""

translate id w3PunishGame_2636b451:
    # rose "Me? I've always been so sensitive up top that I've been curious..."
    rose ""

translate id w3PunishGame_0a81b01a:
    # fel "Oh...you're enjoying this. I don't actually care, I'm not j-judging you... I'd just love to hear you admit it."
    fel ""

translate id w3PunishGame_5f383a49:
    # ver "Huggg---aahh, wwwhooooaoahh--♥♥♥♥♥♥"
    ver ""

translate id w3PunishGame_959ff7a9:
    # ver "Guh, hhgguu--♥♥♥♥♥♥"
    ver ""

translate id w3PunishGame_78d0d505:
    # rose "Enjoying {i}this{/i}? I can honestly say I'm not."
    rose ""

translate id w3PunishGame_147421e8:
    # rose "I'm just happy to be on this side of things for once."
    rose ""

translate id w3PunishGame_fd0af3ca:
    # rose "You're still burning up, aren't you?"
    rose ""

translate id w3PunishGame_6d362c97:
    # fel "H-hhuhh, aa-ahh..."
    fel ""

translate id w3PunishGame_c78e48d2:
    # fel "Guuaah-? Hnnng-♥ Y-you already tested your hypoth--"
    fel ""

translate id w3PunishGame_45bac2af:
    # rose "*Shlurhp, shhwwwup, chwup~* Woohhh-♥♥"
    rose ""

translate id w3PunishGame_6bf3f603:
    # kat "Looks like she's done talking."
    kat ""

translate id w3PunishGame_8217fc81:
    # fel "Gaah, hhnng-♥ W-wooaohhh-hhnnggg-- I'm-"
    fel ""

translate id w3PunishGame_fce4ef3b:
    # ver "Gguuphphh- hhngg, hgauuuh♥♥♥♥♥"
    ver ""

translate id w3PunishGame_47f0ab32:
    # ver "Ehhh--hhnngg♥♥♥"
    ver ""

translate id w3PunishGame_9c150c96:
    # fel "GHhh, hhhu♥♥♥♥♥♥ Fghh-"
    fel ""

translate id w3PunishGame_23377b9c:
    # "A delirious concert of fuck-numbed sounds flooded both the room they stood in and where we sat."
    ""

translate id w3PunishGame_48e7dd69:
    # fel "Wwhh, hhhnnggg- wwowoohh♥♥♥ *Slwhhup, chup, khwwuup!*"
    fel ""

translate id w3PunishGame_6f04ad34:
    # "Rosalind used the paddle in a much more tender and devious way, prying apart and rubbing the blonde's pink insides."
    ""

translate id w3PunishGame_f38c418c:
    # fel "HHuugg-hhhaaat♥♥♥"
    fel ""

translate id w3PunishGame_b5f6633e:
    # "It was her own miniature version of Felicia's game from the last exhibition."
    ""

translate id w3PunishGame_5b2d6741:
    # fel "Www--aahhhhhheeeoo♥"
    fel ""

translate id w3PunishGame_44cc4c10:
    # "If she intended it to mirror that, Rosalind was really quite devious."
    ""

translate id w3PunishGame_eff31b36:
    # "*Slwhhup, chup, khwwuup!*"
    ""

translate id w3PunishGame_0ba239c7:
    # "The sight was pitiful and sad, but even without being under the influence as I was, this sight was..."
    ""

translate id w3PunishGame_eabe24e7:
    # fel "Guhh-hhukk-! HHwwwwoho♥♥♥♥"
    fel ""

translate id w3PunishGame_99c5684b:
    # "It was the kinda sight that moved me."
    ""

translate id w3PunishGame_eeee95f7:
    # mc "G-ghhuuh!"
    mc ""

translate id w3PunishGame_01d6aee3_2:
    # kat "I'm starting to get offended, Mr. [mcl]. Does my hand not suffice?"
    kat ""

translate id w3PunishGame_06035a29_2:
    # kat "Don't you want to taste release?"
    kat ""

translate id w3PunishGame_31a83b72_2:
    # mc "G-gah, f-fuck! It's not like I have any control over it!"
    mc ""

translate id w3PunishGame_fd6b335d_2:
    # sophia "Hmm..."
    sophia ""

translate id w3PunishGame_501f6d67_2:
    # "Every fiber of my being desperately anticipated climax. My reason had been eroded and my purpose replaced by the critical need to expunge every trace of nauseating desire from my body through my cock."
    ""

translate id w3PunishGame_15dcbcac_2:
    # "The hairs on my arm stood on end, the feeling in my extremities came and went like the tide, and a pervasive tingling feeling floated from my neck down to my tailbone - by all means, my body told me that I {b}should{/b} be coming."
    ""

translate id w3PunishGame_cc175353_2:
    # sophia "Interesting."
    sophia ""

translate id w3PunishGame_2438123c_2:
    # "But I wasn't. My bloated dick burned against the soft palm of the old woman's hand, pulsating and twitching from the ceaseless stimulation, filling me with the irrational fear that I might literally explode if I didn't soon go careening into the void of sexual release."
    ""

translate id w3PunishGame_5a852968_2:
    # sophia "All things considered, he is showing a {b}surprising{/b} degree of longevity..."
    sophia ""

translate id w3PunishGame_26f61ebe_2:
    # kat "Maybe your fragrance isn't as effective as you think it is."
    kat ""

translate id w3PunishGame_a2f58e6b_2:
    # mc "G-gaahh!"
    mc ""

translate id w3PunishGame_ecfe8072_2:
    # sophia "Maybe..."
    sophia ""

translate id w3PunishGame_46144357_2:
    # mct "(No! It was definitely fucking effective!)"
    mct ""

translate id w3PunishGame_5db42400_2:
    # sophia "...but unlikely. Hey, [mcf]..."
    sophia ""

translate id w3PunishGame_dab66f0d_2:
    # "Through lidded eyes, I could sense the blonde scooching closer and closer."
    ""

translate id w3PunishGame_08efb90d_2:
    # sophia "The issue is that a young, well-traveled guy like you simply can't get off on an old woman's touch, right?"
    sophia ""

translate id w3PunishGame_d36256e0_2:
    # mc "That's not--"
    mc ""

translate id w3PunishGame_93779299_2:
    # sophia "{b}{i}Isn't it?{/b}{/i}"
    sophia ""

translate id w3PunishGame_76f7a8ba_2:
    # "Even in my lust-addled state..."
    ""

translate id w3PunishGame_db1a6cb6_2:
    # "My brain registered amazement over the willful way Sophia flipped the switch, going from detached aloofness to a man-killer just as easily as one would shed their clothes. "
    ""

translate id w3PunishGame_061fb09e_2:
    # mc "Hnngg...!"
    mc ""

translate id w3PunishGame_beef1333_2:
    # kat "*Scoff* Feeling left out, were you?"
    kat ""

translate id w3PunishGame_bfa0648c_2:
    # sophia "Give them a comparison."
    sophia ""

translate id w3PunishGame_18b36fa6_2:
    # "My hand responded independently, sinking into the pleasant give of the doctor's tit-flesh in an instant as what remained of my faculties disappeared into the whirlpool of her eyes."
    ""

translate id w3PunishGame_04a18dd7_2:
    # sophia "You got a mess lurking somewhere in there that you want to get out?"
    sophia ""

translate id w3PunishGame_0f885ed4_2:
    # mc "Ha... hnngg...! F-fuck it!"
    mc ""

translate id w3PunishGame_5d2b485c_2:
    # sophia "...?!"
    sophia ""

translate id w3PunishGame_cc522673_2:
    # "I kissed her, just as you'd expect a man on the brink of starvation to steal a loaf of bread."
    ""

translate id w3PunishGame_02060d5f_2:
    # sophia "Mmmhhhh....!"
    sophia ""

translate id w3PunishGame_8c6f3eee_2:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Mrs. Pulman."
    ""

translate id w3PunishGame_b5e1efa6_2:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Kathleen."
    ""

translate id w3PunishGame_e13d5d67_2:
    # kat "H-hey, ahh--"
    kat ""

translate id w3PunishGame_2430bc3d_2:
    # "Spurred on by having the attention of two beautiful women, even if it was just the fucked up equivalent of cooking ants with a magnifying glass to them, I knew it wouldn't be long until I finally had my release."
    ""

translate id w3PunishGame_87732b5f_2:
    # kat "Gah, ah... ha... hmpfh."
    kat ""

translate id w3PunishGame_353b1cca_2:
    # "Although I could feel her eyes on my neck, the old woman continued to milk me."
    ""

translate id w3PunishGame_61937b68_2:
    # "*Cwhup, fwhup, chwup...!*"
    ""

translate id w3PunishGame_28eed101_2:
    # kat "I've somehow ended up feeling like a third wheel."
    kat ""

translate id w3PunishGame_278c7b3e_2:
    # "I pawed at both women's breasts with a virgin-like fervor, wholly enamored by an opportunity I might never get again."
    ""

translate id w3PunishGame_5d459ae6_2:
    # mct "(The head of a charity and a world-class scientist... ha, fucking sluts!)"
    mct ""

translate id w3PunishGame_e128df9b_2:
    # "I felt like a king, not of my own making of course, but my brain was so cooked as to not know the difference. Thinking was hard, but what few thoughts I could miraculously formulate were like tidal waves breaking against the shore."
    ""

translate id w3PunishGame_0ed4c646_2:
    # mct "(Who the hell do you think you're fucking with? You think you can toy with me just because of money?!)"
    mct ""

translate id w3PunishGame_b117f61e_2:
    # "I felt smothered, suffocated, and hot. More akin to a wick that was about to burn out than a human being."
    ""

translate id w3PunishGame_9c75dc6b_2:
    # mct "(I'M A GUINEA PIG TO YOU?!)"
    mct ""

translate id w3PunishGame_07bbfc1c_2:
    # "Every sensation felt heightened, and every second felt prolonged... I genuinely feared that the moment I popped would be when I was snuffed out."
    ""

translate id w3PunishGame_a73faf04_2:
    # mct "{size=+30}{font=/gui/fonts/MB-Thin_Worms.ttf}(You like to toy with pe-){/font}{/size}"
    mct ""

translate id w3PunishGame_61867711_2:
    # "My brain gave out, but if I ceased to exist after this, I instinctively knew one thing: these last minutes of my life should be bold."
    ""

translate id w3PunishGame_ee7c2e13_2:
    # mc "I'm about to b-blow..."
    mc ""

translate id w3PunishGame_01a79887_2:
    # kat "You don't need my permission."
    kat ""

translate id w3PunishGame_2221f5ba_2:
    # mc "Can you finish me with your mouth?"
    mc ""

translate id w3PunishGame_fde31e51_2:
    # kat "Hmmmmm, not going to happen."
    kat ""

translate id w3PunishGame_eefd493d_2:
    # mc "A-ah, pretty please? With a cherry on top?"
    mc ""

translate id w3PunishGame_4469f1d1_2:
    # kat "Why don't you ask Miss Lundgren to do it?"
    kat ""

translate id w3PunishGame_6032c009_2:
    # mc "...because I want you, Ma'am."
    mc ""

translate id w3PunishGame_7d48d431_2:
    # kat "...hmmmmmmmm. You have the eyes of a crazed animal, yet..."
    kat ""

translate id w3PunishGame_2d6e0df6_2:
    # kat "You're asking so nicely. God, I love that."
    kat ""

translate id w3PunishGame_af9d9795_2:
    # "She smirked like she had won against me, and perhaps even against Sophia."
    ""

translate id w3PunishGame_76125b85_2:
    # kat "It really is goddamn adorable. Ask me again, [mcf]."
    kat ""

translate id w3PunishGame_8ce7b843_2:
    # "A desperate, cloying urge inside me told me her mouth was the only place I wanted to be right now and if it took a bit of pleading to get me into a position where I could fill that mean bitch's stomach with my seed, then so be it."
    ""

translate id w3PunishGame_ca96d738_2:
    # mc "Hhh, haa... would you PLEASE finish me with your mouth, Mrs. Pulman?"
    mc ""

translate id w3PunishGame_e2b980af_2:
    # "Without another word, Mrs. Pulman lowered herself and..."
    ""

translate id w3PunishGame_fc8cf5a3_2:
    # "Without another word, Kathleen lowered herself and..."
    ""

translate id w3PunishGame_109a7847_2:
    # mc "G-gah...!"
    mc ""

translate id w3PunishGame_d66b4def_2:
    # "The moment her lips wrapped themselves around my cock, I felt like I might pass out."
    ""

translate id w3PunishGame_cca0b4ef_2:
    # "Her seal was so airtight that her mouth felt like an oven. Steady puffs of hot breath passed over the head of my cock as it made its way out through her nose."
    ""

translate id w3PunishGame_d6fb4ea7_2:
    # mc "F-fuck, damn it!"
    mc ""

translate id w3PunishGame_1bd17e11_2:
    # "*Shwuck, fgwhhuck, thwuuuck!*"
    ""

translate id w3PunishGame_b39227ac_1:
    # "What little air could escape from her hold transformed into {b}obscene{/b} noises as she inched herself up and down my shaft."
    ""

translate id w3PunishGame_63238d75_2:
    # mc "H-holy-!"
    mc ""

translate id w3PunishGame_ad464d04_2:
    # "The running narration in my head was a jumbled mess, but through all the muddied noise from the pleasure soaking into my brain, I grasped a simple truth."
    ""

translate id w3PunishGame_fdbb746f_2:
    # mc "Hnnng, you've sucked a lot of cock in your time, haven't you?!"
    mc ""

translate id w3PunishGame_dad9bfec_2:
    # "*Thwaahp, fwhap, swhhuck~!*"
    ""

translate id w3PunishGame_72c8cf4a_2:
    # "The answer to that question played across my glans every time I wedged myself comfortably in her throat with zero fuss."
    ""

translate id w3PunishGame_6b12669e_2:
    # "Swhuck, hhhwhuck, fhhggguuk!*"
    ""

translate id w3PunishGame_a3af0240_2:
    # "{b}Damn it!{/b} A rich woman like her had no business sucking dick this well."
    ""

translate id w3PunishGame_d86d228f_2:
    # mc "Ghhn, haaat, hnnngg...!"
    mc ""

translate id w3PunishGame_88387910_2:
    # "She sucked me with the skill of a woman who had {b}nothing{/b} to her name."
    ""

translate id w3PunishGame_cdfdc63e_2:
    # kat "Mmmhm, hhmhm~"
    kat ""

translate id w3PunishGame_93b1b0e6_2:
    # "Just like her hand job, there was no wasted effort. She didn't tackle it in a clumsy frenzy, nor did she need to cautiously get used to my size."
    ""

translate id w3PunishGame_d628853d_2:
    # "*Shwhuhuck, fwhhucuk, ghhwhuhuk!*"
    ""

translate id w3PunishGame_41aa32dc_2:
    # "She sucked me like she had been sucking {b}my{/b} dick her whole life, pleasuring me at a steady and comfortable pace. I marveled at how, through a mere blowjob, a woman so cold and cruel could instantly imprint herself on my body as warm and welcoming."
    ""

translate id w3PunishGame_d93b0c5e_2:
    # "I was quickly understanding that \"adorable\" disparity she had just spoken."
    ""

translate id w3PunishGame_7a3794d6_2:
    # mc "Hnng, h-how?!"
    mc ""

translate id w3PunishGame_2aa14e7d_2:
    # sophia "It's not performing brain surgery."
    sophia ""

translate id w3PunishGame_9f2a1802_2:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Mrs. Pulman's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_bc9afe80_2:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Kathleen's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_7f97a9d7_2:
    # mct "(Haaaa... hope you like the show, bitch. You're the impetus for this.)"
    mct ""

translate id w3PunishGame_0d35f1a2_2:
    # "*Fwhhuck, ghhhukh, khggguuhhk*"
    ""

translate id w3PunishGame_b1ef8758_2:
    # mc "Ahh... y-you dick swallowing--! Hnnngg...!"
    mc ""

translate id w3PunishGame_57623603_2:
    # "It was coming."
    ""

translate id w3PunishGame_a9d0bbe1_2:
    # mc "You think I'm adorable?!"
    mc ""

translate id w3PunishGame_f8d06f5f_2:
    # "I was moments from blowing my load, and with it, the indignation from earlier swelled in me. I had wanted a bold finish before I met my end, and well..."
    ""

translate id w3PunishGame_26cd0617_2:
    # mct "(I'll show you adorable!)"
    mct ""

translate id w3PunishGame_72c12088_2:
    # mc "I'm c-cumming, fucking drown in it, you damn--"
    mc ""

translate id w3PunishGame_ec061ea7_2:
    # kat "Mmmmh?!"
    kat ""

translate id w3PunishGame_5c240311_2:
    # "Before she could even register what was happening, ropes of piping hot cum barreled down her esophagus on a one-way trip to her stomach."
    ""

translate id w3PunishGame_10060088_2:
    # kat "Ghhkka, hkkaa-!"
    kat ""

translate id w3PunishGame_4bb38f46_2:
    # "I pressed down hard, holding the old woman's head in place as she gargled my seed in abject surprise."
    ""

translate id w3PunishGame_2745bcd2_2:
    # mc "Hnngg, all of it!"
    mc ""

translate id w3PunishGame_716f46e4_2:
    # "I pressed down so hard it hurt my groin."
    ""

translate id w3PunishGame_4e4c5b02_2:
    # kat "Ghhuk, hhuuuh, hwwwhuuuck...!"
    kat ""

translate id w3PunishGame_fe1ef6b3_2:
    # "It was massive, like I was cumming my very soul down the old woman's throat."
    ""

translate id w3PunishGame_4500ed7e_2:
    # kat "Ghuk, khak, uuhhk-!"
    kat ""

translate id w3PunishGame_2239e61c_2:
    # mc "Gah, c-choke on it!"
    mc ""

translate id w3PunishGame_ab7d9ac7_2:
    # "Not all of my load was making its way down her throat. To my perverse glee, I could {i}feel{/i} myself coating the lining of her throat."
    ""

translate id w3PunishGame_7b62aa4f_2:
    # kat "Ghhuk, hhuuuk...!"
    kat ""

translate id w3PunishGame_e09b7aa7_2:
    # mc "Hope you like the fucking taste!"
    mc ""

translate id w3PunishGame_8747fdde_2:
    # kat "Mmmh, hhhngg?!?!"
    kat ""

translate id w3PunishGame_3698f7b8_2:
    # sophia "I don't think she can breathe..."
    sophia ""

translate id w3PunishGame_6d4b68da_2:
    # mc "Good! She can breathe when I'm fucking--!"
    mc ""

translate id w3PunishGame_de4a6cab_2:
    # ".........!"
    ""

translate id w3PunishGame_2068a0d5_2:
    # "......!"
    ""

translate id w3PunishGame_1d9b8a71_2:
    # mc "A-aahh...!"
    mc ""

translate id w3PunishGame_869e1b4e_2:
    # kat "Ugghhuhh..."
    kat ""

translate id w3PunishGame_ba8a394f_4:
    # "Peace."
    ""

translate id w3PunishGame_a48af359_4:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d_4:
    # "Satisfaction."
    ""

translate id w3PunishGame_92962e6b_2:
    # "In this moment, I had it all."
    ""

translate id w3PunishGame_e38ecbb2_2:
    # "With that, I happily died. The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8b8ceb72_4:
    # "............"
    ""

translate id w3PunishGame_278c7e29_5:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_9:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_10:
    # "..."
    ""

translate id w3PunishGame_87f2fe4b_2:
    # kat "Why are you holding your hands out like that? It's weird."
    kat ""

translate id w3PunishGame_2dd84f14_4:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_c2d77e46_2:
    # sophia "She's right. It's distracting."
    sophia ""

translate id w3PunishGame_1516ad37_2:
    # "I didn't know how long I was out of it, but by the time I opened my eyes, the two women had collected themselves, and what I thought was a bold display of defiance was made pitiful and wholly expected by their bemused faces."
    ""

translate id w3PunishGame_69e22dc5_2:
    # "Mrs. Pulman looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_d1fa2418_2:
    # "Kathleen looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_e7145261_2:
    # kat "Oh, by the way... you said some pretty outlandish things to me."
    kat ""

translate id w3PunishGame_526a5615_1:
    # mc "If I recall, I did a bit more than that..."
    mc ""

translate id w3PunishGame_2e195b22_2:
    # kat "It was {b}adorable{/b}."
    kat ""

translate id w3PunishGame_c5563c4b_2:
    # "She had indeed won."
    ""

translate id w3PunishGame_9a64515e_2:
    # ver "Ghh, hhaan, streetthhhchggingg-!!!"
    ver ""

translate id w3PunishGame_d9d8b219_2:
    # "I knew what I wanted."
    ""

translate id w3PunishGame_3576545e_2:
    # sophia "Mmmhh, hhhhh..."
    sophia ""

translate id w3PunishGame_7202d9ed_2:
    # "And that last act of defiance would be leaving the doctor, the root cause of all this, something to remember me by."
    ""

translate id w3PunishGame_293b8be4_2:
    # "*Chwup, fhwup~*"
    ""

translate id w3PunishGame_7a9110af_2:
    # "I wanted to hear this aloof bitch moan, so I focused everything on the woman whose mouth I was currently exploring."
    ""

translate id w3PunishGame_01fdbaa9_2:
    # sophia "Mmmh, hhmmm...?"
    sophia ""

translate id w3PunishGame_1efdf9e0_2:
    # "It was a battle against my wits. Sophia was literally intoxicating. Focusing my senses all on her had me burning twice as fast."
    ""

translate id w3PunishGame_d14b5b96_2:
    # "By instinct, I felt her breast over her clothes."
    ""

translate id w3PunishGame_79a3ccdd_2:
    # sophia "Mmhh..."
    sophia ""

translate id w3PunishGame_a90ff98f_2:
    # "Her coos told me she was receptive, but it wasn't quite the noise I wanted to drag out from the blonde."
    ""

translate id w3PunishGame_754fce10_2:
    # sophia "Ghu, gha~ghhuu...!"
    sophia ""

translate id w3PunishGame_be4204b5_2:
    # "Deciding that over the clothes touching wouldn't do it, I switched focus on the erogenous zone in her mouth, intensifying my efforts in an intricate dance to subdue her tongue with my own."
    ""

translate id w3PunishGame_f43fc47f_2:
    # sophia "Mmmmh, mmhhhh- *chwuph, fhwhup~*"
    sophia ""

translate id w3PunishGame_b0084126_2:
    # "She wasn't a passive observer in my hunt; her tongue played coy, darting away and slipping from my grasp whenever I thought I had her pinned."
    ""

translate id w3PunishGame_a03c3549_2:
    # "It was mere seconds, but--"
    ""

translate id w3PunishGame_27c1fdac_2:
    # "*Chwhu, fhwwwup, chup!*"
    ""

translate id w3PunishGame_f4de7f68_2:
    # "--but, in my fogged-over state, time held little sway over my perception. What was in reality a clumsy tussle felt like a long and protracted siege."
    ""

translate id w3PunishGame_2ec99952_2:
    # "I don't know if it was because of the perfume, but all bodily sensations gradually began to meld into one."
    ""

translate id w3PunishGame_45c987c8_2:
    # "The feeling of the old woman's hand milking my cock, the wet warmness of our kiss, the bubbling desire so strong that I was confident it would be the end of me..."
    ""

translate id w3PunishGame_15b0522f_2:
    # mc "Mmmh, hhhmmm-!"
    mc ""

translate id w3PunishGame_7c16ed74_2:
    # "All of it spelled an overload melded and combined into a singular feeling of otherness. I was sure I had lost this battle until..."
    ""

translate id w3PunishGame_7d144a01_2:
    # "My brain registered a change."
    ""

translate id w3PunishGame_fbfb812a_2:
    # mct "Was she...?"
    mct ""

translate id w3PunishGame_7459555f_2:
    # sophia "Mmmh, hhmhm~aahhh...!"
    sophia ""

translate id w3PunishGame_68d04ab6_2:
    # "She was closer."
    ""

translate id w3PunishGame_08a39e71_2:
    # "She was pressing into me."
    ""

translate id w3PunishGame_883ad25a_2:
    # "She felt it."
    ""

translate id w3PunishGame_b0685c2c_2:
    # "She was {b}heated{/b}."
    ""

translate id w3PunishGame_5c6a5db6_2:
    # "{i}She was kissing me back.{/i}"
    ""

translate id w3PunishGame_26629cc2_2:
    # "Whether it was done deliberately or on an instinctual level, her body responded favorably."
    ""

translate id w3PunishGame_a4d39526_2:
    # sophia "Ahh, hhhnnggg, mmmmhh-!"
    sophia ""

translate id w3PunishGame_d8ca8c90_2:
    # "My hand found a place on her ass and I used that to spur her on even more. With this, I had physical confirmation of my success."
    ""

translate id w3PunishGame_44602cec_2:
    # mct "(Fuck, what a great-!)"
    mct ""

translate id w3PunishGame_9fec863b_2:
    # "I could feel the hot dampness of her delicate spot."
    ""

translate id w3PunishGame_cb435f7f_2:
    # sophia "Mmhh, hhaa...♥♥♥"
    sophia ""

translate id w3PunishGame_71583afd_2:
    # "She purred openly and I felt a petty relief over my trivial victory. The succubus driving me to this desperate state wouldn't escape our battle unscathed."
    ""

translate id w3PunishGame_3d67a16d_2:
    # "*Chwup, fhwwup, khwwup*!"
    ""

translate id w3PunishGame_b6fdc206_2:
    # "--and that wasn't the only sense of relief I felt."
    ""

translate id w3PunishGame_cb9361ae_2:
    # mc "{b}Hnng, ah-!!!{/b}"
    mc ""

translate id w3PunishGame_cbea8557_2:
    # "I didn't know when it started, but ropes of cum geysered from my cock and rained down from the sky."
    ""

translate id w3PunishGame_4380fa10_2:
    # mc "Ghh, aaahh- sssshit--"
    mc ""

translate id w3PunishGame_a3230c6c_2:
    # kat "Huh, {b}WOW{/b}."
    kat ""

translate id w3PunishGame_74ff6633_2:
    # mc "Hngg, hhhhmmmhh-~"
    mc ""

translate id w3PunishGame_290360e4_2:
    # "All the suffocating vileness in me was expelled and released into the world. All my desperation spilled out onto our clothes and thighs."
    ""

translate id w3PunishGame_83af968d_2:
    # "Soon need was replaced with..."
    ""

translate id w3PunishGame_ba8a394f_5:
    # "Peace."
    ""

translate id w3PunishGame_a48af359_5:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d_5:
    # "Satisfaction."
    ""

translate id w3PunishGame_2a3c0c6e_2:
    # "Emptiness."
    ""

translate id w3PunishGame_ea0f012b_2:
    # "The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8728f5e5_2:
    # "I was dead."
    ""

translate id w3PunishGame_8b8ceb72_5:
    # "............"
    ""

translate id w3PunishGame_278c7e29_6:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_10:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_11:
    # "..."
    ""

translate id w3PunishGame_c49ce7ae_2:
    # sophia "Great, now we'll spend the rest of the show reeking of semen."
    sophia ""

translate id w3PunishGame_2dd84f14_5:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_2f1146d1_2:
    # kat "You should've thought of that before you entered the crossfire."
    kat ""

translate id w3PunishGame_9177b213_2:
    # sophia "You were taking too long. I wanted to put an end to the distraction."
    sophia ""

translate id w3PunishGame_0e5cc7d3_2:
    # "I didn't know how long I was out of it. Still, by the time I opened my eyes, the two women had collected themselves. The damage I thought I had done to Sophia with my counter-attack was made pitiful by their bemused faces."
    ""

translate id w3PunishGame_23aa7f4f_2:
    # kat "Oh, please. Are those the kind of sounds you make matter-of-factly?"
    kat ""

translate id w3PunishGame_a5d22b22_2:
    # "Sophia looked as composed as ever, and except for a little blush, her attention turned back to the Carnations."
    ""

translate id w3PunishGame_449fa9fb_2:
    # kat "I hope that provided you a measure of relief, Mr. [mcl]. That certainly was an... {i}explosive{/i} finish."
    kat ""

translate id w3PunishGame_b85bc4ed_2:
    # mc "A, ahem... thaaaaaanks?"
    mc ""

translate id w3PunishGame_fada21bd_2:
    # kat "Don't mention it. It was... interesting."
    kat ""

translate id w3PunishGame_9a64515e_3:
    # ver "Ghh, hhaan, streetthhhchggingg-!!!"
    ver ""

translate id w3PunishGame_dddb6f7e_2:
    # mc "........."
    mc ""

translate id w3PunishGame_3b21f354_2:
    # mc "......"
    mc ""

translate id w3PunishGame_5b4c9fd4_2:
    # "...this was my limit."
    ""

translate id w3PunishGame_964e84bc_2:
    # mc "Aaeeh, I'll be riiiiiight back."
    mc ""

translate id w3PunishGame_0fd4d656_11:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_12:
    # "..."
    ""

translate id w3PunishGame_e6cfc671_2:
    # kat "Is that a milder scent you are wearing?"
    kat ""

translate id w3PunishGame_837e9b2b_2:
    # sophia "It's the same batch as the bottle I gave you."
    sophia ""

translate id w3PunishGame_256d5f3e_2:
    # kat "The effect seemed... less? Why's that?"
    kat ""

translate id w3PunishGame_548f5e80_2:
    # sophia "It's possible that his body adapted and partially metabolized the odor molecules. All I can say is that it's remarkable he held out that long."
    sophia ""

translate id w3PunishGame_b4f48c1e_2:
    # sophia "It could be a matter of willpower."
    sophia ""

translate id w3PunishGame_caedd2e6_2:
    # kat "When I had dinner with Charles the other night, he didn't even flinch."
    kat ""

translate id w3PunishGame_b5500f59_2:
    # sophia "Dr. Kohler is a scary man."
    sophia ""

translate id w3PunishGame_4510a9da_2:
    # kat "Is that your appraisal after demolishing him game after game?"
    kat ""

translate id w3PunishGame_64ecae1b_2:
    # sophia "How else should I describe a man who has no need but keeps his passions tightly controlled."
    sophia ""

translate id w3PunishGame_610ec2fb_2:
    # kat "That's why he and I are incompatible. He treats fucking like it's Christmas."
    kat ""

translate id w3PunishGame_560752c9_2:
    # kat "You don't have anything to worry about as long as you don't lose."
    kat ""

translate id w3PunishGame_b19fae41_2:
    # sophia "I don't plan on it, but I have a feeling he eventually wins one way or another."
    sophia ""

translate id w3PunishGame_f588a7d6_2:
    # kat "Abel wouldn't let it come to that, would he?"
    kat ""

translate id w3PunishGame_0fd4d656_12:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_13:
    # "..."
    ""

translate id w3PunishGame_1324c38b_2:
    # sophia "Thank you for indulging my little whim, Kathleen. It was amusing."
    sophia ""

translate id w3PunishGame_4db92c34_2:
    # kat "Don't mention it. One should make their own fun in life. Still..."
    kat ""

translate id w3PunishGame_0cb22879_2:
    # kat "...you're very competent. Don't you ever desire more?"
    kat ""

translate id w3PunishGame_0cdff961_2:
    # sophia "A frivolous bitch like you has no business saying that to me."
    sophia ""

translate id w3PunishGame_9765acec_2:
    # kat "Ah, fuck. There it is."
    kat ""

translate id w3PunishGame_55e6ab51_2:
    # kat "I almost felt sorry for you."
    kat ""

translate id w3PunishGame_c2705aa0:
    # ver "Hnngguuhh..."
    ver ""

translate id w3PunishGame_7b1f5f13:
    # fel "Hwhuuck, hhnanaa.."
    fel ""

translate id w3PunishGame_76427b99:
    # rose "Wow... ahah..."
    rose ""

translate id w3PunishGame_e97b51b4:
    # rose "I don't smoke, but I feel like I should have a cigarette..."
    rose ""

translate id w3PunishGame_d3b828d6:
    # ver "Ahhwwhh-"
    ver ""

translate id w3PunishGame_8068fec6:
    # fel "Wwhhhaah- hnnn..."
    fel ""

translate id w3PunishGame_660ce9bc:
    # rose "Are you girls holding up alright?"
    rose ""

translate id w3PunishGame_3ab53704:
    # ver "Hnngg.. haa... w-what the fuck do you think?"
    ver ""

translate id w3PunishGame_923da0fc:
    # rose "I don't think it's over until Mrs. Pulman returns, but... I'd say a little break is in order."
    rose ""

translate id w3PunishGame_092bd748:
    # fel "Y-you were j-juust s-shoving a paddle up my cooch and now you're playing n-nice? Goddamn it..."
    fel ""

translate id w3PunishGame_57915452:
    # rose "I--"
    rose ""

translate id w3PunishGame_f930b854:
    # fel "F-for the love of God... p-please don't say this is part of the game again... f-fuck... hnnh... t-the itch is starting again...?"
    fel ""

translate id w3PunishGame_2574dc32:
    # ver "Ghha, hhaa... s-shit... you just left me hanging t-there?"
    ver ""

translate id w3PunishGame_a9424b55:
    # rose "Ahehe... there's only one of me."
    rose ""

translate id w3PunishGame_053f17d1:
    # ver "G-gah, you said I was annoying...?"
    ver ""

translate id w3PunishGame_608001d3:
    # rose "You remember that...?"
    rose ""

translate id w3PunishGame_e5918cd4:
    # ver "Y-you think I'm no better than Blondie?"
    ver ""

translate id w3PunishGame_0a4d1978:
    # rose "I didn't say {i}that{/i}..."
    rose ""

translate id w3PunishGame_5c907c3b:
    # fel "Y-yeah, she said you run your mouth, acting like you're better than everyone else!"
    fel ""

translate id w3PunishGame_0d0f451d:
    # rose "I didn't mean it like that, I..."
    rose ""

translate id w3PunishGame_d3a3eda8:
    # ver "Bah! So what if I do, we all have our fucking pretenses here! Rosie's m-mousey s-shit is just as annoying as my dumb bravado!"
    ver ""

translate id w3PunishGame_d02b6e9b:
    # fel "O-ohh, s-so you're aware of how you sound?!"
    fel ""

translate id w3PunishGame_70cbeada:
    # ver "Even if it's fucking futile, it's something, right? Both of you l-left me hanging when I tried to get ALL of us paid earlier! All of us!"
    ver ""

translate id w3PunishGame_bdfd6c21:
    # fel "It was deluded!"
    fel ""

translate id w3PunishGame_320c381c:
    # ver "So what?! Of course it is! I wasn't expecting it to work!"
    ver ""

translate id w3PunishGame_1ecb47fd:
    # rose "If you know that, why do you draw Mrs. Pulman's ire then...?"
    rose ""

translate id w3PunishGame_8838af23:
    # ver "......"
    ver ""

translate id w3PunishGame_4ad05321:
    # ver "...h-ow could I not?"
    ver ""

translate id w3PunishGame_16fce8f5:
    # fel "You're making it purposefully harder on yourself? Stop being stupid!"
    fel ""

translate id w3PunishGame_3ba9706b:
    # ver "I don't want to hear that from you, bitch."
    ver ""

translate id w3PunishGame_4a652750:
    # fel "Well, you're hearing it!"
    fel ""

translate id w3PunishGame_8c1065b9:
    # ver "Even if you're going to get your ass kicked... ahaa-- hnngg- hnnn-- you..."
    ver ""

translate id w3PunishGame_d88b9f1d:
    # ver "...you don't just put your guard down and make it easy for them."
    ver ""

translate id w3PunishGame_23a61642:
    # ver "You just... {b}don't{/b}."
    ver ""

translate id w3PunishGame_d65df59a_5:
    # rose "......"
    rose ""

translate id w3PunishGame_abd837cd_5:
    # rose "..."
    rose ""

translate id w3PunishGame_5d10a7ef:
    # rose "We should begin again."
    rose ""

translate id w3PunishGame_d9ae5846:
    # fel "H-hhaaa... I think you struck a n-nerve with her..."
    fel ""

translate id w3PunishGame_0cf18be8:
    # rose "Shut up, Felicia."
    rose ""

translate id w3PunishGame_47970c22:
    # rose "You asked me if I actually found you annoying? If I put it mildly, it's not you; this whole thing is an annoyance."
    rose ""

translate id w3PunishGame_7e26a163:
    # rose "When it comes to you or Felicia, I actually don't think much about either of you."
    rose ""

translate id w3PunishGame_52b091df:
    # rose "What do I care if you're here for frivolous reasons?"
    rose ""

translate id w3PunishGame_533c46c1:
    # rose "Why would I give a damn about putting up a fight just to make myself feel better?"
    rose ""

translate id w3PunishGame_eb181c14:
    # ver "That's not what I was saying!"
    ver ""

translate id w3PunishGame_6e9873a7:
    # rose "I know, hun. What I'm saying, though, is... there's only one thing important to me."
    rose ""

translate id w3PunishGame_b8b59c92:
    # "Bzzzt, bzzzt!"
    ""

translate id w3PunishGame_c282ae65:
    # rose "So I gotta do a good job."
    rose ""

translate id w3PunishGame_13e58e74:
    # ver "It's kinda fucking weird being on this side of things... h-hhhnn, f-fuck, I'm feeling crazy..."
    ver ""

translate id w3PunishGame_ca6c54be:
    # ver "I f-feel like I lose no matter what I do here."
    ver ""

translate id w3PunishGame_883079fc:
    # rose "T-that's... uh... wow..."
    rose ""

translate id w3PunishGame_e197e7db:
    # rose "I d-don't have any that looks like that."
    rose ""

translate id w3PunishGame_68ebe6e8:
    # ver "You like the looks of it, eh?"
    ver ""

translate id w3PunishGame_8838af23_1:
    # ver "......"
    ver ""

translate id w3PunishGame_d14637c4_1:
    # ver "..."
    ver ""

translate id w3PunishGame_c28d4ece:
    # rose "W-what are--?"
    rose ""

translate id w3PunishGame_c9a444b0:
    # ver "A-ah, w-what am I doing? Fuuuuck... you're pretty... g-gahh...♥ T-total, {b}major{/b} MILF."
    ver ""

translate id w3PunishGame_f1ac86c5:
    # rose "U-mmm... t-thanks...?"
    rose ""

translate id w3PunishGame_246fc805:
    # fel "H-hurry up! You're not the only one going crazy!"
    fel ""

translate id w3PunishGame_f6171f71:
    # ver "Shut up, Blondie! This was meant to be a punishment, so you'll wait as long as I say."
    ver ""

translate id w3PunishGame_112e2c83:
    # fel "G-aah, cooohm ooooon!"
    fel ""

translate id w3PunishGame_90b1e690:
    # ver "Hnnngg-♥"
    ver ""

translate id w3PunishGame_f93573e0:
    # ver "Why do I feel like not myself and completely like myself at the same time?"
    ver ""

translate id w3PunishGame_730ea34d:
    # ver "I d-don't know which it is... or--hhnn! Or... if I hate it or love it..."
    ver ""

translate id w3PunishGame_67a4b8a2:
    # fel "All I feel like is I need a cock more than I have my entire life?"
    fel ""

translate id w3PunishGame_ed5e77f8:
    # ver "So a normal Monday for you, then?"
    ver ""

translate id w3PunishGame_8174f354:
    # rose "H-haa, aa-ck, s-shoot... d-don't make me laugh..."
    rose ""

translate id w3PunishGame_dc5af190:
    # fel "Fuck you, don't just stand there!"
    fel ""

translate id w3PunishGame_b73ec712:
    # ver "The more you complain...♥ Ahh-hhha...♥ The more I'm just going to leave you sitting there and focus on Rosie."
    ver ""

translate id w3PunishGame_c477e092:
    # fel "Earth to meathead! You're not even doing anything to her!"
    fel ""

translate id w3PunishGame_8bad34c7:
    # ver "Ahhh s-shit, I'm not, am I...?"
    ver ""

translate id w3PunishGame_66294348:
    # ver "S-suck on it. This is going in you."
    ver ""

translate id w3PunishGame_88a55842:
    # rose "Ah... I'm a-already so... ah... I don't think it'll have any trouble sliding in..."
    rose ""

translate id w3PunishGame_e833ce33:
    # ver "I don't care. {b}I wanna see it.{/b}"
    ver ""

translate id w3PunishGame_7bb4f6af:
    # rose "....."
    rose ""

translate id w3PunishGame_9ba212d0:
    # rose "A-ah--?"
    rose ""

translate id w3PunishGame_704e74a0:
    # rose "Hnngg, kuk-!"
    rose ""

translate id w3PunishGame_7ceca700:
    # rose "Ah, hhhmmm-"
    rose ""

translate id w3PunishGame_d7c2cc98:
    # "*Bzzz-bzzzt!*"
    ""

translate id w3PunishGame_4d3467bc:
    # rose "Hkkk, hhhuuk--aahh{b}SSSUUK?!{/b}"
    rose ""

translate id w3PunishGame_68de6a55:
    # ver "Hnngg... that's... {b}goddamn hot!{/b}"
    ver ""

translate id w3PunishGame_5740d24c:
    # rose "HKkhuh, ghhuuhk-! {b}*cough*{/b} Gaah, hhnnk-!"
    rose ""

translate id w3PunishGame_83d08876:
    # ver "{b}SO{/b} goddamn--"
    ver ""

translate id w3PunishGame_95cc610b:
    # kat "For all Miss Lynch's disdain, she does seem to be enjoying the situation, no?"
    kat ""

translate id w3PunishGame_12858723_6:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_b09fb99f:
    # kat "Sluts like her are the best. I love it when they're a mess of contradictions."
    kat ""

translate id w3PunishGame_4b026443_2:
    # "No, in my current state, my mind was completely consumed by the old woman's grip on my cock."
    ""

translate id w3PunishGame_3a010861_3:
    # mc "H-haahaa...!"
    mc ""

translate id w3PunishGame_c9ff4c1c:
    # kat "I really do love Miss Lynch. I wish I had more than a month with her."
    kat ""

translate id w3PunishGame_93651fc0_3:
    # mc "G-gahh...!"
    mc ""

translate id w3PunishGame_22eda143_3:
    # "The pitiful noise of a rutting animal punctuated the women's conversation."
    ""

translate id w3PunishGame_5ddac938_6:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_66527633_3:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious..."
    kat ""

translate id w3PunishGame_5f8d0d83_3:
    # mc "Huuuhghh...!"
    mc ""

translate id w3PunishGame_a7f739bd_3:
    # kat "What gets a woman like you going?"
    kat ""

translate id w3PunishGame_16f23753_3:
    # mc "H-huhh... ack...!"
    mc ""

translate id w3PunishGame_90eda0c7_6:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019_6:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355_6:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_4c9db659_3:
    # kat "We got that in common at least - {b}all three of us.{/b}"
    kat ""

translate id w3PunishGame_29e481f2_6:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_10187d9b:
    # ver "S-shit, this isn't what I had in mind when I--"
    ver ""

translate id w3PunishGame_95cc610b_1:
    # kat "For all Miss Lynch's disdain, she does seem to be enjoying the situation, no?"
    kat ""

translate id w3PunishGame_12858723_7:
    # "In my current over-cooked state, what was playing out on the television screen was a muddled mess of pixels that barely registered meaning to me."
    ""

translate id w3PunishGame_b09fb99f_1:
    # kat "Sluts like her are the best. I love it when they're a mess of contradictions."
    kat ""

translate id w3PunishGame_73380823_2:
    # mc "She... ah..."
    mc ""

translate id w3PunishGame_c79a38a2:
    # kat "Oh, how I really do love Miss Lynch. I wish I had more than a month with her."
    kat ""

translate id w3PunishGame_5ddac938_7:
    # sophia "You have a {i}unique{/i} sense of aesthetics, Kathleen."
    sophia ""

translate id w3PunishGame_1c55b166_3:
    # kat "Well, what do you find hot, Miss Lundgren? I'm genuinely curious... what gets a woman like you going?"
    kat ""

translate id w3PunishGame_90eda0c7_7:
    # sophia "You ever give much thought to what makes your skin itch? I tend not to pontificate over my bodily responses."
    sophia ""

translate id w3PunishGame_bb592019_7:
    # kat "Unlike me, you mean?"
    kat ""

translate id w3PunishGame_32ca6355_7:
    # sophia "When I feel an itch, I simply scratch it."
    sophia ""

translate id w3PunishGame_68542e62_3:
    # kat "Oh, lovely, at least we have that in common."
    kat ""

translate id w3PunishGame_29e481f2_7:
    # sophia "Stop distracting me. I'm trying to watch--"
    sophia ""

translate id w3PunishGame_10187d9b_1:
    # ver "S-shit, this isn't what I had in mind when I--"
    ver ""

translate id w3PunishGame_e8bdf0e1:
    # "*Bzzzt*!"
    ""

translate id w3PunishGame_94be8e4f:
    # rose "HHnnhh...♥ Oo-oohohh...♥"
    rose ""

translate id w3PunishGame_dcd79226:
    # rose "It-♥♥♥"
    rose ""

translate id w3PunishGame_9b0c8927:
    # rose "Wooooaaah-- hnnng...♥"
    rose ""

translate id w3PunishGame_d9b17068:
    # ver "A-hh, h-hang in there, Rosie...♥"
    ver ""

translate id w3PunishGame_af963ae3:
    # fel "U-ugh-!"
    fel ""

translate id w3PunishGame_41cedbd6:
    # "{b}*Swwwaaahp!*{/b}"
    ""

translate id w3PunishGame_3d6baced:
    # fel "D-damn it! Give me what she's having, at least!"
    fel ""

translate id w3PunishGame_a337fcff:
    # ver "A-ahh... I'm curious just how skin as tan as yours will turn red..."
    ver ""

translate id w3PunishGame_8f1adb7c:
    # fel "Ahh, f-f-hhhu...♥ Y-you're a real piece of work!"
    fel ""

translate id w3PunishGame_76ac9e75:
    # fel "Despite all your bitching, you're having fun!"
    fel ""

translate id w3PunishGame_f1fa0882:
    # ver "G-gah, hnn...♥ Maybe it's a bit hypocritical, b-buht..."
    ver ""

translate id w3PunishGame_7d2069cd:
    # fel "Ghhh♥"
    fel ""

translate id w3PunishGame_2697910e:
    # ver "...silver linings or {b}whatever{/b}!"
    ver ""

translate id w3PunishGame_d319a7a8:
    # "*Thwap!*"
    ""

translate id w3PunishGame_ab9b2f2d:
    # ver "Does that feel good, you crazy slut!"
    ver ""

translate id w3PunishGame_3c9fe3c4:
    # fel "G-gah... f-fuck... {b}k-kinda{/b}...?!"
    fel ""

translate id w3PunishGame_577aaee6:
    # ver "Judging by h-how I'm currently feeling... with h-how much my skin is burning... I bet this thing even feels like a brief distraction and relief...♥♥♥"
    ver ""

translate id w3PunishGame_4d48b395:
    # fel "Maybe it does, but I'd still rather have-"
    fel ""

translate id w3PunishGame_7cbf613b:
    # rose "Gh, hhh--♥"
    rose ""

translate id w3PunishGame_c718a81e:
    # fel "...what she's having."
    fel ""

translate id w3PunishGame_8ddfc543:
    # ver "N-no shit...♥ Me, t-tooo...♥"
    ver ""

translate id w3PunishGame_38b39e53:
    # ver "I've got it worse than you guys-♥ I have two of you in front of me, l-looking like you do...♥"
    ver ""

translate id w3PunishGame_5c743df9:
    # ver "Hnnn...♥♥♥ But no one to h-help me f-feel... aaach...♥ {b}D-damn it!{/b}"
    ver ""

translate id w3PunishGame_d140bcdd:
    # ver "{b}This is inadequate...!{/b}"
    ver ""

translate id w3PunishGame_44e50c07:
    # rose "W-what are you...?"
    rose ""

translate id w3PunishGame_637a113b:
    # fel "H-hey, what about me?"
    fel ""

translate id w3PunishGame_81bb6044:
    # ver "What did I tell you about waiting your turn, Blondie?"
    ver ""

translate id w3PunishGame_3f559866:
    # rose "W-woaahh...? Ahh...? Don't dro--"
    rose ""

translate id w3PunishGame_b4efd0d4:
    # ver "You think you got that to worry about with me? Don't worry, I've got you, beautiful."
    ver ""

translate id w3PunishGame_f4a668a0:
    # rose "B-but... aah... I don't think you were supposed to take me down... Mrs. Pulman took a lot of time setting us-"
    rose ""

translate id w3PunishGame_1bd83f4d:
    # kat "I figured it might go this way."
    kat ""

translate id w3PunishGame_e6ae23fd:
    # mc "B-bba- y-you did?!"
    mc ""

translate id w3PunishGame_80640464:
    # kat "Of course. It's no surprise that Veronica would go off script."
    kat ""

translate id w3PunishGame_f21a0216:
    # kat "The real kicker was always going to come after she comes to her senses and realizes just how easily she gives into her lust."
    kat ""

translate id w3PunishGame_d8409389:
    # mc "You are--"
    mc ""

translate id w3PunishGame_6b20472e:
    # kat "What I AM surprised about is how you haven't soiled my hand yet."
    kat ""

translate id w3PunishGame_5bb63c34_3:
    # mc "Hnngg!"
    mc ""

translate id w3PunishGame_7f926880_3:
    # kat "I know one time won't suffice. You don't have to fear; I'll milk you as much as you require~"
    kat ""

translate id w3PunishGame_c6f5cfb8_2:
    # "At this point, a stiff breeze would have gotten me off, but for how slow it was, the old woman's technique was thorough and {i}immaculate{/i} in its consistency. She never slowed down or sped up; she just jerked me like her wrist operated on a piston."
    ""

translate id w3PunishGame_7a26e7dc_3:
    # mc "G-ahh, s-shut the fuck up and just k-keep--!"
    mc ""

translate id w3PunishGame_40ab8cb0_3:
    # "The real magic lay in how she used her pinky, tracing its black nail up and down the underside of my shaft, never deviating a single millimeter from the same tract of overwrought nerve endings."
    ""

translate id w3PunishGame_ca9271b5_2:
    # kat "*Whisper* You should speak kindly to someone doing you a favor, [mcf]."
    kat ""

translate id w3PunishGame_fa952a65_3:
    # mc "*Gulp* H-a, haa... favor...? Yeah right..."
    mc ""

translate id w3PunishGame_56b5e418_2:
    # "It was like the words apparated on my lips without even being thought of. I said what I felt without control or filter."
    ""

translate id w3PunishGame_93bce481_2:
    # mc "T-take a sit and spin on it if you really want to do me a favor!"
    mc ""

translate id w3PunishGame_a2ad0a0e_3:
    # kat "You're adorable right now, you know that?"
    kat ""

translate id w3PunishGame_1bd83f4d_1:
    # kat "I figured it might go this way."
    kat ""

translate id w3PunishGame_995dad76:
    # mc "Y-you did...?"
    mc ""

translate id w3PunishGame_56d4a825_3:
    # "With the way I counted it, I had a couple of options for managing my sickening arousal."
    ""

translate id w3PunishGame_10efc326_3:
    # "I could let my mind wander, try to think of something else... but invariably, my senses fell on the women next to me, to the curves of their busts and their waists..."
    ""

translate id w3PunishGame_e4fa996b_3:
    # "These two women were so far above me... the old woman decades my senior. Sophia, a brilliant scientist... it was so easy to imagine the delight in grabbing one of them right now and just--"
    ""

translate id w3PunishGame_2389bacb_3:
    # mct "(Y-yeah... no... I should focus on what's playing out in front of me...)"
    mct ""

translate id w3PunishGame_3427b534_3:
    # "The other option was to let Kathleen's game preoccupy my attention."
    ""

translate id w3PunishGame_9bb4dc8e:
    # kat "Of course. It's no surprise that she'd go off script."
    kat ""

translate id w3PunishGame_f21a0216_1:
    # kat "The real kicker was always going to come after she comes to her senses and realizes just how easily she gives into her lust."
    kat ""

translate id w3PunishGame_20ec7c8b_1:
    # mct "(...that's gotta be the best way of dealing with this, right?)"
    mct ""

translate id w3PunishGame_ef6218db:
    # kat "By the way... do you need to be excused, Mr. [mcl]? Perhaps you need to go to the bathroom?"
    kat ""

translate id w3PunishGame_c8c82b40_3:
    # mc "N-no, I'm f-fine..."
    mc ""

translate id w3PunishGame_1050a010_3:
    # "She teased me, and strangely, I felt an unusual rush of pride. Of course, it was irrational, but that would be akin to losing..."
    ""

translate id w3PunishGame_54c72150_3:
    # "Losing what and to who, I did not know... but my willpower... I was stronger than this..."
    ""

translate id w3PunishGame_041c023d_3:
    # mct "(I ain't a fucking animal... right?)"
    mct ""

translate id w3PunishGame_baea541c:
    # rose "W-w-ah-? P-put me doo--"
    rose ""

translate id w3PunishGame_b21ee2ea:
    # ver "Ahh- relax, you're in good hands!"
    ver ""

translate id w3PunishGame_8ac7c9d7:
    # "In an exceedingly impressive display of back strength, Veronica flipped Rosalind around and had the MILF hold onto her body as if she was climbing a tree."
    ""

translate id w3PunishGame_fa81231a:
    # rose "N-neee... d-don't drop-"
    rose ""

translate id w3PunishGame_a26062a3:
    # ver "You have no idea how much I want to..."
    ver ""

translate id w3PunishGame_28f3abad:
    # rose "Ah, m-my-♥"
    rose ""

translate id w3PunishGame_91162de2:
    # "Finishing her sentence, the redhead dived into Rosalind's bust, submerging herself in a sea of sensuous tit-flesh."
    ""

translate id w3PunishGame_f01f9c7e:
    # rose "{b}Haaaaa-♥{/b}"
    rose ""

translate id w3PunishGame_dd079b9c:
    # "With how her head bobbed and shook, you could make a pretty good guess about what was going through Veronica's mind."
    ""

translate id w3PunishGame_b4e73c59:
    # fel "S-seriously?! Are you fucking motorboating her?!"
    fel ""

translate id w3PunishGame_57d537d0:
    # "In my own state, I intimately sympathized with her. She was getting a fix and quelling a poisonous desire, breathing in Rosalind like her life depended on it."
    ""

translate id w3PunishGame_4ac14d5b:
    # ver "Fwwwuuah, fmmmhmwww-♥"
    ver ""

translate id w3PunishGame_7fffd1aa:
    # "-gnawing at her chest like she would sustain her."
    ""

translate id w3PunishGame_14e1a789:
    # fel "Come- aah- w-what the fuck? You barely even-"
    fel ""

translate id w3PunishGame_884a95cf:
    # "Felicia's complaint fell on deaf ears; Veronica was too rightfully enamored with the bounty before her."
    ""

translate id w3PunishGame_081b6e49:
    # ver "Mmmh, mmhmhh-♥"
    ver ""

translate id w3PunishGame_731e1b76:
    # "Before long, Rosalind became less concerned with her precarious footing and more concerned with scratching her own itch."
    ""

translate id w3PunishGame_91e390d6:
    # rose "Mmmmmh-♥"
    rose ""

translate id w3PunishGame_246d0743:
    # "It started from a series of simple, pleasurable shudders. Her body squirmed and rocked until all that energy concentrated and found an outlet in her waist."
    ""

translate id w3PunishGame_ffb7d077:
    # rose "Oh, o-ohhhh-♥"
    rose ""

translate id w3PunishGame_65ea54e0:
    # "In a desperate bid for {i}something{/i}... {b}anything...{/b} like a cat in heat, Rosalind rubbed herself against Veronica's unbending form, looking for relief."
    ""

translate id w3PunishGame_0922a025:
    # rose "Oh, mmy-♥♥"
    rose ""

translate id w3PunishGame_f1225204:
    # "*Chwup, fwhup~!*"
    ""

translate id w3PunishGame_e5062a03:
    # "The redhead knew where to attack. {b}She knew{/b} her weakness."
    ""

translate id w3PunishGame_9bacc8c1:
    # rose "Oh, g-god-♥♥♥ Oh, g-gosh-♥♥♥♥"
    rose ""

translate id w3PunishGame_a2fb9f17:
    # "That wonderfully perfect weakness for a woman like Rosalind."
    ""

translate id w3PunishGame_5423cef1:
    # rose "V-vero-♥"
    rose ""

translate id w3PunishGame_0aa58df0:
    # "The old woman's grip on my cock did nothing to assuage the envious pangs I felt as I desired to get my hands and cock inside all three women who occupied the screen."
    ""

translate id w3PunishGame_98fdcd07:
    # "{b}Damn it{/b}, this wasn't enough..."
    ""

translate id w3PunishGame_f1bc2787:
    # "Envious pangs assailed me as I desired to get my own hands and cock inside all three women who occupied the screen."
    ""

translate id w3PunishGame_273aeb88:
    # mct "(I'm going crazy here...)"
    mct ""

translate id w3PunishGame_f837ed28:
    # "*{b}Chwup, khwup, chwhhup!{/b}*"
    ""

translate id w3PunishGame_980a7424:
    # rose "W-wooah-♥ That feels... it's s-so h-hhoooot~ b-buh-burning up... every time - I f-forg-♥"
    rose ""

translate id w3PunishGame_0b361ecb:
    # "Every time Veronica {i}inhaled{/i}, Rosalind's expression said it all."
    ""

translate id w3PunishGame_d29fabdb:
    # rose "Woohh, g-god, gggoooood-♥ V-verhherronica-♥"
    rose ""

translate id w3PunishGame_f3384496:
    # "On a normal day, she was critically weak there... on a day like today, in a situation like this..."
    ""

translate id w3PunishGame_3f11de2f:
    # rose "Hhhnnwwooo, o-ohohhhgoawwwd-♥"
    rose ""

translate id w3PunishGame_7d23a4f0:
    # "{b}She had no hope{/b}."
    ""

translate id w3PunishGame_63aca84a:
    # rose "Wooohhaaa-♥"
    rose ""

translate id w3PunishGame_d7623fcd:
    # "No hope but to surrender to the strong woman's whims and desires."
    ""

translate id w3PunishGame_a7abf686:
    # "{b}*Chwup, fhwwup, fhhwwup, gwhhup!*{/b}"
    ""

translate id w3PunishGame_3119b693:
    # "No choice but to play her part in Mrs. Pulman's game of erotic theater and relinquish all control to her competitor."
    ""

translate id w3PunishGame_826301ae:
    # "No choice but to play her part in Kathleen's game of erotic theater and relinquish all control to her competitor."
    ""

translate id w3PunishGame_67670d33:
    # rose "Woohh-♥♥ G-gughh-♥"
    rose ""

translate id w3PunishGame_c9a3a38f:
    # "If she didn't understand that intelligently, then at least her body was quick on the take."
    ""

translate id w3PunishGame_28dca0ba:
    # rose "Hhhuk-♥ Blawwwnkss-♥"
    rose ""

translate id w3PunishGame_160b501c:
    # "{b}*Chwup, fhwwup, fhhwwup, gwhhup, whhwhhup, chwup, chhup-!!!!*{/b}"
    ""

translate id w3PunishGame_b660ea57:
    # rose "Hngngggg- ohh, noo... ggghhhhoodd~♥"
    rose ""

translate id w3PunishGame_547dcbc8:
    # "Rosalind's expression soon went cross-eyed."
    ""

translate id w3PunishGame_48735aaa:
    # "*Chwup, fwhup, kwhup!*"
    ""

translate id w3PunishGame_749e5340:
    # rose "Heehhy♥ Oohwwwh♥"
    rose ""

translate id w3PunishGame_bf032848:
    # "In no time at all, pleasure quickly replaced all semblance of intelligence in her eyes."
    ""

translate id w3PunishGame_94c41685:
    # rose "Feeelss-aaahh-"
    rose ""

translate id w3PunishGame_39991b60:
    # "Veronica's ministrations were her ticket to forget how badly her body burned."
    ""

translate id w3PunishGame_17e120ba:
    # rose "Wwee-♥ Ggoowwdd, Verrrhho-♥"
    rose ""

translate id w3PunishGame_f0d4c6d1:
    # "A comfort that Veronica hadn't yet afforded her own self."
    ""

translate id w3PunishGame_be14d666:
    # rose "Ghh-♥ Huuhh-♥"
    rose ""

translate id w3PunishGame_278c7e29_7:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_13:
    # "......"
    ""

translate id w3PunishGame_1e46b70e:
    # ver "{b}Ghhaah{/b}...!"
    ver ""

translate id w3PunishGame_d14a8b79:
    # rose "W-wwooah, ww-why you sttohp? Ahehh..."
    rose ""

translate id w3PunishGame_d2366dbb:
    # ver "S-sorry, Rose."
    ver ""

translate id w3PunishGame_79eda955:
    # rose "Whha-?"
    rose ""

translate id w3PunishGame_3ec59ce4:
    # ver "Felicia is looking extra fine over there."
    ver ""

translate id w3PunishGame_9fdff224:
    # rose "Ah...? Oh--"
    rose ""

translate id w3PunishGame_ce6dc6ad:
    # ver "Sit tight for a minute."
    ver ""

translate id w3PunishGame_204231cd:
    # fel "H-aahh... f-finally! G-give me one of those t-toys!"
    fel ""

translate id w3PunishGame_875bbfcd:
    # ver "That's not what I had in mind..."
    ver ""

translate id w3PunishGame_1613afcf:
    # kat "Well, this direction is bringing up some recent memories."
    kat ""

translate id w3PunishGame_2ad14dd3:
    # sophia "Is that right?"
    sophia ""

translate id w3PunishGame_d97839b9:
    # kat "She was much less friendly with me, but Miss Lynch is rather straightforward."
    kat ""

translate id w3PunishGame_4a991672_2:
    # mc "H-hhhhngg...!"
    mc ""

translate id w3PunishGame_274a48d8_3:
    # "Like hell my mind was registering any of her bullshit right now."
    ""

translate id w3PunishGame_731bfb96:
    # fel "H-heuuuh, w-waaah?"
    fel ""

translate id w3PunishGame_66bcf210:
    # sophia "Did you expect this kind of fervor?"
    sophia ""

translate id w3PunishGame_83473487_3:
    # mc "H-a, hnnggaa..."
    mc ""

translate id w3PunishGame_ec324fcf:
    # ver "How the hell does this thing open?!"
    ver ""

translate id w3PunishGame_e7a38a2a_3:
    # "The inside of my head was mush right now. My capacity for language was utterly eroded."
    ""

translate id w3PunishGame_93fd3301_6:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_de5fea97:
    # kat "I'm well acquainted with the peculiar ways people respond when they're horny. For a woman like her, getting off is as simple as going from point A to point B."
    kat ""

translate id w3PunishGame_36b179c8:
    # sophia "I'm impressed."
    sophia ""

translate id w3PunishGame_2b5022f9_3:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your owner."
    kat ""

translate id w3PunishGame_d2a0be0e_6:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df_6:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108_6:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418_6:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_087867e3_6:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_ee058e63:
    # fel "Ehh-? Where the hell do you think you're putting your fat--"
    fel ""

translate id w3PunishGame_1613afcf_1:
    # kat "Well, this direction is bringing up some recent memories."
    kat ""

translate id w3PunishGame_731bfb96_1:
    # fel "H-heuuuh, w-waaah?"
    fel ""

translate id w3PunishGame_7fbd2e1f_3:
    # "To my annoyance, whenever one of them spoke, my attention would turn back to just how fuckable the women next to me were. "
    ""

translate id w3PunishGame_ec324fcf_1:
    # ver "How the hell does this thing open?!"
    ver ""

translate id w3PunishGame_cc8fba32:
    # sophia "Is that right? You counted on this kind of... {i}fervor{/i}?"
    sophia ""

translate id w3PunishGame_96a9d185_3:
    # mct "(Christ, I want to make that mouthy hag choke on my dick until she passes out!)"
    mct ""

translate id w3PunishGame_f94b31a8_3:
    # "Intrusive thought upon intrusive thought flooded my head."
    ""

translate id w3PunishGame_93fd3301_7:
    # kat "I've been doing this for a long time. Before the club, I had my {i}other{/i} enterprises."
    kat ""

translate id w3PunishGame_de5fea97_1:
    # kat "I'm well acquainted with the peculiar ways people respond when they're horny. For a woman like her, getting off is as simple as going from point A to point B."
    kat ""

translate id w3PunishGame_36b179c8_1:
    # sophia "I'm impressed."
    sophia ""

translate id w3PunishGame_5a5d8551_3:
    # kat "You shouldn't be. I'm an amateur when compared to someone like your Master."
    kat ""

translate id w3PunishGame_d2a0be0e_7:
    # kat "I think the way Van Doren has you under his thumb is much more impressive."
    kat ""

translate id w3PunishGame_6af6f0df_7:
    # sophia "Thank you."
    sophia ""

translate id w3PunishGame_1cdbd108_7:
    # kat "You took that as a compliment?"
    kat ""

translate id w3PunishGame_aca2d418_7:
    # sophia "Of course I did. Abel only has the best things."
    sophia ""

translate id w3PunishGame_087867e3_7:
    # kat "Your self-awareness in your choice of words is commendable, dear."
    kat ""

translate id w3PunishGame_ee058e63_1:
    # fel "Ehh-? Where the hell do you think you're putting your fat--"
    fel ""

translate id w3PunishGame_ec645b76:
    # fel "E..eeh? Who do you think you are pushing me to the-"
    fel ""

translate id w3PunishGame_8e411e9e:
    # ver "I'm the one that old bat didn't tie up."
    ver ""

translate id w3PunishGame_3ef7a36c:
    # ver "We're in the middle of a show, or did you forget?"
    ver ""

translate id w3PunishGame_78394d1d:
    # fel "Euugh, yeah, but y-you're usually not so--"
    fel ""

translate id w3PunishGame_818e0130:
    # ver "Feeling neglected? Like you're not the center of attention for once?"
    ver ""

translate id w3PunishGame_96e40917:
    # fel "Mmhuihhno-"
    fel ""

translate id w3PunishGame_b8157afb:
    # ver "That must be tough for you, Blondie."
    ver ""

translate id w3PunishGame_db4c0349:
    # ver "D-don't get mad at me if I take advantage a little... you understand, right? You're feeling the same thing I am... a-ahh...♥"
    ver ""

translate id w3PunishGame_10755793:
    # ver "Just be a good girl for me and I'll be nice in return."
    ver ""

translate id w3PunishGame_f33ee24f:
    # "Veronica's words came out husky and drenched in honey."
    ""

translate id w3PunishGame_da3fa95f:
    # fel "Hnnggg-♥"
    fel ""

translate id w3PunishGame_a8034576:
    # "Felicia shuddered."
    ""

translate id w3PunishGame_2080e0cb:
    # ver "'atta girl, you're a lot more passive when you have something in your mouth, aren't you?"
    ver ""

translate id w3PunishGame_27335fea:
    # "Hell, even I shuddered from the other side of TV land."
    ""

translate id w3PunishGame_199f13fd:
    # rose "This is supposed to be a punishment... Mrs. Pulman might get ma-"
    rose ""

translate id w3PunishGame_c388acfd:
    # ver "Shut up, Rosie. Where's that backbone you grew when you slapped that cunt?"
    ver ""

translate id w3PunishGame_17115caa:
    # rose "I- tsssk-!"
    rose ""

translate id w3PunishGame_97487c92:
    # ver "A good show is a good show, and if you haven't figured it out, she wants to see how we respond when she throws a lit match in a tinderbox and leaves us to our own devices."
    ver ""

translate id w3PunishGame_cf3ac76b:
    # ver "Well... this is how {b}I{/b} respond."
    ver ""

translate id w3PunishGame_cfb59459:
    # fel "Heheh... you don't like me, Red, but you sure want to {b}fuck me.{/b} I've s-seen the way you occasionally look at me during the shows."
    fel ""

translate id w3PunishGame_cb285e36:
    # ver "Well, Blondie... not {i}everything{/i} you say is bullshit. For now..."
    ver ""

translate id w3PunishGame_4fd92fb3:
    # fel "Mmmhh-?!"
    fel ""

translate id w3PunishGame_689227b6:
    # ver "{b}A-ah-♥{/b} L-let's be friends-♥"
    ver ""

translate id w3PunishGame_28bad329:
    # fel "Mmmhh, mmhhh-!!!"
    fel ""

translate id w3PunishGame_7ce22cf9:
    # "Without much warning, Veronica's large ass smothered Felicia's face and drummed up a startled reaction."
    ""

translate id w3PunishGame_1886eec9:
    # ver "H-haa- a-and what better way of being friends than this?"
    ver ""

translate id w3PunishGame_81b6e18f:
    # ver "{b}...lick!{/b}"
    ver ""

translate id w3PunishGame_616596b4:
    # ver "J-just FYI, I make it a habit to reciprocate the kindness of friends-♥"
    ver ""

translate id w3PunishGame_731d8a04:
    # fel "Mmmhhmmhmhh-!"
    fel ""

translate id w3PunishGame_05ae57ec:
    # ver "How do I taste Blondie?"
    ver ""

translate id w3PunishGame_9170c0c8:
    # fel "Mmmhh-♥ Eeuhuh-♥"
    fel ""

translate id w3PunishGame_eecc9c68:
    # "From the looks of it, Felicia {i}was{/i} using her tongue, but she was on the back foot."
    ""

translate id w3PunishGame_5573f842:
    # ver "A-ah, don't try to multitask by answering t-that-♥ I want all your focus on-"
    ver ""

translate id w3PunishGame_4a61f1ab:
    # "*Shlick, shlwap, fwhiich-!*"
    ""

translate id w3PunishGame_8eb42ac5:
    # "Felicia fought desperately to match the Amazon's erratic rhythm, to find a way to consistently pry apart Veronica's sex and use her pink tongue to plumb her depths."
    ""

translate id w3PunishGame_3f922619:
    # fel "Eeuhh-♥ Eeuuhh-"
    fel ""

translate id w3PunishGame_31eb4f59:
    # "She was clearly {i}trying{/b}, but the unhinged way Veronica dragged her sex across her face made it difficult."
    ""

translate id w3PunishGame_c5aae977:
    # ver "A-ah, o-oh-♥ I h-haven't been this turned on since I had that bitch over my knee-"
    ver ""

translate id w3PunishGame_51aefa70:
    # "The scene playing out on screen was more akin to a desperate housewife grinding herself against the corner of a washing machine than the act of two people sharing in cunnilingus."
    ""

translate id w3PunishGame_59fbddb0:
    # fel "Mmhh, hhmhm-♥"
    fel ""

translate id w3PunishGame_fbb65242:
    # ver "Or m-maybe since... a-ahh... last week's photo shoot? Aeeeuhh-♥ W-who can f-fucking sayyyy-♥♥"
    ver ""

translate id w3PunishGame_de89e484:
    # fel "Eeeuh, wwwhaa-♥"
    fel ""

translate id w3PunishGame_faea57d1:
    # ver "N-nice work. You're not amazing at this, but I can tell you're hungry for it. Keep at it and you'll get-- nnggg!"
    ver ""

translate id w3PunishGame_1727a64c:
    # "I don't think Veronica cared about Felicia's actual skill or effort. She was too high on the power of it all."
    ""

translate id w3PunishGame_457ba2ed:
    # fel "Hnnggg-♥ Hnngguuk-♥"
    fel ""

translate id w3PunishGame_57de2b00:
    # "She was getting off on having the beautiful face of a woman she held in contempt buried in her ass."
    ""

translate id w3PunishGame_fcf9ec36:
    # ver "Ah, y-yes. {b}YES!{/b} By the way, you were right..."
    ver ""

translate id w3PunishGame_d6f67f6f:
    # "Veronica had been playing it remarkably slowly up to this point, enduring her sexual urge by teasing the girls, but now it was all coming out."
    ""

translate id w3PunishGame_d1ce1612:
    # fel "Mmhh, ueeeuhh-♥?"
    fel ""

translate id w3PunishGame_43d37c6d:
    # ver "I do think you're beautiful. I, m-mean... h-how could I not?"
    ver ""

translate id w3PunishGame_1825e817:
    # fel "Mmmffff-!"
    fel ""

translate id w3PunishGame_cc1711fd:
    # ver "Hnn-♥ N-now- a-ah, you've been a good friend! So I think it's time... ah..."
    ver ""

translate id w3PunishGame_e1d3b6da:
    # fel "MMhmh.. ohmhmhwoohh..? A-aahammwwhh-♥"
    fel ""

translate id w3PunishGame_126fbf21:
    # "Finally, Veronica made good on her promise of lick and be licked."
    ""

translate id w3PunishGame_905b0b06:
    # fel "MMmhh, aaa-hh- hhaa-aaah♥♥♥"
    fel ""

translate id w3PunishGame_f9e636e5:
    # "Veronica leaned forward and buried herself in Felicia's crotch, creating an attractive, parallel line of two bodacious figures for the camera."
    ""

translate id w3PunishGame_a2fd0c49:
    # "*Shlick, fwhiick...!"
    ""

translate id w3PunishGame_f433a399:
    # "It was a work of art how they grasped and held each other."
    ""

translate id w3PunishGame_cb53f660:
    # fel "Hnngg-♥"
    fel ""

translate id w3PunishGame_b5d809e7:
    # "Long gorgeous legs moved to get out of the way, finding tight purchase hugging and forcing each other's bodies closer."
    ""

translate id w3PunishGame_016824b7:
    # "*Shlick, fwhiick, hhwwiick...!*"
    ""

translate id w3PunishGame_3fba1648:
    # rose "Uugghh... now I'm the one left out..."
    rose ""

translate id w3PunishGame_016824b7_1:
    # "*Shlick, fwhiick, hhwwiick...!*"
    ""

translate id w3PunishGame_447885b7:
    # "Felicia and Veronica continued to pleasure each other, while I..."
    ""

translate id w3PunishGame_afe11dd4:
    # "{b}*Shlick, shlick, shlick, fwhhick, hwwwiiicki!*{/b}"
    ""

translate id w3PunishGame_852090ea:
    # ver "Then get over here!"
    ver ""

translate id w3PunishGame_01d6aee3_3:
    # kat "I'm starting to get offended, Mr. [mcl]. Does my hand not suffice?"
    kat ""

translate id w3PunishGame_06035a29_3:
    # kat "Don't you want to taste release?"
    kat ""

translate id w3PunishGame_31a83b72_3:
    # mc "G-gah, f-fuck! It's not like I have any control over it!"
    mc ""

translate id w3PunishGame_fd6b335d_3:
    # sophia "Hmm..."
    sophia ""

translate id w3PunishGame_501f6d67_3:
    # "Every fiber of my being desperately anticipated climax. My reason had been eroded and my purpose replaced by the critical need to expunge every trace of nauseating desire from my body through my cock."
    ""

translate id w3PunishGame_15dcbcac_3:
    # "The hairs on my arm stood on end, the feeling in my extremities came and went like the tide, and a pervasive tingling feeling floated from my neck down to my tailbone - by all means, my body told me that I {b}should{/b} be coming."
    ""

translate id w3PunishGame_cc175353_3:
    # sophia "Interesting."
    sophia ""

translate id w3PunishGame_2438123c_3:
    # "But I wasn't. My bloated dick burned against the soft palm of the old woman's hand, pulsating and twitching from the ceaseless stimulation, filling me with the irrational fear that I might literally explode if I didn't soon go careening into the void of sexual release."
    ""

translate id w3PunishGame_5a852968_3:
    # sophia "All things considered, he is showing a {b}surprising{/b} degree of longevity..."
    sophia ""

translate id w3PunishGame_26f61ebe_3:
    # kat "Maybe your fragrance isn't as effective as you think it is."
    kat ""

translate id w3PunishGame_a2f58e6b_3:
    # mc "G-gaahh!"
    mc ""

translate id w3PunishGame_ecfe8072_3:
    # sophia "Maybe..."
    sophia ""

translate id w3PunishGame_46144357_3:
    # mct "(No! It was definitely fucking effective!)"
    mct ""

translate id w3PunishGame_5db42400_3:
    # sophia "...but unlikely. Hey, [mcf]..."
    sophia ""

translate id w3PunishGame_dab66f0d_3:
    # "Through lidded eyes, I could sense the blonde scooching closer and closer."
    ""

translate id w3PunishGame_08efb90d_3:
    # sophia "The issue is that a young, well-traveled guy like you simply can't get off on an old woman's touch, right?"
    sophia ""

translate id w3PunishGame_d36256e0_3:
    # mc "That's not--"
    mc ""

translate id w3PunishGame_93779299_3:
    # sophia "{b}{i}Isn't it?{/b}{/i}"
    sophia ""

translate id w3PunishGame_76f7a8ba_3:
    # "Even in my lust-addled state..."
    ""

translate id w3PunishGame_db1a6cb6_3:
    # "My brain registered amazement over the willful way Sophia flipped the switch, going from detached aloofness to a man-killer just as easily as one would shed their clothes. "
    ""

translate id w3PunishGame_061fb09e_3:
    # mc "Hnngg...!"
    mc ""

translate id w3PunishGame_beef1333_3:
    # kat "*Scoff* Feeling left out, were you?"
    kat ""

translate id w3PunishGame_bfa0648c_3:
    # sophia "Give them a comparison."
    sophia ""

translate id w3PunishGame_18b36fa6_3:
    # "My hand responded independently, sinking into the pleasant give of the doctor's tit-flesh in an instant as what remained of my faculties disappeared into the whirlpool of her eyes."
    ""

translate id w3PunishGame_04a18dd7_3:
    # sophia "You got a mess lurking somewhere in there that you want to get out?"
    sophia ""

translate id w3PunishGame_0f885ed4_3:
    # mc "Ha... hnngg...! F-fuck it!"
    mc ""

translate id w3PunishGame_5d2b485c_3:
    # sophia "...?!"
    sophia ""

translate id w3PunishGame_cc522673_3:
    # "I kissed her, just as you'd expect a man on the brink of starvation to steal a loaf of bread."
    ""

translate id w3PunishGame_02060d5f_3:
    # sophia "Mmmhhhh....!"
    sophia ""

translate id w3PunishGame_8c6f3eee_3:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Mrs. Pulman."
    ""

translate id w3PunishGame_b5e1efa6_3:
    # "She might not have expected it or even wanted it, but she quickly accepted it as a consequence of the \"just a bit of chaos\" she wielded in her joust against Kathleen."
    ""

translate id w3PunishGame_e13d5d67_3:
    # kat "H-hey, ahh--"
    kat ""

translate id w3PunishGame_2430bc3d_3:
    # "Spurred on by having the attention of two beautiful women, even if it was just the fucked up equivalent of cooking ants with a magnifying glass to them, I knew it wouldn't be long until I finally had my release."
    ""

translate id w3PunishGame_87732b5f_3:
    # kat "Gah, ah... ha... hmpfh."
    kat ""

translate id w3PunishGame_353b1cca_3:
    # "Although I could feel her eyes on my neck, the old woman continued to milk me."
    ""

translate id w3PunishGame_61937b68_3:
    # "*Cwhup, fwhup, chwup...!*"
    ""

translate id w3PunishGame_28eed101_3:
    # kat "I've somehow ended up feeling like a third wheel."
    kat ""

translate id w3PunishGame_278c7b3e_3:
    # "I pawed at both women's breasts with a virgin-like fervor, wholly enamored by an opportunity I might never get again."
    ""

translate id w3PunishGame_5d459ae6_3:
    # mct "(The head of a charity and a world-class scientist... ha, fucking sluts!)"
    mct ""

translate id w3PunishGame_e128df9b_3:
    # "I felt like a king, not of my own making of course, but my brain was so cooked as to not know the difference. Thinking was hard, but what few thoughts I could miraculously formulate were like tidal waves breaking against the shore."
    ""

translate id w3PunishGame_0ed4c646_3:
    # mct "(Who the hell do you think you're fucking with? You think you can toy with me just because of money?!)"
    mct ""

translate id w3PunishGame_b117f61e_3:
    # "I felt smothered, suffocated, and hot. More akin to a wick that was about to burn out than a human being."
    ""

translate id w3PunishGame_9c75dc6b_3:
    # mct "(I'M A GUINEA PIG TO YOU?!)"
    mct ""

translate id w3PunishGame_07bbfc1c_3:
    # "Every sensation felt heightened, and every second felt prolonged... I genuinely feared that the moment I popped would be when I was snuffed out."
    ""

translate id w3PunishGame_a73faf04_3:
    # mct "{size=+30}{font=/gui/fonts/MB-Thin_Worms.ttf}(You like to toy with pe-){/font}{/size}"
    mct ""

translate id w3PunishGame_61867711_3:
    # "My brain gave out, but if I ceased to exist after this, I instinctively knew one thing: these last minutes of my life should be bold."
    ""

translate id w3PunishGame_ee7c2e13_3:
    # mc "I'm about to b-blow..."
    mc ""

translate id w3PunishGame_01a79887_3:
    # kat "You don't need my permission."
    kat ""

translate id w3PunishGame_2221f5ba_3:
    # mc "Can you finish me with your mouth?"
    mc ""

translate id w3PunishGame_fde31e51_3:
    # kat "Hmmmmm, not going to happen."
    kat ""

translate id w3PunishGame_eefd493d_3:
    # mc "A-ah, pretty please? With a cherry on top?"
    mc ""

translate id w3PunishGame_4469f1d1_3:
    # kat "Why don't you ask Miss Lundgren to do it?"
    kat ""

translate id w3PunishGame_6032c009_3:
    # mc "...because I want you, Ma'am."
    mc ""

translate id w3PunishGame_7d48d431_3:
    # kat "...hmmmmmmmm. You have the eyes of a crazed animal, yet..."
    kat ""

translate id w3PunishGame_2d6e0df6_3:
    # kat "You're asking so nicely. God, I love that."
    kat ""

translate id w3PunishGame_af9d9795_3:
    # "She smirked like she had won against me, and perhaps even against Sophia."
    ""

translate id w3PunishGame_76125b85_3:
    # kat "It really is goddamn adorable. Ask me again, [mcf]."
    kat ""

translate id w3PunishGame_8ce7b843_3:
    # "A desperate, cloying urge inside me told me her mouth was the only place I wanted to be right now and if it took a bit of pleading to get me into a position where I could fill that mean bitch's stomach with my seed, then so be it."
    ""

translate id w3PunishGame_ca96d738_3:
    # mc "Hhh, haa... would you PLEASE finish me with your mouth, Mrs. Pulman?"
    mc ""

translate id w3PunishGame_e2b980af_3:
    # "Without another word, Mrs. Pulman lowered herself and..."
    ""

translate id w3PunishGame_fc8cf5a3_3:
    # "Without another word, Kathleen lowered herself and..."
    ""

translate id w3PunishGame_109a7847_3:
    # mc "G-gah...!"
    mc ""

translate id w3PunishGame_d66b4def_3:
    # "The moment her lips wrapped themselves around my cock, I felt like I might pass out."
    ""

translate id w3PunishGame_cca0b4ef_3:
    # "Her seal was so airtight that her mouth felt like an oven. Steady puffs of hot breath passed over the head of my cock as it made its way out through her nose."
    ""

translate id w3PunishGame_d6fb4ea7_3:
    # mc "F-fuck, damn it!"
    mc ""

translate id w3PunishGame_1bd17e11_3:
    # "*Shwuck, fgwhhuck, thwuuuck!*"
    ""

translate id w3PunishGame_b39227ac_2:
    # "What little air could escape from her hold transformed into {b}obscene{/b} noises as she inched herself up and down my shaft."
    ""

translate id w3PunishGame_63238d75_3:
    # mc "H-holy-!"
    mc ""

translate id w3PunishGame_ad464d04_3:
    # "The running narration in my head was a jumbled mess, but through all the muddied noise from the pleasure soaking into my brain, I grasped a simple truth."
    ""

translate id w3PunishGame_fdbb746f_3:
    # mc "Hnnng, you've sucked a lot of cock in your time, haven't you?!"
    mc ""

translate id w3PunishGame_dad9bfec_3:
    # "*Thwaahp, fwhap, swhhuck~!*"
    ""

translate id w3PunishGame_72c8cf4a_3:
    # "The answer to that question played across my glans every time I wedged myself comfortably in her throat with zero fuss."
    ""

translate id w3PunishGame_6b12669e_3:
    # "Swhuck, hhhwhuck, fhhggguuk!*"
    ""

translate id w3PunishGame_a3af0240_3:
    # "{b}Damn it!{/b} A rich woman like her had no business sucking dick this well."
    ""

translate id w3PunishGame_d86d228f_3:
    # mc "Ghhn, haaat, hnnngg...!"
    mc ""

translate id w3PunishGame_88387910_3:
    # "She sucked me with the skill of a woman who had {b}nothing{/b} to her name."
    ""

translate id w3PunishGame_cdfdc63e_3:
    # kat "Mmmhm, hhmhm~"
    kat ""

translate id w3PunishGame_93b1b0e6_3:
    # "Just like her hand job, there was no wasted effort. She didn't tackle it in a clumsy frenzy, nor did she need to cautiously get used to my size."
    ""

translate id w3PunishGame_d628853d_3:
    # "*Shwhuhuck, fwhhucuk, ghhwhuhuk!*"
    ""

translate id w3PunishGame_41aa32dc_3:
    # "She sucked me like she had been sucking {b}my{/b} dick her whole life, pleasuring me at a steady and comfortable pace. I marveled at how, through a mere blowjob, a woman so cold and cruel could instantly imprint herself on my body as warm and welcoming."
    ""

translate id w3PunishGame_d93b0c5e_3:
    # "I was quickly understanding that \"adorable\" disparity she had just spoken."
    ""

translate id w3PunishGame_7a3794d6_3:
    # mc "Hnng, h-how?!"
    mc ""

translate id w3PunishGame_2aa14e7d_3:
    # sophia "It's not performing brain surgery."
    sophia ""

translate id w3PunishGame_9f2a1802_3:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Mrs. Pulman's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_bc9afe80_3:
    # "{b}There{/b} was another perverse delight, speeding me toward my climax. Sophia's attention was on the back of Kathleen's head, watching intently as she debased herself in pursuit of degrading me."
    ""

translate id w3PunishGame_7f97a9d7_3:
    # mct "(Haaaa... hope you like the show, bitch. You're the impetus for this.)"
    mct ""

translate id w3PunishGame_0d35f1a2_3:
    # "*Fwhhuck, ghhhukh, khggguuhhk*"
    ""

translate id w3PunishGame_b1ef8758_3:
    # mc "Ahh... y-you dick swallowing--! Hnnngg...!"
    mc ""

translate id w3PunishGame_57623603_3:
    # "It was coming."
    ""

translate id w3PunishGame_a9d0bbe1_3:
    # mc "You think I'm adorable?!"
    mc ""

translate id w3PunishGame_f8d06f5f_3:
    # "I was moments from blowing my load, and with it, the indignation from earlier swelled in me. I had wanted a bold finish before I met my end, and well..."
    ""

translate id w3PunishGame_26cd0617_3:
    # mct "(I'll show you adorable!)"
    mct ""

translate id w3PunishGame_72c12088_3:
    # mc "I'm c-cumming, fucking drown in it, you damn--"
    mc ""

translate id w3PunishGame_ec061ea7_3:
    # kat "Mmmmh?!"
    kat ""

translate id w3PunishGame_5c240311_3:
    # "Before she could even register what was happening, ropes of piping hot cum barreled down her esophagus on a one-way trip to her stomach."
    ""

translate id w3PunishGame_10060088_3:
    # kat "Ghhkka, hkkaa-!"
    kat ""

translate id w3PunishGame_4bb38f46_3:
    # "I pressed down hard, holding the old woman's head in place as she gargled my seed in abject surprise."
    ""

translate id w3PunishGame_2745bcd2_3:
    # mc "Hnngg, all of it!"
    mc ""

translate id w3PunishGame_716f46e4_3:
    # "I pressed down so hard it hurt my groin."
    ""

translate id w3PunishGame_4e4c5b02_3:
    # kat "Ghhuk, hhuuuh, hwwwhuuuck...!"
    kat ""

translate id w3PunishGame_fe1ef6b3_3:
    # "It was massive, like I was cumming my very soul down the old woman's throat."
    ""

translate id w3PunishGame_4500ed7e_3:
    # kat "Ghuk, khak, uuhhk-!"
    kat ""

translate id w3PunishGame_2239e61c_3:
    # mc "Gah, c-choke on it!"
    mc ""

translate id w3PunishGame_ab7d9ac7_3:
    # "Not all of my load was making its way down her throat. To my perverse glee, I could {i}feel{/i} myself coating the lining of her throat."
    ""

translate id w3PunishGame_7b62aa4f_3:
    # kat "Ghhuk, hhuuuk...!"
    kat ""

translate id w3PunishGame_e09b7aa7_3:
    # mc "Hope you like the fucking taste!"
    mc ""

translate id w3PunishGame_8747fdde_3:
    # kat "Mmmh, hhhngg?!?!"
    kat ""

translate id w3PunishGame_3698f7b8_3:
    # sophia "I don't think she can breathe..."
    sophia ""

translate id w3PunishGame_6d4b68da_3:
    # mc "Good! She can breathe when I'm fucking--!"
    mc ""

translate id w3PunishGame_de4a6cab_3:
    # ".........!"
    ""

translate id w3PunishGame_2068a0d5_3:
    # "......!"
    ""

translate id w3PunishGame_1d9b8a71_3:
    # mc "A-aahh...!"
    mc ""

translate id w3PunishGame_869e1b4e_3:
    # kat "Ugghhuhh..."
    kat ""

translate id w3PunishGame_ba8a394f_6:
    # "Peace."
    ""

translate id w3PunishGame_a48af359_6:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d_6:
    # "Satisfaction."
    ""

translate id w3PunishGame_92962e6b_3:
    # "In this moment, I had it all."
    ""

translate id w3PunishGame_e38ecbb2_3:
    # "With that, I happily died. The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8b8ceb72_6:
    # "............"
    ""

translate id w3PunishGame_278c7e29_8:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_14:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_14:
    # "..."
    ""

translate id w3PunishGame_87f2fe4b_3:
    # kat "Why are you holding your hands out like that? It's weird."
    kat ""

translate id w3PunishGame_2dd84f14_6:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_c2d77e46_3:
    # sophia "She's right. It's distracting."
    sophia ""

translate id w3PunishGame_1516ad37_3:
    # "I didn't know how long I was out of it, but by the time I opened my eyes, the two women had collected themselves, and what I thought was a bold display of defiance was made pitiful and wholly expected by their bemused faces."
    ""

translate id w3PunishGame_69e22dc5_3:
    # "Mrs. Pulman looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_d1fa2418_3:
    # "Kathleen looked {i}totally{/i} unaffected by my rough treatment, showing no signs of having mercilessly guzzled my cum just moments ago..."
    ""

translate id w3PunishGame_e7145261_3:
    # kat "Oh, by the way... you said some pretty outlandish things to me."
    kat ""

translate id w3PunishGame_526a5615_2:
    # mc "If I recall, I did a bit more than that..."
    mc ""

translate id w3PunishGame_2e195b22_3:
    # kat "It was {b}adorable{/b}."
    kat ""

translate id w3PunishGame_c5563c4b_3:
    # "She had indeed won."
    ""

translate id w3PunishGame_d9d8b219_3:
    # "I knew what I wanted."
    ""

translate id w3PunishGame_3576545e_3:
    # sophia "Mmmhh, hhhhh..."
    sophia ""

translate id w3PunishGame_7202d9ed_3:
    # "And that last act of defiance would be leaving the doctor, the root cause of all this, something to remember me by."
    ""

translate id w3PunishGame_293b8be4_3:
    # "*Chwup, fhwup~*"
    ""

translate id w3PunishGame_7a9110af_3:
    # "I wanted to hear this aloof bitch moan, so I focused everything on the woman whose mouth I was currently exploring."
    ""

translate id w3PunishGame_01fdbaa9_3:
    # sophia "Mmmh, hhmmm...?"
    sophia ""

translate id w3PunishGame_1efdf9e0_3:
    # "It was a battle against my wits. Sophia was literally intoxicating. Focusing my senses all on her had me burning twice as fast."
    ""

translate id w3PunishGame_d14b5b96_3:
    # "By instinct, I felt her breast over her clothes."
    ""

translate id w3PunishGame_79a3ccdd_3:
    # sophia "Mmhh..."
    sophia ""

translate id w3PunishGame_a90ff98f_3:
    # "Her coos told me she was receptive, but it wasn't quite the noise I wanted to drag out from the blonde."
    ""

translate id w3PunishGame_754fce10_3:
    # sophia "Ghu, gha~ghhuu...!"
    sophia ""

translate id w3PunishGame_be4204b5_3:
    # "Deciding that over the clothes touching wouldn't do it, I switched focus on the erogenous zone in her mouth, intensifying my efforts in an intricate dance to subdue her tongue with my own."
    ""

translate id w3PunishGame_f43fc47f_3:
    # sophia "Mmmmh, mmhhhh- *chwuph, fhwhup~*"
    sophia ""

translate id w3PunishGame_b0084126_3:
    # "She wasn't a passive observer in my hunt; her tongue played coy, darting away and slipping from my grasp whenever I thought I had her pinned."
    ""

translate id w3PunishGame_a03c3549_3:
    # "It was mere seconds, but--"
    ""

translate id w3PunishGame_27c1fdac_3:
    # "*Chwhu, fhwwwup, chup!*"
    ""

translate id w3PunishGame_f4de7f68_3:
    # "--but, in my fogged-over state, time held little sway over my perception. What was in reality a clumsy tussle felt like a long and protracted siege."
    ""

translate id w3PunishGame_2ec99952_3:
    # "I don't know if it was because of the perfume, but all bodily sensations gradually began to meld into one."
    ""

translate id w3PunishGame_45c987c8_3:
    # "The feeling of the old woman's hand milking my cock, the wet warmness of our kiss, the bubbling desire so strong that I was confident it would be the end of me..."
    ""

translate id w3PunishGame_15b0522f_3:
    # mc "Mmmh, hhhmmm-!"
    mc ""

translate id w3PunishGame_7c16ed74_3:
    # "All of it spelled an overload melded and combined into a singular feeling of otherness. I was sure I had lost this battle until..."
    ""

translate id w3PunishGame_7d144a01_3:
    # "My brain registered a change."
    ""

translate id w3PunishGame_fbfb812a_3:
    # mct "Was she...?"
    mct ""

translate id w3PunishGame_7459555f_3:
    # sophia "Mmmh, hhmhm~aahhh...!"
    sophia ""

translate id w3PunishGame_68d04ab6_3:
    # "She was closer."
    ""

translate id w3PunishGame_08a39e71_3:
    # "She was pressing into me."
    ""

translate id w3PunishGame_883ad25a_3:
    # "She felt it."
    ""

translate id w3PunishGame_b0685c2c_3:
    # "She was {b}heated{/b}."
    ""

translate id w3PunishGame_5c6a5db6_3:
    # "{i}She was kissing me back.{/i}"
    ""

translate id w3PunishGame_26629cc2_3:
    # "Whether it was done deliberately or on an instinctual level, her body responded favorably."
    ""

translate id w3PunishGame_a4d39526_3:
    # sophia "Ahh, hhhnnggg, mmmmhh-!"
    sophia ""

translate id w3PunishGame_d8ca8c90_3:
    # "My hand found a place on her ass and I used that to spur her on even more. With this, I had physical confirmation of my success."
    ""

translate id w3PunishGame_44602cec_3:
    # mct "(Fuck, what a great-!)"
    mct ""

translate id w3PunishGame_9fec863b_3:
    # "I could feel the hot dampness of her delicate spot."
    ""

translate id w3PunishGame_cb435f7f_3:
    # sophia "Mmhh, hhaa...♥♥♥"
    sophia ""

translate id w3PunishGame_71583afd_3:
    # "She purred openly and I felt a petty relief over my trivial victory. The succubus driving me to this desperate state wouldn't escape our battle unscathed."
    ""

translate id w3PunishGame_3d67a16d_3:
    # "*Chwup, fhwwup, khwwup*!"
    ""

translate id w3PunishGame_b6fdc206_3:
    # "--and that wasn't the only sense of relief I felt."
    ""

translate id w3PunishGame_cb9361ae_3:
    # mc "{b}Hnng, ah-!!!{/b}"
    mc ""

translate id w3PunishGame_cbea8557_3:
    # "I didn't know when it started, but ropes of cum geysered from my cock and rained down from the sky."
    ""

translate id w3PunishGame_4380fa10_3:
    # mc "Ghh, aaahh- sssshit--"
    mc ""

translate id w3PunishGame_a3230c6c_3:
    # kat "Huh, {b}WOW{/b}."
    kat ""

translate id w3PunishGame_74ff6633_3:
    # mc "Hngg, hhhhmmmhh-~"
    mc ""

translate id w3PunishGame_290360e4_3:
    # "All the suffocating vileness in me was expelled and released into the world. All my desperation spilled out onto our clothes and thighs."
    ""

translate id w3PunishGame_83af968d_3:
    # "Soon need was replaced with..."
    ""

translate id w3PunishGame_ba8a394f_7:
    # "Peace."
    ""

translate id w3PunishGame_a48af359_7:
    # "Serenity."
    ""

translate id w3PunishGame_e00dde6d_7:
    # "Satisfaction."
    ""

translate id w3PunishGame_2a3c0c6e_3:
    # "Emptiness."
    ""

translate id w3PunishGame_ea0f012b_3:
    # "The oblivion I felt nipping at my heels finally sank its sharp teeth into my throat."
    ""

translate id w3PunishGame_8728f5e5_3:
    # "I was dead."
    ""

translate id w3PunishGame_8b8ceb72_7:
    # "............"
    ""

translate id w3PunishGame_278c7e29_9:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_15:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_15:
    # "..."
    ""

translate id w3PunishGame_c49ce7ae_3:
    # sophia "Great, now we'll spend the rest of the show reeking of semen."
    sophia ""

translate id w3PunishGame_2dd84f14_7:
    # mct "(Ah, fuck... I {i}wasn't{/i} dead? {b}Oh no.{/b} Then that meant I just...)"
    mct ""

translate id w3PunishGame_2f1146d1_3:
    # kat "You should've thought of that before you entered the crossfire."
    kat ""

translate id w3PunishGame_9177b213_3:
    # sophia "You were taking too long. I wanted to put an end to the distraction."
    sophia ""

translate id w3PunishGame_0e5cc7d3_3:
    # "I didn't know how long I was out of it. Still, by the time I opened my eyes, the two women had collected themselves. The damage I thought I had done to Sophia with my counter-attack was made pitiful by their bemused faces."
    ""

translate id w3PunishGame_23aa7f4f_3:
    # kat "Oh, please. Are those the kind of sounds you make matter-of-factly?"
    kat ""

translate id w3PunishGame_a5d22b22_3:
    # "Sophia looked as composed as ever, and except for a little blush, her attention turned back to the Carnations."
    ""

translate id w3PunishGame_449fa9fb_3:
    # kat "I hope that provided you a measure of relief, Mr. [mcl]. That certainly was an... {i}explosive{/i} finish."
    kat ""

translate id w3PunishGame_b85bc4ed_3:
    # mc "A, ahem... thaaaaaanks?"
    mc ""

translate id w3PunishGame_fada21bd_3:
    # kat "Don't mention it. It was... interesting."
    kat ""

translate id w3PunishGame_dddb6f7e_3:
    # mc "........."
    mc ""

translate id w3PunishGame_3b21f354_3:
    # mc "......"
    mc ""

translate id w3PunishGame_5b4c9fd4_3:
    # "...this was my limit."
    ""

translate id w3PunishGame_964e84bc_3:
    # mc "Aaeeh, I'll be riiiiiight back."
    mc ""

translate id w3PunishGame_0fd4d656_16:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_16:
    # "..."
    ""

translate id w3PunishGame_e6cfc671_3:
    # kat "Is that a milder scent you are wearing?"
    kat ""

translate id w3PunishGame_837e9b2b_3:
    # sophia "It's the same batch as the bottle I gave you."
    sophia ""

translate id w3PunishGame_256d5f3e_3:
    # kat "The effect seemed... less? Why's that?"
    kat ""

translate id w3PunishGame_548f5e80_3:
    # sophia "It's possible that his body adapted and partially metabolized the odor molecules. All I can say is that it's remarkable he held out that long."
    sophia ""

translate id w3PunishGame_b4f48c1e_3:
    # sophia "It could be a matter of willpower."
    sophia ""

translate id w3PunishGame_caedd2e6_3:
    # kat "When I had dinner with Charles the other night, he didn't even flinch."
    kat ""

translate id w3PunishGame_b5500f59_3:
    # sophia "Dr. Kohler is a scary man."
    sophia ""

translate id w3PunishGame_4510a9da_3:
    # kat "Is that your appraisal after demolishing him game after game?"
    kat ""

translate id w3PunishGame_64ecae1b_3:
    # sophia "How else should I describe a man who has no need but keeps his passions tightly controlled."
    sophia ""

translate id w3PunishGame_610ec2fb_3:
    # kat "That's why he and I are incompatible. He treats fucking like it's Christmas."
    kat ""

translate id w3PunishGame_560752c9_3:
    # kat "You don't have anything to worry about as long as you don't lose."
    kat ""

translate id w3PunishGame_b19fae41_3:
    # sophia "I don't plan on it, but I have a feeling he eventually wins one way or another."
    sophia ""

translate id w3PunishGame_f588a7d6_3:
    # kat "Abel wouldn't let it come to that, would he?"
    kat ""

translate id w3PunishGame_0fd4d656_17:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_17:
    # "..."
    ""

translate id w3PunishGame_1324c38b_3:
    # sophia "Thank you for indulging my little whim, Kathleen. It was amusing."
    sophia ""

translate id w3PunishGame_4db92c34_3:
    # kat "Don't mention it. One should make their own fun in life. Still..."
    kat ""

translate id w3PunishGame_0cb22879_3:
    # kat "...you're very competent. Don't you ever desire more?"
    kat ""

translate id w3PunishGame_0cdff961_3:
    # sophia "A frivolous bitch like you has no business saying that to me."
    sophia ""

translate id w3PunishGame_9765acec_3:
    # kat "Ah, fuck. There it is."
    kat ""

translate id w3PunishGame_55e6ab51_3:
    # kat "I almost felt sorry for you."
    kat ""

translate id w3PunishGame_554d5fd7:
    # carnations "Mmhh, fweh...♥ Gheee..♥ Mmhhh...♥"
    carnations ""

translate id w3PunishGame_04086689:
    # "While I was preoccupied with expunging my nauseating urges, the Carnations were in the thick of it."
    ""

translate id w3PunishGame_8ea2b73b:
    # rose "Mmhh, ummffh, heeehhuu-♥"
    rose ""

translate id w3PunishGame_dd3273c4:
    # "Rosalind had replaced Felicia beneath the Amazon's wide hips, and just like before, Veronica wasn't shy about crudely grinding the MILF's face under her weight."
    ""

translate id w3PunishGame_df221777:
    # "With the redhead getting \"hers\", it was left to Felicia to find gratification for both herself and her physically-engaged, muff-diving cohort."
    ""

translate id w3PunishGame_cee415e0:
    # fel "*Cwhwup, fwhup* Mmhh, hhueeeh-♥"
    fel ""

translate id w3PunishGame_488c51d8:
    # "The result was a sapphic, Ouroboros-like blending of curves and limbs, making it hard to distinguish where one woman began and the other ended."
    ""

translate id w3PunishGame_602738be:
    # "*Chwup, fwhup, kwhup~*"
    ""

translate id w3PunishGame_a1a0597e:
    # "All pretense had melted away."
    ""

translate id w3PunishGame_aa1e49ed:
    # carnations "Fwweeh, hhnnggg, hhnn, ggwwwweh-♥"
    carnations ""

translate id w3PunishGame_07f52438:
    # "Veronica's domineering streak had all but receded, the only trace remaining was in the decisive way she pawed, kissed, and shook her hips."
    ""

translate id w3PunishGame_03563e0b:
    # fel "Mmhh, hhhnhnaaaatt-♥ Fwwweeeh-♥"
    fel ""

translate id w3PunishGame_fb9c0492:
    # "Felicia's bitching and moaning had been replaced with kiss-locked silence and focused, fiery concentration."
    ""

translate id w3PunishGame_ea0524ad:
    # carnations "Ghaa, gnnhhh, haaa *Chwup, shlick~* hhhaaa-♥"
    carnations ""

translate id w3PunishGame_989e5e37:
    # "Rosalind had practically disappeared off screen, perhaps even according to her own strategy, only to be used like a scratching post and as the engine source for a bevy of amorous noise."
    ""

translate id w3PunishGame_1f9bdb57:
    # rose "{size=10}Mmhh, fweeh, ggwwweeh{/size=/10}"
    rose ""

translate id w3PunishGame_1fbc680c:
    # "Her muffled moans could barely be heard beneath the redhead's ass, and what made it through those freckled cheeks was co-opted into a chorus of sexual need that sounded anemic on the tv's piss-poor speakers."
    ""

translate id w3PunishGame_ed6c2dc5:
    # "*Cwhup, fwwhhuup-*"
    ""

translate id w3PunishGame_9a17557e:
    # fel "N-hh, hhhnhn-♥"
    fel ""

translate id w3PunishGame_39847841:
    # "*Chwup, fhwwu, khwwup~!*"
    ""

translate id w3PunishGame_dbda7b8c:
    # fel "Enh, hhhhnn, wwwhhaaannn-"
    fel ""

translate id w3PunishGame_22fd7d95:
    # ver "A-ahh... you sing lovely for a caged bird..."
    ver ""

translate id w3PunishGame_f66eebc2:
    # fel "H-heh... aaah, y-you think you get it? You're so fucking full of yourself. At least my cage-"
    fel ""

translate id w3PunishGame_55f3d781:
    # rose "MMmhh, mhhhww-"
    rose ""

translate id w3PunishGame_0073c86c:
    # fel "...isn't collapsing on itself in debt."
    fel ""

translate id w3PunishGame_d2b6fee3:
    # rose "MMmhh, unnngg, wwwhaaa-"
    rose ""

translate id w3PunishGame_90daa9b9:
    # ver "This isn't y-your first time doing-"
    ver ""

translate id w3PunishGame_5b9c9a2a:
    # "*Shlick, fwhick, hwwwhick!-"
    ""

translate id w3PunishGame_3fff42d2:
    # fel "Of c-course not! I'm n-not seasoned like you, but sometimes... h-haaa... the ooc-occasion calls for it."
    fel ""

translate id w3PunishGame_40ae2959:
    # rose "Unngng, hheee, ehuugg-"
    rose ""

translate id w3PunishGame_b0e6e41a:
    # fel "Hubby has requests, y'know...?"
    fel ""

translate id w3PunishGame_86636747:
    # ver "H-haa... y-you're a good wife, aren't you? F-fuck... I should've tried being rich-"
    ver ""

translate id w3PunishGame_b7989705:
    # fel "MMhh, mmhhhh-!!!"
    fel ""

translate id w3PunishGame_5a5a6264:
    # "In the wake of their exchange, Veronica intensified her affection, sucking Felicia's face and savoring it like the last pizza slice."
    ""

translate id w3PunishGame_488f2fca:
    # carnations "Mmmh, eeueuuh, feewwwh... *Chwwup, shlick* fewwweee...♥♥♥"
    carnations ""

translate id w3PunishGame_d66b69a4:
    # "Not since her crowning moment during her interview, a night that only Samson and Isaac witnessed, had Veronica looked so comfortable in her element within these four walls."
    ""

translate id w3PunishGame_db968612:
    # "*Chwup, fhwwip~*"
    ""

translate id w3PunishGame_65e8b501:
    # "She hadn't quite \"punished\" the girls, but she gave the camera an unfettered glimpse of what she truly desired."
    ""

translate id w3PunishGame_342df6a5:
    # carnations "Mmmhh, heeeuuhh, gwwwuuuk-!!!"
    carnations ""

translate id w3PunishGame_cd935666:
    # "And it turns out, what she desired was beautiful women and an opportunity to let her id roam free."
    ""

translate id w3PunishGame_1c3f0d35:
    # carnations "Eeeuuhh, gwwwhhha, hwwwaanngg♥"
    carnations ""

translate id w3PunishGame_f7c14f27:
    # "Veronica and I weren't so different in that regard."
    ""

translate id w3PunishGame_54f7bbb2:
    # carnations "Hnnng, eeeuhuhh-♥♥"
    carnations ""

translate id w3PunishGame_87dcd091:
    # "On different terms, she might even fit in at a place like this..."
    ""

translate id w3PunishGame_970a8135:
    # ver "Mmhh, hheee...?!"
    ver ""

translate id w3PunishGame_821d9332:
    # ver "Mmhh, eehh-♥"
    ver ""

translate id w3PunishGame_63433b9c:
    # ver "Ahh, hhn... h-hang o-h, s-snaaaaap-ghh-♥♥♥"
    ver ""

translate id w3PunishGame_2b3d9af0:
    # rose "Hnngg, ggeehhh...?! Mmghhhh...!"
    rose ""

translate id w3PunishGame_67d78119:
    # "From my own vivid experience of being beneath that Amazon, I sympathized with Rosalind."
    ""

translate id w3PunishGame_542b4e89:
    # rose "Mmggwww-!!!"
    rose ""

translate id w3PunishGame_5360c1c7:
    # "As she came, Veronica instinctually pushed her ass toward the source of her pleasure, bearing down on Rosalind with a startling amount of her body weight."
    ""

translate id w3PunishGame_63e06bb5:
    # rose "Mmhwwwh, hhhnnggg, gwwwhhh-"
    rose ""

translate id w3PunishGame_2bb1fb78:
    # ver "Ah- y-yeeeahh...♥"
    ver ""

translate id w3PunishGame_278c7e29_10:
    # "........."
    ""

translate id w3PunishGame_0fd4d656_18:
    # "......"
    ""

translate id w3PunishGame_a20cefa7_18:
    # "..."
    ""

translate id w3PunishGame_d073fcfd:
    # ver "I know you girls aren't finished, right?"
    ver ""

translate id w3PunishGame_57553d7e:
    # fel "Hell no."
    fel ""

translate id w3PunishGame_fd718250:
    # rose "Mmhh, hhnhhh...:"
    rose ""

translate id w3PunishGame_3dba9e8a:
    # ver "Good. I sure as shit ain't either."
    ver ""

translate id w3PunishGame_bbf12c61:
    # "So they carried on, coming together with the common, selfish goal of getting themselves off."
    ""

translate id w3PunishGame_d65637ec:
    # "I'm not sure how long it lasted; I just know..."
    ""

translate id w3WashRoom_219bd14a:
    # mct "(Goddamn it...)"
    mct ""

translate id w3WashRoom_817346be:
    # "It took me a while to collect myself."
    ""

translate id w3WashRoom_f09b6ac7:
    # "Despite unleashing my need and blowing a fat wad down that hag's throat, I was still feeling... {i}out of sorts{/i}."
    ""

translate id w3WashRoom_a1617b93:
    # "Despite that hag's handy and blowing my load into the stratosphere, I was still feeling... {i}out of sorts{/i}."
    ""

translate id w3WashRoom_c912cf3a:
    # "Despite jacking off a couple of times, I was still feeling a bit... {i}frazzled{/i} and out of sorts."
    ""

translate id w3WashRoom_7c615aaf:
    # "It wasn't quite enough... Although I could think clearly and function, everything felt just a little topsy-turvy."
    ""

translate id w3WashRoom_640c94ce:
    # mc "Gah..."
    mc ""

translate id w3WashRoom_d8ac45b8:
    # "Strangely, I no longer felt the anger I had when I felt like I had no control."
    ""

translate id w3WashRoom_495d334f:
    # "I had overwhelmingly felt it when Mrs. Pulman jabbed me with that needle a week before. But now, it all felt {b}normal.{/b}"
    ""

translate id w3WashRoom_f1eb73a7:
    # "But now, it all felt... {b}normal.{/b}"
    ""

translate id w3WashRoom_cc4c148c:
    # "Okay, maybe that's just a tinsy bit of bullshit. I still felt annoyed, but this madness, in an indirect way, was my own making... and I was on the comfortable side of things, unlike the scene I just witnessed."
    ""

translate id w3WashRoom_278c7e29:
    # "........."
    ""

translate id w3WashRoom_0fd4d656:
    # "......"
    ""

translate id w3WashRoom_a20cefa7:
    # "..."
    ""

translate id w3WashRoom_98cda237:
    # kat "There you are. How are you feeling?"
    kat ""

translate id w3WashRoom_7387646f:
    # mc "Like I want to piss and jerk off all over your face."
    mc ""

translate id w3WashRoom_6c4a893a:
    # kat "Amazing, isn't it? For the record, I wasn't aware she would be wearing that when she asked to sit and observe."
    kat ""

translate id w3WashRoom_2201423b:
    # mc "I'm getting used to it. Did you have fun?"
    mc ""

translate id w3WashRoom_5408bb2f:
    # kat "Oh, I {i}always{/i} enjoy myself."
    kat ""

translate id w3WashRoom_a1a33bb9:
    # kat "Even when things don't go to plan or I... {i}lose{/i}, I never feel like I did."
    kat ""

translate id w3WashRoom_98dacc54:
    # kat "It's strange, but I don't think I ever felt an ounce of genuine anger in my life."
    kat ""

translate id w3WashRoom_f664d821:
    # mc "I don't believe that."
    mc ""

translate id w3WashRoom_c8bfa8be:
    # kat "I'm not saying I don't ever get annoyed at the \"obstacles\" that get in my way, or find a situation that calls for the theatrics of anger."
    kat ""

translate id w3WashRoom_2f41b6fe:
    # kat "But at the end of the day, what sense is there in getting angry over a game?"
    kat ""

translate id w3WashRoom_3394bdd8:
    # mc "Spoken like a woman who can stack the deck in her favor."
    mc ""

translate id w3WashRoom_e89e8020:
    # kat "Oh, I don't know... I might very well be turning the corner into a major loss soon. We'll see."
    kat ""

translate id w3WashRoom_3b21f354:
    # mc "......"
    mc ""

translate id w3WashRoom_2b35b6fe:
    # mc "..."
    mc ""

translate id w3WashRoom_f55c0abd:
    # mc "How are the girls?"
    mc ""

translate id w3WashRoom_6ac17ded:
    # kat "They're getting dressed."
    kat ""

translate id w3WashRoom_38c4104c:
    # mc "How {b}are{/b} they?"
    mc ""

translate id w3WashRoom_c321fa26:
    # kat "They probably feel pent up, but they seemed functional enough. It's understandably more soft feelings than hard ones right about now."
    kat ""

translate id w3WashRoom_756908b0:
    # mc "So, do you need anything else from me? Am I done here today?"
    mc ""

translate id w3WashRoom_65127ba4:
    # kat "Nope, you're done! Good job, Mr. [mcl]."
    kat ""

translate id w3WashRoom_aacc3ef8:
    # kat "Before you go, I wanted to ask: how did you find the club's hospitality last Saturday?"
    kat ""

translate id w3WashRoom_652650d3:
    # mc "...?"
    mc ""

translate id w3WashRoom_0ee90b39:
    # kat "Remember, I asked you to enjoy it and give me your thoughts."
    kat ""

translate id w3WashRoom_68b80435:
    # kat "It was mostly an excuse to try and get you to let loose, but a task is a task. What did you think?"
    kat ""

translate id w3WashRoom_5cf3c179:
    # mc "I mostly just walked around... mingled."
    mc ""

translate id w3WashRoom_1c601d1d:
    # kat "Oh, you did more than just mingle. You and Dalia joined Samson in the sauna."
    kat ""

translate id w3WashRoom_117bdce3:
    # mc "You know about that?"
    mc ""

translate id w3WashRoom_9d501909:
    # kat "Of course."
    kat ""

translate id w3WashRoom_2b35b6fe_1:
    # mc "..."
    mc ""

translate id w3WashRoom_f8892e14:
    # kat "I did tell you to have fun, but I didn't expect you to go mano a mano in the sauna with a couple of whores."
    kat ""

translate id w3WashRoom_7fa85d7d:
    # mc "I like his movies."
    mc ""

translate id w3WashRoom_4b9bcff7:
    # kat "Oh, I bet you do..."
    kat ""

translate id w3WashRoom_e4a21b58:
    # mc "...uh, Dalia was great?"
    mc ""

translate id w3WashRoom_cc37869b:
    # kat "She always is."
    kat ""

translate id w3WashRoom_8b7ecba7:
    # kat "Harper might give her a run for her money if only she was a more classical beauty, but they both have their uses."
    kat ""

translate id w3WashRoom_5e44a000:
    # mc "Right..."
    mc ""

translate id w3WashRoom_c5512cfd:
    # kat "I do hope you got what you wanted out of that Has Been."
    kat ""

translate id w3WashRoom_0fd4d656_1:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_1:
    # "..."
    ""

translate id w3WashRoom_0d68475d:
    # mc "...he's a giant prick."
    mc ""

translate id w3WashRoom_08c5c7eb:
    # kat "He has his uses."
    kat ""

translate id w3WashRoom_123a3235:
    # kat "Oh, [mcf]. You're like a diabetic in a candy store."
    kat ""

translate id w3WashRoom_ce37b3c6:
    # mc "I mean... everything seemed fine? I don't really know what the criteria is."
    mc ""

translate id w3WashRoom_e4fa7f64:
    # kat "You most certainly do."
    kat ""

translate id w3WashRoom_0fd4d656_2:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_2:
    # "..."
    ""

translate id w3WashRoom_4a8a8653:
    # kat "Oh, for-- just... let loose next weekend. That's an order."
    kat ""

translate id w3WashRoom_716019b8:
    # mc "Right."
    mc ""

translate id w3WashRoom_f697afec:
    # mc "Nothing too interesting. Jacob did ask me to keep an eye on Emma for him."
    mc ""

translate id w3WashRoom_a4815967:
    # kat "Ah yes, that... {i}romantic.{/i} For a man with such a bloody past, he sure is a softy."
    kat ""

translate id w3WashRoom_d846fea3:
    # "I chose to cautiously file away that bit of information but not draw attention to it."
    ""

translate id w3WashRoom_630b12a1:
    # kat "He sure is an interesting fellow. August did well in finding someone to offset Warren's personality."
    kat ""

translate id w3WashRoom_3d996037:
    # mc "What are you talking about?"
    mc ""

translate id w3WashRoom_f8e7eeb9:
    # kat "Eh, ask him sometime. He's the pimp."
    kat ""

translate id w3WashRoom_f406edd2:
    # kat "The way I understand it, they each have their function when it comes to the discipline of this place."
    kat ""

translate id w3WashRoom_baa84674:
    # mc "You should keep Warren on a tighter leash."
    mc ""

translate id w3WashRoom_9984cbbd:
    # kat "Aw, don't fret. He never does any permanent damage."
    kat ""

translate id w3WashRoom_daf1337b:
    # kat "Besides, the girls now have you to get them off the floor. Smart thinking sending her to Mr. Beaufort, you washed your hands of culpability."
    kat ""

translate id w3WashRoom_468fb5bd:
    # kat "Besides, the girls now have you to get them off the floor. Leaning on Charle's fondness of you to get it done was pretty bold."
    kat ""

translate id w3WashRoom_9d20a335:
    # kat "August never has it in him to question Charles."
    kat ""

translate id w3WashRoom_c67fa09a:
    # kat "Besides, the girls now have you to get them off the floor. Shielding Emma by getting her to film herself was an attractive solution."
    kat ""

translate id w3WashRoom_117bdce3_1:
    # mc "You know about that?"
    mc ""

translate id w3WashRoom_9d501909_1:
    # kat "Of course."
    kat ""

translate id w3WashRoom_0fd4d656_3:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_3:
    # "..."
    ""

translate id w3WashRoom_129efe07:
    # kat "Relax, unlike August, I don't give a shit about the minute-to-minute comings and goings of the house girls. If cutting them some slack where you can makes you feel better about yourself, go for it."
    kat ""

translate id w3WashRoom_d1665a11:
    # kat "I trust if you do, it'll be done with wisdom and discretion."
    kat ""

translate id w3WashRoom_e240f362:
    # mc "...s'alright."
    mc ""

translate id w3WashRoom_0a6a0ba0:
    # mc "Well, I..."
    mc ""

translate id w3WashRoom_d16be0e5:
    # mct "(Was there any point in mentioning this?)"
    mct ""

translate id w3WashRoom_1054b5a7:
    # "It's not like I needed to score any points with the old woman, but it was the most notable thing I could mention."
    ""

translate id w3WashRoom_e3291ef9:
    # mc "Nothing too interesting. I learned that Harper and Lucy are pretty fond of each other."
    mc ""

translate id w3WashRoom_5f5bb4eb:
    # kat "Oh...?"
    kat ""

translate id w3WashRoom_83629346:
    # mc "Yeah, Dalia asked me to peek in on them."
    mc ""

translate id w3WashRoom_b0fde027:
    # kat "Really? On company time?"
    kat ""

translate id w3WashRoom_0fd4d656_4:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_4:
    # "..."
    ""

translate id w3WashRoom_14a4d48f:
    # kat "Well, she's doing a better job than I expected."
    kat ""

translate id w3WashRoom_0e014f21:
    # mc "Wait... you're aware of them--"
    mc ""

translate id w3WashRoom_2674bb42:
    # kat "Of course. I was the one who told her to support our would-be Carnation and help her not go anywhere."
    kat ""

translate id w3WashRoom_9700e45e:
    # mc "Then Harper and Lucy are just because of you...?"
    mc ""

translate id w3WashRoom_1d28dd7e:
    # kat "Oh, it's nothing cynical. It's not like I ordered her to seduce that stupid whore; that must be how it developed between them. I'm sure they're actually quite fond of each other."
    kat ""

translate id w3WashRoom_01f66ba7:
    # kat "You shouldn't mention it to August, though. He's a stickler when it comes to the discipline of the girls."
    kat ""

translate id w3WashRoom_692357c5:
    # kat "He wouldn't see the value of this kind of relationship as I do."
    kat ""

translate id w3WashRoom_b8576a2d:
    # mc "I wasn't planning on it."
    mc ""

translate id w3WashRoom_06e01019:
    # kat "...yet, you mentioned it to me?"
    kat ""

translate id w3WashRoom_2b35b6fe_2:
    # mc "..."
    mc ""

translate id w3WashRoom_9d06c1e7:
    # kat "Good boy."
    kat ""

translate id w3WashRoom_5cf3c179_1:
    # mc "I mostly just walked around... mingled."
    mc ""

translate id w3WashRoom_334e92cb:
    # kat "Oh, you did more than just mingle. You and Dalia joined Samson in the Sauna."
    kat ""

translate id w3WashRoom_117bdce3_2:
    # mc "You know about that?"
    mc ""

translate id w3WashRoom_9d501909_2:
    # kat "Of course."
    kat ""

translate id w3WashRoom_2b35b6fe_3:
    # mc "..."
    mc ""

translate id w3WashRoom_e250176b:
    # kat "I did tell you to have fun, but I didn't expect you to go mano a mano in the sauna with a couple of whores"
    kat ""

translate id w3WashRoom_7fa85d7d_1:
    # mc "I like his movies."
    mc ""

translate id w3WashRoom_4b9bcff7_1:
    # kat "Oh, I bet you do..."
    kat ""

translate id w3WashRoom_e4a21b58_1:
    # mc "...uh, Dalia was great?"
    mc ""

translate id w3WashRoom_cc37869b_1:
    # kat "She always is."
    kat ""

translate id w3WashRoom_ca573aa1:
    # kat "Harper might give her a run for her money if only she were a more classical beauty, but they both have their uses."
    kat ""

translate id w3WashRoom_5e44a000_1:
    # mc "Right..."
    mc ""

translate id w3WashRoom_c5512cfd_1:
    # kat "I do hope you got what you wanted out of that Has Been."
    kat ""

translate id w3WashRoom_0fd4d656_5:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_5:
    # "..."
    ""

translate id w3WashRoom_0d68475d_1:
    # mc "...he's a giant prick."
    mc ""

translate id w3WashRoom_08c5c7eb_1:
    # kat "He has his uses."
    kat ""

translate id w3WashRoom_123a3235_1:
    # kat "Oh, [mcf]. You're like a diabetic in a candy store."
    kat ""

translate id w3WashRoom_ce37b3c6_1:
    # mc "I mean... everything seemed fine? I don't really know what the criteria is."
    mc ""

translate id w3WashRoom_e4fa7f64_1:
    # kat "You most certainly do."
    kat ""

translate id w3WashRoom_0fd4d656_6:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_6:
    # "..."
    ""

translate id w3WashRoom_4a8a8653_1:
    # kat "Oh, for-- just... let loose next weekend. That's an order."
    kat ""

translate id w3WashRoom_716019b8_1:
    # mc "Right."
    mc ""

translate id w3WashRoom_b8a7705e:
    # "*Beep*"
    ""

translate id w3WashRoom_e95868ce:
    # mct "(It's Mina...)"
    mct ""

translate id w3WashRoom_a52a2e59:
    # kat "Well, I'll leave you to it."
    kat ""

translate id w3WashRoom_6f22f736:
    # mct "(What does she...?)"
    mct ""

translate id w3WashRoom_27310a45:
    # "She said she wanted to play some games and a very conspicuous \"whatever\"...?"
    ""

translate id w3WashRoom_fb73a94d:
    # mct "(Well, this was inevitable. With what I promised Hana last night, I should probably tell her I can't help with her list anymore... {b}right{/b}...?)"
    mct ""

translate id w3WashRoom_d036c18d:
    # "The least I can do is tell her in person."
    ""

translate id w3WashRoom_25c9e239:
    # "Well, I am still feeling a little frisky... and quite frankly, Mina's company sounded like sweet relief from the tension of this place."
    ""

translate id w3WashRoom_daa43a51:
    # "Shockingly, she said she was bored and wanted to hang out."
    ""

translate id w3WashRoom_3ce245ac:
    # mct "(I can't bring myself to say no after turning her down like I did the other night...)"
    mct ""

translate id w3WashRoom_0fd4d656_7:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_7:
    # "..."
    ""

translate id w3WashRoom_dabdf789:
    # mc "If you don't mind, I'm going to leave."
    mc ""

translate id w3WashRoom_2ed7c194:
    # kat "Have an enjoyable rest of your evening, Mr. [mcl]."
    kat ""

translate id w3WashRoom_0fd4d656_8:
    # "......"
    ""

translate id w3WashRoom_a20cefa7_8:
    # "..."
    ""

translate id w3WashRoom_abfb4761:
    # "{i}Thanks{/i}."
    ""

translate id w3MinaHanaGFHangStart_54b4ce7f:
    # "After killing time at Ian's"
    ""

translate id w3MinaHanaGFHangStart_05f4d92a:
    # "I thought it best if Mina and I met on familiar, intermediary grounds. If I was going to be frank with her about Hana, there was no sense in having one of us go all the way to the other's place only to awkwardly remain after ending our bizarre deal."
    ""

translate id w3MinaHanaGFHangStart_5da626e9:
    # "Of course..."
    ""

translate id w3MinaHanaGFHangStart_83c9e7f9:
    # mina "[mcf]! Hey!"
    mina ""

translate id w3MinaHanaGFHangStart_ec79957f:
    # mc "Been waiting long?"
    mc ""

translate id w3MinaHanaGFHangStart_a62727bf:
    # mina "Nope, just--"
    mina ""

translate id w3MinaHanaGFHangStart_3381849e:
    # mina "Mmmmh, just got here."
    mina ""

translate id w3MinaHanaGFHangStart_05cdceb0:
    # "Of course, the moment I saw her glowing face and felt the force of her thermonuclear hug, I remembered why I had entered the deal with her in the first place."
    ""

translate id w3MinaHanaGFHangStart_6e67121e:
    # mc "Come to think of it, this is where we first met, wasn't it?"
    mc ""

translate id w3MinaHanaGFHangStart_a38472bd:
    # mina "Sure was!"
    mina ""

translate id w3MinaHanaGFHangStart_63751b3a:
    # "I had come here, intending to do the right thing by Hana..."
    ""

translate id w3MinaHanaGFHangStart_57969778:
    # mct "(Ah, fuck it. There's no harm in being friendly.)"
    mct ""

translate id w3MinaHanaGFHangStart_8488b36b:
    # "For the sake of recollection, even if it was just a few weeks ago."
    ""

translate id w3MinaHanaGFHangStart_8f59568d:
    # mc "How are you?"
    mc ""

translate id w3MinaHanaGFHangStart_7f56a420:
    # mina "I was bored, but now I'm better by the second!"
    mina ""

translate id w3MinaHanaGFHangStart_48e864a8:
    # mc "I expected you might say that, but let's order while the line is so short."
    mc ""

translate id w3MinaHanaGFHangStart_54cbe8ec:
    # "I couldn't deny that-"
    ""

translate id w3MinaHanaGFHangStart_e024b605:
    # "*Cwhup!*"
    ""

translate id w3MinaHanaGFHangStart_844ff169:
    # "...that even if I had only known her for a few weeks, moments like this conjured up a swell of affection."
    ""

translate id w3MinaHanaGFHangStart_77aed5a1:
    # mina "That tickles!"
    mina ""

translate id w3MinaHanaGFHangStart_52cdcdc9:
    # mc "Sorry, but let's order while the line is so short."
    mc ""

translate id w3MinaHanaGFHangStart_21a6a419:
    # mc "We should order while the line is so short."
    mc ""

translate id w3MinaHanaGFHangStart_29d43ae8:
    # mina "Good idea!"
    mina ""

translate id w3MinaHanaGFHangStart_c415773b:
    # mc "This is even the same table as the day we met."
    mc ""

translate id w3MinaHanaGFHangStart_7b0c1aee:
    # mina "Ha ha, is it?"
    mina ""

translate id w3MinaHanaGFHangStart_59587b46:
    # mc "Yep. I can recall it vividly... because, for a measly second, you actually looked annoyed to meet me."
    mc ""

translate id w3MinaHanaGFHangStart_0e94249e:
    # mina "No, I didn't!"
    mina ""

translate id w3MinaHanaGFHangStart_f967fe8b:
    # mc "You did! I remember it because what came after that single second was one hell of a greeting."
    mc ""

translate id w3MinaHanaGFHangStart_5195c7e4:
    # mina "Oh no, you had me pegged as a fake bitch from the beginning, huh?"
    mina ""

translate id w3MinaHanaGFHangStart_04220a14:
    # mc "I saw someone {i}choosing{/i} to make someone feel welcome."
    mc ""

translate id w3MinaHanaGFHangStart_c1ef9833:
    # mina "Hmpfh... feels like a long time ago."
    mina ""

translate id w3MinaHanaGFHangStart_6531b9ea:
    # mc "Yeah... you had a boyfriend back then, if I remember correctly."
    mc ""

translate id w3MinaHanaGFHangStart_6debc42a:
    # mina "...did I?"
    mina ""

translate id w3MinaHanaGFHangStart_cbfb53b1:
    # mina "......"
    mina ""

translate id w3MinaHanaGFHangStart_3439ea50:
    # mina "...feels more like an anchor wrapped around my neck in hindsight."
    mina ""

translate id w3MinaHanaGFHangStart_d54fd336:
    # mc "Are you over the hump?"
    mc ""

translate id w3MinaHanaGFHangStart_04472d9e:
    # mina "I guess Ian told you?"
    mina ""

translate id w3MinaHanaGFHangStart_e7ae8f08:
    # mc "Yep, we hung out Sunday morning, right after you broke the news. Are you over the hump?"
    mc ""

translate id w3MinaHanaGFHangStart_b7dac536:
    # mina "Who knows? Too early to say I'm not gonna get weepy out of nowhere, but I'm feeling pretty good right now. It occurs to me..."
    mina ""

translate id w3MinaHanaGFHangStart_d34ad9ae:
    # mina "This is my first time being single and out with a man in nearly a year."
    mina ""

translate id w3MinaHanaGFHangStart_e020170d:
    # mc "You know, the world's your oyster. You're free to do it with anyone you want now."
    mc ""

translate id w3MinaHanaGFHangStart_b2cae4d7:
    # mina "Don't joke about that! You sound like Felicia~ trying to get me to play the field."
    mina ""

translate id w3MinaHanaGFHangStart_bab89a3e:
    # mc "Well... I mean... {i}shouldn't{/i} you? Your goal is to get some experience, right? Want me to play wingman for you so--"
    mc ""

translate id w3MinaHanaGFHangStart_3137553c:
    # mina "Are you TRYING to hurt my feelings? I know we're not dating, but WHO am I getting coffee with right now?"
    mina ""

translate id w3MinaHanaGFHangStart_0fd4d656:
    # "......"
    ""

translate id w3MinaHanaGFHangStart_a20cefa7:
    # "..."
    ""

translate id w3MinaHanaGFHangStart_c1558db7:
    # "I came here to tell her I couldn't help her with her list, but looking at the anger on Mina's face at my mere suggestion..."
    ""

translate id w3MinaHanaGFHangStart_2e7ea900:
    # mct "(This must be what it feels like to {i}swoon{/i}.)"
    mct ""

translate id w3MinaHanaGFHangStart_0fd4d656_1:
    # "......"
    ""

translate id w3MinaHanaGFHangStart_a20cefa7_1:
    # "..."
    ""

translate id w3MinaHanaGFHangStart_3cbd3900:
    # mina "The only person... ah."
    mina ""

translate id w3MinaHanaGFHangStart_5c042426:
    # "I came here to tell her I had changed my mind, but with the way she was blushing..."
    ""

translate id w3MinaHanaGFHangStart_2e268802:
    # mina "Y-you are..."
    mina ""

translate id w3MinaHanaGFHangStart_1bbcee3c:
    # mina "{size=-10}TheonlypersonIwanttobewithrightnowisyou.{/size=-10}"
    mina ""

translate id w3MinaHanaGFHangStart_0fd4d656_2:
    # "......"
    ""

translate id w3MinaHanaGFHangStart_a20cefa7_2:
    # "..."
    ""

translate id w3MinaHanaGFHangStart_84b95b26:
    # mc "I couldn't hear you."
    mc ""

translate id w3MinaHanaGFHangStart_3b949b8f:
    # mina "I said..."
    mina ""

translate id w3MinaHanaGFHangStart_75cac52d:
    # mina "Right now, the only person I want to do stuff like that with is you, [mcf]."
    mina ""

translate id w3MinaHanaGFHangStart_b370c8a4:
    # "Shit like {b}that{/b}, said with that face... made me realize just how susceptible I was to a woman's wiles despite other intentions."
    ""

translate id w3MinaHanaGFHangStart_ac6f4985:
    # mc "Goddamnit, you're an oustanding actress."
    mc ""

translate id w3MinaHanaGFHangStart_e90ca254:
    # mina "What?! I'm embarrassed as hell right now!"
    mina ""

translate id w3MinaHanaGFHangStart_3b2b772c:
    # mc "Enough that you had to bury your face in your arms?"
    mc ""

translate id w3MinaHanaGFHangStart_510bc7f3:
    # mina "Body language is crucial..."
    mina ""

translate id w3MinaHanaGFHangStart_679df572:
    # "...or perhaps there was something about Mina that I couldn't resist."
    ""

translate id w3MinaHanaGFHangStart_2a06a460:
    # "I knew this was a violation of what I promised Hana last night."
    ""

translate id w3MinaHanaGFHangStart_42d2734a:
    # mc "Sorry. I'm just teasing you."
    mc ""

translate id w3MinaHanaGFHangStart_5ac84a74:
    # "A promise I didn't have to make but did."
    ""

translate id w3MinaHanaGFHangStart_1a19eee0:
    # mc "You know I promised to help you."
    mc ""

translate id w3MinaHanaGFHangStart_cdfe990c:
    # "A promise that I made to Hana despite another promise given to Mina."
    ""

translate id w3MinaHanaGFHangStart_d4ec53a7:
    # mc "Is... that why you called today?"
    mc ""

translate id w3MinaHanaGFHangStart_b5fb9e13:
    # "My mind could justify this decision in any number of ways."
    ""

translate id w3MinaHanaGFHangStart_a81a8346:
    # mina "Mmmhh...?"
    mina ""

translate id w3MinaHanaGFHangStart_d992a73c:
    # "Hana will probably never find out, but even if she did..."
    ""

translate id w3MinaHanaGFHangStart_4ce80219:
    # mina "...a girl's got a hope?"
    mina ""

translate id w3MinaHanaGFHangStart_6d1243f4:
    # "What hope did we actually have to last?"
    ""

translate id w3MinaHanaGFHangStart_a83f930e:
    # mc "Well, then... finish your coffee."
    mc ""

translate id w3MinaHanaGFHangStart_1efa6c9c:
    # "Besides, Mina would come to her senses and realize that our relationship is predicated on her girlish heartbreak and that this whole list thing was asinine and I had nothing to actually offer her as a person..."
    ""

translate id w3MinaHanaGFHangStart_8d9d90cc:
    # mc "Finish your coffee and we'll start seeing about that list of yours."
    mc ""

translate id w3MinaHanaGFHangStart_713e2e12:
    # "--those kinds of justifications. I refused to make them. If you're going to be shitty, I believe you should embrace it."
    ""

translate id w3MinaHanaGFHangStart_e2996c1d:
    # mina "O-oh..."
    mina ""

translate id w3MinaHanaGFHangStart_050a235d:
    # "Acknowledging the consequences of your actions is the most critical step in taking precautions against them."
    ""

translate id w3MinaHanaGFHangStart_02691ef4:
    # "And at the end of the day, even if it casts light on me being a fool, I wanted to enjoy Mina's company more. That was all there was to it."
    ""

translate id w3MinaHanaGFHangStart_3c16cbaf:
    # "I'm a selfish asshole."
    ""

translate id w3MinaHanaGFHangStart_5035c58a:
    # mc "Things have changed."
    mc ""

translate id w3MinaHanaGFHangStart_ee8ca862:
    # "Like diving into a cold pool, I shut off my brain and spit the words out."
    ""

translate id w3MinaHanaGFHangStart_84e0a6a7:
    # mina "Things have... {i}changed{/i}?"
    mina ""

translate id w3MinaHanaGFHangStart_57bcda9a:
    # mc "Yeah... uh... as it happened, I got myself into a committed relationship last night."
    mc ""

translate id w3MinaHanaGFHangStart_0fd4d656_3:
    # "......"
    ""

translate id w3MinaHanaGFHangStart_a20cefa7_3:
    # "..."
    ""

translate id w3MinaHanaGFHangStart_c400e2e6:
    # mina "You know I came here, hoping... {b}w-wow.{/b}"
    mina ""

translate id w3MinaHanaGFHangStart_bb6026ec:
    # mina "Wow, that's a kick in the fucking stomach."
    mina ""

translate id w3MinaHanaGFHangStart_e6b7a288:
    # mina "Hana, right...? Ha... you told me it was a casual thing."
    mina ""

translate id w3MinaHanaGFHangStart_16301483:
    # mc "I {i}really{/i} thought that, but... ah... you have every right to be angry after I promised that I-"
    mc ""

translate id w3MinaHanaGFHangStart_5c9305a5:
    # mina "I guess you two must've had a thing and then I went and forced myself on you the other day."
    mina ""

translate id w3MinaHanaGFHangStart_fa2a8545:
    # mc "It was unexpected and just happened."
    mc ""

translate id w3MinaHanaGFHangStart_62f6ab89:
    # mina "What kind of bullcrap- ah..."
    mina ""

translate id w3MinaHanaGFHangStart_effd07ae:
    # mc "You have every right to be angry after I promised that I-"
    mc ""

translate id w3MinaHanaGFHangStart_5afd181c:
    # mina "I'm not angre- ah, I mean, I kinda am? Feelin' kinda a lot of things..."
    mina ""

translate id w3MinaHanaGFHangStart_c6312baa:
    # mina "In my head, I knew this wasn't a-- I still kinda had some expectations-- and definitely didn't think our fun would end this soon."
    mina ""

translate id w3MinaHanaGFHangStart_f234e51f:
    # mc "I shouldn't have been so haphazard with my promises."
    mc ""

translate id w3MinaHanaGFHangStart_b61e4e29:
    # mina "When you dropped by the other night, I thought..."
    mina ""

translate id w3MinaHanaGFHangStart_918fea83:
    # mc "I'm sorry."
    mc ""

translate id w3MinaHanaGFHangStart_07b3093a:
    # mina "Well, it's not like you strung me along... I came onto you."
    mina ""

translate id w3MinaHanaGFHangStart_0ef8876b:
    # mina "Well, I'm the one who came onto you so hard, so it's not like you strung me along."
    mina ""

translate id w3MinaHanaGFHangStart_7ded9f76:
    # mc "I feel like a fucking asshole considering the timing with you and Ian."
    mc ""

translate id w3MinaHanaGFHangStart_1d4982f0:
    # mina "...I want to agree with you, but those are hurt feelings. I'm actually glad you're a decent guy."
    mina ""

translate id w3MinaHanaGFHangStart_6f228097:
    # mina "You could've dated her and fucked me on the side, but you didn't. You were... upfront. {i}Sort of{/i}."
    mina ""

translate id w3MinaHanaGFHangStart_7768eac8:
    # mina "You could've dated whoever and--"
    mina ""

translate id w3MinaHanaGFHangStart_87f9cc59:
    # mc "It's Hana."
    mc ""

translate id w3MinaHanaGFHangStart_01501426:
    # mina "Hana..? Ha, that makes sense."
    mina ""

translate id w3MinaHanaGFHangStart_6f228097_1:
    # mina "You could've dated her and fucked me on the side, but you didn't. You were... upfront. {i}Sort of{/i}."
    mina ""

translate id w3MinaHanaGFHangStart_37cba058:
    # mina "Gives me some hope about what's out there..."
    mina ""

translate id w3MinaHanaGFHangStart_b41dcb03:
    # mina "Still feel like rejected trash..."
    mina ""

translate id w3MinaHanaGFHangStart_3b21f354:
    # mc "......"
    mc ""

translate id w3MinaHanaGFHangStart_4e2a8876:
    # mina "..."
    mina ""

translate id w3MinaHanaGFHangStart_7bef0821:
    # "We awkwardly finished our coffee, and then Mina wisely thought it best for her to return home."
    ""

translate id w3MinaHanaGFHangStart_a2b9ce59:
    # "She reassured me that she was fine and that she was grateful for my honesty but it all sounded half-hearted."
    ""

translate id w3MinaHanaGFHangStart_27b379a2:
    # "She even said she hoped to remain friends and I said so too, but I doubted. Still, to her immense credit..."
    ""

translate id w3MinaHanaGFHangStart_efdfe7ec:
    # mina "[mcf]...?"
    mina ""

translate id w3MinaHanaGFHangStart_f49fff8f:
    # mc "Yeah?"
    mc ""

translate id w3MinaHanaGFHangStart_30f294ec:
    # mina "Come here!"
    mina ""

translate id w3MinaHanaGFHangStart_aeb2bb08:
    # mc "Mina..."
    mc ""

translate id w3MinaHanaGFHangStart_22b64e9a:
    # "To her credit, her hug goodbye had every ounce of goodwill and warmth as the day we met."
    ""

translate id w3MinaHanaGFHangStart_84d24eea:
    # "I knew, from her hug alone..."
    ""

translate id w3MinaHanaGFHangStart_9b4713f3:
    # mina "Hana's really cool."
    mina ""

translate id w3MinaHanaGFHangStart_3826f700:
    # "I knew from her hug that she really did mean those words."
    ""

translate id w3MinaHanaGFHangStart_8ccc2761:
    # mina "I hope you two build something that makes my dumb feelings right now worth it."
    mina ""

translate id w3MinaHanaGFHangStart_61885424:
    # "I knew from her hug that her fear of only imitating how to respond to situations was a load of crap."
    ""

translate id w3MinaHanaGFHangStart_9da1f935:
    # mc "Well, it's just starting out... so..."
    mc ""

translate id w3MinaHanaGFHangStart_aeabd243:
    # "I knew from her hug that Mina was graced with a beautiful, loving presence."
    ""

translate id w3MinaHanaGFHangStart_3cd65526:
    # mina "Thanks again for being there for me these past few days."
    mina ""

translate id w3MinaHanaGFHangStart_fe6c0532:
    # "I knew from her hug that Mina was the kind of warm person I wondered if I could ever be."
    ""

translate id w3MinaFullSpeedHangStart_0f84b96a:
    # "After leaving Ian's..."
    ""

translate id w3MinaFullSpeedHangStart_34885e9d:
    # "Naturally, given the nature of our relationship, I expected things might take an eventual horizontal turn, but Mina's public company was a treat unto itself."
    ""

translate id w3MinaFullSpeedHangStart_34abfb98:
    # mct "(Man, she looks--)"
    mct ""

translate id w3MinaFullSpeedHangStart_83c9e7f9:
    # mina "[mcf]! Hey!"
    mina ""

translate id w3MinaFullSpeedHangStart_ec79957f:
    # mc "Been waiting long?"
    mc ""

translate id w3MinaFullSpeedHangStart_a62727bf:
    # mina "Nope, just--"
    mina ""

translate id w3MinaFullSpeedHangStart_3381849e:
    # mina "Mmmmh, just got here."
    mina ""

translate id w3MinaFullSpeedHangStart_94e46755:
    # "As she forcefully pressed her body into mine with her characteristic bubbly enthusiasm, I could feel my day getting sweeter by the second."
    ""

translate id w3MinaFullSpeedHangStart_3dd270b9:
    # mc "Stop, you're going to give me diabetes."
    mc ""

translate id w3MinaFullSpeedHangStart_505fbfc6:
    # mina "Ehehe~"
    mina ""

translate id w3MinaFullSpeedHangStart_6e67121e:
    # mc "Come to think of it, this is where we first met, wasn't it?"
    mc ""

translate id w3MinaFullSpeedHangStart_a38472bd:
    # mina "Sure was!"
    mina ""

translate id w3MinaFullSpeedHangStart_5f290d14:
    # "This brought me back to the time Ian introduced the two of us. Just mere weeks, but it..."
    ""

translate id w3MinaFullSpeedHangStart_c16b7611:
    # mct "(Feels like a long time ago.)"
    mct ""

translate id w3MinaFullSpeedHangStart_57969778:
    # mct "(Ah, fuck it. There's no harm in being friendly.)"
    mct ""

translate id w3MinaFullSpeedHangStart_8f59568d:
    # mc "How are you?"
    mc ""

translate id w3MinaFullSpeedHangStart_b3798b41:
    # mina "Mmmmh, I was bored, but now things are looking up!"
    mina ""

translate id w3MinaFullSpeedHangStart_48e864a8:
    # mc "I expected you might say that, but let's order while the line is so short."
    mc ""

translate id w3MinaFullSpeedHangStart_c665f120:
    # mc "Hey, look up for a second."
    mc ""

translate id w3MinaFullSpeedHangStart_64d01d9f:
    # mina "...huh? Wha-"
    mina ""

translate id w3MinaFullSpeedHangStart_103b6b6c:
    # "*Chwup~"
    ""

translate id w3MinaFullSpeedHangStart_0c5f48a6:
    # "That was the one remarkable difference between then and now."
    ""

translate id w3MinaFullSpeedHangStart_4aa20bb1:
    # mina "Mmmhh..."
    mina ""

translate id w3MinaFullSpeedHangStart_b5907ae8:
    # "There was no Ian here, only Mina and myself, and she freely gave her lips to me."
    ""

translate id w3MinaFullSpeedHangStart_919b79b0:
    # mc "Let's order while the line is so short."
    mc ""

translate id w3MinaFullSpeedHangStart_8b80a826:
    # mina "Mmmm-kay!"
    mina ""

translate id w3MinaFullSpeedHangStart_c415773b:
    # mc "This is even the same table as the day we met."
    mc ""

translate id w3MinaFullSpeedHangStart_7b0c1aee:
    # mina "Ha ha, is it?"
    mina ""

translate id w3MinaFullSpeedHangStart_59587b46:
    # mc "Yep. I can recall it vividly... because, for a measly second, you actually looked annoyed to meet me."
    mc ""

translate id w3MinaFullSpeedHangStart_0e94249e:
    # mina "No, I didn't!"
    mina ""

translate id w3MinaFullSpeedHangStart_f967fe8b:
    # mc "You did! I remember it because what came after that single second was one hell of a greeting."
    mc ""

translate id w3MinaFullSpeedHangStart_5195c7e4:
    # mina "Oh no, you had me pegged as a fake bitch from the beginning, huh?"
    mina ""

translate id w3MinaFullSpeedHangStart_04220a14:
    # mc "I saw someone {i}choosing{/i} to make someone feel welcome."
    mc ""

translate id w3MinaFullSpeedHangStart_c1ef9833:
    # mina "Hmpfh... feels like a long time ago."
    mina ""

translate id w3MinaFullSpeedHangStart_18e9e902:
    # mc "Funny, I thought that myself."
    mc ""

translate id w3MinaFullSpeedHangStart_e9b98f62:
    # mina "Is that funny?"
    mina ""

translate id w3MinaFullSpeedHangStart_ee01940f:
    # mc "If I remember correctly... you had a boyfriend back then."
    mc ""

translate id w3MinaFullSpeedHangStart_6debc42a:
    # mina "...did I?"
    mina ""

translate id w3MinaFullSpeedHangStart_cbfb53b1:
    # mina "......"
    mina ""

translate id w3MinaFullSpeedHangStart_3439ea50:
    # mina "...feels more like an anchor wrapped around my neck in hindsight."
    mina ""

translate id w3MinaFullSpeedHangStart_d54fd336:
    # mc "Are you over the hump?"
    mc ""

translate id w3MinaFullSpeedHangStart_04472d9e:
    # mina "I guess Ian told you?"
    mina ""

translate id w3MinaFullSpeedHangStart_e7ae8f08:
    # mc "Yep, we hung out Sunday morning, right after you broke the news. Are you over the hump?"
    mc ""

translate id w3MinaFullSpeedHangStart_b7dac536:
    # mina "Who knows? Too early to say I'm not gonna get weepy out of nowhere, but I'm feeling pretty good right now. It occurs to me..."
    mina ""

translate id w3MinaFullSpeedHangStart_0c312c98:
    # mina "This is my first time being single and out with a man in nearly a year. I can flirt as much as I want!"
    mina ""

translate id w3MinaFullSpeedHangStart_8b282625:
    # mc "If I also recall correctly, that didn't stop you before."
    mc ""

translate id w3MinaFullSpeedHangStart_e00ba1f2:
    # mina "....."
    mina ""

translate id w3MinaFullSpeedHangStart_4e2a8876:
    # mina "..."
    mina ""

translate id w3MinaFullSpeedHangStart_624dca22:
    # mina "Can you blame me? ...and if *I* remember correctly, you flirted RIGHT back."
    mina ""

translate id w3MinaFullSpeedHangStart_6be9a281:
    # mc "...you think *I* know how to flirt? That's all in your head."
    mc ""

translate id w3MinaFullSpeedHangStart_4aa3a41e:
    # mina " I do know some things about you for sure."
    mina ""

translate id w3MinaFullSpeedHangStart_652650d3:
    # mc "...?"
    mc ""

translate id w3MinaFullSpeedHangStart_3ba628d2:
    # mina "You know how to grab me... hold me..."
    mina ""

translate id w3MinaFullSpeedHangStart_c335e8c5:
    # mina "Kiss me up here... and..."
    mina ""

translate id w3MinaFullSpeedHangStart_b2d463f4:
    # mina "...kiss me down-"
    mina ""

translate id w3MinaFullSpeedHangStart_00054fef:
    # mc "You didn't call to play games today, did you?"
    mc ""

translate id w3MinaFullSpeedHangStart_c383af75:
    # mina "Ehehe, maybe not!"
    mina ""

translate id w3MinaFullSpeedHangStart_53241e6e:
    # "Her smile was sumptuous and giggle infectious. Just looking at her filled me with a sense of importance."
    ""

translate id w3MinaFullSpeedHangStart_334852dc:
    # mina "You know, when you dropped by Saturday night... you left me hanging..."
    mina ""

translate id w3MinaFullSpeedHangStart_f7e0441a:
    # mina "After what you did to me last Friday, I've missed you..."
    mina ""

translate id w3MinaFullSpeedHangStart_ae3c5bc8:
    # "This woman was a kingmaker. Her time was perhaps wasted on someone like me, but I would happily and greedily gobble it up if she allowed me."
    ""

translate id w3MinaFullSpeedHangStart_84cd168f:
    # mc "I figured you would've come to your senses by now about this arrangement."
    mc ""

translate id w3MinaFullSpeedHangStart_a43f9bb6:
    # mina "...it's not JUST about the list, stupid."
    mina ""

translate id w3MinaFullSpeedHangStart_c1453b44:
    # mc "What else is it about?"
    mc ""

translate id w3MinaFullSpeedHangStart_cbfb53b1_1:
    # mina "......"
    mina ""

translate id w3MinaFullSpeedHangStart_4e2a8876_1:
    # mina "..."
    mina ""

translate id w3MinaFullSpeedHangStart_73431e62:
    # mina "Who you take the journey with is just as important as the destination."
    mina ""

translate id w3MinaFullSpeedHangStart_9c6727f0:
    # "A kingmaker indeed. I was brimming with confidence I hadn't earned."
    ""

translate id w3MinaFullSpeedHangStart_e1ebc614:
    # mc "Just the right time, right place..."
    mc ""

translate id w3MinaFullSpeedHangStart_f143e9ce:
    # "I was the most convenient person for her to latch onto, was my thought that was meant to temper my rapidly inflating self-worth."
    ""

translate id w3MinaFullSpeedHangStart_948add10:
    # mina "Right time, right place... some kindness, some acceptance..."
    mina ""

translate id w3MinaFullSpeedHangStart_581c0d07:
    # mina "A REALLY killer tongue."
    mina ""

translate id w3MinaFullSpeedHangStart_845755c7:
    # "Okay... that I felt I earned."
    ""

translate id w3MinaFullSpeedHangStart_f38a3bf7:
    # mina "So... will you... show me a \"nice\" time today, [mcf]?"
    mina ""

translate id w3MinaFullSpeedHangStart_e0847027:
    # mc "After the day I've had... goddamnit, I'm happy to hear that."
    mc ""

translate id w3MinaFullSpeedHangStart_91adbccd:
    # mina "It's only early in the afternoon! What kind of day could you possibly have had?"
    mina ""

translate id w3MinaFullSpeedHangStart_1c930c47:
    # mc "A *frustrating* one."
    mc ""

translate id w3MinaFullSpeedHangStart_ce22f3d7:
    # mina "Oh..."
    mina ""

translate id w3MinaFullSpeedHangStart_bb012bf0:
    # mc "{b}Finish{/b} your coffee."
    mc ""

translate id w3MinaFullSpeedHangStart_78d3068a:
    # mina "O-ohh-"
    mina ""

translate id w3MinaFullSpeedHangStart_48085111:
    # mc "You have a long list."
    mc ""

translate id w3MinaPlatonicHangStart_888277da:
    # "After spending time at Ian's..."
    ""

translate id w3MinaPlatonicHangStart_7282b25e:
    # "I was glad to see that despite my rejection, Mina thought of me as a friend."
    ""

translate id w3MinaPlatonicHangStart_22e64eb8:
    # mct "(There she was!)"
    mct ""

translate id w3MinaPlatonicHangStart_83c9e7f9:
    # mina "[mcf]! Hey!"
    mina ""

translate id w3MinaPlatonicHangStart_ec79957f:
    # mc "Been waiting long?"
    mc ""

translate id w3MinaPlatonicHangStart_a62727bf:
    # mina "Nope, just--"
    mina ""

translate id w3MinaPlatonicHangStart_3381849e:
    # mina "Mmmmh, just got here."
    mina ""

translate id w3MinaPlatonicHangStart_2aabf617:
    # "Like always, she hugged me as if it was the most natural thing in the world, and she really made me believe it."
    ""

translate id w3MinaPlatonicHangStart_6e67121e:
    # mc "Come to think of it, this is where we first met, wasn't it?"
    mc ""

translate id w3MinaPlatonicHangStart_a38472bd:
    # mina "Sure was!"
    mina ""

translate id w3MinaPlatonicHangStart_0416f950:
    # "If she held any bitterness from me turning her down last Friday, then she didn't let it show through."
    ""

translate id w3MinaPlatonicHangStart_e67ad6bb:
    # mc "Killing me with kindness?"
    mc ""

translate id w3MinaPlatonicHangStart_5e1fd047:
    # mina "...eh? What?"
    mina ""

translate id w3MinaPlatonicHangStart_e3ddbca5:
    # mc "Nothing. I'm glad to see you too. How are you?"
    mc ""

translate id w3MinaPlatonicHangStart_f2b43d88:
    # mina "{b}Gooooooood!{/b}"
    mina ""

translate id w3MinaPlatonicHangStart_89db4aa8:
    # mc "Line's looking pretty short. Let's get some drinks."
    mc ""

translate id w3MinaPlatonicHangStart_57969778:
    # mct "(Ah, fuck it. There's no harm in being friendly.)"
    mct ""

translate id w3MinaPlatonicHangStart_8f59568d:
    # mc "How are you?"
    mc ""

translate id w3MinaPlatonicHangStart_b3798b41:
    # mina "Mmmmh, I was bored, but now things are looking up!"
    mina ""

translate id w3MinaPlatonicHangStart_48e864a8:
    # mc "I expected you might say that, but let's order while the line is so short."
    mc ""

translate id w3MinaPlatonicHangStart_29d43ae8:
    # mina "Good idea!"
    mina ""

translate id w3MinaPlatonicHangStart_1d4a16fe:
    # mc "I didn't expect to hear from you, at least not so soon."
    mc ""

translate id w3MinaPlatonicHangStart_3a26065a:
    # mina "Oh, no. I hope I wasn't imposing by asking-"
    mina ""

translate id w3MinaPlatonicHangStart_922b61fe:
    # mc "That's not what I was getting at. Just that I would understand if I never heard from you again."
    mc ""

translate id w3MinaPlatonicHangStart_60dd0ac9:
    # mc "I mean, what kind of idiot forces someone to stick around and watch a movie after... well..."
    mc ""

translate id w3MinaPlatonicHangStart_f0c4aad7:
    # mina "No, no, no! I'm glad you did! I said as much, didn't I?"
    mina ""

translate id w3MinaPlatonicHangStart_9e42132e:
    # mc "I don't remember. Maybe?"
    mc ""

translate id w3MinaPlatonicHangStart_8e51f791:
    # "Honestly, the whole of June feels like a fog..."
    ""

translate id w3MinaPlatonicHangStart_7c943ff6:
    # mina "Well, what you did was extremely helpful... it put things into perspective for me..."
    mina ""

translate id w3MinaPlatonicHangStart_ae76590f:
    # mina "I thought a man's first priority was sex... that was what it seemed like to me..."
    mina ""

translate id w3MinaPlatonicHangStart_23468c07:
    # mina "Point is, the fact that you stuck around to be a friend, that was better than if you had given me what I was hoping for."
    mina ""

translate id w3MinaPlatonicHangStart_0fd4d656:
    # "......"
    ""

translate id w3MinaPlatonicHangStart_a20cefa7:
    # "..."
    ""

translate id w3MinaPlatonicHangStart_02da86f1:
    # "Her words felt convenient and I didn't know how to respond without sounding full of myself."
    ""

translate id w3MinaPlatonicHangStart_1e338f42:
    # mina "Like, {b}really{/b}."
    mina ""

translate id w3MinaPlatonicHangStart_e20525b9:
    # mc "...I heard from Ian that you two broke up."
    mc ""

translate id w3MinaPlatonicHangStart_20096fc3:
    # mina "That's right... it went..."
    mina ""

translate id w3MinaPlatonicHangStart_977d37c7:
    # mina "It went a lot smoother than I imagined it."
    mina ""

translate id w3MinaPlatonicHangStart_17b16f93:
    # mc "Great, I'm glad. You got a dating history now!"
    mc ""

translate id w3MinaPlatonicHangStart_a3b38f79:
    # mina "I guess I do, huh? Pretty big baggage for the first go around..."
    mina ""

translate id w3MinaPlatonicHangStart_4c2abd1a:
    # mc "Healthy dose of wisdom too perhaps. Don't fall for the same shit next time, right?"
    mc ""

translate id w3MinaPlatonicHangStart_0580d019:
    # mina "Pff, I'll try not to!"
    mina ""

translate id w3MinaPlatonicHangStart_2b35b6fe:
    # mc "..."
    mc ""

translate id w3MinaPlatonicHangStart_a2d24ca4:
    # mc "...you know, this is the same table we sat at when we first met."
    mc ""

translate id w3MinaPlatonicHangStart_7b0c1aee:
    # mina "Ha ha, is it?"
    mina ""

translate id w3MinaPlatonicHangStart_1c3491c0:
    # mc "Yep. I can recall it vividly... you looked pretty unhappy about meeting me."
    mc ""

translate id w3MinaPlatonicHangStart_4a5a2515:
    # mina "No I didn't! I didn't show that, did-"
    mina ""

translate id w3MinaPlatonicHangStart_49fde3cb:
    # mc "You did! You looked unhappy, but you quickly bounced back in like a fraction of an instant. The only reason I remember it is you gave me a great, big, remarkably warm greeting after that."
    mc ""

translate id w3MinaPlatonicHangStart_893070e3:
    # mina "Uuuhg, you had me pegged as a fake bitch from the beginning?"
    mina ""

translate id w3MinaPlatonicHangStart_6d87c3c2:
    # mc "You didn't let your gut reaction control you. That's cool."
    mc ""

translate id w3MinaPlatonicHangStart_c1ef9833:
    # mina "Hmpfh... feels like a long time ago."
    mina ""

translate id w3MinaPlatonicHangStart_82ce3920:
    # mc "Ha, doesn't it?"
    mc ""

translate id w3MinaPlatonicHangStart_0fd4d656_1:
    # "......"
    ""

translate id w3MinaPlatonicHangStart_a20cefa7_1:
    # "..."
    ""

translate id w3MinaPlatonicHangStart_79c205c0:
    # "As a moment of silence came to pass, Mina just smiled warmly and waited patiently for a sprout of a conversation to grow, well aware that it would only become an {i}awkward{/i} silence if we allowed it to be so."
    ""

translate id w3MinaPlatonicHangStart_16080cd9:
    # "Following suit, I didn't let my mind turn inward and self-analyze. Instead, I just did what was natural: focus on the pretty lady before me."
    ""

translate id w3MinaPlatonicHangStart_a6c69d27:
    # "Mina, like usual, looked stunning today. Of course, that went without saying, but there was just something about a blonde in a simple pair of daisy dukes that... and not to mention the sheerness of her blouse that... ah..."
    ""

translate id w3MinaPlatonicHangStart_24cd97fb:
    # "With any luck, to return hers in kind, looking Mina over put a smile on my face."
    ""

translate id w3MinaPlatonicHangStart_a0e27eb0:
    # mc "So... what did you want to do today?"
    mc ""

translate id w3MinaPlatonicHangStart_016ca28f:
    # "Because, failing that, I probably just look like a pervert."
    ""

translate id w3MinaPlatonicHangStart_2a8d18e9:
    # mina "Umm... well, I don't actually know... ha, I asked you to hang with no plan whatsoever!"
    mina ""

translate id w3MinaPlatonicHangStart_8d16594d:
    # mina "I expected you to blow me off. The only thing we have in common is video games, which goes for everyone..."
    mina ""

translate id w3MinaPlatonicHangStart_dff3bfc6:
    # mc "Don't worry. We'll come up with something. Actually, how about..."
    mc ""

translate id w3MinaPlatonicHangStart_d5387e47:
    # mina "Hmm...?"
    mina ""

translate id w3MinaPlatonicHangStart_514756d4:
    # mc "How 'bout we just spend the day figuring out what other interests we have in common."
    mc ""

translate id w3MinaPlatonicHangStart_eb2b5b0a:
    # mina "That... sounds {i}lovely{/i} to me, [mcf]."
    mina ""

translate id w3MinaPlatonicHangStart_8fe07248:
    # mc "Let's start here. You like coffee?"
    mc ""

translate id w3MinaPlatonicHangStart_cb230fe5:
    # mina "Yeah! Who doesn't?"
    mina ""

translate id w3MinaPlatonicHangStart_fb00020d:
    # mc "Me."
    mc ""

translate id w3MinaPlatonicHangStart_59104a90:
    # mina "Pffh, why are you drinking it?"
    mina ""

translate id w3MinaPlatonicHangStart_eddfcaa8:
    # mc "We're at a coffee shop."
    mc ""

translate id w3MinaPlatonicHangStart_5c62d6ca:
    # mina "Pfffh, haa, hha- what kind of logic is that? Hahahaha!"
    mina ""

translate id w3MinaPlatonicHangStart_8263dae4:
    # mina "You should've got tea!"
    mina ""

translate id w3MinaPlatonicHangStart_6e8bd7e6:
    # mc "I'm even less of a tea drinker."
    mc ""

translate id w3MinaPlatonicHangStart_aaa4939a:
    # mina "I thought we were trying to find things we had in common."
    mina ""

translate id w3MinaPlatonicHangStart_7ecf3a30:
    # mc "You don't ever find yourself going with the flow sometimes?"
    mc ""

translate id w3MinaPlatonicHangStart_906cbf8e:
    # mina "That- ah, I {b}do{/b}..."
    mina ""

translate id w3MinaPlatonicHangStart_b8e15864:
    # mc "Let's mark that down as something we have in common."
    mc ""

translate id w3MinaPlatonicHangStart_08b3a6b5:
    # mina "Ha! Yeah, okay, sure!"
    mina ""

translate id w3MinaPlatonicHangStart_0cb8e1be:
    # mc "See? You're really good with going with the flow. So, let's keep that momentum going and say... go for a walk?"
    mc ""

translate id w3MinaPlatonicHangStart_13d3eb17:
    # mina "Hehehe, sure."
    mina ""

translate id w3MinaPlatonicHangStart_120f0fcb:
    # mina "Let's finish the coffee you don't like first."
    mina ""

translate id w3MinaPlatonicHangStart_182fcfd8:
    # mc "This barely counts as coffee."
    mc ""

translate id w3MinaPlatonicHangStart_63fecdad:
    # "So, we set out on a quest, chatting and aimlessly hoofing it around the city."
    ""

translate id w3MinaPlatonicHangStart_173f2e0b:
    # "Naturally, we only shared a few interests. That was to be expected between me not having very many in the first place and our upbringings being so vastly different."
    ""

translate id w3MinaPlatonicHangStart_19d2ca13:
    # "The one thing that our walk did help me understand was something that was vastly more important than common hobbies or geeking out over the same shit."
    ""

translate id w3MinaPlatonicHangStart_e5beaee6:
    # "Mina simply had something that I lacked."
    ""

translate id w3MinaPlatonicHangStart_c462c56a:
    # "Like a cat basking in the warmth of a radiator, it was a delight to be around her."
    ""

translate id w3MinaPlatonicHangStart_ac2b5e39:
    # "I already knew this, but it confirmed her uncanny ability to make you feel important."
    ""

translate id w3MinaPlatonicHangStart_d6ff175b:
    # "She was a great listener, eye contact and smile never wavering as I griped about the aspects of university life I found annoying."
    ""

translate id w3MinaPlatonicHangStart_f410f34a:
    # mct "(Wait, am I really bitching this much?)"
    mct ""

translate id w3MinaPlatonicHangStart_6ecc1b57:
    # "Sure, I'd admit a thing or two to my mom, but it was pouring freely with Mina."
    ""

translate id w3MinaPlatonicHangStart_3bad29bb:
    # "Up to this point, all we had ever talked about was Ian, her work, her life... everything had a cause, and it was rarely about me. Of course, I preferred that way, but now she was extracting details from me that I was happy to provide."
    ""

translate id w3MinaPlatonicHangStart_dc4e7372:
    # "I told her about my experience growing up without a dad, which she related to herself as a child of divorce."
    ""

translate id w3MinaPlatonicHangStart_c6d49b45:
    # "I yammered in detail about things I often thought about but rarely spoke... memories of summer camp and the rare vacation my mom managed to swing."
    ""

translate id w3MinaPlatonicHangStart_ad7aadeb:
    # "Yep, we had very little in common besides sharing a time, a place, and a mood. That was WONDERFULLY simple in my mind."
    ""

translate id w3MinaPlatonicHangStart_584aaf44:
    # "That was all that was needed for a good friendship."
    ""

translate id w3MinaPlatonicHangStart_d55ecc5a:
    # "...oh, and truth be told, on a superficial level, I enjoyed the jealous look of strangers trying to figure out what the deal was."
    ""

translate id w3MinaPlatonicHangStart_d55ecc5a_1:
    # "...oh, and truth be told, on a superficial level, I enjoyed the jealous look of strangers trying to figure out what the deal was."
    ""

translate id w3MinaPlatonicHangStart_2e513023:
    # "I enjoyed our company so much that a root of self-doubt planted itself in my head."
    ""

translate id w3MinaPlatonicHangStart_e956cfad:
    # "What did the radiator get from the cat?"
    ""

translate id w3MinaPlatonicHangStart_8a74bd75:
    # mc "Can I ask you something? Do you actually enjoy talking to me?"
    mc ""

translate id w3MinaPlatonicHangStart_cf9bb512:
    # mina "Huh...? {b}Yeah!{/b} You're a lot of fun!"
    mina ""

translate id w3MinaPlatonicHangStart_0ffbfcba:
    # "No hesitation."
    ""

translate id w3MinaPlatonicHangStart_fa45f281:
    # mc "Thanks..."
    mc ""

translate id w3MinaPlatonicHangStart_c6052d52:
    # mina "Yeah, no problem! Why the heck do you ask?"
    mina ""

translate id w3MinaPlatonicHangStart_d1f1475d:
    # mc "Just being on brand for a guy like me."
    mc ""

translate id w3MinaPlatonicHangStart_fe0fff96:
    # mina "How do you figure you're like, [mcf]? I'd love to hear it!"
    mina ""

translate id w3MinaPlatonicHangStart_a3af09e0:
    # mc "...lacking in interpersonal value?"
    mc ""

translate id w3MinaPlatonicHangStart_27b4c984:
    # mina "Where would you get that idea? Have we not been \"inter-personalizing\" the last month?"
    mina ""

translate id w3MinaPlatonicHangStart_8f5a54bf:
    # mc "Eh... I chalk it all up to convenience, circumstance, and the brewing storm with Ian..."
    mc ""

translate id w3MinaPlatonicHangStart_5b7073ae:
    # mina "Well, fuck you too."
    mina ""

translate id w3MinaPlatonicHangStart_6f374789:
    # mc "Shit, I didn't mean to make you mad."
    mc ""

translate id w3MinaPlatonicHangStart_0fd4d656_2:
    # "......"
    ""

translate id w3MinaPlatonicHangStart_a20cefa7_2:
    # "..."
    ""

translate id w3MinaPlatonicHangStart_13df6414:
    # mc "Why are you mad?"
    mc ""

translate id w3MinaPlatonicHangStart_565aa8d3:
    # mina "Figure it out, you geek!"
    mina ""

translate id w3MinaPlatonicHangStart_0fd4d656_3:
    # "......"
    ""

translate id w3MinaPlatonicHangStart_a20cefa7_3:
    # "..."
    ""

translate id w3MinaPlatonicHangStart_381c3f99:
    # mc "Sorry, I didn't mean to make you sound opportunistic."
    mc ""

translate id w3MinaPlatonicHangStart_0a0b09aa:
    # mina "Yeeeeeah, you did..."
    mina ""

translate id w3MinaPlatonicHangStart_3438d78c:
    # mc "I'm a cynical person. The things I say reflect more on me than others."
    mc ""

translate id w3MinaPlatonicHangStart_0fd4d656_4:
    # "......"
    ""

translate id w3MinaPlatonicHangStart_a20cefa7_4:
    # "..."
    ""

translate id w3MinaPlatonicHangStart_14a9ba2a:
    # mina "No, I get it..."
    mina ""

translate id w3MinaPlatonicHangStart_0fd4d656_5:
    # "......"
    ""

translate id w3MinaPlatonicHangStart_a20cefa7_5:
    # "..."
    ""

translate id w3MinaPlatonicHangStart_105fece0:
    # mina "I mean, I confessed to you that I think I'm a freakin' robot sometimes. {b}I get it{/b}."
    mina ""

translate id w3MinaPlatonicHangStart_6eaa65cc:
    # mina "I see those qualities in other people too. It's why I can't wrap my head around why you wanted to hang out today."
    mina ""

translate id w3MinaPlatonicHangStart_b17edb07:
    # mina "Men and women... what do they do together besides sex? If you don't want {i}that{/i}, then what the hell do I even offer?"
    mina ""

translate id w3MinaPlatonicHangStart_e3028871:
    # mc "Damn. You're just as bad as me."
    mc ""

translate id w3MinaPlatonicHangStart_0a89f3e9:
    # mina "Maybe!"
    mina ""

translate id w3MinaPlatonicHangStart_7d1448cf:
    # mc "The all-girls-school into dating a shitbag did a number on how you think about guys, huh?"
    mc ""

translate id w3MinaPlatonicHangStart_9ad625fb:
    # mina "Definitely!"
    mina ""

translate id w3MinaPlatonicHangStart_92d0069f:
    # mc "Truth be told, I find you very soothing to be around."
    mc ""

translate id w3MinaPlatonicHangStart_09e98813:
    # mina "H-huh...? Heh, I don't know how I should take that."
    mina ""

translate id w3MinaPlatonicHangStart_16702151:
    # mc "That and I have next to no friends. Can't be picky, but what about you?"
    mc ""

translate id w3MinaPlatonicHangStart_f3bb65d9:
    # mina "Hurt pride...?"
    mina ""

translate id w3MinaPlatonicHangStart_994bea14:
    # mc "Ha, that really it?"
    mc ""

translate id w3MinaPlatonicHangStart_1a24446d:
    # mina "You decide, Mr. Cynic."
    mina ""

translate id w3MinaPlatonicHangStart_0fd4d656_6:
    # "......"
    ""

translate id w3MinaPlatonicHangStart_a20cefa7_6:
    # "..."
    ""

translate id w3MinaPlatonicHangStart_5a8333ab:
    # "Yet another silence, in a long series of silences today..."
    ""

translate id w3MinaPlatonicHangStart_f7678f7a:
    # "As always, Mina smiled openly without showing any doubt or fear."
    ""

translate id w3MinaPlatonicHangStart_362fd241:
    # "All day I found that admirable, but it was also sad, wasn't it?"
    ""

translate id w3MinaPlatonicSwitch_5e179bb1:
    # "This time, I went with the flow - pushing all doubts about motivation and circumstances back down into my gut."
    ""

translate id w3MinaPlatonicSwitch_8fefbabf:
    # "Was the situation all that different from the other day? Nah."
    ""

translate id w3MinaPlatonicSwitch_99ab1fcc:
    # "Not at all."
    ""

translate id w3MinaPlatonicSwitch_2d19a38a:
    # "Did the fact she broke up with Ian change anything?"
    ""

translate id w3MinaPlatonicSwitch_0bf80eee:
    # "I mean... waiting a whole two days? Not to him, but it may have made an appreciable difference for us."
    ""

translate id w3MinaPlatonicSwitch_64cccb62:
    # "It was now in Mina's court if she wanted to return my rejection."
    ""

translate id w3MinaPlatonicSwitch_114626b5:
    # mina "Mmmhhh..."
    mina ""

translate id w3MinaPlatonicSwitch_1e43dd5c:
    # "She accepted, and I refused to ruminate on why."
    ""

translate id w3MinaPlatonicSwitch_544faa47:
    # " Did she really find my company worthwhile? I'll trust her."
    ""

translate id w3MinaPlatonicSwitch_795d4357:
    # mina "A-aah...!"
    mina ""

translate id w3MinaPlatonicSwitch_eb1ba572:
    # "I'll trust her and sit my ass as close to that radiator as I can get."
    ""

translate id w3MinaPlatonicSwitch_b51fdcce:
    # mina "Ha, haa...! S-seriously-!"
    mina ""

translate id w3MinaPlatonicSwitch_11455b53:
    # mc "What?"
    mc ""

translate id w3MinaPlatonicSwitch_2e48d7bf:
    # mina "Mixed signals, much?!"
    mina ""

translate id w3MinaPlatonicSwitch_f575d6fa:
    # mc "As you astutely pointed out earlier... I'm a guy and you're a--"
    mc ""

translate id w3MinaPlatonicSwitch_d704ed20:
    # mina "Shut up."
    mina ""

translate id w3MinaPlatonicSwitch_859c4a33:
    # mina "MMmhh...!"
    mina ""

translate id w3MinaPlatonicSwitch_98bb4036:
    # "This time, she pressed her lips to mine, with an added dose of frustration."
    ""

translate id w3MinaPlatonicSwitch_122ff895:
    # mct "(Man, I got to ask her where she gets her lip balm, she tastes-)"
    mct ""

translate id w3MinaPlatonicSwitch_01b68b61:
    # mina "That was true the other day too, you pasty freak."
    mina ""

translate id w3MinaPlatonicSwitch_0bff83c9:
    # mc "Timing is important."
    mc ""

translate id w3MinaPlatonicSwitch_fcceca73:
    # mina "What? Not good enough to pull you in the first go, but good enough to wear you down?"
    mina ""

translate id w3MinaPlatonicSwitch_feb3988d:
    # mc "Last time it was about Ian."
    mc ""

translate id w3MinaPlatonicSwitch_79fb337c:
    # mina "It was a lot of things..."
    mina ""

translate id w3MinaPlatonicSwitch_3c305af5:
    # mc "Now it felt more like just the two of us sitting on a bench, so I forgot everything else."
    mc ""

translate id w3MinaPlatonicSwitch_cc3df4bb:
    # mina "You don't know how close I was to spitting in your face."
    mina ""

translate id w3MinaPlatonicSwitch_65ac3f6b:
    # mc "But you didn't."
    mc ""

translate id w3MinaPlatonicSwitch_4e2a8876:
    # mina "..."
    mina ""

translate id w3MinaPlatonicSwitch_eda3775c:
    # mc "Thank you for not doing that."
    mc ""

translate id w3MinaPlatonicSwitch_e400290a:
    # mina "Mmmhh...♥"
    mina ""

translate id w3MinaPlatonicSwitch_95ad036e:
    # "*Chwup, fwhup~*"
    ""

translate id w3MinaPlatonicSwitch_62fe19bf:
    # mina "Wanna go back to your place?"
    mina ""

translate id w3MinaPlatonicSwitch_d70984a0:
    # "She was surprisingly forward."
    ""

translate id w3MinaPlatonicSwitch_a91198ec:
    # mc "Ian's sick, but you never know; he could drop by. How about yours?"
    mc ""

translate id w3MinaPlatonicSwitch_65f451b7:
    # mina "When I was brooding this weekend, I realized something."
    mina ""

translate id w3MinaPlatonicSwitch_652650d3:
    # mc "...?"
    mc ""

translate id w3MinaPlatonicSwitch_46ba6502:
    # mina "Way too pink!"
    mina ""

translate id w3MinaPlatonicSwitch_a2b6db97:
    # mc "Pffh- really?"
    mc ""

translate id w3MinaPlatonicSwitch_c9f991f0:
    # mina "*Gulp* Let's... let's get a hotel room..."
    mina ""

translate id w3MinaPlatonicSwitch_a49ed4f8:
    # mc "Okay."
    mc ""

translate id w3MinaPlatonicSwitch_1ba343d8:
    # "Go with the flow."
    ""

translate id w3MinaPlatonicSwitch_3052c1dd:
    # "As I had previously, I felt a tug from inside of myself, compelling me to close the distance between myself and the pretty girl and see where it all led."
    ""

translate id w3MinaPlatonicSwitch_b19f1752:
    # "Just as I had previously, I ignored that urge."
    ""

translate id w3MinaPlatonicSwitch_7736f354:
    # "*Boop!*"
    ""

translate id w3MinaPlatonicSwitch_13568e89:
    # mina "Pffha, ha-haha- w-wha~ ah... you looked so serious for a moment!"
    mina ""

translate id w3MinaPlatonicSwitch_a0c5a24c:
    # mc "Let's go drop by the arcade."
    mc ""

translate id w3MinaPlatonicSwitch_0bd0a4e3:
    # mina "Hehe, sure!"
    mina ""

translate id w3MinaPlatonicSwitch_d280c9fa:
    # mc "You know, I remember last time you ended up owing me a favor. Maybe we'll make it another one."
    mc ""

translate id w3MinaPlatonicSwitch_879ce78d:
    # mina "Fat chance! That's only because you got my brother to distract me!"
    mina ""

translate id w3MinaPlatonicSwitch_0d68a069:
    # mc "You knew?"
    mc ""

translate id w3MinaPlatonicSwitch_8fe0d605:
    # mina "Pff, you try hard! Do you think he didn't tell me?"
    mina ""

translate id w3MinaPlatonicSwitch_41f08026:
    # mina "Maybe I'll get another piggyback out of it."
    mina ""

translate id w3MinaPlatonicSwitch_00ad8e6d:
    # mc "You should aim higher than that."
    mc ""

translate id w3MinaPlatonicSwitch_aeb2bf5f:
    # mina "Should I?"
    mina ""

translate id w3MinaKissRejection_0167c652:
    # mina "You got something on your lip."
    mina ""

translate id w3MinaKissRejection_aad7fd15:
    # mc "Oh..?"
    mc ""

translate id w3MinaKissRejection_4779fece:
    # "Mina was very kind."
    ""

translate id w3MinaKissRejection_67292973:
    # mc "Did I get it?"
    mc ""

translate id w3MinaKissRejection_7bc172df:
    # "I played along."
    ""

translate id w3MinaKissRejection_379e46bd:
    # mina "Yeah, all gone."
    mina ""

translate id w3MinaKissRejection_fa45f281:
    # mc "Thanks..."
    mc ""

translate id w3MinaKissRejection_2613fc75:
    # mina "Don't mention it..."
    mina ""

translate id w3MinaKissRejection_0fd4d656:
    # "......"
    ""

translate id w3MinaKissRejection_a20cefa7:
    # "..."
    ""

translate id w3MinaKissRejection_c9a518bd:
    # "Even now, she smiled."
    ""

translate id w3MinaKissRejection_ee131a6b:
    # mina "Want to go watch a movie?"
    mina ""

translate id w3MinaKissRejection_fc694cbf:
    # mc "Ha, hahaha! Oh-"
    mc ""

translate id w3MinaKissRejection_275ff1d6:
    # "Okay, she didn't let it {i}totally{/i} slide."
    ""

translate id w3MinaKissRejection_3adb32a0:
    # mina "What's so funny? Isn't that like the only thing you do besides studying?"
    mina ""

translate id w3MinaKissRejection_e9ef7817:
    # mc "Ha, ohhaoo..!"
    mc ""

translate id w3MinaKissRejection_9e090842:
    # mina "Dang nerd, but no thanks. I think I'm going to head home."
    mina ""

translate id w3MinaOutsideFrisk_84138732:
    # mina "So... uh..."
    mina ""

translate id w3MinaOutsideFrisk_8569461d:
    # mina "You were implying that we are going to-"
    mina ""

translate id w3MinaOutsideFrisk_080a3e66:
    # mc "So, what do you want to do?"
    mc ""

translate id w3MinaOutsideFrisk_e002f7eb:
    # mina "What?"
    mina ""

translate id w3MinaOutsideFrisk_ff2ca7b8:
    # mc "It's up to you. I'm at your disposal."
    mc ""

translate id w3MinaOutsideFrisk_199146c0:
    # mina "Euech... don't say it like that!"
    mina ""

translate id w3MinaOutsideFrisk_8eaf6bbd:
    # mc "We could play games like you said, maybe watch a movie..."
    mc ""

translate id w3MinaOutsideFrisk_15c9783c:
    # mc "You could pick anything on your \"list\" and we can see about making it a reality..."
    mc ""

translate id w3MinaOutsideFrisk_eee70d23:
    # mina "*Gulp* A-anything...?"
    mina ""

translate id w3MinaOutsideFrisk_6ad91c04:
    # mc "Why don't you tell me what's on it?"
    mc ""

translate id w3MinaOutsideFrisk_ba10a80a:
    # mina "That's, uh... ah..."
    mina ""

translate id w3MinaOutsideFrisk_ca1114a6:
    # mc "We're past the point of you being shy."
    mc ""

translate id w3MinaOutsideFrisk_520cd605:
    # mina "You turd! Stop trying to be suave!"
    mina ""

translate id w3MinaOutsideFrisk_455066ec:
    # mc "You gotta tell me."
    mc ""

translate id w3MinaOutsideFrisk_33cf5958:
    # mina "You're suddenly putting me on the spot, like what do I- ah... let's..."
    mina ""

translate id w3MinaOutsideFrisk_55bce863:
    # mina "Let's go to a hotel."
    mina ""

translate id w3MinaOutsideFrisk_41d578b2:
    # mc "That's on your list?"
    mc ""

translate id w3MinaOutsideFrisk_826477d7:
    # mina "Yeah... I want to see if it'll feel... {i}illicit{/i}... plus... "
    mina ""

translate id w3MinaOutsideFrisk_cbfb53b1:
    # mina "......"
    mina ""

translate id w3MinaOutsideFrisk_50da6c3f:
    # mina "...we haven't even done {i}it{/i} yet."
    mina ""

translate id w3MinaOutsideFrisk_7171a086:
    # "Seeing her conscious of her surroundings made me want to bully her a little."
    ""

translate id w3MinaOutsideFrisk_7132928b:
    # mc "Done what...?"
    mc ""

translate id w3MinaOutsideFrisk_c9c8b1d5:
    # "I would have been mortified right there with her a few weeks ago."
    ""

translate id w3MinaOutsideFrisk_7c9fe05d:
    # mina "I told you to stop trying to be cool."
    mina ""

translate id w3MinaOutsideFrisk_0932bce3:
    # mc "Tell me."
    mc ""

translate id w3MinaOutsideFrisk_62b122a9:
    # mina "Hnnn... waw-wahh-"
    mina ""

translate id w3MinaOutsideFrisk_ceacb431:
    # mc "How can I help without you being explicit about it? What do you want to do today?"
    mc ""

translate id w3MinaOutsideFrisk_cbfb53b1_1:
    # mina "......"
    mina ""

translate id w3MinaOutsideFrisk_4e2a8876:
    # mina "..."
    mina ""

translate id w3MinaOutsideFrisk_81f827fe:
    # mc "We're the only people who matter."
    mc ""

translate id w3MinaOutsideFrisk_4c268ad3:
    # "I did wonder if her list included a public display like this."
    ""

translate id w3MinaOutsideFrisk_4aab1dce:
    # mina "........."
    mina ""

translate id w3MinaOutsideFrisk_e00ba1f2:
    # mina "....."
    mina ""

translate id w3MinaOutsideFrisk_d46378f2:
    # mina "...I want you to mess me up."
    mina ""

translate id w3MinaOutsideFrisk_223eb772:
    # mct "(--!)"
    mct ""

translate id w3MinaOutsideFrisk_3ba6ef6f:
    # mina "Let's just get a hotel and we can tick off a few of the minor items... okay...?"
    mina ""

translate id w3MinaOutsideFrisk_f62913ed:
    # mc "I'm at your disposal."
    mc ""

translate id w3MinaOutsideFrisk_3ef5f1f6:
    # mina "Stop saying it like that!"
    mina ""

translate id w3MinaHotel_d84d02f4:
    # "Did you cheat with Mina or make a move after they broke up?"
    ""

translate id w3MinaHotel_7b08ca7b:
    # "We made our way to the hotel. I was the one that picked the place, a rather upscale accommodation by my old standards."
    ""

translate id w3MinaHotel_39b3ce42:
    # "I figured {i}what the hell.{/i} This would be our first time. Why not splurge?"
    ""

translate id w3MinaHotel_f26daa35:
    # "I figured {i}what the hell.{/i} Why not splurge? If she was hoping for something illicit, let her feel like an {i}expensive{/i} woman at least."
    ""

translate id w3MinaHotel_21db67e3:
    # "I was being paid generously for effectively nothing, after all."
    ""

translate id w3MinaHotel_0aea3943:
    # mct "(...I would do well to guard myself against becoming too indulgent.)"
    mct ""

translate id w3MinaHotel_cbfb53b1:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876:
    # mina "..."
    mina ""

translate id w3MinaHotel_3b21f354:
    # mc "......"
    mc ""

translate id w3MinaHotel_2b35b6fe:
    # mc "..."
    mc ""

translate id w3MinaHotel_2f53598e:
    # "*Ding!*"
    ""

translate id w3MinaHotel_cbfb53b1_1:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876_1:
    # mina "..."
    mina ""

translate id w3MinaHotel_1e1cdee7:
    # mina "Huuh-"
    mina ""

translate id w3MinaHotel_b06b6f89:
    # mc "*Whisper* So, how do you want me to mess you up?"
    mc ""

translate id w3MinaHotel_67a843f0:
    # mina "Tha--"
    mina ""

translate id w3MinaHotel_2f53598e_1:
    # "*Ding!*"
    ""

translate id w3MinaHotel_5ee9eaa1:
    # mc "Let's go."
    mc ""

translate id w3MinaHotel_bac96876:
    # mina "We didn't have to get such an expensive room..."
    mina ""

translate id w3MinaHotel_3a4a6021:
    # mc "I wanted to."
    mc ""

translate id w3MinaHotel_6c121053:
    # mina "Seriously, it's not like we're staying the night."
    mina ""

translate id w3MinaHotel_ef4887b6:
    # mc "We could if you want..."
    mc ""

translate id w3MinaHotel_4f275e35:
    # mc "I don't have much going on. Do you?"
    mc ""

translate id w3MinaHotel_9b389c1e:
    # mina "Shut up! I don't have a change of clothes!"
    mina ""

translate id w3MinaHotel_1b6fb80f:
    # mc "Mmmh... so? Do we need clothes?"
    mc ""

translate id w3MinaHotel_d2ba2494:
    # mc "Take 'em off so they won't get dirty."
    mc ""

translate id w3MinaHotel_34c36e0b:
    # mina "Hnnngg..."
    mina ""

translate id w3MinaHotel_b62f15b2:
    # mina "I don't have any of my morning treatments!"
    mina ""

translate id w3MinaHotel_7247026a:
    # "*Chwup~*"
    ""

translate id w3MinaHotel_246e3db0:
    # mina "Ha, h-ha..."
    mina ""

translate id w3MinaHotel_52bc10b5:
    # mc "This place has everything we need."
    mc ""

translate id w3MinaHotel_f5d9e881:
    # mina "W-we'll see... a whole day and night is a bit..."
    mina ""

translate id w3MinaHotel_72063b28:
    # mina "You'll treat me... {i}nice{/i}, right?"
    mina ""

translate id w3MinaHotel_049b0cc5:
    # mc "How else would I possibly treat you?"
    mc ""

translate id w3MinaHotel_f7ab395a:
    # mc "I'll treat you any way you ask me to, Mina."
    mc ""

translate id w3MinaHotel_73541235:
    # mina "You know, you'll be the second man I've been with... it's funny..."
    mina ""

translate id w3MinaHotel_e4068727:
    # mc "What is?"
    mc ""

translate id w3MinaHotel_982be1ce:
    # mina "I'm just as nervous as if it was the first time... it's like something's pushing down on my chest..."
    mina ""

translate id w3MinaHotel_8cb82270:
    # mina "I know we've already messed around, but... we didn't..."
    mina ""

translate id w3MinaHotel_3880e387:
    # mc "You don't have anything to be nervous about."
    mc ""

translate id w3MinaHotel_cd24ee01:
    # mc "I have a feeling you're going to eat me alive."
    mc ""

translate id w3MinaHotel_03104076:
    # mina "This isn't just about me..."
    mina ""

translate id w3MinaHotel_3a12df0e:
    # mina "I want us to both feel good."
    mina ""

translate id w3MinaHotel_827ca4d0:
    # mc "You call the shots here."
    mc ""

translate id w3MinaHotel_03104076_1:
    # mina "This isn't just about me..."
    mina ""

translate id w3MinaHotel_7bd31c79:
    # mina "I want to make some of your fantasies come true too."
    mina ""

translate id w3MinaHotel_5929397e:
    # mct "(Ah... this woman!)"
    mct ""

translate id w3MinaHotel_935cf4a1:
    # mina "Hmmm..."
    mina ""

translate id w3MinaHotel_54429590:
    # mina "{size=-10}Probably not complimentary...{/size}"
    mina ""

translate id w3MinaHotel_122f358e:
    # mc "What did you say?"
    mc ""

translate id w3MinaHotel_0d90ecd9:
    # mina "Oooh, wow..."
    mina ""

translate id w3MinaHotel_11455b53:
    # mc "What?"
    mc ""

translate id w3MinaHotel_19d1d7c2:
    # mina "Heck of a bed."
    mina ""

translate id w3MinaHotel_fed8a3de:
    # mc "Think it'll do for our purposes?"
    mc ""

translate id w3MinaHotel_69217bdb:
    # mina "Uh huh... a lot of room ..."
    mina ""

translate id w3MinaHotel_95859efe:
    # mina "We're here... so there's only one thing to do..."
    mina ""

translate id w3MinaHotel_de39c5ad:
    # mc "There's no rush."
    mc ""

translate id w3MinaHotel_5e8a5955:
    # mina "...?"
    mina ""

translate id w3MinaHotel_d193cde2:
    # mc "We have all the time in the world."
    mc ""

translate id w3MinaHotel_4d202582:
    # mina "That's different from what you told the front desk."
    mina ""

translate id w3MinaHotel_1b9d059d:
    # mc "I want to make the most of today."
    mc ""

translate id w3MinaHotel_cbfb53b1_2:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876_2:
    # mina "..."
    mina ""

translate id w3MinaHotel_456b40c4:
    # mc "I'll pour us a drink."
    mc ""

translate id w3MinaHotel_5efa040d:
    # mina "What do they have?"
    mina ""

translate id w3MinaHotel_0fd4d656:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7:
    # "..."
    ""

translate id w3MinaHotel_3ce76636:
    # mc "By the way, what was in that bag?"
    mc ""

translate id w3MinaHotel_97b72844:
    # mina "Oh, uh... {b}you'll see.{/b} Later."
    mina ""

translate id w3MinaHotel_876802f8:
    # mc "How mysterious."
    mc ""

translate id w3MinaHotel_bb34f705:
    # mina "You won't have to wait long to find out."
    mina ""

translate id w3MinaHotel_ede25aef:
    # mc "Can't you just tell me now?"
    mc ""

translate id w3MinaHotel_0fd4d656_1:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_1:
    # "..."
    ""

translate id w3MinaHotel_2f76bc34:
    # mina "Nope!"
    mina ""

translate id w3MinaHotel_536e369f:
    # mc "Okay, keep your secrets."
    mc ""

translate id w3MinaHotel_0664c5af:
    # mina "It's funny. We've only hung out a few times..."
    mina ""

translate id w3MinaHotel_4d9a94a6:
    # mc "Let's not reminisce. I want to get to know you better, Mina."
    mc ""

translate id w3MinaHotel_a99eafe5:
    # mina "Sure, but... what should we talk about?"
    mina ""

translate id w3MinaHotel_ad909a8e:
    # mc "Hmmm..."
    mc ""

translate id w3MinaHotel_4a78e133:
    # "*Glug, glug, glug*"
    ""

translate id w3MinaHotel_0c827199:
    # mc "How about... we ask each other some questions? "
    mc ""

translate id w3MinaHotel_b84215d9:
    # mc "Anything either of us would like to know, no matter how personal or embarrassing."
    mc ""

translate id w3MinaHotel_8e5f1ba2:
    # mina "I could think of a few things I'd like to ask, but..."
    mina ""

translate id w3MinaHotel_0fd4d656_2:
    # "......"
    ""

translate id w3MinaHotel_5f9b772e:
    # "...."
    ""

translate id w3MinaHotel_7758b16a:
    # mina "Whaaaaaaaaat if you don't want to answer them?"
    mina ""

translate id w3MinaHotel_2e978299:
    # mc "Only one way to find out: ask me something."
    mc ""

translate id w3MinaHotel_706502b6:
    # mina "Anything? Even if it's weird and might kill the vibe?"
    mina ""

translate id w3MinaHotel_6aecea20:
    # mc "Okay, now I got to know. Hit me."
    mc ""

translate id w3MinaHotel_0fd4d656_3:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_2:
    # "..."
    ""

translate id w3MinaHotel_aafa30cf:
    # mina "How many women have you slept with, [mcf]?"
    mina ""

translate id w3MinaHotel_6eb0c0e1:
    # mc "That's what you want to know?"
    mc ""

translate id w3MinaHotel_f48d6a6d:
    # mina "You know my complete, pitiful history."
    mina ""

translate id w3MinaHotel_a367da6b:
    # mc "Fair enough, uh..."
    mc ""

translate id w3MinaHotel_23ba53cd:
    # "This was easier to calculate before the club. Do I count the old woman tugging me off in her office when I first met Veronica?"
    ""

translate id w3MinaHotel_b5424428:
    # "Do I tell her the number before it began being served to me on a silver platter?"
    ""

translate id w3MinaHotel_9bead92e:
    # mina "Ehehe... you have to think about it that long?"
    mina ""

translate id w3MinaHotel_7db0a61a:
    # mc "Hey! I got three years on you. Of course I have to think about it!"
    mc ""

translate id w3MinaHotel_82a2c2bb:
    # mina "Reeeeeeallly?"
    mina ""

translate id w3MinaHotel_48b9be13:
    # mc "I'm offended that you seem so surprised, quite frankly."
    mc ""

translate id w3MinaHotel_db249b67:
    # mina "It's not that! I just figured that was the kind of thing people kept track of!"
    mina ""

translate id w3MinaHotel_d54df773:
    # mc "{i}Especially{/i} guys?"
    mc ""

translate id w3MinaHotel_83c0a670:
    # mina "Everyone!"
    mina ""

translate id w3MinaHotel_ad909a8e_1:
    # mc "Hmmm..."
    mc ""

translate id w3MinaHotel_b932af65:
    # "I'll go with the number I feel I pulled on my own merit, absent club business, but including Hana and Felicia."
    ""

translate id w3MinaHotel_7f01d019:
    # mc "I've been with five women."
    mc ""

translate id w3MinaHotel_39b031d7:
    # "I'll go with the number I feel I pulled on my own merit, absent club business, but including Felicia."
    ""

translate id w3MinaHotel_63ea45a0:
    # mc "I've been with four women."
    mc ""

translate id w3MinaHotel_2231ee75:
    # "I'll go with the number I feel I pulled on my own merit, absent club business, but including Hana."
    ""

translate id w3MinaHotel_63ea45a0_1:
    # mc "I've been with four women."
    mc ""

translate id w3MinaHotel_9d0baa83:
    # "I'll go with the number I feel I pulled on my own merit, absent club business and before this Summer."
    ""

translate id w3MinaHotel_bd4c31ab:
    # mc "I've been with three women."
    mc ""

translate id w3MinaHotel_5323d6c7:
    # mina "That's less than I expected..."
    mina ""

translate id w3MinaHotel_922c09ae:
    # mina "A-ah, I don't mean that in a bad way or anything!"
    mina ""

translate id w3MinaHotel_2d0dc062:
    # mc "I know you didn't."
    mc ""

translate id w3MinaHotel_0fd4d656_4:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_3:
    # "..."
    ""

translate id w3MinaHotel_14ff30c9:
    # mc "I'm secure with that area of my life."
    mc ""

translate id w3MinaHotel_22bdc126:
    # mina "Ha! I'm not!"
    mina ""

translate id w3MinaHotel_0fd4d656_5:
    # "......"
    ""

translate id w3MinaHotel_ff47d6ce:
    # mina "I've got a lot of things to learn... and probably unlearn too."
    mina ""

translate id w3MinaHotel_a50b6fdf:
    # mc "Can I ask you something now?"
    mc ""

translate id w3MinaHotel_31d27135:
    # mina "Ask away! I owe you something personal or embarrassing~"
    mina ""

translate id w3MinaHotel_ec4f64bc:
    # mc "Whose idea was it to grab a bottle of vermouth and drink it straight?"
    mc ""

translate id w3MinaHotel_3d732bbb:
    # mina "You said the goal was to loosen up, not get blasted."
    mina ""

translate id w3MinaHotel_47a9f8cd:
    # mc "Oh, yeah..."
    mc ""

translate id w3MinaHotel_49e8a433:
    # mina "Heh, feeling loose yet, [mcf]?"
    mina ""

translate id w3MinaHotel_139be395:
    # mc "I've been entirely at ease since we walked into the hotel."
    mc ""

translate id w3MinaHotel_08dd9ee3:
    # mina "You're not nervous? Not even... a {i}little{/i} bit?"
    mina ""

translate id w3MinaHotel_570f257d:
    # "The worried look on her face told me she'd like me to be - and a few weeks ago, I might've been, but it's not like I could tell her that."
    ""

translate id w3MinaHotel_136aee83:
    # mc "I did mention that you had a soothing effect on me."
    mc ""

translate id w3MinaHotel_667c2346:
    # mc "How could I be stuck in my head when what's outside of it has my full attention?"
    mc ""

translate id w3MinaHotel_445ea1c2:
    # mina "Ah... aha... you don't have to try so hard. We've already paid for the room..."
    mina ""

translate id w3MinaHotel_10c5df98:
    # mc "{b}Come here.{/b}"
    mc ""

translate id w3MinaHotel_3ee0b3e5:
    # mina "Why? What do you want?"
    mina ""

translate id w3MinaHotel_f47b63a7:
    # mc "Just come here. Please?"
    mc ""

translate id w3MinaHotel_22149b46:
    # mina "Heh, sure..."
    mina ""

translate id w3MinaHotel_350fc772:
    # mina "What are you thi--"
    mina ""

translate id w3MinaHotel_f9dcfd98:
    # mc "Do I seem nervous to you?"
    mc ""

translate id w3MinaHotel_7f6f72f4:
    # mina "Guess not..."
    mina ""

translate id w3MinaHotel_bcc7989d:
    # mc "Do you think I'm bullshitting you when I say you're so beautiful that you occupy every thought in my head right now?"
    mc ""

translate id w3MinaHotel_cbfb53b1_3:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876_3:
    # mina "..."
    mina ""

translate id w3MinaHotel_808d2891:
    # mc "I'm not even sweating how cheesy that line was."
    mc ""

translate id w3MinaHotel_4e815b67:
    # mina "Ha! Ah... it's getting stuffy..."
    mina ""

translate id w3MinaHotel_1d46c928:
    # mc "May I?"
    mc ""

translate id w3MinaHotel_bd981e9d:
    # mina "Uhhuh..."
    mina ""

translate id w3MinaHotel_2f021740:
    # mc "You look good in black."
    mc ""

translate id w3MinaHotel_db1a8b8f:
    # mina "Thanks..."
    mina ""

translate id w3MinaHotel_a56f0cd4:
    # mc "Is this better?"
    mc ""

translate id w3MinaHotel_9bd77a07:
    # mina "There's still one button to go..."
    mina ""

translate id w3MinaHotel_8d723d0d:
    # mc "By the way, are you still feeling nervous?"
    mc ""

translate id w3MinaHotel_e49aeb5e:
    # mina "No... NERVOUS is not what I'm feeling..."
    mina ""

translate id w3MinaHotel_2902b785:
    # mc "We're on the same page, then."
    mc ""

translate id w3MinaHotel_f9428155:
    # mina "We're not QUITE even-Steven..."
    mina ""

translate id w3MinaHotel_42d4f096:
    # "Mina tugged cutely at my shirt, making her request known before she even said it."
    ""

translate id w3MinaHotel_ba49d103:
    # mina "Lift your arms."
    mina ""

translate id w3MinaHotel_260c5d66:
    # "I did as she asked, of course."
    ""

translate id w3MinaHotel_bec725a3:
    # mina "There we go..."
    mina ""

translate id w3MinaHotel_ffb1ad3e:
    # "In a lovely reversal, her eyes focused on MY chest for once, mapping out its broadness and following down, down, and down beyond the reaches of my stomach."
    ""

translate id w3MinaHotel_4aab1dce:
    # mina "........."
    mina ""

translate id w3MinaHotel_cbfb53b1_4:
    # mina "......"
    mina ""

translate id w3MinaHotel_06191434:
    # mina "...what were we talking about again?"
    mina ""

translate id w3MinaHotel_c1e4eb78:
    # mc "How did your day go after I left yesterday? You told me you were going to see Felicia?"
    mc ""

translate id w3MinaHotel_a5d2ad11:
    # mc "I was about to ask you how your weekend went."
    mc ""

translate id w3MinaHotel_2dd2e356:
    # mina "It was alright... after ending things with--"
    mina ""

translate id w3MinaHotel_dc4c2c0f:
    # mc "You can skip that part."
    mc ""

translate id w3MinaHotel_fb946e4e:
    # mina "Ehehe, I hung out with Felicia yesterday. That was nice."
    mina ""

translate id w3MinaHotel_26a31ea5:
    # mina "We went shopping! Then I went back to her place and we spent all afternoon together and into this morning."
    mina ""

translate id w3MinaHotel_e8aeacf9:
    # mc "You slept over? Was Elias there?"
    mc ""

translate id w3MinaHotel_72bea01a:
    # mina "Nope! It was just me, Felish, the bounty from our retail therapy, and one big and empty penthouse."
    mina ""

translate id w3MinaHotel_7f8131c5:
    # mc "Sounds like it was fun."
    mc ""

translate id w3MinaHotel_e3f95fd3:
    # mina "It was a lot of wine and bad TV..."
    mina ""

translate id w3MinaHotel_8960ad11:
    # mina "Bless her heart, she doesn't like either of those things."
    mina ""

translate id w3MinaHotel_4bb7b684:
    # mc "She clearly likes you."
    mc ""

translate id w3MinaHotel_0c1efe32:
    # mina "Ehehe~ doesn't she?"
    mina ""

translate id w3MinaHotel_d5d20c9e:
    # "Sitting chest to chest like this, so close to Mina's radiant smile, there was an inevitable pull."
    ""

translate id w3MinaHotel_63027ede:
    # "I wanted to kiss her as I had earlier."
    ""

translate id w3MinaHotel_5ac5fd9a:
    # "I wanted to reach out and grab her."
    ""

translate id w3MinaHotel_48395cfc:
    # "I wanted to--"
    ""

translate id w3MinaHotel_163c31cf:
    # mina "Oh! And speeeeeeeaking of wine..."
    mina ""

translate id w3MinaHotel_8db0a2e3:
    # "Mina reached back, jutting her chest out and securely grasping the nearby wine glass."
    ""

translate id w3MinaHotel_46dcbb73:
    # mina "I left mine aaaaaall the way over there. Hehe, can we...?"
    mina ""

translate id w3MinaHotel_ee984e65:
    # mc "{b}We{/b} can share."
    mc ""

translate id w3MinaHotel_505fbfc6:
    # mina "Ehehe~"
    mina ""

translate id w3MinaHotel_46a4ec63:
    # "Without care, she threw her head back, sending flaxen ribbons of hair flittering and exposing the curve of her neck."
    ""

translate id w3MinaHotel_c1d9d73e:
    # "She knew what she was doing. Her nervous energy had been harnessed, redirected, and put toward a purpose."
    ""

translate id w3MinaHotel_06c9a8cb:
    # "Her eyes fell on me, and the way her lips languidly parted from the glass rim was as loud as any spoken word."
    ""

translate id w3MinaHotel_4aa20bb1:
    # mina "Mmmhh..."
    mina ""

translate id w3MinaHotel_971e4426:
    # "The way she danced between demureness and provocation was natural. Unforced."
    ""

translate id w3MinaHotel_f1c911c4:
    # mina "[mcf]..."
    mina ""

translate id w3MinaHotel_df4c750b:
    # "As if by instinct alone, she was going for blood."
    ""

translate id w3MinaHotel_c08fa780:
    # mina "I kinda just want to squeeze you right now, [mcf]. This is... {b}easy{/b} company. You're very disarming."
    mina ""

translate id w3MinaHotel_c4f36b7e:
    # mina "I mean, I followed you here and crawled right into your lap like some sort of floozy."
    mina ""

translate id w3MinaHotel_e9ced93d:
    # mina "We only just kissed and HERE I am..."
    mina ""

translate id w3MinaHotel_555ca815:
    # mc "At the very least, we're both floozies..."
    mc ""

translate id w3MinaHotel_82c30480:
    # mina "Faaaaaair."
    mina ""

translate id w3MinaHotel_76368000:
    # mc "Does looking at it that way excite you?"
    mc ""

translate id w3MinaHotel_03833b27:
    # mina "{b}Yeeeeep...{/b} heh."
    mina ""

translate id w3MinaHotel_31e9595d:
    # mina "Should guys always be the ones to make a move?"
    mina ""

translate id w3MinaHotel_a55e4384:
    # mc "Not at all. That's fucking stupid."
    mc ""

translate id w3MinaHotel_b41418a4:
    # mina "Good..."
    mina ""

translate id w3MinaHotel_097a3b0f:
    # "Mina and I effortlessly fell into a kiss."
    ""

translate id w3MinaHotel_f601303a:
    # mina "Hmmmwh~"
    mina ""

translate id w3MinaHotel_0e277856:
    # "Soft, not {i}too{/i} aggressive, but an ample declaration nonetheless."
    ""

translate id w3MinaHotel_0fd4d656_6:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_4:
    # "..."
    ""

translate id w3MinaHotel_3c0ef2bf:
    # mina "What are you thinking about right now?"
    mina ""

translate id w3MinaHotel_5e9fe091:
    # mc "......."
    mc ""

translate id w3MinaHotel_2b35b6fe_1:
    # mc "..."
    mc ""

translate id w3MinaHotel_998187e4:
    # mina "H-haa...♥ Hello!"
    mina ""

translate id w3MinaHotel_746d70ab:
    # "I didn't play shy. I grabbed her, {b}like a lover.{/b}"
    ""

translate id w3MinaHotel_cb97528a:
    # mina "I'm glad I didn't have to spell it out..."
    mina ""

translate id w3MinaHotel_3538aeb2:
    # mc "I told you: I'm not thinking a thing. It's all you up here."
    mc ""

translate id w3MinaHotel_7f33297f:
    # mina "You're consistent with your sweet talk, I'll give you that."
    mina ""

translate id w3MinaHotel_266a77e0:
    # mc "You're the most gropeable thing in this room."
    mc ""

translate id w3MinaHotel_2e4bcaf7:
    # mina "Heheh, weeeell... you're in the top two."
    mina ""

translate id w3MinaHotel_21fef109:
    # mc "I'm thinking in 20 minutes, you'll be saying my name and panting like a bitch."
    mc ""

translate id w3MinaHotel_cbfb53b1_5:
    # mina "......"
    mina ""

translate id w3MinaHotel_353f747e:
    # mina "...you starting the timer?"
    mina ""

translate id w3MinaHotel_37f240d4:
    # "Once more, silken floss-like tresses swung arrestingly as the glass remaining contents poured into Mina's mouth."
    ""

translate id w3MinaHotel_139d53a7:
    # "I watched her throat, eagerly awaiting her to swallow and make due on a look that told me she had something to share with me."
    ""

translate id w3MinaHotel_0fd4d656_7:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_5:
    # "..."
    ""

translate id w3MinaHotel_bc5af470:
    # "What she had to share, it turns out, wasn't words. Once more, she made her move, bringing her lips to mine."
    ""

translate id w3MinaHotel_dda25f7c:
    # "This time the kiss wasn't soft. I knew what she intended, and I met her halfway, parting my lips in tandem with hers and accepting the herbaceous and bitter elixir as best I could."
    ""

translate id w3MinaHotel_8a23b6bd:
    # mc "Mmhh-"
    mc ""

translate id w3MinaHotel_a1698a90:
    # "We shared the wine, keeping our lips sealed so as not to spill while our tongues entangled violently and worked against that endeavor."
    ""

translate id w3MinaHotel_e21640a2:
    # mina "Euuhwwh~ *gulp*"
    mina ""

translate id w3MinaHotel_310d47a6:
    # "Eventually, the wine disappeared down our throats, leaving nothing in between us except our lust."
    ""

translate id w3MinaHotel_f071a769:
    # mina "Ha, heheeh... haaa..."
    mina ""

translate id w3MinaHotel_68e82d8c:
    # "Notes of vanilla carried on hot breath reached my nostrils as we parted and stared hazily into each other's eyes, waiting for either's initiative."
    ""

translate id w3MinaHotel_1a46bb08:
    # mina "...saw that in a movie one time."
    mina ""

translate id w3MinaHotel_5ca031ae:
    # mc "That was a complicated maneuver."
    mc ""

translate id w3MinaHotel_a1209306:
    # mina "I know, right? Did I surprise you?"
    mina ""

translate id w3MinaHotel_959dc139:
    # "Mina was {b}full{/b} of surprises. That was for sure."
    ""

translate id w3MinaHotel_03fcc765:
    # mc "Yeah, you were pretty cool just now."
    mc ""

translate id w3MinaHotel_b4ead74d:
    # mina "Hehehehehe~♥ You better believe it."
    mina ""

translate id w3MinaHotel_0fd4d656_8:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_6:
    # "..."
    ""

translate id w3MinaHotel_a8046f97:
    # mina "There is A LOT I want to try with you..."
    mina ""

translate id w3MinaHotel_ab88637b:
    # mina "We're going to have A LOT of fun together..."
    mina ""

translate id w3MinaHotel_6ea3e370:
    # mina "It's time to show you what's in that bag."
    mina ""

translate id w3MinaHotel_79f44070:
    # mc "I don't think I care what's in the bag anymore..."
    mc ""

translate id w3MinaHotel_3564a978:
    # mina "You will... you'll like it... I have {b}ample{/b} confidence in that."
    mina ""

translate id w3MinaHotel_98b031b0:
    # "So Mina darted to the bathroom, leaving me painfully bereft of a cute blonde in my lap."
    ""

translate id w3MinaHotel_d854d353:
    # "In the meantime, I poured myself another glass of wine and waited..."
    ""

translate id w3MinaHotel_02cd4669:
    # mina "Okay, I'm ready! Close your eyes!"
    mina ""

translate id w3MinaHotel_f8127f11:
    # "I didn't have to wait long."
    ""

translate id w3MinaHotel_07f233d4:
    # mc "Come on, I... {i}alright{/i}."
    mc ""

translate id w3MinaHotel_40004d8d:
    # mc "They're closed. Promise."
    mc ""

translate id w3MinaHotel_0fd4d656_9:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_7:
    # "..."
    ""

translate id w3MinaHotel_71a4766e:
    # mina "Umm... you can open them..."
    mina ""

translate id w3MinaHotel_6b75b08a:
    # "I thought she sounded so cute right now, but when I opened my eyes..."
    ""

translate id w3MinaHotel_e8abb1c4:
    # mc "Wh- ah, {b}holy shit...{/b}"
    mc ""

translate id w3MinaHotel_6b1ccb5b:
    # "When I opened my eyes, there stood a man-killer, presenting herself with the complete confidence of a model."
    ""

translate id w3MinaHotel_de52d7f5:
    # mina "Judging by your reaction, I don't need to ask what you think..."
    mina ""

translate id w3MinaHotel_d394bd85:
    # mc "You just had THAT in your car?"
    mc ""

translate id w3MinaHotel_391b29e8:
    # mina "Something I picked up during my shopping trip yesterday..."
    mina ""

translate id w3MinaHotel_ea92194a:
    # mina "I was planning on returning it later, but..."
    mina ""

translate id w3MinaHotel_f5499677:
    # mc "Well, God bless my timing..."
    mc ""

translate id w3MinaHotel_4a45744c:
    # mina "I was hoping it might see some use today."
    mina ""

translate id w3MinaHotel_56e08544:
    # mc "Hope is a beautiful thing..."
    mc ""

translate id w3MinaHotel_4a07819d:
    # mc "Can I see it from behind?"
    mc ""

translate id w3MinaHotel_4e2a8876_4:
    # mina "..."
    mina ""

translate id w3MinaHotel_8712cbff:
    # "She did as I asked, sticking her butt out with just the right amount of unnatural to make me want to immediately leap out of my seat."
    ""

translate id w3MinaHotel_7d20e642:
    # mc "Did Felicia help you pick that out?"
    mc ""

translate id w3MinaHotel_f5682176:
    # mina "She had a hand in it."
    mina ""

translate id w3MinaHotel_446db883:
    # "I made a mental note to thank that wonderful slut."
    ""

translate id w3MinaHotel_55010f20:
    # mina "She did suggest I try a more traditional color like red or black, but..."
    mina ""

translate id w3MinaHotel_5a0be1ec:
    # mc "You'd look good in both those colors, but no... this {i}suits{/i} you."
    mc ""

translate id w3MinaHotel_2f04cb6e:
    # mina "I wear stuff like this for work, but it feels different when there's no camera."
    mina ""

translate id w3MinaHotel_64e0385a:
    # "I knew the feeling... I've seen a lot of beautiful women in expensive clothing recently, but the intimacy and privacy of a hotel room had it feeling a whole lot different."
    ""

translate id w3MinaHotel_03137e9a:
    # mina "Now, shall we pick back up where we left off?"
    mina ""

translate id w3MinaHotel_59af77f8:
    # mc "What the hell are you still doing over there? Get over here."
    mc ""

translate id w3MinaHotel_4e2a8876_5:
    # mina "..."
    mina ""

translate id w3MinaHotel_c03af34d:
    # "She closed the gap with a purposeful slowness that had me on the edge of my seat."
    ""

translate id w3MinaHotel_e6e94ebd:
    # mina "Want a touch?"
    mina ""

translate id w3MinaHotel_7ea6b928:
    # "Goddamn... what a piece of expensive lingerie does to an already impeccable body..."
    ""

translate id w3MinaHotel_bfde99ce:
    # "{i}Yeah, I wanted to touch.{/i}"
    ""

translate id w3MinaHotel_fd2bbd73:
    # mina "O-ohh!"
    mina ""

translate id w3MinaHotel_4f2d1c31:
    # mc "Christ, you're fucking- ah... you don't mind me being...?"
    mc ""

translate id w3MinaHotel_df03e36a:
    # mina "Oh, ah... I don't mind. I like it."
    mina ""

translate id w3MinaHotel_0c10cb9c:
    # mina "Why else would I put this on if not for this kind of reactio--?"
    mina ""

translate id w3MinaHotel_2978252a:
    # mina "You had the same look in your eyes as when we first... ah..."
    mina ""

translate id w3MinaHotel_e400290a:
    # mina "Mmmhh...♥"
    mina ""

translate id w3MinaHotel_54b24c5f:
    # mina "Shall we pick back up where we left off?"
    mina ""

translate id w3MinaHotel_eea25f11:
    # mc "I will have to work hard to live up to how you're making me feel..."
    mc ""

translate id w3MinaHotel_4aab1dce_1:
    # mina "........."
    mina ""

translate id w3MinaHotel_cbfb53b1_6:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876_6:
    # mina "..."
    mina ""

translate id w3MinaHotel_e8b7b8e4:
    # mina "...you can do anything you want with me, [mcf]."
    mina ""

translate id w3MinaHotel_cd60b427:
    # mc "Anything?"
    mc ""

translate id w3MinaHotel_26c00eea:
    # mina "Like I said, I'm putting my trust in you, okay?"
    mina ""

translate id w3MinaHotel_aeb2bb08:
    # mc "Mina..."
    mc ""

translate id w3MinaHotel_a21fd75e:
    # mina "Can I ask you something weird? Do you have any {i}fantasies{/i}?"
    mina ""

translate id w3MinaHotel_3b9b299e:
    # mc "Everyone does I would think..."
    mc ""

translate id w3MinaHotel_f56b9271:
    # mina "A lot of them?"
    mina ""

translate id w3MinaHotel_9c1105c8:
    # mc "I've never counted... and I don't really know what's an abnormal amount..."
    mc ""

translate id w3MinaHotel_5c1ba3bd:
    # mina "Weeeeell... I've got an abnormal amount of them. {b}Tons{/b}."
    mina ""

translate id w3MinaHotel_88d2349c:
    # mina "Maybe even a disgusting amount..."
    mina ""

translate id w3MinaHotel_4df7efd4:
    # mc "I don't think fantasies are anything to fret over. What makes them disgusting?"
    mc ""

translate id w3MinaHotel_02437a34:
    # mina "Nothing, I guess... except... ah..."
    mina ""

translate id w3MinaHotel_9513d7ce:
    # mina "Would you share one of yours with me?"
    mina ""

translate id w3MinaHotel_21b862bd:
    # "It's funny. This was an area I've given little thought to, even though, in theory, the club could make any of my imaginations a reality..."
    ""

translate id w3MinaHotel_3f36ff10:
    # mc "...huh, are {i}you{/i} gonna make my fantasies come true?"
    mc ""

translate id w3MinaHotel_ae8eb1f6:
    # mina "Hehe, I don't know... maybe? I {b}cooooould...{/b}"
    mina ""

translate id w3MinaHotel_84fa9775:
    # "The problem with being truthful was that telling her I'd like to fuck her in the ass while shoving her pretty face in a toilet wasn't keeping with the current mood..."
    ""

translate id w3MinaHotel_7e9d20df:
    # "Probably wouldn't ever BE the mood, but I got the impression right now that she thought she was genuinely curious and open to my tastes."
    ""

translate id w3MinaHotel_8b717a24:
    # mc "Why don't we just go with the flow? No need to complicate it."
    mc ""

translate id w3MinaHotel_fb9b0766:
    # mina "True, it's just... I'm curious about what other people like. I don't have many points of comparison, y'know?"
    mina ""

translate id w3MinaHotel_e61f179d:
    # mc "Got any requests?"
    mc ""

translate id w3MinaHotel_6bf5c0d8:
    # mina "{b}Tons{/b}... but today, how about you... {i}just do anything you want?{/i}?"
    mina ""

translate id w3MinaHotel_312a0e10:
    # mc "Anything? What about your list?"
    mc ""

translate id w3MinaHotel_36034b1f:
    # mina "This is our first {b}real{/b} time; let's avoid getting bogged down with details..."
    mina ""

translate id w3MinaHotel_b0ffe668:
    # mina "If anything, I want to get a taste of who {b}you{/b} are."
    mina ""

translate id w3MinaHotel_bd285078:
    # mc "Who am I?"
    mc ""

translate id w3MinaHotel_2e24a11c:
    # mct "(I'm someone who wants to fuck the shit out of you right now...)"
    mct ""

translate id w3MinaHotel_3fdb0a39:
    # mina "You know, what kind of stuff do you like? What would you like to... {i}do{/i}?"
    mina ""

translate id w3MinaHotel_95a9bdb9:
    # mc "I don't want for much right now."
    mc ""

translate id w3MinaHotel_cdf2c644:
    # mina "But... you have fantasies like I do, right? Things you might want to try? Helping me with my junk..."
    mina ""

translate id w3MinaHotel_2516c733:
    # mina "I was thinking, {i}this{/i} doesn't have to be so one-sided."
    mina ""

translate id w3MinaHotel_2f2ce452:
    # mc "Sure, everyone's got fantasies, but... I try not to dwell on them too much."
    mc ""

translate id w3MinaHotel_75e5a0fa:
    # mina "I've told you some of mine..."
    mina ""

translate id w3MinaHotel_1ed89ac1:
    # "The problem with being truthful was that telling her I'd like to fuck her in the ass while shoving her pretty face in a toilet wasn't the current mood..."
    ""

translate id w3MinaHotel_43d5de5c:
    # "Probably wouldn't ever BE the mood, but I got the impression right now that she might truly meet me halfway if I only asked."
    ""

translate id w3MinaHotel_7a62a129:
    # mina "I'd at least like to hear about some of them... you know how much of a freak I am, and... I don't have many points of comparison, y'know?"
    mina ""

translate id w3MinaHotel_29faaaa1:
    # "Playing out my desires with the club was one thing, but having someone my age... happy and open.... it was a {b}tempting{/b} fantasy that was better saved for future consideration."
    ""

translate id w3MinaHotel_fe7fbc3a:
    # mct "(What is something relatively benign I could share with her that wouldn't make things overly weird...?)"
    mct ""

translate id w3MinaHotel_45fdc9db:
    # "I mean, I could come up with a lie, but the earnest expression on her face MADE me want to be truthful. "
    ""

translate id w3MinaHotel_0fd4d656_10:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_8:
    # "..."
    ""

translate id w3MinaHotel_98a6a25d:
    # mc "Let's put a pin in that thought for now."
    mc ""

translate id w3MinaHotel_376daa51:
    # "I HAD what I thought to be an answer to her question, but the explanation was best worked up to."
    ""

translate id w3MinaHotel_f0a08184:
    # mc "Instead..."
    mc ""

translate id w3MinaHotel_1ba52fad:
    # "Plus, eating Mina up {i}too{/i} fast would be a shame. Some meals are best savored."
    ""

translate id w3MinaHotel_bcbcf275:
    # mc "Come over here, sweetheart."
    mc ""

translate id w3MinaHotel_b1dd231c:
    # "I directed Mina to the table and had her sit as if putting her on display."
    ""

translate id w3MinaHotel_062f37b0:
    # "We momentarily shared an exploratory look as I considered what part of her I wanted to taste first."
    ""

translate id w3MinaHotel_2184f72a:
    # mina "[mcf]...? Could you...?"
    mina ""

translate id w3MinaHotel_7d1e372c:
    # "It seemed Mina's mind had gone through a similar process."
    ""

translate id w3MinaHotel_d84560ce:
    # mina "Could you take off your pants... I want to..."
    mina ""

translate id w3MinaHotel_cdadd76e:
    # mina "I want to see it..."
    mina ""

translate id w3MinaHotel_a5bfb73a:
    # mina "I want to see it again..."
    mina ""

translate id w3MinaHotel_8619c674:
    # "Now, it was my time to be put on display and gawked at."
    ""

translate id w3MinaHotel_cacc48cc:
    # mc "Should I do it slowly?"
    mc ""

translate id w3MinaHotel_5a913bf0:
    # "No matter... they had to come off at some point."
    ""

translate id w3MinaHotel_dd902dd6:
    # mina "J-just... take your {i}dick{/i} out."
    mina ""

translate id w3MinaHotel_3f5a6638:
    # "There was a deliberate emphasis that she knew fully well betrayed her girlishly coy cadence."
    ""

translate id w3MinaHotel_e119df09:
    # mc "As you wish."
    mc ""

translate id w3MinaHotel_8eaa4976:
    # "In no time, [mcf] had disappeared from her view. I no longer existed."
    ""

translate id w3MinaHotel_cbfb53b1_7:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876_7:
    # mina "..."
    mina ""

translate id w3MinaHotel_214cca3a:
    # "She wasn't looking at me so much as what stood proudly at half-mast."
    ""

translate id w3MinaHotel_4cb39f01:
    # mina "It can get bigger, right? Could you... I mean, I want to see..."
    mina ""

translate id w3MinaHotel_c74928b2:
    # mina "I know it's bigger than that... Could you... I mean, I want to see..."
    mina ""

translate id w3MinaHotel_e29044e4:
    # mina "{b}Stroke it for me.{/b}"
    mina ""

translate id w3MinaHotel_e7fd3cdf:
    # "Again, she oscillated between shyly choosing her words and a more forceful command. I didn't know if she intended for the contrast to turn me on or if she was simply feeling out her own feelings, but the former was the end result."
    ""

translate id w3MinaHotel_c0e6c319:
    # mc "What? You like watching men jerk off?"
    mc ""

translate id w3MinaHotel_038184ab:
    # mina "Y-yeah, it's... {b}hot{/b}."
    mina ""

translate id w3MinaHotel_fd168a05:
    # mc "Is this one of those fantasies you mentioned?"
    mc ""

translate id w3MinaHotel_9e305e7c:
    # mc "Is this one of your things?"
    mc ""

translate id w3MinaHotel_641bcc66:
    # "She just nodded, too transfixed on my cock to say more than she had to."
    ""

translate id w3MinaHotel_1a74043a:
    # mina "Mmmhh... actually, more fetish than fantasy?"
    mina ""

translate id w3MinaHotel_f0956aaf:
    # mc "What do you like about it?"
    mc ""

translate id w3MinaHotel_fc89e262:
    # mina "*Gulp* It's just so... {i}animalistic?{/i}"
    mina ""

translate id w3MinaHotel_99df64b0:
    # mina "You're going to be a doctor one day, [mcf]. You'll help people, but right now, you have a look in your eye that's so far removed from decent. I {b}love{/b} it..."
    mina ""

translate id w3MinaHotel_d3f75e39:
    # mc "Mina, like... {b}damn{/b}."
    mc ""

translate id w3MinaHotel_6fe3fe74:
    # mina "Ahhhh, I'm w-weird, aren't I?"
    mina ""

translate id w3MinaHotel_9caee9d9:
    # mc "You're not weird; all sex is weird... you just know what you like."
    mc ""

translate id w3MinaHotel_afeceac2:
    # "How she could articulate the underpinning of her desires surprised and impressed me."
    ""

translate id w3MinaHotel_62816eb4:
    # mc "The world would be better if everyone could be honest with themselves and find a healthy outlet."
    mc ""

translate id w3MinaHotel_7a941ef0:
    # mina "Uyyhh... mmmhuh...♥"
    mina ""

translate id w3MinaHotel_bde42596:
    # "It seems I had lost her to whatever deliberations were happening inside her head."
    ""

translate id w3MinaHotel_9b136879:
    # mina "When did you-"
    mina ""

translate id w3MinaHotel_a10dafa9:
    # "Poor Mina was so caught up on me jacking off that she didn't even register me inching closer. The spell was only broken when my hand left my cock."
    ""

translate id w3MinaHotel_85bad438:
    # mc "You were REALLY into that..."
    mc ""

translate id w3MinaHotel_131fea6d:
    # mina "Hnnngg... don't-"
    mina ""

translate id w3MinaHotel_7489f40c:
    # mc "And why not? From where I stood, the look in your eyes was equally as fuck hungry as the one you saw in mine."
    mc ""

translate id w3MinaHotel_f705158d:
    # mc "Don't be embarrassed about what you like."
    mc ""

translate id w3MinaHotel_dde04850:
    # mina "Share with me one of your fantasies and maybe I won't be..."
    mina ""

translate id w3MinaHotel_5ae1f9c3:
    # mc "It's that important to you, huh?"
    mc ""

translate id w3MinaHotel_a65f7568:
    # mina "Absolutely! What's more intimate than knowing what the other person gets off to?"
    mina ""

translate id w3MinaHotel_1f05051f:
    # mc "Don't worry. I'm not avoiding your question."
    mc ""

translate id w3MinaHotel_332546be:
    # mina "O-ohh..."
    mina ""

translate id w3MinaHotel_0aba0d75:
    # mc "I have a {b}show and tell{/b} in mind. But first..."
    mc ""

translate id w3MinaHotel_b1439453:
    # mina "{b}O-ooooooh!{/b}"
    mina ""

translate id w3MinaHotel_2e83ebab:
    # "I mean, {i}how could I {b}fucking{/b} not?{/i} The pull of the fabric made Mina's already beautiful chest even more delectable. "
    ""

translate id w3MinaHotel_6f6aae3f:
    # mc "Let's discover just how sensitive you are here."
    mc ""

translate id w3MinaHotel_ca906ffc:
    # mc "Let's make you sing."
    mc ""

translate id w3MinaHotel_28cdcd32:
    # "My hands possessed a will of their own, gripping the underside of the blonde's bust, teasing and massaging the mounds to create an eye-pleasing feast."
    ""

translate id w3MinaHotel_acabeb85:
    # mina "Hnn- ahh... aren't we past this?"
    mina ""

translate id w3MinaHotel_b2b9e19d:
    # mc "What do you mean?"
    mc ""

translate id w3MinaHotel_f9c05bdf:
    # mina "You c-can just put it in if you want... y-you're so hard, it's painful for a guy to hold back, right?"
    mina ""

translate id w3MinaHotel_35759e8f:
    # "I stopped myself short of asking who told her that. Maybe that was something invented in her own head, but more realistically, I could see that being a line Ian fed her."
    ""

translate id w3MinaHotel_1fb488ca:
    # mc "Painful, no. This is one of my {b}favorite{/b} parts."
    mc ""

translate id w3MinaHotel_ea4a819d:
    # mct "(God, the sight of these things!)"
    mct ""

translate id w3MinaHotel_dc9bc548:
    # mc "It's like when you slowly crawl to the top of a roller coaster, and you get that funny feeling in the pit of your stomach."
    mc ""

translate id w3MinaHotel_8ac2353c:
    # mina "Hhh- i-is it?"
    mina ""

translate id w3MinaHotel_6de08c1f:
    # mc "It is. The climb, the anticipation, is just as important as the fall."
    mc ""

translate id w3MinaHotel_7d382bc1:
    # mina "Roller coasters are scary..."
    mina ""

translate id w3MinaHotel_79e801da:
    # mc "Maybe, but... the ride to the top gives you time to think. Lets you come to terms with the inevitable."
    mc ""

translate id w3MinaHotel_8e987d39:
    # mc "In this case, that being... you're gonna get {b}fucked{/b}, Mina. So, go ahead and think about that for a bit."
    mc ""

translate id w3MinaHotel_f33ccbee:
    # mct "(Aha!)"
    mct ""

translate id w3MinaHotel_4bb76819:
    # mina "Eeeh, eeh?"
    mina ""

translate id w3MinaHotel_f6bbe5ad:
    # "That taste I was looking for before I got sidetracked."
    ""

translate id w3MinaHotel_40a67721:
    # "Mina's nipples struck a wonderful balance between puffy and hard, the perfect morsel to latch onto and gratify my own personal fixation."
    ""

translate id w3MinaHotel_cf244415:
    # mina "Ah, wwhh--"
    mina ""

translate id w3MinaHotel_69d1ac79:
    # "Something in me compelled me to {i}devour{/i} these things."
    ""

translate id w3MinaHotel_7ae3e1ea:
    # "I felt just as compelled to devour these things as if it was my first time sucking on these beauties."
    ""

translate id w3MinaHotel_d8e9a0df:
    # mina "S-sooo... ahhh..."
    mina ""

translate id w3MinaHotel_806ed389:
    # "On the ride to the top, a thought occurred to me."
    ""

translate id w3MinaHotel_40346ed0:
    # mina "Alright... ha... t-thinking about it..."
    mina ""

translate id w3MinaHotel_b1fc57fc:
    # "Mina was like a canvas, and I felt the tremendous weight of knowing I would add my own touch to this painting."
    ""

translate id w3MinaHotel_fb9fbfbf:
    # mina "Hnng- wwhhha, do whatever you want..."
    mina ""

translate id w3MinaHotel_6340d94d:
    # "Would I paint over some of what Ian added? Who knew, but it might be fun to try."
    ""

translate id w3MinaHotel_8b68396e:
    # mina "I-if you like my breasts that much, h-have your f-fill..."
    mina ""

translate id w3MinaHotel_b4f92912:
    # "Would the things I add be for better or worse? Beats the fuck out of me."
    ""

translate id w3MinaHotel_b3d73720:
    # mina "I'll learn all about y-you while you d-do-"
    mina ""

translate id w3MinaHotel_97fa368f:
    # "Would a little of her rub off onto me?"
    ""

translate id w3MinaHotel_3e6694bd:
    # mina "E-eehh?"
    mina ""

translate id w3MinaHotel_c65213ac:
    # "I hoped so."
    ""

translate id w3MinaHotel_1f1f8685:
    # mina "Oh, hhhnn-"
    mina ""

translate id w3MinaHotel_9223d266:
    # "My exploratory taste confirmed what I already suspected to be the case: Mina was sufficiently tuned up and waiting for a cock."
    ""

translate id w3MinaHotel_628bf878:
    # mina "Hyyeee-♥ [mcf]!"
    mina ""

translate id w3MinaHotel_17d07e5f:
    # "I pried Mina apart with my tongue and explored her insides. This would serve as a nice appetizer."
    ""

translate id w3MinaHotel_8d56ff1b:
    # "I didn't plan on a repeat of the other day. This was just an appetizer."
    ""

translate id w3MinaHotel_a009be97:
    # mina "G-give me a warning before you, o-ohhh-"
    mina ""

translate id w3MinaHotel_2960aa50:
    # "As I shot up and angled my penis toward her opening..."
    ""

translate id w3MinaHotel_5d5230f6:
    # mina "Oohwwooh- ahh.. hweh..."
    mina ""

translate id w3MinaHotel_b4ea88bc:
    # "...Mina's words stuck in her throat, waiting for the shoe to drop."
    ""

translate id w3MinaHotel_9b152712:
    # mina "......."
    mina ""

translate id w3MinaHotel_4e2a8876_8:
    # mina "..."
    mina ""

translate id w3MinaHotel_0081e210:
    # mina "*Gulp* W-what are you doing?"
    mina ""

translate id w3MinaHotel_f66d81c8:
    # "Hovering this close, there was a very tempting prospect to simply put it in, but I couldn't fight my sadistic streak."
    ""

translate id w3MinaHotel_7827fa1f:
    # "Let's hold out, just a little while longer..."
    ""

translate id w3MinaHotel_c9fda89f:
    # "Why use words when you can say a lot more with action?"
    ""

translate id w3MinaHotel_69617eb8:
    # mina "Euug-!"
    mina ""

translate id w3MinaHotel_355fc1cf:
    # "As the bottom of my shaft came in contact with her sweet parts, the breath Mina held in abruptly escaped her throat."
    ""

translate id w3MinaHotel_09a51665:
    # mina "Hnngg... ehhh... "
    mina ""

translate id w3MinaHotel_41f09ac7:
    # "The tip of my cock agonizingly poked through the cleft of her sex, prying apart and teasing Mina's sex."
    ""

translate id w3MinaHotel_d15219a3:
    # mina "You're still on this t-teasing thing, huh?"
    mina ""

translate id w3MinaHotel_5070bc5f:
    # "{i}This{/i} approach was a double-edged sword."
    ""

translate id w3MinaHotel_3bcd0f88:
    # "Every time the underside of my shaft would meander over her precipice, a warmness spread down my length and all the way to my nuts."
    ""

translate id w3MinaHotel_a5192531:
    # "Every time my head bumbled into her ridges, a violent, tingling feeling shot up my body."
    ""

translate id w3MinaHotel_dc9d0d0d:
    # "Still, standing over her like this, I could drink in every bit of her frustrated expression. I could bask in the foreknowledge of how tight she would be once we finally joined."
    ""

translate id w3MinaHotel_533a177a:
    # mina "Or... eheh... d-do you not know how to do it?"
    mina ""

translate id w3MinaHotel_ebba41d5:
    # "I wondered... what would her face look like when I entered her for the first time?"
    ""

translate id w3MinaHotel_249b94b0:
    # "Would it be on me? Would she carefully observe where we were connected?"
    ""

translate id w3MinaHotel_275f7603:
    # mina "Hnnngg... ggahh... c'mon [mcf]..."
    mina ""

translate id w3MinaHotel_80ef784b:
    # "What would her expression be like? How much of me would she take before that pretty face of hers went askew?"
    ""

translate id w3MinaHotel_e19805d9:
    # "How deep would my cock need to reach for me to rend all thoughts from her head? How fast would I need to go before her eyes rolled back in her head?"
    ""

translate id w3MinaHotel_1f130c14:
    # mina "Stop being meeeeean... ah, pleeeease?"
    mina ""

translate id w3MinaHotel_551633f9:
    # "The future was full of lewd possibilities."
    ""

translate id w3MinaHotel_1ff8fd27:
    # "I nodded, signaling to her what was to come."
    ""

translate id w3MinaHotel_14324ddb:
    # "Mina smiled back, signaling to me that she was ready."
    ""

translate id w3MinaHotel_0474e22e:
    # mc "You want it?"
    mc ""

translate id w3MinaHotel_69617eb8_1:
    # mina "Euug-!"
    mina ""

translate id w3MinaHotel_355fc1cf_1:
    # "As the bottom of my shaft came in contact with her sweet parts, the breath Mina held in abruptly escaped her throat."
    ""

translate id w3MinaHotel_fce07294:
    # mina "Hnngg... ehhh... I'm r-ready... ah..."
    mina ""

translate id w3MinaHotel_08040056:
    # "The tip of my cock agonizingly poked through the cleft of her sex."
    ""

translate id w3MinaHotel_336afb29:
    # mc "Your little pussy is going to be a tight fit for my cock."
    mc ""

translate id w3MinaHotel_3d34ca45:
    # mina "A-ahh...♥"
    mina ""

translate id w3MinaHotel_d8f7d85d:
    # mc "You think you can handle it?"
    mc ""

translate id w3MinaHotel_ffc70ede:
    # mina "I m-mean, ha..."
    mina ""

translate id w3MinaHotel_f49f4d50:
    # "*Fwap!*"
    ""

translate id w3MinaHotel_341c7e32:
    # mina "O-ohh-!"
    mina ""

translate id w3MinaHotel_25d407be:
    # mina "Hnnng... that's what it does... it's made to take.. a-ah~"
    mina ""

translate id w3MinaHotel_9281cf93:
    # mc "{b}What{/b} is it made to take?"
    mc ""

translate id w3MinaHotel_4aad114e:
    # mina "C-c'mon, [mcf]..."
    mina ""

translate id w3MinaHotel_a9be64ff:
    # mc "What is your pussy made for, Mina?"
    mc ""

translate id w3MinaHotel_13c7833c:
    # mina "Ah... euuug..."
    mina ""

translate id w3MinaHotel_ca1a77ef:
    # "She looked at me with a complete understanding of what I was getting at."
    ""

translate id w3MinaHotel_f043ffb9:
    # mina "It's made to take {i}cock{/i}."
    mina ""

translate id w3MinaHotel_114aa989:
    # "Mina didn't dance around it quite as much as I had hoped."
    ""

translate id w3MinaHotel_404ff6ac:
    # mc "I'm going to put it in now."
    mc ""

translate id w3MinaHotel_f708df6c:
    # mina "Alright, but... b-be gentle, okay?"
    mina ""

translate id w3MinaHotel_7a77b33b:
    # "She asked with such a cute look..."
    ""

translate id w3MinaHotel_9bd2927d:
    # mina "Your thing is {i}huuuuuuuuge{/i}. Do it slowly, at least at first..."
    mina ""

translate id w3MinaHotel_9ea7aa32:
    # "She said, perhaps purposefully inflating my ego for her cause."
    ""

translate id w3MinaHotel_1f56feb7:
    # mc "Ready?"
    mc ""

translate id w3MinaHotel_f23cff43:
    # mina "R-ready..."
    mina ""

translate id w3MinaHotel_3928a113:
    # "......."
    ""

translate id w3MinaHotel_a20cefa7_9:
    # "..."
    ""

translate id w3MinaHotel_00e80e09:
    # mina "Hh-"
    mina ""

translate id w3MinaHotel_84223a2b:
    # "Slowly, I slid the head of my penis into Mina's welcoming embrace."
    ""

translate id w3MinaHotel_1a842516:
    # mina "Hh-hat...!"
    mina ""

translate id w3MinaHotel_58874a22:
    # "I held myself in place, my eyes never leaving hers, letting her know I was intently aware of what she was feeling."
    ""

translate id w3MinaHotel_0ea03c1b:
    # mina "Deeper..."
    mina ""

translate id w3MinaHotel_f1c44085:
    # "Deeper I went, one-third of the way down my shaft."
    ""

translate id w3MinaHotel_6db363f8:
    # mina "It's t-thick...♥"
    mina ""

translate id w3MinaHotel_45f46d5d:
    # "I stopped once more, holding for a second, enjoying the warmness of Mina's insides clinging sweetly to my shaft."
    ""

translate id w3MinaHotel_adc54e7b:
    # mina "You're pulling me apart, [mcf]..."
    mina ""

translate id w3MinaHotel_d23b92d2:
    # mina "Achk, h-hard...♥ You're so...!"
    mina ""

translate id w3MinaHotel_eb9b37a2:
    # "Halfway."
    ""

translate id w3MinaHotel_4e2a8876_9:
    # mina "..."
    mina ""

translate id w3MinaHotel_427cb25c:
    # "Mina kept her eyes glued to me, the longing expression on her face never wavering. A look that she was very intentionally making known."
    ""

translate id w3MinaHotel_c34e5645:
    # "{i}She was reassuring me that she wanted me.{/i}"
    ""

translate id w3MinaHotel_d33bf6c3:
    # mina "{b}Deeper{/b}."
    mina ""

translate id w3MinaHotel_55feb8f4:
    # mc "--!"
    mc ""

translate id w3MinaHotel_8b54b31b:
    # "One final push, with more strength than I intended, emboldened by Mina's impassioned demand."
    ""

translate id w3MinaHotel_27d896c8:
    # mina "Mmhh, aaaaaaahh-♥♥♥♥♥"
    mina ""

translate id w3MinaHotel_77476704:
    # mina "Y-you're in! F-finally...! Aaaah- {i}FUCK!{/i} Ha...!"
    mina ""

translate id w3MinaHotel_fb52b93e:
    # "Instead of reeling back, Mina clung to me, keeping me buried at the base."
    ""

translate id w3MinaHotel_5e7efb58:
    # "An intense wave of pleasure washed over me and things quickly became hazy."
    ""

translate id w3MinaHotel_18a4411c:
    # mina "You're in... all of you... ha... s-so much of you... ah...♥"
    mina ""

translate id w3MinaHotel_648a4389:
    # "Mina felt so good, her body was so soft, she smiled so lovelily..."
    ""

translate id w3MinaHotel_234933be:
    # mina "D-don't move yet, okay...?"
    mina ""

translate id w3MinaHotel_e7c364a0:
    # "With the way she held me, I could feel her heartbeat."
    ""

translate id w3MinaHotel_ea078830:
    # mina "Haa..."
    mina ""

translate id w3MinaHotel_6b99918a:
    # "I wanted to move, but at the same time, being joined like this... chest-to-chest... {b}felt so secure.{/b}"
    ""

translate id w3MinaHotel_9d3df431:
    # "Even in a situation like this, she had an uncanny knack for putting others at ease."
    ""

translate id w3MinaHotel_9dde6b30:
    # "Gently I laid her down on the table's surface, withdrawing a little and giving her a moment to adjust to my size."
    ""

translate id w3MinaHotel_0fd4d656_11:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_10:
    # "..."
    ""

translate id w3MinaHotel_fd011907:
    # "After a moment, she gave me a nod and..."
    ""

translate id w3MinaHotel_4712bbd3:
    # "So it began, with me gently pushing into Mina's constraining depths."
    ""

translate id w3MinaHotel_cd6dd9fe:
    # mina "Haa...♥"
    mina ""

translate id w3MinaHotel_463555f2:
    # "Mina was no virgin, but there was a tempo to these things."
    ""

translate id w3MinaHotel_700e6051:
    # mc "You alright?"
    mc ""

translate id w3MinaHotel_9c3b8015:
    # mina "Y-yeeahh... you're... {b}heh...♥{/b}"
    mina ""

translate id w3MinaHotel_f5fde511:
    # "Pulling out from her grip took effort."
    ""

translate id w3MinaHotel_3cff1de1:
    # mina "My heart jumped into my throat when you went all the way in..."
    mina ""

translate id w3MinaHotel_caa3d0a4:
    # "Pushing back into her hold took strength."
    ""

translate id w3MinaHotel_48d196a1:
    # mina "You're so big, hard, and thick...! God...!"
    mina ""

translate id w3MinaHotel_2de70f54:
    # "The table-fucked blonde's hold on me was incredible."
    ""

translate id w3MinaHotel_7a7deeff:
    # mina "Y-yeah, I'm alright..."
    mina ""

translate id w3MinaHotel_67a6b016:
    # "Thankfully, Mina had a good sense of rhythm. She matched my pace and timing, joining with me at the height of every thrust."
    ""

translate id w3MinaHotel_e079dae2:
    # mina "I'm glad I'm doing this with {i}you{/i}, [mcf]."
    mina ""

translate id w3MinaHotel_f1826d56:
    # mina "Even now, you're being patient... ahh...♥"
    mina ""

translate id w3MinaHotel_f1970df0:
    # "Mina's usual exuberance carried on even here, with a tremendously playful smile plastered on her face."
    ""

translate id w3MinaHotel_4a9552e8:
    # mc "It's just... you're so damn tight... ha, if I moved any faster than this, I would explode."
    mc ""

translate id w3MinaHotel_5cef5f73:
    # mina "Ehh... ha...♥ Hehe... why do people ever do anything but {b}fuck{/b}?"
    mina ""

translate id w3MinaHotel_eb6e52da:
    # "Her smile gave me a sense of overwhelming {i}togetherness{/i}, but it was also something I'd like to see messed up."
    ""

translate id w3MinaHotel_e1d1439c:
    # mc "You're something else, you know that?"
    mc ""

translate id w3MinaHotel_94bb686d:
    # "It would be lovely to see that smile unfurl in pleasure."
    ""

translate id w3MinaHotel_dccbf0a6:
    # mct "(But, I guess you could say the same about me.)"
    mct ""

translate id w3MinaHotel_ea61e618:
    # mina "Hehe~"
    mina ""

translate id w3MinaHotel_07bdde05:
    # "....."
    ""

translate id w3MinaHotel_a20cefa7_11:
    # "..."
    ""

translate id w3MinaHotel_c901b242:
    # "Our chatter abated, and the conversation continued with our eyes."
    ""

translate id w3MinaHotel_d4e7bb47:
    # "She gifted me her rapt attention, expression brimming with eagerness and hunger."
    ""

translate id w3MinaHotel_c6fb6774:
    # "Through a hazy stare, she told me she wanted more."
    ""

translate id w3MinaHotel_af20d4b8:
    # "Her lips parted but carried no words - only breathy affirmations that conveyed to me {i}don't stop{/i}."
    ""

translate id w3MinaHotel_72c56f84:
    # "Every time I pushed {i}deep{/i} into her, there was a flicker of satisfaction that would fast die on the way back out."
    ""

translate id w3MinaHotel_7e6f5c47:
    # "It was a small drama, one that played out over and over in time with our rhythm, but each performance was as fresh as the last."
    ""

translate id w3MinaHotel_5f283c24:
    # "Each rendition of Mina's sultry expression aggravated my desire."
    ""

translate id w3MinaHotel_e8faf77c:
    # "Each dive into Mina's lust-filled eyes propelled my hips forward with growing emotion. "
    ""

translate id w3MinaHotel_fd390903:
    # "Every apsect of Mina's visage carried a mandate to leave her well and truly fucked."
    ""

translate id w3MinaHotel_36db9a59:
    # mc "Hnn, M-mina...! So, you wanted to hear a fantasy of mine?"
    mc ""

translate id w3MinaHotel_6bd603dd:
    # mina "Of c-course! Tell me! Let's do it!"
    mina ""

translate id w3MinaHotel_3a33db10:
    # mc "You haven't heard it yet!"
    mc ""

translate id w3MinaHotel_cadd5221:
    # mina "I don't care! Let's do whatever it is!"
    mina ""

translate id w3MinaHotel_c9a77392:
    # mc "You're out of your mind!"
    mc ""

translate id w3MinaHotel_a37ce85d:
    # mina "M-maybe...♥ I want to make this special for you, [mcf]!"
    mina ""

translate id w3MinaHotel_0fd4d656_12:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_12:
    # "..."
    ""

translate id w3MinaHotel_9a12049d:
    # "Mina, with the city as her backdrop, was like a dream."
    ""

translate id w3MinaHotel_9bace233:
    # mc "The window!"
    mc ""

translate id w3MinaHotel_e8960c5b:
    # mina "The window...? What about it?"
    mina ""

translate id w3MinaHotel_94cdfd9f:
    # mc "Doesn't it appeal to you?"
    mc ""

translate id w3MinaHotel_cbfb53b1_8:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876_10:
    # mina "..."
    mina ""

translate id w3MinaHotel_69accb51:
    # mina "We have a good view of the city from here, don't we?"
    mina ""

translate id w3MinaHotel_8be17f56:
    # mc "Thirty-four floors, to be precise. I see you're thinking what I'm thinking."
    mc ""

translate id w3MinaHotel_cd0f1d8a:
    # mina "Kiiiiinda?"
    mina ""

translate id w3MinaHotel_50d94d78:
    # "It was a simple and innocuous choice, one fitting for the moment."
    ""

translate id w3MinaHotel_1aed4931:
    # "If she was honest about wanting to explore our fantasies together, I wanted to start with a simple and safe one."
    ""

translate id w3MinaHotel_950b95d0:
    # mina "A-aahh...! C-cold~!"
    mina ""

translate id w3MinaHotel_45eafc2a:
    # "I pressed Mina hard against the window pane to make my point."
    ""

translate id w3MinaHotel_c161bb7e:
    # mc "You know, I've lived my whole life and never really been that far from another person. I've never had a mile between me and any other human before."
    mc ""

translate id w3MinaHotel_4e0f92fd:
    # mc "Kinda fucking crazy when you really think about it..."
    mc ""

translate id w3MinaHotel_20390e31:
    # "Mina's response to that was to wiggle her ass. A {i}polite{/i} way of telling someone to get back to work if I've ever experienced one."
    ""

translate id w3MinaHotel_e97e4ca3:
    # mina "Hhha-♥"
    mina ""

translate id w3MinaHotel_f8ab75c0:
    # mc "You ever look up and imagine all the little dramas playing out in the buildings around you?"
    mc ""

translate id w3MinaHotel_d77a6a67:
    # "Slowly, I picked up back where we left off, looking to set the scene."
    ""

translate id w3MinaHotel_1db6ac5d:
    # mc "Joyous celebrations, babies being born, dreams being fulfilled..."
    mc ""

translate id w3MinaHotel_fb8a001f:
    # mina "Hnnng...♥ Ah, the angle you're pushh- w-woah...♥"
    mina ""

translate id w3MinaHotel_5be1eefb:
    # mc "Spouses fighting, lovers quarreling, people having the worst days of their lives..."
    mc ""

translate id w3MinaHotel_52c4825f:
    # mina "Ha-♥ You c-can go faster if you want!"
    mina ""

translate id w3MinaHotel_e0cba23a:
    # "I ignored her request."
    ""

translate id w3MinaHotel_7540d8c9:
    # mc "You ever look up and imagine all that?"
    mc ""

translate id w3MinaHotel_ae82a3de:
    # mina "H-haaat-♥ Hnng, mmmh-♥ F-faster!"
    mina ""

translate id w3MinaHotel_dc1f7512:
    # mc "I do. There's so much happening all around us that it can be suffocating to think about."
    mc ""

translate id w3MinaHotel_34a595ba:
    # "I ignored her request a second time."
    ""

translate id w3MinaHotel_15f97a3c:
    # mc "The city's teeming with life, but you tend to forget that once you're locked away behind some walls with some soundproofing."
    mc ""

translate id w3MinaHotel_2ef8965b:
    # mina "Fhh, [mcf] faster!"
    mina ""

translate id w3MinaHotel_12a098d9:
    # mc "Isn't it hot sitting up high, peering down on all those meaningful moments?"
    mc ""

translate id w3MinaHotel_347cf8fd:
    # mina "Hha, hhhaa I g-guess? The city looks nice from up here..."
    mina ""

translate id w3MinaHotel_0649f15f:
    # mc "Doesn't it turn you on to think about the people going about their daily lives as you tower above them, a dick buried in your twat?"
    mc ""

translate id w3MinaHotel_5da2c62a:
    # mina "Mmmh, when you put it t-that way..."
    mina ""

translate id w3MinaHotel_3aabbec1:
    # mc "None of that matters. None of {i}them{/o} matter."
    mc ""

translate id w3MinaHotel_79f8a943:
    # mina "Ha, hhh-♥"
    mina ""

translate id w3MinaHotel_01f4d852:
    # mc "There's just the sensation of being {b}above{/b} it all, getting fucked. Isn't it kind of a thrill?"
    mc ""

translate id w3MinaHotel_e07c6a57:
    # mina "Y-yeeah! It feels gooood!"
    mina ""

translate id w3MinaHotel_099a4207:
    # mc "It does for me too. It turns me on to imagine I'm lording above all that shit, balls deep in a woman of their fantasies."
    mc ""

translate id w3MinaHotel_7cbe14b3:
    # mina "Huuah, hhaaa...! W-w-heh?"
    mina ""

translate id w3MinaHotel_eab54b76:
    # mc "God, I'm the luckiest man alive right now!"
    mc ""

translate id w3MinaHotel_2d34b145:
    # "To signify my feelings, I rolled my hips forward, driving my cock harder and faster into Mina's cunt."
    ""

translate id w3MinaHotel_25097834:
    # mina "Hwwwahh-♥"
    mina ""

translate id w3MinaHotel_734e1b81:
    # "The harder I pounded, the more she was fucked complacently into the window."
    ""

translate id w3MinaHotel_9aa9a741:
    # mina "Gyyah... euhhh--♥ You're going so-"
    mina ""

translate id w3MinaHotel_73c9ea61:
    # "Fast."
    ""

translate id w3MinaHotel_e40325dd:
    # mina "Yhhwe-♥ Hwwwuu-♥"
    mina ""

translate id w3MinaHotel_8b9be3d0:
    # "Fast and deep."
    ""

translate id w3MinaHotel_459e0143:
    # mc "This good?"
    mc ""

translate id w3MinaHotel_a33bb007:
    # mina "Shh, y-yeah...♥ It f-feels g-good! Like r-really, {b}really{/b} g-good!"
    mina ""

translate id w3MinaHotel_30c8a376:
    # "Mina used her upper body to push back, but I had almost all the leverage from this position."
    ""

translate id w3MinaHotel_b8579f96:
    # mc "Take a look at the city, take a look at the streets... take a look at all the people...!"
    mc ""

translate id w3MinaHotel_9aa3faf6:
    # "I could lay into her pussy to my heart's content, taking in the lovely curve of Mina's back while basking in the view of the city."
    ""

translate id w3MinaHotel_f4f43619:
    # mc "They got no fucking idea we're screwing right above their heads."
    mc ""

translate id w3MinaHotel_4031aa6c:
    # mina "Uunnggh- haa-!"
    mina ""

translate id w3MinaHotel_6c06990b:
    # mc "They got no idea I'm screwing the most beautiful woman they've never seen!"
    mc ""

translate id w3MinaHotel_12485a52:
    # mina "Ahh, haa..? Ehehe...♥ You weirdo!"
    mina ""

translate id w3MinaHotel_a1c24289:
    # "Mina laughed, taking this relatively innocent fantasy in good stride."
    ""

translate id w3MinaHotel_7a63a445:
    # mc "It turns me on to imagine my name literally screamed into the high heavens!"
    mc ""

translate id w3MinaHotel_526faca4:
    # mina "Yeeuuuah...?!"
    mina ""

translate id w3MinaHotel_f30c1981:
    # mc "Say my name!"
    mc ""

translate id w3MinaHotel_edc0b945:
    # "To further make my point, I slammed my cock harder and faster into Mina's cunt."
    ""

translate id w3MinaHotel_7f71aebd:
    # mina "Hnnn, yyeee-♥ [mcf]!"
    mina ""

translate id w3MinaHotel_212a1023:
    # "Each smack of her ass was well-received, Mina's bubble butt providing the perfect target."
    ""

translate id w3MinaHotel_eab1bfaa:
    # mc "Say my name, {b}bitch!{/b}"
    mc ""

translate id w3MinaHotel_d9a7ba67:
    # mina "Gyehh, [mcf]!"
    mina ""

translate id w3MinaHotel_9e1cc13e:
    # "Each smack yielded an eye-pleasurable jiggle that caused Mina's pussy to strangle my dick even harder."
    ""

translate id w3MinaHotel_1c9696bf:
    # mc "Good girl!"
    mc ""

translate id w3MinaHotel_fb5034b0:
    # mina "Eh, yyeeah, aaah...♥ [mcf]."
    mina ""

translate id w3MinaHotel_5f2856be:
    # mc "Good Mina!"
    mc ""

translate id w3MinaHotel_515571fa:
    # mina "Ehehe~♥"
    mina ""

translate id w3MinaHotel_4814651b:
    # "She giggled, none too disturbed by my sudden shift in attitude."
    ""

translate id w3MinaHotel_1cfc9916:
    # mina "Ah, heee-♥ [mcf]!"
    mina ""

translate id w3MinaHotel_63c9d577:
    # "In fact, she sounded like she found this direction to be genuinely endearing."
    ""

translate id w3MinaHotel_893086b9:
    # mina "Ah, hhaa... maybe we should try this on a rooftop sometime?! Wouldn't that be wild?"
    mina ""

translate id w3MinaHotel_0bd23dca:
    # mc "Is that on your list?"
    mc ""

translate id w3MinaHotel_b22f2b77:
    # mina "N-nope! C-could be on {i}yours{/i} though!"
    mina ""

translate id w3MinaHotel_3c1fb3a6:
    # mc "Hnngg, hauuch... no thanks! I'm afraid of heights!"
    mc ""

translate id w3MinaHotel_3258d1a1:
    # mc "N-no thanks... I'm afraid of heights!"
    mc ""

translate id w3MinaHotel_cc072418:
    # mina "Pffh, w-what? Hah-"
    mina ""

translate id w3MinaHotel_a2d3eb7d:
    # mina "Yeeaahhuuuhh-♥ Ah, c-craaap-♥"
    mina ""

translate id w3MinaHotel_b14b8b63:
    # "Mina's joy was so pervasive that I found myself smiling too."
    ""

translate id w3MinaHotel_287d39f7:
    # mina "Ah, you're hitting me in {i}all{/i} the r-right spots!"
    mina ""

translate id w3MinaHotel_9a1800c5:
    # "Fucking her was unlike any other experience. It felt edifying and {i}wholesome{/i} of all things."
    ""

translate id w3MinaHotel_2d424326:
    # mina "[mcf], [mcf], [mcf], [mcf], [mcf]!"
    mina ""

translate id w3MinaHotel_0bd65160:
    # "Even being pounded into the glass, she shined brilliantly..."
    ""

translate id w3MinaHotel_e6c92d93:
    # mc "You slut!"
    mc ""

translate id w3MinaHotel_00fa4f64:
    # mina "Hehe, you really think you're l-lucky?"
    mina ""

translate id w3MinaHotel_0bd65160_1:
    # "Even being pounded into the glass, she shined brilliantly..."
    ""

translate id w3MinaHotel_569d3f59:
    # mc "Let me prove it to you!"
    mc ""

translate id w3MinaHotel_7c69f65c:
    # "I poured my focus into giving Mina my {b}all{/b}."
    ""

translate id w3MinaHotel_e83078c7:
    # mina "Fuh, hhaa-♥ [mcf]!"
    mina ""

translate id w3MinaHotel_39b5c8ce:
    # "By now, I had a pretty good idea of what angle of attack she found most gratifying."
    ""

translate id w3MinaHotel_a0ebd258:
    # mina "Yyeh, haa... r-right there! Right there, you fhhweeuh-♥"
    mina ""

translate id w3MinaHotel_57ba01fd:
    # "My cockhead dug into the outer wall of her vagina, following the bend of her pelvis, scrapping and gouging all the way."
    ""

translate id w3MinaHotel_74ad6d43:
    # mc "Take it!"
    mc ""

translate id w3MinaHotel_67e2bddd:
    # "I felt confident I could make her cum first."
    ""

translate id w3MinaHotel_70c2fe03:
    # mc "Take it all, you bitch!"
    mc ""

translate id w3MinaHotel_95cae8e7:
    # mc "Take it all!"
    mc ""

translate id w3MinaHotel_4961044b:
    # "I wasn't anywhere near my limit and my recent experiences had given me plenty confidence in my stamina."
    ""

translate id w3MinaHotel_37577baa:
    # mina "Y-yeahh, eeugh... g-give it to me-♥"
    mina ""

translate id w3MinaHotel_fff65b55:
    # "Sensing the change in focus, Mina did her best with what little leverage she could muster to pleasure me back."
    ""

translate id w3MinaHotel_11b0e85f:
    # mina "Gnngh, hnnng-! G-give it all to me!"
    mina ""

translate id w3MinaHotel_7515cfd7:
    # "That window of good humor had closed in favor of a concentrated feeling of lust."
    ""

translate id w3MinaHotel_8172d7b3:
    # "Her giggles and sweet-natured cooing were now loud, vulgar grunts."
    ""

translate id w3MinaHotel_509d0521:
    # mina "Guah, hhuugnhh, hnnn-! Give it to me, Daddy!"
    mina ""

translate id w3MinaHotel_7e454268:
    # mct "(...ha, Daddy?)"
    mct ""

translate id w3MinaHotel_431eb29f:
    # mct "(There it was again. Just like the first time we fooled around.)"
    mct ""

translate id w3MinaHotel_de65081d:
    # mct "(...did Ian teach her that or is that all her, I wonder?)"
    mct ""

translate id w3MinaHotel_a3d9d23d:
    # mina "Ye, hha, fwhh- hehe~ fwuhh-♥"
    mina ""

translate id w3MinaHotel_69999ade:
    # "I could tell from her body language she was getting close."
    ""

translate id w3MinaHotel_88a061a5:
    # mina "Gyhh, y-ye- ah, eeh?"
    mina ""

translate id w3MinaHotel_d61e7666:
    # "The way she babbled, the way her legs buckled, the way her hands fruitlessly tried to find a more secure hold on the glass."
    ""

translate id w3MinaHotel_e2538e00:
    # mina "Gyee, aaahhh.. eeeuugghh-"
    mina ""

translate id w3MinaHotel_baee0a2a:
    # "Yeah, she was getting {i}damn{/i} close. Very--"
    ""

translate id w3MinaHotel_7f108467:
    # mina "Sh, wwwyyyaaaeehhhhhhh-♥♥♥♥"
    mina ""

translate id w3MinaHotel_7f3a0357:
    # "Suddenly, her body went taut. Every muscle, including the one wrapped around my dick, froze up."
    ""

translate id w3MinaHotel_ea3b48cd:
    # mina "Ueehhhh, aaahh- hhhnnn-♥♥♥♥"
    mina ""

translate id w3MinaHotel_ea1102f7:
    # "She cried... {i}howled{/i} in a beautifully filthy way as my momentum ground to a stop in opposition to Mina's unwavering tightness."
    ""

translate id w3MinaHotel_0afeaf7e:
    # mc "Gah, hhnn-!"
    mc ""

translate id w3MinaHotel_c302feb0:
    # mina "Ah, wwooahh.. s-shit.. d-dang... ahahaha... oh, w-wow... heh.."
    mina ""

translate id w3MinaHotel_5b9ee466:
    # "Mina was ego-inducingly scatterbrained. Moments passed before... "
    ""

translate id w3MinaHotel_9ea90323:
    # mina "Haa... mmmh... why have you stopped? You still haven't cummed... hehh..."
    mina ""

translate id w3MinaHotel_96905d09:
    # mc "Slow and steady..."
    mc ""

translate id w3MinaHotel_0ffeaaaf:
    # mina "Mmmhhh....?"
    mina ""

translate id w3MinaHotel_523ce6f9:
    # mc "I paid a lot of money for this room. We're going to use it."
    mc ""

translate id w3MinaHotel_e56e7d8b:
    # mc "My goal isn't cumming, remember?"
    mc ""

translate id w3MinaHotel_0ffeaaaf_1:
    # mina "Mmmhhh....?"
    mina ""

translate id w3MinaHotel_674eb684:
    # mc "You asked me to mess you up and there's still a lot to go."
    mc ""

translate id w3MinaHotel_e2996c1d:
    # mina "O-oh..."
    mina ""

translate id w3MinaHotel_dca89831:
    # mc "C'mon, let's try out the bed."
    mc ""

translate id w3MinaHotel_c48f2ad7:
    # "I still had a lot of gas in the tank, but even if I didn't..."
    ""

translate id w3MinaHotel_bd198ffb:
    # "The way Mina's doe-like eyes teemed with desire was reason enough for me not to speed toward an orgasm."
    ""

translate id w3MinaHotel_aeb2bb08_1:
    # mc "Mina..."
    mc ""

translate id w3MinaHotel_0e288f14:
    # mina "Hehehe~"
    mina ""

translate id w3MinaHotel_00bb1029:
    # "In anticipation, she purred like a kitten."
    ""

translate id w3MinaHotel_6052aa78:
    # mina "Mmmmm~"
    mina ""

translate id w3MinaHotel_6bb7a14a:
    # "Adding a touch of sensuality to the moment, Mina's tiny mouth ensnared my thumb, sucking on it desirously."
    ""

translate id w3MinaHotel_95f76053:
    # mc "Mina, Mina, Mina..."
    mc ""

translate id w3MinaHotel_c23886c6:
    # "I briefly considered giving her something {b}else{/b} to suck on, but my taste of Mina's lower mouth had been all too brief."
    ""

translate id w3MinaHotel_aa2dfb8f:
    # "She watched me so attentively, so intently, that there was no need to explain what I wanted."
    ""

translate id w3MinaHotel_4aab1dce_2:
    # mina "........."
    mina ""

translate id w3MinaHotel_cbfb53b1_9:
    # mina "......"
    mina ""

translate id w3MinaHotel_4e2a8876_11:
    # mina "..."
    mina ""

translate id w3MinaHotel_f43ddca3:
    # "She understood that I wanted to see her this time."
    ""

translate id w3MinaHotel_572f680a:
    # "To be face-to-face."
    ""

translate id w3MinaHotel_2be47627:
    # "She smiled and began to spread her legs. Inching them apart until..."
    ""

translate id w3MinaHotel_f4b0491c:
    # mina "Come on, [mcf]..."
    mina ""

translate id w3MinaHotel_472454fa:
    # "I didn't need a better invitation."
    ""

translate id w3MinaHotel_c257d4c4:
    # mina "Gyeh- ahh.. just right into-"
    mina ""

translate id w3MinaHotel_8a3daf9d:
    # "--!"
    ""

translate id w3MinaHotel_9492f3f1:
    # mina "Ww-wahha-♥"
    mina ""

translate id w3MinaHotel_8a3daf9d_1:
    # "--!"
    ""

translate id w3MinaHotel_bb978c84:
    # mc "{b}Mina{/b}."
    mc ""

translate id w3MinaHotel_6cb09b9b:
    # mina "Ah- ah... [mcf]..."
    mina ""

translate id w3MinaHotel_07f01f9f:
    # mc "Are you gonna be a slut for Daddy?"
    mc ""

translate id w3MinaHotel_50f6b285:
    # mina "A-ah! Whaaa-"
    mina ""

translate id w3MinaHotel_d5d6926d:
    # "If she had a predilection for this kind of play, might as well lean into it. Not like I wasn't into this shit, either."
    ""

translate id w3MinaHotel_9d483655:
    # "In a deliberate effort, she kept eye contact with me, simply nodding and giving me approval at this direction."
    ""

translate id w3MinaHotel_a49b72e5:
    # mc "That's my baby girl."
    mc ""

translate id w3MinaHotel_8251f820:
    # "I guess this really was a kink of hers."
    ""

translate id w3MinaHotel_c257d4c4_1:
    # mina "Gyeh- ahh.. just right into-"
    mina ""

translate id w3MinaHotel_8a3daf9d_2:
    # "--!"
    ""

translate id w3MinaHotel_9492f3f1_1:
    # mina "Ww-wahha-♥"
    mina ""

translate id w3MinaHotel_585391e2:
    # mc "We're past warming up!"
    mc ""

translate id w3MinaHotel_d1e6f027:
    # "For the next leg of the evening, there was no need to build to tempo."
    ""

translate id w3MinaHotel_71166e9c:
    # mina "Ah, hhhnn-♥"
    mina ""

translate id w3MinaHotel_8ef3e87f:
    # "Mina's sex, while still nut-wrenchingly tight, had grown used to my size and shape."
    ""

translate id w3MinaHotel_43567bab:
    # mc "Goddamn it, you feel amazing!"
    mc ""

translate id w3MinaHotel_85cc4de2:
    # "Using her arms for leverage, I had no trouble slipping in and out of her at speed, carving out a long and deep path."
    ""

translate id w3MinaHotel_6c5e87ec:
    # mina "Euughh, hhhaak- {b}y-you t-toooo!{/b}"
    mina ""

translate id w3MinaHotel_b55ae01f:
    # "Mina admirably did what she could to take an active part in our shared pleasure, using her legs to steady and guide my hips home, but I had the dominant position."
    ""

translate id w3MinaHotel_a05155ac:
    # "She could grind and writhe, but she had little recourse but to lie there, look up at me, and watch herself get fucked - all with an enamoring look on her face."
    ""

translate id w3MinaHotel_3873e1b7:
    # mina "Whh, hhnn-♥ Sex with y-you was such a GOOD idea! Hehehe, ha-♥"
    mina ""

translate id w3MinaHotel_692520b8:
    # "--but it wasn't like I had much choice either. I may have had a physical hold on her, but my head was preoccupied with Mina."
    ""

translate id w3MinaHotel_381baf34:
    # mc "You were BUILT for this, Mina."
    mc ""

translate id w3MinaHotel_1f7bd77e:
    # "What else could I do but spear her over and over and over again on my cock?"
    ""

translate id w3MinaHotel_a12d1223:
    # mc "Daddy's good little whore!"
    mc ""

translate id w3MinaHotel_e37f5a61:
    # "What else could I do but marvel at Mina's perfect bust and its appetizingly engorged pink peaks?"
    ""

translate id w3MinaHotel_902966f8:
    # mc "Daddy's perfect slut!"
    mc ""

translate id w3MinaHotel_5cd7f860:
    # "No... I had little recourse of my own."
    ""

translate id w3MinaHotel_04224655:
    # mina "Whh, hhaa-♥ D-daddy! Ahhh...♥♥♥"
    mina ""

translate id w3MinaHotel_a64d0763:
    # mc "Ha! You took the -hnn- words RIGHT out of my mouth."
    mc ""

translate id w3MinaHotel_7eced805:
    # mc "We should make this a regular thing, y-you know?"
    mc ""

translate id w3MinaHotel_3241b397:
    # "What else would I do but spear her over and over and over again on my cock?"
    ""

translate id w3MinaHotel_c66b6695:
    # mina "Hnnhh- w-whenyouever you want, [mcf]."
    mina ""

translate id w3MinaHotel_e37f5a61_1:
    # "What else could I do but marvel at Mina's perfect bust and its appetizingly engorged pink peaks?"
    ""

translate id w3MinaHotel_fd05deba:
    # mina "I'm yours... Ahh...♥"
    mina ""

translate id w3MinaHotel_5cd7f860_1:
    # "No... I had little recourse of my own."
    ""

translate id w3MinaHotel_fb19d197:
    # mina "...you o-only have to ask~♥ Haaht, crapeeeuuguhh-♥"
    mina ""

translate id w3MinaHotel_87ba7614:
    # "Fuck and be fucked."
    ""

translate id w3MinaHotel_f076803e:
    # "We had carved out, for only a painfully fleeting moment, a slice of meaning in this world."
    ""

translate id w3MinaHotel_12ca4026:
    # mina "Gehhuu, aahh-♥"
    mina ""

translate id w3MinaHotel_b8b66697:
    # "Meaning uncomplicated and lacking in nuance."
    ""

translate id w3MinaHotel_682480ec:
    # "Meaning found in the most straightforward and essential human act."
    ""

translate id w3MinaHotel_e80df858:
    # "Meaning at its most hedonistic and pure."
    ""

translate id w3MinaHotel_676766dd:
    # mina "Ghhu, hffhhuh- fuck me, Daddy!"
    mina ""

translate id w3MinaHotel_3731e648:
    # mina "Ghhu, hffhhuh- fuck me, [mcf]!!"
    mina ""

translate id w3MinaHotel_79bbe75e:
    # "In this brief moment, sex with Mina was the most obvious thing in the world. A cure-all for the soul."
    ""

translate id w3MinaHotel_dd3741a6:
    # mc "Don't worry! I'm -hnnn- giving it my b-best!"
    mc ""

translate id w3MinaHotel_aea631c7:
    # "Oh, how easy it was to forget the messy events that led to this."
    ""

translate id w3MinaHotel_effd8cec:
    # "Oh, how quickly this charade would be over."
    ""

translate id w3MinaHotel_d6db0122:
    # mina "Gyahh, hhaa, wwh♥"
    mina ""

translate id w3MinaHotel_7635c9f6:
    # "No... I had to make this last, as long as I could."
    ""

translate id w3MinaHotel_6fcf5a48:
    # "I would carve this experience into Mina's body to leave her no doubt about coming back for me."
    ""

translate id w3MinaHotel_41b7def8:
    # mina "Hnn, hwwwah, hhaaa♥♥"
    mina ""

translate id w3MinaHotel_52f2fa37:
    # "There it was! That familiar feeling."
    ""

translate id w3MinaHotel_7efe9332:
    # mina "Ghh, hhnn, wwah, ewwuuuuhh♥♥♥"
    mina ""

translate id w3MinaHotel_b05bef7d:
    # "The tension..."
    ""

translate id w3MinaHotel_bab881ea:
    # mina "Wwoohhh, ghh, www-hhhhnnnnn♥♥♥♥♥♥♥♥♥♥♥♥♥!"
    mina ""

translate id w3MinaHotel_c4c5d29b:
    # "The tension {b}broke.{/b}"
    ""

translate id w3MinaHotel_c9b89c5f:
    # mina "Ehhhuuuhhh-♥"
    mina ""

translate id w3MinaHotel_4ea0c626:
    # "Mina found herself descending the peak for the second time this evening."
    ""

translate id w3MinaHotel_2dcfa546:
    # "I wasn't nearly finished."
    ""

translate id w3MinaHotel_ab9f99bd:
    # mc "No time to rest, Babygirl."
    mc ""

translate id w3MinaHotel_85f83ea2:
    # mc "No time to rest, beautiful."
    mc ""

translate id w3MinaHotel_33097dda:
    # mina "Wha..? Whhhh-?!"
    mina ""

translate id w3MinaHotel_03a46b1f:
    # mina "Gyy, hhhn- hhaaa-♥"
    mina ""

translate id w3MinaHotel_e53b72c9:
    # "This time a well-fucked Mina was on top, but in her half-insensate state, she could only concede me the initiative."
    ""

translate id w3MinaHotel_290cc545:
    # mina "Hnhh... n-no time out, aah-ah?"
    mina ""

translate id w3MinaHotel_2f0881e2:
    # mc "Nope!"
    mc ""

translate id w3MinaHotel_3337e4ee:
    # mina "Ehh.. ohh, h a- ohkkay-!"
    mina ""

translate id w3MinaHotel_29a5fe39:
    # "She instinctually bucked and bounced, but it was up to me to do the lifting."
    ""

translate id w3MinaHotel_58d0f2e3:
    # mc "Haahh, thanks for being so understanding!"
    mc ""

translate id w3MinaHotel_ba7d5a2d:
    # "Fortunately, Mina, contrary to all her bluster and immense personality, was a very light young lady."
    ""

translate id w3MinaHotel_aa5d760a:
    # "Even in my fuck-focused state, prying Mina off my cock and dropping her back down was a trivial task."
    ""

translate id w3MinaHotel_ed472299:
    # mina "Hheeh, hheeh-♥ You're very persuasive! Ahh-♥"
    mina ""

translate id w3MinaHotel_1e2f1113:
    # "An immensely FUN and trivial task."
    ""

translate id w3MinaHotel_89b22080:
    # mina "Ahh, vvheeeery-♥♥"
    mina ""

translate id w3MinaHotel_e8e8665a:
    # "One with an immensely gratifying angle... {i}not that she had any bad ones.{/i}"
    ""

translate id w3MinaHotel_df18079c:
    # mina "Persuusshhive-♥"
    mina ""

translate id w3MinaHotel_3693ef35:
    # "I could just dumbly watch her go up and down, up and down to my utmost satisfaction."
    ""

translate id w3MinaHotel_2c697103:
    # mc "If you say so, slut!"
    mc ""

translate id w3MinaHotel_5755055c:
    # mc "Heh, if you say so!"
    mc ""

translate id w3MinaHotel_9fb24e64:
    # "Up and down, sinking my fingertips into the pale flesh of her hefty mammaries."
    ""

translate id w3MinaHotel_64d2872e:
    # mina "Gyy, hhhn- hhhhn-♥ Hwwwhh-♥!"
    mina ""

translate id w3MinaHotel_4ff82bb6:
    # "Up and down, focusing on {i}the sensations{/i} as her lower body tried to milk me for all my pleasurable worth."
    ""

translate id w3MinaHotel_3bea7c06:
    # mct "(Ahh... ha... Christ-!)"
    mct ""

translate id w3MinaHotel_a80c9d0b:
    # "{b}Riiiight{/b}, I've been so preoccupied with Mina's reactions that I forgot I was getting it as good as I was giving."
    ""

translate id w3MinaHotel_13e60006:
    # mc "Ah, hhhhaa-"
    mc ""

translate id w3MinaHotel_98201a65:
    # "My own moans unconsciously escaped my lips as my mind shifted awareness to the tingly feeling traveling up my spine."
    ""

translate id w3MinaHotel_cabaf219:
    # mina "Hehe- ah, hh--- y-you're feeling-? H-ha, {b}I'm--{/b}!"
    mina ""

translate id w3MinaHotel_154498c9:
    # "{i}How could I not{/i}, I thought to myself, but it was a thought better served through action rather than voice."
    ""

translate id w3MinaHotel_a614c23b:
    # mina "Eehhh, hhhnn-♥"
    mina ""

translate id w3MinaHotel_685acd53:
    # mc "C'mon, shake it harder!"
    mc ""

translate id w3MinaHotel_b4454a43:
    # mina "Eeueuhhh- aaahh...! I a.. aaa--hhh-♥♥"
    mina ""

translate id w3MinaHotel_c2af9833:
    # mc "You can do it better than that!"
    mc ""

translate id w3MinaHotel_9f3d9ffd:
    # mina "Waah, hhhnaa-♥♥♥ I'm c-close-!"
    mina ""

translate id w3MinaHotel_05348639:
    # "Once she starts, she just can't stop, huh?"
    ""

translate id w3MinaHotel_960df770:
    # mc "Shake it for Daddy, you bitch!"
    mc ""

translate id w3MinaHotel_f04bfe5a:
    # mina "I --aa, eeheeem- aahh..."
    mina ""

translate id w3MinaHotel_ac469b4e:
    # mc "Come for me, Babygirl!"
    mc ""

translate id w3MinaHotel_de9d5c19:
    # mc "C'mon... b-bounce! A-ah!"
    mc ""

translate id w3MinaHotel_b4454a43_1:
    # mina "Eeueuhhh- aaahh...! I a.. aaa--hhh-♥♥"
    mina ""

translate id w3MinaHotel_0c8d28f5:
    # mc "C-come on... you're aaahh... you- you feel amazing!!"
    mc ""

translate id w3MinaHotel_7b501bc0:
    # mina "Waah, hhhnaa-♥♥♥ Hnggg, I-I'm close!"
    mina ""

translate id w3MinaHotel_05348639_1:
    # "Once she starts, she just can't stop, huh?"
    ""

translate id w3MinaHotel_b7dd0369:
    # mc "Come on, do it, then!"
    mc ""

translate id w3MinaHotel_dc051276:
    # mina "Wwwh, hhngg, wwaa--♥♥♥"
    mina ""

translate id w3MinaHotel_7b75c0db:
    # mc "Come on... {i}come{/i} for me again-"
    mc ""

translate id w3MinaHotel_4e28943e:
    # mina "I'm..."
    mina ""

translate id w3MinaHotel_c32ea60f:
    # mina "I'm, I'm..."
    mina ""

translate id w3MinaHotel_382b5f05:
    # mina "I'm, I'm, I'm-!"
    mina ""

translate id w3MinaHotel_d5761e53:
    # mina "Euuugaaaaaaaaaaaaaaaaaaaghhhhhhhh-♥♥♥♥♥♥♥♥♥♥♥♥"
    mina ""

translate id w3MinaHotel_4f66aef9:
    # "A warmness spread across my stomach..."
    ""

translate id w3MinaHotel_fe3b8e64:
    # mina "Wh, haa, hhnn-"
    mina ""

translate id w3MinaHotel_39d2f3ed:
    # mina "Heeh, hhoo... oh... waahhh..."
    mina ""

translate id w3MinaHotel_018e3e91:
    # mina "Gaah, hehe, like... {b}fuck!{/b}"
    mina ""

translate id w3MinaHotel_223ee38f:
    # mina "H-hey, uhh... waah..."
    mina ""

translate id w3MinaHotel_a2bb69c8:
    # mina "{b}Finish... go ahead... ahh... I want you to finish while fucking me... c-cum insiiide mee....♥♥♥♥♥ Pleeeeeease♥♥♥♥♥"
    mina ""

translate id w3MinaHotel_58dd2447:
    # mina "I w-want to f-feel like I... ahhh...♥"
    mina ""

translate id w3MinaHotel_badf132c:
    # mina "Make me feel-- aa-hhh...♥ Just... {b}ruin me{/b}, [mcf]."
    mina ""

translate id w3MinaHotel_017af1f7:
    # mc "W-what...? Ahh... haa..."
    mc ""

translate id w3MinaHotel_fa1a0ba4:
    # mina "Make me forget I'm even a person, o-okay? It's..."
    mina ""

translate id w3MinaHotel_bdb4f825:
    # mina "...it's a fantasy of mine. {b}Destroy me.{/b}"
    mina ""

translate id w3MinaHotel_6190b081:
    # mina "...c-consider it part of my list. {b}Destroy me.{/b}"
    mina ""

translate id w3MinaHotel_3b21f354_1:
    # mc "......"
    mc ""

translate id w3MinaHotel_73b0b755:
    # mc "...aaaaaah, shit. {b}You fucking got it!{/b}"
    mc ""

translate id w3MinaHotel_1bccb5d9:
    # mina "W-wahh, aaahhh...♥♥♥"
    mina ""

translate id w3MinaHotel_41cc0eee:
    # mct "(How the fuck did I end up with the tame fantasy here?)"
    mct ""

translate id w3MinaHotel_96e04a5c:
    # mc "Get the fucking ass up!"
    mc ""

translate id w3MinaHotel_2c6ce4b1:
    # "*Slap!*"
    ""

translate id w3MinaHotel_3a5c4878:
    # mina "W-wwah, hhhngngggg..."
    mina ""

translate id w3MinaHotel_bda03986:
    # mc "Point it this way!"
    mc ""

translate id w3MinaHotel_54218141:
    # mina "W-wah, ehh...?"
    mina ""

translate id w3MinaHotel_5325b9db:
    # mc "Face down, ass up!"
    mc ""

translate id w3MinaHotel_278c7e29:
    # "........."
    ""

translate id w3MinaHotel_0fd4d656_13:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_13:
    # "..."
    ""

translate id w3MinaHotel_d0f10c2d:
    # mc "F-fuuuuck!"
    mc ""

translate id w3MinaHotel_0542b8a0:
    # "I lost it."
    ""

translate id w3MinaHotel_060b94c1:
    # mina "Guhh, hhhu, hee, hhhu-!"
    mina ""

translate id w3MinaHotel_571fdb2f:
    # "I fucking lost it."
    ""

translate id w3MinaHotel_556b2007:
    # mc "D-damn it, Mina... uggh... s-saying something like that♥"
    mc ""

translate id w3MinaHotel_b3bc6c4c:
    # "It would be unfair to Mina's heartfelt and earnest request {i}not{/i} to fuck her like her only purpose was to wrench the semen from my fat and heavy balls."
    ""

translate id w3MinaHotel_65b7bd9a:
    # mc "In a voice like that, from a girl like you...?"
    mc ""

translate id w3MinaHotel_0625b713:
    # "Did she have any idea what sort of power she wielded?"
    ""

translate id w3MinaHotel_f710f35b:
    # mina "Wwhh- hhhnn- wwwhhh-♥"
    mina ""

translate id w3MinaHotel_6cd9a17f:
    # "Did she know what sort of effect those words would have on a man?"
    ""

translate id w3MinaHotel_1118cc3c:
    # mc "Hnng, y-you filthy, beautiful bitch!"
    mc ""

translate id w3MinaHotel_85fdeb74:
    # mc "Hnng, goddamn it!"
    mc ""

translate id w3MinaHotel_95ac75f2:
    # mc "Of course, I'll ruin you!"
    mc ""

translate id w3MinaHotel_929fa020:
    # "My instincts had taken over."
    ""

translate id w3MinaHotel_ee4f12ef:
    # mina "Guu, hhhnn- wwahhh-♥♥"
    mina ""

translate id w3MinaHotel_98eb8582:
    # "Instincts prone to sloppy, violent, impact-stricken sex."
    ""

translate id w3MinaHotel_0fa4bde2:
    # mina "Wwhhu, wwhhaa- hhaahhh-♥♥♥"
    mina ""

translate id w3MinaHotel_02953f80:
    # "With a tight grip on Mina's hips, I did everything in my power to destroy not just her, but the both of us."
    ""

translate id w3MinaHotel_afbbf594:
    # mina "Whhh, gghhhu, iihhhz, uuoohhh-♥♥♥♥"
    mina ""

translate id w3MinaHotel_1ef45845:
    # "Destroy."
    ""

translate id w3MinaHotel_355b7f09:
    # "Destroy. Destroy."
    ""

translate id w3MinaHotel_a86f8b39:
    # mc "Ah, fhhuuuck! Haaaa!"
    mc ""

translate id w3MinaHotel_7909e0bb:
    # "Right now, {b}MY{/b} only purpose was to plug, stuff, and pull at Mina's fuck-ready insides."
    ""

translate id w3MinaHotel_8a8b1439:
    # "{i}Gouge, scrape, prod, rub, stroke, swab, scratch.{/i}"
    ""

translate id w3MinaHotel_c4ddc7b5:
    # mina "Ghhn, hhhuaa.. wwhahahaa-♥♥♥♥♥"
    mina ""

translate id w3MinaHotel_b6191e50:
    # "Right now, Mina had more sense in her pussy than she did in her head."
    ""

translate id w3MinaHotel_82b718cd:
    # "Destroy. Destroy. Destroy. Destroy."
    ""

translate id w3MinaHotel_6ae7a20a:
    # "Destroy. Destroy. Destroy. Destroy. Destroy. Destroy. Destroy. Destroy."
    ""

translate id w3MinaHotel_675577c6:
    # mina "Whh, chhok, hhwwhh.. chhhum-♥♥♥♥♥♥"
    mina ""

translate id w3MinaHotel_b6ebfb51:
    # "All I could do was fuck, and all she could do was moan like a cock-stuffed bitch drunk on dick."
    ""

translate id w3MinaHotel_ca16555d:
    # mina "Whh, hhaa..!!"
    mina ""

translate id w3MinaHotel_ebcc4759:
    # "Luckily, her lower mouth did all the talking required, inviting me, swallowing, and {b}consuming{/b} completely."
    ""

translate id w3MinaHotel_edaacf03:
    # mc "You want me to... Aahh... fine!"
    mc ""

translate id w3MinaHotel_6cd2c910:
    # "...wait, it's not a question!"
    ""

translate id w3MinaHotel_bf4ae7cc:
    # mc "Take this, bitch!"
    mc ""

translate id w3MinaHotel_ad89b553:
    # mina "Gwwhhh... hnnggg... awww-!!!!"
    mina ""

translate id w3MinaHotel_16426513:
    # "As I brought my palm down on Mina's bouncing ass, the picture in my head was a wonderful dichotomy."
    ""

translate id w3MinaHotel_0eb9414b:
    # mina "Ghhu, hhnngg, wwwahh- wwwhwhwhw-!!!!!"
    mina ""

translate id w3MinaHotel_f897f738:
    # "On one side was the Mina I knew as warm, caring, bright, bubbly... a source of affection..."
    ""

translate id w3MinaHotel_059a7d1b:
    # mina "GGhhh, hhwwaa, wwwahh, hhehhh-"
    mina ""

translate id w3MinaHotel_7f5a39c0:
    # "On the other was the bleating fucktoy beneath me, a Mina consumed by a hedonistic state of bliss."
    ""

translate id w3MinaHotel_2cb95846:
    # mc "Gahh, {b}fhuuuuuuuuuuuuuuuuuuuuuuck!{/b}"
    mc ""

translate id w3MinaHotel_b9e4ff13:
    # "Neither was wrong. Neither was ugly."
    ""

translate id w3MinaHotel_9238ab7b:
    # "We were two people letting out our frustrations... sharing and living out a fantasy... getting temporarily lost..."
    ""

translate id w3MinaHotel_4daf5a76:
    # "Fuck, fuck, fuck, fuck, fuck, fuck...!"
    ""

translate id w3MinaHotel_d8bb9039:
    # mc "You w-wonderful...!"
    mc ""

translate id w3MinaHotel_8ec95d15:
    # "Take it, take it, take it, take it, take it, take it...!"
    ""

translate id w3MinaHotel_0ebc2b6b:
    # mc "You lovely...!"
    mc ""

translate id w3MinaHotel_f493c353:
    # "Fuck, damn, fuck, damn, fuck, damn...!"
    ""

translate id w3MinaHotel_f097305e:
    # "As I felt my dick {b}boil{/i} over, as all feeling left my body... I could see it all."
    ""

translate id w3MinaHotel_e406f4dc:
    # "Mina and someone who {i}resembled{/i} me, teetering on the edge of the track, about to take that final plummet..."
    ""

translate id w3MinaHotel_ee53285d:
    # mc "Mina... ahh... y-you..!"
    mc ""

translate id w3MinaHotel_036bc378:
    # "In the last moments, that doomed soul called out..."
    ""

translate id w3MinaHotel_f00964cf:
    # mc "Mina... I... I... {b}loooooooove you!{/b}"
    mc ""

translate id w3MinaHotel_6b23ee0b:
    # mc "Take it, slut!"
    mc ""

translate id w3MinaHotel_fdfe781b:
    # mc "T-take it!"
    mc ""

translate id w3MinaHotel_31e301c4:
    # mc "Gyyh- asahhhh-♥♥♥♥♥"
    mc ""

translate id w3MinaHotel_c1379bf8:
    # "Without even thinking about it, I gave Mina what she wanted."
    ""

translate id w3MinaHotel_99c1094f:
    # mc "Gyy, hhnhn-"
    mc ""

translate id w3MinaHotel_1a1267f9:
    # "I buried myself entirely in Mina's embrace, shooting everything I had been working toward into Mina's womb."
    ""

translate id w3MinaHotel_820303e9:
    # mina "Eehhhh.... hhaa.. chhum, hmmmh, mmuhuhh...♥♥♥"
    mina ""

translate id w3MinaHotel_fa497379:
    # "I had given her my all."
    ""

translate id w3MinaHotel_3d1fa144:
    # mc "Ghh, hhhaaaa..."
    mc ""

translate id w3MinaHotel_ce7cec3c:
    # "In the best possible way, like a pus-engorged abscess split asunder by its own putrid contents, I felt an exquisite sense of relief."
    ""

translate id w3MinaHotel_116b771b:
    # mc "Ughhh..."
    mc ""

translate id w3MinaHotel_d577f6ed:
    # mc "W-aah, uhh... that was... hnnngg..."
    mc ""

translate id w3MinaHotel_9feba5fa:
    # mina "W-woww... ahh..."
    mina ""

translate id w3MinaHotel_9bc9055b:
    # mina "I am so fhhhhuuuooool.... hehe, {size=-10}mmmhhh...{/size}"
    mina ""

translate id w3MinaHotel_bff9ce1d:
    # "Even dissolved into one big pile of goop, Mina's voice hadn't lost its candor."
    ""

translate id w3MinaHotel_4aab1dce_3:
    # mina "........."
    mina ""

translate id w3MinaHotel_cbfb53b1_10:
    # mina "......"
    mina ""

translate id w3MinaHotel_4b53a660:
    # mina "...hey, [mcf]?"
    mina ""

translate id w3MinaHotel_e7f04357:
    # mc "Y-yeah...?"
    mc ""

translate id w3MinaHotel_b5ddf2a6:
    # mina "I can't breathe."
    mina ""

translate id w3MinaHotel_ed7d4fd6:
    # mc "Oh, I... yeeeeah, let me-"
    mc ""

translate id w3MinaHotel_0a155cf0:
    # mina "{b}This{/b} is better~"
    mina ""

translate id w3MinaHotel_d85f85c6:
    # mina "Aaaaah... {i}big spoon secured{/i}."
    mina ""

translate id w3MinaHotel_b91f5ada:
    # "Mina simply held me for a little bit."
    ""

translate id w3MinaHotel_159d9823:
    # "She spoke no words, but her affections were communicated all the same through touch."
    ""

translate id w3MinaHotel_2bcff6a3:
    # "How she clung to me filled me with an irrational peace of mind."
    ""

translate id w3MinaHotel_caa2d57f:
    # "The way she ran her nose up and down my naked back made me feel, of all things, {b}present{/b} in the moment and acknowledged."
    ""

translate id w3MinaHotel_1593c9bf:
    # mina "{size=-10}Hmmm, mmhhh...{/size}"
    mina ""

translate id w3MinaHotel_a0b514b8:
    # "Even her sighs sounded like a lullaby to my ears and let me know that, on some level, she felt a measure of comfort from {i}my{/i} presence."
    ""

translate id w3MinaHotel_4fe168a4:
    # mc "I'm empty in the best possible way."
    mc ""

translate id w3MinaHotel_f2b56219:
    # mina "Hehe~ It's a great feeling, isn't it?"
    mina ""

translate id w3MinaHotel_92e605ad:
    # mc "Mmmmhhmmm."
    mc ""

translate id w3MinaHotel_c6fb07fb:
    # "{i}To see and to be seen {b}was{/b} a wonderful feeling indeed.{/i}"
    ""

translate id w3MinaHotel_a4883fe8:
    # mina "Ha! Then again, I'm not {i}soooooo{/i} empty~"
    mina ""

translate id w3MinaHotel_c1b8d277:
    # mina "I can feel it leaking out of me! I should probably get wash--"
    mina ""

translate id w3MinaHotel_e84a8bdf:
    # mc "{b}No{/b}. Don't get up."
    mc ""

translate id w3MinaHotel_7de98257:
    # mina "The sheets are going to get dirty. You wanna lie in your own filth?"
    mina ""

translate id w3MinaHotel_bc527c31:
    # mc "Who cares. {i}This{/i} is too nice."
    mc ""

translate id w3MinaHotel_a2e39686:
    # mina "Yeah, {b}it{/b} is..."
    mina ""

translate id w3MinaHotel_0fd4d656_14:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_14:
    # "..."
    ""

translate id w3MinaHotel_ac180bad:
    # mina "Hey [mcf]. When you think of yourself, do you feel like somebody, or do you feel like a nobody?"
    mina ""

translate id w3MinaHotel_625b3aa1:
    # "As she spoke, her hold on me grew more secure."
    ""

translate id w3MinaHotel_acd48efe:
    # mc "That's out of nowhere."
    mc ""

translate id w3MinaHotel_81910b9b:
    # mina "No, it's not. Don't guys get all introspective after they cum?"
    mina ""

translate id w3MinaHotel_bca4ef2f:
    # mc "Yeah, well, you got me there."
    mc ""

translate id w3MinaHotel_0691617f:
    # mina "What even is that? Is it some kind of survival thing?"
    mina ""

translate id w3MinaHotel_9b3abc66:
    # mina "Like, hey, you made a baby. So take stock, get your ducks in order?"
    mina ""

translate id w3MinaHotel_93e1ceae:
    # mc "Usually, it's just guilt over the fucked up thing you just masturbated to."
    mc ""

translate id w3MinaHotel_d6775d21:
    # mina "Tell me about that later, but let's return to my original question..."
    mina ""

translate id w3MinaHotel_44f703f5:
    # mina "Gun to your head, if you had to pick one or the other..."
    mina ""

translate id w3MinaHotel_9d8e4593:
    # mc "Right now? I'm feeling like somebody. What about you?"
    mc ""

translate id w3MinaHotel_0fd4d656_15:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_15:
    # "..."
    ""

translate id w3MinaHotel_76f0bc2d:
    # mina "I don't know. When things get quiet, my mind starts to wonder and wander."
    mina ""

translate id w3MinaHotel_f6e4e3ca:
    # mina "I know it's messed up, but all I can think of is that if you got something out of me, then I'm good. I put something into the world."
    mina ""

translate id w3MinaHotel_4effed40:
    # mc "I didn't {i}get{/i} something from you. We shared an afternoon."
    mc ""

translate id w3MinaHotel_3f35843c:
    # mina "Sorry, I didn't mean it in a bad way."
    mina ""

translate id w3MinaHotel_3c861b43:
    # mc "It's okay. I know you didn't."
    mc ""

translate id w3MinaHotel_1c248676:
    # mina "My dad once told me that if you give the world love, the world will provide you with love in return. It's not true, but I like to pretend it is."
    mina ""

translate id w3MinaHotel_837fa330:
    # mc "I can tell you try to live like it's true. Every part of you is so... {i}it must be exhausting{/i}."
    mc ""

translate id w3MinaHotel_d03ea4c2:
    # mina "Sometimes I think giving other people love and kindness is like shouting into an {i}empty cave{/i}, and all you get reflected back is a dying echo of your voice that you can't even fucking stand."
    mina ""

translate id w3MinaHotel_a76afce7:
    # mc "...how do you find the acoustics with me?"
    mc ""

translate id w3MinaHotel_7a932978:
    # mina "You haven't told me to just shut up and go to sleep."
    mina ""

translate id w3MinaHotel_c7f68ca8:
    # mina "You have NO idea how much I appreciate that."
    mina ""

translate id w3MinaHotel_7909d460:
    # mc "...for what it's worth, my dad had a similar philosophy. He was a lawyer who helped people who couldn't help themselves."
    mc ""

translate id w3MinaHotel_a662e538:
    # mc "He had a lot of love, and that love is reflected every time my mom speaks about him. It's reflected in the affection she gives me and it hasn't faded a single decibel."
    mc ""

translate id w3MinaHotel_33750d83:
    # mina "He sounds awesome."
    mina ""

translate id w3MinaHotel_8c63d24e:
    # mc "Your dad sounds pretty cool himself. I have a good feeling there's part of him in you as well."
    mc ""

translate id w3MinaHotel_2ecd9001:
    # mina "That's another thing I appreciate, [mcf]. You saying that."
    mina ""

translate id w3MinaHotel_4b23bbd7:
    # mc "Well, I appreciate you letting me blow your back out."
    mc ""

translate id w3MinaHotel_f9c30f4c:
    # mina "Pfffh- {b}ha!{/b} You just can't let a moment happen without making a joke, can you?"
    mina ""

translate id w3MinaHotel_ca8a4121:
    # mc "Who says I'm joking? I appreciate it. It was very cool of you."
    mc ""

translate id w3MinaHotel_2b33bafb:
    # mina "Shut up and go to bed."
    mina ""

translate id w3MinaHotel_100ec019:
    # mc "Noooot sleepy~"
    mc ""

translate id w3MinaHotel_fef85983:
    # mina "I don't sound like that!"
    mina ""

translate id w3MinaHotel_278c7e29_1:
    # "........."
    ""

translate id w3MinaHotel_0fd4d656_16:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_16:
    # "..."
    ""

translate id w3MinaHotel_4abe8607:
    # "We laid around for another five or ten minutes before finally dredging ourselves out of bed."
    ""

translate id w3MinaHotel_a745f1af:
    # "Mina had rebounded a lot quicker than I had, full of pep and giving credence to my suspicion that she might just be a succubus."
    ""

translate id w3MinaHotel_29bcd063:
    # mina "Mmmh, hmhhmmm~"
    mina ""

translate id w3MinaHotel_6e3631f1:
    # mina "Da da da da~ hmmmm~"
    mina ""

translate id w3MinaHotel_cf7ab368:
    # mina "La, la, laaaa~ dadadada~"
    mina ""

translate id w3MinaHotel_b16caa68:
    # mina "So... what kind of girl do you like, [mcf]? ~you can't say perky and blonde, either!"
    mina ""

translate id w3MinaHotel_e7894404:
    # mc "Like romantically? Put little thought into it."
    mc ""

translate id w3MinaHotel_e700c36d:
    # mina "None whatsoever...?"
    mina ""

translate id w3MinaHotel_c8a7822e:
    # mina "Okay, say you go to a porn site. What are you searching for?"
    mina ""

translate id w3MinaHotel_e5b36c19:
    # mc "Those are two completely different things."
    mc ""

translate id w3MinaHotel_b819fdc2:
    # mina "Mmmh, hmm... ARE theeeeey...?"
    mina ""

translate id w3MinaHotel_e3cd9c29:
    # mc "Well, not {i}completely{/i}, but... it's not like I search for say... {i}kind{/i}, {b}trustworthy{/b} blondes."
    mc ""

translate id w3MinaHotel_11b63e75:
    # mina "So you {i}do{/i} know the kind of girl you like?"
    mina ""

translate id w3MinaHotel_2bb361cc:
    # mc "Would a vague answer like that satisfy you?"
    mc ""

translate id w3MinaHotel_3743d0ad:
    # mina "Not in the least!"
    mina ""

translate id w3MinaHotel_38f763ff:
    # mc "I didn't think so."
    mc ""

translate id w3MinaHotel_fea2edac:
    # mc "Well, what kind of man do you like? And don't say nerdy med students."
    mc ""

translate id w3MinaHotel_bd90de8e:
    # mina "I don't know either."
    mina ""

translate id w3MinaHotel_f5e54757:
    # mc "No clue, eh? Okay then... say YOU go to a porn site..."
    mc ""

translate id w3MinaHotel_d708b3a6:
    # mina "Two dudes going at it! Hard, haha!"
    mina ""

translate id w3MinaHotel_43c2ca3d:
    # mc "Sounds like you could always squeeze yourself into a throuple."
    mc ""

translate id w3MinaHotel_06f53b18:
    # mina "Like you and Hana, for example?"
    mina ""

translate id w3MinaHotel_2b35b6fe_2:
    # mc "..."
    mc ""

translate id w3MinaHotel_3654cc89:
    # mina "You told me about it, remember?"
    mina ""

translate id w3MinaHotel_5443d8c2:
    # mc "I told you it was just a casual thing."
    mc ""

translate id w3MinaHotel_549c5695:
    # mina "So you did..."
    mina ""

translate id w3MinaHotel_5443d8c2_1:
    # mc "I told you it was just a casual thing."
    mc ""

translate id w3MinaHotel_431f7c7c:
    # mina "Hehe, casual's good~"
    mina ""

translate id w3MinaHotel_ff373020:
    # mina "Any guys out there you want to sleep with so we can make that happen?"
    mina ""

translate id w3MinaHotel_d7afdc3b:
    # mc "I-"
    mc ""

translate id w3MinaHotel_e1925431:
    # mina "Wait, don't answer that!"
    mina ""

translate id w3MinaHotel_94ceeef6:
    # mc "I wasn't."
    mc ""

translate id w3MinaHotel_ea61e618_1:
    # mina "Hehe~"
    mina ""

translate id w3MinaHotel_d75749c0:
    # mc "Soooooooo..."
    mc ""

translate id w3MinaHotel_0fd4d656_17:
    # "......"
    ""

translate id w3MinaHotel_ac8aea0c:
    # mc "...you watch a lot of porn, huh?"
    mc ""

translate id w3MinaHotel_a2b49ba4:
    # mina "Is that... {i}weird{/i}?"
    mina ""

translate id w3MinaHotel_10fb80e6:
    # mc "It would be one of the more normal things about you. What's with that look?"
    mc ""

translate id w3MinaHotel_493ce2ef:
    # mina "Hmmmgg, that's the same thing as calling me weird, right?"
    mina ""

translate id w3MinaHotel_01c4ec60:
    # mc "I meant it as a compliment. Sure, you could fill in the blank and say the absence of normal is {i}weird{/i}, but there's also {b}exceptional{/b} or {b}unique{/b}."
    mc ""

translate id w3MinaHotel_54b200e0:
    # mc "...but the porn thing, that's pretty normal in itself."
    mc ""

translate id w3MinaHotel_cbfb53b1_11:
    # mina "......"
    mina ""

translate id w3MinaHotel_c76dc221:
    # mina "...Ian thought it was weird for a girl to look at porn."
    mina ""

translate id w3MinaHotel_c78185ca:
    # mc "Heh, that is rich coming from a guy with his own homemade collection. Did he actually say as much?"
    mc ""

translate id w3MinaHotel_0a0d8ae6:
    # "That was, as usual for the pair, very bizarre."
    ""

translate id w3MinaHotel_8adf5b17:
    # mina "In so many different ways, over so many different things... it wasn't always direct, but I think he found a whole bunch of stuff about me off-putting."
    mina ""

translate id w3MinaHotel_c65069db:
    # mina "{i}I learned to keep my thoughts to myself and just go along with whatever he wanted to do{/i}."
    mina ""

translate id w3MinaHotel_39e395d6:
    # mc "Ah... {b}fuck Ian.{/b} Forget that shit."
    mc ""

translate id w3MinaHotel_1f0fd6d5:
    # mina "Heh, yeeeeeah. It seems obvious now, but it's funny how you don't realize how the {i}little{/i} things affect your self-esteem until you get some distance."
    mina ""

translate id w3MinaHotel_0fd4d656_18:
    # "......"
    ""

translate id w3MinaHotel_a20cefa7_17:
    # "..."
    ""

translate id w3MinaHotel_50d27b07:
    # mc "Do you know what might be fun?"
    mc ""

translate id w3MinaHotel_b1484de8:
    # mina "...a game of Jenga?"
    mina ""

translate id w3MinaHotel_6cbf6c94:
    # mc "{i}No{/i}. Let's go see what kind of boring porn they have on VOD and watch some together."
    mc ""

translate id w3MinaHotel_ddc716fa:
    # mina "We don't have to. I know what you're doing."
    mina ""

translate id w3MinaHotel_ba0709f5:
    # mc "What? There's nothing weird about a horny couple renting a hotel room and setting the mood with a little visual stimulation."
    mc ""

translate id w3MinaHotel_f64a3eed:
    # mina "That seems a bit out of order."
    mina ""

translate id w3MinaHotel_9b6c277c:
    # mc "Does it? I don't know about you, but..."
    mc ""

translate id w3MinaHotel_8e05287a:
    # mina "O-oomf!"
    mina ""

translate id w3MinaHotel_83c97c51:
    # mc "I'm still feeling pretty naughty."
    mc ""

translate id w3MinaHotel_a9e7139c:
    # mina "Hehe~ alright... it does sound pretty fun."
    mina ""

translate id w3MinaHotel_7b471f6f:
    # mc "Good."
    mc ""

translate id w3MinaPornWatch_ef1ab9d3:
    # "We spent the next hour and a half watching some random run-of-the-mill feature, the type you'd expect to see available from a chain hotel."
    ""

translate id w3MinaPornWatch_c874b1c4:
    # "It didn't {i}kick things off again{/i}, but it was fun in its own way. I hoped it might alleviate whatever dumb hangup Ian planted in her head."
    ""

translate id w3MinaPornWatch_6ba3415d:
    # mct "(Hmm... she seems familiar...)"
    mct ""

translate id w3MinaPornWatch_c1d2698b:
    # "It was more like one of those bad movie nights I used to have with Mom and Ian. We poked fun at the acting, sounds, and expressions the couple on screen made."
    ""

translate id w3MinaPornWatch_e475db91:
    # mina "What? You made a face just like that!"
    mina ""

translate id w3MinaPornWatch_eda68264:
    # mc "Of course, I did! Everyone looks stupid when they're fucking!"
    mc ""

translate id w3MinaPornWatch_f91eb48b:
    # mina "Hehe~ {b}I don't!{/b}"
    mina ""

translate id w3MinaPornWatch_0d74569b:
    # "By the time it was over, we had spoken on a number of topics. One way or another, it always came back to sex."
    ""

translate id w3MinaPornWatch_03dff96f:
    # "From sexual politics to Mina's conservative upbringing and her mother's contradictory love of men. And why wouldn't it, when sitting butt naked, skin-to-skin on a hotel bed?"
    ""

translate id w3MinaPornWatch_5b34f4db:
    # mina "Yeeeeeah, Felicia said that isn't as fun as it sounds..."
    mina ""

translate id w3MinaPornWatch_741ed3af:
    # "{i}...and why wouldn't it? My livelihood wholly revolved around sex."
    ""

translate id w3MinaPornWatch_7b1d0181:
    # mina "What about you? You have some, right?"
    mina ""

translate id w3MinaPornWatch_b0af3d9a:
    # "We touched on her myriad fantasies before she turned the question back on me."
    ""

translate id w3MinaPornWatch_53880114:
    # mc "Honestly, when you wanted to compare notes, I low-balled it with the window thing."
    mc ""

translate id w3MinaPornWatch_e7727f59:
    # mina "Compared to what? Using the bathroom on my chest?"
    mina ""

translate id w3MinaPornWatch_0fd4d656:
    # "......"
    ""

translate id w3MinaPornWatch_a20cefa7:
    # "..."
    ""

translate id w3MinaPornWatch_5895ecea:
    # mina "Ahem, ahh... ah, {i}ew{/i}."
    mina ""

translate id w3MinaPornWatch_b95dbe37:
    # mc "No, it's not that. Not necessarily..."
    mc ""

translate id w3MinaPornWatch_e19636ef:
    # mina "Not necessarily?!"
    mina ""

translate id w3MinaPornWatch_b1a95527:
    # mc "My biggest fantasy... it's..."
    mc ""

translate id w3MinaPornWatch_df805011:
    # "Mina was so good at putting a man to ease that I felt comfortable enough treading in deeper waters."
    ""

translate id w3MinaPornWatch_0196d641:
    # mc "My biggest fantasy is to {i}really{/i} let go."
    mc ""

translate id w3MinaPornWatch_d77edc2b:
    # mina "Let go of what?"
    mina ""

translate id w3MinaPornWatch_dc35bb28:
    # mc "...{b}everything{/b}. Not just my cares and worries, like a lot of people would mean, but my entire identity."
    mc ""

translate id w3MinaPornWatch_aebcd134:
    # mc "My memories, the love of the people in my life, {i}the love I feel in return{/i}, my purported morals, my past, my dreams, my ambitions--"
    mc ""

translate id w3MinaPornWatch_921cb053:
    # mina "--your doubts, fears, anxiety, the nagging voice in your head. {i}Everything{/i}. I got you."
    mina ""

translate id w3MinaPornWatch_32838df4:
    # mc "You understand, then?"
    mc ""

translate id w3MinaPornWatch_3874a949:
    # mina "All of it makes me feel uneasy too, but moving to California and reinventing yourself won't shut up the voice in your head."
    mina ""

translate id w3MinaPornWatch_9efa8d8d:
    # mc "You've considered that, huh?"
    mc ""

translate id w3MinaPornWatch_e841c1ee:
    # mina "Hasn't everyone dreamed of going to someplace no one knows you? {i}Heh{/i}, too scary!"
    mina ""

translate id w3MinaPornWatch_ea7f7054:
    # mina "Besides, you can't unbuild yourself; you can only... redirect. {i}I think{/i}."
    mina ""

translate id w3MinaPornWatch_3928a113:
    # "......."
    ""

translate id w3MinaPornWatch_a20cefa7_1:
    # "..."
    ""

translate id w3MinaPornWatch_a78f48ef:
    # mina "Ah, but what the hell do I know? What does any of this have to do with cumming?"
    mina ""

translate id w3MinaPornWatch_5ae7897c:
    # mc "I don't have any complex fantasies. All I want to do is to be mean."
    mc ""

translate id w3MinaPornWatch_b82aff48:
    # mina "Name-calling and hair-pulling?"
    mina ""

translate id w3MinaPornWatch_8d3d3154:
    # mc "Not quite..."
    mc ""

translate id w3MinaPornWatch_fd2c2556:
    # mina "We could try S&M sometime if you want."
    mina ""

translate id w3MinaPornWatch_a39c0d4b:
    # mc "That's a small part of it, but it's not exactly that..."
    mc ""

translate id w3MinaPornWatch_685397e6:
    # mina "Take your time. I won't judge you."
    mina ""

translate id w3MinaPornWatch_0fd4d656_1:
    # "......"
    ""

translate id w3MinaPornWatch_a20cefa7_2:
    # "..."
    ""

translate id w3MinaPornWatch_8dbdf67b:
    # mc "It's difficult to put into words. I want to capture a {i}feeling{/i} more than a specific act."
    mc ""

translate id w3MinaPornWatch_c26beba2:
    # "Mina patiently waited for me to continue while I ordered my thoughts."
    ""

translate id w3MinaPornWatch_5bd68e8a:
    # mc "There's a feeling of {i}revulsion{/i} that I want to turn inside out and direct at the world."
    mc ""

translate id w3MinaPornWatch_e29031ab:
    # mina "Like how sometimes I get so disgusted with things that I just want to scream and let it all out?"
    mina ""

translate id w3MinaPornWatch_eca16d72:
    # mc "Exactly like that. {i}Too much like that{/i}."
    mc ""

translate id w3MinaPornWatch_6b1cefb3:
    # mina "Oh, [mcf]... {b}every{/b} part of you is valid. There's not a thing wrong with wanting all your feelings seen and acknowledged."
    mina ""

translate id w3MinaPornWatch_4cd2bb83:
    # mc "Is that what I want...? Ah, ha..."
    mc ""

translate id w3MinaPornWatch_a5bb42e9:
    # man "What are you?!"
    man ""

translate id w3MinaPornWatch_46ff96a9:
    # vic "A-ahh, hhnnggg-♥ I'm a stupid whore!"
    vic ""

translate id w3MinaPornWatch_f5a5446a:
    # man "Do you like having sex for money?!"
    man ""

translate id w3MinaPornWatch_f950353b:
    # vic "Ahh, hhn-♥ Y-yees-♥"
    vic ""

translate id w3MinaPornWatch_c623e58e:
    # man "What's better, dick or money?!"
    man ""

translate id w3MinaPornWatch_2f488068:
    # vic "I, hhh- that's... ahh, hnggg... c-cccumming-"
    vic ""

translate id w3MinaPornWatch_f7e99025:
    # man "Haha, you bitch!"
    man ""

translate id w3MinaPornWatch_62fcd736:
    # mc "...sometimes I think all I want to do is find something holy and venerated and step on it until it's ugly, but that would just be a toddler having a temper tantrum."
    mc ""

translate id w3MinaPornWatch_59987616:
    # mina "Ah, [mcf]..."
    mina ""

translate id w3MinaPornWatch_50b69c21:
    # mina "I don't want to presume, but I enjoyed what we did today. I'd like to do it again... {i}many{/i} times, in fact."
    mina ""

translate id w3MinaPornWatch_88002a50:
    # mc "So did I. Ha, I'm glad I changed my mind about you."
    mc ""

translate id w3MinaPornWatch_66fa820c:
    # mina "Don't be smug! I'm trying to say something serious!"
    mina ""

translate id w3MinaPornWatch_8524ee3e:
    # mina "What I mean is... there are things I want to experience, things you want to work through... what if we helped each other?"
    mina ""

translate id w3MinaPornWatch_948f099d:
    # mc "Well, I won't say no to continuing this sort of relationship, but that last part..."
    mc ""

translate id w3MinaPornWatch_adaf4417:
    # mina "I know I've already brought this up, but I mean it even more now. You're helping me with my stuff; what if I could be an out for your feelings as well?"
    mina ""

translate id w3MinaPornWatch_ce766d7d:
    # mc "Truthfully, it sounds scary."
    mc ""

translate id w3MinaPornWatch_1b51af52:
    # mina "Well, heh...♥ You don't have to commit to anything concrete... just know that..."
    mina ""

translate id w3MinaPornWatch_98c4731f:
    # mina "I'm pathetic and eager to please? {i}Hehe{/i}... sorry, I'm getting a little self-conscious here."
    mina ""

translate id w3MinaPornWatch_242e5d13:
    # mina "...what I really want to say is I'd like to experience a {b}relationship{/b} where you don't have to measure out what you give and get."
    mina ""

translate id w3MinaPornWatch_83f91ef0:
    # "In this moment, I couldn't help but think of the monogamy I promised Hana."
    ""

translate id w3MinaPornWatch_f13dbb9c:
    # mct "(Gah... why does life pile so many good things in my lap?)"
    mct ""

translate id w3MinaPornWatch_976bda21:
    # "Life's really piling many good things in my lap lately, huh..."
    ""

translate id w3MinaPornWatch_e7972ce2:
    # mc "When you say relationship, do you mean...?"
    mc ""

translate id w3MinaPornWatch_3bd897d4:
    # mina "I'll be blunt. When I think of the future, I picture a lot of things, and one of those things is having someone to call {i}my{/i} own, but that's {i}the future{/i}."
    mina ""

translate id w3MinaPornWatch_a8c65893:
    # mina "I don't want to be one of those women who jump from relationship to relationship, never able to live for themselves. My mother is like that."
    mina ""

translate id w3MinaPornWatch_db1b6902:
    # mc "She's on husband number four, right?"
    mc ""

translate id w3MinaPornWatch_134c1bbf:
    # mina "I know myself enough to see that's where I could head. I mean... even now, there's a voice in my head telling me that {i}I need someone{/i}, but..."
    mina ""

translate id w3MinaPornWatch_3b00fe0d:
    # mina "I want to be a more self-reliant Mina and have the next someone come from the benefit of perspective."
    mina ""

translate id w3MinaPornWatch_4d4b0e65:
    # mc "The fact that you can put it into words makes it seem like you're already pointed in that direction."
    mc ""

translate id w3MinaPornWatch_5425e82a:
    # mina "Hehe~ I hope! And maybe one day, knowing myself... I {i}will{/i} want something from you. If that happens, I'll try not to be angry if you don't feel the same."
    mina ""

translate id w3MinaPornWatch_58e0df4a:
    # mc "You... {i}really{/i} make it easy for a guy."
    mc ""

translate id w3MinaPornWatch_ff7ce9bb:
    # mina "I'd ask the same of you... not that you ever would want something {b}more{/b}, right?"
    mina ""

translate id w3MinaPornWatch_0fd4d656_2:
    # "......"
    ""

translate id w3MinaPornWatch_a20cefa7_3:
    # "..."
    ""

translate id w3MinaPornWatch_933d2730:
    # mina "Kidding! Kidding! Testing the waters! Kidding!"
    mina ""

translate id w3MinaPornWatch_d97162c6:
    # "I couldn't help but share in her laugh."
    ""

translate id w3MinaPornWatch_f7ce8037:
    # mina "So... I can cross fucking in a hotel off my list."
    mina ""

translate id w3MinaPornWatch_406f5c1c:
    # mc "You have a list?"
    mc ""

translate id w3MinaPornWatch_d8299e5a:
    # mina "That was, uh... what I wanted help with the other day."
    mina ""

translate id w3MinaPornWatch_fb22156c:
    # mc "Oh, yeah. I forgot about that after you--"
    mc ""

translate id w3MinaPornWatch_85a0b7f5:
    # mina "{b}Yeeeep.{/b}"
    mina ""

translate id w3MinaPornWatch_7b9366d3:
    # mc "That was an easy one."
    mc ""

translate id w3MinaPornWatch_8f27de10:
    # mina "--and fun one to boot, hehe~"
    mina ""

translate id w3MinaPornWatch_e5b8d9b1:
    # mc "What else do you have on your list?"
    mc ""

translate id w3MinaPornWatch_5f7fbc82:
    # mina "Hmmm, weeeeeell..."
    mina ""

translate id w3MinaPornWatch_230409e3:
    # "Mina looked like she was giving it serious thought."
    ""

translate id w3MinaPornWatch_d3532a93:
    # mc "Tell me the big ones."
    mc ""

translate id w3MinaPornWatch_5420d437:
    # mina "Something big? You didn't even give me anything concrete!"
    mina ""

translate id w3MinaPornWatch_bebe3312:
    # mc "What happened to not measuring things out? Besides..."
    mc ""

translate id w3MinaPornWatch_ed0e3ba7:
    # "*Chwup, fhwhup~*"
    ""

translate id w3MinaPornWatch_b87f06cc:
    # mina "Ahh, hhnn- n-no f-fair...!"
    mina ""

translate id w3MinaPornWatch_34c36e0b:
    # mina "Hnnngg..."
    mina ""

translate id w3MinaPornWatch_d6a1c0af:
    # mc "C'mon, just a few immediate ones you feel like you're ready to try? We're talking about making them a reality, remember?"
    mc ""

translate id w3MinaPornWatch_339b33de:
    # mina "Ah, alright... in no order..."
    mina ""

translate id w3MinaPornWatch_a29fcdaa:
    # mina "Sex outside."
    mina ""

translate id w3MinaPornWatch_60245d3f:
    # mc "Doable."
    mc ""

translate id w3MinaPornWatch_f57abb73:
    # "I knew I was capable of that thanks to Rosalind."
    ""

translate id w3MinaPornWatch_a5776f5e:
    # mina "I'd like to try being in the lead for once. Like {i}really{/i} in the lead."
    mina ""

translate id w3MinaPornWatch_04f20bbf:
    # mc "Very doable."
    mc ""

translate id w3MinaPornWatch_f06ec7fc:
    # mina "Some costumes and roleplay would be fun."
    mina ""

translate id w3MinaPornWatch_bf8e2182:
    # mc "Extremely doable, but isn't that your job?"
    mc ""

translate id w3MinaPornWatch_fb7fd666:
    # mina "I should be good at it, right? It could double as practice!"
    mina ""

translate id w3MinaPornWatch_cd2c0272:
    # mc "What would you like to roleplay?"
    mc ""

translate id w3MinaPornWatch_7e1e4343:
    # mina "I... {b}don't know.{/b}"
    mina ""

translate id w3MinaPornWatch_a0157f43:
    # mc "Ah, well... we can figure out the specifics later. Anything else?"
    mc ""

translate id w3MinaPornWatch_d8fbf2c3:
    # mina "I want to... I want to... ah..."
    mina ""

translate id w3MinaPornWatch_2570b6e0:
    # mina "I want to try having sex with a woman."
    mina ""

translate id w3MinaPornWatch_c2756dd9:
    # mc "Even if we limit it to someone we both know, that's also workable."
    mc ""

translate id w3MinaPornWatch_c9da6d8c:
    # mina "...eh?! You think? You can't just ask someone, especially someone you know to--"
    mina ""

translate id w3MinaPornWatch_8d76ac4d:
    # mc "Sure you can."
    mc ""

translate id w3MinaPornWatch_03418f5d:
    # mc "Veronica was super into you."
    mc ""

translate id w3MinaPornWatch_8fda06c2:
    # mina "She was?!"
    mina ""

translate id w3MinaPornWatch_6229deed:
    # mc "She stopped short of molesting you the day we joined her gym. How the hell could you not tell?"
    mc ""

translate id w3MinaPornWatch_825cee78:
    # mina "Ah, umm... {i}all girls school{/i}?"
    mina ""

translate id w3MinaPornWatch_c724560b:
    # mc "That's your excuse for everything, huh?"
    mc ""

translate id w3MinaPornWatch_febe7749:
    # mina "It's a good one!"
    mina ""

translate id w3MinaPornWatch_db1ff465:
    # mc "We could also ask Hana. You two seemed friendly enough."
    mc ""

translate id w3MinaPornWatch_03732d5a:
    # mina "Just because two women got along socially ONE time doesn't mean--"
    mina ""

translate id w3MinaPornWatch_7f86f6f8:
    # mc "I'm just putting it out there as an option. Neither of us could go to a bar and pick up a third without embarrassing ourselves."
    mc ""

translate id w3MinaPornWatch_37bd27fa:
    # mct "(Although, kinda weird fucking idea to bring up when I just promised her monogamy... yeeeah, that {b}could{/b} go south, but great things can't be done without risk.)"
    mct ""

translate id w3MinaPornWatch_09a43f7e:
    # mc "You don't know her {i}that{/i} well, so if we could get her to agree, there is no chance of ruining a good thing."
    mc ""

translate id w3MinaPornWatch_5f978349:
    # mina "Do you think she would be down?"
    mina ""

translate id w3MinaPornWatch_ead81e15:
    # mina "Do you two have the kind of relationship where you could just ask-"
    mina ""

translate id w3MinaPornWatch_09934654:
    # mc "Not exactly, but we could all get together and see how it goes."
    mc ""

translate id w3MinaPornWatch_b858bd73:
    # mc "You'll have to put in some work too. Try to be a little more seductive than you were with me."
    mc ""

translate id w3MinaPornWatch_0f4b3287:
    # mina "Ah, yy-- aahh...!"
    mina ""

translate id w3MinaPornWatch_de325fe1:
    # mc "Sorry, it's just too fun - and hey, if you don't want to try either of those, there's always Felicia."
    mc ""

translate id w3MinaPornWatch_6d3e4966:
    # mina "Felicia?! I c--"
    mina ""

translate id w3MinaPornWatch_03ae4f63:
    # mc "You could, and she would. You know she's permanently down to fuck, but even if she wasn't, she'd probably even do it as a favor."
    mc ""

translate id w3MinaPornWatch_65d8e79c:
    # mina "I wouldn't want her to do it as a favor!"
    mina ""

translate id w3MinaPornWatch_e2103406:
    # mina "She's my only female friend. That sounds like a bad idea."
    mina ""

translate id w3MinaPornWatch_bda3bccc:
    # mc "We could also ask Felicia. If anyone would be amenable, it'd be here. You might even be most comfort--"
    mc ""

translate id w3MinaPornWatch_9c479d55:
    # mina "A-ahh... you think? She's my only female friend... that sounds like a bad idea."
    mina ""

translate id w3MinaPornWatch_a384e95c:
    # mc "You know she's permanently down to fuck, but she would probably do it as a favor."
    mc ""

translate id w3MinaPornWatch_65d8e79c_1:
    # mina "I wouldn't want her to do it as a favor!"
    mina ""

translate id w3MinaPornWatch_e34cfd66:
    # mc "I'm just saying it's an option. I don't think either of us could go to a bar and pick up a third without embarrassing ourselves--"
    mc ""

translate id w3MinaPornWatch_a2b73f28:
    # mc "You and Hana had some chemistry."
    mc ""

translate id w3MinaPornWatch_69545968:
    # mina "Just because two women got along socially one time doesn't mean--"
    mina ""

translate id w3MinaPornWatch_7f86f6f8_1:
    # mc "I'm just putting it out there as an option. Neither of us could go to a bar and pick up a third without embarrassing ourselves."
    mc ""

translate id w3MinaPornWatch_37bd27fa_1:
    # mct "(Although, kinda weird fucking idea to bring up when I just promised her monogamy... yeeeah, that {b}could{/b} go south, but great things can't be done without risk.)"
    mct ""

translate id w3MinaPornWatch_09a43f7e_1:
    # mc "You don't know her {i}that{/i} well, so if we could get her to agree, there is no chance of ruining a good thing."
    mc ""

translate id w3MinaPornWatch_5f978349_1:
    # mina "Do you think she would be down?"
    mina ""

translate id w3MinaPornWatch_ead81e15_1:
    # mina "Do you two have the kind of relationship where you could just ask-"
    mina ""

translate id w3MinaPornWatch_09934654_1:
    # mc "Not exactly, but we could all get together and see how it goes."
    mc ""

translate id w3MinaPornWatch_b858bd73_1:
    # mc "You'll have to put in some work too. Try to be a little more seductive than you were with me."
    mc ""

translate id w3MinaPornWatch_0f4b3287_1:
    # mina "Ah, yy-- aahh...!"
    mina ""

translate id w3MinaPornWatch_de325fe1_1:
    # mc "Sorry, it's just too fun - and hey, if you don't want to try either of those, there's always Felicia."
    mc ""

translate id w3MinaPornWatch_6d3e4966_1:
    # mina "Felicia?! I c--"
    mina ""

translate id w3MinaPornWatch_03ae4f63_1:
    # mc "You could, and she would. You know she's permanently down to fuck, but even if she wasn't, she'd probably even do it as a favor."
    mc ""

translate id w3MinaPornWatch_65d8e79c_2:
    # mina "I wouldn't want her to do it as a favor!"
    mina ""

translate id w3MinaPornWatch_e2103406_1:
    # mina "She's my only female friend. That sounds like a bad idea."
    mina ""

translate id w3MinaPornWatch_a4319a6c:
    # mc "You think? She's pretty fucking casual about--"
    mc ""

translate id w3MinaPornWatch_a9950873:
    # mc "Who would you be more comfortable with than Felicia? You know she's always down to--"
    mc ""

translate id w3MinaPornWatch_526ef592:
    # mina "Felicia? She's my only female friend. I couldn't. it'd be so--"
    mina ""

translate id w3MinaPornWatch_15cf8251:
    # mc "Would it?"
    mc ""

translate id w3MinaPornWatch_ec1c3481:
    # mina "I, uhh... I don't know? It would be new territory for me."
    mina ""

translate id w3MinaPornWatch_07c82104:
    # mc "You know she'd be down, right?"
    mc ""

translate id w3MinaPornWatch_0b6a0848:
    # mina "Y-yeeah... {i}probably{/i}."
    mina ""

translate id w3MinaPornWatch_f045ffa6:
    # mc "She'd probably even do it as a favor."
    mc ""

translate id w3MinaPornWatch_d033e527:
    # mina "I don't want her to do it as a favor!"
    mina ""

translate id w3MinaPornWatch_a5ac7a75:
    # mc "It could be a good chance for you two to grow closer."
    mc ""

translate id w3MinaPornWatch_e1958ec5:
    # mina "You just want to sleep with both of--"
    mina ""

translate id w3MinaPornWatch_332673bb:
    # mc "Hey, hey... this is your list. I'm just putting it out there as an option."
    mc ""

translate id w3MinaPornWatch_460cb94a:
    # mc "I don't think either of us could pick up a third at a bar without embarrassing ourselv--"
    mc ""

translate id w3MinaPornWatch_f37fca07:
    # mina "I want to... I want to--"
    mina ""

translate id w3MinaPornWatch_52a00c2f:
    # "*Ring, ring*"
    ""

translate id w3MinaPornWatch_885687ab:
    # mc "Ah, hold that thought."
    mc ""

translate id w3MinaPornWatch_853c5276:
    # mct "(I should've just let it go to--)"
    mct ""

translate id w3MinaPornWatch_be4317fd:
    # "Unknown number. I usually wouldn't answer, but something in my mind {i}told{/i} me I should."
    ""

translate id w3MinaPornWatch_4e85cb4d:
    # mc "Hello?"
    mc ""

translate id w3MinaPornWatch_f3843656:
    # sophia "Hello, Mr. [mcl]. I didn't think we would speak again so soon."
    sophia ""

translate id w3MinaPornWatch_db606737:
    # mc "...you're the one who called me, Miss Lundgren."
    mc ""

translate id w3MinaPornWatch_01a72a15:
    # sophia "It's not a social call. Well, not one on my behalf."
    sophia ""

translate id w3MinaPornWatch_1734049e:
    # sophia "How soon will you arrive home?"
    sophia ""

translate id w3MinaPornWatch_5f01280f:
    # mc "I don't know. I wasn't planning on it anytime-"
    mc ""

translate id w3MinaPornWatch_aa69de74:
    # sophia "Mr. Van Doren has graciously found time in his schedule to educate you. He'll be here in one hour, {b}exactly.{/b}"
    sophia ""

translate id w3MinaPornWatch_01468a4f:
    # mc "What? You can't just--"
    mc ""

translate id w3MinaPornWatch_f6e4e62d:
    # sophia "Please be here. It's to your benefit."
    sophia ""

translate id w3MinaPornWatch_f6dc581a:
    # mc "Huh, wait- ah, shit."
    mc ""

translate id w3MinaPornWatch_ac466f98:
    # mina "Something wrong?"
    mina ""

translate id w3MinaPornWatch_46128a2d:
    # mc "My neighbor had a pipe burst. I've got to-"
    mc ""

translate id w3MinaPornWatch_f596588f:
    # mina "Sure. No problem."
    mina ""

translate id w3MinaPornWatch_87bd2e36:
    # mc "You're not mad I have to run off?"
    mc ""

translate id w3MinaPornWatch_c2b16ae8:
    # mina "Why would I be? Saves me from having to awkwardly end our fun because I {i}really{/i} don't want to spend an entire night at a hotel without my stuff."
    mina ""

translate id w3MinaPornWatch_76512740:
    # mc "Yeeeah, what would we have done? Get dinner and watch TV?"
    mc ""

translate id w3MinaPornWatch_91f68f60:
    # mina "That wouldn't be so bad sometime, but not without access to my make up."
    mina ""

translate id w3MinaPornWatch_b11bda7d:
    # mc "...what were we talking about before, ah. Yeah."
    mc ""

translate id w3MinaPornWatch_d93a83e1:
    # mina "Call me tonight, okay?"
    mina ""

translate id w3MinaPornWatch_d1c9623b:
    # mc "Sure, will do."
    mc ""

translate id w3MinaPornWatch_7eb85110:
    # "Ah, goddamn it. I {b}hate{/b} rushing. What the hell does that old man even want."
    ""

translate id june15end_0546ea10:
    # "When I got back, I should've been more surprised at what I found, but honestly... part of me expected it."
    ""

translate id june15end_6e4df342:
    # abel "Oh, good you're here."
    abel ""

translate id june15end_37f1c879:
    # "Abel was sitting comfortably in a chair, a glass of tea in front of him, while Sophia idled in the kitchen."
    ""

translate id june15end_e5d3b6ea:
    # abel "I feared you might stand me up like the ugly girl at a winter formal."
    abel ""

translate id june15end_9e7147a9:
    # mc "You broke into my home?"
    mc ""

translate id june15end_b00cb76b:
    # "A quick inspection of the room told me it was likely just us, although my mind doubled back and second-guessed the conspicuously suited man I had passed at the building's entrance."
    ""

translate id june15end_1f8aa65f:
    # abel "I did, although \"breaking in\" would be a generous term for the piddly security of this building."
    abel ""

translate id june15end_9ffc3b74:
    # abel "I do apologize about that, but I don't like visiting places that aren't verified by my staff."
    abel ""

translate id june15end_ada378e6:
    # mc "What kind of danger could you possibly expect in a pl--"
    mc ""

translate id june15end_f38ec420:
    # sophia "Look here."
    sophia ""

translate id june15end_a1fede74:
    # mc "What is...?"
    mc ""

translate id june15end_b7c5e2f4:
    # sophia "It's a listening audio device that broadcasts sound above a specific decibel."
    sophia ""

translate id june15end_47303c33:
    # mc "...I've been spied on?"
    mc ""

translate id june15end_96c071a0:
    # sophia "I don't think so. This is the only one we found, likely forgotten after the others had been shoddily removed."
    sophia ""

translate id june15end_3d2359dd:
    # sophia "What's more, the battery likely ran out some time ago. This model can operate for 75 days in standby mode, so I suspect a previous tenant who had something to worry about."
    sophia ""

translate id june15end_c5c84d8d:
    # abel "Point is, I only go places that are verified by my staff. Forgive the intrusion."
    abel ""

translate id june15end_0fd4d656:
    # "......"
    ""

translate id june15end_a20cefa7:
    # "..."
    ""

translate id june15end_1dbf79d0:
    # mc "Why are you here?"
    mc ""

translate id june15end_562a04b4:
    # abel "Very respectably to the point. Good eye contact too."
    abel ""

translate id june15end_16f3e1de:
    # mc "What is this, some kind of performance review?"
    mc ""

translate id june15end_62df4d1c:
    # abel "Everything in life is performative, Mr. [mcl] - and every performance is judged."
    abel ""

translate id june15end_02f3aa93:
    # mc "Like how you broke into my home? Is that supposed to imply something?"
    mc ""

translate id june15end_18cf3582:
    # abel "I hoped it might show how serious I am about this conversation."
    abel ""

translate id june15end_c5239cab:
    # abel "I don't make house calls. {b}People come to me.{/b}"
    abel ""

translate id june15end_adec8486:
    # abel "Sit down, please. It's rude to stand."
    abel ""

translate id june15end_08f24ef5:
    # "Part of me wanted to desperately tell the old man to fuck off, but there was something about him that made refusing seem impossible."
    ""

translate id june15end_e71877a2:
    # "Whatever. {i}Let's talk{/i}."
    ""

translate id june15end_1dbf79d0_1:
    # mc "Why are you here?"
    mc ""

translate id june15end_ddd2eb6c:
    # abel "Would you like Sophia to provide you some relief?"
    abel ""

translate id june15end_1dbf79d0_2:
    # mc "Why are you here?"
    mc ""

translate id june15end_eadbf388:
    # abel "The third time is less charming, but I understand your desire to cut to the heart of matters."
    abel ""

translate id june15end_a5b9aac6:
    # abel "I'm an overly familiar old man, but I do come bearing sound advice. I looked into you."
    abel ""

translate id june15end_22af2b6c:
    # mc "You're not the first, unfortunately..."
    mc ""

translate id june15end_dc48d043:
    # abel "I am indeed the most thorough though, so I feel comfortable prescribing the following verdict."
    abel ""

translate id june15end_9fe69f20:
    # abel "{i}There is nothing remotely remarkable about you.{/i} You are shockingly unexceptional."
    abel ""

translate id june15end_fa45f281:
    # mc "Thanks..."
    mc ""

translate id june15end_761a0e22:
    # abel "At first, I thought it would be appropriate to toy with you, just as he has seen fit to do with my Sophia, but your connection to Charles is tenuous at best."
    abel ""

translate id june15end_35cdbd42:
    # abel "And even if you were his protégé, I doubt he would mind me sniffing around. Undoubtedly, he would find it an interesting note in {i}his{/i} story."
    abel ""

translate id june15end_2b35b6fe:
    # mc "..."
    mc ""

translate id june15end_c366ff8b:
    # "What the fuck could I say to any of this? So I just kept my mouth shut and waited for him to further enlighten me."
    ""

translate id june15end_557ba9b8:
    # abel "No, I'm here talking to you for a less petty purpose. How much do you know about the people who now surround you?"
    abel ""

translate id june15end_b41bb3b9:
    # mc "They're not the savory sort."
    mc ""

translate id june15end_f960d9c7:
    # abel "No one of consequence ever is, but do you know the specifics?"
    abel ""

translate id june15end_797ae6d9:
    # mc "Just what I could find on the internet."
    mc ""

translate id june15end_4fd47d69:
    # abel "Ah, so nothing at all."
    abel ""

translate id june15end_067e3af9:
    # abel "No one in this world {b}deserves{/b} a fair shake, despite what they tell you, but thankfully you fall within my providence. You see there are two types of people in this world..."
    abel ""

translate id june15end_e4f71dd2:
    # mc "There's probably a lot more than two..."
    mc ""

translate id june15end_d2a93b1f:
    # abel "Hush. There are those who think nothing of killing a spider in their own home. They simply can't be bothered to do otherwise."
    abel ""

translate id june15end_86d04e98:
    # abel "The other kind of person is the type who takes time to capture and release the spider safely, cognizant that a mere bug doesn't choose to encroach where it doesn't belong."
    abel ""

translate id june15end_6db1adb0:
    # abel "Maybe I'm getting soft in my old age, but I'm here because you should be familiar with your environment. So you can have no one else to blame but yourself if you are dispossessed."
    abel ""

translate id june15end_7b51b8b5:
    # mc "{b}Please{/b}, educate me."
    mc ""

translate id june15end_a48ca847:
    # "What else could I say? Dr. Van Doren's motives aside, I wish he wasn't here, but it would be naïve to think I could make it through years at the Carnation Club without facing some obvious truths."
    ""

translate id june15end_fb0af589:
    # mct "(That is... if he even tells me the truth. {i}Why the hell is he truly here{/i}?)"
    mct ""

translate id june15end_0f7c078e:
    # abel "Do you trust your bosses?"
    abel ""

translate id june15end_c801ef2c:
    # mc "One's a gangster, one's a bomb-making hedonist, and one of them is Kathleen Pulman; I trust them to have their best interests at heart, but that is true of most employer-employee relationships isn't it?"
    mc ""

translate id june15end_13a32f94:
    # mc "Part of working for {i}anyone{/i} is becoming their best interest."
    mc ""

translate id june15end_c40c1c85:
    # abel "I'm wondering what I'm even doing here."
    abel ""

translate id june15end_eb2753d8:
    # mc "For the record, I don't trust you have good intentions either."
    mc ""

translate id june15end_985f1d4c:
    # abel "Because of the present circumstances?"
    abel ""

translate id june15end_65b33118:
    # mc "No. Because you're a head of a major pharmaceutical company, cavorting with pimps, refining a sex drug alongside your whore Nazi scientist."
    mc ""

translate id june15end_7ea94438:
    # sophia "Nazi?!"
    sophia ""

translate id june15end_6864e56b:
    # mc "No offense Sophia, but being blonde, sharply dressed in black, and testing drugs on people without their consent does conjure up a specific image."
    mc ""

translate id june15end_26020803:
    # abel "You're mad."
    abel ""

translate id june15end_707e1de5:
    # mc "You claim you're here to be candid? I'll be blunt too."
    mc ""

translate id june15end_a1f3f00e:
    # sophia "You have no idea who--"
    sophia ""

translate id june15end_f561d155:
    # abel "Pffh, hahaha! Hahahaha! I like you! I'm REALLY starting to think I've wasted my time."
    abel ""

translate id june15end_340c1b07:
    # mc "{b}Don't{/b}. Educate me about my environment and the people I work for."
    mc ""

translate id june15end_36106b17:
    # abel "Okay, let's start with the gangster. You have a vague impression of his character, but you don't know his crimes do you?"
    abel ""

translate id june15end_fca52711:
    # mc "No."
    mc ""

translate id june15end_cbe380be:
    # abel "It's so easy to get lulled into a sense of security when you're only exposed to a particular side of a person. Still, the man is an extortionist at heart."
    abel ""

translate id june15end_cefc9937:
    # abel "Loan sharking, protection rackets, insurance scams, gambling, prostitution. He's very much destroyed lives - not to mention he's killed people."
    abel ""

translate id june15end_cd6ba9d5:
    # mc "...*ahem*, yeah?"
    mc ""

translate id june15end_203da5c9:
    # abel "Well, he's not a convicted murderer. He's a person of interest in four disappearances."
    abel ""

translate id june15end_af426698:
    # mc "That can mean a lot of things."
    mc ""

translate id june15end_f5b78653:
    # abel "{i}You want it to.{/i} And if he's not responsible, he's undoubtedly a factor in a number of suicides."
    abel ""

translate id june15end_32f5b7bf:
    # abel "Have you ever wondered if he knew your mother?"
    abel ""

translate id june15end_085722e2:
    # mc "What?!"
    mc ""

translate id june15end_736829eb:
    # abel "How many videos did she shoot? How big do you think the porn industry of Morehead Hills is?"
    abel ""

translate id june15end_0fd4d656_1:
    # "......"
    ""

translate id june15end_a20cefa7_1:
    # "..."
    ""

translate id june15end_2dd832e4:
    # mct "(What the fuck, does everyone--)"
    mct ""

translate id june15end_ec8d54f2:
    # abel "It's better not to know, I suppose. I can tell you who HAS killed people."
    abel ""

translate id june15end_a1140c23:
    # mc "Warren?"
    mc ""

translate id june15end_d3655876:
    # abel "For one... that man is a menace."
    abel ""

translate id june15end_92625396:
    # abel "It takes a lot to get kicked out of the kind of mercenary company that would take a piece of trash like him, but he's pulled it off multiple times - for the same company, no less."
    abel ""

translate id june15end_d4ec4d7e:
    # abel "A by-product of being good at your job, but his \"Sandman\" moniker wasn't given to him because he excelled at fighting. {b}No{/b}, it has more to do with why he's currently a wanted man, living out of your club."
    abel ""

translate id june15end_0fd4d656_2:
    # "......"
    ""

translate id june15end_a20cefa7_2:
    # "..."
    ""

translate id june15end_b196a9b0:
    # mct "(Holy shit, but I guess no fucking surprise there...)"
    mct ""

translate id june15end_e01e756f:
    # mc "You said for one? Who's the other?"
    mc ""

translate id june15end_8e8d8553:
    # abel "Who do you suspect?"
    abel ""

translate id june15end_6f79b5d7:
    # mct "(Jacob...)"
    mct ""

translate id june15end_71f47df4:
    # abel "Warren's partner is more professional but a killer all the same. He got into trouble as a young man in Quebec that saw him flee the country."
    abel ""

translate id june15end_c1e727a9:
    # abel "After his time in the French Foreign Legion, he came to the States. Worked protection for various unsavory sorts."
    abel ""

translate id june15end_fcd077f4:
    # abel "Did other jobs too. The high-risk kind, the sort you contract out so it doesn't lead back to you."
    abel ""

translate id june15end_405ec5df:
    # abel "I don't think he took to it, considering he now guards a brothel. Still, it begs the question of what the man remains capable of."
    abel ""

translate id june15end_a1990a5e:
    # mc "And Dr. Kohler?"
    mc ""

translate id june15end_fe25492e:
    # abel "He's everything you imagine him to be, but worse."
    abel ""

translate id june15end_44555a37:
    # abel "War profiteering, bribery, falsification of documents, state-sanctioned smuggling. He's done a lot for his country."
    abel ""

translate id june15end_a9299100:
    # abel "A true American hero. A self-made man. {b}An irredeemable nihilist{/b}, of the lowest order - and that's before you even get into his hobbies..."
    abel ""

translate id june15end_ce9dbe25:
    # mc "Are you any better? You profit obscenely off the backs of sick people. {i}You lobby{/i}."
    mc ""

translate id june15end_bee7e063:
    # abel "I serve a purpose that will merit no appreciable distinction from the likes of you."
    abel ""

translate id june15end_2a3f5f4f:
    # mc "{i}Riiiight{/i}... what about Kathleen? You neglected to mention her."
    mc ""

translate id june15end_72d10b88:
    # abel "She has Charles' nasty disposition but none of his accomplishments. As you noted, you can count on her to act in her own best interests. The only problem is I don't think she knows what that is."
    abel ""

translate id june15end_84428be6:
    # mct "(He talks like he knows something...)"
    mct ""

translate id june15end_e5a02fa5:
    # "It pisses me off."
    ""

translate id june15end_68d48519:
    # abel "She's harmless."
    abel ""

translate id june15end_bb686ac7:
    # mct "(I'm sure some people would disagree with that...)"
    mct ""

translate id june15end_03d24cf6:
    # abel "You're young; you have your whole life ahead of you. You may even make something of yourself if you keep your eyes open."
    abel ""

translate id june15end_d011c8b1:
    # abel "Success is built upon and sustained by reading the wind and the changing tides. Nothing in this world lasts forever, from vaunted intuitions, to your favorite ice cream shop, and especially criminal enterprises."
    abel ""

translate id june15end_07af68fd:
    # mc "Are you suggesting I should quit?"
    mc ""

translate id june15end_4ea983de:
    # abel "Not in the least! Quite the contrary, I believe you'd find quitting a more strenuous process. No, where you are is best."
    abel ""

translate id june15end_61ca0579:
    # abel "I'm suggesting you pay attention, so you don't get caught unaware."
    abel ""

translate id june15end_8cbf2c0e:
    # mct "(He's DEFINITELY getting at something.)"
    mct ""

translate id june15end_aa0408bd:
    # abel "If you ever find yourself in a bind, give Sophia a call. You should have her personal number now."
    abel ""

translate id june15end_c9ba147e:
    # "Boop!"
    ""

translate id june15end_2f1217ff:
    # mc "What kind of bind would I find myself in?"
    mc ""

translate id june15end_9046c15e:
    # abel "Perhaps the kind that would see your home bugged and surveilled. Do you know who lived here last?"
    abel ""

translate id june15end_7befe0d5:
    # mc "I do..."
    mc ""

translate id june15end_c5467fa5:
    # "Darius. The man who allegedly blackmailed Kathleen before running off."
    ""

translate id june15end_e0966d66:
    # abel "Do you know about him? Well, whatever he did, I would avoid making that same mistake."
    abel ""

translate id june15end_b79801be:
    # mc "Be straight with me. You know something."
    mc ""

translate id june15end_7e60c39c:
    # abel "I'm only making an inference, but it's entirely possible that the listening device has been here for years. I just wanted to underline my point."
    abel ""

translate id june15end_affd7977:
    # sophia "It's a recent model. {b}A year old{/b}."
    sophia ""

translate id june15end_9122e818:
    # abel "Is it? That's good to know."
    abel ""

translate id june15end_bd4b393e:
    # abel "We shall take our leave now. Sorry for the intrusion."
    abel ""

translate id june15end_ff4175bb:
    # mc "Wait... so let me get this straight."
    mc ""

translate id june15end_a7bb8db4:
    # mc "An important man like you came here to give me vague advice out of the goodness of your heart, is that it?"
    mc ""

translate id june15end_01f3a317:
    # abel "[mcf]... let me ask you something... what could I {i}possibly{/i} get from a nobody like you?"
    abel ""

translate id june15end_d37f72ba:
    # abel "Think about it. Makes no sense."
    abel ""

translate id june15end_ef7936d4:
    # abel "This cost me nothing."
    abel ""

translate id june15end_0fd4d656_3:
    # "......"
    ""

translate id june15end_a20cefa7_3:
    # "..."
    ""

translate id june15end_21e59b50:
    # sophia "Have a good afternoon, [mcf]."
    sophia ""

translate id june15end_1abc98c7:
    # "They showed themselves out just as they had let themselves in, leaving me confused."
    ""

translate id june15end_668b737f:
    # "I hadn't learned anything I hadn't {i}known{i}. August was a piece of shit criminal, Chuck wasn't as I remembered him, and the two soldiers guarding the place had a body count."
    ""

translate id june15end_ac8b390d:
    # "None of that should surprise me, even if I tried not to think about it, but I felt disturbed and unsafe."
    ""

translate id june15end_aafb9db3:
    # mct "(I mean, fuck... I'm just a pre-med student from the suburbs.)"
    mct ""

translate id june15end_e8cae819:
    # mc "What the hell was the deal with that bug?"
    mc ""

translate id june15end_16c257c0:
    # "Was that {i}really{/i} the only one? Were there more? Did they plant it? Did they pretend it was here - no {i}why would they do that{/i}?"
    ""

translate id june15end_12463c4a:
    # "It made no sense, but I knew the rest of my night would be spent researching how to detect listening devices. I wouldn't have peace of mind otherwise."
    ""

translate id june15end_74c4a44f:
    # mct "(Makes no sense, makes no sense...)"
    mct ""

translate id june15end_20e3b24c:
    # "Dr. Van Doren's question repeated in my head."
    ""

translate id june15end_4cd0236a:
    # "What could he possibly get from a nobody like me?"
    ""

translate id june15end_2a8df5f8:
    # mct "(God damn it, I don't like people coming and going as they please.)"
    mct ""

translate id june15end_b8cfd8d1:
    # "*Knock, knock*"
    ""

translate id june15end_a8528247:
    # vic "[mcf]?"
    vic ""

translate id june15end_2561a1cc:
    # "What I did know is I didn't want to sleep in my apartment tonight. So tomorrow, I'd ensure I wasn't being spied on, but for now..."
    ""

translate id june15end_8a46b6e7:
    # mc "Heeeeeeey. Fancy seeing you here."
    mc ""

translate id june15end_26c1ff2f:
    # "The comfort of home."
    ""

translate id june15end_e304c06c:
    # vic "Twice in so few days? That's--"
    vic ""

translate id june15end_99dd840d:
    # vic "Hey, hun! What are you doing here?"
    vic ""

translate id june15end_c77e8633:
    # mc "Dropping in unannounced is a family trait."
    mc ""

translate id june15end_8f86b57e:
    # vic "..."
    vic ""

translate id june15end_0575293d:
    # mc "Are you busy? Want to watch some movies?"
    mc ""

translate id june15end_4672d772:
    # vic "Are you ok- ah..."
    vic ""

translate id june15end_d3c44e7c:
    # vic "{i}Always{/i}."
    vic ""

translate id june15end_a84ea6a0:
    # "Don't ask."
    ""

translate id june15end_e25eff33:
    # mc "Hey, say..."
    mc ""

translate id june15end_48139df8:
    # "Don't ask. Don't ask."
    ""

translate id june15end_1be929f6:
    # mc "Do you know a..."
    mc ""

translate id june15end_abf6996f:
    # "Don't ask. Don't ask. Don't ask. Don't ask."
    ""

translate id june15end_834a3aab:
    # mc "Do you know an August Byrnes?"
    mc ""

translate id june15end_94f2ae06:
    # vic "Hmm...?"
    vic ""

translate id june15end_db523d80:
    # "WHY THE FUCK ARE YOU ASKING?!"
    ""

translate id june15end_0fd4d656_4:
    # "......"
    ""

translate id june15end_a20cefa7_4:
    # "..."
    ""

translate id june15end_0cc273d1:
    # "IF SHE DOES, YOU JUST EXPOSED--"
    ""

translate id june15end_ca0d8d30:
    # vic "Is he a director? Has he done anything I've seen?"
    vic ""

translate id june15end_a61a9b28:
    # vic "O-oh, wwhaaa...?!"
    vic ""

translate id june15end_a3ed877a:
    # "Oh, thank God."
    ""

translate id june15end_2a0fcd57:
    # "Thank you. Thank you. Thank you. Thank you. Thank you. Thank you. Thank you. Thank you."
    ""

translate id june15end_192020a8:
    # mc "I'm going to cook you dinner tonight."
    mc ""

translate id june15end_8a3239db:
    # vic "Ah, um... sure..."
    vic ""

translate id june15end_3d38afdf:
    # mc "What? Huh? This is all you have in the fridge? How are you staying healthy?!"
    mc ""

translate id june15end_0fd4d656_5:
    # "......"
    ""

translate id june15end_a20cefa7_5:
    # "..."
    ""

translate id w3ExNoMinaFuckCall_0fd4d656:
    # "......"
    ""

translate id w3ExNoMinaFuckCall_a20cefa7:
    # "..."
    ""

translate id w3ExNoMinaFuckCall_63065c24:
    # "After everything, I found myself alone."
    ""

translate id w3ExNoMinaFuckCall_ebb509fe:
    # "Not without accompaniment, but actually {i}alone{/i}."
    ""

translate id w3ExNoMinaFuckCall_60cd0e8f:
    # "Just me by my lonesome in a big dark room, not a single soul around, enjoying an afternoon matinée."
    ""

translate id w3ExNoMinaFuckCall_dce31e66:
    # woman "It's coming! {b}Ruuuuuuun!{/b}"
    woman ""

translate id w3ExNoMinaFuckCall_ddf88e09:
    # trailer "Coming this summer."
    trailer ""

translate id w3ExNoMinaFuckCall_7a56b864:
    # mct "(Hmmm, Allison Smith...)"
    mct ""

translate id w3ExNoMinaFuckCall_0f0d915b:
    # mct "(What movie do I know her from...?)"
    mct ""

translate id w3ExNoMinaFuckCall_fb861072:
    # trailer "Storms of Andromeda"
    trailer ""

translate id w3ExNoMinaFuckCall_bc1cd5ab:
    # mct "(Ah, hell. Who knows. This looks like shit, anyway.)"
    mct ""

translate id w3ExNoMinaFuckCall_bd49c552:
    # movieguy "...please put your phones away. Now, for the feature presentation."
    movieguy ""

translate id w3ExNoMinaFuckCall_b75d739d:
    # mct "(Man, what a day already...)"
    mct ""

translate id w3ExNoMinaFuckCall_d5aa7f5a:
    # mct "(The shoot was taxing. I should check in on the Carnations tomorrow, right?)"
    mct ""

translate id w3ExNoMinaFuckCall_5700f64c:
    # mct "(Ah... what am I doing with my life? Why am I even watching this?)"
    mct ""

translate id w3ExNoMinaFuckCall_1f17be9a:
    # mct "(Hmmm. This kicks ass.)"
    mct ""

translate id w3ExNoMinaFuckCall_8b06dbb8:
    # "*Ring, Ring*"
    ""

translate id w3ExNoMinaFuckCall_aca38df4:
    # mct "(Unknown number... well, I mean...)"
    mct ""

translate id w3ExNoMinaFuckCall_23101d0c:
    # mc "I'm the only one here."
    mc ""

translate id w3ExNoMinaFuckCall_4e85cb4d:
    # mc "Hello?"
    mc ""

translate id w3ExNoMinaFuckCall_f3843656:
    # sophia "Hello, Mr. [mcl]. I didn't think we would speak again so soon."
    sophia ""

translate id w3ExNoMinaFuckCall_db606737:
    # mc "...you're the one who called me, Miss Lundgren."
    mc ""

translate id w3ExNoMinaFuckCall_01a72a15:
    # sophia "It's not a social call. Well, not one on my behalf."
    sophia ""

translate id w3ExNoMinaFuckCall_1734049e:
    # sophia "How soon will you arrive home?"
    sophia ""

translate id w3ExNoMinaFuckCall_5f01280f:
    # mc "I don't know. I wasn't planning on it anytime-"
    mc ""

translate id w3ExNoMinaFuckCall_aa69de74:
    # sophia "Mr. Van Doren has graciously found time in his schedule to educate you. He'll be here in one hour, {b}exactly.{/b}"
    sophia ""

translate id w3ExNoMinaFuckCall_01468a4f:
    # mc "What? You can't just--"
    mc ""

translate id w3ExNoMinaFuckCall_f6e4e62d:
    # sophia "Please be here. It's to your benefit."
    sophia ""

translate id w3ExNoMinaFuckCall_1f573b85:
    # mc "Huh, wait! Ah, s-shit!"
    mc ""

translate id w3ExNoMinaFuckCall_fb84c896:
    # mct "(After fucking with me, she thinks she can just--)"
    mct ""

translate id w3ExNoMinaFuckCall_f47bfcd0:
    # mc "Ah, damn it."
    mc ""

translate id w3ExNoMinaFuckCall_330aa564:
    # mct "(Damn it. Damn it. Damn it.)"
    mct ""

translate id w3ExNoMinaFuckCall_8a2c46c9:
    # mct "(Educate me? What the... ah...)"
    mct ""

translate id w3ExNoMinaFuckCall_bc8eb826:
    # "What other option do I have? I need to see what this crap is about."
    ""

translate id w3ExNoMinaFuckCall_d9f967e7:
    # mct "(Whatever it is, it probably isn't good.)"
    mct ""

translate id w3ExNoMinaFuckCall_3928a113:
    # "......."
    ""

translate id w3ExNoMinaFuckCall_0fd4d656_1:
    # "......"
    ""

translate id w3ExNoMinaFuckCall_a20cefa7_1:
    # "..."
    ""

translate id w3IanVisit_9706f7f0:
    # grace "Is that all you have to say for yourself?"
    grace ""

translate id w3IanVisit_ed327136:
    # kil "It will do."
    kil ""

translate id w3IanVisit_46967b65:
    # grace "........."
    grace ""

translate id w3IanVisit_cf02ea30:
    # grace "......"
    grace ""

translate id w3IanVisit_fdb4e830:
    # grace "..."
    grace ""

translate id w3IanVisit_ab989b26:
    # kil "Stop breaking my balls, alright?!!"
    kil ""

translate id w3IanVisit_4b5e1e67:
    # grace "From where I'm standing, you still need to drop them."
    grace ""

translate id w3IanVisit_1605f437:
    # kil "Ah, y-you- hhhnn- {b}d-damn it!{/b}"
    kil ""

translate id w3IanVisit_efd85f70:
    # grace "What?"
    grace ""

translate id w3IanVisit_2b3e9b0f:
    # kil "*sigh* That was a good one."
    kil ""

translate id w3IanVisit_fb1b5575:
    # grace "Thank you."
    grace ""

translate id w3IanVisit_a20cefa7:
    # "..."
    ""

translate id w3IanVisit_df3351bb:
    # grace "You don't return my calls, you miss your little cousin's graduation, your father's birthday party, and now you skip out on our anniversary party."
    grace ""

translate id w3IanVisit_09574a38:
    # grace "You've made it abundantly clear you don't want to be a part of this family."
    grace ""

translate id w3IanVisit_1f6981db:
    # kil "...have I? Then why are you here?"
    kil ""

translate id w3IanVisit_8590a2ed:
    # grace "I can't accept that. Family's important, Ian."
    grace ""

translate id w3IanVisit_050538f9:
    # grace "Besides Charlie, I had no one. He raised and put me through school, where I met your father."
    grace ""

translate id w3IanVisit_a1ce80d1:
    # grace "Suddenly, I had a lot of family. You shouldn't throw that away just because of our disagreements."
    grace ""

translate id w3IanVisit_5dfb9f43:
    # grace "Your cousin adores you; she has nothing to do with your perpetual temper tantrum."
    grace ""

translate id w3IanVisit_1dc04034:
    # kil "I was busy."
    kil ""

translate id w3IanVisit_847b6297:
    # grace "Don't lie to me. Your father and I gave you everything you ever asked for. You never wanted for anything."
    grace ""

translate id w3IanVisit_27679373:
    # kil "There are some things I shouldn't have to ask for."
    kil ""

translate id w3IanVisit_e786c1cf:
    # grace "I tried everything. When I thought I was smothering you, I gave you space. When that didn't work, I didn't know what the fuck to do."
    grace ""

translate id w3IanVisit_52a00c2f:
    # "*Ring, ring*"
    ""

translate id w3IanVisit_0a70da08:
    # kil "Yeah, Doc?"
    kil ""

translate id w3IanVisit_8b2e5b0b:
    # "After setting up the time and place with Mina, I had time to kill, so I thought I'd drop in on my hung-over and ailing friend."
    ""

translate id w3IanVisit_1e5cfe95:
    # "Whether it was some manifestation of guilt or if I was truly concerned, the irony of my visit wasn't lost on me."
    ""

translate id w3IanVisit_e2895f9b:
    # "With nothing better to do, I thought I'd drop in on my hung-over, ailing friend."
    ""

translate id w3IanVisit_64b5221a:
    # kil "Uh, huh. It's open. Just let yourself in."
    kil ""

translate id w3IanVisit_b8a7705e:
    # "*Beep*"
    ""

translate id w3IanVisit_d5c1776b:
    # kil "That was [mcf], so let's finish this conversation never."
    kil ""

translate id w3IanVisit_342505ac:
    # grace "I don't know what to do, but... just... {i}I love you, son.{/i}"
    grace ""

translate id w3IanVisit_e6f79c63:
    # grace "Time is more precious than you realize. I won't always be here. Ignoring your feelings today will just make you bitter tomorrow."
    grace ""

translate id w3IanVisit_64f565ab:
    # kil "[mcf] will be here soon."
    kil ""

translate id w3IanVisit_5f1e0d4f:
    # grace "Yeah, I get the message."
    grace ""

translate id w3IanVisit_78b33fb9:
    # mc "Oh, uh... hello, Mrs. Beaufort. How are you?"
    mc ""

translate id w3IanVisit_7b40ea55:
    # grace "I'm good, [mcf]. I was just leaving."
    grace ""

translate id w3IanVisit_f074a452:
    # mc "Oh, well. It was nice to--"
    mc ""

translate id w3IanVisit_2f98335d:
    # kil "Thank God you showed up when you did! You cut her bitching short."
    kil ""

translate id w3IanVisit_436d3fe9:
    # mc "Glad to be of use."
    mc ""

translate id w3IanVisit_61c2df44:
    # kil "So what's up? Dropping by out of the blue isn't like you."
    kil ""

translate id w3IanVisit_4f73bd28:
    # mc "I just got off \"work.\" Wanted to come by and see how my sick little boy is doing."
    mc ""

translate id w3IanVisit_066d71b6:
    # kil "Yeah, yeah. Sorry. I just didn't feel like it, okay?"
    kil ""

translate id w3IanVisit_74bbda5d:
    # mc "You must have been really knocking them back last night if it put {b}you{/b} out of commission, huh?"
    mc ""

translate id w3IanVisit_3a2aece4:
    # kil "What can I say? It was a special occasion."
    kil ""

translate id w3IanVisit_482030a5:
    # mc "Drowning your sorrows?"
    mc ""

translate id w3IanVisit_7573a38f:
    # kil "Not exactly, but... I don't know. It felt like a thing to do."
    kil ""

translate id w3IanVisit_c19e0371:
    # kil "Even if I made my own bed, Mina is the longest relationship I have ever had."
    kil ""

translate id w3IanVisit_b7d25c42:
    # kil "Honestly, I should've been drinking out of relief. Mina was a far too intense woman for me."
    kil ""

translate id w3IanVisit_ab553278:
    # mc "Wasn't she more like a doormat?"
    mc ""

translate id w3IanVisit_05bcb2ad:
    # kil "In some ways, but she also put off this weird vibe. I don't know how to explain it. I could never get a solid read on her like I could other girls."
    kil ""

translate id w3IanVisit_721d4483:
    # mc "What do you mean by that?"
    mc ""

translate id w3IanVisit_24b4f6ec:
    # kil "I've been wondering that myself."
    kil ""

translate id w3IanVisit_a84df2a4:
    # kil "Like I'm the type of person who likes to test the limits of what I can get away with. I do it with my parents, I do it with my Uncle, and with the women I fuck. So naturally, I did it with the woman I..."
    kil ""

translate id w3IanVisit_1e88301e:
    # kil "I don't know. Typically when I piss someone off, I see {i}their{/i} shit."
    kil ""

translate id w3IanVisit_adf08aff:
    # kil "Their fucked up messes, what they {i}demanded{/i} of me, just stuff that made me feel good about twisting the knife."
    kil ""

translate id w3IanVisit_74b7128e:
    # mc "But with Mina?"
    mc ""

translate id w3IanVisit_ef0b186e:
    # kil "Sometimes it felt like she was a mirror, right? Whenever I fucked up or made her jealous, it was like seeing what other people saw when they looked at me."
    kil ""

translate id w3IanVisit_35755adb:
    # kil "For the first time, instead of testing other people's limits, I was seeing what I could get away with myself. Whenever she looked hurt, {i}I felt it{/i}."
    kil ""

translate id w3IanVisit_a8e8d43d:
    # kil "Don't just look at me like that. Does that make any fucked up sense?"
    kil ""

translate id w3IanVisit_349cace1:
    # mc "Yeah, that's kinda messed up, dude - but I get it."
    mc ""

translate id w3IanVisit_47ee05b2:
    # kil "You do?"
    kil ""

translate id w3IanVisit_1b3d8277:
    # mc "Our views of ourselves are just unsubstantiated delusions until they meet opposition."
    mc ""

translate id w3IanVisit_3cc84d6f:
    # kil "Hell no, man. I live a dope life. I don't need some bitch to shatter that."
    kil ""

translate id w3IanVisit_4fbbef77:
    # mc "...yeah?"
    mc ""

translate id w3IanVisit_2cfe33cf:
    # kil "I don't know, man. She's just not as stupid as she looks and I don't fucking get it."
    kil ""

translate id w3IanVisit_fdc7b505:
    # kil "I woke up one night and she was just staring at me, lost in thought. Like the way Uncle sometimes gets when we play games."
    kil ""

translate id w3IanVisit_d289c080:
    # mc "You sure she wasn't just 'mirin you, hot stuff?"
    mc ""

translate id w3IanVisit_9362b181:
    # kil "God, don't say that word."
    kil ""

translate id w3IanVisit_d623739d:
    # mc "What word?"
    mc ""

translate id w3IanVisit_f56248e8:
    # kil "That actually made my skin crawl."
    kil ""

translate id w3IanVisit_0497190c:
    # mc "I mean, how could a girl not look? You're sooooo good-looking, Ian."
    mc ""

translate id w3IanVisit_9d299b71:
    # kil "Stop."
    kil ""

translate id w3IanVisit_fd643874:
    # mc "God's gift to womankind."
    mc ""

translate id w3IanVisit_d5691ceb:
    # kil "STOOOOOOOP. I'm not that kind of full of myself!"
    kil ""

translate id w3IanVisit_85ba07bd:
    # mc "Oh, you're not?"
    mc ""

translate id w3IanVisit_1011f105:
    # kil "No! I'm a fucking gremlin with money and I know that, okay? A gremlin with a {b}dope life.{/b}"
    kil ""

translate id w3IanVisit_1956d12e:
    # mc "So you've said before."
    mc ""

translate id w3IanVisit_b90d4dbe:
    # kil "How long are you staying? I got all day."
    kil ""

translate id w3IanVisit_84c68eb8:
    # mc "I could fuck around for a couple of hours. Enough time to get a bite to eat and binge an episode or two of whatever shitty reality show we can find."
    mc ""

translate id w3IanVisit_49c97a24:
    # mc "Let's kick it all day, then."
    mc ""

translate id w3IanVisit_fe2401d1:
    # kil "What a lucky week. You telling me I get some of my best friend's time two days in a row?"
    kil ""

translate id w3IanVisit_f525de0c:
    # mc "This is why Mina thought you wanted to fuck me, you know that?"
    mc ""

translate id w3IanVisit_1f69a6ad:
    # kil "Did she say that?!"
    kil ""

translate id w3IanVisit_4d7549db:
    # mc "She didn't need to. I could feel the jealousy in the air the day we met."
    mc ""

translate id w3IanVisit_f03b2493:
    # kil "Nah, dude. It doesn't come off like that. I don't have any positive male examples to gauge my tone against. That's all!"
    kil ""

translate id w3IanVisit_0fd4d656:
    # "......"
    ""

translate id w3IanVisit_a20cefa7_1:
    # "..."
    ""

translate id w3IanVisit_c2088dca:
    # mck "Pfhh, hahaha!"
    mck ""

translate id w3IanVisit_a4d19f00:
    # kil "...who would top?"
    kil ""

translate id w3IanVisit_c1def2ca:
    # mc "Is that a fucking question? If I asked you to, you'd spread it so wide that I could see what you had for lunch."
    mc ""

translate id w3IanVisit_14bc0193:
    # kil "Oh, fuck off, that's gross! Who's the gay one now?!"
    kil ""

translate id w3IanVisit_087bf354:
    # "How could I laugh like this when I'm about to go see his ex after we..."
    ""

translate id w3IanVisit_5b4c5880:
    # mct "(Part of me wanted to tell him, but my sense of self-preservation prevented it. I didn't need to complicate my work life.)"
    mct ""

translate id w3IanVisit_9464afe3:
    # mct "(Yeah, not afraid to rock the boat. Riiiiiight.)"
    mct ""

translate id w3IanVisit_5a38e5ef:
    # mc "The real question should be, \"who's your fucking daddy?\" You {i}definitely{/i} have those sorts of issues."
    mc ""

translate id w3IanVisit_23bcfb3e:
    # kil "Uhhhhhhhhh! I can't even deny that, can I?"
    kil ""

translate id w3IanVisit_4ade26fa:
    # "So we shot the shit, ate, watched TV... all that junk. It was fun, but... {b}yeah.{/b}"
    ""

translate id w3IanVisit_d95266fb:
    # "I did have to go see his ex-girlfriend."
    ""

translate id w3IanVisit_ea5ee905:
    # "So we shot the shit, ate, watched TV, played games, watched a movie..."
    ""

translate id w3IanVisit_cda3dbd2:
    # "As time passed, the question of when to go home was answered for me."
    ""

translate id w3IanVisit_52a00c2f_1:
    # "*Ring, ring*"
    ""

translate id w3IanVisit_53ff46ad:
    # kil "Who is it?"
    kil ""

translate id w3IanVisit_60205dab:
    # mc "Unknown number."
    mc ""

translate id w3IanVisit_ede65559:
    # kil "Answer it."
    kil ""

translate id w3IanVisit_4e85cb4d:
    # mc "Hello?"
    mc ""

translate id w3IanVisit_f3843656:
    # sophia "Hello, Mr. [mcl]. I didn't think we would speak again so soon."
    sophia ""

translate id w3IanVisit_db606737:
    # mc "...you're the one who called me, Miss Lundgren."
    mc ""

translate id w3IanVisit_01a72a15:
    # sophia "It's not a social call. Well, not one on my behalf."
    sophia ""

translate id w3IanVisit_1734049e:
    # sophia "How soon will you arrive home?"
    sophia ""

translate id w3IanVisit_5f01280f:
    # mc "I don't know. I wasn't planning on it anytime-"
    mc ""

translate id w3IanVisit_aa69de74:
    # sophia "Mr. Van Doren has graciously found time in his schedule to educate you. He'll be here in one hour, {b}exactly.{/b}"
    sophia ""

translate id w3IanVisit_01468a4f:
    # mc "What? You can't just--"
    mc ""

translate id w3IanVisit_f6e4e62d:
    # sophia "Please be here. It's to your benefit."
    sophia ""

translate id w3IanVisit_1f573b85:
    # mc "Huh, wait! Ah, s-shit!"
    mc ""

translate id w3IanVisit_fb84c896:
    # mct "(After fucking with me, she thinks she can just--)"
    mct ""

translate id w3IanVisit_f47bfcd0:
    # mc "Ah, damn it."
    mc ""

translate id w3IanVisit_3e27324a:
    # kil "What's wrong?"
    kil ""

translate id w3IanVisit_330aa564:
    # mct "(Damn it. Damn it. Damn it.)"
    mct ""

translate id w3IanVisit_8a2c46c9:
    # mct "(Educate me? What the... ah...)"
    mct ""

translate id w3IanVisit_3db3710d:
    # mc "I'm going to have a house guest."
    mc ""

translate id w3IanVisit_4eafb2eb:
    # kil "Who?"
    kil ""

translate id w3IanVisit_c0c99438:
    # mc "Abel Van Doren."
    mc ""

translate id w3IanVisit_6001be59:
    # kil "What the fuck? Why would he-"
    kil ""

translate id w3IanVisit_2c946869:
    # mc "I've got no goddamn clue, man."
    mc ""

translate id w3IanVisit_b80d29a9:
    # kil "I'll come with you."
    kil ""

translate id w3IanVisit_0e50e140:
    # mc "No, it's best you don't. I'll call you later about it, though."
    mc ""

translate id w3IanVisit_251b6db4:
    # kil "...ah, alright. Nice hanging with you, bro."
    kil ""

translate id w3IanVisit_927a017e:
    # mc "You too, Ian."
    mc ""

translate id w3IanVisit_278c7e29:
    # "........."
    ""

translate id w3IanVisit_0fd4d656_1:
    # "......"
    ""

translate id w3IanVisit_a20cefa7_2:
    # "..."
    ""


translate id strings:
    old "You guess you were too tired to care.(Dialouge/render changes only)"
    new "You guess you were too tired to care.(Dialouge/render changes only)"

    old "It's probably because she's so cute?"
    new "It's probably because she's so cute?"

    old "Tell her you like the way she smelled."
    new "Tell her you like the way she smelled."

    old "First, give yourself a test shock."
    new "First, give yourself a test shock."

    old "Just get straight to it."
    new "Just get straight to it."

    old "Get it over with."
    new "Get it over with."

    old "Put your all into it(w3RosalindMeanShock=True)."
    new "Put your all into it(w3RosalindMeanShock=True)."

    old "Don't go easy on her.(w3VeronicaMeanShock=True,)"
    new "Don't go easy on her.(w3VeronicaMeanShock=True,)"

    old "Conclude this quickly.(if w3RosalindMeanShock=True)"
    new "Conclude this quickly.(if w3RosalindMeanShock=True)"

    old "Be cruel and COMMIT FULLY to your assigned role.(w3FeliciaMeanShock = True)"
    new "Be cruel and COMMIT FULLY to your assigned role.(w3FeliciaMeanShock = True)"

    old "Be plausibly rough, but level and work with Felicia."
    new "Be plausibly rough, but level and work with Felicia."

    old "Shock Mrs. Pulman.(Dialouge/render changes only)"
    new "Shock Mrs. Pulman.(Dialouge/render changes only)"

    old "Ignore it."
    new "Ignore it."

    old "Take her hand(w3SophiaPolite = True)."
    new "Take her hand(w3SophiaPolite = True)."

    old "Ignore her and lead the way to the sofa."
    new "Ignore her and lead the way to the sofa."

    old "Stomach.(Dialouge/render changes only)"
    new "Stomach.(Dialouge/render changes only)"

    old "Thighs."
    new "Thighs."

    old "Be glib. Sorry. You have a girlfriend, so you shouldn't say."
    new "Be glib. Sorry. You have a girlfriend, so you shouldn't say."

    old "Put her hand back where it belongs(w3HagHandie = True)."
    new "Put her hand back where it belongs(w3HagHandie = True)."

    old "Carry on, untouched."
    new "Carry on, untouched."

    old "Goad the old woman into letting you finish in her mouth.(w3KatPromoFinish = True)"
    new "Goad the old woman into letting you finish in her mouth.(w3KatPromoFinish = True)"

    old "Leave an impression on the doctor.(w3SophiaPromoFinish = True)"
    new "Leave an impression on the doctor.(w3SophiaPromoFinish = True)"

    old "Leave an impression on the doctor(w3SophiaPromoFinish = True)."
    new "Leave an impression on the doctor(w3SophiaPromoFinish = True)."

    old "Goad the old woman into letting you finish in her mouth(w3KatPromoFinish = True)."
    new "Goad the old woman into letting you finish in her mouth(w3KatPromoFinish = True)."

    old "Tell her you just walked around.(Dialouge/render changes only)"
    new "Tell her you just walked around.(Dialouge/render changes only)"

    old "Mention the Emma situation."
    new "Mention the Emma situation."

    old "Mention Harper and Lucy(if w2ExLezSeen == True)"
    new "Mention Harper and Lucy(if w2ExLezSeen == True)"

    old "Hug her tighter.(Dialouge/render changes only)"
    new "Hug her tighter.(Dialouge/render changes only)"

    old "Kiss her on the forehead."
    new "Kiss her on the forehead."

    old "Suggest we order."
    new "Suggest we order."

    old "Reaffirm your intention to help her."
    new "Reaffirm your intention to help her."

    old "Resist. Tell Mina about your new relationship with Hana(hanaTwoTime = False,minaBreakOff = True)."
    new "Resist. Tell Mina about your new relationship with Hana(hanaTwoTime = False,minaBreakOff = True)."

    old "Hug her tighter(Dialouge/render changes only)."
    new "Hug her tighter(Dialouge/render changes only)."

    old "Make a joke(Dialouge/render changes only)"
    new "Make a joke(Dialouge/render changes only)"

    old "{color=#FF1493}[Romance]{/color} Fuck it. You're only a man."
    new "{color=#FF1493}[Romance]{/color} Fuck it. You're only a man."

    old "{color=#FF1493}[Friend]{/color} Let's go play some video games."
    new "{color=#FF1493}[Friend]{/color} Let's go play some video games."

    old "You cheated(minaCheat = True)."
    new "You cheated(minaCheat = True)."

    old "You waited(minaCleanStart = True)."
    new "You waited(minaCleanStart = True)."

    old "Be straight with her.(Dialouge/render changes only)"
    new "Be straight with her.(Dialouge/render changes only)"

    old "Talk dirty."
    new "Talk dirty."

    old "Ask her to turn around.(Dialouge/render changes only)"
    new "Ask her to turn around.(Dialouge/render changes only)"

    old "Fuck waiting. Beckon over the little minx."
    new "Fuck waiting. Beckon over the little minx."

    old "Tease her silently(Dialouge/render changes only)."
    new "Tease her silently(Dialouge/render changes only)."

    old "Get vulgar with it."
    new "Get vulgar with it."

    old "Pick up the speed.(Dialouge/render changes only)"
    new "Pick up the speed.(Dialouge/render changes only)"

    old "\"Say my name, bitch.\"(w3MinaSayMyName = True)"
    new "\"Say my name, bitch.\"(w3MinaSayMyName = True)"

    old "Wordlessly take her.(Dialouge/render changes only)"
    new "Wordlessly take her.(Dialouge/render changes only)"

    old "You're her daddy.(w3MinaDaddy = True)"
    new "You're her daddy.(w3MinaDaddy = True)"

    old "Destroy?(Dialouge/render changes only)"
    new "Destroy?(Dialouge/render changes only)"

    old "Destroy!"
    new "Destroy!"

    old "\"I love you!\"(Dialouge/render changes only)"
    new "\"I love you!\"(Dialouge/render changes only)"

    old "\"Take it!\""
    new "\"Take it!\""
